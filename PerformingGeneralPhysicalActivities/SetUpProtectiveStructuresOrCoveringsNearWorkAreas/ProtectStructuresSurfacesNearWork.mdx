...
```mermaid
erDiagram
    ProtectionPlan ||--|{ ProtectedAsset : covers
    ProtectionPlan ||--|{ ProtectionMaterial : specifies
    ProtectedAsset ||--o{ DamageAssessment : evaluated_by
    ProtectionLog ||--|| ProtectionPlan : tracks
    ProtectedAsset ||--o{ IncidentReport : "may generate"
    DamageAssessment ||--o{ IncidentReport : triggers
```
...
stateDiagram-v2
    [*] --> NeedsAssessment
    NeedsAssessment --> Assessed : protectionNeedsAssessed
    Assessed --> Planned : protectionPlanCreated
    Planned --> Installed : protectionInstalled
    Installed --> Monitoring : protectionMonitored
    Monitoring --> Monitoring : protectionMonitored
    Monitoring --> DamageFound : damageDetected
    DamageFound --> Installed : repairProtection
    Monitoring --> Removed : protectionRemoved
    Removed --> FinalAssessment : damageAssessed
    FinalAssessment --> [*]
```
