...
```mermaid
erDiagram
    AthleticEvent ||--o{ Registration : accepts
    AthleticEvent ||--|{ EventSchedule : contains
    Registration ||--|| EligibilityCertificate : requires
    Registration ||--o| CompetitionResult : produces
    CompetitionResult }o--|| PerformanceRecord : updates
    AthleticEvent ||--o{ CompetitionResult : generates
```
...
stateDiagram-v2
    ...
    Registered --> Verified : eligibilityVerified
    Verified --> CheckedIn : preEventCheckinCompleted
    CheckedIn --> Competed : performanceExecuted
    Competed --> Scored : resultSubmitted
    Scored --> Reviewed : performanceReviewed
    Reviewed --> Archived : athleteRecordUpdated
    Reviewed --> CheckedIn : re-entered next event
    Archived --> [*]
```
