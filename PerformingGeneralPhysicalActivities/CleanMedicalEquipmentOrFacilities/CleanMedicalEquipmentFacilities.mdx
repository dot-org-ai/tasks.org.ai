...
```mermaid
erDiagram
    CleaningTask ||--|| FacilityZone : targets
    CleaningTask ||--|| DisinfectionProtocol : follows
    CleaningTask ||--|| ComplianceRecord : produces
    FacilityZone ||--o{ HighTouchSurface : contains
    FacilityZone ||--|| CleaningSchedule : governed
    CleaningSchedule ||--o{ CleaningTask : generates
    HighTouchSurface ||--o{ ComplianceRecord : verified
```
...
```mermaid
stateDiagram-v2
    [*] --> Scheduled : cleaningTaskScheduled
    Scheduled --> RoutineCleaned : routineCleaningCompleted
    Scheduled --> TerminalCleaned : terminalCleaningCompleted
    RoutineCleaned --> SurfacesDisinfected : highTouchSurfacesDisinfected
    TerminalCleaned --> EquipmentSanitized : sharedEquipmentSanitized
    SurfacesDisinfected --> Verified : cleaningComplianceVerified
    EquipmentSanitized --> Verified : cleaningComplianceVerified
    Verified --> NonCompliant : cleaningNoncomplianceDetected
    NonCompliant --> Scheduled : cleaningTaskScheduled
    Verified --> [*]
```
