...
```mermaid
erDiagram
    MedicalDevice ||--|| CleaningProtocol : follows
    MedicalDevice ||--o{ CleaningRecord : tracks
    MedicalDevice ||--o{ InspectionResult : verified
    CleaningProtocol ||--|| DetergentSolution : specifies
    WasherDisinfector ||--o{ CleaningRecord : processes
    CleaningRecord ||--|| InspectionResult : confirmedBy
    MedicalDevice ||--o{ WasherDisinfector : processedIn
```
...
stateDiagram-v2
    ...
    Received --> Soaked : equipmentPreSoaked
    Soaked --> ManualCleaned : manualCleanCompleted
    Soaked --> AutoCleaned : automatedCleanCompleted
    ManualCleaned --> Inspected : cleanlinessVerified
    AutoCleaned --> Inspected : cleanlinessVerified
    Inspected --> ManualCleaned : re-clean required
    Inspected --> Documented : cleaningDocumented
    Documented --> [*]
```
