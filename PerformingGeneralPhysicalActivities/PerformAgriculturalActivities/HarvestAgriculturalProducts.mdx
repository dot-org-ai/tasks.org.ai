...
erDiagram
    Field ||--|| Crop : grows
    Field ||--|| HarvestPlan : "scheduled in"
    HarvestPlan ||--o{ Field : covers
    Field ||--o{ YieldRecord : produces
    Crop ||--o{ GradeReport : "graded by"
    StorageFacility ||--o{ Crop : stores
```
...
stateDiagram-v2
    [*] --> Growing
    Growing --> Assessed : harvestReadinessAssessed
    Assessed --> Growing : Not ready
    Assessed --> Planned : harvestPlanCreated
    Planned --> CrewDeployed : harvestCrewDeployed
    CrewDeployed --> Harvested : fieldHarvested
    Harvested --> Graded : productGraded
    Graded --> Transported : productTransported
    Transported --> Recorded : yieldRecorded
    Recorded --> [*]
```
