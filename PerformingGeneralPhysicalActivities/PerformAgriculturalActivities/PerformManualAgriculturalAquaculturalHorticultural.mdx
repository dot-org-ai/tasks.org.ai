...
```mermaid
erDiagram
    TaskAssignment ||--o{ HarvestBatch : produces
    TaskAssignment ||--|| Field : "performed in"
    TaskAssignment ||--|| Pond : "performed at"
    TaskAssignment ||--|| Greenhouse : "performed in"
    HarvestBatch ||--o{ QualityCheck : "inspected by"
    TaskAssignment ||--|| ProductionLog : "recorded in"
```
...
stateDiagram-v2
    [*] --> Unassigned
    Unassigned --> Assigned : tasksAssigned
    Assigned --> Harvesting : cropHarvested
    Assigned --> Weeding : fieldWeeded
    Assigned --> Pruning : plantsPruned
    Assigned --> Feeding : stockFed
    Assigned --> Transplanting : seedlingsTransplanted
    Harvesting --> Logged : productionLogged
    Weeding --> Logged : productionLogged
    Pruning --> Logged : productionLogged
    Feeding --> Logged : productionLogged
    Transplanting --> Logged : productionLogged
    Logged --> Unassigned : Next day
```
