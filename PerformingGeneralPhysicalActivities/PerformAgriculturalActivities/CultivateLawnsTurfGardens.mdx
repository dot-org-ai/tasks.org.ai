...
```mermaid
erDiagram
    LawnArea ||--|| MaintenancePlan : follows
    LawnArea ||--o{ SoilTest : "tested by"
    MaintenancePlan ||--o{ MowingSchedule : includes
    MaintenancePlan ||--o{ TreatmentRecord : generates
    GardenBed ||--|| MaintenancePlan : follows
    SoilTest ||--o{ TreatmentRecord : informs
```
...
stateDiagram-v2
    [*] --> NeedsAssessment
    NeedsAssessment --> Assessed : conditionAssessed
    Assessed --> Tested : soilTested
    Tested --> Aerated : soilAerated
    Tested --> Fertilized : fertilizerApplied
    Aerated --> Fertilized : fertilizerApplied
    Fertilized --> WeedTreated : weedsControlled
    WeedTreated --> Overseeded : overseedingComplete
    WeedTreated --> Mowed : turfMowed
    Overseeded --> Mowed : turfMowed
    Mowed --> NeedsAssessment : Next cycle
```
