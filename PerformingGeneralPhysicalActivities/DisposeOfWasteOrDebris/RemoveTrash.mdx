...
```mermaid
erDiagram
    WasteBin ||--|{ CollectionRoute : "serviced on"
    CollectionRoute ||--|| PickupSchedule : "defined by"
    CollectionRoute ||--|{ WasteTicket : "generates"
    WasteBin ||--o{ ContaminationReport : "flagged in"
    ServiceContract ||--|{ PickupSchedule : "governs"
    ServiceContract ||--|{ WasteTicket : "documented under"
```
...
stateDiagram-v2
    [*] --> BinFilling
    BinFilling --> Serviced : binsServiced
    Serviced --> Staged : bagsStaged
    Staged --> InRoute : routeCompleted
    InRoute --> Logged : pickupLogged
    Logged --> Analyzed : volumeAnalyzed
    Analyzed --> ScheduleModified : scheduleAdjusted
    Analyzed --> BinFilling : stable
    ScheduleModified --> BinFilling : nextCycle
    Serviced --> Contaminated : contaminationReported
    Contaminated --> Serviced : contaminationResolved
```
