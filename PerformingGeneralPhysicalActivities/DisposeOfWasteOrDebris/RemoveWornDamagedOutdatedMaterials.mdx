...
```mermaid
erDiagram
    MaterialItem ||--|| ConditionReport : "assessed in"
    ConditionReport ||--|{ RemovalOrder : "triggers"
    RemovalOrder ||--|{ DispositionRecord : "resolved by"
    RemovalOrder ||--|| ReplacementOrder : "generates"
    MaterialItem ||--o| ExpirationTracker : "monitored by"
    ExpirationTracker ||--o{ ConditionReport : "triggers"
```
...
stateDiagram-v2
    [*] --> InService
    InService --> Inspected : materialsInspected
    Inspected --> Flagged : itemsFlagged
    Inspected --> InService : conditionAcceptable
    Flagged --> Authorized : removalAuthorized
    Authorized --> Removed : materialsRemoved
    Removed --> Dispositioned : dispositionDetermined
    Dispositioned --> Replacing : replacementOrdered
    Replacing --> InService : replacementReceived
```
