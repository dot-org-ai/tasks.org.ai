...
```mermaid
erDiagram
    DebrisType ||--|{ RemovalOrder : "categorized in"
    RemovalOrder ||--|{ Dumpster : "filled into"
    Dumpster ||--|{ HaulTicket : "tracked by"
    RemovalOrder ||--o{ RecyclingManifest : "diverts via"
    RemovalOrder ||--|| SiteClearance : "verified by"
    DebrisType ||--o{ RecyclingManifest : "documented in"
```
...
stateDiagram-v2
    [*] --> Assessing
    Assessing --> OrderCreated : removalOrderCreated
    OrderCreated --> Collecting : debrisCollected
    Collecting --> Diverting : recyclablesDiverted
    Collecting --> AwaitingHaul : nonRecyclable
    Diverting --> AwaitingHaul : remainderStaged
    AwaitingHaul --> Hauled : dumpsterHauled
    Hauled --> Verifying : clearanceVerified
    Verifying --> [*] : areaClear
    Verifying --> Collecting : additionalDebrisFound
```
