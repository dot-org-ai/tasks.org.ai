...
```mermaid
erDiagram
    WorkSite ||--|| CleanupPlan : follows
    WorkSite ||--o{ DebrisManifest : generates
    WorkSite ||--o{ HazardFlag : "may have"
    CleanupPlan ||--o{ CleaningEquipment : specifies
    WorkSite ||--|| SafetyClearance : "requires"
    DebrisManifest ||--|| SafetyClearance : supports
```
...
stateDiagram-v2
    [*] --> Active
    Active --> Assessed : siteAssessed
    Assessed --> Planned : cleanupPlanCreated
    Planned --> Cleaned : cleanupExecuted
    Cleaned --> WasteSorted : wasteSegregated
    WasteSorted --> Pending : clearanceRequested
    Pending --> Cleared : clearanceIssued
    Pending --> Deficient : Address Deficiencies
    Deficient --> Cleaned : Re-clean
    Cleared --> [*]
```
