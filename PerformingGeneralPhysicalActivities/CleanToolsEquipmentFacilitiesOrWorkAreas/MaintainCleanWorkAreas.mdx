...
```mermaid
erDiagram
    WorkArea ||--|{ CleaningRotation : "has scheduled"
    WorkArea ||--o{ SpotCleanRequest : "generates"
    WorkArea ||--|{ HousekeepingLog : "tracks"
    WorkArea }o--|| ComplianceStandard : "governed by"
    WorkArea ||--o{ SupplyStation : "contains"
    CleaningRotation ||--|{ HousekeepingLog : "produces"
    SpotCleanRequest ||--o| HousekeepingLog : "recorded in"
```
...
stateDiagram-v2
    [*] --> Clean
    Clean --> MonitoringScheduled : rotationDefined
    MonitoringScheduled --> CleaningInProgress : activityLogged
    CleaningInProgress --> Clean : spotCleanCompleted
    Clean --> AuditInProgress : auditCleanliness
    AuditInProgress --> Clean : auditPassed
    AuditInProgress --> NeedsAttention : auditFailed
    NeedsAttention --> CleaningInProgress : spotCleanRequested
    NeedsAttention --> HazardFlagged : hazardReported
    HazardFlagged --> CleaningInProgress : spotCleanRequested
    CleaningInProgress --> AuditInProgress : auditCleanliness
    Clean --> [*]
```
