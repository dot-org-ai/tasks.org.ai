...
```mermaid
erDiagram
    Tool ||--o{ CleaningRecord : "tracked in"
    Tool ||--o{ ConditionAssessment : "evaluated by"
    Tool ||--|| StorageLocation : "stored at"
    CleaningKit ||--o{ CleaningRecord : "used in"
    ConditionAssessment ||--o{ DamageReport : "may generate"
    Tool ||--o{ DamageReport : "subject of"
```
...
stateDiagram-v2
    [*] --> InUse
    InUse --> Returned : toolReceived
    Returned --> Cleaned : toolCleaned
    Cleaned --> Assessed : conditionAssessed
    Assessed --> Lubricated : lubricantApplied
    Lubricated --> Stored : toolStored
    Assessed --> DamageReported : damageReported
    DamageReported --> Repaired : Schedule Repair
    DamageReported --> Replaced : Schedule Replacement
    Repaired --> Cleaned : Re-clean after repair
    Stored --> [*]
```
