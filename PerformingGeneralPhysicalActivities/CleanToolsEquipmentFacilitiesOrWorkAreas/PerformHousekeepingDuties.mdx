...
```mermaid
erDiagram
    Room ||--|{ HousekeepingTask : "requires"
    Room ||--|| RoomStatus : "has current"
    Room ||--o{ GuestRequest : "receives"
    HousekeepingTask ||--o| InspectionChecklist : "verified by"
    Room ||--o{ LinenInventory : "consumes from"
    Room ||--o{ AmenityKit : "restocked with"
    GuestRequest ||--o| HousekeepingTask : "generates"
```
...
stateDiagram-v2
    [*] --> Dirty
    Dirty --> Assigned : roomsAssigned
    Assigned --> CleaningInProgress : roomCleaned
    CleaningInProgress --> Restocking : amenitiesRestocked
    Restocking --> Inspection : roomInspected
    Inspection --> Ready : roomStatusUpdated
    Inspection --> CleaningInProgress : recleanRequired
    Ready --> Occupied : guestCheckedIn
    Occupied --> Dirty : guestCheckedOut
    Occupied --> Dirty : guestRequestFulfilled
    Ready --> [*]
```
