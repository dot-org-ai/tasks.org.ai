...
```mermaid
erDiagram
    Facility ||--o{ CleaningTask : schedules
    EquipmentAsset ||--o{ CleaningTask : requires
    CleaningTask ||--|| CleaningSchedule : governed
    CleaningTask ||--|| InspectionReport : verifiedBy
    Facility ||--|| SupplyInventory : stocks
    CleaningSchedule ||--o{ CleaningTask : generates
    EquipmentAsset ||--o{ InspectionReport : tracked
```
...
stateDiagram-v2
    ...
    Assigned --> AreaCleaned : areaCleaned
    Assigned --> EquipmentCleaned : equipmentCleaned
    AreaCleaned --> Inspected : cleanlinessApproved
    EquipmentCleaned --> Inspected : cleanlinessApproved
    AreaCleaned --> Flagged : issueReported
    Flagged --> Assigned : taskAssigned
    Inspected --> Closed : taskClosed
    Closed --> Restocked : suppliesRestocked
    Restocked --> [*]
```
