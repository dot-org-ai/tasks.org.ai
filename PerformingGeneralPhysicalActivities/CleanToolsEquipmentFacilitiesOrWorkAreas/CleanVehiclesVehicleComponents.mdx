...
```mermaid
erDiagram
    Vehicle ||--o{ CleaningOrder : receives
    CleaningOrder ||--|| ServicePackage : specifies
    CleaningOrder ||--|| WashBay : "assigned to"
    Vehicle ||--o{ InspectionReport : "inspected via"
    CleaningOrder ||--o{ WasteWaterRecord : generates
    ServicePackage ||--o{ CleaningOrder : "applied in"
```
...
stateDiagram-v2
    [*] --> Dirty
    Dirty --> Scheduled : washScheduled
    Scheduled --> ExteriorDone : exteriorWashCompleted
    ExteriorDone --> InteriorDone : interiorDetailCompleted
    ExteriorDone --> Inspecting : vehicleInspected
    InteriorDone --> EngineClean : engineDegreased
    EngineClean --> Inspecting : vehicleInspected
    InteriorDone --> Inspecting : vehicleInspected
    Inspecting --> InService : vehicleReturnedToService
    Inspecting --> ExteriorDone : Re-clean
    InService --> [*]
```
