...
```mermaid
erDiagram
    SurfaceProfile ||--|| CleaningMethod : determines
    CleaningTask ||--|| SurfaceProfile : targets
    CleaningTask ||--|| CleaningMethod : uses
    CleaningTask ||--o| ChemicalApplication : applies
    CleaningTask ||--|| InspectionResult : verifiedBy
    FloorEquipment ||--o{ CleaningTask : usedIn
    SurfaceProfile ||--o{ ChemicalApplication : specifies
```
...
stateDiagram-v2
    ...
    MethodChosen --> Cleaned : cleaningExecuted
    Cleaned --> Finished : finishApplied
    Cleaned --> Inspected : surfaceInspected
    Finished --> Inspected : surfaceInspected
    Inspected --> Cleaned : re-clean needed
    Inspected --> Logged : completionLogged
    Logged --> [*]
```
