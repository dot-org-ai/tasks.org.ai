...
```mermaid
erDiagram
    Machine ||--o{ CleaningProcedure : follows
    Machine ||--|| LockoutPermit : requires
    CleaningProcedure ||--o{ CleaningAgent : uses
    Machine ||--o{ PartsList : "composed of"
    Machine ||--o{ MaintenanceLog : "tracked in"
    MaintenanceLog ||--o{ FunctionalTest : records
```
...
stateDiagram-v2
    [*] --> Operating
    Operating --> LockedOut : lockoutPermitIssued
    LockedOut --> Disassembled : componentsDisassembled
    Disassembled --> Cleaning : cleaningAgentApplied
    Cleaning --> Rinsed : equipmentRinsed
    Rinsed --> Reassembled : componentsReassembled
    Reassembled --> Testing : functionalTestPassed
    Reassembled --> Failed : functionalTestFailed
    Failed --> Disassembled : Diagnose and re-clean
    Testing --> Documented : maintenanceLogged
    Documented --> [*]
```
