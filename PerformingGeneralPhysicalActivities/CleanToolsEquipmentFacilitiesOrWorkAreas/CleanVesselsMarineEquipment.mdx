...
```mermaid
erDiagram
    Vessel ||--o{ CleaningWorkOrder : receives
    Vessel ||--|| HullConditionReport : "assessed by"
    Vessel ||--o{ BallastTank : contains
    CleaningWorkOrder ||--o{ WasteDischargeLog : generates
    Vessel ||--|| MaintenanceSchedule : follows
    CleaningWorkOrder ||--|| MaintenanceSchedule : "scheduled by"
```
...
stateDiagram-v2
    [*] --> InService
    InService --> Assessed : hullAssessed
    Assessed --> HullClean : hullScrubbed
    HullClean --> DeckClean : deckWashed
    DeckClean --> BallastClean : ballastTankFlushed
    BallastClean --> EngineClean : engineDegreased
    EngineClean --> Certified : cleaningCertified
    EngineClean --> Violation : dischargeViolationFlagged
    Violation --> Assessed : Re-assess
    Certified --> [*]
```
