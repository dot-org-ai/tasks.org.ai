...
```mermaid
erDiagram
    TransportRequest ||--|| PatientRecord : references
    TransportRequest ||--|| TransportEquipment : requires
    TransportRequest ||--|| Route : follows
    TransportRequest ||--|| SafetyChecklist : validates
    TransportRequest ||--|| Handoff : "completes with"
    PatientRecord ||--|| Handoff : "transferred via"
```
...
stateDiagram-v2
    [*] --> Requested
    Requested --> Verified : patientVerified
    Verified --> Assigned : transporterAssigned
    Assigned --> InTransit : patientInTransit
    InTransit --> Delivered : patientDelivered
    Delivered --> HandedOff : handoffCompleted
    HandedOff --> Documented : transportDocumented
    Documented --> [*]
    Verified --> Requested : verificationFailed
    InTransit --> Assigned : routeBlocked
```
...
graph TD
    ...
    D -->|Yes| E[movePatient]
    D -->|No| F[Resolve Safety Issue]
    F --> C
    E --> G[performHandoff]
    G --> H[documentTransport]
    H --> I{More Transports?}
    I -->|Yes| A
    I -->|No| J[Shift Complete]
```
