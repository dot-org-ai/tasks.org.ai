...
```mermaid
erDiagram
    TripRequest ||--|| Vehicle : assigns
    TripRequest ||--|| Route : follows
    TripRequest ||--o{ AccessibilityRequirement : specifies
    TripRequest ||--|| TripLog : "documented by"
    TripLog ||--|| BillingRecord : generates
    Vehicle ||--o{ TripRequest : serves
```
...
stateDiagram-v2
    [*] --> Scheduled
    Scheduled --> DriverAssigned : driverAssigned
    Scheduled --> Cancelled : tripCancelled
    DriverAssigned --> EnRoute
    EnRoute --> ClientPickedUp : clientPickedUp
    ClientPickedUp --> Delivered : clientDelivered
    Delivered --> WaitingForReturn
    WaitingForReturn --> Returning : returnClient
    Returning --> ClientReturned : clientReturned
    Delivered --> Logged : tripLogged
    ClientReturned --> Logged : tripLogged
    Logged --> [*]
    Cancelled --> [*]
```
