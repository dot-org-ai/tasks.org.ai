...
erDiagram
    ...
    Order ||--o{ AllergyAlert : flags
    Order ||--|| ServiceTicket : tracks
    ServiceTicket ||--|| Check : generates
...
stateDiagram-v2
    [*] --> OrderTaken
    OrderTaken --> Submitted : orderSubmitted
    Submitted --> FoodInPrep
    FoodInPrep --> FoodDelivered : foodDelivered
    Submitted --> BeverageServed : beverageServed
    BeverageServed --> FoodDelivered : foodDelivered
    FoodDelivered --> SatisfactionCheck : checkSatisfaction
    SatisfactionCheck --> Satisfied : satisfactionConfirmed
    SatisfactionCheck --> Remaking : issueDetected
    Remaking --> FoodDelivered : foodDelivered
    Satisfied --> CheckPresented : checkPresented
    CheckPresented --> Cleared : tableCleared
    Cleared --> [*]
```
