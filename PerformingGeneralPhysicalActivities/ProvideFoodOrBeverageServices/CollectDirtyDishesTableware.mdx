...
```mermaid
erDiagram
    TableStatus ||--o{ DirtyDishBatch : generates
    DirtyDishBatch ||--|{ Serviceware : contains
    DirtyDishBatch ||--o{ FoodWaste : produces
    BussTub ||--|{ DirtyDishBatch : carries
    TableStatus ||--|| BussTub : "cleared into"
```
...
stateDiagram-v2
    [*] --> Occupied
    Occupied --> NeedsClearing : tableIdentified
    NeedsClearing --> Clearing : clearTable
    Clearing --> Scraping : foodWasteScraped
    Scraping --> Sorting : servicewareSorted
    Sorting --> InTransit : transportToDishRoom
    InTransit --> Delivered : dishesDelivered
    Delivered --> [*]
    NeedsClearing --> NeedsClearing : tableIdentified
    Clearing --> ReadyToReset : tableStatusUpdated
    ReadyToReset --> [*]
```
