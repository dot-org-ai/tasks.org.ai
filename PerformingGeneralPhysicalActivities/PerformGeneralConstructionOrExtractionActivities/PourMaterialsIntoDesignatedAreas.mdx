...
```mermaid
erDiagram
    PourPlan ||--|| MixDesign : specifies
    PourPlan ||--o{ BatchTicket : schedules
    BatchTicket ||--o{ SlumpTest : verifies
    PourPlan ||--o{ CylinderSample : produces
    CylinderSample }o--|| MixDesign : validates
    PourPlan ||--|| CuringSchedule : follows
```
...
stateDiagram-v2
    [*] --> Planned : pourPlanned
    Planned --> Scheduled : batchesScheduled
    Scheduled --> Placed : materialPlaced
    Placed --> Consolidated : pourConsolidated
    Consolidated --> Leveled : surfaceScreeded
    Leveled --> Sampled : samplesCollected
    Sampled --> Curing : curingInitiated
    Curing --> Curing : continue curing
    Curing --> [*]
```
