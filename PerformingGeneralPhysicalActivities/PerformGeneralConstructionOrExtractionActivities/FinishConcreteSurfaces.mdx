...
```mermaid
erDiagram
    FinishSpecification ||--|| SetTimeWindow : constrains
    FinishSpecification ||--o{ TrowelPass : requires
    FinishSpecification ||--|| CuringMethod : specifies
    TrowelPass }o--|| FlatnessReport : measures
    ControlJoint ||--|| FinishSpecification : follows
    CuringMethod ||--|| FlatnessReport : affects
```
...
```mermaid
stateDiagram-v2
    [*] --> Assessed : setTimeAssessed
    Assessed --> Floated : surfaceBullFloated
    Floated --> Troweled : surfacePowerTroweled
    Troweled --> Textured : textureApplied
    Troweled --> JointsCut : controlJointsCut
    Textured --> JointsCut : controlJointsCut
    JointsCut --> Cured : curingApplied
    Cured --> Measured : flatnessMeasured
    Measured --> Troweled : grinding required
    Measured --> [*]
```
