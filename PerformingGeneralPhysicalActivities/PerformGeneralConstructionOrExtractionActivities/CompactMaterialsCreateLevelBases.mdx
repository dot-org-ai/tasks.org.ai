...
```mermaid
erDiagram
    CompactionSpecification ||--o{ FillLift : governs
    FillLift ||--|| DensityTestResult : validates
    FillLift ||--|| MoistureConditionReport : documents
    ProctorTest ||--|| CompactionSpecification : establishes
    GradeStake ||--o{ FillLift : targets
    DensityTestResult }o--|| ProctorTest : compares
```
...
```mermaid
stateDiagram-v2
    [*] --> Placed : fillLiftPlaced
    Placed --> Conditioned : moistureConditioned
    Conditioned --> Compacted : liftCompacted
    Compacted --> Tested : densityTested
    Tested --> Verified : gradeVerified
    Tested --> Compacted : recompact
    Verified --> Placed : next lift
    Verified --> Documented : compactionDocumented
    Documented --> [*]
```
