...
```mermaid
erDiagram
    StackingPattern ||--|{ PalletLoad : "arranges"
    PalletLoad ||--|| StackHeightLimit : "constrained by"
    PalletLoad ||--|| WrapSpecification : "secured per"
    PalletLoad ||--|| PalletLabel : "identified by"
    PalletLoad ||--|| StagingLocation : "assigned to"
    StackingPattern ||--|| WrapSpecification : "paired with"
```
...
stateDiagram-v2
    [*] --> PatternSelected
    PatternSelected --> Palletized : itemsPalletized
    Palletized --> Wrapped : stackWrapped
    Wrapped --> HeightChecked : stackHeightVerified
    HeightChecked --> Labeled : palletLabeled
    HeightChecked --> Restacking : heightExceeded
    Restacking --> Palletized : restacked
    Labeled --> Staged : stagingLocationAssigned
    Staged --> Released : stackReleasedForPickup
    Released --> [*]
```
