...
```mermaid
erDiagram
    DockAssignment ||--|{ PackingList : "stages"
    PackingList ||--|| BillOfLading : "summarized in"
    BillOfLading ||--|| LoadPlan : "arranged per"
    LoadPlan ||--|| WeightTicket : "weighed via"
    BillOfLading ||--|| SealRecord : "secured by"
    DockAssignment ||--|| WeightTicket : "generates"
```
...
stateDiagram-v2
    [*] --> Scheduled
    Scheduled --> Staged : itemsStaged
    Staged --> Verified : shipmentVerified
    Verified --> Loaded : vehicleLoaded
    Loaded --> Balanced : loadBalanced
    Balanced --> Sealed : containerSealed
    Balanced --> Redistributing : overweight
    Redistributing --> Balanced : rebalanced
    Sealed --> Departed : departureConfirmed
    Departed --> [*]
```
