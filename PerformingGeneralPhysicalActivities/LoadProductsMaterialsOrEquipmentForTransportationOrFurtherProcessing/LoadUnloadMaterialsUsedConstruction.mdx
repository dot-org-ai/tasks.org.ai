...
```mermaid
erDiagram
    PurchaseOrder ||--|{ DeliveryTicket : "fulfilled by"
    DeliveryTicket ||--|| RiggingPlan : "lifted per"
    DeliveryTicket ||--|| MaterialStagingArea : "placed at"
    DeliveryTicket ||--|| ReceivingReport : "verified in"
    DeliveryTicket ||--|| WeightCertificate : "weighed via"
    ReceivingReport ||--o{ PurchaseOrder : "reconciles"
```
...
stateDiagram-v2
    [*] --> DeliveryScheduled
    DeliveryScheduled --> RiggingReady : riggingPrepared
    RiggingReady --> Unloaded : materialsUnloaded
    Unloaded --> Verified : deliveryVerified
    Verified --> InventoryRecorded : inventoryUpdated
    Verified --> DiscrepancyFlagged : discrepancyReported
    DiscrepancyFlagged --> InventoryRecorded : discrepancyResolved
    InventoryRecorded --> [*]
```
