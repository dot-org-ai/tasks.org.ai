...
```mermaid
erDiagram
    DeliveryAppointment ||--|| ReceivingOrder : "expects"
    ReceivingOrder ||--|| ReceivingTally : "counted in"
    ReceivingOrder ||--o{ DamageReport : "may generate"
    ReceivingTally ||--|| ProofOfDelivery : "signed via"
    ProofOfDelivery ||--|{ PutAwayDirective : "triggers"
    DeliveryAppointment ||--|| ProofOfDelivery : "completed by"
```
...
stateDiagram-v2
    [*] --> Scheduled
    Scheduled --> CheckedIn : vehicleCheckedIn
    CheckedIn --> Unloaded : itemsUnloaded
    Unloaded --> Inspected : damageInspected
    Inspected --> DamageFiled : damageFound
    Inspected --> Tallied : quantityTallied
    DamageFiled --> Tallied : quantityTallied
    Tallied --> SignedOff : proofOfDeliverySigned
    Tallied --> ShortageReported : discrepancyDetected
    ShortageReported --> SignedOff : proofOfDeliverySigned
    SignedOff --> PutAway : putAwayDirected
    PutAway --> [*]
```
