...
```mermaid
erDiagram
    LoadManifest ||--|{ HandlingInstruction : "includes"
    LoadManifest ||--o| LiftPlan : "may require"
    LoadManifest ||--|| TieDownConfiguration : "secured by"
    TieDownConfiguration ||--|| DepartureInspection : "verified in"
    LoadManifest ||--o{ DamageWaiver : "documents"
    DepartureInspection ||--|| LoadManifest : "clears"
```
...
stateDiagram-v2
    [*] --> Planning
    Planning --> MethodSelected : handlingMethodSelected
    MethodSelected --> Positioning : itemsPositioned
    Positioning --> Securing : loadSecured
    Securing --> Inspecting : preDepartureInspected
    Inspecting --> Documented : manifestGenerated
    Inspecting --> Resecuring : loadInsecure
    Resecuring --> Inspecting : reinspected
    Documented --> Released : loadReleased
    Released --> [*]
```
