...
```mermaid
erDiagram
    GradeReport ||--|| LoadingTally : "qualifies"
    LoadingTally ||--|| WeightTicket : "weighed via"
    LoadingTally ||--|| BillOfLading : "documented in"
    MoistureTest ||--|| GradeReport : "supplements"
    BillOfLading ||--o| PhytosanitaryCertificate : "may require"
    GradeReport ||--|| BillOfLading : "referenced by"
```
...
stateDiagram-v2
    [*] --> Grading
    Grading --> Staged : productStaged
    Staged --> Loading : vehicleLoaded
    Loading --> Weighing : shipmentWeighed
    Weighing --> MoistureTesting : moistureTested
    MoistureTesting --> ExportClearing : exportRequired
    MoistureTesting --> Documented : billOfLadingGenerated
    MoistureTesting --> DryingRequired : moistureTooHigh
    DryingRequired --> Grading : regradeAfterDrying
    ExportClearing --> Documented : phytosanitaryCleared
    Documented --> [*]
```
