...
```mermaid
erDiagram
    SecurementPlan ||--|{ TieDown : "specifies"
    SecurementPlan ||--|{ BlockingDevice : "includes"
    SecurementPlan ||--|{ BracingSystem : "includes"
    TieDown ||--|| WorkingLoadLimit : "rated by"
    SecurementPlan ||--|| SecurementInspection : "verified by"
    SecurementPlan ||--o{ DunnageMaterial : "may use"
```
...
stateDiagram-v2
    [*] --> Assessing
    Assessing --> Planned : securementPlanned
    Planned --> TiedDown : tieDownsApplied
    TiedDown --> Blocked : blockingInstalled
    Blocked --> Braced : bracingInstalled
    Braced --> Inspecting : securementInspected
    Inspecting --> Documented : complianceDocumented
    Inspecting --> Correcting : deficienciesFound
    Correcting --> Inspecting : reinspected
    Documented --> InTransit : released
    InTransit --> EnRouteCheck : mileageThresholdReached
    EnRouteCheck --> InTransit : stillSecure
    EnRouteCheck --> Correcting : adjustmentNeeded
```
