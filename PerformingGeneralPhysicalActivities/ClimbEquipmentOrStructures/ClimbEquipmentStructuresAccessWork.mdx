...
```mermaid
erDiagram
    Structure ||--|{ AccessRoute : "has"
    AccessRoute ||--|| FallArrestSystem : "protected by"
    Structure ||--|{ InspectionTag : "certified by"
    AccessPermit }o--|| Structure : "grants access to"
    AccessPermit }o--|| ClimbingCertification : "requires"
    Structure ||--|| RescuePlan : "covered by"
    ClimbingCertification ||--o{ AccessPermit : "enables"
```
...
stateDiagram-v2
    [*] --> Planning
    Planning --> CompetencyCheck : accessRoutePlanned
    CompetencyCheck --> StructureCheck : competencyVerified
    CompetencyCheck --> Blocked : certificationExpired
    Blocked --> CompetencyCheck : certificationRenewed
    StructureCheck --> Rigging : structureInspected
    StructureCheck --> Planning : structureUnsafe
    Rigging --> Authorized : fallProtectionRigged
    Authorized --> Ascending : accessAuthorized
    Ascending --> AtHeight : ascentCompleted
    AtHeight --> Descending : workComplete
    Descending --> Complete : descentCompleted
    Complete --> [*]
```
