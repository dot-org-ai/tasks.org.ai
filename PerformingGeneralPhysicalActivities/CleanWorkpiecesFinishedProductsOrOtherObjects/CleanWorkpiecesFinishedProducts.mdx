...
```mermaid
erDiagram
    CleaningBatch ||--|{ Workpiece : "contains"
    CleaningBatch ||--|| CleaningSolution : "uses"
    Workpiece ||--|| CleanlinessSpec : "must meet"
    Workpiece ||--o{ InspectionRecord : "verified by"
    CleaningEquipment ||--o{ CleaningBatch : "processes"
    CleaningSolution ||--o{ InspectionRecord : "effectiveness tracked in"
```
...
stateDiagram-v2
    [*] --> Initiated
    Initiated --> CycleRunning : cleaningInitiated
    CycleRunning --> CycleComplete : cycleCompleted
    CycleComplete --> Inspecting : inspectCleanliness
    Inspecting --> Verified : cleanlinessVerified
    Inspecting --> DefectFound : defectDetected
    DefectFound --> CycleRunning : recleanRequired
    Verified --> Released : batchReleased
    Released --> WasteHandled : wasteDisposed
    CycleRunning --> SolutionChanged : solutionReplaced
    SolutionChanged --> CycleRunning : cycleResumed
    WasteHandled --> [*]
```
