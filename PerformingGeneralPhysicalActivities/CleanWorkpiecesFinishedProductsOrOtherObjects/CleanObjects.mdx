...
```mermaid
erDiagram
    CleaningOrder ||--|{ CleanableObject : "contains"
    CleanableObject ||--|| CleaningMethod : "treated with"
    CleaningMethod ||--o{ CleaningSolution : "uses"
    CleanableObject ||--o{ InspectionRecord : "verified by"
    CleaningStation ||--o{ CleanableObject : "processes"
    CleaningOrder ||--o{ InspectionRecord : "documented in"
```
...
stateDiagram-v2
    [*] --> OrderCreated
    OrderCreated --> Assessed : conditionAssessed
    Assessed --> Cleaning : cleaningProcessExecuted
    Cleaning --> Rinsing : objectRinsed
    Rinsing --> Inspecting : objectInspected
    Inspecting --> Released : objectReleased
    Inspecting --> Cleaning : recleanRequired
    Released --> [*]
```
