...
```mermaid
erDiagram
    GarmentTag ||--o{ CleaningCycle : "processed through"
    CleaningCycle ||--|| SolventCharge : "uses"
    GarmentTag ||--o{ SpottingFormula : "treated with"
    GarmentTag ||--o| PressProgram : "finished using"
    CleaningCycle ||--o{ MaintenanceLog : "tracked in"
    MaintenanceLog ||--o{ EmissionReport : "feeds into"
    SolventCharge ||--o{ EmissionReport : "reported in"
```
...
stateDiagram-v2
    [*] --> Intake
    Intake --> SpotTreating : stainSpotTreated
    Intake --> MachineLoading : machineLoaded
    SpotTreating --> MachineLoading : machineLoaded
    MachineLoading --> CycleRunning : cycleSelected
    CycleRunning --> CycleComplete : cycleCompleted
    CycleComplete --> Inspecting : finishInspected
    Inspecting --> SpotTreating : stainsRemain
    Inspecting --> Pressing : garmentPressed
    Pressing --> FinalInspection : finishInspected
    FinalInspection --> Complete : meetsStandards
    FinalInspection --> Pressing : rePressNeeded
    Complete --> [*]
```
