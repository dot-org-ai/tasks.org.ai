...
```mermaid
erDiagram
    SortCategory ||--|{ WashLoad : "groups into"
    WashLoad ||--|| WashFormula : "processed with"
    WashLoad ||--|| DryingProgram : "dried using"
    WashLoad ||--o{ QualityCheck : "inspected by"
    WashLoad ||--|| PackagingSpec : "packaged per"
    WashFormula ||--o{ ChemicalInventory : "draws from"
```
...
stateDiagram-v2
    [*] --> Received
    Received --> Sorted : incomingSorted
    Sorted --> FormulaSet : washFormulaSelected
    FormulaSet --> Washing : washLoadProcessed
    Washing --> Drying : itemsDried
    Drying --> Inspecting : cleanlinessInspected
    Inspecting --> Packaging : finishedPackaged
    Inspecting --> FormulaSet : rewashRequired
    Packaging --> Complete : chemicalUsageTracked
    Complete --> [*]
```
