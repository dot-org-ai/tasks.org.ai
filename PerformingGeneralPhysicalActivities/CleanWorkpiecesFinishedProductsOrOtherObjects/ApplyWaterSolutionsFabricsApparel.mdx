...
```mermaid
erDiagram
    TreatmentRecipe ||--|{ DyeBath : "prepared as"
    TreatmentRecipe ||--o{ TreatmentLog : "recorded in"
    DyeBath ||--o{ PadPickup : "measured by"
    ColorStandard ||--o{ FastnessTest : "validated through"
    TreatmentLog ||--|| ColorStandard : "compared against"
    DyeBath ||--o{ WastewaterSample : "generates"
    TreatmentRecipe ||--|| ColorStandard : "targets"
```
...
stateDiagram-v2
    [*] --> RecipePreparation
    RecipePreparation --> MachineSetup : recipePrepared
    MachineSetup --> Applying : machineConfigured
    Applying --> DwellMonitoring : treatmentApplied
    DwellMonitoring --> PickupTesting : dwellTimeCompleted
    PickupTesting --> ColorEvaluation : pickupMeasured
    PickupTesting --> Applying : pickupOutOfRange
    ColorEvaluation --> FastnessTesting : colorEvaluated
    ColorEvaluation --> RecipePreparation : colorMismatch
    FastnessTesting --> Approved : fastnessTestCompleted
    FastnessTesting --> RecipePreparation : fastnessFailure
    Approved --> [*]
```
