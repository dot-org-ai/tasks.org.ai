...
```mermaid
erDiagram
    MaterialBatch ||--|| CleaningMethod : "treated with"
    MaterialBatch ||--|| SurfaceSpecification : "must meet"
    CleaningMethod ||--|{ CleaningAgent : "uses"
    MaterialBatch ||--o{ ContaminationReport : "assessed by"
    MaterialBatch ||--o| ReadinessCertificate : "certified by"
    SurfaceSpecification ||--o{ ContaminationReport : "compared against"
```
...
stateDiagram-v2
    [*] --> Received
    Received --> MethodSelected : cleaningMethodSelected
    MethodSelected --> Cleaning : cleaningExecuted
    Cleaning --> Rinsing : materialsRinsed
    Rinsing --> Verifying : cleanlinessVerified
    Verifying --> Certified : readinessCertified
    Verifying --> MethodSelected : reCleanRequired
    Certified --> WasteDisposed : cleaningWasteDisposed
    WasteDisposed --> [*]
```
