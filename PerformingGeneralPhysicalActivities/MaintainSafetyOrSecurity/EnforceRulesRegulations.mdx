...
```mermaid
erDiagram
    Rule ||--|{ Violation : "breached by"
    Violation ||--|| Citation : "issued as"
    Citation ||--|| Penalty : "results in"
    Citation ||--o| AppealCase : "may trigger"
    InspectionReport ||--|{ Violation : "discovers"
    Rule ||--|{ InspectionReport : "checked in"
```
...
stateDiagram-v2
    [*] --> Inspecting
    Inspecting --> Compliant : complianceCheckCompleted
    Inspecting --> ViolationFound : violationDetected
    Compliant --> [*]
    ViolationFound --> Cited : citationIssued
    Cited --> Penalized : penaltyAssessed
    Cited --> Escalated : violationEscalated
    Penalized --> Appealed : appealFiled
    Penalized --> Closed : caseClosed
    Appealed --> Closed : appealProcessed
    Escalated --> Closed : caseClosed
```
