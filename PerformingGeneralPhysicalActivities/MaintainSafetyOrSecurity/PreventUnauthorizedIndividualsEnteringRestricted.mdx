...
```mermaid
erDiagram
    AccessPoint ||--|{ Credential : "accepts"
    Credential ||--|| AccessRequest : "presented in"
    AccessRequest ||--o| DenialRecord : "may result in"
    AccessPoint ||--|| RestrictedZone : "controls entry to"
    DenialRecord ||--o| IncidentReport : "may escalate to"
    RestrictedZone ||--|{ AccessPoint : "accessed via"
```
...
stateDiagram-v2
    [*] --> CredentialPresented
    CredentialPresented --> Verified : credentialVerified
    CredentialPresented --> Invalid : credentialInvalid
    Verified --> Granted : entryGranted
    Invalid --> Denied : entryDenied
    Granted --> Logged : accessAttemptLogged
    Denied --> Logged : accessAttemptLogged
    Denied --> Escalated : incidentEscalated
    Escalated --> LockedDown : accessPointLockedDown
    Escalated --> Logged : accessAttemptLogged
    LockedDown --> Logged : accessAttemptLogged
    Logged --> [*]
```
