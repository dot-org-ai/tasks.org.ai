...
```mermaid
erDiagram
    SecurityPlan ||--|{ DeploymentOrder : "generates"
    DeploymentOrder ||--|{ PatrolZone : "assigns"
    PatrolZone ||--|{ CrowdAssessment : "evaluated via"
    CrowdAssessment ||--o{ IncidentReport : "may trigger"
    SecurityPlan ||--|| CommunicationsLog : "tracked in"
    IncidentReport ||--|| CommunicationsLog : "logged in"
```
...
stateDiagram-v2
    [*] --> Planning
    Planning --> Deployed : personnelDeployed
    Deployed --> Monitoring : crowdConditionsAssessed
    Monitoring --> Monitoring : conditionsStable
    Monitoring --> Deescalating : situationDeescalated
    Monitoring --> Responding : disturbanceDetected
    Deescalating --> Monitoring : tensionsReduced
    Responding --> Coordinating : agenciesCoordinated
    Coordinating --> AdvisoryIssued : publicAdvisoryIssued
    AdvisoryIssued --> Monitoring : situationStabilized
    AdvisoryIssued --> Responding : escalation
```
