...
```mermaid
erDiagram
    SafetyInspection ||--|{ Hazard : "discovers"
    Hazard ||--|{ CorrectiveAction : "resolved by"
    SafetyProtocol ||--|{ SafetyInspection : "checked in"
    Hazard ||--o{ IncidentReport : "may cause"
    CorrectiveAction ||--|| SafetyEquipment : "may require"
    SafetyProtocol ||--|{ SafetyEquipment : "specifies"
```
...
stateDiagram-v2
    ...
    Inspecting --> Compliant : complianceVerified
    HazardFound --> ActionAssigned : correctiveActionIssued
    ActionAssigned --> Enforcing : protocolEnforced
    Enforcing --> Verifying : complianceVerified
    Verifying --> Resolved : hazardResolved
    Verifying --> ActionAssigned : notResolved
    Resolved --> PlanUpdated : safetyPlanUpdated
    PlanUpdated --> Inspecting : nextCycle
    Compliant --> Inspecting : nextCycle
    HazardFound --> IncidentLogged : incidentReported
    IncidentLogged --> ActionAssigned : correctiveActionIssued
```
