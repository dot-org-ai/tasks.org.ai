...
```mermaid
erDiagram
    EscortRequest ||--|| Principal : "protects"
    EscortRequest ||--|| ThreatAssessment : "informed by"
    ThreatAssessment ||--|| Routeplan : "shapes"
    EscortRequest ||--|| EscortAssignment : "fulfilled by"
    EscortAssignment ||--|| Routeplan : "follows"
    EscortAssignment ||--o{ IncidentReport : "may generate"
```
...
stateDiagram-v2
    [*] --> RequestReceived
    RequestReceived --> ThreatAssessed : threatAssessmentCompleted
    ThreatAssessed --> RoutePlanned : escortRoutePlanned
    RoutePlanned --> DetailAssigned : escortDetailAssigned
    DetailAssigned --> InProgress : escortStarted
    InProgress --> IncidentHandling : escortIncidentReported
    IncidentHandling --> InProgress : incidentResolved
    IncidentHandling --> RouteChanged : alternateRouteActivated
    RouteChanged --> InProgress : resumed
    InProgress --> Completed : escortCompleted
    Completed --> [*]
```
