...
```mermaid
erDiagram
    ActivitySchedule ||--|{ ParticipantProfile : "includes"
    ParticipantProfile ||--|| ConsentForm : "authorized by"
    ActivitySchedule ||--|| SupervisionLog : "documented in"
    SupervisionLog ||--o{ IncidentReport : "may generate"
    ActivitySchedule ||--|| ActivityReport : "summarized in"
    ConsentForm }o--|| ActivitySchedule : "required for"
```
...
stateDiagram-v2
    [*] --> Scheduled
    Scheduled --> Confirmed : participationConfirmed
    Confirmed --> Briefed : participantsBriefed
    Confirmed --> Cancelled : consentMissing
    Briefed --> InTransit : groupDeparted
    InTransit --> InProgress : activityCompleted
    InProgress --> IncidentHandling : incidentOccurred
    IncidentHandling --> InProgress : incidentResolved
    InProgress --> Documenting : outcomeDocumented
    Documenting --> Returned : participantsReturned
    Returned --> [*]
```
