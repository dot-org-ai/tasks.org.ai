...
```mermaid
erDiagram
    TransportRequest ||--|| DriverAssignment : "fulfilled by"
    DriverAssignment ||--|| Vehicle : "uses"
    TransportRequest ||--|{ PassengerManifest : "lists passengers in"
    TransportRequest ||--|| TripRecord : "documented in"
    TripRecord ||--|| MileageLog : "tracked in"
    Vehicle ||--|{ MileageLog : "accumulates"
```
...
stateDiagram-v2
    [*] --> Requested
    Requested --> Assigned : driverAssigned
    Assigned --> PickedUp : pickupConfirmed
    Assigned --> Reassigned : driverUnavailable
    Reassigned --> Assigned : newDriverAssigned
    PickedUp --> InTransit : transportInProgress
    InTransit --> Delivered : dropoffConfirmed
    Delivered --> Logged : tripDetailsLogged
    Logged --> Closed : transportRequestClosed
    Closed --> [*]
```
...
graph TD
    ...
    E --> F[logTripDetails]
    F --> G{Trip Complete?}
    G -->|Yes| H[closeTransportRequest]
    G -->|No| I[Schedule Return Trip]
    I --> A
```
