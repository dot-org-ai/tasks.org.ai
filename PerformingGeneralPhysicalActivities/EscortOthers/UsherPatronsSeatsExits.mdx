...
```mermaid
erDiagram
    Ticket ||--|| SeatingAssignment : "specifies"
    SeatingAssignment ||--|| VenueSection : "located in"
    VenueSection ||--|| CapacityCount : "tracked by"
    VenueSection ||--|| EvacuationPlan : "covered by"
    VenueSection ||--o{ IncidentLog : "may generate"
    Ticket }|--|| VenueSection : "admits to"
```
...
stateDiagram-v2
    [*] --> AwaitingEntry
    AwaitingEntry --> Scanned : ticketScanned
    Scanned --> Denied : invalidTicket
    Scanned --> Escorting : patronSeated
    Denied --> [*]
    Escorting --> Monitoring : sectionMonitored
    Monitoring --> IncidentHandling : incidentLogged
    IncidentHandling --> Monitoring : incidentResolved
    Monitoring --> Exiting : patronsDirectedToExit
    Exiting --> [*]
```
