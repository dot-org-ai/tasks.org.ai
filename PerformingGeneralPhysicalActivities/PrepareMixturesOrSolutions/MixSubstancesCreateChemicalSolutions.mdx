...
```mermaid
erDiagram
    Formulation ||--o{ Reagent : lists
    Formulation ||--|| ConcentrationSpec : targets
    Reagent ||--|| Certificate : certifies
    Formulation ||--o{ Solution : produces
    Solution ||--|| PreparationLog : documents
    Solvent ||--o{ Solution : dissolves
```
...
```mermaid
stateDiagram-v2
    [*] --> Selected : reagentsSelected
    Selected --> Calculated : concentrationCalculated
    Calculated --> Weighed : reagentsWeighed
    Weighed --> Dissolved : solutesDissolved
    Dissolved --> Adjusted : pHAdjusted
    Adjusted --> Verified : solutionVerified
    Verified --> Adjusted : out of spec
    Verified --> Logged : preparationLogged
    Logged --> [*]
```
