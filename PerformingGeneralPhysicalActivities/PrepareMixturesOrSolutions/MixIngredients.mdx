...
erDiagram
    ...
    Batch ||--o{ QualitySample : yields
    QualitySample }o--|| BatchRecord : documents
    BatchRecord }o--|| Recipe : references
```
...
stateDiagram-v2
    [*] --> Loaded : recipeLoaded
    Loaded --> Measured : ingredientsMeasured
    Measured --> Added : ingredientsAdded
    Added --> Blended : mixtureBlended
    Blended --> Sampled : batchSampled
    Sampled --> Validated : batchValidated
    Validated --> Recorded : batchRecorded
    Validated --> Blended : adjust and reblend
    Recorded --> [*]
```
