...
```mermaid
erDiagram
    Chemical ||--|| SafetyDataSheet : documents
    Chemical ||--|| DilutionSpec : follows
    DilutionSpec ||--o{ PreparedSolution : produces
    PreparedSolution ||--|| PPERequirement : mandates
    ApplicationOrder ||--|| Chemical : specifies
    PreparedSolution ||--|| ComplianceRecord : generates
```
...
```mermaid
stateDiagram-v2
    [*] --> Reviewed : safetyDataReviewed
    Reviewed --> Calculated : dilutionCalculated
    Calculated --> Measured : chemicalMeasured
    Measured --> Diluted : solutionDiluted
    Diluted --> Verified : solutionVerified
    Verified --> Diluted : out of spec
    Verified --> Labeled : containerLabeled
    Labeled --> Staged : solutionStaged
    Staged --> [*]
```
