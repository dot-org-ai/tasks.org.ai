...
```mermaid
erDiagram
    PrescriptionOrder ||--|| CompoundFormula : selects
    CompoundFormula ||--o{ ActiveIngredient : contains
    CompoundFormula ||--o{ PreparedMedication : produces
    PreparedMedication ||--|| SterileHood : compounded
    PreparedMedication ||--|| BeyondUseDate : expires
    PrescriptionOrder ||--|| PreparedMedication : fulfills
```
...
stateDiagram-v2
    ...
    Reviewed --> FormulaReady : formulaSelected
    FormulaReady --> Measured : componentsMeasured
    Measured --> Compounded : medicationCompounded
    Compounded --> QCPassed : qualityCheckPassed
    Compounded --> Measured : discard and recompound
    QCPassed --> Labeled : preparationLabeled
    Labeled --> Dispensed : medicationDispensed
    Dispensed --> [*]
```
