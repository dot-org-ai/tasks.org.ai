...
```mermaid
erDiagram
    DeliveryRun ||--o{ SupplyCart : uses
    DeliveryRun ||--|| DeliveryManifest : follows
    DeliveryManifest ||--o{ MealTray : lists
    DeliveryManifest ||--o{ ParLevel : references
    SupplyCart ||--o{ MealTray : carries
    DeliveryRun ||--o{ ParLevel : replenishes
```
...
stateDiagram-v2
    [*] --> Scheduled
    Scheduled --> CartReady : cartPrepared
    CartReady --> EnRoute : runDeparted
    EnRoute --> Delivering : itemsDelivered
    Delivering --> Confirmed : receiptConfirmed
    Confirmed --> Delivering : More stops
    Confirmed --> Logged : distributionLogged
    Logged --> Restocking : locationRestocked
    Restocking --> Logged : Re-check levels
    Logged --> [*]
```
