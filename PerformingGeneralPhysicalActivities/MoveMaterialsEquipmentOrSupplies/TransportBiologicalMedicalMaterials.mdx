...
```mermaid
erDiagram
    Specimen ||--|| ShipmentPackage : "sealed in"
    ShipmentPackage ||--|| TransportManifest : "listed in"
    ShipmentPackage ||--|| HazardClassification : "classified as"
    Specimen ||--o{ ChainOfCustody : "tracked by"
    ShipmentPackage ||--o{ TemperatureLog : "monitored by"
    ChainOfCustody ||--o{ TemperatureLog : includes
```
...
stateDiagram-v2
    [*] --> Collected
    Collected --> Packaged : materialPackaged
    Packaged --> InTransit : transportInitiated
    InTransit --> Monitored : conditionsMonitored
    Monitored --> InTransit : Continue transit
    Monitored --> Transferred : custodyTransferred
    Transferred --> InTransit : Next leg
    InTransit --> Arrived : materialDelivered
    Arrived --> Verified : integrityVerified
    Verified --> Logged : transportLogged
    Logged --> [*]
```
