...
```mermaid
erDiagram
    DeliveryOrder ||--|| Package : "contains"
    DeliveryOrder ||--|| Route : "assigned to"
    Package ||--|{ TrackingEvent : "tracked by"
    DeliveryOrder ||--|| ProofOfDelivery : "confirmed by"
    DeliveryOrder ||--o{ DeliveryException : "may have"
    Route ||--|{ TrackingEvent : "generates"
```
...
stateDiagram-v2
    [*] --> OrderCreated
    OrderCreated --> RouteAssigned : routeAssigned
    RouteAssigned --> PickedUp : itemPickedUp
    PickedUp --> InTransit : itemInTransit
    InTransit --> Delivered : itemDelivered
    InTransit --> FailedAttempt : deliveryFailed
    Delivered --> Confirmed : deliveryConfirmed
    FailedAttempt --> InTransit : retryScheduled
    FailedAttempt --> ReturnedToSender : maxAttemptsReached
    Confirmed --> [*]
    ReturnedToSender --> [*]
```
