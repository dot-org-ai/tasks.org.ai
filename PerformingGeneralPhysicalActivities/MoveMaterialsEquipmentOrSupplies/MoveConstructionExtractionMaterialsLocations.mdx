...
```mermaid
erDiagram
    MaterialLoad ||--|| DeliveryTicket : "documented by"
    MaterialRequest ||--|| MaterialLoad : fulfills
    MaterialLoad ||--|| HaulRoute : "travels via"
    JobSite ||--o{ StagingArea : contains
    StagingArea ||--o{ MaterialLoad : holds
    JobSite ||--o{ MaterialRequest : generates
```
...
stateDiagram-v2
    [*] --> Requested
    Requested --> Scheduled : deliveryScheduled
    Scheduled --> Dispatched : loadDispatched
    Dispatched --> Received : loadReceived
    Received --> Staged : materialStaged
    Staged --> Distributed : materialDistributed
    Staged --> Holding : Hold in staging
    Holding --> Distributed : materialDistributed
    Distributed --> Logged : deliveryLogged
    Logged --> [*]
```
