...
```mermaid
erDiagram
    LuggageItem ||--|| ClaimTicket : "identified by"
    LuggageItem ||--o{ HandlingRequest : "subject of"
    HandlingRequest ||--|| StorageArea : "routed to"
    LuggageItem ||--o{ DamageReport : "may have"
    StorageArea ||--o{ LuggageItem : holds
    ClaimTicket ||--|| HandlingRequest : references
```
...
stateDiagram-v2
    [*] --> WithPatron
    WithPatron --> Collected : luggageCollected
    Collected --> Tagged : luggageTagged
    Tagged --> InTransit : luggageTransported
    InTransit --> Stored : luggageStored
    InTransit --> Delivered : luggageDelivered
    Stored --> Retrieved : luggageRetrieved
    Retrieved --> InTransit : luggageTransported
    Delivered --> DamageFiled : damageReported
    DamageFiled --> [*]
    Delivered --> [*]
```
