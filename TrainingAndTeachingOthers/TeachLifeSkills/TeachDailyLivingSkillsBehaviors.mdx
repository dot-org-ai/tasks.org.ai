...
```mermaid
erDiagram
    SkillAssessment ||--|{ LearningPlan : "informs"
    LearningPlan ||--|{ InstructionSession : "schedules"
    LearningPlan ||--|{ BehaviorGoal : "defines"
    InstructionSession ||--o{ ProgressRecord : "generates"
    InstructionSession }o--o{ AdaptiveAid : "utilizes"
    BehaviorGoal ||--o{ ProgressRecord : "measured by"
```
...
stateDiagram-v2
    [*] --> Intake
    Intake --> Assessed : abilitiesAssessed
    Assessed --> Planned : learningPlanCreated
    Planned --> InSession : sessionConducted
    InSession --> Demonstrated : skillDemonstrated
    Demonstrated --> Tracked : progressTracked
    Tracked --> PlanRevised : planAdjusted
    PlanRevised --> InSession : sessionConducted
    Tracked --> Certified : competencyCertified
    Certified --> [*]
```
