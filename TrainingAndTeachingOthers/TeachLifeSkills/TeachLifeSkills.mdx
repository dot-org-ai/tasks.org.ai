...
```mermaid
erDiagram
    Curriculum ||--|{ Lesson : "contains"
    ProgramCohort ||--|{ ParticipantProfile : "enrolls"
    ParticipantProfile ||--o{ CompetencyRubric : "evaluated by"
    Lesson }o--|| CompetencyRubric : "assessed with"
    ProgramCohort ||--|| Curriculum : "follows"
    ParticipantProfile ||--o{ Certificate : "earns"
```
...
stateDiagram-v2
    [*] --> Enrolled
    Enrolled --> GapsAssessed : skillGapsAssessed
    GapsAssessed --> LearningInProgress : lessonDelivered
    LearningInProgress --> PracticingSkills : rolePlayFacilitated
    PracticingSkills --> UnderEvaluation : competencyEvaluated
    UnderEvaluation --> LearningInProgress : needsRemediation
    UnderEvaluation --> Certified : certificateIssued
    Certified --> Reported : outcomesReported
    Reported --> [*]
```
