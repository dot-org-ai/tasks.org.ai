...
```mermaid
erDiagram
    EducationProgram ||--|{ OutreachEvent : "delivers"
    EducationProgram ||--|{ SafetyMaterial : "produces"
    OutreachEvent ||--o{ InspectionChecklist : "uses"
    OutreachEvent }o--o{ SafetyMaterial : "distributes"
    IncidentReport ||--o{ EducationProgram : "informs"
    OutreachEvent ||--o{ EvacuationPlan : "reviews"
```
...
stateDiagram-v2
    [*] --> ProgramDesign
    ProgramDesign --> Scheduled : outreachScheduled
    Scheduled --> InProgress : presentationConducted
    InProgress --> MaterialsSent : materialsDistributed
    MaterialsSent --> Inspecting : safetyAssessmentPerformed
    Inspecting --> DataCollected : participationTracked
    DataCollected --> Evaluated : impactEvaluated
    Evaluated --> ProgramDesign : needsRevision
    Evaluated --> Active : programEffective
    Active --> Scheduled : newCycleBegins
    Active --> [*]
```
