...
```mermaid
erDiagram
    DiscussionPlan ||--|{ DiscussionPrompt : contains
    DiscussionPlan ||--|{ LearningOutcome : targets
    DiscussionSession ||--|| DiscussionPlan : follows
    DiscussionSession ||--|{ ParticipationRecord : generates
    SessionSummary }o--|| DiscussionSession : documents
    ParticipationRecord }o--|| LearningOutcome : measuresAgainst
```
...
stateDiagram-v2
    [*] --> PlanPrepared
    PlanPrepared --> Scheduled : discussionPlanPrepared
    Scheduled --> InProgress : discussionSessionScheduled
    InProgress --> Monitored : discussionFacilitated
    Monitored --> Summarized : participationMonitored
    Summarized --> Assessed : discussionSummarized
    Assessed --> PlanPrepared : objectives not met
    Assessed --> FeedbackDelivered : discussionOutcomesAssessed
    FeedbackDelivered --> [*]
```
...
graph TD
    ...
    D --> E{Engagement Adequate?}
    E -->|No| F[Adjust Facilitation Approach]
    F --> C
    E -->|Yes| G[summarizeDiscussion]
    G --> H[assessDiscussionOutcomes]
    H --> I{Objectives Met?}
    I -->|No| J[Revise Discussion Plan]
    J --> A
    I -->|Yes| K[provideParticipationFeedback]
```
