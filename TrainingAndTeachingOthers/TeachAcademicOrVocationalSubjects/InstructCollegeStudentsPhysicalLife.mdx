...
```mermaid
erDiagram
    Course ||--|{ LaboratorySession : includes
    Course ||--|{ Examination : assesses
    LaboratorySession ||--|{ LabReport : produces
    LaboratorySession ||--|| SafetyProtocol : requires
    LaboratorySession ||--o{ EquipmentReservation : reserves
    Course ||--|{ LabReport : collects
```
...
stateDiagram-v2
    [*] --> CurriculumReady
    CurriculumReady --> LectureDelivered : lectureDelivered
    LectureDelivered --> SafetyVerified : labSafetyEnforced
    SafetyVerified --> LabComplete : labSessionConducted
    LabComplete --> ReportAssigned : labReportAssigned
    ReportAssigned --> Graded : submissionsGraded
    Graded --> LectureDelivered : termContinues
    Graded --> ExamReady : termEnding
    ExamReady --> ExamComplete : examAdministered
    ExamComplete --> [*]
```
