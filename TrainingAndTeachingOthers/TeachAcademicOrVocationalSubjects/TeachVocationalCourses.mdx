...
```mermaid
erDiagram
    VocationalCourse ||--|{ CompetencyStandard : benchmarks
    VocationalCourse ||--|{ ShopSession : schedules
    ShopSession ||--|{ PracticalExam : assesses
    PracticalExam ||--|| SkillPortfolio : documents
    SkillPortfolio ||--|| CertificationRecord : earns
    CompetencyStandard }o--|| PracticalExam : evaluates
```
...
stateDiagram-v2
    [*] --> CurriculumReady
    CurriculumReady --> Instructed : instructionDelivered
    Instructed --> ShopComplete : shopSessionConducted
    ShopComplete --> Assessed : competencyAssessed
    Assessed --> ShopComplete : notProficient
    Assessed --> Certified : certificationEarned
    Certified --> Instructed : programContinues
    Certified --> Placed : placementCoordinated
    Placed --> Reviewed : programOutcomesReviewed
    Reviewed --> [*]
```
