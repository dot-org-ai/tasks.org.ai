...
```mermaid
erDiagram
    SpecializedCourse ||--|{ ExpertLecture : delivers
    SpecializedCourse ||--o{ CapstoneProject : assigns
    SpecializedCourse ||--|{ SpecializedAssessment : evaluates
    SpecializedCourse ||--|| CoursePortfolio : documents
    CapstoneProject ||--|| PeerReview : reviews
    ExpertLecture }o--|| CoursePortfolio : archives
```
...
stateDiagram-v2
    [*] --> CourseDesigned
    CourseDesigned --> LectureDelivered : expertLectureDelivered
    LectureDelivered --> StudentsGuided : advancedStudentsMentored
    StudentsGuided --> CapstoneAssigned : capstoneProjectAssigned
    CapstoneAssigned --> MasteryChecked : masteryEvaluated
    MasteryChecked --> StudentsGuided : notYetMastered
    MasteryChecked --> PeerReviewed : peerReviewConducted
    PeerReviewed --> Updated : courseContentUpdated
    Updated --> [*]
```
