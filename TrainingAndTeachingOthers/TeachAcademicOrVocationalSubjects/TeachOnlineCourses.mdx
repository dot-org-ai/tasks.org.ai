...
```mermaid
erDiagram
    OnlineCourse ||--|{ CourseModule : contains
    OnlineCourse ||--|{ LiveSession : schedules
    OnlineCourse ||--|{ DiscussionForum : hosts
    CourseModule ||--|{ OnlineAssessment : includes
    OnlineCourse ||--|| LearnerAnalytics : tracks
    DiscussionForum }o--|| CourseModule : discusses
```
...
stateDiagram-v2
    [*] --> Designed
    Designed --> Published : contentPublished
    Published --> LiveDelivered : liveSessionConducted
    LiveDelivered --> Discussed : discussionModerated
    Discussed --> AssessmentOut : assessmentAssigned
    AssessmentOut --> Graded : submissionsGraded
    Graded --> EngagementReviewed : engagementAnalyzed
    EngagementReviewed --> LiveDelivered : termContinues
    EngagementReviewed --> Finalized : termComplete
    Finalized --> [*]
```
