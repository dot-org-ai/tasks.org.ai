...
```mermaid
erDiagram
    TrainingCurriculum ||--|{ TrainingSession : "delivered via"
    TrainingCurriculum ||--|{ TrainingMaterial : "uses"
    TrainingSession ||--|{ LearnerProfile : "tracks"
    TrainingSession ||--o{ CompetencyAssessment : "evaluates"
    CompetencyAssessment ||--o{ CompletionCertificate : "awards"
    LearnerProfile }o--|| CompetencyAssessment : "informs"
```
...
stateDiagram-v2
    [*] --> GapsAssessed
    GapsAssessed --> CurriculumReady : curriculumDesigned
    CurriculumReady --> SessionBooked : sessionScheduled
    SessionBooked --> InTraining : trainingDelivered
    InTraining --> CompetencyCheck : competencyEvaluated
    CompetencyCheck --> InTraining : remediation
    CompetencyCheck --> Certified : certificateIssued
    Certified --> CurriculumReady : materialsUpdated
    Certified --> [*]
```
