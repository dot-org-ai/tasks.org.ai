...
```mermaid
erDiagram
    AssistiveDevice ||--|| FittingRecord : "adjusted via"
    AssistiveDevice ||--|{ InstructionSession : "trained on"
    InstructionSession ||--o{ CompetencyChecklist : "evaluated with"
    AssistiveDevice ||--|| MaintenanceGuide : "maintained per"
    CompetencyChecklist ||--o{ UsageLog : "tracked in"
    FittingRecord }o--|| InstructionSession : "informs"
```
...
stateDiagram-v2
    [*] --> NeedsAssessed
    NeedsAssessed --> DeviceFitted : deviceFitted
    DeviceFitted --> UseDemonstrated : usedemonstrated
    UseDemonstrated --> InPractice : practiceSupervised
    InPractice --> CompetencyCheck : evaluating
    CompetencyCheck --> DeviceFitted : competencyNotAchieved
    CompetencyCheck --> MaintenanceTrained : competencyAchieved
    MaintenanceTrained --> Documented : maintenanceTaught
    Documented --> [*]
```
