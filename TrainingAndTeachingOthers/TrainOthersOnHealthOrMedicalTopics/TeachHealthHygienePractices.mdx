...
```mermaid
erDiagram
    HealthCurriculum ||--|{ InstructionSession : "delivered through"
    HealthCurriculum ||--|{ EducationalMaterial : "uses"
    HygieneAssessment ||--|| HealthCurriculum : "informs"
    InstructionSession ||--o{ OutcomeIndicator : "measures"
    CommunityOutreachPlan ||--|{ InstructionSession : "schedules"
    CommunityOutreachPlan }o--o{ EducationalMaterial : "distributes"
```
...
stateDiagram-v2
    [*] --> BaselineAssessment
    BaselineAssessment --> CurriculumReady : curriculumDeveloped
    CurriculumReady --> ContentAdapted : contentAdapted
    ContentAdapted --> InSession : sessionConducted
    InSession --> Demonstrated : techniqueDemonstrated
    Demonstrated --> ResourcesProvided : resourcesDistributed
    ResourcesProvided --> OutcomesReviewed : outcomesMeasured
    OutcomesReviewed --> ContentAdapted : noImprovement
    OutcomesReviewed --> ProgramExpansion : improvementConfirmed
    ProgramExpansion --> BaselineAssessment : newPopulation
    ProgramExpansion --> [*]
```
