...
```mermaid
erDiagram
    Curriculum ||--|{ ProcedureModule : "contains"
    ProcedureModule ||--|{ SimulationScenario : "practiced via"
    ProcedureModule ||--o{ CompetencyEvaluation : "assessed by"
    CompetencyEvaluation ||--o{ CredentialRecord : "awards"
    SimulationScenario ||--o{ TrainingLog : "records"
    TrainingLog }o--|| CompetencyEvaluation : "informs"
```
...
stateDiagram-v2
    ...
    CurriculumDevelopment --> DidacticPhase : didacticCompleted
    DidacticPhase --> SimulationPhase : simulationCompleted
    SimulationPhase --> DidacticPhase : simulationFailed
    SimulationPhase --> SupervisedPractice : procedureSupervised
    SupervisedPractice --> CaseLogging : procedureLogged
    CaseLogging --> SupervisedPractice : insufficientCases
    CaseLogging --> CompetencyReview : competencyPassed
    CaseLogging --> AdditionalTraining : competencyNotMet
    AdditionalTraining --> SimulationPhase : retraining
    CompetencyReview --> Credentialed : credentialIssued
    Credentialed --> [*]
```
