...
```mermaid
erDiagram
    CMEProgram ||--|{ SimulationScenario : "includes"
    CMEProgram ||--|{ ClinicalProtocol : "teaches"
    CMEProgram ||--o{ CompetencyAssessment : "evaluates via"
    CompetencyAssessment ||--o{ CMECredit : "awards"
    CMECredit ||--|| CredentialRecord : "recorded in"
    ClinicalProtocol }o--|| CompetencyAssessment : "assessed by"
```
...
stateDiagram-v2
    [*] --> ProgramDevelopment
    ProgramDevelopment --> Scheduled : clinicalTrainingScheduled
    Scheduled --> DidacticPhase : lectureDelivered
    DidacticPhase --> SimulationPhase : simulationConducted
    SimulationPhase --> CompetencyReview : competencyAssessed
    CompetencyReview --> DidacticPhase : additionalTrainingNeeded
    CompetencyReview --> CreditsAwarded : cmeCreditsAwarded
    CreditsAwarded --> CredentialsCurrent : credentialsMaintained
    CredentialsCurrent --> ProgramDevelopment : renewalCycleBegins
    CredentialsCurrent --> [*]
```
