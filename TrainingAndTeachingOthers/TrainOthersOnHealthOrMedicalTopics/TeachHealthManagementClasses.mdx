...
```mermaid
erDiagram
    ClassCurriculum ||--|{ ClassSession : "comprises"
    ClassSession ||--|{ Enrollment : "tracks"
    ClassSession ||--o{ AssessmentQuiz : "includes"
    Enrollment ||--o{ ParticipantOutcome : "measures"
    ClassCurriculum ||--|{ EducationMaterial : "uses"
    AssessmentQuiz ||--o{ ParticipantOutcome : "informs"
```
...
stateDiagram-v2
    [*] --> CurriculumDesign
    CurriculumDesign --> Scheduled : classScheduled
    Scheduled --> Enrolling : participantEnrolled
    Enrolling --> PreAssessment : assessmentCompleted
    PreAssessment --> InSession : sessionDelivered
    InSession --> MaterialsProvided : materialsDistributed
    MaterialsProvided --> InSession : moreSessionsRemaining
    MaterialsProvided --> PostAssessment : assessmentCompleted
    PostAssessment --> OutcomesReviewed : outcomesEvaluated
    OutcomesReviewed --> CurriculumDesign : revisionsNeeded
    OutcomesReviewed --> [*]
```
