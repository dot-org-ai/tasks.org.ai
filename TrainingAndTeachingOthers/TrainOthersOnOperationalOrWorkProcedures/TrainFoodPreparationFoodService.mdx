...
```mermaid
erDiagram
    TrainingProgram ||--|{ RecipeStandard : "covers"
    TrainingProgram ||--|{ ServiceProtocol : "includes"
    TrainingProgram ||--|{ TrainingSession : "delivered via"
    TrainingSession ||--o{ CompetencyEvaluation : "produces"
    CompetencyEvaluation ||--o{ FoodSafetyCertification : "awards"
    RecipeStandard }o--|| CompetencyEvaluation : "tested in"
```
...
stateDiagram-v2
    [*] --> ProgramDeveloped
    ProgramDeveloped --> Scheduled : foodTrainingScheduled
    Scheduled --> SafetyTrained : foodSafetyTrainingCompleted
    SafetyTrained --> TechniquePractice : culinaryTechniqueDemonstrated
    TechniquePractice --> ServiceTrained : serviceProtocolsTrained
    ServiceTrained --> CompetencyReview : staffCompetencyEvaluated
    CompetencyReview --> TechniquePractice : remediationRequired
    CompetencyReview --> Certified : foodSafetyCertificationIssued
    Certified --> ProgramDeveloped : renewalCycle
    Certified --> [*]
```
