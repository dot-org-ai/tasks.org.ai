...
```mermaid
erDiagram
    WorkProcess ||--|| ProcessMap : "documented as"
    WorkProcess ||--|{ TrainingModule : "taught via"
    TrainingModule ||--|{ JobAid : "supplements"
    TrainingModule ||--o{ CompetencyCheckpoint : "evaluated at"
    CompetencyCheckpoint ||--o{ TrainingFeedback : "generates"
    ProcessMap }o--|| TrainingModule : "informs"
```
...
stateDiagram-v2
    [*] --> ProcessMapped
    ProcessMapped --> TrainingDeveloped : processTrainingDeveloped
    TrainingDeveloped --> Scheduled : processTrainingScheduled
    Scheduled --> InInstruction : processInstructionDelivered
    InInstruction --> HandsOnPractice : handsOnPracticeCompleted
    HandsOnPractice --> CompetencyReview : processCompetencyAssessed
    CompetencyReview --> FeedbackLoop : trainingFeedbackIncorporated
    FeedbackLoop --> InInstruction : redelivered
    CompetencyReview --> Qualified : competencyMet
    Qualified --> ProcessMapped : processRevised
    Qualified --> [*]
```
