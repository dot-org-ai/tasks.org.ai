...
```mermaid
erDiagram
    WorkInstruction ||--|{ ProceduralTrainingSession : "taught via"
    WorkInstruction ||--|| WorkStandard : "defines"
    ProceduralTrainingSession ||--o{ CompetencyEvaluation : "produces"
    CompetencyEvaluation ||--o{ QualificationRecord : "awards"
    CompetencyEvaluation ||--o{ CorrectiveActionPlan : "triggers"
    QualificationRecord }o--|| WorkStandard : "validates against"
```
...
stateDiagram-v2
    ...
    RequirementsIdentified --> ContentPrepared : instructionalContentPrepared
    ContentPrepared --> InTraining : trainingSessionConducted
    InTraining --> CompetencyReview : competencyAssessed
    CompetencyReview --> Qualified : qualificationIssued
    CompetencyReview --> CorrectiveAction : correctiveActionInitiated
    CorrectiveAction --> InTraining : retrainingScheduled
    Qualified --> RecertDue : recertificationScheduled
    RecertDue --> CompetencyReview : reassessment
    Qualified --> [*]
```
