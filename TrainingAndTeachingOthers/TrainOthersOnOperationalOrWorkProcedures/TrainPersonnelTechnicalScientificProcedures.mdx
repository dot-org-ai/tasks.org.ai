...
```mermaid
erDiagram
    StandardOperatingProcedure ||--|{ TrainingModule : "taught via"
    TrainingModule ||--|{ TrainingSession : "delivered as"
    TrainingSession ||--o{ CompetencyRecord : "produces"
    CompetencyRecord ||--o{ InstrumentQualification : "awards"
    StandardOperatingProcedure ||--o{ ProcedureRevision : "updated by"
    ProcedureRevision }o--|| TrainingModule : "triggers retraining in"
```
...
stateDiagram-v2
    [*] --> TrainingDeveloped
    TrainingDeveloped --> SessionScheduled : trainingSessionScheduled
    SessionScheduled --> InInstruction : proceduralInstructionDelivered
    InInstruction --> PracticePhase : practiceExerciseCompleted
    PracticePhase --> CompetencyReview : proceduralCompetencyEvaluated
    CompetencyReview --> InInstruction : remediationRequired
    CompetencyReview --> Qualified : instrumentQualificationIssued
    Qualified --> TrainingDeveloped : retrainingTriggered
    Qualified --> [*]
```
