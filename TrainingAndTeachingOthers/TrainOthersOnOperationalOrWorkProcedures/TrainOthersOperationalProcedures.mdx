...
```mermaid
erDiagram
    OperatingProcedure ||--|{ TrainingProgram : "taught via"
    TrainingProgram ||--|{ TrainingSession : "delivered as"
    TrainingProgram ||--|{ TrainingMaterial : "uses"
    TrainingSession ||--o{ CompetencyRecord : "produces"
    ComplianceRequirement ||--|{ TrainingProgram : "mandates"
    OperatingProcedure }o--|| ComplianceRequirement : "governed by"
```
...
stateDiagram-v2
    [*] --> NeedsIdentified
    NeedsIdentified --> ProgramDeveloped : trainingProgramDeveloped
    ProgramDeveloped --> Scheduled : trainingScheduled
    Scheduled --> InTraining : trainingDelivered
    InTraining --> CompetencyReview : competencyAssessed
    CompetencyReview --> InTraining : retrainingRequired
    CompetencyReview --> Completed : completionRecorded
    Completed --> ProgramDeveloped : proceduresUpdated
    Completed --> [*]
```
