...
```mermaid
erDiagram
    ComplianceProgram ||--|{ TrainingModule : "contains"
    ComplianceProgram ||--|| PolicyDocument : "implements"
    TrainingModule ||--o{ Attestation : "produces"
    Attestation ||--|| ComplianceRecord : "logged in"
    ComplianceRecord ||--o{ AuditReport : "summarized in"
    PolicyDocument }o--|| ComplianceProgram : "mandates"
```
...
stateDiagram-v2
    [*] --> RequirementsDefined
    RequirementsDefined --> ModuleDeveloped : trainingModuleDeveloped
    ModuleDeveloped --> Assigned : trainingAssigned
    Assigned --> InTraining : trainingCompleted
    InTraining --> Attested : attestationCollected
    Attested --> InTraining : outstandingAttestations
    Attested --> AuditReady : auditReportGenerated
    AuditReady --> ModuleDeveloped : complianceContentUpdated
    AuditReady --> [*]
```
