...
```mermaid
erDiagram
    TrainingProgram ||--|{ ProgramModule : "contains"
    TrainingProgram ||--|{ SessionSchedule : "scheduled via"
    TrainingProgram ||--|{ LearningObjective : "defines"
    SessionSchedule ||--|{ EnrollmentRecord : "tracks"
    TrainingProgram ||--o{ ProgramEvaluation : "assessed by"
    EnrollmentRecord }o--|| ProgramEvaluation : "contributes to"
```
...
stateDiagram-v2
    [*] --> Designed
    Designed --> Enrolling : trainingProgramDesigned
    Enrolling --> Scheduled : trainingSessionsScheduled
    Scheduled --> InSession : trainingSessionDelivered
    InSession --> Assessed : learningOutcomesAssessed
    Assessed --> FeedbackPhase : programFeedbackCollected
    FeedbackPhase --> Reported : programEffectivenessReported
    Reported --> InSession : additionalSessionsNeeded
    Reported --> Complete : allSessionsDelivered
    Complete --> [*]
```
...
graph TD
    ...
    E --> F{Outcomes Acceptable?}
    F -->|Yes| G[collectProgramFeedback]
    F -->|No| D
    G --> H[reportProgramEffectiveness]
    H --> I{Additional Sessions?}
    I -->|Yes| D
    I -->|No| J[End]
```
