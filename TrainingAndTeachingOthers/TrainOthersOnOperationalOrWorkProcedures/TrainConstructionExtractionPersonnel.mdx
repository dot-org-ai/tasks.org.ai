...
```mermaid
erDiagram
    TradeTrainingProgram ||--|{ SafetyOrientation : "requires"
    TradeTrainingProgram ||--o{ SkillCertification : "awards"
    TradeTrainingProgram ||--|{ ApprenticeshipPlan : "follows"
    SafetyOrientation ||--o{ HazardAssessment : "based on"
    ApprenticeshipPlan ||--|{ TrainingRecord : "logged in"
    SkillCertification ||--|| TrainingRecord : "recorded in"
```
...
stateDiagram-v2
    [*] --> ProgramDeveloped
    ProgramDeveloped --> SafetyOriented : safetyOrientationCompleted
    SafetyOriented --> TrainingScheduled : jobsiteTrainingScheduled
    TrainingScheduled --> SkillDemonstrated : tradeSkillDemonstrated
    SkillDemonstrated --> CompetencyReview : tradeCompetencyAssessed
    CompetencyReview --> SkillDemonstrated : additionalTrainingNeeded
    CompetencyReview --> Certified : skillCertificationIssued
    Certified --> ApprenticeProgress : apprenticeshipMilestoneReached
    ApprenticeProgress --> TrainingScheduled : nextPhaseBegins
    ApprenticeProgress --> [*]
```
