...
```mermaid
erDiagram
    PolicyDocument ||--|{ ProcedureManual : "details"
    PolicyDocument ||--|{ TrainingSession : "taught via"
    TrainingSession ||--o{ AcknowledgmentRecord : "produces"
    TrainingSession ||--o{ ComprehensionAssessment : "includes"
    PolicyUpdate ||--|| PolicyDocument : "revises"
    ComprehensionAssessment }o--|| AcknowledgmentRecord : "validates"
```
...
stateDiagram-v2
    [*] --> MaterialsDeveloped
    MaterialsDeveloped --> SessionScheduled : policyTrainingScheduled
    SessionScheduled --> BriefingDelivered : policyBriefingDelivered
    BriefingDelivered --> Assessed : comprehensionAssessed
    Assessed --> BriefingDelivered : failedNeedsRefresher
    Assessed --> Acknowledged : acknowledgmentRecorded
    Acknowledged --> Compliant : complianceAuditCompleted
    Compliant --> MaterialsDeveloped : policyUpdateNotified
    Compliant --> [*]
```
