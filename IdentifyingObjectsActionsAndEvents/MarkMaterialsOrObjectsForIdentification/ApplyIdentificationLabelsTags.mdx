...
```mermaid
erDiagram
    LabelTemplate ||--|{ Label : "designs"
    LabelBatch ||--|{ Label : "produces"
    Label ||--|| Tag : "encodes"
    Label ||--o{ ScanRecord : "verified-by"
    LabelingStandard ||--|| LabelTemplate : "governs"
```
...
stateDiagram-v2
    ...
    Generated --> Affixed : labelApplied
    Affixed --> Confirmed : labelVerified
    Confirmed --> Tagged : tagAssigned
    Confirmed --> Failed : labelVerificationFailed
    Failed --> Reprinted : labelReprinted
    Reprinted --> Affixed : labelApplied
    Tagged --> Deactivated : tagDeactivated
    Tagged --> [*]
```
