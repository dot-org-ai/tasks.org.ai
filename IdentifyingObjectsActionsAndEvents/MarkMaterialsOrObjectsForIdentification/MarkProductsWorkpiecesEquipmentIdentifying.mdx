...
```mermaid
erDiagram
    SerialNumber ||--|| Barcode : "printed-as"
    SerialNumber ||--|| RFIDTag : "encoded-on"
    MarkingTemplate ||--|{ SerialNumber : "formats"
    SerialNumber ||--o{ ProductRecord : "registered-in"
    EquipmentAssetTag ||--|| SerialNumber : "references"
```
...
```mermaid
stateDiagram-v2
    [*] --> Serialized : serialNumberGenerated
    Serialized --> Marked : productMarkApplied
    Marked --> Scanned : markScannedAndVerified
    Scanned --> Registered : productIdentityRegistered
    Marked --> Defective : defectiveMarkReplaced
    Defective --> Marked : productMarkApplied
    Registered --> EquipmentTagged : equipmentTagAttached
    Registered --> TemplateRevised : markingTemplateUpdated
    TemplateRevised --> Serialized : reserialize
    EquipmentTagged --> [*]
```
