...
```mermaid
erDiagram
    Identifier ||--|| Tag : "encoded-on"
    Container ||--|{ Product : "holds"
    Product ||--|| Identifier : "traced-by"
    EncodingProfile ||--|{ Identifier : "formats"
    Identifier ||--o{ AttachmentRecord : "documented-by"
    Identifier ||--o{ VerificationScan : "confirmed-by"
```
...
```mermaid
stateDiagram-v2
    [*] --> Provisioned : identifierProvisioned
    Provisioned --> Encoded : tagEncoded
    Encoded --> Attached : tagAttached
    Attached --> Verified : attachmentVerified
    Verified --> Linked : containerLinked
    Attached --> Failed : tagFailed
    Failed --> Replaced : tagReplaced
    Replaced --> Attached : tagAttached
    Linked --> Retired : tagDecommissioned
    Linked --> [*]
```
