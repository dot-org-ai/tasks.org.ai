...
```mermaid
erDiagram
    LabelTemplate ||--|{ PrintJob : "produces"
    MaterialLot ||--|{ LabelRecord : "labeled-by"
    PrintJob ||--o{ LabelRecord : "outputs"
    ComplianceSpec ||--|{ LabelTemplate : "governs"
    LabelStock ||--|{ PrintJob : "consumed-by"
    LabelRecord ||--o{ ScanEvent : "verified-by"
```
...
```mermaid
stateDiagram-v2
    [*] --> Templated : templateCreated
    Templated --> Generated : labelsGenerated
    Generated --> Printed : labelsPrinted
    Printed --> Applied : labelApplied
    Applied --> Scanned : labelScanned
    Scanned --> Archived : labelScanned
    Scanned --> Mismatched : labelMismatchDetected
    Mismatched --> Reprinted : labelReprinted
    Reprinted --> Applied : labelApplied
    Archived --> [*]
```
