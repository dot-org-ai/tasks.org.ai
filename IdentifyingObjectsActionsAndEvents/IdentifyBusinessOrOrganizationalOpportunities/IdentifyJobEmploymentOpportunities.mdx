...
```mermaid
erDiagram
    CandidateProfile ||--o{ JobOpportunity : "matches"
    JobOpportunity ||--|| SkillsGapReport : "reveals"
    LaborMarketReport ||--|{ JobOpportunity : "forecasts"
    ApplicationPipeline ||--|{ JobOpportunity : "tracks"
    JobOpportunity ||--|| EmployerProfile : "posted-by"
    JobOpportunity ||--|| MatchScore : "rated-by"
```
...
```mermaid
stateDiagram-v2
    [*] --> MarketAnalyzed : laborMarketAnalyzed
    MarketAnalyzed --> Profiled : candidateProfileBuilt
    Profiled --> Scanned : opportunitiesScanned
    Scanned --> GapAssessed : skillsGapAssessed
    GapAssessed --> Scored : matchScored
    Scored --> EmployerReviewed : employerEvaluated
    EmployerReviewed --> InPipeline : opportunityAdded
    InPipeline --> Applied : applicationSubmitted
    GapAssessed --> Scanned : broaderSearch
    Applied --> [*]
```
