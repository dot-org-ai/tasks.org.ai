...
```mermaid
erDiagram
    Opportunity ||--|| InvestmentThesis : "justified-by"
    ScreeningCriteria ||--o{ Opportunity : "filters"
    Opportunity ||--|| RiskProfile : "assessed-by"
    AssetClass ||--|{ Opportunity : "categorizes"
    Opportunity ||--|| Valuation : "valued-at"
    OpportunityPipeline ||--|{ Opportunity : "ranks"
```
...
```mermaid
stateDiagram-v2
    [*] --> CriteriaDefined : screeningCriteriaDefined
    CriteriaDefined --> Screened : marketScreened
    Screened --> Analyzed : opportunityAnalyzed
    Analyzed --> RiskAssessed : riskAssessed
    RiskAssessed --> ThesisReady : thesisFormulated
    ThesisReady --> Scored : opportunityScored
    Scored --> Pipelined : pipelineUpdated
    Pipelined --> Recommended : allocationRecommended
    Scored --> Screened : rescanNeeded
    Recommended --> [*]
```
