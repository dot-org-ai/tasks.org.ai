...
```mermaid
erDiagram
    Process ||--|{ Bottleneck : "constrained-by"
    Process ||--o{ EfficiencyMetric : "measured-by"
    ImprovementOpportunity ||--|| Bottleneck : "addresses"
    ProcessMap ||--|| Process : "documents"
    BaselineAssessment ||--|| Process : "benchmarks"
    ImprovementOpportunity ||--|| ImprovementPlan : "executed-via"
```
...
```mermaid
stateDiagram-v2
    [*] --> Mapped : processMapped
    Mapped --> Measured : performanceMeasured
    Measured --> BottleneckFound : bottleneckIdentified
    BottleneckFound --> Benchmarked : operationsBenchmarked
    Benchmarked --> Scored : opportunityScored
    Scored --> Projected : savingsEstimated
    Projected --> Planned : improvementPlanCreated
    Planned --> Completed : initiativeCompleted
    Scored --> Measured : insufficientImpact
    Completed --> [*]
```
