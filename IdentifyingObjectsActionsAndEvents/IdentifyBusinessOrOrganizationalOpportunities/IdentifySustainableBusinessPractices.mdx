...
```mermaid
erDiagram
    Practice ||--|| MaterialityAssessment : "prioritized-by"
    Practice ||--|| ESGScorecard : "scored-on"
    SustainabilityFramework ||--|{ Practice : "benchmarks"
    AdoptionRoadmap ||--|{ Practice : "sequences"
    Practice ||--o{ ImpactMetric : "measured-by"
    StakeholderReport ||--|{ Practice : "discloses"
```
...
```mermaid
stateDiagram-v2
    [*] --> Audited : auditCompleted
    Audited --> MaterialityDone : materialityAssessed
    MaterialityDone --> Benchmarked : esgBenchmarked
    Benchmarked --> Flagged : practiceIdentified
    Flagged --> SupplierChecked : supplierEvaluated
    SupplierChecked --> Roadmapped : roadmapBuilt
    Roadmapped --> Scored : scorecardGenerated
    Scored --> Published : reportPublished
    Scored --> Benchmarked : gapsRemain
    Published --> [*]
```
