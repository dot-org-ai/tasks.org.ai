...
```mermaid
erDiagram
    Prospect ||--|| IdealCustomerProfile : "evaluated-against"
    LeadSource ||--|{ Prospect : "sources"
    Prospect ||--|| QualificationScore : "scored-by"
    MarketSegment ||--|{ Prospect : "segments"
    ProspectList ||--|{ Prospect : "curates"
    Prospect ||--o{ IntentSignal : "exhibits"
```
...
```mermaid
stateDiagram-v2
    [*] --> ProfileDefined : profileDefined
    ProfileDefined --> Segmented : marketSegmented
    Segmented --> Sourced : leadsSourced
    Sourced --> Enriched : prospectEnriched
    Enriched --> Scored : prospectScored
    Scored --> Qualified : leadQualified
    Qualified --> Listed : prospectListBuilt
    Listed --> HandedOff : leadHandedOff
    Scored --> Sourced : nurtureCycle
    HandedOff --> [*]
```
