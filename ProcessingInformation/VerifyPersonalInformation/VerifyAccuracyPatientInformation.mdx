...
```mermaid
erDiagram
    PatientRecord ||--|| DemographicData : contains
    PatientRecord ||--|| InsuranceVerification : "verified by"
    PatientRecord ||--|{ MedicationList : includes
    PatientRecord ||--|{ AllergyRecord : documents
    PatientRecord ||--o{ ConsentForm : "authorized by"
    PatientRecord ||--o{ DataDiscrepancy : "may have"
    DataDiscrepancy ||--|| PatientRecord : "corrected in"
```
...
stateDiagram-v2
    [*] --> DataCollected
    DataCollected --> DemographicsVerified : demographicsVerified
    DemographicsVerified --> InsuranceChecked : insuranceEligibilityChecked
    InsuranceChecked --> MedicationsReconciled : medicationsReconciled
    MedicationsReconciled --> AllergiesConfirmed : allergiesConfirmed
    AllergiesConfirmed --> DiscrepancyFound : discrepancyResolved
    AllergiesConfirmed --> Locked : recordLocked
    DiscrepancyFound --> DemographicsVerified : reverify
    Locked --> [*]
```
