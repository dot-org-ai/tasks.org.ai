...
```mermaid
erDiagram
    Batch ||--|{ Material : contains
    Material ||--|| SortCriteria : "classified by"
    Material ||--|| QualityGrade : "rated as"
    Material ||--|| Destination : "routed to"
    Batch ||--|{ TransportOrder : "fulfills"
    TransportOrder ||--|{ Material : "includes"
    QualityGrade ||--|| Destination : determines
```
...
stateDiagram-v2
    [*] --> Unsorted
    Unsorted --> QualitySorted : materialsSortedByQuality
    QualitySorted --> Rejected : qualityFail
    QualitySorted --> DestinationSorted : materialsSortedByDestination
    DestinationSorted --> SizeSorted : materialsSortedBySize
    SizeSorted --> SequenceSorted : materialsSortedBySequence
    SequenceSorted --> Labeled : sortedBatchLabeled
    Labeled --> Staged : materialsStaged
    Staged --> Validated : sortAccuracyValidated
    Validated --> [*]
    Rejected --> [*]
```
