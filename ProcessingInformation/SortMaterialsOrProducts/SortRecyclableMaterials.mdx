...
```mermaid
erDiagram
    InboundLoad ||--|{ SortBatch : "processed in"
    SortBatch ||--|{ MaterialStream : "separated into"
    MaterialStream ||--|| GradeSpecification : "rated against"
    MaterialStream ||--o{ ContaminationReport : "may have"
    MaterialStream ||--|{ Bale : "compressed into"
    SortBatch ||--|| DivertedWeight : "measured as"
    ContaminationReport ||--o{ Bale : "may affect"
```
...
stateDiagram-v2
    [*] --> LoadReceived
    LoadReceived --> Classifying : classifyMaterial
    Classifying --> Separated : streamSeparated
    Separated --> Screening : screenContamination
    Screening --> Graded : materialGraded
    Screening --> Contaminated : contaminationDetected
    Contaminated --> Rejected : rejectMaterial
    Rejected --> Classifying : resort
    Graded --> Baled : baleCreated
    Baled --> Recorded : yieldRecorded
    Recorded --> [*]
```
