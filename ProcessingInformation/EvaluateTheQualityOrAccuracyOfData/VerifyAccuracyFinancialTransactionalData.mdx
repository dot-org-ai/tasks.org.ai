...
```mermaid
erDiagram
    Transaction ||--|| Invoice : "billed by"
    Transaction ||--|| Payment : "settled by"
    Payment ||--|| Settlement : "cleared in"
    Transaction ||--|| Receipt : proves
    Transaction ||--o{ Mismatch : "may have"
    Invoice ||--o{ Mismatch : "may trigger"
```
...
stateDiagram-v2
    [*] --> PendingVerification
    PendingVerification --> PaymentChecked : paymentValidated
    PaymentChecked --> InvoiceMatched : invoiceMatched
    InvoiceMatched --> SettlementReconciled : settlementReconciled
    SettlementReconciled --> Approved : transactionApproved
    PaymentChecked --> MismatchFound : mismatchDetected
    InvoiceMatched --> MismatchFound : mismatchDetected
    MismatchFound --> Correcting : correctTransaction
    Correcting --> PendingVerification : transactionCorrected
    Approved --> [*]
```
...
transactions.invoiceMatched(async ({ invoiceId, paymentId, matchConfidence, amount }) => {
  if (matchConfidence === 1.0 && amount < 500) {
