...
```mermaid
erDiagram
    Transaction ||--|{ LineItem : contains
    Transaction ||--|| Payment : "settled by"
    LineItem ||--o{ Discount : "may have"
    Transaction ||--|| TaxCalculation : "taxed by"
    Transaction ||--o{ Discrepancy : "may produce"
    Payment ||--o{ Discrepancy : "may trigger"
```
...
stateDiagram-v2
    [*] --> PendingReview
    PendingReview --> Reviewing : reviewTransaction
    Reviewing --> PricingChecked : pricingValidated
    PricingChecked --> TaxChecked : taxCalculationChecked
    TaxChecked --> PaymentReconciled : paymentReconciled
    PaymentReconciled --> Approved : transactionApproved
    Reviewing --> DiscrepancyFound : discrepancyFlagged
    PricingChecked --> DiscrepancyFound : discrepancyFlagged
    TaxChecked --> DiscrepancyFound : discrepancyFlagged
    DiscrepancyFound --> Correcting : correctTransaction
    Correcting --> PendingReview : transactionCorrected
    Approved --> [*]
```
...
transactions.transactionReviewed(async ({ transactionId, totalAmount, discrepancies }) => {
  if (totalAmount < 50 && discrepancies.length === 0) {
