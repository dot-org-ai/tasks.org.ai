...
```mermaid
erDiagram
    CreditApplication ||--|| CreditReport : "evaluated with"
    CreditReport ||--|| CreditScore : produces
    CreditApplication ||--|| CreditDecision : "results in"
    CreditDecision ||--|| CreditLimit : establishes
    CreditScore ||--|| CreditLimit : determines
    CreditApplication ||--|| PaymentHistory : references
```
...
stateDiagram-v2
    [*] --> ApplicationReceived
    ApplicationReceived --> IdentityChecked : identityValidated
    IdentityChecked --> ReportPulled : creditReportChecked
    ReportPulled --> ScoreCalculated : creditScoreCalculated
    ScoreCalculated --> Approved : creditDecisionApproved
    ScoreCalculated --> Denied : scoreBelowThreshold
    Approved --> LimitSet : creditLimitSet
    LimitSet --> Monitoring : creditworthinessMonitored
    Monitoring --> Monitoring : creditworthinessMonitored
    Monitoring --> UnderReview : riskChange
    UnderReview --> ScoreCalculated : reEvaluate
    Denied --> [*]
    LimitSet --> [*]
```
