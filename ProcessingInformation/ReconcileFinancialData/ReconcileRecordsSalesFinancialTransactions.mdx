...
```mermaid
erDiagram
    ReconciliationBatch ||--|{ TransactionRecord : contains
    TransactionRecord ||--o| MatchedPair : "paired in"
    MatchedPair ||--o{ Discrepancy : "may reveal"
    Discrepancy ||--|| Adjustment : "resolved by"
    ReconciliationBatch ||--|| ReconciliationReport : "summarized in"
    Adjustment ||--|| ReconciliationReport : "included in"
    TransactionRecord ||--o{ Discrepancy : "may have"
```
...
stateDiagram-v2
    [*] --> Importing
    Importing --> Matching : transactionsImported
    Matching --> Reviewing : transactionsMatched
    Reviewing --> Investigating : discrepancyIdentified
    Reviewing --> Approved : reconciliationApproved
    Investigating --> Adjusting : varianceInvestigated
    Adjusting --> Matching : adjustmentPosted
    Approved --> ReportGenerated : reportGenerated
    ReportGenerated --> [*]
```
