...
| integrateLayers | Merge multiple spatial datasets into a unified compilation |
...
```mermaid
erDiagram
    SpatialDataset ||--|{ FeatureLayer : contains
    FeatureLayer ||--|| CoordinateSystem : "referenced by"
    FieldSurvey ||--|{ FeatureLayer : produces
    FeatureLayer ||--|| AttributeTable : "linked to"
    BaseMap ||--o{ FeatureLayer : underlays
    SpatialDataset ||--|| MapProduct : "rendered as"
```
...
stateDiagram-v2
    [*] --> Acquiring
    Acquiring --> Acquired : dataAcquired
    Acquired --> Reprojecting : reprojectData
    Reprojecting --> Reprojected : dataReprojected
    Reprojected --> Integrating : integrateLayers
    Integrating --> Integrated : layersIntegrated
    Integrated --> Validating : validateTopology
    Validating --> Integrated : topologyFailed
    Validating --> Validated : topologyValidated
    Validated --> Enriched : attributesEnriched
    Enriched --> MapReady : mapProductGenerated
    MapReady --> Published : datasetPublished
    Published --> [*]
```
...
await compiler.integrateLayers({
