...
```mermaid
erDiagram
    RequirementsDocument ||--o{ TechnicalSpecification : "defines"
    TechnicalSpecification ||--|| ScopeBoundary : "bounded by"
    TechnicalSpecification ||--|| QualityStandard : "measured by"
    TechnicalSpecification ||--|| AcceptanceCriteria : "validated by"
    SpecificationAgreement ||--o{ TechnicalSpecification : "captures"
    SpecificationAgreement ||--o{ ChangeRequest : "amended by"
    TradeoffMatrix ||--|| ScopeBoundary : "informs"
    ChangeRequest ||--|| TechnicalSpecification : "modifies"
```
...
```mermaid
stateDiagram-v2
    [*] --> RequirementsCollected : requirementsGathered
    RequirementsCollected --> SpecsDrafted : specificationsDefined
    SpecsDrafted --> ScopeAgreed : scopeNegotiated
    ScopeAgreed --> QualityAgreed : qualityStandardsNegotiated
    QualityAgreed --> ConflictsAddressed : conflictsResolved
    ConflictsAddressed --> CriteriaLocked : acceptanceCriteriaFinalized
    CriteriaLocked --> ScopeAgreed : not all agreed
    CriteriaLocked --> Documented : specificationsDocumented
    Documented --> [*]
```
