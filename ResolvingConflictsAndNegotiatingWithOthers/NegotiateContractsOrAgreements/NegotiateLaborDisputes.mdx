...
```mermaid
erDiagram
    DisputeCase ||--|| GrievanceFiling : "initiated by"
    DisputeCase ||--o{ MediationSession : "negotiated in"
    DisputeCase ||--o{ SettlementProposal : "offered via"
    DisputeCase ||--o| ArbitrationAward : "decided by"
    SettlementProposal ||--o| BackPayCalculation : "includes"
    DisputeCase ||--o{ ComplianceRecord : "monitored via"
    ArbitrationAward ||--o{ ComplianceRecord : "enforced via"
```
...
stateDiagram-v2
    ...
    Assessed --> GrievanceFiled : grievanceFiled
    GrievanceFiled --> InMediation : mediationConducted
    InMediation --> ProposalMade : settlementProposed
    ProposalMade --> Settled : settlementExecuted
    ProposalMade --> Arbitration : arbitrationEscalated
    Arbitration --> Settled : settlementExecuted
    Settled --> Monitoring : complianceMonitored
    Monitoring --> [*]
```
