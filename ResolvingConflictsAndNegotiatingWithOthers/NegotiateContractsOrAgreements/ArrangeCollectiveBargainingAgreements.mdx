...
```mermaid
erDiagram
    BargainingUnit ||--o{ NegotiationProposal : "represented by"
    BargainingUnit ||--o{ BargainingSession : "negotiated in"
    BargainingUnit ||--|| EconomicPackage : "compensated via"
    NegotiationProposal ||--o{ BargainingSession : "discussed in"
    BargainingSession ||--o{ MemorandumOfUnderstanding : "produces"
    ContractDraft ||--|| EconomicPackage : "includes"
    ContractDraft ||--|| GrievanceProcedure : "defines"
    ContractDraft ||--|| RatificationVote : "approved by"
```
...
```mermaid
stateDiagram-v2
    [*] --> ContractReview : contractAnalyzed
    ContractReview --> ProposalsDeveloped : proposalsPrepared
    ProposalsDeveloped --> InSession : sessionConducted
    InSession --> CounterproposalPhase : counterproposalsExchanged
    CounterproposalPhase --> InSession : no agreement
    CounterproposalPhase --> Drafting : agreementDrafted
    Drafting --> Ratification : ratificationSubmitted
    Ratification --> Executed : agreementExecuted
    Ratification --> InSession : not ratified
    Executed --> [*]
```
