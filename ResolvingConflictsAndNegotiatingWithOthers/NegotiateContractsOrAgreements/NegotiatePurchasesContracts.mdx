...
```mermaid
erDiagram
    PurchaseRequest ||--|| RequestForProposal : "formalized as"
    RequestForProposal ||--o{ VendorBid : "responded to by"
    VendorBid ||--|| Evaluation : "scored in"
    Evaluation ||--|| PurchaseOrder : "selects"
    PurchaseOrder ||--|| Contract : "governed by"
    Contract ||--o{ Amendment : "modified by"
    PurchaseRequest ||--|| Contract : "fulfilled by"
```
...
stateDiagram-v2
    ...
    RequirementsDefined --> BidPhase : bidsSolicited
    BidPhase --> Evaluated : bidsEvaluated
    Evaluated --> Negotiating : termsNegotiated
    Negotiating --> Evaluated : terms not acceptable
    Negotiating --> Awarded : contractAwarded
    Awarded --> Executed : contractExecuted
    Executed --> Amended : contractAmended
    Executed --> [*]
    Amended --> Executed
```
