...
```mermaid
erDiagram
    PersonnelCase ||--o{ AttendanceRecord : "references"
    PersonnelCase ||--o{ ConductReport : "documented by"
    PersonnelCase ||--o{ CounselingSession : "addressed in"
    PersonnelCase ||--o| DisciplinaryNotice : "escalated via"
    PersonnelCase ||--o| RemediationPlan : "corrected by"
    PersonnelCase ||--o| SeparationAgreement : "terminated via"
    CounselingSession ||--o| RemediationPlan : "produces"
    DisciplinaryNotice ||--o| RemediationPlan : "triggers"
```
...
```mermaid
stateDiagram-v2
    [*] --> Identified : issueIdentified
    Identified --> Investigated : circumstancesInvestigated
    Investigated --> Counseled : counselingConducted
    Counseled --> Closed : caseClosed
    Counseled --> Disciplined : disciplineApplied
    Disciplined --> Remediation : remediationDeveloped
    Remediation --> Closed : improvement confirmed
    Remediation --> EAPReferred : eapReferralMade
    EAPReferred --> Closed : caseClosed
```
