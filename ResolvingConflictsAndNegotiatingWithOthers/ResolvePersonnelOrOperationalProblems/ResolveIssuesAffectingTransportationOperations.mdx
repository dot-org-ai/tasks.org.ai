...
```mermaid
erDiagram
    DisruptionIncident ||--o| VehicleBreakdown : "caused by"
    DisruptionIncident ||--o| WeatherAlert : "caused by"
    DisruptionIncident ||--o| CapacityShortage : "caused by"
    DisruptionIncident ||--o{ RouteDeviation : "results in"
    DisruptionIncident ||--o{ CorrectiveAction : "addressed by"
    ServiceRecoveryPlan ||--o{ CorrectiveAction : "contains"
    DisruptionIncident ||--|| ServiceRecoveryPlan : "resolved via"
```
...
```mermaid
stateDiagram-v2
    [*] --> NormalOps : operationsMonitored
    NormalOps --> Disrupted : disruptionIdentified
    Disrupted --> Diagnosed : causeDiagnosed
    Diagnosed --> Responding : responseCoordinated
    Responding --> Rerouted : shipmentsRerouted
    Responding --> Recovering : recoveryExecuted
    Rerouted --> Recovering : recoveryExecuted
    Recovering --> Reported : resolutionReported
    Reported --> NormalOps
```
