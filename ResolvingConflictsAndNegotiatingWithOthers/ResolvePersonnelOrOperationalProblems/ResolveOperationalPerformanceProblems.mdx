...
```mermaid
erDiagram
    PerformanceIncident ||--|| RootCauseAnalysis : "investigated by"
    PerformanceIncident ||--|| PerformanceMetric : "measured against"
    RootCauseAnalysis ||--|| ImprovementPlan : "generates"
    ImprovementPlan ||--o{ CorrectiveMeasure : "contains"
    PerformanceIncident ||--|| IncidentReport : "documented in"
    ProcessAudit ||--o{ PerformanceMetric : "validates"
    ImprovementPlan ||--|| ProcessAudit : "verified by"
```
...
stateDiagram-v2
    [*] --> OnTarget
    OnTarget --> DeviationFound : deviationDetected
    DeviationFound --> RootCauseFound : rootCauseAnalyzed
    RootCauseFound --> PlanDeveloped : improvementPlanDeveloped
    PlanDeveloped --> MeasuresApplied : correctiveMeasuresImplemented
    MeasuresApplied --> RecoveryCheck : recoveryVerified
    RecoveryCheck --> RootCauseFound : not resolved
    RecoveryCheck --> Documented : resolutionDocumented
    Documented --> Audited : processAudited
    Audited --> OnTarget
```
