...
```mermaid
erDiagram
    LevelReading ||--o{ MeasurementPoint : "contains"
    MeasurementPoint }o--|| Sensor : "tracks"
    Sensor ||--o{ Threshold : "references"
    Threshold }o--|| CalibrationRecord : "generates"
    CalibrationRecord ||--o{ MeasurementLog : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : levelReadingCaptured
    InProgress --> InProgress : thresholdBreached
    InProgress --> InProgress : sensorConfigured
    InProgress --> Review : instrumentCalibrated
    Review --> InProgress
    InProgress --> InProgress : levelReportGenerated
    InProgress --> InProgress : alertAcknowledged
    InProgress --> Completed : measurementScheduled
    Completed --> [*]
```
## Workflow
