...
```mermaid
erDiagram
    Workpiece ||--o{ Dimension : "contains"
    Dimension }o--|| Tolerance : "tracks"
    Tolerance ||--o{ Specification : "references"
    Specification }o--|| MeasurementRecord : "generates"
    MeasurementRecord ||--o{ CalibrationCertificate : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : specificationRetrieved
    InProgress --> InProgress : dimensionCalculated
    InProgress --> InProgress : toleranceVerified
    InProgress --> Review : measurementRecorded
    Review --> InProgress
    InProgress --> InProgress : outOfToleranceFlagged
    InProgress --> InProgress : dimensionRecalculated
    InProgress --> Completed : dimensionReportGenerated
    Completed --> [*]
```
## Workflow
