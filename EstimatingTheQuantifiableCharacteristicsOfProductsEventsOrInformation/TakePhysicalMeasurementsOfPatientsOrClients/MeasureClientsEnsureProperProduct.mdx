...
```mermaid
erDiagram
    MeasurementRecord ||--o{ ClientProfile : "contains"
    ClientProfile }o--|| SizingChart : "tracks"
    SizingChart ||--o{ FitAssessment : "references"
    FitAssessment }o--|| ProductRecommendation : "generates"
    ProductRecommendation ||--o{ FittingAppointment : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : measurementCaptured
    InProgress --> InProgress : clientProfileCreated
    InProgress --> InProgress : fitAssessed
    InProgress --> Review : productRecommended
    Review --> InProgress
    InProgress --> InProgress : fittingScheduled
    InProgress --> InProgress : accuracyValidated
    InProgress --> Completed : refitRequested
    Completed --> [*]
```
## Workflow
