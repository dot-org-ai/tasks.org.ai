...
```mermaid
erDiagram
    SurveyProject ||--o{ Parcel : "contains"
    Parcel }o--|| BoundaryPoint : "tracks"
    BoundaryPoint ||--o{ Measurement : "references"
    Measurement }o--|| Plat : "generates"
    Plat ||--o{ LegalDescription : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : surveyInitiated
    InProgress --> InProgress : measurementRecorded
    InProgress --> InProgress : boundaryEstablished
    InProgress --> Review : areaCalculated
    Review --> InProgress
    InProgress --> InProgress : platGenerated
    InProgress --> InProgress : surveyCertified
    InProgress --> Completed : discrepancyReconciled
    Completed --> [*]
```
## Workflow
