...
```mermaid
erDiagram
    OperationalBudget ||--o{ CostCategory : "contains"
    CostCategory }o--|| CostDriver : "tracks"
    CostDriver ||--o{ ScenarioModel : "references"
    ScenarioModel }o--|| CostCenter : "generates"
    CostCenter ||--o{ TrendAnalysis : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : operationalBudgetCreated
    InProgress --> InProgress : expensesCategorized
    InProgress --> InProgress : costDriversApplied
    InProgress --> Review : scenarioModeled
    Review --> InProgress
    InProgress --> InProgress : historicalTrendsAnalyzed
    InProgress --> InProgress : projectionCalculated
    InProgress --> Completed : operationalBudgetApproved
    Completed --> [*]
```
## Workflow
