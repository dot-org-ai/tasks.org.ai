...
```mermaid
erDiagram
    WorkBreakdownStructure ||--o{ EffortEstimate : "contains"
    EffortEstimate }o--|| ResourceAssignment : "tracks"
    ResourceAssignment ||--o{ CostProjection : "references"
    CostProjection }o--|| ProjectSchedule : "generates"
    ProjectSchedule ||--o{ EstimationBaseline : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : workBreakdownCreated
    InProgress --> InProgress : effortEstimated
    InProgress --> InProgress : resourcesAssigned
    InProgress --> Review : costProjectionCalculated
    Review --> InProgress
    InProgress --> InProgress : scheduleBuilt
    InProgress --> InProgress : estimateBaselined
    InProgress --> Completed : reEstimationTriggered
    Completed --> [*]
```
## Workflow
