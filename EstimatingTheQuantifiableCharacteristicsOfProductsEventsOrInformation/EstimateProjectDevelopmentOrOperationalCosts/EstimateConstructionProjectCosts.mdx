...
```mermaid
erDiagram
    CostEstimate ||--o{ QuantityTakeoff : "contains"
    QuantityTakeoff }o--|| LineItem : "tracks"
    LineItem ||--o{ UnitCost : "references"
    UnitCost }o--|| BidPackage : "generates"
    BidPackage ||--o{ CostRevision : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : estimateCreated
    InProgress --> InProgress : takeoffCompleted
    InProgress --> InProgress : unitCostsApplied
    InProgress --> Review : quoteIncorporated
    Review --> InProgress
    InProgress --> InProgress : totalsCalculated
    InProgress --> InProgress : estimateReviewed
    InProgress --> Completed : bidPrepared
    Completed --> [*]
```
## Workflow
