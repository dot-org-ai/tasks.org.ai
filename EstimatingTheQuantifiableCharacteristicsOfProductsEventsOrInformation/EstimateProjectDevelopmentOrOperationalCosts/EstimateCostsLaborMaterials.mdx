...
```mermaid
erDiagram
    LaborCostEstimate ||--o{ MaterialCostEstimate : "contains"
    MaterialCostEstimate }o--|| WageRate : "tracks"
    WageRate ||--o{ MaterialQuantity : "references"
    MaterialQuantity }o--|| UnitPrice : "generates"
    UnitPrice ||--o{ CostSummary : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : laborCategoriesDefined
    InProgress --> InProgress : wageRatesApplied
    InProgress --> InProgress : laborHoursCalculated
    InProgress --> Review : materialsQuantified
    Review --> InProgress
    InProgress --> InProgress : materialPricesSourced
    InProgress --> InProgress : costSummaryCompiled
    InProgress --> Completed : conditionsAdjusted
    Completed --> [*]
```
## Workflow
