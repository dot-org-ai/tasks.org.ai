...
```mermaid
erDiagram
    MaintenancePlan ||--o{ AssetRecord : "contains"
    AssetRecord }o--|| ServiceInterval : "tracks"
    ServiceInterval ||--o{ PartsCost : "references"
    PartsCost }o--|| LaborEstimate : "generates"
    LaborEstimate ||--o{ FailureForecast : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : assetsInventoried
    InProgress --> InProgress : conditionAssessed
    InProgress --> InProgress : serviceIntervalsDefined
    InProgress --> Review : partsCostsEstimated
    Review --> InProgress
    InProgress --> InProgress : laborCostsEstimated
    InProgress --> InProgress : failuresForecasted
    InProgress --> Completed : maintenanceBudgetCompiled
    Completed --> [*]
```
## Workflow
