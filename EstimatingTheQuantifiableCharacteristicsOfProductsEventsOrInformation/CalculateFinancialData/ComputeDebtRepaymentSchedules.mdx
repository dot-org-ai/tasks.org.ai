...
```mermaid
erDiagram
    Loan ||--o{ AmortizationSchedule : "contains"
    AmortizationSchedule }o--|| Payment : "tracks"
    Payment ||--o{ InterestRate : "references"
    InterestRate }o--|| PayoffProjection : "generates"
    PayoffProjection ||--o{ Modification : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : scheduleGenerated
    InProgress --> InProgress : paymentCalculated
    InProgress --> InProgress : extraPaymentApplied
    InProgress --> Review : scheduleRecalculated
    Review --> InProgress
    InProgress --> InProgress : payoffProjected
    InProgress --> InProgress : scenariosCompared
    InProgress --> Completed : scheduleExported
    Completed --> [*]
```
## Workflow
