...
```mermaid
erDiagram
    Wager ||--o{ Payout : "contains"
    Payout }o--|| GameSession : "tracks"
    GameSession ||--o{ HouseEdge : "references"
    HouseEdge }o--|| WinLossStatement : "generates"
    WinLossStatement ||--o{ DropBox : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : wagerRecorded
    InProgress --> InProgress : outcomeComputed
    InProgress --> InProgress : payoutCalculated
    InProgress --> Review : dropReconciled
    Review --> InProgress
    InProgress --> InProgress : winLossStatementGenerated
    InProgress --> InProgress : taxableWinReported
    InProgress --> Completed : gameIntegrityAudited
    Completed --> [*]
```
## Workflow
