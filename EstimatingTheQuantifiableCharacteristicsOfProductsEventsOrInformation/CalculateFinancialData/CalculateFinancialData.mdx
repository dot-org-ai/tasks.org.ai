...
```mermaid
erDiagram
    FinancialRecord ||--o{ Calculation : "contains"
    Calculation }o--|| Ledger : "tracks"
    Ledger ||--o{ Report : "references"
    Report }o--|| Adjustment : "generates"
    Adjustment ||--o{ Formula : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : financialDataCollected
    InProgress --> InProgress : calculationExecuted
    InProgress --> InProgress : resultValidated
    InProgress --> Review : calculationAdjusted
    Review --> InProgress
    InProgress --> InProgress : reportGenerated
    InProgress --> InProgress : discrepancyDetected
    InProgress --> Completed : calculationArchived
    Completed --> [*]
```
## Workflow
