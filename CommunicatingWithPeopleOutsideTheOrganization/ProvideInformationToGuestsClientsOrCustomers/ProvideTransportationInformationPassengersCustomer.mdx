...
```mermaid
erDiagram
    Route ||--|| Schedule : "timed by"
    Route ||--|{ Connection : "includes"
    Route ||--|| Fare : "priced at"
    ServiceStatus }o--|| Route : "affects"
    Route ||--o{ Accommodation : "offers"
    Connection }o--|| Schedule : "synchronized via"
```
...
stateDiagram-v2
    [*] --> ScheduleProvided
    ScheduleProvided --> RouteDescribed : routeDescribed
    ScheduleProvided --> FareQuoted : fareQuoted
    RouteDescribed --> ConnectionPlanned : connectionPlanned
    FareQuoted --> AccommodationArranged : accommodationArranged
    ConnectionPlanned --> AccommodationArranged : accommodationArranged
    FareQuoted --> ServiceStatusUpdated : serviceStatusUpdated
    AccommodationArranged --> ServiceStatusUpdated : serviceStatusUpdated
    ServiceStatusUpdated --> AlertIssued : alertIssued
...
graph TD
    ...
    K --> M{Alternative Available?}
    M -->|Yes| B
    M -->|No| N[Wait for Resolution]
```
