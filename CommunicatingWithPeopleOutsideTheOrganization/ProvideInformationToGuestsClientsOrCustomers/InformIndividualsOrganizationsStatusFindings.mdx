...
```mermaid
erDiagram
    StatusUpdate ||--|| Notification : "delivered as"
    Finding ||--|| Notification : "communicated via"
    Notification ||--o| DeliveryReceipt : "confirmed by"
    Notification ||--o| Response : "replied with"
    Notification }o--|| Template : "formatted from"
    DeliveryReceipt }o--|| Response : "precedes"
```
...
stateDiagram-v2
    ...
    NotificationPrepared --> NotificationSent : notificationSent
    NotificationSent --> DeliveryConfirmed : deliveryConfirmed
    NotificationSent --> DeliveryFailed : deliveryFailed
    DeliveryFailed --> NotificationSent : resent
    DeliveryConfirmed --> AcknowledgmentReceived : acknowledgmentReceived
    DeliveryConfirmed --> ResponseReceived : responseReceived
...
graph TD
    ...
    C -->|No| E[resendNotification]
    E --> F{Retry Limit?}
    F -->|No| B
    F -->|Yes| G[Escalate]
    D --> H{Acknowledgment Required?}
    H -->|Yes| I[requestAcknowledgment]
    H -->|No| J[Complete]
    I --> K{Received?}
    K -->|Yes| J
    K -->|No| L[scheduleNotification]
    L --> B
```
