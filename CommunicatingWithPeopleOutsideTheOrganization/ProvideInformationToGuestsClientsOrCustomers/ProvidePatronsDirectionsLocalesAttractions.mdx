...
```mermaid
erDiagram
    Destination ||--|{ Route : "reached via"
    Route ||--|{ Landmark : "passes through"
    Route ||--o| AccessiblePath : "alternative"
    Map ||--|{ Landmark : displays
    Route ||--o{ TransportOption : "serviced by"
    Destination }o--|| Map : "located on"
```
...
stateDiagram-v2
    [*] --> DirectionsProvided
    DirectionsProvided --> AccessibilityHighlighted : accessibilityHighlighted
    DirectionsProvided --> DistanceEstimated : distanceEstimated
    AccessibilityHighlighted --> CustomRouteCreated : customRouteCreated
    DistanceEstimated --> CustomRouteCreated : customRouteCreated
    CustomRouteCreated --> LandmarksIdentified : landmarksIdentified
    CustomRouteCreated --> TransportRecommended : transportRecommended
    LandmarksIdentified --> [*]
...
graph TD
    ...
    H --> J{Route Clear?}
    J -->|Yes| K[Patron Departs]
    J -->|No| L[updateRouteStatus]
    L --> F
    I --> K
```
