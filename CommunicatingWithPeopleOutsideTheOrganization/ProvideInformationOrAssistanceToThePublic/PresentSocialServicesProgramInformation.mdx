...
```mermaid
erDiagram
    ProgramDescription ||--|| EligibilityCriteria : defines
    ResourceDirectory ||--|{ ProgramDescription : catalogs
    EligibilityCriteria ||--o| ApplicationForm : "unlocks"
    ApplicationForm ||--|| EnrollmentSession : "completed in"
    ProgramDescription ||--o{ AccessBarrier : "may have"
    EnrollmentSession }o--|| ResourceDirectory : "navigated via"
```
...
stateDiagram-v2
    [*] --> ProgramExplained
    ProgramExplained --> EligibilityChecked : eligibilityAssessed
    EligibilityChecked --> ApplicationReady : applicationProvided
    EligibilityChecked --> AlternativesFound : resourcesConnected
    ApplicationReady --> Enrolling : enrollmentAssisted
    Enrolling --> FollowingUp : followupScheduled
    AlternativesFound --> FollowingUp : followupScheduled
    FollowingUp --> Documented : outreachDocumented
    Documented --> [*]
```
