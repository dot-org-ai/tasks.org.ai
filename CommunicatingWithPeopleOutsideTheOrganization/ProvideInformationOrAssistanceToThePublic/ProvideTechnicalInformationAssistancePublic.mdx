...
```mermaid
erDiagram
    TechnicalInquiry ||--|| SupportTicket : "tracked as"
    SupportTicket ||--o| Troubleshooting : "resolved via"
    SupportTicket ||--o| Demonstration : "clarified by"
    KnowledgeArticle }o--|| Troubleshooting : "documents"
    TechnicalSpecification ||--|{ KnowledgeArticle : "referenced in"
    SupportTicket }o--|| TechnicalSpecification : "relates to"
```
...
stateDiagram-v2
    [*] --> InquiryReceived
    InquiryReceived --> ConceptTranslated : conceptTranslated
    InquiryReceived --> IssueEscalated : issueEscalated
    ConceptTranslated --> TroubleshootingProvided : troubleshootingProvided
    TroubleshootingProvided --> ResolutionTracked : resolutionTracked
    TroubleshootingProvided --> DemonstrationConducted : demonstrationConducted
    DemonstrationConducted --> DocumentationCreated : documentationCreated
    IssueEscalated --> DocumentationCreated : documentationCreated
    DocumentationCreated --> ResolutionTracked : resolutionTracked
    ResolutionTracked --> [*]
```
