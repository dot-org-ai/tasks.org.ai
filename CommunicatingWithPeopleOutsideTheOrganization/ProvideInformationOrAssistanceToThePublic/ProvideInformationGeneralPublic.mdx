...
```mermaid
erDiagram
    Inquiry ||--|| Response : "answered by"
    Channel ||--|{ Publication : distributes
    Resource ||--o{ Channel : "published on"
    Inquiry }o--|| Channel : "received via"
    Publication ||--|{ EngagementMetric : "measured by"
    Response }o--|| Resource : "references"
```
...
stateDiagram-v2
    [*] --> InquiryReceived
    InquiryReceived --> ResponseProvided : responseProvided
    ResponseProvided --> ChannelUpdated : channelUpdated
    InquiryReceived --> ResourcePublished : resourcePublished
    ResourcePublished --> ResponseProvided : responseProvided
    ChannelUpdated --> EngagementTracked : engagementTracked
    EngagementTracked --> ContentTranslated : contentTranslated
    EngagementTracked --> AccessibilityEnsured : accessibilityEnsured
    ContentTranslated --> AccessibilityEnsured : accessibilityEnsured
    AccessibilityEnsured --> [*]
```
