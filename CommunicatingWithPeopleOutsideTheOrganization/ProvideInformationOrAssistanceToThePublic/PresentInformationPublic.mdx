...
```mermaid
erDiagram
    Event ||--|| Presentation : hosts
    Presentation ||--|{ Content : includes
    Event ||--|{ Question : receives
    Event ||--|{ Feedback : collects
    Presentation ||--o| Recording : "captured as"
    Feedback }o--|| Event : "submitted at"
```
...
stateDiagram-v2
    [*] --> Prepared
    Prepared --> Scheduled : eventScheduled
    Scheduled --> Delivered : presentationDelivered
    Delivered --> QA : questionResponded
    QA --> QA : moreQuestions
    QA --> FeedbackPhase : feedbackCollected
    FeedbackPhase --> Published : recordingPublished
    Published --> FollowUpPlanned : followUpScheduled
    Published --> [*]
    FollowUpPlanned --> Prepared : nextSession
```
