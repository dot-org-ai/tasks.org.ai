...
erDiagram
    ...
    Case ||--|{ Pleading : contains
    Case ||--|{ Evidence : supports
    Case ||--o| Settlement : "may resolve"
    Case ||--o{ Order : "governed by"
    Motion }o--|| Order : "results in"
```
...
stateDiagram-v2
    [*] --> Retained
    Retained --> Analyzed : caseAnalyzed
    Analyzed --> Litigating : pleadingSubmitted
    Analyzed --> Negotiating : settlementNegotiated
    Litigating --> MotionPending : motionFiled
    MotionPending --> InCourt : argumentPresented
    InCourt --> Resolved : orderObtained
    Negotiating --> Resolved : settlementReached
    Negotiating --> Litigating : settlementFailed
    Resolved --> [*]
```
