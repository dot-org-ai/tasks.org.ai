...
```mermaid
erDiagram
    FareStructure ||--o{ FareTransaction : "governs"
    FareTransaction ||--o| FareMedia : "paid via"
    FareTransaction }o--|| CollectionSession : "belongs to"
    CollectionSession ||--|| RevenueReport : "summarized in"
    FareMedia ||--o{ FareTransaction : "used in"
```
...
```mermaid
stateDiagram-v2
    [*] --> Calculated : fareCalculated
    Calculated --> Accepted : paymentAccepted
    Calculated --> Declined : paymentDeclined
    Declined --> Calculated : fareCalculated
    Accepted --> Validated : fareMediaValidated
    Validated --> Transferred : transferIssued
    Validated --> Reconciled : sessionReconciled
    Transferred --> Reconciled : sessionReconciled
    Reconciled --> Reported : revenueReported
    Reported --> [*]
```
