...
```mermaid
erDiagram
    MedicalSupply ||--o{ LotNumber : "identified by"
    MedicalSupply ||--|| ExpirationDate : has
    MedicalEquipment ||--o{ CalibrationRecord : "verified by"
    ControlledSubstance ||--o{ MedicalSupply : "subset of"
    MedicalSupply ||--|| ExpirationDate : expires
    CalibrationRecord }o--|| MedicalEquipment : calibrates
```
...
stateDiagram-v2
    [*] --> Stocked
    Stocked --> Replenished : supplyReplenished
    Stocked --> CriticalLow : criticalStockLow
    CriticalLow --> Replenished : supplyReplenished
    Stocked --> ExpiringWarning : expirationWarning
    ExpiringWarning --> Stocked : supplyReplenished
    Stocked --> Recalled : recallIssued
    Recalled --> Stocked : supplyReplenished
    Stocked --> Calibrated : equipmentCalibrated
    Calibrated --> Stocked : complianceDocumented
```
