...
```mermaid
erDiagram
    Product ||--o{ Allocation : "assigned via"
    Allocation }o--|| Location : "delivered to"
    DemandForecast }o--|| Product : "predicts for"
    Transfer ||--o{ Product : moves
    ServiceLevel }o--|| Location : "measured at"
    DemandForecast ||--o{ Allocation : informs
```
...
stateDiagram-v2
    [*] --> Forecasted
    Forecasted --> Allocated : inventoryAllocated
    Allocated --> TransferPlanned : transferPlanned
    TransferPlanned --> Transferred : transferCompleted
    Transferred --> Measured : serviceLevelMeasured
    Allocated --> Measured : serviceLevelMeasured
    Measured --> Balanced : resourcesBalanced
    Balanced --> Optimized : networkOptimized
    Optimized --> Forecasted : demandForecasted
    Measured --> Forecasted : demandForecasted
```
