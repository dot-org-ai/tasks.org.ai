...
```mermaid
erDiagram
    FoodItem ||--|| StockLevel : "tracked by"
    BeverageItem ||--|| StockLevel : "tracked by"
    Equipment ||--o{ StockLevel : "tracked by"
    FoodItem ||--|| ExpirationDate : has
    StockLevel ||--|| ReorderPoint : "compared to"
    BeverageItem ||--|| ExpirationDate : has
```
...
stateDiagram-v2
    [*] --> InStock
    InStock --> Updated : stockUpdated
    Updated --> Reordering : reorderTriggered
    Reordering --> InStock : deliveryReceived
    InStock --> ExpiringNotice : expirationApproaching
    ExpiringNotice --> Rotated : stockRotated
    Rotated --> InStock : stockUpdated
    ExpiringNotice --> Retired : itemRetired
    Retired --> [*]
```
