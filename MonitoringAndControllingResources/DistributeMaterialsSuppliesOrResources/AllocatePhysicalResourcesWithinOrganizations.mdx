...
```mermaid
erDiagram
    ResourceRequest ||--|| AllocationRecord : "fulfilled by"
    PhysicalAsset ||--o{ AllocationRecord : "assigned via"
    PhysicalAsset ||--o{ UtilizationReport : "tracked in"
    AllocationRecord ||--o| ReallocationOrder : "changed by"
    PhysicalAsset }o--|| AssetInventory : "cataloged in"
    ReallocationOrder }o--|| AllocationRecord : "creates new"
```
...
```mermaid
stateDiagram-v2
    [*] --> Requested : requestSubmitted
    Requested --> Assessed : availabilityAssessed
    Assessed --> Prioritized : requestsPrioritized
    Prioritized --> Allocated : resourceAllocated
    Allocated --> Tracked : utilizationTracked
    Tracked --> Reallocated : resourceReallocated
    Reallocated --> Tracked : utilizationTracked
    Tracked --> Retired : assetRetired
    Retired --> [*]
```
