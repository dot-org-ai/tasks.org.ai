...
```mermaid
erDiagram
    EducationalMaterial ||--o{ Distribution : "delivered via"
    EducationalMaterial }o--|| DistributionChannel : "available through"
    CommunityRequest ||--|| Distribution : "fulfilled by"
    Distribution ||--|| ImpactMetric : "measured by"
    DistributionChannel ||--o{ AccessPoint : "provides"
    EducationalMaterial ||--o{ ImpactMetric : "assessed by"
```
...
stateDiagram-v2
    ...
    Developed --> Curated : collectionCurated
    Curated --> ChannelReady : distributionChannelEstablished
    ChannelReady --> Distributed : materialsDistributed
    ChannelReady --> RequestReceived : materialsRequested
    RequestReceived --> Distributed : materialsDistributed
    Distributed --> Tracked : distributionTracked
    Tracked --> Measured : impactMeasured
    Measured --> Developed : materialDeveloped
    Measured --> [*]
```
