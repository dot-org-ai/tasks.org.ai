...
```mermaid
erDiagram
    LibraryMaterial ||--o{ Checkout : "loaned via"
    LibraryMaterial ||--o{ Reservation : "held by"
    LibraryMaterial ||--o| CourseReserve : "designated as"
    Checkout }o--|| CirculationPolicy : "governed by"
    Reservation ||--|| Checkout : "fulfilled as"
    LibraryMaterial ||--o| DeliveryRequest : "shipped via"
```
...
stateDiagram-v2
    [*] --> Available
    Available --> CheckedOut : materialCheckedOut
    Available --> Reserved : reservationPlaced
    CheckedOut --> Renewed : checkoutRenewed
    Renewed --> CheckedOut : materialCheckedOut
    CheckedOut --> Returned : materialReturned
    Returned --> Available
    Reserved --> FulfillReady : reservationFulfilled
    FulfillReady --> CheckedOut : materialCheckedOut
    Available --> Delivered : materialDelivered
    Delivered --> [*]
```
