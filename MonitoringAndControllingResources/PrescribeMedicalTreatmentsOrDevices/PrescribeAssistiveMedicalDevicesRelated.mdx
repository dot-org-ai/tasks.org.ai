...
```mermaid
erDiagram
    DevicePrescription ||--|| Device : specifies
    DevicePrescription ||--o{ Authorization : requires
    DevicePrescription ||--o{ Fitting : schedules
    Device ||--o{ Adjustment : receives
    Fitting ||--o{ Training : includes
    Authorization }o--|| DevicePrescription : approves
```
...
stateDiagram-v2
    [*] --> Assessed
    Assessed --> DeviceSelected : deviceSelected
    DeviceSelected --> Prescribed : devicePrescribed
    Prescribed --> AwaitingAuth : authorizationRequested
    AwaitingAuth --> Authorized : authorizationApproved
    AwaitingAuth --> Denied : authorizationDenied
    Denied --> Prescribed : devicePrescribed
    Authorized --> FittingScheduled : fittingScheduled
    FittingScheduled --> Delivered : deviceDelivered
    Delivered --> [*]
```
