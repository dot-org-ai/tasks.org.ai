...
```mermaid
erDiagram
    SupplyOrder ||--o{ MedicalItem : contains
    SupplyOrder }o--|| Contract : "priced by"
    MedicalItem ||--o{ LotNumber : "tracked by"
    Inventory ||--o{ MedicalItem : stocks
    Inventory ||--o{ ReorderPoint : "triggers at"
    ReorderPoint }o--|| SupplyOrder : generates
```
...
stateDiagram-v2
    [*] --> Monitoring
    Monitoring --> BelowReorder : inventoryChecked
    BelowReorder --> OrderCreated : orderCreated
    OrderCreated --> ContractVerified : contractVerified
    ContractVerified --> Ordered : purchaseOrderPlaced
    Ordered --> InTransit : shipmentTracked
    InTransit --> Received : suppliesReceived
    Received --> Monitoring : inventoryUpdated
    Monitoring --> Monitoring : inventoryChecked
```
