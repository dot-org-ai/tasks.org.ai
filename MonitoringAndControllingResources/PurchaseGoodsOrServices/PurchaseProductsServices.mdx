...
```mermaid
erDiagram
    PurchaseRequest ||--|| PurchaseOrder : "converts to"
    PurchaseOrder }o--|| Contract : "governed by"
    PurchaseOrder ||--o{ Invoice : "billed via"
    Product }o--|| PurchaseOrder : "ordered in"
    Service }o--|| PurchaseOrder : "ordered in"
    PurchaseRequest }o--|| Contract : references
```
...
stateDiagram-v2
    [*] --> Requested
    Requested --> VendorSelected : vendorSelected
    VendorSelected --> Negotiated : priceNegotiated
    Negotiated --> Approved : purchaseApproved
    Negotiated --> Requested : requestCreated
    Approved --> Ordered : orderPlaced
    Ordered --> Tracked : orderTracked
    Tracked --> Invoiced : invoiceProcessed
    Invoiced --> [*]
```
