...
```mermaid
erDiagram
    MaterialOrder ||--o{ Delivery : schedules
    MaterialOrder }o--|| JobSite : "delivered to"
    MaterialOrder ||--o{ Specification : requires
    Equipment ||--o{ Rental : "leased via"
    Rental }o--|| JobSite : "deployed at"
    Delivery }o--|| MaterialOrder : fulfills
```
...
stateDiagram-v2
    [*] --> Estimated
    Estimated --> Ordered : materialsOrdered
    Ordered --> Scheduled : deliveryScheduled
    Scheduled --> InTransit : shipmentTracked
    InTransit --> Delivered : qualityVerified
    Delivered --> Rejected : qualityVerified
    Rejected --> Ordered : materialsOrdered
    Delivered --> [*]
```
