...
erDiagram
    ...
    PurchaseOrder }o--|| VendorQuote : "based on"
    PurchaseOrder ||--o{ Receipt : "fulfilled by"
    Receipt ||--|| Invoice : "triggers"
    Invoice ||--|| PaymentTerms : "governed by"
    VendorQuote }o--|| Requisition : "responds to"
```
...
stateDiagram-v2
    [*] --> Requisitioned
    Requisitioned --> QuotesReceived : quotesReceived
    QuotesReceived --> Ordered : purchaseOrderIssued
    Ordered --> Shipped : shipmentDispatched
    Shipped --> Received : goodsReceived
    Received --> Discrepancy : discrepancyDetected
    Discrepancy --> Ordered : purchaseOrderIssued
    Received --> Paid : invoiceProcessed
    Paid --> [*]
```
