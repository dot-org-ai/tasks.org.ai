...
erDiagram
    RFP ||--o{ Bid : receives
    Bid }o--|| RFP : "responds to"
    Contract }o--|| Bid : "awarded from"
    CapitalRequest ||--|| RFP : initiates
    TCO }o--|| Bid : "calculated for"
    SpendAnalysis ||--o{ Contract : tracks
```
...
stateDiagram-v2
    [*] --> RFPIssued
    RFPIssued --> BidsReceived : bidsReceived
    BidsReceived --> Evaluated : bidsEvaluated
    Evaluated --> Negotiating : termsNegotiated
    Negotiating --> Approved : contractApproved
    Negotiating --> Evaluated : bidsEvaluated
    Approved --> Purchased : purchaseIssued
    Purchased --> Tracking : performanceTracked
    Tracking --> [*]
```
