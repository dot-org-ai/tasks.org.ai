...
```mermaid
erDiagram
    SecurityPolicy ||--|{ AccessControl : "defines"
    Vulnerability ||--o{ Patch : "remediated by"
    Firewall ||--|{ AccessControl : "enforces"
    EncryptionKey ||--|| SecurityPolicy : "governed by"
    Incident ||--o{ AuditLog : "recorded in"
    Vulnerability }o--|| AuditLog : "logged in"
    Patch }o--|| Vulnerability : "addresses"
```
...
stateDiagram-v2
    [*] --> Assessing
    Assessing --> VulnFound : vulnerabilityDetected
    VulnFound --> ControlsDeployed : firewallDeployed
    ControlsDeployed --> AccessEnforced : accessControlEnforced
    AccessEnforced --> DataProtected : dataEncrypted
    DataProtected --> Monitoring : configureMonitoring
    Monitoring --> IncidentResponse : incidentDetected
    Monitoring --> Auditing : auditCompleted
    IncidentResponse --> Remediated : incidentResolved
    Remediated --> Patching : patchApplied
    Patching --> Assessing
    Auditing --> Assessing
```
