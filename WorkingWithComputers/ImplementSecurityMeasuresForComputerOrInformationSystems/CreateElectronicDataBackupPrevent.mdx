...
```mermaid
erDiagram
    BackupJob ||--|{ BackupSet : "produces"
    BackupSet ||--|{ RecoveryPoint : "creates"
    RetentionPolicy ||--|| BackupSet : "governs"
    StorageTarget ||--|{ BackupSet : "holds"
    RestoreRequest ||--|| RecoveryPoint : "restores from"
    BackupReport }o--|| BackupJob : "summarizes"
```
...
stateDiagram-v2
    [*] --> Scheduled
    Scheduled --> Running : backupStarted
    Running --> Completed : backupCompleted
    Running --> Failed : backupFailed
    Failed --> Running : retried
    Completed --> Verified : backupVerified
    Verified --> Archived : rotateStorage
    Archived --> Expired : retentionExpired
    Expired --> [*]
    Verified --> Restoring : restoreRequested
    Restoring --> Restored : dataRestored
    Restored --> [*]
```
