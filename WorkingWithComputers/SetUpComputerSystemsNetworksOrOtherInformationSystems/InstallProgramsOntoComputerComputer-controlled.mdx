...
```mermaid
erDiagram
    FirmwarePackage ||--|| ControlProgram : "contains"
    ControlProgram ||--|| EquipmentController : "installed on"
    InstallationManifest ||--|{ FirmwarePackage : "lists"
    InstallationManifest ||--|| EquipmentController : "targets"
    ValidationReport }o--|| ControlProgram : "verifies"
    RollbackImage }o--|| FirmwarePackage : "preserves prior"
    DeploymentLog ||--|{ FirmwarePackage : "records"
    DeploymentLog ||--|| EquipmentController : "tracks"
```
...
stateDiagram-v2
    [*] --> Prepared
    Prepared --> Transferred : programTransferred
    Transferred --> Activated : programActivated
    Activated --> Validated : installationValidated
    Activated --> Failed : installationFailed
    Failed --> RolledBack : programRolledBack
    RolledBack --> Prepared : diagnoseAndRetry
    Validated --> Documented : documentInstallation
    Documented --> InService : operational
    InService --> Updated : firmwareUpdated
    Updated --> Validated : revalidate
    InService --> [*]
```
