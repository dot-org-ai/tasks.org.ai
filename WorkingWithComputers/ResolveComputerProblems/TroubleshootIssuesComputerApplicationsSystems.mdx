...
```mermaid
erDiagram
    SupportTicket ||--|{ ErrorLog : "references"
    SupportTicket ||--o| StackTrace : "contains"
    SupportTicket ||--o| KnowledgeArticle : "resolved by"
    SupportTicket ||--o| Hotfix : "fixed with"
    PerformanceMetric }o--|| SupportTicket : "informs"
    ConfigurationSnapshot ||--|| SupportTicket : "captured for"
    KnowledgeArticle }o--|| Hotfix : "documents"
```
...
stateDiagram-v2
    [*] --> TicketCreated
    TicketCreated --> LogsAnalyzed : logsAnalyzed
    LogsAnalyzed --> Reproduced : issueReproduced
    LogsAnalyzed --> HotfixApplied : hotfixApplied
    Reproduced --> HotfixApplied : hotfixApplied
    HotfixApplied --> Resolved : issueResolved
    HotfixApplied --> RolledBack : changeRolledBack
    RolledBack --> Escalated : escalateToVendor
    Escalated --> HotfixApplied : vendorPatch
    Resolved --> Documented : publishKnowledgeArticle
    Documented --> [*]
```
