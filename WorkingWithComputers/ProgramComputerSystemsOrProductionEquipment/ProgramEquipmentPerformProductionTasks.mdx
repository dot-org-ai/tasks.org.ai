...
```mermaid
erDiagram
    MachineProgram ||--|{ Toolpath : "contains"
    MachineProgram ||--|{ ProgramVersion : "tracked as"
    Toolpath ||--|| PostProcessor : "converted by"
    Simulation ||--|| MachineProgram : "verifies"
    ProductionRun }o--|| MachineProgram : "executes"
    ProductionRun ||--|| FixtureSetup : "requires"
    ProgramVersion }o--|| MachineProgram : "revisions of"
```
...
stateDiagram-v2
    [*] --> Authored
    Authored --> Simulating : simulationCompleted
    Simulating --> CollisionFound : collisionDetected
    CollisionFound --> Authored : programRevised
    Simulating --> PostProcessed : postProcessed
    PostProcessed --> Uploaded : programUploaded
    Uploaded --> FirstArticle : firstArticleApproved
    FirstArticle --> Archived : programArchived
    FirstArticle --> Authored : programRevised
    Archived --> [*]
```
