...
```mermaid
erDiagram
    RobotProgram ||--|{ MotionPath : "defines"
    MotionPath ||--|{ Waypoint : "composed of"
    RobotProgram ||--|| EndEffector : "operates"
    SafetyZone }o--|| MotionPath : "constrains"
    DigitalTwin ||--|| RobotProgram : "simulates"
    IOMapping }o--|| EndEffector : "controls"
    IOMapping }o--|| RobotProgram : "configures"
```
...
stateDiagram-v2
    [*] --> ProgramCreated
    ProgramCreated --> WaypointsTaught : waypointTaught
    WaypointsTaught --> WaypointsTaught : additionalWaypoints
    WaypointsTaught --> Simulating : simulateMotion
    Simulating --> CollisionRisk : collisionRiskDetected
    CollisionRisk --> WaypointsTaught : adjustWaypoints
    Simulating --> SimulationPassed : simulationPassed
    SimulationPassed --> Calibrated : robotCalibrated
    Calibrated --> Deployed : programDeployed
    Deployed --> SafetyValidated : safetyValidated
    SafetyValidated --> [*]
```
