...
```mermaid
erDiagram
    ContentDraft ||--|| WebPage : "publishes to"
    ContentDraft ||--|{ MediaAsset : "includes"
    ContentTemplate ||--|| WebPage : "structures"
    PublishSchedule ||--|| ContentDraft : "times"
    SEOMetadata ||--|| WebPage : "optimizes"
    Redirect }o--|| WebPage : "points from"
```
...
stateDiagram-v2
    [*] --> Drafted
    Drafted --> MediaAttached : mediaUploaded
    MediaAttached --> InReview : contentSubmitted
    InReview --> Approved : contentApproved
    InReview --> Drafted : revisionsNeeded
    Approved --> Published : pagePublished
    Approved --> Scheduled : pageScheduled
    Scheduled --> Published : publishDateReached
    Published --> Archived : pageArchived
    Archived --> [*]
```
