...
```mermaid
erDiagram
    ImageBatch ||--|{ DigitalImage : "contains"
    DigitalImage ||--|| ImageMetadata : "has"
    ImageProfile ||--|| DigitalImage : "optimizes"
    DigitalImage ||--|{ ThumbnailSet : "generates"
    MediaLibrary ||--|{ DigitalImage : "stores"
    PublishTarget }o--|| DigitalImage : "displays"
```
...
stateDiagram-v2
    [*] --> Imported
    Imported --> Processed : imageProcessed
    Processed --> Optimized : imageOptimized
    Optimized --> Thumbnailed : thumbnailsGenerated
    Thumbnailed --> Tagged : imageTagged
    Tagged --> Published : imagePublished
    Tagged --> Processed : needsEditing
    Published --> [*]
```
