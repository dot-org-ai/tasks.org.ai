...
```mermaid
erDiagram
    DocumentTemplate ||--|{ FormattedDocument : "applied to"
    StyleGuide ||--|| DocumentTemplate : "defines standards for"
    FormattedDocument ||--|{ DataTable : "contains"
    FormattedDocument ||--|{ ChartGraphic : "contains"
    OutputProfile ||--|| FormattedDocument : "specifies output for"
    AccessibilityReport }o--|| FormattedDocument : "evaluates"
```
...
stateDiagram-v2
    [*] --> ContentReceived
    ContentReceived --> TemplateApplied : templateApplied
    TemplateApplied --> Styled : stylesStandardized
    Styled --> TablesFormatted : tableFormatted
    TablesFormatted --> ChartsCreated : chartCreated
    ChartsCreated --> AccessChecked : accessibilityChecked
    AccessChecked --> ChartsCreated : formattingErrorDetected
    AccessChecked --> OutputReady : outputGenerated
    OutputReady --> [*]
```
