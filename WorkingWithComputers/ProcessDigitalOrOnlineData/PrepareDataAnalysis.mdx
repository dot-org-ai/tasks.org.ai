...
```mermaid
erDiagram
    RawDataset ||--|{ TransformationRule : "processed by"
    DataPipeline ||--|{ TransformationRule : "orchestrates"
    DataPipeline ||--|{ DataQualityCheck : "validates with"
    StagingTable ||--|| RawDataset : "holds"
    AnalyticsDataset }o--|| DataPipeline : "produced by"
    DataLineageRecord }o--|| AnalyticsDataset : "traces"
```
...
stateDiagram-v2
    [*] --> Extracted
    Extracted --> Cleansed : dataCleansed
    Cleansed --> Transformed : dataTransformed
    Transformed --> QualityCheck : qualityCheckPassed
    Transformed --> QualityFailed : qualityCheckFailed
    QualityFailed --> Cleansed : fixDataIssues
    QualityCheck --> Loaded : dataLoaded
    Loaded --> Traced : lineageTraced
    Traced --> Complete : pipelineCompleted
    Complete --> [*]
```
