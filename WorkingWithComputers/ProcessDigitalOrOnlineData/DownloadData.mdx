...
```mermaid
erDiagram
    DownloadRequest ||--|| TransferJob : "initiates"
    TransferJob ||--|| DataFile : "produces"
    DataFile ||--|| Checksum : "verified by"
    DownloadSchedule ||--|{ TransferJob : "triggers"
    StorageQuota ||--|{ DataFile : "constrains"
    AccessCredential ||--|| DownloadRequest : "authenticates"
```
...
stateDiagram-v2
    [*] --> Requested
    Requested --> InProgress : downloadStarted
    InProgress --> Completed : downloadCompleted
    InProgress --> Failed : downloadFailed
    Failed --> InProgress : resumeDownload
    Completed --> Verified : integrityVerified
    Completed --> VerifyFailed : integrityFailed
    VerifyFailed --> Requested : redownload
    Verified --> Cataloged : catalogDownload
    Cataloged --> [*]
```
