...
```mermaid
erDiagram
    ImagingDevice ||--|| CalibrationProfile : "configured with"
    ImagingDevice ||--|{ CaptureSession : "hosts"
    CaptureSession ||--|{ CapturedImage : "produces"
    OutputSpecification ||--|| CapturedImage : "defines requirements for"
    DeviceLog ||--|{ CaptureSession : "records"
    MaintenanceSchedule ||--|| ImagingDevice : "governs"
```
...
stateDiagram-v2
    [*] --> Configured
    Configured --> Calibrated : equipmentCalibrated
    Calibrated --> Capturing : imageCaptured
    Capturing --> Reviewed : captureReviewed
    Reviewed --> Capturing : recapture
    Reviewed --> Exported : outputExported
    Exported --> MaintenanceCheck : maintenanceDue
    MaintenanceCheck --> Configured
    Exported --> [*]
```
