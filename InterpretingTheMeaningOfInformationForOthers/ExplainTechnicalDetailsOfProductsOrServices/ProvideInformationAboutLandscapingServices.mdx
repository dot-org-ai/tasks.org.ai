...
```mermaid
erDiagram
    SiteAssessment ||--o{ DesignConcept : generates
    SiteAssessment ||--|| CostEstimate : "priced in"
    DesignConcept ||--|| PlantSchedule : specifies
    LandscapeProposal ||--|| CostEstimate : includes
    LandscapeProposal ||--o| MaintenancePlan : "accompanied by"
    LandscapeProposal }o--|| SiteAssessment : "based on"
    ServiceCatalog }o--|{ LandscapeProposal : "referenced by"
```
...
```mermaid
stateDiagram-v2
    [*] --> SiteAssessed : siteAssessed
    SiteAssessed --> OptionsPresented : serviceOptionsPresented
    OptionsPresented --> DesignShown : designConceptShown
    DesignShown --> CostsCalculated : costsEstimated
    CostsCalculated --> MaintenanceExplained : maintenancePlanExplained
    MaintenanceExplained --> ProposalReady : proposalPrepared
    ProposalReady --> ProposalDelivered : proposalDelivered
    ProposalDelivered --> QuestionsHandled : costQuestionsAnswered
    QuestionsHandled --> ProposalDelivered : additionalQuestions
    QuestionsHandled --> [*]
```
