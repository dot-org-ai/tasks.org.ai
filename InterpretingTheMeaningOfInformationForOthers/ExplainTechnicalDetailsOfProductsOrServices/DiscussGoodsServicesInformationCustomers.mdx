...
```mermaid
erDiagram
    ProductInquiry ||--o| ServiceDescription : "may include"
    ProductInquiry ||--o| PriceQuote : "results in"
    ProductInquiry ||--|| AvailabilityCheck : triggers
    CustomerPreference }o--|| ProductInquiry : "derived from"
    ComparisonSheet }o--|{ PriceQuote : compares
    FollowUpNote }o--|| ProductInquiry : "created from"
    PriceQuote }o--|| AvailabilityCheck : "depends on"
```
...
```mermaid
stateDiagram-v2
    [*] --> Greeted : customerGreeted
    Greeted --> ProductShared : productDescribed
    ProductShared --> ServiceShared : serviceExplained
    ServiceShared --> StockChecked : availabilityChecked
    StockChecked --> Quoted : quoteProvided
    Quoted --> Compared : optionsCompared
    Compared --> PreferencesRecorded : preferencesDocumented
    Compared --> FollowUpSet : followUpScheduled
    FollowUpSet --> ProductShared : returnVisit
    PreferencesRecorded --> [*]
```
