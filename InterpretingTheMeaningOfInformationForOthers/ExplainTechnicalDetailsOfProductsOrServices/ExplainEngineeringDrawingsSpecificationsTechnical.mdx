...
```mermaid
erDiagram
    EngineeringDrawing ||--|{ Specification : "governed by"
    EngineeringDrawing ||--o{ RequestForInformation : generates
    EngineeringDrawing ||--o{ DrawingMarkup : "annotated with"
    EngineeringDrawing ||--o{ DrawingRevision : "updated as"
    EngineeringDrawing ||--|| BillOfMaterials : "quantified in"
    RequestForInformation ||--|| TechnicalClarification : "answered by"
    TechnicalClarification }o--o| DrawingRevision : "may trigger"
```
...
```mermaid
stateDiagram-v2
    [*] --> DrawingsReviewed : drawingsReviewed
    DrawingsReviewed --> WalkthroughDone : walkthroughConducted
    WalkthroughDone --> DimensionsClarified : dimensionsInterpreted
    DimensionsClarified --> MaterialsExplained : materialsExplained
    MaterialsExplained --> RFIResolution : rfiResponded
    RFIResolution --> MarkedUp : drawingMarkedUp
    MarkedUp --> Revised : revisionIssued
    Revised --> WalkthroughDone : reWalkthrough
    MarkedUp --> Confirmed : understandingConfirmed
    MaterialsExplained --> Confirmed : noRFIs
    Confirmed --> [*]
```
