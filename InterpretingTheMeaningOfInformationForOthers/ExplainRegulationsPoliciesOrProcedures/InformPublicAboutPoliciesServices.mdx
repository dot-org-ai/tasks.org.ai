...
```mermaid
erDiagram
    PolicyAnnouncement ||--o{ ServiceDescription : "introduces"
    PolicyAnnouncement ||--o| ProceduralGuide : "accompanied by"
    ServiceDescription ||--o{ FAQ : generates
    ProceduralGuide ||--o{ FAQ : generates
    OutreachEvent ||--o{ InformationBrochure : distributes
    WebContent }o--|| PolicyAnnouncement : publishes
    WebContent }o--|| ServiceDescription : hosts
```
...
```mermaid
stateDiagram-v2
    [*] --> MessagingCreated : messagingDeveloped
    MessagingCreated --> Published : announcementPublished
    Published --> WebUpdated : webContentUpdated
    WebUpdated --> MaterialsOut : materialsDistributed
    MaterialsOut --> OutreachDone : outreachConducted
    OutreachDone --> InquiriesHandled : inquiriesAnswered
    InquiriesHandled --> Translated : materialsTranslated
    Translated --> ReachMeasured : reachMeasured
    ReachMeasured --> OutreachDone : insufficientReach
    ReachMeasured --> [*]
```
