...
```mermaid
erDiagram
    RegulationSummary ||--o{ PolicyExplanation : "broken into"
    PolicyExplanation ||--o{ ProcedureWalkthrough : "demonstrated by"
    RegulationSummary ||--o{ ComplianceExample : illustrates
    ProcedureWalkthrough ||--o| ExceptionGuidance : "may include"
    ComplianceExample }o--|| QARecord : "referenced in"
    RegulationSummary ||--|| ComplianceTimeline : "scheduled via"
    QARecord }o--|| ProcedureWalkthrough : clarifies
```
...
```mermaid
stateDiagram-v2
    [*] --> Summarized : regulationSummarized
    Summarized --> Contextualized : contextProvided
    Contextualized --> ExamplesProvided : complianceExamplesProvided
    ExamplesProvided --> WalkedThrough : procedureWalkedThrough
    WalkedThrough --> ExceptionsHandled : exceptionsAddressed
    ExceptionsHandled --> QAComplete : qaSessionConducted
    QAComplete --> Documented : understandingDocumented
    QAComplete --> FollowUpSet : followUpScheduled
    FollowUpSet --> Contextualized : additionalExplanation
    Documented --> [*]
```
