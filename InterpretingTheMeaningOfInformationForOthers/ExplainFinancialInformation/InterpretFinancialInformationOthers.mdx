...
```mermaid
erDiagram
    FinancialStatement ||--o{ AnalystInterpretation : "interpreted by"
    FinancialStatement ||--o{ VarianceAnalysis : generates
    FinancialStatement ||--o{ TrendReport : "feeds into"
    AnalystInterpretation ||--o| ExecutiveSummary : "summarized in"
    EconomicIndicator }o--|| AnalystInterpretation : contextualizes
    FinancialDashboard }o--|{ FinancialStatement : visualizes
    VarianceAnalysis }o--|| ExecutiveSummary : "included in"
```
...
```mermaid
stateDiagram-v2
    [*] --> DataCollection : dataAnalyzed
    DataCollection --> TrendAnalysis : trendsIdentified
    TrendAnalysis --> VarianceReview : variancesExplained
    VarianceReview --> MarketContext : marketContextProvided
    MarketContext --> InterpretationDraft : interpretationPrepared
    InterpretationDraft --> DashboardCreation : dashboardGenerated
    DashboardCreation --> Presentation : presentationDelivered
    Presentation --> QA : financialQuestionsAnswered
    QA --> Presentation : additionalClarification
    QA --> Complete : allQuestionsResolved
    Complete --> [*]
    Complete --> DataCollection : nextPeriod
```
