...
```mermaid
erDiagram
    AccountStatement ||--|{ TransactionDetail : contains
    AccountStatement ||--o{ FeeSchedule : references
    LoanDisclosure ||--|| PaymentSchedule : includes
    LoanDisclosure ||--|| InterestCalculation : details
    ProductComparison }o--|{ FeeSchedule : compares
    ProductComparison }o--|{ LoanDisclosure : compares
    AccountStatement }o--|| ProductComparison : "may trigger"
```
...
```mermaid
stateDiagram-v2
    [*] --> StatementReview : statementReviewed
    StatementReview --> FeeExplanation : feeStructureClarified
    FeeExplanation --> ProductExplanation : productFeaturesDescribed
    ProductExplanation --> LoanReview : loanTermsExplained
    LoanReview --> DisclosureDelivery : disclosureProvided
    DisclosureDelivery --> QuestionHandling : customerHasQuestions
    DisclosureDelivery --> Confirmation : noQuestions
    QuestionHandling --> TransactionResolved : transactionQuestionResolved
    TransactionResolved --> ProductComparison : productsCompared
    ProductComparison --> QuestionHandling : additionalQuestions
    ProductComparison --> Confirmation : understandingConfirmed
    Confirmation --> [*]
```
