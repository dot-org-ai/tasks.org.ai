...
```mermaid
erDiagram
    LabResult ||--o| PatientSummary : "summarized in"
    ImagingReport ||--o| PatientSummary : "summarized in"
    MedicationProfile ||--o| PatientSummary : "summarized in"
    AnatomicalDiagram }o--|| VisualExplanation : "type of"
    MedicalTermGlossary }o--|| PatientSummary : supports
    VisualExplanation }o--|| LabResult : illustrates
    VisualExplanation }o--|| ImagingReport : illustrates
```
...
```mermaid
stateDiagram-v2
    [*] --> TerminologySimplified : terminologySimplified
    TerminologySimplified --> LabsInterpreted : labResultsInterpreted
    LabsInterpreted --> ImagingExplained : imagingFindingsExplained
    ImagingExplained --> MedsDescribed : medicationMechanismsDescribed
    MedsDescribed --> VisualsUsed : visualAidsUsed
    VisualsUsed --> ComprehensionCheck : comprehensionAssessed
    ComprehensionCheck --> QuestionsPhase : lowComprehension
    QuestionsPhase --> VisualsUsed : technicalQuestionsAnswered
    ComprehensionCheck --> SummaryProvided : writtenSummaryProvided
    SummaryProvided --> [*]
```
