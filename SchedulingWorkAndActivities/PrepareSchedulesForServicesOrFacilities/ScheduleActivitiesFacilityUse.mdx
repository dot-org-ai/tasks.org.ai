...
```mermaid
erDiagram
    Activity ||--o{ Booking : requires
    FacilitySpace ||--o{ Booking : hosts
    Activity ||--|| RecurrencePattern : follows
    Activity ||--o{ Registration : enrolls
    Booking }o--|| ActivityCalendar : appears_in
    ActivityCalendar ||--o{ Activity : schedules
```
...
stateDiagram-v2
    [*] --> Drafted
    Drafted --> Created : activityCreated
    Created --> Booked : facilitySpaceBooked
    Booked --> Recurring : recurrenceSet
    Booked --> Published : calendarPublished
    Recurring --> Published : calendarPublished
    Published --> Open : participantRegistered
    Open --> Rescheduled : activityRescheduled
    Rescheduled --> Published : calendarPublished
    Open --> Cancelled : activityCancelled
    Cancelled --> [*]
```
