...
```mermaid
erDiagram
    SeasonalPlan ||--o{ FieldOperation : contains
    FieldOperation ||--|| Parcel : targets
    FieldOperation ||--|| CrewAssignment : requires
    FieldOperation }o--|| WeatherWindow : depends_on
    SeasonalPlan ||--o{ HarvestSchedule : includes
    HarvestSchedule ||--o{ Parcel : covers
```
...
stateDiagram-v2
    [*] --> Planning
    Planning --> Planned : seasonalPlanCreated
    Planned --> Scheduled : fieldOperationScheduled
    Scheduled --> WeatherCheck : weatherWindowAssessed
    WeatherCheck --> Crewed : crewAssigned
    WeatherCheck --> Adjusted : scheduleAdjusted
    Adjusted --> Scheduled : fieldOperationScheduled
    Crewed --> Recorded : progressRecorded
    Recorded --> Scheduled : fieldOperationScheduled
    Recorded --> Closed : seasonalPlanClosed
    Closed --> [*]
```
