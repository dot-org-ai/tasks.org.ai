...
```mermaid
erDiagram
    MaintenanceOrder ||--|{ MaintenanceTask : contains
    MaintenanceOrder ||--|| TechnicianAssignment : assigns
    Asset ||--o{ MaintenanceOrder : requires
    MaintenanceSchedule ||--o{ MaintenanceOrder : plans
    MaintenanceOrder ||--|| CompletionReport : produces
    TechnicianAssignment }o--|| MaintenanceTask : executes
```
...
stateDiagram-v2
    [*] --> Created
    Created --> Scheduled : maintenanceWindowScheduled
    Scheduled --> Assigned : technicianAssigned
    Assigned --> PartsReady : partsProcured
    PartsReady --> InProgress : maintenanceTaskStarted
    InProgress --> Completed : maintenanceOrderCompleted
    InProgress --> Deferred : maintenanceDeferred
    Deferred --> Scheduled : maintenanceWindowScheduled
    Completed --> [*]
```
