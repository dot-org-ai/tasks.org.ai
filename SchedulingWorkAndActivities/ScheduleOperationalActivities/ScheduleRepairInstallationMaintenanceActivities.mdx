...
```mermaid
erDiagram
    ServiceOrder ||--|{ ScheduleSlot : "assigned to"
    ServiceOrder ||--o{ MaterialKit : requires
    ServiceOrder ||--|| ServiceReport : "documented by"
    ServiceOrder }o--|| JobSite : "performed at"
    TechnicianRoster ||--o{ ScheduleSlot : "fills"
    MaterialKit }o--|| JobSite : "delivered to"
```
...
stateDiagram-v2
    [*] --> Created
    Created --> Scheduled : jobScheduled
    Scheduled --> MaterialsReady : materialKitPrepared
    MaterialsReady --> Confirmed : appointmentConfirmed
    Confirmed --> Dispatched : technicianDispatched
    Dispatched --> Completed : serviceOrderCompleted
    Scheduled --> Rescheduled : jobRescheduled
    Rescheduled --> Scheduled : jobScheduled
    Confirmed --> Rescheduled : jobRescheduled
    Completed --> [*]
```
