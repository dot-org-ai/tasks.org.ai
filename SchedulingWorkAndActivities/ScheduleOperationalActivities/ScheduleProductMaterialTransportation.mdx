...
```mermaid
erDiagram
    TransportOrder ||--o{ Shipment : contains
    TransportOrder ||--|{ CarrierBooking : "fulfilled by"
    CarrierBooking }o--|| Route : follows
    Shipment ||--o{ DockAppointment : "scheduled at"
    Shipment ||--|| BillOfLading : "documented by"
    Route ||--o{ DockAppointment : "includes stop"
```
...
stateDiagram-v2
    [*] --> OrderCreated
    OrderCreated --> CarrierSelected : carrierSelected
    CarrierSelected --> ShipmentBooked : shipmentBooked
    ShipmentBooked --> DockScheduled : dockAppointmentScheduled
    DockScheduled --> PickedUp : shipmentPickedUp
    PickedUp --> InTransit : shipmentInTransit
    InTransit --> InTransit : shipmentInTransit
    InTransit --> Delivered : deliveryConfirmed
    DockScheduled --> DockScheduled : dockAppointmentScheduled
    Delivered --> [*]
```
