...
```mermaid
erDiagram
    OperationalSchedule ||--o{ Activity : contains
    Activity ||--o{ ResourceAllocation : requires
    OperationalSchedule ||--o{ Milestone : defines
    OperationalSchedule ||--|| ShiftPlan : staffs
    Activity ||--o{ ScheduleException : disrupts
    ShiftPlan ||--o{ ResourceAllocation : provides
```
...
stateDiagram-v2
    [*] --> Created
    Created --> Populated : activityAdded
    Populated --> Resourced : resourcesAllocated
    Resourced --> Published : schedulePublished
    Published --> InProgress : milestoneReached
    InProgress --> Disrupted : exceptionResolved
    Disrupted --> Adjusted : scheduleAdjusted
    Adjusted --> InProgress : milestoneReached
    InProgress --> Complete : milestoneReached
    Complete --> [*]
```
