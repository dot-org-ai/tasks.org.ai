...
```mermaid
erDiagram
    RiskAssessment ||--o{ PreventiveCounsel : informs
    RiskAssessment ||--o{ ScreeningRecommendation : triggers
    PreventiveCounsel ||--o{ LifestyleGoal : establishes
    LifestyleGoal ||--o{ WellnessPlan : "included in"
    WellnessPlan ||--o{ ProgressTracking : "monitored by"
    ScreeningRecommendation ||--o{ WellnessPlan : "scheduled in"
```
...
```mermaid
stateDiagram-v2
    [*] --> RiskKnown : riskFactorsAssessed
    RiskKnown --> Counseled : counselingProvided
    Counseled --> ScreeningsSet : screeningsRecommended
    ScreeningsSet --> GoalsEstablished : lifestyleGoalsSet
    GoalsEstablished --> Planned : wellnessPlanCreated
    Planned --> Monitoring : progressMonitored
    Monitoring --> Adjusted : recommendationsAdjusted
