...
```mermaid
erDiagram
    FinancialProfile ||--o{ Goal : defines
    Goal ||--o{ Strategy : "achieved by"
    Strategy ||--o{ Portfolio : implements
    Portfolio ||--o{ Review : "assessed in"
    Plan ||--|{ Goal : tracks
    Plan ||--|{ Strategy : incorporates
    Review ||--o{ Strategy : "may revise"
```
...
```mermaid
stateDiagram-v2
    [*] --> Assessed : financesAssessed
    Assessed --> GoalsDefined : goalsSet
    GoalsDefined --> Planned : strategyDeveloped
    Planned --> Recommended : productsRecommended
    Recommended --> Optimized : taxesOptimized
    Optimized --> UnderReview : progressReviewed
    UnderReview --> Adjusted : strategyAdjusted
    Adjusted --> Recommended : productsRecommended
    UnderReview --> [*]
```
