...
```mermaid
erDiagram
    InvestorProfile ||--o{ Recommendation : receives
    InvestorProfile ||--|| Portfolio : owns
    Portfolio ||--o{ Security : contains
    Portfolio ||--|| Allocation : "distributed by"
    Portfolio ||--o{ Performance : "measured by"
    Recommendation ||--o{ Security : suggests
```
...
```mermaid
stateDiagram-v2
    [*] --> Profiled : riskAssessed
    Profiled --> Researched : securitiesResearched
    Researched --> Constructed : portfolioConstructed
    Constructed --> Recommended : investmentRecommended
    Recommended --> Monitoring : performanceMonitored
    Monitoring --> Rebalancing : holdingsRebalanced
    Rebalancing --> Monitoring : performanceMonitored
    Monitoring --> Updated : recommendationsUpdated
    Updated --> Recommended : investmentRecommended
```
