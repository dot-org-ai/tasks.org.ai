...
```mermaid
erDiagram
    SustainabilityAudit ||--o{ Practice : identifies
    SustainabilityAudit ||--o{ Metric : measures
    Goal ||--o{ Metric : tracks
    Initiative ||--o{ Practice : contains
    Initiative ||--o| Certification : achieves
    Goal }o--|| Initiative : drives
```
...
stateDiagram-v2
    ...
    Audited --> Identified : opportunitiesIdentified
    Identified --> Recommended : practicesRecommended
    Recommended --> GoalSet : goalsSet
    GoalSet --> Tracking : metricsTracked
    Tracking --> Reviewed : performanceReviewed
    Reviewed --> Recommended : goals not met
    Tracking --> Certified : certificationPursued
    Certified --> [*]
```
