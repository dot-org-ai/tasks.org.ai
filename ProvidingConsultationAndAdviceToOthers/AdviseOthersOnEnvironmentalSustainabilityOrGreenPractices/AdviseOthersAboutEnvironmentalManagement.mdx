...
```mermaid
erDiagram
    EnvironmentalAssessment ||--o{ ImpactMitigation : identifies
    EnvironmentalAssessment ||--|| SustainabilityPlan : informs
    SustainabilityPlan ||--o{ ConservationStrategy : includes
    ConservationStrategy ||--o{ ImpactMitigation : implements
    EnvironmentalPermit ||--|| ComplianceReport : documents
    ComplianceReport }o--|| EnvironmentalAssessment : references
```
...
```mermaid
stateDiagram-v2
    [*] --> Assessed : environmentalAssessmentCompleted
    Assessed --> Advised : complianceAdvised
    Assessed --> Planned : sustainabilityPlanDeveloped
    Advised --> Planned : sustainabilityPlanDeveloped
    Planned --> Conservation : conservationMeasuresRecommended
    Conservation --> Mitigated : mitigationPlanDesigned
    Mitigated --> Monitored : environmentalPerformanceReviewed
    Monitored --> Planned : targets not met
    Monitored --> Reported : complianceStatusReported
    Reported --> [*]
```
...
graph TD
    ...
    G --> H{Targets Met?}
    H -->|No| D
    H -->|Yes| I[reportComplianceStatus]
```
