...
```mermaid
erDiagram
    SupportCase ||--o{ BugReport : documents
    BugReport ||--o{ Patch : "fixed by"
    BugReport ||--o{ Workaround : "bypassed by"
    Patch ||--o{ ReleaseNote : "described in"
    SupportCase ||--o{ UserGuide : "may update"
    Workaround ||--o{ UserGuide : "documented in"
```
...
stateDiagram-v2
    [*] --> Created : caseCreated
    Created --> Reproduced : bugReproduced
    Reproduced --> WorkedAround : workaroundProvided
    Reproduced --> DocumentedOnly : documentationUpdated
    WorkedAround --> Escalated : escalatedToDevelopment
    Escalated --> Patched : patchApplied
    Patched --> Published : releaseNotesPublished
    Published --> Created : caseCreated
    Published --> [*]
```
...
graph TD
    ...
    F --> G[applyPatch]
    G --> H{Patch Verified?}
    H -->|Yes| I[publishReleaseNotes]
    H -->|No| F
    I --> J[updateDocumentation]
```
