...
```mermaid
erDiagram
    Assessment ||--o{ FunctionalGoal : identifies
    Assessment ||--o{ EnvironmentProfile : evaluates
    FunctionalGoal ||--o{ AssistiveDevice : "met by"
    AssistiveDevice ||--|| Recommendation : "prescribed in"
    Recommendation ||--|| TrainingPlan : "followed by"
...
stateDiagram-v2
    ...
    Assessed --> Identified : devicesIdentified
    Identified --> Prescribed : devicePrescribed
    Prescribed --> Justified : selectionJustified
    Justified --> TrainingScheduled : trainingCoordinated
    TrainingScheduled --> Monitoring : outcomesMonitored
    Monitoring --> Adjusted : recommendationAdjusted
    Adjusted --> Prescribed : devicePrescribed
    Monitoring --> [*]
```
