...
```mermaid
erDiagram
    AcademicProfile ||--|{ ProgramRecommendation : "matched to"
    AcademicProfile ||--|| CoursePlan : "structured in"
    ProgramRecommendation ||--o| EnrollmentApplication : "may lead to"
    CoursePlan ||--|| ProgramRecommendation : "aligned with"
    EnrollmentApplication ||--o| FinancialAidPackage : "funded by"
    AcademicProfile ||--o| FinancialAidPackage : "qualified for"
```
...
stateDiagram-v2
    [*] --> ReadinessAssessed
    ReadinessAssessed --> ProgramsRecommended : programsRecommended
    ProgramsRecommended --> CoursePlanDeveloped : coursePlanDeveloped
    CoursePlanDeveloped --> FinancialAidAdvised : financialAidAdvised
    FinancialAidAdvised --> ApplicationSubmitted : applicationSubmitted
    ApplicationSubmitted --> Monitoring : monitorAcademicProgress
    Monitoring --> ProgressReviewed : academicProgressReviewed
    ProgressReviewed --> PlanAdjusted : academicPlanAdjusted
    PlanAdjusted --> CoursePlanDeveloped : revise
    ProgressReviewed --> [*]
```
