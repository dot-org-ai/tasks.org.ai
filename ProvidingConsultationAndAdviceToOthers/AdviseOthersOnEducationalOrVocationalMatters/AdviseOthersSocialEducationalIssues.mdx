...
```mermaid
erDiagram
    NeedsAssessment ||--|{ RiskIndicator : identifies
    NeedsAssessment ||--|| InterventionPlan : "results in"
    InterventionPlan ||--|{ ResourceReferral : "includes"
    InterventionPlan ||--|| CaseRecord : "documented in"
    CaseRecord ||--|| OutcomeReport : "summarized in"
    ResourceReferral ||--|| OutcomeReport : "tracked in"
    RiskIndicator ||--|| InterventionPlan : "addressed by"
```
...
stateDiagram-v2
    [*] --> NeedsAssessed
    NeedsAssessed --> RisksIdentified : riskFactorsIdentified
    RisksIdentified --> CrisisReferral : immediateCrisis
    RisksIdentified --> PlanDeveloped : interventionPlanDeveloped
    CrisisReferral --> PlanDeveloped : resourceReferralMade
    PlanDeveloped --> ConferenceHeld : conferenceHeld
    ConferenceHeld --> Monitoring : monitorInterventionProgress
    Monitoring --> PlanDeveloped : notImproving
    Monitoring --> OutcomeReported : outcomeReported
    OutcomeReported --> [*]
```
