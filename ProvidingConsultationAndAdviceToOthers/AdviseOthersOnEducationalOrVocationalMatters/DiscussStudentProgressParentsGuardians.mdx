...
```mermaid
erDiagram
    ProgressReport ||--o{ GradeRecord : compiles
    ProgressReport ||--|| AttendanceLog : includes
    ProgressReport ||--|| ConferenceNotes : discussed
    ConferenceNotes ||--o{ ImprovementGoal : produces
    ImprovementGoal }o--|| GradeRecord : targets
    ProgressReport }o--|| ImprovementGoal : tracks
```
...
```mermaid
stateDiagram-v2
    [*] --> Compiled : progressDataCompiled
    Compiled --> Prepared : progressReportPrepared
    Prepared --> Scheduled : conferenceScheduled
    Scheduled --> Discussed : conferenceCompleted
    Discussed --> GoalsSet : improvementGoalsSet
    Discussed --> Updated : progressUpdateSent
    GoalsSet --> Tracking : goalProgressTracked
    Tracking --> Compiled : not improving
    Tracking --> Updated : progressUpdateSent
    Updated --> [*]
```
