...
```mermaid
erDiagram
    ComplianceAssessment ||--o{ ComplianceGap : identifies
    ComplianceAssessment ||--o{ RegulatoryRequirement : evaluates
    ComplianceGap ||--o{ AdvisoryOpinion : "addressed by"
    ComplianceGap ||--|| RemediationPlan : "resolved by"
    AuditFinding ||--o{ ComplianceGap : documents
    RemediationPlan ||--o{ AuditFinding : closes
```
...
```mermaid
stateDiagram-v2
    [*] --> Surveyed : regulatoryLandscapeAssessed
    Surveyed --> GapsFound : complianceGapsIdentified
    GapsFound --> Advised : advisoryOpinionDelivered
    Advised --> Remediating : remediationPlanDeveloped
    Remediating --> Reviewing : complianceReviewCompleted
    Reviewing --> Advised : advisoryOpinionDelivered
    Reviewing --> Compliant : complianceStatusReported
    Compliant --> Monitoring : regulatoryChangeDetected
    Monitoring --> Surveyed : regulatoryLandscapeAssessed
```
