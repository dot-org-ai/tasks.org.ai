...
```mermaid
erDiagram
    Situation ||--o{ Option : "generates"
    Option ||--o{ Alternative : "compared via"
    Situation ||--|| Recommendation : "resolved by"
    Recommendation ||--|| Strategy : "part of"
    Strategy ||--|| Timeline : "scheduled in"
    Option ||--o{ Timeline : "constrained by"
```
...
```mermaid
stateDiagram-v2
    [*] --> Evaluated : situationEvaluated
    Evaluated --> OptionsKnown : optionsIdentified
    OptionsKnown --> Compared : alternativesAnalyzed
    Compared --> Costed : costsAssessed
    Costed --> Projected : outcomesProjected
    Projected --> Recommended : actionRecommended
    Recommended --> Compared : alternativesAnalyzed
