...
```mermaid
erDiagram
    EmergencyPlan ||--o{ ResponseProcedure : contains
    EmergencyPlan ||--o{ IncidentScenario : addresses
    HazardousSubstance ||--o{ IncidentScenario : "may cause"
    DrillExercise ||--o{ IncidentScenario : simulates
    DrillExercise ||--o{ IncidentDebrief : "reviewed in"
    ResponseProcedure ||--o{ DrillExercise : "tested by"
```
...
```mermaid
stateDiagram-v2
    [*] --> HazmatAssessed : hazardousMaterialsAssessed
    HazmatAssessed --> PlanReady : emergencyPlanDeveloped
    PlanReady --> TeamsTrained : responseTeamsTrained
    TeamsTrained --> DrillDone : drillConducted
    DrillDone --> ActiveResponse : incidentGuidanceProvided
    ActiveResponse --> Coordinated : responseCoordinated
    Coordinated --> Debriefed : incidentDebriefed
    Debriefed --> PlanReady : emergencyPlanDeveloped
    DrillDone --> PlanReady : emergencyPlanDeveloped
```
