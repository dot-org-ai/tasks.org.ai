...
```mermaid
erDiagram
    SupportSession ||--|{ CareNeed : "addresses"
    SupportSession ||--o{ ProgressNote : "documents"
    CareNeed ||--o{ EncouragementPlan : "shapes"
    CareNeed ||--o{ SpiritualAssessment : "triggers"
    EncouragementPlan ||--o{ ResourceConnection : "identifies"
    SpiritualAssessment ||--o{ EncouragementPlan : "informs"
    ResourceConnection ||--o{ ProgressNote : "tracked in"
```
...
stateDiagram-v2
    [*] --> NeedIdentified
    NeedIdentified --> Listening : listeningProvided
    Listening --> Validated : experienceValidated
    Validated --> ExploringMeaning : meaningExplored
    ExploringMeaning --> SpiritualCare : spiritualCareOffered
    ExploringMeaning --> HopeBuilding : hopeInstilled
    SpiritualCare --> HopeBuilding : hopeInstilled
    HopeBuilding --> ResiliencePhase : resilienceBuilt
    ResiliencePhase --> Connected : resourcesConnected
    ResiliencePhase --> Listening : additional concerns surface
    Connected --> OngoingSupport
    OngoingSupport --> Listening : follow-up session
    OngoingSupport --> [*]
```
...
graph TD
    ...
    H -->|No| J{Further Sessions Needed?}
    I --> J
    J -->|Yes| A
    J -->|No| K[Close Support Episode]
```
