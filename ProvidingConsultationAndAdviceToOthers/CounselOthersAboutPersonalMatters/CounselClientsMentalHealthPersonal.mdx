...
```mermaid
erDiagram
    ClientIntake ||--|| TreatmentPlan : informs
    TreatmentPlan ||--o{ CounselingSession : "delivered via"
    CounselingSession ||--o{ ProgressNote : "documented in"
    TreatmentPlan ||--o{ OutcomeAssessment : "measured by"
    ClientIntake ||--o{ SafetyPlan : "may require"
    OutcomeAssessment ||--o{ TreatmentPlan : "may revise"
```
...
```mermaid
stateDiagram-v2
    [*] --> IntakeComplete : intakeAssessmentCompleted
    IntakeComplete --> SafetyPlanned : safetyPlanCreated
    IntakeComplete --> PlanReady : treatmentPlanDeveloped
    SafetyPlanned --> PlanReady : treatmentPlanDeveloped
    PlanReady --> InSession : counselingSessionDelivered
    InSession --> ProgressReviewed : progressAssessed
    ProgressReviewed --> PlanUpdated : treatmentPlanUpdated
    PlanUpdated --> InSession : counselingSessionDelivered
    ProgressReviewed --> Completed : outcomeDocumented
    Completed --> [*]
```
