...
```mermaid
erDiagram
    PsychosocialAssessment ||--|{ InterventionPlan : informs
    InterventionPlan ||--o{ CounselingSession : guides
    CounselingSession ||--o{ ClientJournal : generates
    CounselingSession ||--o{ ReferralRecord : triggers
    InterventionPlan ||--|| DischargesSummary : concludes
    PsychosocialAssessment }o--|| ReferralRecord : identifies
```
...
stateDiagram-v2
    [*] --> Referred
    Referred --> Assessed : psychosocialAssessmentCompleted
    Assessed --> PlanActive : interventionPlanFormulated
    PlanActive --> InSession : sessionFacilitated
    InSession --> Monitoring : wellBeingMonitored
    Monitoring --> InSession : sessionFacilitated
    Monitoring --> ExternalSupport : supportServiceReferralMade
    ExternalSupport --> InSession : sessionFacilitated
    Monitoring --> Discharged : dischargeCompleted
    Discharged --> [*]
```
