...
```mermaid
erDiagram
    AdvisoryConsultation ||--o{ ClinicalRecommendation : produces
    AdvisoryConsultation ||--o{ CareProtocol : reviews
    CareProtocol ||--o{ QualityMetric : "measured by"
    ClinicalRecommendation ||--o{ OutcomeReport : "tracked in"
    RegulatoryGuideline ||--o{ CareProtocol : governs
    ClinicalRecommendation ||--o{ RegulatoryGuideline : references
```
...
```mermaid
stateDiagram-v2
    [*] --> NeedsAssessed : healthcareNeedsAssessed
    NeedsAssessed --> Clinical : clinicalRecommendationDelivered
    NeedsAssessed --> ProtocolReview : careProtocolReviewed
    NeedsAssessed --> PolicyChange : policyChangeRecommended
    Clinical --> QualityReview : qualityReviewCompleted
    ProtocolReview --> QualityReview : qualityReviewCompleted
    PolicyChange --> QualityReview : qualityReviewCompleted
    QualityReview --> Evaluated : advisoryOutcomeEvaluated
    Evaluated --> Urgent : urgentAdvisoryIssued
    Evaluated --> [*]
    Urgent --> NeedsAssessed : healthcareNeedsAssessed
```
