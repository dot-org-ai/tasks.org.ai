...
```mermaid
erDiagram
    NeedsAssessment ||--o{ HealthProgram : informs
    HealthProgram ||--o{ Intervention : deploys
    HealthProgram ||--o{ Policy : "guided by"
    Intervention ||--o{ Outcome : produces
    Outcome ||--o{ Metric : "measured by"
    Policy ||--o{ Intervention : enables
```
...
stateDiagram-v2
    ...
    Assessed --> RisksKnown : risksIdentified
    RisksKnown --> ProgramReady : programDeveloped
    ProgramReady --> PolicySet : policyRecommended
    PolicySet --> Deployed : interventionImplemented
    Deployed --> Monitoring : outcomesMonitored
    Monitoring --> Evaluated : impactEvaluated
    Evaluated --> ProgramReady : programDeveloped
    Monitoring --> [*]
```
