...
```mermaid
erDiagram
    PositionRequisition ||--|| PersonnelRecommendation : "fulfilled by"
    PersonnelRecommendation ||--o| PerformanceRecord : "references"
    PersonnelRecommendation ||--o| CompensationBenchmark : "informed by"
    PersonnelRecommendation ||--o| DisciplinaryCase : "may address"
    SuccessionPlan ||--|{ PersonnelRecommendation : "drives"
    CompensationBenchmark ||--|| PersonnelRecommendation : "supports"
    PerformanceRecord ||--o| DisciplinaryCase : "may trigger"
```
...
stateDiagram-v2
    [*] --> NeedsAssessed
    NeedsAssessed --> HireRecommended : hireRecommended
    NeedsAssessed --> PromotionRecommended : promotionRecommended
    NeedsAssessed --> TransferRecommended : transferRecommended
    HireRecommended --> Approved : recommendationApproved
    PromotionRecommended --> Approved : recommendationApproved
    TransferRecommended --> Approved : recommendationApproved
    Approved --> CompensationAdjusted : compensationChangeRecommended
    Approved --> Implemented : implemented
    CompensationAdjusted --> Implemented : implemented
    Implemented --> [*]
```
