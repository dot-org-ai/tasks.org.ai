...
```mermaid
erDiagram
    NonConformance ||--|| RootCauseAnalysis : "investigated by"
    RootCauseAnalysis ||--|{ CorrectiveAction : produces
    RootCauseAnalysis ||--o{ PreventiveAction : "may produce"
    CorrectiveAction ||--|| ChangeRequest : "formalized as"
    CorrectiveAction ||--|| EffectivenessReview : "verified by"
    NonConformance ||--o{ EffectivenessReview : "may recur after"
    ChangeRequest ||--|| CorrectiveAction : authorizes
```
...
```mermaid
stateDiagram-v2
    [*] --> NonConformanceLogged
    NonConformanceLogged --> RootCauseIdentified : rootCauseIdentified
    RootCauseIdentified --> ActionProposed : correctiveActionProposed
    ActionProposed --> Approved : changeRequestApproved
    Approved --> Implemented : correctiveActionImplemented
    Implemented --> Verified : effectivenessVerified
    Implemented --> Recurring : recurrenceEscalated
    Recurring --> RootCauseIdentified : reanalyze
    Verified --> [*]
```
