...
```mermaid
erDiagram
    PolicyProposal ||--|| ProcessMap : "modifies"
    PolicyProposal ||--|| ImpactAssessment : "evaluated by"
    PolicyProposal ||--|{ StakeholderFeedback : "reviewed via"
    PolicyProposal ||--|| ApprovalRecord : "decided in"
    ApprovalRecord ||--|| ImplementationTracker : "tracked by"
    ImpactAssessment ||--|| ApprovalRecord : informs
    StakeholderFeedback ||--|| PolicyProposal : "may revise"
```
...
stateDiagram-v2
    [*] --> PolicyAssessed
    PolicyAssessed --> ProposalDrafted : proposalDrafted
    ProposalDrafted --> ImpactAnalyzed : impactAssessmentCompleted
    ImpactAnalyzed --> FeedbackCollected : stakeholderFeedbackCollected
    FeedbackCollected --> SubmittedForApproval : submitForApproval
    SubmittedForApproval --> Approved : proposalApproved
    SubmittedForApproval --> ProposalDrafted : revisionsRequired
    Approved --> Implemented : policyChangeImplemented
    Implemented --> Monitoring : adoptionMonitored
    Monitoring --> [*]
```
