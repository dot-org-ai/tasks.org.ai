...
```mermaid
erDiagram
    AdvisoryRequest ||--|| Recommendation : "results in"
    Recommendation ||--|{ PolicyReference : "cites"
    AdvisoryRequest ||--|| ConsultationRecord : "logged in"
    Recommendation ||--|| ConsultationRecord : "documented in"
    AdvisoryRequest ||--o| ComplianceAssessment : "may trigger"
    ComplianceAssessment ||--|{ PolicyReference : "evaluates against"
```
...
stateDiagram-v2
    [*] --> RequestSubmitted
    RequestSubmitted --> ComplianceAssessed : complianceAssessed
    ComplianceAssessed --> LegalEscalation : escalationTriggered
    ComplianceAssessed --> GuidanceDelivered : guidanceDelivered
    LegalEscalation --> GuidanceDelivered : guidanceDelivered
    GuidanceDelivered --> Documented : recommendationDocumented
    Documented --> Tracking : trackImplementation
    Tracking --> Adopted : implementationVerified
    Tracking --> GuidanceDelivered : notAdopted
    Adopted --> [*]
```
