...
```mermaid
erDiagram
    ProductProfile ||--|| DamageRiskAssessment : "evaluated by"
    ProductProfile ||--|| PackagingSpecification : "packed per"
    PackagingSpecification ||--|| ShippingRecommendation : "shipped via"
    ShippingRecommendation ||--|| CostEstimate : "priced in"
    ProductProfile ||--|{ ComplianceRequirement : "subject to"
    DamageRiskAssessment ||--|| PackagingSpecification : informs
    ComplianceRequirement ||--|| ShippingRecommendation : constrains
```
...
stateDiagram-v2
    [*] --> ProfileAssessed
    ProfileAssessed --> RiskAssessed : damageRiskAssessed
    RiskAssessed --> PackagingRecommended : packagingRecommended
    PackagingRecommended --> ModeSelected : shippingModeSelected
    ModeSelected --> CarriersEvaluated : carrierOptionsEvaluated
    CarriersEvaluated --> ComplianceVerified : complianceVerified
    ComplianceVerified --> CostEstimated : shippingCostEstimated
    CostEstimated --> Approved : clientApproved
    CostEstimated --> ModeSelected : clientRevision
    Approved --> [*]
```
