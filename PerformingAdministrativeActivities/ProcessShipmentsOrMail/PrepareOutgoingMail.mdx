...
```mermaid
erDiagram
    MailItem ||--|| AddressRecord : "addressedTo"
    MailItem ||--|| MailingLabel : "labeledWith"
    MailBatch ||--|{ MailItem : "contains"
    MailBatch ||--|| PickupSchedule : "scheduledPer"
    PostageAccount ||--|{ MailItem : "fundsPostageFor"
    MailingLabel }o--|| AddressRecord : "displaysDestination"
```
...
stateDiagram-v2
    [*] --> Collected
    Collected --> AddressValidation : documentsCollected
    AddressValidation --> Collected : invalidAddress
    AddressValidation --> Postaged : addressesValidated
    Postaged --> Sorted : postageApplied
    Sorted --> Batched : mailSorted
    Batched --> PickupReady : mailBatchCreated
    PickupReady --> Dispatched : pickupScheduled
    Dispatched --> [*] : mailDispatched
```
