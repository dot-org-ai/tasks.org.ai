...
erDiagram
    ...
    Shipment ||--|| ReceivingReport : generates
    ReceivingReport ||--o{ DamageReport : documents
    ReceivingReport ||--|| InventoryReceipt : triggers
    PurchaseOrder ||--o{ Shipment : receives
    Shipment ||--o{ DamageReport : records
```
...
stateDiagram-v2
    ...
    Accepted --> Inspected : contentsInspected
    Inspected --> Discrepant : discrepanciesDocumented
    Inspected --> Reported : receivingReportCreated
    Discrepant --> Damaged : damageReported
    Discrepant --> Reported : receivingReportCreated
    Damaged --> Reported : receivingReportCreated
    Reported --> Inventoried : processedIntoInventory
    Inventoried --> Notified : purchasingNotified
    Notified --> [*]
```
