...
```mermaid
erDiagram
    Material ||--|| ClassificationRecord : "assignedIn"
    ClassificationRecord ||--|| ClassificationCode : "references"
    ClassificationCode }o--|| Taxonomy : "belongsTo"
    Taxonomy ||--|{ CodeTable : "definedIn"
    AuditReport ||--|{ ClassificationRecord : "reviews"
    Material }o--|| AuditReport : "auditedIn"
```
...
stateDiagram-v2
    [*] --> Identified
    Identified --> Classified : materialIdentified
    Classified --> Validated : classificationAssigned
    Validated --> Published : classificationValidated
    Validated --> Reclassifying : invalidClassification
    Reclassifying --> Validated : materialReclassified
    Published --> Audited : catalogPublished
    Audited --> Reclassifying : issuesFound
    Audited --> [*] : classificationsAudited
```
