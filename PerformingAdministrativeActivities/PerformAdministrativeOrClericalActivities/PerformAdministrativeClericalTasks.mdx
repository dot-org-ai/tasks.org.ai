...
```mermaid
erDiagram
    TaskAssignment ||--o| Record : "produces"
    TaskAssignment ||--o| Correspondence : "generates"
    TaskAssignment ||--o| MeetingMinutes : "documents"
    Record ||--|| FileIndex : "catalogedIn"
    OfficeSupplyOrder }o--|| TaskAssignment : "triggeredBy"
    Correspondence }o--|| Record : "filedAs"
```
...
stateDiagram-v2
    [*] --> RequestReceived
    RequestReceived --> DataEntry : dataEntryTask
    RequestReceived --> Filing : filingTask
    RequestReceived --> CorrespondenceHandling : correspondenceTask
    RequestReceived --> Scheduling : meetingTask
    RequestReceived --> Procurement : supplyTask
    DataEntry --> Reporting : dataEntered
    Filing --> Reporting : documentFiled
    CorrespondenceHandling --> Reporting : correspondenceProcessed
    Scheduling --> [*] : meetingScheduled
    Procurement --> [*] : suppliesOrdered
