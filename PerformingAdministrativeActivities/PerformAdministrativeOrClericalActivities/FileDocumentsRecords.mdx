...
```mermaid
erDiagram
    Record ||--|| FileCategory : "classifiedAs"
    Record ||--|| FilingIndex : "indexedIn"
    Record ||--|| StorageLocation : "storedAt"
    Record ||--|| RetentionSchedule : "governedBy"
    RetrievalRequest }o--|| Record : "targets"
    FilingIndex ||--|{ Record : "catalogs"
```
...
stateDiagram-v2
    [*] --> Received
    Received --> Classified : documentReceived
    Classified --> Indexed : recordClassified
    Indexed --> Stored : recordIndexed
    Stored --> Retrieved : recordRetrieved
    Retrieved --> Stored : returnedToStorage
    Stored --> Transferred : recordTransferred
    Transferred --> Stored : newLocationConfirmed
    Stored --> RetentionExpired : retentionPeriodEnded
    RetentionExpired --> Destroyed : recordDestroyed
    Destroyed --> [*]
```
