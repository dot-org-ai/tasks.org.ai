...
```mermaid
erDiagram
    Document ||--|| Template : "createdFrom"
    Document ||--|{ Revision : "trackedBy"
    Document ||--o{ Attachment : "includes"
    Document ||--|| DistributionList : "sentTo"
    Document ||--|| ApprovalRecord : "approvedVia"
    ApprovalRecord }o--|| Revision : "approvesVersion"
```
...
stateDiagram-v2
    [*] --> Created
    Created --> Drafting : documentCreated
    Drafting --> Formatted : contentDrafted
    Formatted --> UnderReview : documentFormatted
    UnderReview --> Drafting : revisionsRequired
    UnderReview --> Approved : documentApproved
    Approved --> Distributed : documentDistributed
    Distributed --> Archived : documentArchived
    Archived --> [*]
```
