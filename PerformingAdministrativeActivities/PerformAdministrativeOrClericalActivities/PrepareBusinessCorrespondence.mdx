...
```mermaid
erDiagram
    Correspondence ||--|| Template : "basedOn"
    Correspondence ||--|{ DraftVersion : "evolvesThroughDrafts"
    Correspondence ||--o{ Attachment : "includes"
    Correspondence ||--|| DistributionList : "sentTo"
    Correspondence ||--|| CorrespondenceLog : "recordedIn"
    DraftVersion }o--|| CorrespondenceLog : "loggedIn"
```
...
stateDiagram-v2
    [*] --> TemplateSelected
    TemplateSelected --> Drafting : templateSelected
    Drafting --> InReview : correspondenceDrafted
    InReview --> Drafting : revisionsRequired
    InReview --> Approved : draftApproved
    Approved --> Formatted : documentFormatted
    Formatted --> Distributed : correspondenceDistributed
    Distributed --> Archived : correspondenceArchived
    Archived --> [*]
```
