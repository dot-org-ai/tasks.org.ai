...
erDiagram
    ...
    Order ||--|| PriceQuote : "pricedBy"
    Order ||--|| OrderConfirmation : "confirmedWith"
    Order ||--|| FulfillmentRequest : "fulfilledVia"
    OrderLineItem }o--|| ProductCatalog : "referencesItemIn"
    PriceQuote }o--|| ProductCatalog : "calculatedFrom"
```
...
stateDiagram-v2
    [*] --> Received
    Received --> AvailabilityCheck : orderReceived
    AvailabilityCheck --> OutOfStock : itemsUnavailable
    OutOfStock --> Received : backorderNotified
    AvailabilityCheck --> Priced : availabilityChecked
    Priced --> Confirming : pricingCalculated
    Confirming --> PaymentAuth : orderConfirmed
    PaymentAuth --> PaymentFailed : paymentDeclined
    PaymentFailed --> PaymentAuth : retryPayment
    PaymentAuth --> Fulfilled : paymentAuthorized
    Fulfilled --> [*] : orderRoutedToFulfillment
```
