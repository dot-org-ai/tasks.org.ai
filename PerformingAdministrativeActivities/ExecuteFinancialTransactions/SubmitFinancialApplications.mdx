...
```mermaid
erDiagram
    Application ||--|| ApplicantProfile : "submittedBy"
    Application ||--|{ SupportingDocument : "includes"
    Application ||--|| EligibilityCriteria : "evaluatedAgainst"
    Application ||--|| DecisionRecord : "decidedIn"
    Application ||--|| ApplicationStatus : "trackedAs"
    DecisionRecord }o--|| EligibilityCriteria : "basedOn"
```
...
stateDiagram-v2
    [*] --> Created
    Created --> Documenting : applicationCreated
    Documenting --> Validating : documentationGathered
    Validating --> Ineligible : eligibilityFailed
    Ineligible --> [*]
    Validating --> Submitted : eligibilityValidated
    Submitted --> UnderReview : applicationSubmitted
    UnderReview --> Approved : applicationApproved
    UnderReview --> Denied : applicationDenied
    UnderReview --> Submitted : additionalInfoRequested
    Approved --> [*]
    Denied --> [*]
    UnderReview --> Withdrawn : applicationWithdrawn
    Withdrawn --> [*]
```
