...
```mermaid
erDiagram
    CoverageRequest ||--|{ Quote : "generatesQuotesFor"
    Quote ||--o| Policy : "convertsTo"
    Policy ||--o{ Certificate : "provesVia"
    Policy ||--o{ Endorsement : "modifiedBy"
    Policy ||--o{ Claim : "paysOut"
    CoverageRequest }o--|| Policy : "renewsThrough"
```
...
stateDiagram-v2
    [*] --> NeedsAssessment
    NeedsAssessment --> QuoteCollection : coverageNeedsAssessed
    QuoteCollection --> QuoteReview : quotesRequested
    QuoteReview --> QuoteCollection : noneAcceptable
    QuoteReview --> Binding : quotesEvaluated
    Binding --> Active : coverageBound
    Active --> Active : certificateIssued
    Active --> Endorsing : changeRequested
    Endorsing --> Active : policyEndorsed
    Active --> Renewal : approachingExpiration
    Renewal --> QuoteCollection : policyRenewed
    Renewal --> [*] : coverageTerminated
```
