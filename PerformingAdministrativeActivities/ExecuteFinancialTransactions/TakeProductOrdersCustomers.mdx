...
erDiagram
    ...
    Order ||--|| CustomerAccount : "placedBy"
    Order ||--|| PriceQuote : "totaledIn"
    Order ||--|| OrderConfirmation : "acknowledgedBy"
    OrderLineItem }o--|| ProductCatalog : "referencesItemIn"
    PriceQuote }o--|| ProductCatalog : "derivedFrom"
```
...
stateDiagram-v2
    ...
    Validating --> Valid : orderValidated
    Validating --> OnHold : validationFailed
    OnHold --> Validating : issueResolved
    OnHold --> Cancelled : orderCancelled
    Cancelled --> [*]
    Valid --> Priced : totalCalculated
    Priced --> Confirmed : orderConfirmed
    Confirmed --> Fulfilled : orderTransferredToFulfillment
    Fulfilled --> [*]
```
