...
```mermaid
erDiagram
    Obligation ||--|| Invoice : "documentedBy"
    Obligation ||--|| PaymentSchedule : "scheduledIn"
    Obligation ||--|| PaymentRecord : "settledBy"
    Obligation ||--o| TaxFiling : "accompaniedBy"
    Obligation ||--o{ Penalty : "incursIfLate"
    PaymentRecord }o--|| Invoice : "satisfies"
```
...
stateDiagram-v2
    [*] --> Identified
    Identified --> Calculated : obligationIdentified
    Calculated --> Scheduled : amountCalculated
    Scheduled --> PendingAuth : paymentScheduled
    PendingAuth --> Scheduled : authorizationDenied
    PendingAuth --> Remitting : paymentAuthorized
    Remitting --> Recorded : paymentRemitted
    Recorded --> TaxFiling : taxPayment
    Recorded --> [*] : paymentRecorded
    TaxFiling --> [*] : taxReturnFiled
    Scheduled --> Overdue : pastDueDate
    Overdue --> PendingAuth : latePaymentInitiated
```
