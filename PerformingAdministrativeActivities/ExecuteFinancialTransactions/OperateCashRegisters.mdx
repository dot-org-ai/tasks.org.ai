...
```mermaid
erDiagram
    Register ||--|| Drawer : "contains"
    Register ||--|{ Transaction : "processes"
    Transaction ||--|| TenderType : "paidVia"
    Transaction ||--|| Receipt : "confirmedWith"
    Drawer ||--|| DrawerReport : "summarizedIn"
    DrawerReport ||--|{ Transaction : "aggregates"
```
...
stateDiagram-v2
    [*] --> Closed
    Closed --> Open : drawerOpened
    Open --> Scanning : customerPresent
    Scanning --> TenderAccepted : itemsScanned
    TenderAccepted --> ChangeDispensed : changeMade
    TenderAccepted --> ReceiptPrinted : tenderApplied
    ChangeDispensed --> ReceiptPrinted : receiptIssued
    ReceiptPrinted --> Open : transactionComplete
    Open --> ReturnProcessing : returnInitiated
    ReturnProcessing --> Open : returnProcessed
    Open --> Balancing : shiftEnd
    Balancing --> Closed : drawerClosed
```
