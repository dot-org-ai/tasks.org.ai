...
```mermaid
erDiagram
    IssuanceRequest ||--o| CreditNote : "produces"
    IssuanceRequest ||--o| Voucher : "produces"
    IssuanceRequest ||--o| Disbursement : "produces"
    CreditNote ||--o| RedemptionRecord : "redeemedVia"
    Voucher ||--o| RedemptionRecord : "redeemedVia"
    Voucher ||--|| ExpirationPolicy : "governedBy"
    CreditNote ||--|| ExpirationPolicy : "governedBy"
```
...
stateDiagram-v2
    [*] --> Requested
    Requested --> PendingApproval : issuanceRequested
    PendingApproval --> Requested : issuanceDenied
    PendingApproval --> Generating : issuanceAuthorized
    Generating --> Distributing : instrumentGenerated
    Distributing --> Outstanding : instrumentDistributed
    Outstanding --> Redeemed : instrumentRedeemed
    Outstanding --> Voided : instrumentVoided
    Outstanding --> Expired : pastExpirationDate
    Expired --> Voided : instrumentVoided
    Redeemed --> Reconciled : outstandingReconciled
    Voided --> Reconciled : outstandingReconciled
    Reconciled --> [*]
```
