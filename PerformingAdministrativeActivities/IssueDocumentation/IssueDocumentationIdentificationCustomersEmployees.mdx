...
```mermaid
erDiagram
    IdentificationDocument ||--|| IdentityVerification : "validatedBy"
    IdentificationDocument ||--|| DocumentTemplate : "generatedFrom"
    IdentificationDocument ||--|| AccessProfile : "grantsAccessVia"
    BadgeRequest ||--|| IdentificationDocument : "produces"
    IssuanceLog ||--|{ IdentificationDocument : "records"
    IdentificationDocument }o--|| AccessProfile : "linkedTo"
```
...
stateDiagram-v2
    [*] --> IdentityCheck
    IdentityCheck --> Verified : identityVerified
    IdentityCheck --> IdentityCheck : additionalInfoNeeded
    Verified --> Created : documentCreated
    Created --> Issued : documentIssued
    Issued --> Active : inUse
    Active --> ReplacementPending : lostOrDamaged
    ReplacementPending --> Deactivated : documentDeactivated
    Deactivated --> Created : documentReplaced
    Active --> Deactivated : documentDeactivated
    Active --> AccessModified : accessProfileUpdated
    AccessModified --> Active : profileConfirmed
    Deactivated --> [*]
```
