...
```mermaid
erDiagram
    Proceeding ||--|{ Oath : "requires"
    Proceeding ||--|{ Affirmation : "requires"
    Oath ||--|| OathRecord : "documentedIn"
    Affirmation ||--|| OathRecord : "documentedIn"
    OathRecord ||--|| ParticipantIdentification : "verifiedBy"
    Proceeding ||--|| ProceedingTranscript : "transcribedIn"
    OathRecord }o--|| ProceedingTranscript : "referencedIn"
```
...
stateDiagram-v2
    ...
    IdentityVerification --> TypeSelection : participantIdentified
    TypeSelection --> OathDelivery : oathTypeSelected
    OathDelivery --> Acknowledged : oathAdministered
    Acknowledged --> Recorded : acknowledgmentRecorded
    Recorded --> MoreParticipants : oathRecordCreated
    MoreParticipants --> IdentityVerification : additionalParticipant
    MoreParticipants --> Certified : allOathsComplete
    Certified --> Archived : proceedingCertified
    Archived --> [*] : oathRecordsArchived
```
