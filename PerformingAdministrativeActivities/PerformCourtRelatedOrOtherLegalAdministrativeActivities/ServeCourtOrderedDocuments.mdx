...
```mermaid
erDiagram
    ServiceOrder ||--|{ CourtDocument : "contains"
    ServiceOrder ||--|{ ServiceAttempt : "trackedThrough"
    ServiceOrder ||--|| ProofOfService : "provedBy"
    ServiceOrder ||--|| ServiceMethod : "deliveredVia"
    ServiceOrder ||--|| CaseRecord : "linkedTo"
    ServiceAttempt }o--|| ProofOfService : "culminatesIn"
```
...
stateDiagram-v2
    [*] --> Received
    Received --> Locating : serviceOrderReceived
    Locating --> AttemptingService : namedPartyLocated
    Locating --> Escalated : partyNotFound
    AttemptingService --> Served : serviceCompleted
    AttemptingService --> RetryRequired : serviceAttempted
    RetryRequired --> AttemptingService : additionalAttempt
    RetryRequired --> Escalated : maxAttemptsReached
    Escalated --> AttemptingService : alternativeMethodApproved
    Served --> ProofFiled : proofOfServiceFiled
    ProofFiled --> Closed : serviceOrderClosed
    Closed --> [*]
```
