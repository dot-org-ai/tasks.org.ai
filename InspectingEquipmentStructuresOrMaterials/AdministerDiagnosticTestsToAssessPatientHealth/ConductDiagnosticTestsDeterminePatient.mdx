...
```mermaid
erDiagram
    TestOrder ||--|{ Specimen : "requires collection of"
    TestOrder ||--o{ TestPanel : "groups into"
    Specimen ||--|{ LabResult : "produces"
    Specimen ||--|| ChainOfCustody : "tracked by"
    LabResult ||--o{ ReferenceRange : "compared against"
    LabResult ||--o{ CriticalValue : "may trigger"
    QualityControlRecord ||--o{ LabResult : "validates"
```
...
stateDiagram-v2
    [*] --> Ordered
    Ordered --> Collected : specimenCollected
    Collected --> Processing : specimenProcessed
    Processing --> Analyzed : analysisCompleted
    Analyzed --> Validated : resultsValidated
    Validated --> Critical : criticalValueFlagged
    Validated --> Reported : resultsReported
    Critical --> Reported : resultsReported
    Reported --> FollowUp : followUpOrdered
    FollowUp --> Ordered : testsOrdered
    Reported --> [*]
```
