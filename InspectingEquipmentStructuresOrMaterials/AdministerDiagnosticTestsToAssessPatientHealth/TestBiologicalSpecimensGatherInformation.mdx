...
```mermaid
erDiagram
    Specimen ||--|| AccessionRecord : "registered as"
    Specimen ||--o{ HistologySlide : "sectioned into"
    Specimen ||--o{ CultureResult : "cultured into"
    Specimen ||--o{ MolecularResult : "tested for"
    HistologySlide ||--o{ SpecialStain : "stained with"
    CultureResult ||--|{ SensitivityReport : "tested against"
    PathologyReport }o--|| Specimen : "diagnoses"
    PathologyReport }o--o{ MolecularResult : "incorporates"
```
...
stateDiagram-v2
    [*] --> Accessioned
    Accessioned --> HistologyProcessing : histologyProcessed
    Accessioned --> Culturing : cultureGrown
    Accessioned --> MolecularTesting : molecularResultReady
    HistologyProcessing --> MicroscopyReview : microscopyCompleted
    Culturing --> OrganismID : organismIdentified
    OrganismID --> SensitivityTesting : sensitivityCompleted
    MicroscopyReview --> ReportDrafting : reportIssued
    SensitivityTesting --> ReportDrafting : reportIssued
    MolecularTesting --> ReportDrafting : reportIssued
    ReportDrafting --> Issued : reportIssued
    Issued --> [*]
```
...
graph TD
    ...
    G --> H{Organism Found?}
    H -->|Yes| I[performSensitivity]
    H -->|No| J[issuePathologyReport]
    F --> J
    I --> J
    E --> J
```
