...
```mermaid
erDiagram
    ExamRecord ||--|{ VisualAcuityResult : "contains"
    ExamRecord ||--|{ RefractionData : "contains"
    ExamRecord ||--o{ TonometryReading : "includes"
    ExamRecord ||--o{ RetinalImage : "includes"
    ExamRecord ||--o{ ColorVisionResult : "includes"
    RefractionData ||--|| Prescription : "generates"
    ExamRecord ||--o{ ReferralOrder : "may produce"
```
...
stateDiagram-v2
    [*] --> Ordered
    Ordered --> AcuityTesting : acuityMeasured
    AcuityTesting --> Refraction : refractionCompleted
    Refraction --> PressureCheck : pressureMeasured
    PressureCheck --> Imaging : retinalImageCaptured
    Imaging --> ColorTesting : colorVisionAssessed
    ColorTesting --> Evaluated : prescriptionGenerated
    ColorTesting --> AbnormalFinding : abnormalityDetected
    AbnormalFinding --> Referred : abnormalityDetected
    Referred --> Evaluated : prescriptionGenerated
    Evaluated --> [*]
```
