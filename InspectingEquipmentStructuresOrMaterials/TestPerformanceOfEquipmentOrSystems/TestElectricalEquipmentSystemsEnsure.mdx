...
```mermaid
erDiagram
    ElectricalSystem ||--|{ Equipment : contains
    ElectricalSystem ||--o{ FunctionalTest : undergoes
    Equipment ||--o{ LoadTest : undergoes
    Equipment ||--o{ SafetyTest : undergoes
    FunctionalTest }o--|| TestReport : "documented in"
    LoadTest }o--|| TestReport : "documented in"
    SafetyTest }o--|| TestReport : "documented in"
```
...
```mermaid
stateDiagram-v2
    [*] --> FunctionalCheck : functionalityTested
    FunctionalCheck --> GroundingCheck : groundingChecked
    GroundingCheck --> ProtectionCheck : protectionTested
    ProtectionCheck --> SafetyReview : safetyValidated
    ProtectionCheck --> Remediation : safetyFailed
    Remediation --> FunctionalCheck : retestRequired
    SafetyReview --> LoadTesting : loadTestPerformed
    LoadTesting --> EfficiencyReview : efficiencyMeasured
    EfficiencyReview --> Certified : systemCertified
    EfficiencyReview --> Remediation : belowThreshold
    Certified --> [*]
```
