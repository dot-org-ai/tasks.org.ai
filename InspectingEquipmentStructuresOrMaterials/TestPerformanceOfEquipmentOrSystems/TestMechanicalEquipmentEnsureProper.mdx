...
```mermaid
erDiagram
    Equipment ||--o{ PerformanceTest : undergoes
    Equipment ||--o{ VibrationAnalysis : undergoes
    Equipment ||--o{ LoadTest : undergoes
    Equipment ||--o| Alignment : requires
    PerformanceTest }o--|| Certification : "contributes to"
    LoadTest }o--|| Certification : "contributes to"
    VibrationAnalysis }o--|| Certification : "contributes to"
```
...
```mermaid
stateDiagram-v2
    [*] --> OperationCheck : operationTested
    OperationCheck --> AlignmentCheck : alignmentChecked
    AlignmentCheck --> VibrationCheck : vibrationMeasured
    VibrationCheck --> Acceptable : withinLimits
    VibrationCheck --> NeedsRepair : excessiveVibration
    NeedsRepair --> OperationCheck : repairCompleted
    Acceptable --> LoadTesting : loadTestPerformed
    LoadTesting --> NoiseCheck : noiseAssessed
    NoiseCheck --> SafetyReview : safetyValidated
    SafetyReview --> Certified : equipmentCertified
    SafetyReview --> NeedsRepair : failedSafety
    Certified --> [*]
```
