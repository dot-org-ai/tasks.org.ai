...
```mermaid
erDiagram
    SamplingPlan ||--o{ InspectionRecord : "documents"
    NonConformanceReport }o--|| SamplingPlan : "documents"
    SamplingPlan ||--|{ MaterialCertification : "documents"
    SamplingPlan ||--o{ MeasurementData : "contains"
    LotDisposition }o--|| SamplingPlan : "contains"
    SamplingPlan ||--|{ QualityStandard : "documents"
    SamplingPlan ||--o{ InspectionGauge : "uses"
    InspectionRecord ||--o{ MaterialCertification : "documents"
```
...
stateDiagram-v2
    [*] --> SamplingPlanDefined
    SamplingPlanDefined --> IncomingInspectionCompleted : incomingInspectionCompleted
    IncomingInspectionCompleted --> InProcessCheckPassed : inProcessCheckPassed
    InProcessCheckPassed --> FinalInspectionCompleted : finalInspectionCompleted
    FinalInspectionCompleted --> MeasurementsRecorded : measurementsRecorded
    MeasurementsRecorded --> LotEvaluated : lotEvaluated
    LotEvaluated --> NonConformanceIssued : nonConformanceIssued
    NonConformanceIssued --> LotDisposed : lotDisposed
    MeasurementsRecorded --> NeedsReview : measurementsRecorded
    NeedsReview --> FinalInspectionCompleted : finalInspectionCompleted
    LotDisposed --> [*]
```
