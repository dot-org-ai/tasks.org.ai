...
```mermaid
erDiagram
    TestPlan ||--o{ TestResult : "documents"
    DefectReport }o--|| TestPlan : "documents"
    TestPlan ||--|{ ProductLot : "contains"
    TestPlan ||--o{ TestFixture : "uses"
    AcceptanceCriteria }o--|| TestPlan : "contains"
    TestPlan ||--|{ DispositionRecord : "documents"
    TestPlan ||--o{ CalibrationCertificate : "references"
...
stateDiagram-v2
    [*] --> TestPlanCreated
    TestPlanCreated --> FixtureReady : fixtureReady
    FixtureReady --> TestExecuted : testExecuted
    TestExecuted --> ResultEvaluated : resultEvaluated
    ResultEvaluated --> DefectLogged : defectLogged
    DefectLogged --> LotDisposed : lotDisposed
    LotDisposed --> TestReportGenerated : testReportGenerated
    TestReportGenerated --> SystematicFailureDetected : systematicFailureDetected
    DefectLogged --> NeedsReview : defectLogged
    NeedsReview --> ResultEvaluated : resultEvaluated
    SystematicFailureDetected --> [*]
```
