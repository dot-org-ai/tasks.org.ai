...
```mermaid
erDiagram
    ComplianceChecklist ||--o{ SecurityAssessment : "contains"
    SafetyInspectionReport }o--|| ComplianceChecklist : "documents"
    ComplianceChecklist ||--|{ Violation : "contains"
    ComplianceChecklist ||--o{ CorrectiveActionPlan : "contains"
    ComplianceCertificate }o--|| ComplianceChecklist : "contains"
    SecurityAssessment ||--o{ Violation : "contains"
```
...
stateDiagram-v2
    [*] --> RegulationsReviewed
    RegulationsReviewed --> SecuritySystemsInspected : securitySystemsInspected
    SecuritySystemsInspected --> LifeSafetyChecked : lifeSafetyChecked
    LifeSafetyChecked --> AlarmSystemsTested : alarmSystemsTested
    AlarmSystemsTested --> ViolationsIdentified : violationsIdentified
    ViolationsIdentified --> CorrectiveActionDeveloped : correctiveActionDeveloped
    CorrectiveActionDeveloped --> ComplianceCertified : complianceCertified
    AlarmSystemsTested --> NeedsReview : alarmSystemsTested
    NeedsReview --> LifeSafetyChecked : lifeSafetyChecked
    ComplianceCertified --> [*]
```
