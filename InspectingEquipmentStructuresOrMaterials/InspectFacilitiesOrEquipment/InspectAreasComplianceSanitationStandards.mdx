...
```mermaid
erDiagram
    SanitationReport ||--o{ ViolationCitation : "contains"
    SurfaceSwabResult }o--|| SanitationReport : "contains"
    SanitationReport ||--|{ TemperatureLog : "documents"
    SanitationReport ||--o{ HACCPRecord : "documents"
    PestActivityLog }o--|| SanitationReport : "documents"
    SanitationReport ||--|{ CorrectionOrder : "contains"
    SanitationReport ||--o{ SanitationScore : "contains"
    ViolationCitation ||--o{ TemperatureLog : "documents"
```
...
```mermaid
stateDiagram-v2
    [*] --> FoodHandlingInspected
    FoodHandlingInspected --> TemperatureComplianceChecked : temperatureComplianceChecked
    TemperatureComplianceChecked --> SurfaceCleanlinessAssessed : surfaceCleanlinessAssessed
    SurfaceCleanlinessAssessed --> HandwashingEvaluated : handwashingEvaluated
    HandwashingEvaluated --> PestControlInspected : pestControlInspected
    PestControlInspected --> WasteManagementAudited : wasteManagementAudited
    WasteManagementAudited --> CitationIssued : citationIssued
    CitationIssued --> SanitationScoreAssigned : sanitationScoreAssigned
    PestControlInspected --> NeedsReview : pestControlInspected
    NeedsReview --> HandwashingEvaluated : handwashingEvaluated
    SanitationScoreAssigned --> [*]
```
