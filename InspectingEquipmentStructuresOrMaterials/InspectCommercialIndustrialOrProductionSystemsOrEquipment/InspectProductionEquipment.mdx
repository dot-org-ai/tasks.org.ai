...
```mermaid
erDiagram
    ProductionMachine ||--o{ ToolingSet : "uses"
    InspectionChecklist }o--|| ProductionMachine : "references"
    ProductionMachine ||--|{ MachineConditionReport : "produces"
    ProductionMachine ||--o{ ProcessParameter : "contains"
    PreShiftCheckRecord }o--|| ProductionMachine : "documents"
    ProductionMachine ||--|{ DowntimeEvent : "documents"
    ToolingSet ||--o{ MachineConditionReport : "produces"
```
...
```mermaid
stateDiagram-v2
    [*] --> PreShiftCheckCompleted
    PreShiftCheckCompleted --> ToolingConditionAssessed : toolingConditionAssessed
    ToolingConditionAssessed --> ProcessParametersVerified : processParametersVerified
    ProcessParametersVerified --> FluidSystemsChecked : fluidSystemsChecked
    FluidSystemsChecked --> SafetyFeaturesInspected : safetyFeaturesInspected
    SafetyFeaturesInspected --> AlignmentMeasured : alignmentMeasured
    AlignmentMeasured --> MachineConditionDocumented : machineConditionDocumented
    MachineConditionDocumented --> ProductionAuthorized : productionAuthorized
    SafetyFeaturesInspected --> NeedsReview : safetyFeaturesInspected
    NeedsReview --> FluidSystemsChecked : fluidSystemsChecked
