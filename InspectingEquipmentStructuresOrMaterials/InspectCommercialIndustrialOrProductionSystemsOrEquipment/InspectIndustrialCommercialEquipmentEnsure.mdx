...
```mermaid
erDiagram
    IndustrialEquipment ||--o{ OperatingParameter : "contains"
    InspectionReport }o--|| IndustrialEquipment : "documents"
    IndustrialEquipment ||--|{ ServiceAgreement : "contains"
    IndustrialEquipment ||--o{ DeficiencyNotice : "uses"
    ComplianceCertificate }o--|| IndustrialEquipment : "documents"
    IndustrialEquipment ||--|{ PartsList : "contains"
    OperatingParameter ||--o{ ServiceAgreement : "contains"
```
...
stateDiagram-v2
    [*] --> InspectionScheduled
    InspectionScheduled --> VisualInspectionCompleted : visualInspectionCompleted
    VisualInspectionCompleted --> OperatingParametersMeasured : operatingParametersMeasured
    OperatingParametersMeasured --> ControlSystemsVerified : controlSystemsVerified
    ControlSystemsVerified --> LubricationChecked : lubricationChecked
    LubricationChecked --> SafetyDevicesTested : safetyDevicesTested
    SafetyDevicesTested --> DeficiencyNoticeIssued : deficiencyNoticeIssued
    DeficiencyNoticeIssued --> CertificateIssued : certificateIssued
    LubricationChecked --> NeedsReview : lubricationChecked
    NeedsReview --> ControlSystemsVerified : controlSystemsVerified
    CertificateIssued --> [*]
```
