...
```mermaid
erDiagram
    ValidationProtocol ||--o{ TestExecution : "documents"
    DeviationReport }o--|| ValidationProtocol : "documents"
    ValidationProtocol ||--|{ ValidationSummary : "produces"
    ValidationProtocol ||--o{ AcceptanceCriteria : "uses"
    ChangeControl }o--|| ValidationProtocol : "contains"
    TestExecution ||--o{ ValidationSummary : "produces"
```
...
stateDiagram-v2
    [*] --> ProtocolCreated
    ProtocolCreated --> TestExecuted : testExecuted
    TestExecuted --> DeviationRecorded : deviationRecorded
    DeviationRecorded --> ResultsAnalyzed : resultsAnalyzed
    ResultsAnalyzed --> ValidationApproved : validationApproved
    ValidationApproved --> ReportIssued : reportIssued
    ReportIssued --> RevalidationTriggered : revalidationTriggered
    ResultsAnalyzed --> NeedsReview : resultsAnalyzed
    NeedsReview --> DeviationRecorded : deviationRecorded
