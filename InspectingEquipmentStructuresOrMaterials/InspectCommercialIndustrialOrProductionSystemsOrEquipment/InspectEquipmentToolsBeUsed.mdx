...
```mermaid
erDiagram
    HeavyEquipment ||--o{ PowerTool : "uses"
    PreUseChecklist }o--|| HeavyEquipment : "uses"
    HeavyEquipment ||--|{ OperatorClearance : "uses"
    HeavyEquipment ||--o{ DeficiencyTag : "uses"
    MaintenanceRequest }o--|| HeavyEquipment : "produces"
    PowerTool ||--o{ OperatorClearance : "uses"
```
...
```mermaid
stateDiagram-v2
    [*] --> PreUseCheckCompleted
    PreUseCheckCompleted --> SafetyDevicesVerified : safetyDevicesVerified
    SafetyDevicesVerified --> DeficiencyTagged : deficiencyTagged
    DeficiencyTagged --> EquipmentCleared : equipmentCleared
    EquipmentCleared --> MaintenanceRequested : maintenanceRequested
    MaintenanceRequested --> ControlsTestPassed : controlsTestPassed
    ControlsTestPassed --> EquipmentQuarantined : equipmentQuarantined
    EquipmentCleared --> NeedsReview : equipmentCleared
    NeedsReview --> DeficiencyTagged : deficiencyTagged
    EquipmentQuarantined --> [*]
```
