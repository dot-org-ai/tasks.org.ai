...
```mermaid
erDiagram
    ElectricalSystem ||--o{ Defect : "documents"
    InspectionReport }o--|| ElectricalSystem : "documents"
    ElectricalSystem ||--|{ CircuitDiagram : "contains"
    ElectricalSystem ||--o{ ThermalImage : "contains"
    TestMeasurement }o--|| ElectricalSystem : "documents"
    ElectricalSystem ||--|{ CorrectiveAction : "documents"
    Defect ||--o{ CircuitDiagram : "contains"
```
...
```mermaid
stateDiagram-v2
    [*] --> SystemInspectionInitiated
    SystemInspectionInitiated --> VisualCheckCompleted : visualCheckCompleted
    VisualCheckCompleted --> ElectricalParametersMeasured : electricalParametersMeasured
    ElectricalParametersMeasured --> ThermalAnomalyDetected : thermalAnomalyDetected
    ThermalAnomalyDetected --> ContinuityFaultFound : continuityFaultFound
    ContinuityFaultFound --> DefectClassified : defectClassified
    DefectClassified --> CorrectiveActionPrescribed : correctiveActionPrescribed
    CorrectiveActionPrescribed --> InspectionClosed : inspectionClosed
    ContinuityFaultFound --> NeedsReview : continuityFaultFound
    NeedsReview --> ThermalAnomalyDetected : thermalAnomalyDetected
    InspectionClosed --> [*]
```
