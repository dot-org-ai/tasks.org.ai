...
```mermaid
erDiagram
    Installation ||--|| CommissioningPlan : "verified by"
    Installation ||--|| PerformanceGuarantee : "bound by"
    CommissioningPlan ||--|{ MonitoringData : "produces"
    MonitoringData ||--|{ PerformanceRatio : "calculated from"
    Installation ||--o{ DeficiencyLog : "may have"
    VerificationReport }o--|| Installation : "evaluates"
    VerificationReport }o--o{ PerformanceRatio : "references"
    CertificateOfCompletion }o--|| VerificationReport : "issued from"
```
...
stateDiagram-v2
    [*] --> Inspected
    Inspected --> PlanDeveloped : commissioningPlanDeveloped
    PlanDeveloped --> Testing : commissioningTestConducted
    Testing --> Monitoring : performanceMonitored
    Monitoring --> Calculated : performanceRatioCalculated
    Calculated --> Deficient : deficiencyIdentified
    Deficient --> Remediation : deficiencyIdentified
    Remediation --> Testing : commissioningTestConducted
    Calculated --> Verified : verificationReportIssued
    Verified --> Certified : completionCertified
    Certified --> [*]
```
