...
```mermaid
erDiagram
    CountSession ||--o{ CountRecord : "documents"
    Variance }o--|| CountSession : "contains"
    CountSession ||--|{ Adjustment : "documents"
    CountSession ||--o{ ProductionBatch : "contains"
    SKU }o--|| CountSession : "contains"
    CountRecord ||--o{ Adjustment : "documents"
```
...
stateDiagram-v2
    [*] --> CountScheduled
    CountScheduled --> CountPerformed : countPerformed
    CountPerformed --> QuantityRecorded : quantityRecorded
    QuantityRecorded --> VarianceCalculated : varianceCalculated
    VarianceCalculated --> DiscrepancyInvestigated : discrepancyInvestigated
    DiscrepancyInvestigated --> AdjustmentApproved : adjustmentApproved
    AdjustmentApproved --> ReportGenerated : reportGenerated
    VarianceCalculated --> NeedsReview : varianceCalculated
    NeedsReview --> QuantityRecorded : quantityRecorded
    ReportGenerated --> [*]
```
