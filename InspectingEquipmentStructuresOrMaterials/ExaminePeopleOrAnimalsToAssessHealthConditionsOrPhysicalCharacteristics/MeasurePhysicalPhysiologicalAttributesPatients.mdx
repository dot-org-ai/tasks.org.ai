...
```mermaid
erDiagram
    MeasurementSeries ||--|{ Measurement : "contains"
    MeasurementSeries ||--|| Baseline : "compared against"
    MeasurementSeries ||--|{ Threshold : "monitored by"
    Measurement ||--o{ Threshold : "evaluated against"
    Calibration }o--|| Measurement : "validates"
    Trend }o--|| MeasurementSeries : "derived from"
```
...
stateDiagram-v2
    [*] --> Calibrated
    Calibrated --> SeriesCreated : seriesCreated
    SeriesCreated --> BaselineEstablished : baselineSet
    BaselineEstablished --> Monitoring : measurementCaptured
    Monitoring --> ThresholdAlert : thresholdExceeded
    Monitoring --> Monitoring : measurementCaptured
    ThresholdAlert --> Monitoring : measurementCaptured
    ThresholdAlert --> TrendAnalysis : trendDetected
    Monitoring --> TrendAnalysis : trendDetected
    TrendAnalysis --> Reported : reportGenerated
    Reported --> Monitoring : measurementCaptured
    Reported --> [*]
```
...
graph TD
    ...
    G --> I[analyzeTrends]
    H --> I
    I --> J{Significant Trend?}
    J -->|Yes| K[generateReport]
    J -->|No| E
    K --> E
```
