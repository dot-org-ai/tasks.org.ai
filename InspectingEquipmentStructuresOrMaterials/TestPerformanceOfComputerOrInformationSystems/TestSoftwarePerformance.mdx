...
```mermaid
erDiagram
    Application ||--o{ LoadTest : undergoes
    Application ||--o{ StressTest : undergoes
    Application ||--o{ PerformanceProfile : generates
    LoadTest ||--|{ Bottleneck : reveals
    StressTest ||--|{ Bottleneck : reveals
    PerformanceProfile }o--|| Benchmark : "compared against"
    LoadTest }o--|| Benchmark : "measured against"
```
...
stateDiagram-v2
    ...
    BenchmarkSet --> LoadTestRunning : loadTestExecuted
    LoadTestRunning --> LatencyAnalysis : latencyMeasured
    LatencyAnalysis --> Profiling : performanceProfiled
    Profiling --> Passed : resultsCompared
    Profiling --> BottleneckFound : bottleneckIdentified
    BottleneckFound --> Optimizing : remediate
    Optimizing --> LoadTestRunning : retest
    Passed --> StressTesting : stressTestRun
    StressTesting --> Passed : resultsCompared
    StressTesting --> BottleneckFound : bottleneckIdentified
    Passed --> [*]
```
