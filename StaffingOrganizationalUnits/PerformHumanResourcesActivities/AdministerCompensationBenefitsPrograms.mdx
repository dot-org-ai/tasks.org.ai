...
```mermaid
erDiagram
    PayStructure ||--o{ CompensationPackage : "defines bands for"
    CompensationPackage ||--o{ MeritIncrease : "adjusted via"
    BenefitsPlan ||--o{ BenefitsElection : "selected in"
    RetirementPlan ||--o{ BenefitsElection : "elected via"
    OpenEnrollment ||--|{ BenefitsElection : "collects"
    CompensationPackage }o--|| PayStructure : "graded by"
    BenefitsElection }o--|| CompensationPackage : "part of"
```
...
stateDiagram-v2
    [*] --> PayStructureDefined
    PayStructureDefined --> Benchmarked : compensationBenchmarked
    Benchmarked --> PayStructureDefined : payStructureDefined
    Benchmarked --> MeritApplied : meritIncreaseProcessed
    MeritApplied --> BonusDisbursed : bonusProcessed
    BonusDisbursed --> EnrollmentOpen : enrollmentOpened
    EnrollmentOpen --> Enrolled : employeeEnrolled
    Enrolled --> Reconciled : invoiceReconciled
    Reconciled --> [*]
```
...
graph TD
    ...
    I --> J{Discrepancies?}
    J -->|Yes| I
    J -->|No| K[generateCompensationReport]
```
