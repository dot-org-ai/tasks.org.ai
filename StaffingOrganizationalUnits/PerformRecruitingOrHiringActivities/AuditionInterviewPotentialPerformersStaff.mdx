...
```mermaid
erDiagram
    AuditionSession ||--|{ CandidateProfile : "evaluates"
    AuditionSession ||--o{ AuditionRecording : "captured in"
    CandidateProfile ||--o{ InterviewSlot : "scheduled for"
    CandidateProfile ||--o{ EvaluationCriteria : "scored against"
    AuditionSession ||--|| EvaluationCriteria : "judged by"
    CandidateProfile }o--|| CastingDecision : "selected in"
```
...
stateDiagram-v2
    [*] --> AuditionScheduled
    AuditionScheduled --> ProfileReviewed : candidateProfileReviewed
    ProfileReviewed --> Auditioned : auditionConducted
    ProfileReviewed --> Interviewed : interviewConducted
    Auditioned --> Evaluated : performanceEvaluated
    Interviewed --> Evaluated : performanceEvaluated
    Evaluated --> CallbackRequested : callbackRequested
    CallbackRequested --> Auditioned : auditionConducted
    Evaluated --> DecisionMade : castingDecisionMade
    DecisionMade --> [*]
```
