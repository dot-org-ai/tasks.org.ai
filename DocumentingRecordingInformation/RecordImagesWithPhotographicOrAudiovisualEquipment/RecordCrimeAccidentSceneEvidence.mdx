...
```mermaid
erDiagram
    EvidenceRecord ||--o{ SceneReport : "contains"
    SceneReport }o--|| ChainOfCustody : "tracks"
    ChainOfCustody ||--o{ CaseFile : "references"
    CaseFile }o--|| CaptureDevice : "generates"
    CaptureDevice ||--o{ EvidenceMarker : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : sceneEvidenceCaptured
    InProgress --> InProgress : evidenceMediaCataloged
    InProgress --> InProgress : chainOfCustodyEstablished
    InProgress --> Review : sceneImageAnnotated
    Review --> InProgress
    InProgress --> InProgress : evidenceTransferred
    InProgress --> InProgress : evidenceIntegrityValidated
    InProgress --> Completed : sceneDocumentationArchived
    Completed --> [*]
```
## Workflow
