...
```mermaid
erDiagram
    Transcript ||--o{ AudioRecording : "contains"
    AudioRecording }o--|| Annotation : "tracks"
    Annotation ||--o{ SpeakerIdentification : "references"
    SpeakerIdentification }o--|| CertifiedCopy : "generates"
    CertifiedCopy ||--o{ RealTimeFeed : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : recordingStarted
    InProgress --> InProgress : speakerIdentified
    InProgress --> InProgress : eventAnnotated
    InProgress --> Review : realTimeFeedActive
    Review --> InProgress
    InProgress --> InProgress : recordingStopped
    InProgress --> InProgress : transcriptEdited
    InProgress --> Completed : transcriptCertified
    Completed --> [*]
```
## Workflow
