...
```mermaid
erDiagram
    Decision ||--o{ FindingsOfFact : "contains"
    FindingsOfFact }o--|| ConclusionsOfLaw : "tracks"
    ConclusionsOfLaw ||--o{ Order : "references"
    Order }o--|| Dissent : "generates"
    Dissent ||--o{ Citation : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : precedentResearched
    InProgress --> InProgress : decisionDrafted
    InProgress --> InProgress : draftReviewed
    InProgress --> Review : findingsOfFactRevised
    Review --> InProgress
    InProgress --> InProgress : decisionFinalized
    InProgress --> InProgress : decisionIssued
    InProgress --> Completed : decisionPublished
    Completed --> [*]
```
## Workflow
