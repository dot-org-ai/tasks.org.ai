...
```mermaid
erDiagram
    SuspectProfile ||--o{ CriminalHistory : "contains"
    CriminalHistory }o--|| BiometricRecord : "tracks"
    BiometricRecord ||--o{ Alias : "references"
    Alias }o--|| CaseAssociation : "generates"
    CaseAssociation ||--o{ FieldContact : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : suspectProfileCreated
    InProgress --> InProgress : biometricsRecorded
    InProgress --> InProgress : criminalHistoryLogged
    InProgress --> Review : aliasRegistered
    Review --> InProgress
    InProgress --> InProgress : suspectLinkedToCase
    InProgress --> InProgress : fieldContactRecorded
    InProgress --> Completed : dispositionUpdated
    Completed --> [*]
```
## Workflow
