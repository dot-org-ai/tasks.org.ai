...
```mermaid
erDiagram
    CaseFile ||--o{ Exhibit : "contains"
    Exhibit }o--|| Deposition : "tracks"
    Deposition ||--o{ CourtFiling : "references"
    CourtFiling }o--|| EvidenceItem : "generates"
    EvidenceItem ||--o{ TranscriptRecord : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : exhibitCataloged
    InProgress --> InProgress : depositionRecorded
    InProgress --> InProgress : courtDocumentFiled
    InProgress --> Review : chainOfCustodyUpdated
    Review --> InProgress
    InProgress --> InProgress : caseFileIndexed
    InProgress --> InProgress : privilegeReviewCompleted
    InProgress --> Completed : documentsProduced
    Completed --> [*]
```
## Workflow
