...
```mermaid
erDiagram
    FinancialStatement ||--o{ TaxReturn : contains
    FinancialStatement ||--o{ JournalEntry : contains
    FinancialStatement ||--o{ SupportingSchedule : follows
    FinancialStatement ||--o{ FilingDeadline : contains
    FinancialStatement ||--o{ ReviewChecklist : contains
    TrialBalance ||--o{ JournalEntry : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> DataGather : dataGathered
    DataGather --> StatementDraft : statementDrafted
    StatementDraft --> AccountsReconcil : accountsReconciled
    AccountsReconcil --> DocumentReview : documentReviewed
    DocumentReview --> DocumentApprov : documentApproved
    DocumentApprov --> DocumentFil : documentFiled
    DocumentFil --> DeadlineApproaching : deadlineApproaching
    DeadlineApproaching --> [*]
```
