...
```mermaid
erDiagram
    TransactionDocument ||--o{ PurchaseOrder : contains
    TransactionDocument ||--o{ ClosingPackage : contains
    TransactionDocument ||--o{ Exhibit : contains
    TransactionDocument ||--o{ Escrow : contains
    TransactionDocument ||--o{ ExecutionRecord : tracks
    Contract ||--o{ ClosingPackage : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> PackageAssembl : packageAssembled
    PackageAssembl --> DocumentDraft : documentDrafted
    DocumentDraft --> ExhibitAttach : exhibitAttached
    ExhibitAttach --> ReviewComplet : reviewCompleted
    ReviewComplet --> DocumentsExecut : documentsExecuted
    DocumentsExecut --> DocumentsFil : documentsFiled
    DocumentsFil --> TransactionArchiv : transactionArchived
    TransactionArchiv --> [*]
```
