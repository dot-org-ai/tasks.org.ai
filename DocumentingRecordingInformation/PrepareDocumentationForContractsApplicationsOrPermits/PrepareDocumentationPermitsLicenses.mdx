...
```mermaid
erDiagram
    Application ||--o{ License : contains
    Application ||--o{ SupportingDocument : contains
    Application ||--o{ InspectionReport : contains
    Application ||--o{ RenewalNotice : contains
    Application ||--o{ FeeSchedule : follows
    Application ||--o{ ComplianceCertificate : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> ApplicationInitializ : applicationInitialized
    ApplicationInitializ --> DocumentsAssembl : documentsAssembled
    DocumentsAssembl --> ApplicationSubmitt : applicationSubmitted
    ApplicationSubmitt --> DeficiencyNotifi : deficiencyNotified
    DeficiencyNotifi --> InspectionSchedul : inspectionScheduled
    InspectionSchedul --> PermitApprov : permitApproved
    PermitApprov --> PermitDeni : permitDenied
    PermitDeni --> RenewalDue : renewalDue
    RenewalDue --> [*]
```
