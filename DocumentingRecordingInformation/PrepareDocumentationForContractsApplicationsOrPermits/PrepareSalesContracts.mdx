...
erDiagram
    ...
    Contract ||--o{ Amendment : contains
    Contract ||--o{ Signature : contains
    Contract ||--o{ Attachment : contains
    Contract ||--o{ ApprovalRecord : tracks
    Contract ||--o{ PaymentSchedule : follows
```
...
stateDiagram-v2
    [*] --> Start
    Start --> ContractDraft : contractDrafted
    ContractDraft --> ContractSubmittedForReview : contractSubmittedForReview
    ContractSubmittedForReview --> TermsNegotiat : termsNegotiated
    TermsNegotiat --> ContractApprov : contractApproved
    ContractApprov --> ContractExecut : contractExecuted
    ContractExecut --> ContractAmen : contractAmended
    ContractAmen --> ContractExpiring : contractExpiring
    ContractExpiring --> [*]
```
