...
```mermaid
erDiagram
    ResearchReport ||--|| StudyProtocol : "documents"
    ResearchReport ||--|| LiteratureReview : "includes"
    ResearchReport ||--o{ DataCollection : "analyzes"
    ResearchReport ||--o{ Finding : "presents"
    ResearchReport ||--o{ Appendix : "supplemented-by"
    StudyProtocol ||--o{ DataCollection : "guides"
```
...
stateDiagram-v2
    [*] --> Structuring
    Structuring --> LiteraturePhase : reportOutlined
    LiteraturePhase --> MethodologyPhase : literatureReviewCompiled
    MethodologyPhase --> FindingsPhase : methodologySummarized
    FindingsPhase --> DraftingPhase : findingsSynthesized
    DraftingPhase --> PeerReview : reportDrafted
    PeerReview --> Revision : reportSubmittedForReview
    Revision --> DraftingPhase
    PeerReview --> Finalized : reportSubmittedForReview
    Finalized --> [*]
```
...
graph TD
    ...
    I --> J{Publication Required?}
    J -->|Yes| K[Submit to Journal]
    J -->|No| L[Archive]
```
