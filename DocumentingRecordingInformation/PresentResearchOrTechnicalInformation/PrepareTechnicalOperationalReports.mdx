...
```mermaid
erDiagram
    OperationalReport ||--o{ MetricSet : "contains"
    MetricSet }o--|| IncidentSummary : "tracks"
    IncidentSummary ||--o{ ReportSchedule : "references"
    ReportSchedule }o--|| TechnicalDiagram : "generates"
    TechnicalDiagram ||--o{ DistributionList : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : reportParametersDefined
    InProgress --> InProgress : operationalDataCollected
    InProgress --> InProgress : reportComposed
    InProgress --> Review : diagramsEmbedded
    Review --> InProgress
    InProgress --> InProgress : draftReviewed
    InProgress --> InProgress : reportApproved
    InProgress --> Completed : reportDistributed
    Completed --> [*]
```
## Workflow
