...
```mermaid
erDiagram
    AnalyticalReport ||--|| AnalysisScope : "defined-by"
    AnalysisScope ||--o{ DataSource : "uses"
    AnalyticalReport ||--o{ Finding : "presents"
    AnalyticalReport ||--o{ Visualization : "contains"
    ReportDistribution }o--|| AnalyticalReport : "delivers"
    Finding }o--o{ Visualization : "illustrated-by"
```
...
stateDiagram-v2
    [*] --> ScopeDefinition
    ScopeDefinition --> DataAggregation : scopeDefined
    DataAggregation --> Analysis : dataAggregated
    Analysis --> Drafting : analysisExecuted
    Drafting --> Visualization : findingsDrafted
    Visualization --> QualityReview : visualizationsGenerated
    QualityReview --> Revision : reportReviewed
    Revision --> Drafting
    QualityReview --> Distribution : reportReviewed
    Distribution --> [*]
```
...
graph TD
    ...
    E --> F[reviewReport]
    F --> G{Approved?}
    G -->|No| H[Revise Findings]
    H --> D
    G -->|Yes| I[distributeReport]
    I --> J{Additional Analysis Needed?}
    J -->|Yes| A
    J -->|No| K[Archive]
```
