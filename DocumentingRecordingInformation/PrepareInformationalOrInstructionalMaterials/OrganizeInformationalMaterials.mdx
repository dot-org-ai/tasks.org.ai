...
```mermaid
erDiagram
    InformationalMaterial ||--o{ Taxonomy : contains
    InformationalMaterial ||--o{ ContentIndex : contains
    InformationalMaterial ||--o{ DistributionPackage : contains
    InformationalMaterial ||--o{ VersionRecord : tracks
    ContentCollection ||--o{ ContentIndex : contains
    ContentCollection ||--o{ DistributionPackage : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> MaterialClassifi : materialClassified
    MaterialClassifi --> CollectionStructur : collectionStructured
    CollectionStructur --> MaterialsIndex : materialsIndexed
    MaterialsIndex --> OrganizationReview : organizationReviewed
    OrganizationReview --> DistributionBundl : distributionBundled
    DistributionBundl --> MaterialArchiv : materialArchived
    MaterialArchiv --> ClassificationUpdat : classificationUpdated
    ClassificationUpdat --> [*]
```
