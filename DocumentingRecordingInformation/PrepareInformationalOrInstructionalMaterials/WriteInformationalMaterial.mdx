...
```mermaid
erDiagram
    ContentBrief ||--o{ PublishedMaterial : contains
    ContentBrief ||--o{ EditorialComment : contains
    ContentBrief ||--o{ StyleGuide : contains
    ContentBrief ||--o{ ContentAsset : contains
    Draft ||--o{ EditorialComment : contains
    Draft ||--o{ StyleGuide : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> ContentBriefCreat : contentBriefCreated
    ContentBriefCreat --> MaterialDraft : materialDrafted
    MaterialDraft --> DraftSubmittedForReview : draftSubmittedForReview
    DraftSubmittedForReview --> DraftRevis : draftRevised
    DraftRevis --> MaterialApprov : materialApproved
    MaterialApprov --> MaterialPublish : materialPublished
    MaterialPublish --> MaterialRetir : materialRetired
    MaterialRetir --> [*]
```
