...
```mermaid
erDiagram
    TrainingCourse ||--o{ Exercise : contains
    TrainingCourse ||--o{ Assessment : contains
    TrainingCourse ||--o{ SlideDecK : contains
    TrainingCourse ||--o{ Handout : contains
    TrainingCourse ||--o{ TrainingPlan : follows
    TrainingCourse ||--o{ CompetencyFramework : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> CompetenciesDefin : competenciesDefined
    CompetenciesDefin --> CourseCreat : courseCreated
    CourseCreat --> LessonAuthor : lessonAuthored
    LessonAuthor --> ExerciseBuilt : exerciseBuilt
    ExerciseBuilt --> AssessmentCreat : assessmentCreated
    AssessmentCreat --> MaterialsReview : materialsReviewed
    MaterialsReview --> CoursePublish : coursePublished
    CoursePublish --> CourseRetir : courseRetired
    CourseRetir --> [*]
```
