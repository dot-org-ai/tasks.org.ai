...
```mermaid
erDiagram
    ClientRecord ||--o{ ProgressNote : contains
    ClientRecord ||--o{ ConsentForm : contains
    ClientRecord ||--o{ ServicePlan : follows
    ClientRecord ||--o{ AssessmentReport : contains
    ClientRecord ||--o{ DischargeSum : contains
    IntakeForm ||--o{ ConsentForm : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> ClientRecordCreat : clientRecordCreated
    ClientRecordCreat --> RecordUpdat : recordUpdated
    RecordUpdat --> ProgressNoteRecor : progressNoteRecorded
    ProgressNoteRecor --> ConsentExpiring : consentExpiring
    ConsentExpiring --> AssessmentComplet : assessmentCompleted
    AssessmentComplet --> ClientDischarg : clientDischarged
    ClientDischarg --> RecordArchiv : recordArchived
    RecordArchiv --> [*]
```
