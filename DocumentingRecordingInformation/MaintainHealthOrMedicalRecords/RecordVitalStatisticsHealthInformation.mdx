...
```mermaid
erDiagram
    VitalSignRecord ||--o{ PatientHealthRecord : tracks
    VitalSignRecord ||--o{ BiometricMeasurement : contains
    VitalSignRecord ||--o{ ClinicalObservation : contains
    VitalSignRecord ||--o{ AlertThreshold : contains
    VitalSignRecord ||--o{ HealthDataAuditTrail : contains
    PatientHealthRecord ||--o{ ClinicalObservation : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> VitalSignsRecor : vitalSignsRecorded
    VitalSignsRecor --> ClinicalObservationRecor : clinicalObservationRecorded
    ClinicalObservationRecor --> HealthDataValidat : healthDataValidated
    HealthDataValidat --> AbnormalVitalsFlagg : abnormalVitalsFlagged
    AbnormalVitalsFlagg --> PatientRecordUpdat : patientRecordUpdated
    PatientRecordUpdat --> HealthSummaryGenerat : healthSummaryGenerated
    HealthSummaryGenerat --> HealthRecordsAudit : healthRecordsAudited
    HealthRecordsAudit --> [*]
```
