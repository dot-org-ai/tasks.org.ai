...
erDiagram
    ...
    Transaction ||--o{ Discount : contains
    Transaction ||--o{ TaxEntry : contains
    LineItem ||--o{ Receipt : contains
    LineItem ||--o{ Discount : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> SaleRecor : saleRecorded
    SaleRecor --> PaymentProcess : paymentProcessed
    PaymentProcess --> DiscountAppli : discountApplied
    DiscountAppli --> ReceiptGenerat : receiptGenerated
    ReceiptGenerat --> TransactionVoi : transactionVoided
    TransactionVoi --> ReturnProcess : returnProcessed
    ReturnProcess --> DailyRegisterClos : dailyRegisterClosed
    DailyRegisterClos --> [*]
```
