...
```mermaid
erDiagram
    CaseFile ||--o{ Pleading : "contains"
    CaseFile ||--o{ EvidencePackage : "includes"
    CaseFile ||--o{ InvestigationReport : "has"
    CaseFile ||--o{ Deposition : "holds"
    EvidencePackage ||--o{ ChainOfCustody : "tracked-by"
    Pleading ||--o{ FilingReceipt : "confirmed-by"
    Subpoena }o--|| CaseFile : "relates-to"
```
...
stateDiagram-v2
    [*] --> FileOpened
    FileOpened --> EvidenceCollection : caseFileOpened
    EvidenceCollection --> DraftingPhase : evidenceCompiled
    DraftingPhase --> InternalReview : pleadingDrafted
    InternalReview --> RevisionRequired : documentsReviewed
    RevisionRequired --> DraftingPhase
    InternalReview --> FiledWithCourt : documentsFiled
    FiledWithCourt --> DiscoveryPhase : discoveryResponded
    DiscoveryPhase --> FiledWithCourt
    FiledWithCourt --> Archived : caseFileSealed
    Archived --> [*]
```
