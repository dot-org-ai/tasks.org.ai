...
```mermaid
erDiagram
    LegalRecord ||--o{ ComplianceDetermination : contains
    LegalRecord ||--o{ LegalOpinion : contains
    LegalRecord ||--o{ RegulatoryCorrespondence : contains
    LegalRecord ||--o{ KnowledgeEntry : contains
    LegalRecord ||--o{ ChangeAlert : contains
    RegulatoryChange ||--o{ LegalOpinion : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> LegalEventRecor : legalEventRecorded
    LegalEventRecor --> RegulatoryChangeLogg : regulatoryChangeLogged
    RegulatoryChangeLogg --> OpinionCatalog : opinionCataloged
    OpinionCatalog --> CorrespondenceDocument : correspondenceDocumented
    CorrespondenceDocument --> RecordClassifi : recordClassified
    RecordClassifi --> AlertPublish : alertPublished
    AlertPublish --> RecordArchiv : recordArchived
    RecordArchiv --> [*]
```
