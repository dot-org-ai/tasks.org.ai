...
```mermaid
erDiagram
    Policy ||--o{ Procedure : "implements"
    Policy ||--o{ ControlMatrix : "mapped-in"
    RegulatoryRequirement ||--o{ ControlMatrix : "tracked-in"
    FilingPackage ||--o{ Policy : "includes"
    FilingPackage ||--o{ Attestation : "contains"
    FilingPackage ||--o{ EvidenceArtifact : "supports"
    ComplianceCalendar }o--|| FilingPackage : "schedules"
```
...
stateDiagram-v2
    ...
    RequirementsAnalysis --> PolicyDrafting : requirementsAnalyzed
    PolicyDrafting --> ProcedureCreation : policyDrafted
    ProcedureCreation --> ControlMapping : procedureCreated
    ControlMapping --> PackageAssembly : controlsMapped
    PackageAssembly --> ValidationCheck : filingPackageAssembled
    ValidationCheck --> GapRemediation : documentationValidated
    GapRemediation --> PackageAssembly
    ValidationCheck --> AttestationGeneration : documentationValidated
    AttestationGeneration --> Submitted : filingSubmitted
    Submitted --> [*]
```
