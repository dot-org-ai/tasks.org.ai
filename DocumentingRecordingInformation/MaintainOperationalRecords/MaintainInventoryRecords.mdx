...
```mermaid
erDiagram
    InventoryItem ||--o{ ReceiptRecord : tracks
    InventoryItem ||--o{ IssueRecord : tracks
    InventoryItem ||--o{ CycleCount : contains
    InventoryItem ||--o{ AdjustmentEntry : contains
    InventoryItem ||--o{ ValuationReport : contains
    StockLevel ||--o{ ReceiptRecord : tracks
```
...
stateDiagram-v2
    ...
    ReceiptRecor --> IssueRecor : issueRecorded
    IssueRecor --> CycleCountComplet : cycleCountCompleted
    CycleCountComplet --> AdjustmentProcess : adjustmentProcessed
    AdjustmentProcess --> InventoryReconcil : inventoryReconciled
    InventoryReconcil --> ReorderPointReach : reorderPointReached
    ReorderPointReach --> ValuationGenerat : valuationGenerated
    ValuationGenerat --> [*]
```
