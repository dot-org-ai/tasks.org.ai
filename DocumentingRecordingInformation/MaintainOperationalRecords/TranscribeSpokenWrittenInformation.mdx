...
```mermaid
erDiagram
    TranscriptionJob ||--o{ AudioRecording : tracks
    TranscriptionJob ||--o{ SourceDocument : contains
    TranscriptionJob ||--o{ Transcript : contains
    TranscriptionJob ||--o{ StyleGuide : contains
    TranscriptionJob ||--o{ QualityReport : contains
    TranscriptionJob ||--o{ Glossary : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> JobSubmitt : jobSubmitted
    JobSubmitt --> TranscriptionComplet : transcriptionCompleted
    TranscriptionComplet --> AccuracyReview : accuracyReviewed
    AccuracyReview --> FormattingAppli : formattingApplied
    FormattingAppli --> TranscriptDeliver : transcriptDelivered
    TranscriptDeliver --> RevisionRequest : revisionRequested
    RevisionRequest --> TurnaroundExcee : turnaroundExceeded
    TurnaroundExcee --> [*]
```
