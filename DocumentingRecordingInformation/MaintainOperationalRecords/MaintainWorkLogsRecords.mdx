...
```mermaid
erDiagram
    WorkLog ||--o{ LogEntry : tracks
    WorkLog ||--o{ ShiftHandover : contains
    WorkLog ||--o{ IncidentNote : contains
    WorkLog ||--o{ TaskCompletion : contains
    WorkLog ||--o{ ActivitySummary : contains
    WorkLog ||--o{ MilestoneRecord : tracks
```
...
stateDiagram-v2
    [*] --> Start
    Start --> LogEntryCreat : logEntryCreated
    LogEntryCreat --> TaskComplet : taskCompleted
    TaskComplet --> IncidentNot : incidentNoted
    IncidentNot --> HandoverPrepar : handoverPrepared
    HandoverPrepar --> LogReview : logReviewed
    LogReview --> SummaryGenerat : summaryGenerated
    SummaryGenerat --> LogArchiv : logArchived
    LogArchiv --> [*]
```
