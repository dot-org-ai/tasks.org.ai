...
```mermaid
erDiagram
    DataRecord ||--o{ ValidationRule : contains
    DataRecord ||--o{ DuplicateCluster : contains
    DataRecord ||--o{ AuditTrail : contains
    DataRecord ||--o{ ArchivePolicy : implements
    DataRecord ||--o{ DataQualityScore : contains
    DataDictionary ||--o{ DuplicateCluster : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> DataEnter : dataEntered
    DataEnter --> RecordUpdat : recordUpdated
    RecordUpdat --> DataValidat : dataValidated
    DataValidat --> DuplicatesDetect : duplicatesDetected
    DuplicatesDetect --> DataCleans : dataCleansed
    DataCleans --> DataArchiv : dataArchived
    DataArchiv --> DataQualityDegra : dataQualityDegraded
    DataQualityDegra --> UnauthorizedChangeDetect : unauthorizedChangeDetected
    UnauthorizedChangeDetect --> [*]
```
