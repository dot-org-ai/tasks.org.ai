...
```mermaid
erDiagram
    CallRecord ||--o{ RetentionPolicy : implements
    CallRecord ||--o{ CallDisposition : contains
    CallRecord ||--o{ ArchiveBatch : contains
    CallCategory ||--o{ RetentionPolicy : implements
    CallCategory ||--o{ CallDisposition : contains
    CallCategory ||--o{ ArchiveBatch : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> CallLogg : callLogged
    CallLogg --> CallCategoriz : callCategorized
    CallCategoriz --> DispositionUpdat : dispositionUpdated
    DispositionUpdat --> RecordsArchiv : recordsArchived
    RecordsArchiv --> ExpiredRecordsPurg : expiredRecordsPurged
    ExpiredRecordsPurg --> RetentionPolicyAppli : retentionPolicyApplied
    RetentionPolicyAppli --> CallDataExport : callDataExported
    CallDataExport --> [*]
```
