...
```mermaid
erDiagram
    ProductionOrder ||--o{ OutputRecord : tracks
    ProductionOrder ||--o{ MaterialConsumption : contains
    ProductionOrder ||--o{ YieldCalculation : contains
    ProductionOrder ||--o{ ScrapRecord : tracks
    ProductionOrder ||--o{ ReworkOrder : contains
    ProductionOrder ||--o{ ProductionReport : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> OutputRecor : outputRecorded
    OutputRecor --> MaterialConsumptionLogg : materialConsumptionLogged
    MaterialConsumptionLogg --> YieldCalculat : yieldCalculated
    YieldCalculat --> ScrapDocument : scrapDocumented
    ScrapDocument --> ReworkInitiat : reworkInitiated
    ReworkInitiat --> BomVarianceDetect : bomVarianceDetected
    BomVarianceDetect --> ProductionReportGenerat : productionReportGenerated
    ProductionReportGenerat --> [*]
```
