...
```mermaid
erDiagram
    ProductionLog ||--o{ OutputRecord : tracks
    ProductionLog ||--o{ DowntimeEntry : contains
    ProductionLog ||--o{ ChangeoverRecord : tracks
    ProductionLog ||--o{ QualityCheckpoint : contains
    ProductionLog ||--o{ ShiftSummary : contains
    ProductionLog ||--o{ ProductionTarget : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> ShiftLogStart : shiftLogStarted
    ShiftLogStart --> OutputRecor : outputRecorded
    OutputRecor --> DowntimeLogg : downtimeLogged
    DowntimeLogg --> ChangeoverDocument : changeoverDocumented
    ChangeoverDocument --> QualityCheckRecor : qualityCheckRecorded
    QualityCheckRecor --> ShiftSummaryGenerat : shiftSummaryGenerated
    ShiftSummaryGenerat --> TargetMiss : targetMissed
    TargetMiss --> [*]
```
