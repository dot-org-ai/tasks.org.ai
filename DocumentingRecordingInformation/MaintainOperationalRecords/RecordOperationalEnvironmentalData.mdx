...
```mermaid
erDiagram
    DataReading ||--o{ CalibrationRecord : tracks
    DataReading ||--o{ ComplianceReport : contains
    DataReading ||--o{ Threshold : contains
    DataReading ||--o{ DataLog : tracks
    MonitoringStation ||--o{ CalibrationRecord : tracks
    MonitoringStation ||--o{ ComplianceReport : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> ReadingCaptur : readingCaptured
    ReadingCaptur --> ReadingsBatchImport : readingsBatchImported
    ReadingsBatchImport --> DataValidat : dataValidated
    DataValidat --> AnomalyFlagg : anomalyFlagged
    AnomalyFlagg --> ComplianceReportGenerat : complianceReportGenerated
    ComplianceReportGenerat --> InstrumentCalibrat : instrumentCalibrated
    InstrumentCalibrat --> DataLogArchiv : dataLogArchived
    DataLogArchiv --> [*]
```
