...
```mermaid
erDiagram
    ExperimentRecord ||--o{ FieldObservation : contains
    ExperimentRecord ||--o{ Dataset : contains
    ExperimentRecord ||--o{ CalibrationLog : tracks
    ExperimentRecord ||--o{ DataManagementPlan : follows
    ExperimentRecord ||--o{ ResearchNotebook : contains
    DataPoint ||--o{ Dataset : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> MeasurementRecor : measurementRecorded
    MeasurementRecor --> ObservationLogg : observationLogged
    ObservationLogg --> DatasetCreat : datasetCreated
    DatasetCreat --> DataValidat : dataValidated
    DataValidat --> AnomalyDetect : anomalyDetected
    AnomalyDetect --> DatasetArchiv : datasetArchived
    DatasetArchiv --> DataExport : dataExported
    DataExport --> [*]
```
