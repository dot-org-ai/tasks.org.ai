...
```mermaid
erDiagram
    WorkInstruction ||--o{ SafetyProcedure : contains
    WorkInstruction ||--o{ ProcedureRevision : contains
    WorkInstruction ||--o{ StepDiagram : contains
    WorkInstruction ||--o{ ReferenceDocument : contains
    WorkInstruction ||--o{ TrainingCheckoff : contains
    OperatingProcedure ||--o{ ProcedureRevision : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> ProcessCaptur : processCaptured
    ProcessCaptur --> StepsWritten : stepsWritten
    StepsWritten --> VisualAidsAd : visualAidsAdded
    VisualAidsAd --> ProcedureValidat : procedureValidated
    ProcedureValidat --> ProcedureApprov : procedureApproved
    ProcedureApprov --> ProcedureIssu : procedureIssued
    ProcedureIssu --> StepsRevis : stepsRevised
    StepsRevis --> [*]
```
