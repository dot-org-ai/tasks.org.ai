...
```mermaid
erDiagram
    TimeEntry ||--o{ OvertimeRecord : tracks
    TimeEntry ||--o{ ProjectAllocation : contains
    TimeEntry ||--o{ TimeOffRequest : contains
    TimeEntry ||--o{ BreakRecord : tracks
    TimeEntry ||--o{ LaborCostReport : contains
    Timesheet ||--o{ OvertimeRecord : tracks
```
...
stateDiagram-v2
    [*] --> Start
    Start --> TimeEntryRecor : timeEntryRecorded
    TimeEntryRecor --> TimesheetSubmitt : timesheetSubmitted
    TimesheetSubmitt --> TimesheetApprov : timesheetApproved
    TimesheetApprov --> TimesheetReject : timesheetRejected
    TimesheetReject --> OvertimeCalculat : overtimeCalculated
    OvertimeCalculat --> OvertimeThresholdApproaching : overtimeThresholdApproaching
    OvertimeThresholdApproaching --> TimeOffRequest : timeOffRequested
    TimeOffRequest --> LaborReportGenerat : laborReportGenerated
    LaborReportGenerat --> [*]
```
