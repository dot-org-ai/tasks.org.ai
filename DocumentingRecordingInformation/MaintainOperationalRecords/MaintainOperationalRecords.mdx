...
```mermaid
erDiagram
    OperationalRecord ||--o{ RecordCategory : tracks
    OperationalRecord ||--o{ RetentionSchedule : follows
    OperationalRecord ||--o{ LitigationHold : contains
    OperationalRecord ||--o{ DispositionRequest : contains
    OperationalRecord ||--o{ AuditTrail : contains
    OperationalRecord ||--o{ StorageLocation : contains
```
...
stateDiagram-v2
    ...
    RecordCreat --> RecordClassifi : recordClassified
    RecordClassifi --> RecordUpdat : recordUpdated
    RecordUpdat --> RetentionExpiring : retentionExpiring
    RetentionExpiring --> HoldPlac : holdPlaced
    HoldPlac --> HoldReleas : holdReleased
    HoldReleas --> RecordDispos : recordDisposed
    RecordDispos --> ComplianceViolationDetect : complianceViolationDetected
    ComplianceViolationDetect --> [*]
```
