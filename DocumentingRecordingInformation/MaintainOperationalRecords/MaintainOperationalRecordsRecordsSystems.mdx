...
```mermaid
erDiagram
    Record ||--o{ RecordClass : tracks
    Record ||--o{ RetentionSchedule : follows
    Record ||--o{ RecordsSystem : tracks
    Record ||--o{ LitigationHold : contains
    Record ||--o{ DispositionCertificate : contains
    RecordClass ||--o{ RetentionSchedule : follows
```
...
stateDiagram-v2
    ...
    RecordClassifi --> RecordStor : recordStored
    RecordStor --> RecordRetriev : recordRetrieved
    RecordRetriev --> RetentionPolicyAppli : retentionPolicyApplied
    RetentionPolicyAppli --> LitigationHoldPlac : litigationHoldPlaced
    LitigationHoldPlac --> RecordsDispos : recordsDisposed
    RecordsDispos --> [*]
```
