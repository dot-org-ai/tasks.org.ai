...
```mermaid
erDiagram
    DocumentRecord ||--o{ RetentionSchedule : follows
    DocumentRecord ||--o{ LitigationHold : contains
    DocumentRecord ||--o{ AccessLog : tracks
    DocumentRecord ||--o{ DisposalCertificate : contains
    DocumentRecord ||--o{ FileIndex : contains
    FileCategory ||--o{ RetentionSchedule : follows
```
...
stateDiagram-v2
    ...
    DocumentClassifi --> DocumentFil : documentFiled
    DocumentFil --> DocumentRetriev : documentRetrieved
    DocumentRetriev --> DocumentUpdat : documentUpdated
    DocumentUpdat --> RetentionScheduleAppli : retentionScheduleApplied
    RetentionScheduleAppli --> HoldPlac : holdPlaced
    HoldPlac --> RecordsDispos : recordsDisposed
    RecordsDispos --> RetentionExpiring : retentionExpiring
    RetentionExpiring --> [*]
```
