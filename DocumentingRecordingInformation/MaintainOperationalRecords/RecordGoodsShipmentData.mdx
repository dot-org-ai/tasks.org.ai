...
```mermaid
erDiagram
    ShipmentRecord ||--o{ CommercialInvoice : contains
    ShipmentRecord ||--o{ TrackingMilestone : contains
    ShipmentRecord ||--o{ CustomsDeclaration : contains
    ShipmentRecord ||--o{ FreightInvoice : contains
    ShipmentRecord ||--o{ PackingList : contains
    BillOfLading ||--o{ TrackingMilestone : contains
```
...
stateDiagram-v2
    ...
    ShipmentCreat --> BillOfLadingGenerat : billOfLadingGenerated
    BillOfLadingGenerat --> CustomsDocumentsPrepar : customsDocumentsPrepared
    CustomsDocumentsPrepar --> MilestoneLogg : milestoneLogged
    MilestoneLogg --> DeliveryConfirm : deliveryConfirmed
    DeliveryConfirm --> FreightReconcil : freightReconciled
    FreightReconcil --> ShipmentDelay : shipmentDelayed
    ShipmentDelay --> [*]
```
