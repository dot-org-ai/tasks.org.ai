...
```mermaid
erDiagram
    MeetingMinutes ||--o{ Agenda : contains
    MeetingMinutes ||--o{ ActionItem : contains
    MeetingMinutes ||--o{ Resolution : contains
    MeetingMinutes ||--o{ AttendanceRecord : tracks
    Transcript ||--o{ ActionItem : contains
    Transcript ||--o{ Resolution : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> ProceedingsCaptur : proceedingsCaptured
    ProceedingsCaptur --> MinutesDraft : minutesDrafted
    MinutesDraft --> RecordingTranscrib : recordingTranscribed
    RecordingTranscrib --> MinutesApprov : minutesApproved
    MinutesApprov --> MinutesDistribut : minutesDistributed
    MinutesDistribut --> ProceedingsArchiv : proceedingsArchived
    ProceedingsArchiv --> ActionItemsLogg : actionItemsLogged
    ActionItemsLogg --> [*]
```
