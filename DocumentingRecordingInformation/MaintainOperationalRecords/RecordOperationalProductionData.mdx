...
```mermaid
erDiagram
    ProductionLog ||--o{ QualityMeasurement : contains
    ProductionLog ||--o{ DowntimeEvent : contains
    ProductionLog ||--o{ BatchRecord : tracks
    ProductionLog ||--o{ ShiftReport : contains
    ProductionLog ||--o{ YieldMetric : contains
    MachineParameter ||--o{ DowntimeEvent : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> OutputLogg : outputLogged
    OutputLogg --> ParametersRecor : parametersRecorded
    ParametersRecor --> DowntimeLogg : downtimeLogged
    DowntimeLogg --> QualityDataRecor : qualityDataRecorded
    QualityDataRecor --> ShiftReportGenerat : shiftReportGenerated
    ShiftReportGenerat --> DataValidat : dataValidated
    DataValidat --> ThresholdExcee : thresholdExceeded
    ThresholdExcee --> [*]
```
