...
```mermaid
erDiagram
    BatchRecord ||--o{ RawMaterialLog : tracks
    BatchRecord ||--o{ EquipmentReading : contains
    BatchRecord ||--o{ DeviationReport : contains
    BatchRecord ||--o{ LotTraceabilityRecord : tracks
    BatchRecord ||--o{ BatchReleaseDecision : contains
    InProcessTest ||--o{ RawMaterialLog : tracks
```
...
stateDiagram-v2
    [*] --> Start
    Start --> BatchRecordInitializ : batchRecordInitialized
    BatchRecordInitializ --> ProcessStepRecor : processStepRecorded
    ProcessStepRecor --> InProcessTestLogg : inProcessTestLogged
    InProcessTestLogg --> MaterialUsageDocument : materialUsageDocumented
    MaterialUsageDocument --> DeviationReport : deviationReported
    DeviationReport --> BatchRecordReview : batchRecordReviewed
    BatchRecordReview --> BatchReleas : batchReleased
    BatchReleas --> [*]
```
