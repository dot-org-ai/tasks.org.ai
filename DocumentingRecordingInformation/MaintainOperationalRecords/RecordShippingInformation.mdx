...
```mermaid
erDiagram
    ShipmentRecord ||--o{ BillOfLading : contains
    ShipmentRecord ||--o{ PackingSlip : contains
    ShipmentRecord ||--o{ CustomsDeclaration : contains
    ShipmentRecord ||--o{ FreightInvoice : contains
    ShipmentRecord ||--o{ ProofOfDelivery : contains
    TrackingEntry ||--o{ PackingSlip : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> ShipmentCreat : shipmentCreated
    ShipmentCreat --> PackageDetailsRecor : packageDetailsRecorded
    PackageDetailsRecor --> CarrierAssign : carrierAssigned
    CarrierAssign --> TrackingCaptur : trackingCaptured
    TrackingCaptur --> CustomsDocsComplet : customsDocsCompleted
    CustomsDocsComplet --> TransitUpdateLogg : transitUpdateLogged
    TransitUpdateLogg --> DeliveryConfirm : deliveryConfirmed
    DeliveryConfirm --> ShipmentException : shipmentException
    ShipmentException --> [*]
```
