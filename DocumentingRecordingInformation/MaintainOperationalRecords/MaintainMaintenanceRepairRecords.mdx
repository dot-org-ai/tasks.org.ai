...
```mermaid
erDiagram
    WorkOrder ||--o{ ServiceLog : tracks
    WorkOrder ||--o{ PartsUsageRecord : tracks
    WorkOrder ||--o{ LaborEntry : contains
    WorkOrder ||--o{ WarrantyClaim : contains
    WorkOrder ||--o{ PreventiveMaintenanceSchedule : follows
    WorkOrder ||--o{ FailureReport : contains
```
...
stateDiagram-v2
    ...
    WorkOrderCreat --> ServiceDocument : serviceDocumented
    ServiceDocument --> PartsUsageLogg : partsUsageLogged
    PartsUsageLogg --> LaborRecor : laborRecorded
    LaborRecor --> WarrantyClaimFil : warrantyClaimFiled
    WarrantyClaimFil --> PreventiveMaintenanceDue : preventiveMaintenanceDue
    PreventiveMaintenanceDue --> FailureAnalyz : failureAnalyzed
    FailureAnalyz --> [*]
```
