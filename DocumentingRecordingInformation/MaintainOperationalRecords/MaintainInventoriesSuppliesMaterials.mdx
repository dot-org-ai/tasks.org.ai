...
```mermaid
erDiagram
    SupplyItem ||--o{ IssuanceRecord : tracks
    SupplyItem ||--o{ ParLevel : contains
    SupplyItem ||--o{ PhysicalCount : contains
    SupplyItem ||--o{ ConsumptionReport : contains
    SupplyItem ||--o{ ReorderRequest : contains
    StockReceipt ||--o{ IssuanceRecord : tracks
```
...
stateDiagram-v2
    [*] --> Start
    Start --> ReceiptRecor : receiptRecorded
    ReceiptRecor --> SuppliesIssu : suppliesIssued
    SuppliesIssu --> StockAdjust : stockAdjusted
    StockAdjust --> ParLevelBreach : parLevelBreached
    ParLevelBreach --> PhysicalCountComplet : physicalCountCompleted
    PhysicalCountComplet --> ConsumptionReportGenerat : consumptionReportGenerated
    ConsumptionReportGenerat --> ReorderInitiat : reorderInitiated
    ReorderInitiat --> [*]
```
