...
```mermaid
erDiagram
    TripLog ||--o{ HoursOfServiceRecord : tracks
    TripLog ||--o{ FuelRecord : tracks
    TripLog ||--o{ RouteManifest : contains
    TripLog ||--o{ VehicleInspectionReport : contains
    TripLog ||--o{ MileageEntry : contains
    HoursOfServiceRecord ||--o{ FuelRecord : tracks
```
...
stateDiagram-v2
    [*] --> Start
    Start --> TripLogg : tripLogged
    TripLogg --> FuelConsumptionRecor : fuelConsumptionRecorded
    FuelConsumptionRecor --> DriverHoursLogg : driverHoursLogged
    DriverHoursLogg --> InspectionReportSubmitt : inspectionReportSubmitted
    InspectionReportSubmitt --> HoursLimitApproaching : hoursLimitApproaching
    HoursLimitApproaching --> TravelSummaryGenerat : travelSummaryGenerated
    TravelSummaryGenerat --> [*]
```
