...
```mermaid
erDiagram
    CustomerAccount ||--o{ PaymentRecord : tracks
    CustomerAccount ||--o{ CreditMemo : contains
    CustomerAccount ||--o{ AccountStatement : contains
    CustomerAccount ||--o{ AgingBucket : contains
    CustomerAccount ||--o{ CreditTerms : contains
...
stateDiagram-v2
    [*] --> Start
    Start --> AccountCreat : accountCreated
    AccountCreat --> TransactionRecor : transactionRecorded
    TransactionRecor --> InvoiceGenerat : invoiceGenerated
    InvoiceGenerat --> PaymentAppli : paymentApplied
    PaymentAppli --> CreditMemoIssu : creditMemoIssued
    CreditMemoIssu --> AccountReconcil : accountReconciled
    AccountReconcil --> PaymentOverdue : paymentOverdue
    PaymentOverdue --> CreditLimitExcee : creditLimitExceeded
    CreditLimitExcee --> [*]
```
