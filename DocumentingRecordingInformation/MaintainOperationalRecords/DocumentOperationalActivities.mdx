...
```mermaid
erDiagram
    ShiftLog ||--o{ ActivityEntry : contains
    ShiftLog ||--o{ DailyBriefing : contains
    ShiftLog ||--o{ DecisionJournal : contains
    ShiftLog ||--o{ ProductionRunRecord : tracks
    ShiftLog ||--o{ ObservationNote : contains
    IncidentReport ||--o{ DailyBriefing : contains
```
...
stateDiagram-v2
    ...
    ShiftLogStart --> TaskCompletionLogg : taskCompletionLogged
    TaskCompletionLogg --> IncidentRecor : incidentRecorded
    IncidentRecor --> DecisionNot : decisionNoted
    DecisionNot --> ObservationLogg : observationLogged
    ObservationLogg --> ShiftLogEn : shiftLogEnded
    ShiftLogEn --> ShiftReportGenerat : shiftReportGenerated
    ShiftReportGenerat --> [*]
```
