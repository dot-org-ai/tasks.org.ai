...
```mermaid
erDiagram
    WorkOrder ||--o{ MaintenanceLog : tracks
    WorkOrder ||--o{ PartRecord : tracks
    WorkOrder ||--o{ LaborRecord : tracks
    WorkOrder ||--o{ WarrantyRecord : tracks
    WorkOrder ||--o{ ConditionAssessment : contains
    WorkOrder ||--o{ MaintenanceCostReport : contains
```
...
stateDiagram-v2
    [*] --> Start
    Start --> WorkOrderOpen : workOrderOpened
    WorkOrderOpen --> ServiceActivityLogg : serviceActivityLogged
    ServiceActivityLogg --> PartsTrack : partsTracked
    PartsTrack --> LaborTimeRecor : laborTimeRecorded
    LaborTimeRecor --> WarrantyProcess : warrantyProcessed
    WarrantyProcess --> ConditionAssess : conditionAssessed
    ConditionAssess --> MaintenanceCostThresholdExcee : maintenanceCostThresholdExceeded
    MaintenanceCostThresholdExcee --> [*]
```
