...
```mermaid
erDiagram
    ReportDraft ||--|| ReportOutline : "structured-by"
    ReportDraft ||--o{ FindingsSection : "contains"
    ReportDraft ||--o{ Recommendation : "proposes"
    ReportDraft ||--o{ SupportingEvidence : "supported-by"
    ReportDraft ||--|| ExecutiveSummary : "summarized-by"
    ReportDraft ||--o{ ReviewComment : "receives"
```
...
stateDiagram-v2
    ...
    Drafting --> EvidenceInclusion : draftWritten
    EvidenceInclusion --> RecommendationPhase : evidenceIncorporated
    RecommendationPhase --> Summarization : recommendationsFormulated
    Summarization --> Review : executiveSummaryComposed
    Review --> Revision : reviewFeedbackReceived
    Revision --> Drafting
    Review --> Finalized : reportFinalized
    Finalized --> [*]
```
