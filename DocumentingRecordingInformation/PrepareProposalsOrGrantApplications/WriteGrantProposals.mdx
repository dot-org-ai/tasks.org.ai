...
```mermaid
erDiagram
    FundingOpportunity ||--o{ ProposalNarrative : "solicits"
    ProposalNarrative ||--|| ProposalBudget : "paired-with"
    ProposalBudget ||--|| BudgetJustification : "explained-by"
    ProposalNarrative ||--o{ LetterOfSupport : "supported-by"
    ProposalNarrative ||--o{ ComplianceCertification : "requires"
    SubmissionPackage ||--|| ProposalNarrative : "contains"
    SubmissionPackage ||--|| ProposalBudget : "includes"
    ReviewCriteria }o--|| FundingOpportunity : "defines"
```
...
stateDiagram-v2
    ...
    Drafting --> BudgetPreparation : narrativeDrafted
    BudgetPreparation --> DocumentCollection : budgetPrepared
    DocumentCollection --> InternalReview : internalReviewCompleted
    InternalReview --> Revision
    Revision --> Drafting
    InternalReview --> Submitted : proposalSubmitted
    Submitted --> AwaitingDecision : proposalSubmitted
    AwaitingDecision --> Awarded : proposalAwarded
    AwaitingDecision --> Declined : proposalDeclined
    Awarded --> [*]
    Declined --> [*]
```
