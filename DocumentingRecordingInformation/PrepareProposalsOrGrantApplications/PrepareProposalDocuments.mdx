...
```mermaid
erDiagram
    Solicitation ||--|| ComplianceMatrix : "generates"
    Solicitation ||--|| ProposalOutline : "drives"
    ProposalOutline ||--o{ TechnicalVolume : "includes"
    ProposalOutline ||--o{ ManagementVolume : "includes"
    ProposalOutline ||--o{ CostVolume : "includes"
    TechnicalVolume ||--o{ ReviewComment : "receives"
    ManagementVolume ||--o{ ReviewComment : "receives"
    CostVolume ||--o{ ReviewComment : "receives"
    BoilerplateLibrary }o--o{ TechnicalVolume : "provides-content"
```
...
stateDiagram-v2
    ...
    RequirementsAnalysis --> Planning : solicitationAnalyzed
    Planning --> Drafting : outlineCreated
    Drafting --> InReview : sectionDrafted
    InReview --> Revision : colorReviewCompleted
    Revision --> Drafting
    InReview --> Integration : colorReviewCompleted
    Integration --> ComplianceCheck : volumesIntegrated
    ComplianceCheck --> Formatting : complianceVerified
    ComplianceCheck --> Revision
    Formatting --> Submitted : proposalDelivered
    Submitted --> [*]
```
...
const analysis = await proposals.analyzeSolicitation({
  ...
  documentUrl: 'https://sam.gov.ai/rfp/2026-0042',
