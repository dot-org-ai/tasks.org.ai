...
```mermaid
erDiagram
    Proposal ||--|| GrantOpportunity : "applies-to"
    Proposal ||--|| Budget : "includes"
    Proposal ||--|| Narrative : "contains"
    Proposal ||--|| Submission : "results-in"
    Submission ||--o| AwardNotice : "receives"
    GrantOpportunity ||--o{ Proposal : "attracts"
```
...
stateDiagram-v2
    [*] --> OpportunitySearch
    OpportunitySearch --> NarrativeDevelopment : opportunityIdentified
    NarrativeDevelopment --> BudgetPreparation : narrativeDrafted
    BudgetPreparation --> Assembly : budgetPrepared
    Assembly --> InternalReview : applicationAssembled
    InternalReview --> Revision : internalReviewCompleted
    Revision --> NarrativeDevelopment
    InternalReview --> Submitted : proposalSubmitted
    Submitted --> AwaitingDecision : awardDecisionReceived
    AwaitingDecision --> [*]
```
