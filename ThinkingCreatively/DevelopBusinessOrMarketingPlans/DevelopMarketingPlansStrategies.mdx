...
erDiagram
    ...
    MarketingPlan ||--|| CompetitiveAnalysis : informs
    MarketingPlan ||--|| MarketingBudget : allocates
    Campaign ||--|| BrandGuideline : follows
    MarketSegment }o--|| Campaign : receives
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> Analyzed : marketAnalysisCompleted
    Analyzed --> Segmented : targetSegmentsDefined
    Segmented --> Positioned : positioningStrategyDeveloped
    Positioned --> Budgeted : budgetAllocated
    Budgeted --> Live : campaignLaunched
    Live --> Evaluated : performanceEvaluated
    Evaluated --> Positioned : targetsMissed
    Evaluated --> Complete : targetsMetOrExceeded
    Complete --> [*]
```
