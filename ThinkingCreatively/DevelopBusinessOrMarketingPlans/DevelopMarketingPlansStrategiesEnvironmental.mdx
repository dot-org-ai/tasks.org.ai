...
```mermaid
erDiagram
    MarketingPlan ||--|{ Campaign : launches
    MarketingPlan ||--|{ AudienceSegment : targets
    Campaign ||--|{ GreenClaim : asserts
    Campaign ||--|| ChannelStrategy : distributes
    Campaign ||--|{ PerformanceMetric : measures
    GreenClaim }o--|| AudienceSegment : resonates
```
...
stateDiagram-v2
    [*] --> PlanCreated
    PlanCreated --> AudienceDefined : audienceSegmentsDefined
    AudienceDefined --> MessagingReady : greenMessagingDeveloped
    MessagingReady --> ClaimsVerified : claimsValidated
    ClaimsVerified --> MessagingReady : claimsRejected
    ClaimsVerified --> ChannelsSelected : channelMixDesigned
    ChannelsSelected --> Live : campaignLaunched
    Live --> Measured : campaignImpactMeasured
    Measured --> MessagingReady : goalsNotMet
    Measured --> Complete : goalsAchieved
    Complete --> [*]
```
