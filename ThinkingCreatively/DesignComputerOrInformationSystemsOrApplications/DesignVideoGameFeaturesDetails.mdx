...
```mermaid
erDiagram
    GameDesignDocument ||--o{ GameMechanic : specifies
    GameDesignDocument ||--o{ LevelLayout : contains
    GameDesignDocument ||--o{ CharacterSpec : defines
    GameMechanic ||--|| ProgressionSystem : governs
    ProgressionSystem ||--|| BalanceSheet : tunes
    CharacterSpec }o--|| BalanceSheet : calibrates
```
...
stateDiagram-v2
    [*] --> Concept
    Concept --> Defined : mechanicsDefined
    Defined --> LevelDesign : levelDesigned
    Defined --> CharacterDesign : characterSpecified
    LevelDesign --> Balanced : systemsBalanced
    CharacterDesign --> Balanced : systemsBalanced
    Balanced --> Prototyped : prototypeCreated
    Prototyped --> Tested : playtestCompleted
    Tested --> Iterated : designIterated
    Iterated --> Balanced : systemsBalanced
    Tested --> Production : playtestCompleted
    Production --> [*]
```
