...
```mermaid
erDiagram
    ScientificApplication ||--o{ Algorithm : implements
    ScientificApplication ||--o{ Pipeline : processes
    Pipeline ||--o{ Dataset : consumes
    Algorithm ||--o{ Experiment : runs
    Experiment ||--o{ Dataset : produces
    ScientificApplication ||--o{ Publication : referenced_in
```
...
stateDiagram-v2
    [*] --> Requirements
    Requirements --> Coded : algorithmImplemented
    Coded --> PipelineReady : pipelineBuilt
    PipelineReady --> Executed : experimentCompleted
    Executed --> Validated : accuracyValidated
    Validated --> Coded : algorithmImplemented
    Validated --> Documented : methodologyDocumented
    Documented --> Packaged : distributionPackaged
    Packaged --> [*]
```
