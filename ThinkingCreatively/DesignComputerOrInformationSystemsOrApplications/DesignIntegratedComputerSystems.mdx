...
```mermaid
erDiagram
    SystemArchitecture ||--o{ IntegrationLayer : contains
    SystemArchitecture ||--o{ HardwareSpec : specifies
    IntegrationLayer ||--o{ InterfaceContract : defines
    SystemArchitecture ||--|| DeploymentTopology : maps_to
    DeploymentTopology ||--|| CapacityPlan : projects
    HardwareSpec }o--|| DeploymentTopology : deploys_on
```
...
stateDiagram-v2
    [*] --> Assessment
    Assessment --> Architected : architectureDefined
    Architected --> Specified : interfacesSpecified
    Specified --> Selected : componentsSelected
    Selected --> Mapped : topologyDesigned
    Mapped --> Validated : integrationValidated
    Validated --> Specified : interfacesSpecified
    Validated --> Planned : capacityPlanned
    Planned --> [*]
```
