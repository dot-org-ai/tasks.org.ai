...
```mermaid
erDiagram
    Sitemap ||--o{ Wireframe : structures
    Wireframe ||--o{ DesignMockup : refines_into
    DesignMockup ||--o{ ComponentLibrary : uses
    ComponentLibrary ||--|| StyleGuide : follows
    DesignMockup ||--|| PerformanceBudget : constrained_by
    Sitemap ||--|| StyleGuide : branded_by
```
...
stateDiagram-v2
    [*] --> Structured
    Structured --> Wireframed : wireframesCreated
    Wireframed --> MockedUp : mockupsBuilt
    MockedUp --> ComponentsReady : componentLibraryAssembled
    ComponentsReady --> Interactive : interactionsSpecified
    Interactive --> Testing : accessibilityTested
    Testing --> MockedUp : mockupsBuilt
    Testing --> Delivered : designDelivered
    Delivered --> [*]
```
