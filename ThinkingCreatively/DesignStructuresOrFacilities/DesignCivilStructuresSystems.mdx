...
```mermaid
erDiagram
    GeotechnicalReport ||--|| StructuralDesign : informs
    StructuralDesign ||--|{ LoadCalculation : calculates
    StructuralDesign ||--o{ HydraulicModel : models
    StructuralDesign ||--|| DesignStandard : complies
    StructuralDesign ||--|| ConstructionDocument : produces
    RightOfWay ||--|| ConstructionDocument : constrains
```
...
stateDiagram-v2
    [*] --> SiteInvestigated
    SiteInvestigated --> LoadsAnalyzed : loadsAnalyzed
    LoadsAnalyzed --> HydraulicsModeled : hydraulicsModeled
    HydraulicsModeled --> DesignReady : designDeveloped
    DesignReady --> CodeChecked : codeComplianceChecked
    CodeChecked --> DesignReady : nonCompliant
    CodeChecked --> DocumentsReady : constructionDocumentsPrepared
    DocumentsReady --> Permitted : permitSubmitted
    Permitted --> [*]
```
