...
```mermaid
erDiagram
    DesignConcept ||--|| CADModel : models
    CADModel ||--|| MaterialSpecification : specifies
    CADModel ||--|| ErgonomicAssessment : evaluates
    MaterialSpecification ||--o{ FinishSample : samples
    CADModel ||--|| Prototype : fabricates
    Prototype ||--|| CostSheet : estimates
    CADModel ||--|| ProductionDrawing : details
```
...
stateDiagram-v2
    [*] --> Conceptual
    Conceptual --> Modeled : cadModelCreated
    Modeled --> MaterialsChosen : materialsSpecified
    MaterialsChosen --> ErgonomicReviewed : ergonomicsAssessed
    ErgonomicReviewed --> Modeled : notErgonomic
    ErgonomicReviewed --> Prototyped : prototypeBuilt
    Prototyped --> Costed : costsCalculated
    Costed --> MaterialsChosen : notViable
    Costed --> Released : releasedForProduction
    Released --> [*]
```
