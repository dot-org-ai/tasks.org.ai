...
```mermaid
erDiagram
    ResearchMethodology ||--o| SurveyInstrument : uses
    ResearchMethodology ||--o| EconometricModel : employs
    ResearchMethodology ||--o| QualitativeFramework : applies
    ResearchMethodology ||--|{ PilotStudy : testedBy
    PilotStudy }o--|| MethodologyPublication : validatesFor
    SurveyInstrument }o--|{ PilotStudy : testedIn
    EconometricModel }o--|{ PilotStudy : testedIn
```
...
stateDiagram-v2
    [*] --> Designed
    Designed --> InstrumentBuilding : researchMethodologyDesigned
    InstrumentBuilding --> Piloting : surveyInstrumentDeveloped
    InstrumentBuilding --> Piloting : econometricModelBuilt
    InstrumentBuilding --> Piloting : qualitativeFrameworkCreated
    Piloting --> Designed : pilot failed
    Piloting --> Validating : pilotStudyConducted
    Validating --> Published : methodologyValidated
    Published --> [*]
```
