...
```mermaid
erDiagram
    ResearchPlan ||--|| ResearchQuestion : investigates
    ResearchPlan ||--|| Methodology : follows
    ResearchPlan ||--|| SamplingStrategy : defines
    ResearchPlan ||--|{ DataCollectionInstrument : uses
    ResearchPlan ||--o| EthicsApplication : approvedBy
    Methodology }o--|| SamplingStrategy : informs
    DataCollectionInstrument }o--|| SamplingStrategy : implementedBy
```
...
stateDiagram-v2
    [*] --> QuestionFormulated
    QuestionFormulated --> MethodologyDesigned : researchQuestionFormulated
    MethodologyDesigned --> SamplingDefined : methodologyDesigned
    SamplingDefined --> InstrumentReady : samplingStrategyDefined
    InstrumentReady --> EthicsReview : instrumentDeveloped
    EthicsReview --> InstrumentReady : revisions needed
    EthicsReview --> Budgeting : ethicsApprovalGranted
    Budgeting --> Finalized : researchPlanFinalized
    Finalized --> [*]
```
...
graph TD
    ...
    F -->|No| H{Major or Minor Revision?}
    H -->|Major| I[Revise Methodology]
    I --> B
    H -->|Minor| D
    G --> J{Budget Adequate?}
    J -->|No| K[Reduce Scope]
    K --> C
    J -->|Yes| L[finalizeResearchPlan]
```
