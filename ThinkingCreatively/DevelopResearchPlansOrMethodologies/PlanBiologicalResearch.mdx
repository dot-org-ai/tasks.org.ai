...
```mermaid
erDiagram
    StudyPlan ||--|| Hypothesis : tests
    StudyPlan ||--|| ExperimentalProtocol : follows
    StudyPlan ||--|| ModelOrganism : uses
    StudyPlan ||--|| SamplePlan : defines
    StudyPlan ||--o| BiosafetyClearance : approvedBy
    ExperimentalProtocol }o--|| ModelOrganism : targets
    SamplePlan }o--|| ExperimentalProtocol : implementedBy
```
...
stateDiagram-v2
    [*] --> HypothesisReady
    HypothesisReady --> ExperimentDesigned : hypothesisFormulated
    ExperimentDesigned --> OrganismSelected : experimentDesigned
    OrganismSelected --> ProtocolReady : modelOrganismSelected
    ProtocolReady --> BiosafetyReview : protocolDeveloped
    BiosafetyReview --> ProtocolReady : revisions needed
    BiosafetyReview --> SamplePlanning : biosafetyClearanceGranted
    SamplePlanning --> Finalized : studyPlanFinalized
    Finalized --> [*]
```
...
graph TD
    ...
    H --> D
    G --> I{Sample Plan Feasible?}
    I -->|No| J[Adjust Experiment]
    J --> B
    I -->|Yes| K[finalizeStudyPlan]
```
