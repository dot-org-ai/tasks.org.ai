...
```mermaid
erDiagram
    ResearchProposal ||--|{ StudySite : investigates
    ResearchProposal ||--|| FieldSamplingPlan : follows
    ResearchProposal ||--|{ ResearchPermit : requires
    ResearchProposal ||--|| BudgetAllocation : fundedBy
    ResearchProposal ||--|| MilestoneSchedule : trackedBy
    StudySite }o--|{ ResearchPermit : coveredBy
    FieldSamplingPlan }o--|{ StudySite : samplesFrom
```
...
stateDiagram-v2
    [*] --> ProposalDrafted
    ProposalDrafted --> SitesSelected : researchProposalCreated
    SitesSelected --> SamplingPlanned : studySitesSelected
    SamplingPlanned --> PermitsPending : samplingPlanDeveloped
    PermitsPending --> SitesSelected : permits denied
    PermitsPending --> Budgeting : permitsGranted
    Budgeting --> MilestonesSet : budgetAllocated
    MilestonesSet --> FundingReview : milestones defined
    FundingReview --> ProposalDrafted : funding denied
    FundingReview --> Active : fundingApproved
    Active --> [*]
```
