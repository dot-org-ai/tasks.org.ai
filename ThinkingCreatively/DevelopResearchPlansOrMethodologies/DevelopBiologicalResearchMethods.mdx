...
```mermaid
erDiagram
    BiologicalMethod ||--|{ AssayProtocol : includes
    BiologicalMethod ||--o| BioinformaticsPipeline : analyzedBy
    BiologicalMethod ||--|{ ValidationExperiment : validatedBy
    AssayProtocol ||--|{ ReagentSpecification : requires
    ValidationExperiment }o--|| StandardOperatingProcedure : produces
    BioinformaticsPipeline }o--|| ValidationExperiment : testedBy
```
...
stateDiagram-v2
    [*] --> Designed
    Designed --> ProtocolDevelopment : biologicalMethodDesigned
    ProtocolDevelopment --> PipelineBuilt : bioinformaticsPipelineBuilt
    ProtocolDevelopment --> AssayReady : assayProtocolDeveloped
    AssayReady --> BiosafetyReview : biosafetyReviewCompleted
    BiosafetyReview --> AssayReady : revisions needed
    BiosafetyReview --> Validation : approved
    PipelineBuilt --> Validation : ready for validation
    Validation --> ProtocolDevelopment : validationExperimentConducted failed
    Validation --> Standardized : methodStandardized
    Standardized --> Published : methodPublished
    Published --> [*]
```
