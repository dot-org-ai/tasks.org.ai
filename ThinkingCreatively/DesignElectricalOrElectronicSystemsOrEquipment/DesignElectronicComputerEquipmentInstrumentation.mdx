...
```mermaid
erDiagram
    Schematic ||--|| BillOfMaterials : generates
    Schematic ||--|| PCBLayout : translates_to
    PCBLayout ||--|| Prototype : fabricates
    Prototype ||--o{ SimulationResult : validates
    Schematic ||--o{ DesignReviewRecord : reviewed_in
    BillOfMaterials }o--|| Prototype : sources
```
...
stateDiagram-v2
    ...
    RequirementsCaptured --> SchematicDone : schematicCreated
    SchematicDone --> BOMReady : bomGenerated
    BOMReady --> Simulated : simulationPassed
    Simulated --> SchematicDone : schematicCreated
    Simulated --> Laid_Out : pcbLayoutCompleted
    Laid_Out --> Prototyped : prototypeBuilt
    Prototyped --> Verified : designVerified
    Verified --> SchematicDone : schematicCreated
    Verified --> Production : designVerified
    Production --> [*]
```
