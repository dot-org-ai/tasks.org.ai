...
```mermaid
erDiagram
    SingleLineDiagram ||--o{ LoadSchedule : represents
    SingleLineDiagram ||--|| ProtectionScheme : protected_by
    ProtectionScheme ||--o{ EquipmentSpecification : sizes
    ShortCircuitStudy ||--|| ProtectionScheme : informs
    LoadSchedule ||--o{ PanelSchedule : details
    EquipmentSpecification }o--|| PanelSchedule : populates
```
...
stateDiagram-v2
    [*] --> LoadAnalyzed
    LoadAnalyzed --> Diagrammed : singleLineDiagramCreated
    Diagrammed --> FaultStudied : shortCircuitStudyCompleted
    FaultStudied --> EquipmentReady : equipmentSelected
    EquipmentReady --> Protected : protectionCoordinated
    Protected --> Scheduled : panelSchedulesGenerated
    Scheduled --> Issued : constructionDocumentsIssued
    Issued --> Diagrammed : singleLineDiagramCreated
    Issued --> [*]
```
