...
```mermaid
erDiagram
    HazardAssessment ||--|{ EmergencyProcedure : informs
    EmergencyProcedure ||--|{ EvacuationRoute : defines
    EmergencyProcedure ||--|{ EmergencyContact : references
    EmergencyProcedure ||--|{ TrainingRecord : requires
    EvacuationRoute }o--|| EmergencyContact : notifies
    TrainingRecord }o--|| EmergencyProcedure : certifies
```
...
stateDiagram-v2
    [*] --> HazardsIdentified
    HazardsIdentified --> Drafted : procedureDrafted
    Drafted --> RoutesSet : evacuationRoutesDefined
    RoutesSet --> UnderReview : procedureReviewed
    UnderReview --> Drafted : revisions required
    UnderReview --> Approved : procedureApproved
    Approved --> TrainingComplete : trainingConducted
    TrainingComplete --> DrillReady : drillScheduled
    DrillReady --> Approved : drill reveals gaps
    DrillReady --> Active : drill passed
    Active --> HazardsIdentified : annual reassessment
    Active --> [*]
```
...
graph TD
    ...
    H -->|No| G
    H -->|Yes| I[scheduleDrill]
    I --> J{Drill Passed?}
    J -->|No| B
    J -->|Yes| K[Procedure Active]
```
