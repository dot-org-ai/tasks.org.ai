...
```mermaid
erDiagram
    EmergencyResponsePlan ||--|| IncidentCommandStructure : establishes
    EmergencyResponsePlan ||--|{ ResourceStaging : allocates
    EmergencyResponsePlan ||--|{ MutualAidAgreement : references
    EmergencyResponsePlan ||--|| CommunicationPlan : includes
    EmergencyResponsePlan ||--|{ AfterActionReport : generates
    MutualAidAgreement }o--|| ResourceStaging : supplies
    AfterActionReport }o--|| IncidentCommandStructure : evaluates
```
...
stateDiagram-v2
    [*] --> AnalysisPhase
    AnalysisPhase --> Structuring : hazardAnalysisCompleted
    Structuring --> Drafting : commandStructureDesigned
    Drafting --> Partnering : responsePlanDrafted
    Partnering --> CommunicationDesign : mutualAidEstablished
    CommunicationDesign --> Validation : communicationPlanDeveloped
    Validation --> Drafting : gaps identified
    Validation --> Published : planValidated
    Published --> AnalysisPhase : annual review cycle
    Published --> [*]
```
