...
```mermaid
erDiagram
    OperationalCriterion ||--|| PerformanceBenchmark : referencedBy
    OperationalCriterion ||--o| CapacityRequirement : includes
    OperationalCriterion ||--o| UptimeTarget : includes
    OperationalCriterion ||--o| QualityThreshold : includes
    OperatingProcedure }o--|{ OperationalCriterion : implements
    StaffingLevel }o--|| OperatingProcedure : supports
```
...
stateDiagram-v2
    [*] --> NeedsAssessed
    NeedsAssessed --> Benchmarked : operationalNeedsAssessed
    Benchmarked --> CriteriaDefined : benchmarksEstablished
    CriteriaDefined --> CapacitySet : criteriaDefined
    CapacitySet --> UptimeSet : capacitySpecified
    UptimeSet --> Documented : uptimeTargetsSet
    Documented --> Validating : proceduresDocumented
    Validating --> CriteriaDefined : criteria not met
    Validating --> Finalized : criteriaValidated
    Finalized --> [*]
```
...
graph TD
    ...
    G --> H{Criteria Met?}
    H -->|No| I{Benchmark or Capacity Issue?}
    I -->|Benchmark| B
    I -->|Capacity| D
    H -->|Yes| J[Criteria Finalized]
```
