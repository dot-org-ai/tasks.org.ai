...
```mermaid
erDiagram
    NetworkSpec ||--|| TopologyDiagram : defines
    NetworkSpec ||--|{ VLANDefinition : segments
    NetworkSpec ||--|{ BandwidthAllocation : allocates
    NetworkSpec ||--|{ SecurityPolicy : enforces
    NetworkSpec ||--|| FailoverConfig : protectedBy
    MonitoringProfile }o--|| NetworkSpec : monitors
```
...
stateDiagram-v2
    [*] --> TopologyDefined
    TopologyDefined --> VLANsConfigured : topologyDefined
    VLANsConfigured --> BandwidthAllocated : vlansConfigured
    BandwidthAllocated --> SecuritySet : bandwidthAllocated
    SecuritySet --> FailoverDefined : securityPoliciesSet
    FailoverDefined --> MonitoringReady : failoverDefined
    MonitoringReady --> TopologyDefined : review revisions
    MonitoringReady --> Published : networkSpecPublished
    Published --> [*]
```
...
graph TD
    ...
    F --> G{Review Complete?}
    G -->|No| H{Topology or Security Issue?}
    H -->|Topology| A
    H -->|Security| D
    G -->|Yes| I[publishNetworkSpec]
```
