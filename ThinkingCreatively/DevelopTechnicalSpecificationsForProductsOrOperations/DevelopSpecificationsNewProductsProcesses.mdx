...
```mermaid
erDiagram
    ProductSpec ||--|| ProcessSpec : implements
    ProductSpec ||--|| BillOfMaterials : lists
    ProductSpec ||--|{ DimensionalDrawing : illustratedBy
    ProductSpec ||--|{ TestProtocol : validatedBy
    ProductSpec ||--|{ RevisionHistory : tracks
    ComplianceMatrix }o--|| ProductSpec : mapsTo
```
...
stateDiagram-v2
    [*] --> Drafted
    Drafted --> ParametersDefined : specificationDrafted
    ParametersDefined --> BOMCreated : processParametersDefined
    BOMCreated --> DrawingsAttached : billOfMaterialsCreated
    DrawingsAttached --> TestReady : drawingsAttached
    TestReady --> UnderReview : testProtocolDefined
    UnderReview --> Drafted : revisions required
    UnderReview --> Released : specificationReleased
    Released --> [*]
```
...
graph TD
    ...
    E --> F[submitForReview]
    F --> G{Approved?}
    G -->|No| H{Major or Minor Changes?}
    H -->|Major| A
    H -->|Minor| E
    G -->|Yes| I[releaseToProduction]
```
