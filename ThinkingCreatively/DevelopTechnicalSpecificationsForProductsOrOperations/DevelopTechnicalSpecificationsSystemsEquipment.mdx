...
```mermaid
erDiagram
    SystemSpec ||--|{ InterfaceDefinition : defines
    SystemSpec ||--|{ PerformanceRequirement : specifies
    SystemSpec ||--|| EnvironmentalEnvelope : boundBy
    SystemSpec ||--|| ReliabilityTarget : requires
    SystemSpec ||--|| MaintenanceSchedule : scheduledBy
    ConformanceReport }o--|| SystemSpec : verifies
```
...
stateDiagram-v2
    ...
    RequirementsCaptured --> InterfacesDefined : requirementsCaptured
    InterfacesDefined --> PerformanceSet : interfacesDefined
    PerformanceSet --> EnvLimitsSet : performanceSpecified
    EnvLimitsSet --> ReliabilitySet : environmentalLimitsSet
    ReliabilitySet --> Authored : reliabilityTargetsEstablished
    Authored --> UnderReview : specificationAuthored
    UnderReview --> Authored : revisions needed
    UnderReview --> Released : designReviewConducted
    Released --> [*]
```
...
graph TD
    ...
    G --> H{Approved?}
    H -->|No| I{Performance or Interface Issue?}
    I -->|Performance| C
    I -->|Interface| B
    H -->|Yes| J[releaseSpecification]
```
