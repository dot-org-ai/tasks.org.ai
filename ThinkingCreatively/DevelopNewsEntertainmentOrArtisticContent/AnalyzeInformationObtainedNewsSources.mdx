...
```mermaid
erDiagram
    NewsSource ||--|{ StoryLead : generates
    NewsSource ||--|| SourceCredibilityScore : ratedBy
    StoryLead ||--|{ FactCheckReport : verifiedBy
    StoryLead ||--|| AnalysisBrief : synthesizedIn
    FactCheckReport }o--|| SourceCredibilityScore : updates
    AnalysisBrief }o--|| FactCheckReport : incorporates
```
...
stateDiagram-v2
    [*] --> Ingested
    Ingested --> CredibilityCheck : sourceIngested
    CredibilityCheck --> CrossReferencing : credibilityAssessed
    CredibilityCheck --> Flagged : discrepancyFlagged
    CrossReferencing --> Verification : facts cross-referenced
    Flagged --> Verification : still needs verification
    Verification --> Synthesized : factVerified
    Synthesized --> Prioritized : analysisSynthesized
    Prioritized --> Approved : analysisBriefApproved
    Prioritized --> Ingested : more sources needed
    Approved --> [*]
```
