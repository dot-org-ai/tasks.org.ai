...
```mermaid
erDiagram
    PitchDeck ||--|| ContentPlaybook : guidedBy
    PitchDeck ||--|{ EngagementAnalytics : tracks
    ProductSheet }o--|| PitchDeck : supplements
    CaseStudy }o--|| PitchDeck : supports
    BattleCard }o--|| PitchDeck : informs
    ProposalTemplate ||--|{ CaseStudy : references
    ContentPlaybook ||--|{ BattleCard : organizes
```
...
stateDiagram-v2
    ...
    IntelligenceGathering --> NarrativeDesign : intelligenceGathered
    NarrativeDesign --> ContentCreation : narrativeStructured
    ContentCreation --> SalesReview : presentationCreated
    SalesReview --> ContentCreation : revisions needed
    SalesReview --> Published : approved
    Published --> Tracking : contentPublished
    Tracking --> ContentCreation : low engagement
    Tracking --> Active : engagementTracked
    Active --> IntelligenceGathering : new product cycle
    Active --> [*]
```
...
graph TD
    ...
    D --> E[buildBattleCard]
    E --> F{Sales Review?}
    F -->|Revise| G{Content or Messaging?}
    G -->|Content| C
    G -->|Messaging| B
    F -->|Approved| H[publishToLibrary]
    H --> I[trackEngagement]
    I --> J{High Performing?}
    J -->|No| C
```
