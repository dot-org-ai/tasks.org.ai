...
```mermaid
erDiagram
    CreativeBrief ||--|{ DesignConcept : generates
    DesignConcept ||--|| CopyDraft : pairs_with
    DesignConcept ||--|| ProofApproval : requires
    DesignConcept ||--|| PrintSpecification : defines
    DistributionPlan }o--|| CreativeBrief : schedules
    AssetLibrary ||--|{ DesignConcept : stores
    ProofApproval }o--|| PrintSpecification : triggers
```
...
stateDiagram-v2
    [*] --> Briefing
    Briefing --> ConceptDesign : briefCreated
    ConceptDesign --> Copywriting : conceptDesigned
    Copywriting --> ProofReview : copyWritten
    ProofReview --> ConceptDesign : not approved
    ProofReview --> ProductionSpec : proofReviewed
    ProductionSpec --> Production : productionSpecified
    Production --> Distribution : runProduced
    Distribution --> Complete : materialsDistributed
    Complete --> Briefing : new campaign
    Complete --> [*]
```
...
graph TD
    ...
    C --> D[reviewProof]
    D --> E{Approved?}
    E -->|No| F{Design or Copy Issue?}
    F -->|Design| B
    F -->|Copy| C
    E -->|Yes| G[specifyProduction]
    G --> H[produceRun]
    H --> I[distributeMaterials]
```
