...
```mermaid
erDiagram
    EngineeringModel ||--|| DesignSpecification : satisfies
    EngineeringModel ||--|{ SimulationRun : executes
    EngineeringModel ||--|{ DesignIteration : versions
    EngineeringModel ||--|{ ParameterSet : configuredWith
    SimulationRun }o--|| ParameterSet : uses
    ValidationReport }o--|| SimulationRun : analyzes
    DesignIteration }o--|| ValidationReport : informs
```
...
stateDiagram-v2
    [*] --> RequirementCapture
    RequirementCapture --> ModelCreation : designRequirementsCaptured
    ModelCreation --> ParameterSetup : engineeringModelCreated
    ParameterSetup --> Simulation : parametersConfigured
    Simulation --> Analysis : simulationCompleted
    Analysis --> ModelCreation : does not meet spec
    Analysis --> Validation : resultsAnalyzed
    Validation --> ModelCreation : validation issues
    Validation --> Released : designValidated
    Released --> [*]
```
...
graph TD
    ...
    D --> E[analyzeResults]
    E --> F{Meets Spec?}
    F -->|No| G{Parameter or Model Issue?}
    G -->|Parameters| C
    G -->|Model| B
    F -->|Yes| H[validateDesign]
    H --> I{Validated?}
    I -->|No| B
    I -->|Yes| J[releaseDesign]
```
