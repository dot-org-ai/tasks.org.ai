...
```mermaid
erDiagram
    CatalogRecord ||--o{ DigitalAsset : digitizes
    CatalogRecord ||--o{ AuthorityRecord : linked_to
    CatalogRecord }o--|| MetadataSchema : conforms_to
    ArchivalCollection ||--o{ CatalogRecord : contains
    ArchivalCollection ||--|| FindingAid : described_by
    CatalogRecord ||--o{ CirculationRecord : tracks
    CatalogRecord ||--|| HoldingsRecord : located_by
```
...
stateDiagram-v2
    [*] --> SchemaReady
    SchemaReady --> Migrated : recordsMigrated
    Migrated --> Cataloged : catalogRecordAdded
    Cataloged --> Linked : authorityRecordLinked
    Linked --> Digitized : digitalAssetIngested
    Digitized --> Published : findingAidPublished
    Published --> Searchable : searchIndexRebuilt
    Searchable --> Cataloged : catalogRecordAdded
    Searchable --> [*]
```
...
graph TD
    ...
    D --> E{Digital asset available?}
    E -->|Yes| F[ingestDigitalAsset]
    E -->|No| G[buildFindingAid]
    F --> G
    G --> H[configureSearch]
    H --> I{More records to catalog?}
    I -->|Yes| C
    I -->|No| J[searchIndexRebuilt]
```
