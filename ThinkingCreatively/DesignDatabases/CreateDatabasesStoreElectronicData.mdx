...
```mermaid
erDiagram
    DatabaseInstance ||--o{ TableDefinition : contains
    DatabaseInstance ||--|| StorageConfiguration : configured_by
    DatabaseInstance ||--o{ ConnectionPool : serves
    DatabaseInstance ||--|| BackupSchedule : protected_by
    TableDefinition ||--o{ MigrationScript : modified_by
    ConnectionPool ||--o{ AccessCredential : authenticates
```
...
stateDiagram-v2
    [*] --> Provisioned
    Provisioned --> Configured : storageConfigured
    Configured --> SchemaReady : schemaDeployed
    SchemaReady --> Connected : connectionPoolCreated
    Connected --> Secured : accessGranted
    Secured --> BackedUp : backupScheduled
    BackedUp --> Verified : connectionPoolCreated
    Verified --> Ready : schemaDeployed
    Ready --> Migrating : migrationExecuted
    Migrating --> Ready : migrationExecuted
    Ready --> [*]
```
