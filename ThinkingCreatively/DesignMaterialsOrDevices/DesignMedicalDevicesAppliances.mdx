...
```mermaid
erDiagram
    UserNeedsDocument ||--|| DesignInputSpecification : derives
    DesignInputSpecification ||--|| DesignOutputPackage : produces
    DesignOutputPackage ||--|| RiskAnalysis : assesses
    DesignOutputPackage ||--|| VerificationReport : verifies
    VerificationReport ||--|| ValidationProtocol : validates
    ValidationProtocol ||--|| RegulatorySubmission : supports
    DesignHistoryFile ||--|{ DesignOutputPackage : records
```
...
stateDiagram-v2
    [*] --> NeedsGathered
    NeedsGathered --> InputsDefined : designInputsDefined
    InputsDefined --> ConceptReady : conceptDeveloped
    ConceptReady --> RiskAssessed : riskAnalysisConducted
    RiskAssessed --> Verified : designVerified
    Verified --> ConceptReady : failsVerification
    Verified --> Validated : clinicallyValidated
    Validated --> InputsDefined : failsValidation
    Validated --> Submitted : regulatorySubmitted
    Submitted --> [*]
```
