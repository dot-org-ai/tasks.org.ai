...
```mermaid
erDiagram
    DeviceConcept ||--|| SimulationModel : models
    SimulationModel ||--|| MaskLayout : patterns
    MaskLayout ||--|| ProcessFlow : sequences
    ProcessFlow ||--|| FabricatedWafer : produces
    FabricatedWafer ||--|{ CharacterizationResult : measures
    CharacterizationResult }o--|| DevicePackage : packages
```
...
stateDiagram-v2
    [*] --> FeasibilityChecked
    FeasibilityChecked --> Simulated : deviceSimulated
    Simulated --> FeasibilityChecked : performanceFails
    Simulated --> MaskDesigned : maskLayoutDesigned
    MaskDesigned --> ProcessDefined : processFlowDeveloped
    ProcessDefined --> Fabricated : devicesFabricated
    Fabricated --> Characterized : devicesCharacterized
    Characterized --> MaskDesigned : outOfSpec
    Characterized --> Packaged : devicesPackaged
    Packaged --> [*]
```
