...
```mermaid
erDiagram
    MasterPattern ||--|{ GradedPattern : scales
    MasterPattern ||--|| ToleranceSpecification : constrains
    GradedPattern ||--o{ CuttingLayout : arranges
    CuttingLayout ||--|| DigitalTemplate : digitizes
    MasterPattern ||--o{ PrototypeSample : tests
    MasterPattern }o--|| PatternLibrary : archives
```
...
stateDiagram-v2
    [*] --> DimensionsSet
    DimensionsSet --> Drafted : patternDrafted
    Drafted --> Graded : patternGraded
    Graded --> LayoutReady : layoutOptimized
    LayoutReady --> PrototypeCut : prototypeCut
    PrototypeCut --> Validated : accuracyValidated
    Validated --> Drafted : outOfTolerance
    Validated --> Released : templateReleased
    Released --> [*]
```
