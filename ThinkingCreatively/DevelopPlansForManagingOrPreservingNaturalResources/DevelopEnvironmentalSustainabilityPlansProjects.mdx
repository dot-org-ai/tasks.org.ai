...
```mermaid
erDiagram
    SustainabilityPlan ||--|| ImpactAssessment : basedOn
    SustainabilityPlan ||--|{ ReductionTarget : sets
    SustainabilityPlan ||--|{ SustainabilityProject : implements
    SustainabilityPlan ||--|| CarbonInventory : quantifies
    SustainabilityPlan ||--|{ ProgressReport : generates
    Certification }o--|| SustainabilityPlan : validates
    ReductionTarget }o--|| SustainabilityProject : achievedBy
```
...
stateDiagram-v2
    [*] --> ImpactBaseline
    ImpactBaseline --> CarbonAccounting : impactAssessed
    CarbonAccounting --> TargetSetting : carbonInventoryCompleted
    TargetSetting --> ProjectDesign : targetsSet
    ProjectDesign --> Implementation : projectDesigned
    Implementation --> Tracking : in progress
    Tracking --> ProjectDesign : targets not met
    Tracking --> Reporting : progressTracked
    Reporting --> CertificationPursuit : resultsReported
    CertificationPursuit --> ImpactBaseline : next cycle
...
graph TD
    ...
    D --> E[trackProgress]
    E --> F{Targets Met?}
    F -->|No| G{Redesign or New Project?}
    G -->|Redesign| D
    G -->|New| C
    F -->|Yes| H[reportResults]
    H --> I[seekCertification]
```
