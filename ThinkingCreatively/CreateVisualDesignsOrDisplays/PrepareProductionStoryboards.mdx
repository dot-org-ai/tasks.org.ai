...
```mermaid
erDiagram
    ScriptBreakdown ||--|{ ThumbnailSequence : "sketched as"
    ThumbnailSequence ||--|{ StoryboardPanel : "rendered as"
    StoryboardPanel ||--|| CameraNote : "annotated with"
    StoryboardPanel ||--|| TimingSheet : "timed in"
    StoryboardPanel }o--|| Animatic : "assembled into"
    Animatic ||--o{ RevisionSet : "revised via"
```
...
stateDiagram-v2
    [*] --> ScriptBrokenDown
    ScriptBrokenDown --> ThumbnailsSketched : thumbnailsSketched
    ThumbnailsSketched --> PanelsRendered : panelsRendered
    PanelsRendered --> CameraAnnotated : cameraWorkAnnotated
    CameraAnnotated --> TimingSet : timingSpecified
    TimingSet --> AnimaticReady : animaticAssembled
    AnimaticReady --> PanelsRendered : panelsRendered
    AnimaticReady --> Delivered : storyboardDelivered
    Delivered --> [*]
```
