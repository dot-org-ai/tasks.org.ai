...
```mermaid
erDiagram
    DesignSpecification ||--|| Schematic : "defines"
    Schematic ||--|{ ComponentSymbol : "uses"
    Schematic ||--o| SiteLayout : "positioned in"
    Schematic ||--o| FlowDiagram : "flows through"
    Schematic ||--|| RenderOutput : "exported as"
    SiteLayout }o--|| DesignSpecification : "constrained by"
```
...
stateDiagram-v2
    [*] --> DataGathered
    DataGathered --> SchematicCreated : schematicCreated
    SchematicCreated --> LayoutRendered : siteLayoutRendered
    LayoutRendered --> Annotated : graphicAnnotated
    Annotated --> Exported : renderExported
    Exported --> Submitted : reviewSubmitted
    Submitted --> SchematicCreated : revisionRequested
    Submitted --> Complete : reviewSubmitted
    Complete --> [*]
```
