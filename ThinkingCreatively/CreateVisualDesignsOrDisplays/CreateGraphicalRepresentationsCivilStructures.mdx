...
```mermaid
erDiagram
    SitePlan ||--|{ StructuralDrawing : "contains"
    StructuralDrawing ||--o{ CrossSection : "sectioned in"
    StructuralDrawing ||--o{ DetailSheet : "detailed in"
    StructuralDrawing ||--o{ RevisionMark : "annotated with"
    DrawingSet ||--|{ StructuralDrawing : "compiles"
    DrawingSet ||--|{ DetailSheet : "includes"
```
...
stateDiagram-v2
    [*] --> SurveyDataImported
    SurveyDataImported --> SitePlanCreated : sitePlanCreated
    SitePlanCreated --> CrossSectioned : crossSectionGenerated
    CrossSectioned --> DrawingSetAssembled : drawingSetAssembled
    DrawingSetAssembled --> Submitted : approvalSubmitted
    Submitted --> Revised : revisionApplied
    Revised --> CrossSectioned : crossSectionGenerated
    Submitted --> Approved : drawingApproved
    Approved --> [*]
```
