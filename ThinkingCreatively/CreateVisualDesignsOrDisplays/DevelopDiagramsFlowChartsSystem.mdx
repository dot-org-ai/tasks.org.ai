...
```mermaid
erDiagram
    Diagram ||--|{ SystemComponent : "depicts"
    Diagram ||--|{ DataFlow : "connects via"
    FlowChart ||--|{ SystemComponent : "sequences"
    Diagram ||--o| DiagramTemplate : "based on"
    Diagram ||--o{ RevisionHistory : "versioned in"
    DataFlow }o--|| SystemComponent : "links"
```
...
stateDiagram-v2
    [*] --> DiagramCreated
    DiagramCreated --> ComponentsMapped : componentsMappped
    ComponentsMapped --> FlowsDefined : dataFlowsDefined
    FlowsDefined --> Validated : diagramValidated
    Validated --> Revised : diagramRevised
    Revised --> Validated : diagramValidated
    Validated --> Published : diagramPublished
    Published --> Archived : diagramArchived
    Published --> Revised : diagramRevised
    Archived --> [*]
```
