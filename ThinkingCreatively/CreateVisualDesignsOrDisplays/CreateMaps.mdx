...
```mermaid
erDiagram
    MapProject ||--|{ SpatialDataset : "sources from"
    MapProject ||--|| CoordinateSystem : "projected in"
    MapProject ||--|{ MapSheet : "composed of"
    MapSheet ||--|| MapSymbology : "styled with"
    MapSheet ||--|{ LabelPlacement : "annotated with"
    MapSheet ||--o{ MapRevision : "versioned as"
    SpatialDataset }o--|| CoordinateSystem : "referenced to"
```
...
stateDiagram-v2
    [*] --> DataAcquired
    DataAcquired --> ProjectionDefined : projectionDefined
    ProjectionDefined --> FeaturesCompiled : featuresCompiled
    FeaturesCompiled --> SymbologyApplied : symbologyApplied
    SymbologyApplied --> LabelsPlaced : labelsPlaced
    LabelsPlaced --> AccuracyReviewed : accuracyReviewed
    AccuracyReviewed --> FeaturesCompiled : featuresCompiled
    AccuracyReviewed --> MapPublished : mapPublished
    MapPublished --> [*]
```
