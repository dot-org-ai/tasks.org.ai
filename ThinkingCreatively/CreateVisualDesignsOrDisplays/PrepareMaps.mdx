...
```mermaid
erDiagram
    MapRequest ||--|{ DataLayer : "assembles"
    MapRequest ||--|| MapTemplate : "laid out with"
    DataLayer ||--o| ThematicOverlay : "styled as"
    MapTemplate ||--|| LegendSpecification : "includes"
    MapRequest ||--|| ExportProfile : "exported via"
    MapRequest ||--o| FieldSheet : "formatted as"
```
...
stateDiagram-v2
    [*] --> RequestReceived
    RequestReceived --> DataAssembled : dataAssembled
    DataAssembled --> TemplateApplied : templateApplied
    TemplateApplied --> OverlaysStyled : overlaysStyled
    OverlaysStyled --> LegendComposed : legendComposed
    LegendComposed --> MapProofed : mapProofed
    MapProofed --> OverlaysStyled : overlaysStyled
    MapProofed --> MapExported : mapExported
    MapExported --> [*]
```
