...
```mermaid
erDiagram
    ProcessFlowDiagram ||--|{ PipingInstrumentationDiagram : "detailed in"
    PipingInstrumentationDiagram ||--|{ ControlLoopDiagram : "automates via"
    PipingInstrumentationDiagram ||--o{ EquipmentSpecSheet : "references"
    PlantLayout ||--|{ ProcessFlowDiagram : "arranges"
    PipingInstrumentationDiagram ||--o{ DiagramRevision : "versioned as"
    ControlLoopDiagram }o--|| EquipmentSpecSheet : "instruments from"
```
...
stateDiagram-v2
    [*] --> ProcessFlowCreated
    ProcessFlowCreated --> PIDCreated : pidCreated
    PIDCreated --> PlantLaid : plantLayoutCompleted
    PlantLaid --> ControlsDocumented : controlLoopsDocumented
    ControlsDocumented --> Reviewed : diagramReviewed
    Reviewed --> Revised : diagramRevised
    Revised --> Reviewed : diagramReviewed
    Reviewed --> Published : diagramPublished
    Published --> [*]
```
