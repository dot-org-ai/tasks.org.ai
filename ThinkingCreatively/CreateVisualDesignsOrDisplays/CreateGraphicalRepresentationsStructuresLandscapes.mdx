...
```mermaid
erDiagram
    SitePlan ||--o{ ArchitecturalRendering : "visualized in"
    SitePlan ||--|{ ElevationDrawing : "elevated in"
    SitePlan ||--o| LandscapePlan : "landscaped via"
    ThreeDModel ||--|{ ArchitecturalRendering : "renders"
    DesignPresentation ||--|{ ArchitecturalRendering : "includes"
    DesignPresentation ||--|{ ElevationDrawing : "compiles"
    DesignPresentation ||--o| LandscapePlan : "features"
```
...
stateDiagram-v2
    [*] --> SiteAnalyzed
    SiteAnalyzed --> ConceptDeveloped : conceptDeveloped
    ConceptDeveloped --> RenderingCreated : renderingCreated
    ConceptDeveloped --> ElevationDrafted : elevationDrafted
    ConceptDeveloped --> LandscapeDesigned : landscapeDesigned
    RenderingCreated --> PresentationCompiled : presentationCompiled
    ElevationDrafted --> PresentationCompiled : presentationCompiled
    LandscapeDesigned --> PresentationCompiled : presentationCompiled
    PresentationCompiled --> Submitted : designSubmitted
    Submitted --> ConceptDeveloped : conceptDeveloped
    Submitted --> [*]
```
