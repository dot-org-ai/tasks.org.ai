...
```mermaid
erDiagram
    ResourceAssessment ||--|{ SystemSizingStudy : informs
    SystemSizingStudy ||--o{ EquipmentSpecification : selects
    EquipmentSpecification ||--o{ SingleLineDiagram : integrates
    SingleLineDiagram ||--|| InterconnectionAgreement : governs
    SystemSizingStudy ||--o{ PerformanceModel : simulates
    PerformanceModel }o--|| CommissioningProtocol : validates
```
...
stateDiagram-v2
    [*] --> Conceptual
    Conceptual --> Assessed : resourceAssessed
    Assessed --> Sized : systemSized
    Sized --> Specified : equipmentSelected
    Specified --> Designed : electricalDesigned
    Designed --> Modeled : performanceModeled
    Modeled --> Sized : performanceShortfall
    Modeled --> Interconnected : interconnectionEngineered
    Interconnected --> ReadyToCommission : commissioningPlanDeveloped
    ReadyToCommission --> [*]
```
