...
```mermaid
erDiagram
    ProcessFlowDiagram ||--|{ MassEnergyBalance : calculates
    MassEnergyBalance ||--o{ EquipmentDataSheet : sizes
    EquipmentDataSheet ||--o{ PipingInstrumentationDiagram : connects
    PipingInstrumentationDiagram ||--|| HAZOPStudy : reviews
    EquipmentDataSheet ||--o{ PlotPlan : arranges
    PlotPlan ||--|| ConstructionPackage : compiles
```
...
stateDiagram-v2
    [*] --> Conceptual
    Conceptual --> FlowDefined : processFlowDeveloped
    FlowDefined --> Balanced : balancesCalculated
    Balanced --> Sized : equipmentSized
    Sized --> PipingComplete : pipingDesigned
    PipingComplete --> SafetyReviewed : hazopConducted
    SafetyReviewed --> Conceptual : unacceptableRisk
    SafetyReviewed --> Laid_Out : plotPlanCreated
    Laid_Out --> Released : constructionPackageIssued
    Released --> [*]
```
