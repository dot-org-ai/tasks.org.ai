...
```mermaid
erDiagram
    ConfigurationBaseline ||--|{ BillOfMaterials : specifies
    ConfigurationBaseline ||--o{ InterfaceSpecification : defines
    BillOfMaterials ||--o{ AssemblyDrawing : illustrates
    ConfigurationBaseline ||--|| IntegrationTestPlan : validates
    ConfigurationBaseline ||--o{ ChangeRequest : modifies
    ChangeRequest }o--|| CompatibilityMatrix : checks
```
...
stateDiagram-v2
    ...
    RequirementsGathered --> Configured : configurationDesigned
    Configured --> InterfacesDefined : interfacesSpecified
    InterfacesDefined --> BOMComplete : bomCreated
    BOMComplete --> Tested : integrationTested
    Tested --> Configured : failsTest
    Tested --> Baselined : baselineEstablished
    Baselined --> ChangeInReview : changeRequestProcessed
    ChangeInReview --> Configured : changeApproved
    Baselined --> [*]
```
