...
erDiagram
    ...
    Menu ||--|{ MenuCategory : organizedBy
    MenuItem ||--|| NutritionalProfile : describes
    MenuItem ||--|| CostSheet : costedBy
    SeasonalCalendar ||--|{ Menu : rotates
    MenuCategory ||--|{ MenuItem : groups
```
...
stateDiagram-v2
    [*] --> Created
    Created --> ItemsAdded : menuCreated
    ItemsAdded --> Costed : menuItemAdded
    Costed --> NutritionReview : foodCostCalculated
    NutritionReview --> ItemsAdded : out of targets
    NutritionReview --> SeasonalSetup : nutritionAnalyzed
    SeasonalSetup --> Approval : seasonalRotationSet
    Approval --> Published : menuApproved
    Published --> Created : new season
    Published --> [*]
```
...
graph TD
    ...
    C --> D{Cost Within Margin?}
    D -->|No| B
    D -->|Yes| E[analyzeNutrition]
    E --> F{Nutrition Compliant?}
    F -->|No| B
    F -->|Yes| G[setSeasonalRotation]
    G --> H[approveMenu]
    H --> I[publishMenu]
```
