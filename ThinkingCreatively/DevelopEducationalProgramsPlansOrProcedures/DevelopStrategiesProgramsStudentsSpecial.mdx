...
```mermaid
erDiagram
    EligibilityDetermination ||--|| IndividualizedEducationProgram : qualifies
    IndividualizedEducationProgram ||--|{ AccommodationPlan : includes
    IndividualizedEducationProgram ||--o| BehaviorSupportPlan : mayInclude
    IndividualizedEducationProgram ||--|{ ProgressReport : generates
    IndividualizedEducationProgram ||--o| TransitionPlan : leads_to
    ServiceDeliveryLog }o--|| IndividualizedEducationProgram : records
    ProgressReport }o--|| AccommodationPlan : evaluates
```
...
stateDiagram-v2
    [*] --> Referral
    Referral --> Evaluation : eligibilityEvaluated
    Evaluation --> GeneralEd : not eligible
    Evaluation --> IEPDevelopment : eligible
    IEPDevelopment --> Accommodated : iepDeveloped
    Accommodated --> ServiceDelivery : accommodationsDesigned
    ServiceDelivery --> Monitoring : servicesCoordinated
    Monitoring --> IEPDevelopment : goals not met
    Monitoring --> TransitionPlanning : progressMonitored
    TransitionPlanning --> [*]
    GeneralEd --> [*]
```
...
graph TD
    ...
    E --> F{Behavioral Needs?}
    F -->|Yes| G[planBehaviorSupport]
    F -->|No| H[coordinateServices]
    G --> H
    H --> I[monitorProgress]
    I --> J{Goals Met?}
    J -->|No| D
    J -->|Yes| K{Transition Age?}
    K -->|Yes| L[planTransition]
    K -->|No| I
```
