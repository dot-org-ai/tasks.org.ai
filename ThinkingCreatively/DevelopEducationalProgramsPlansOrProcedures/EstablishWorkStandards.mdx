...
```mermaid
erDiagram
    WorkStandard ||--|| TimeStudy : basedOn
    WorkStandard ||--|{ QualityThreshold : defines
    WorkStandard ||--|| StandardOperatingProcedure : documents
    WorkStandard ||--|{ ProductivityMetric : measures
    ComplianceReport }o--|| WorkStandard : evaluates
    ProductivityMetric }o--|| TimeStudy : derivedFrom
```
...
stateDiagram-v2
    [*] --> TaskAnalysis
    TaskAnalysis --> TimeStudyPhase : taskRequirementsAnalyzed
    TimeStudyPhase --> ThresholdSetting : timeStudyConducted
    ThresholdSetting --> Documentation : qualityThresholdsDefined
    Documentation --> WorkforceValidation : standardsDocumented
    WorkforceValidation --> TimeStudyPhase : not feasible
    WorkforceValidation --> Published : workforceValidated
    Published --> Monitored : standardsPublished
    Monitored --> TaskAnalysis : process changes
    Monitored --> [*]
```
...
graph TD
    ...
    E --> F{Feasible?}
    F -->|No| G{Threshold Issue?}
    G -->|Yes| C
    G -->|No| B
    F -->|Yes| H[publishStandards]
    H --> I[monitorCompliance]
    I --> J{Compliance Below 85%?}
    J -->|Yes| D
```
