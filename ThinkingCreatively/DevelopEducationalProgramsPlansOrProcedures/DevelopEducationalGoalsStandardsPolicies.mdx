...
```mermaid
erDiagram
    InstitutionalGoal ||--|{ AcademicStandard : defines
    AcademicStandard ||--|{ GovernancePolicy : implements
    GovernancePolicy ||--|{ ProcedureManual : details
    StrategicPlan ||--|{ InstitutionalGoal : drives
    AssessmentFramework }o--|| AcademicStandard : measures
    AssessmentFramework }o--|| InstitutionalGoal : evaluates
```
...
stateDiagram-v2
    [*] --> GoalSetting
    GoalSetting --> StandardsDrafting : goalsArticulated
    StandardsDrafting --> PolicyDevelopment : standardsDrafted
    PolicyDevelopment --> ProcedureWriting : policiesDeveloped
    ProcedureWriting --> StrategicAlignment : proceduresDocumented
    StrategicAlignment --> GovernanceReview : strategicPlanAligned
    GovernanceReview --> StandardsDrafting : not adopted
    GovernanceReview --> Adopted : governanceAdopted
    Adopted --> UnderReview : reviewCompleted
    UnderReview --> StandardsDrafting : revisions needed
    UnderReview --> [*]
```
...
graph TD
    ...
    F --> G{Adopted?}
    G -->|No| H{Major Revisions?}
    H -->|Yes| B
    H -->|No| D
    G -->|Yes| I[reviewAndUpdate]
