...
```mermaid
erDiagram
    BusinessPlan ||--|| MarketAnalysis : includes
    BusinessPlan ||--|| RevenueModel : projects
    BusinessPlan ||--|| ExpenseForecast : estimates
    BusinessPlan ||--|{ CapitalRequirement : specifies
    BusinessPlan ||--|{ ProFormaStatement : contains
    BusinessPlan ||--|| ExecutiveSummary : summarizedBy
    BusinessPlan ||--|{ MilestoneSchedule : tracks
    RevenueModel }o--|| ExpenseForecast : informs
```
...
stateDiagram-v2
    [*] --> Research
    Research --> Modeling : marketAnalysisCompleted
    Modeling --> Forecasting : revenueModelBuilt
    Forecasting --> CapitalPlanning : expensesForecasted
    CapitalPlanning --> ProFormaCreation : capitalNeedsCalculated
    ProFormaCreation --> SummaryWriting : proFormasCreated
    SummaryWriting --> Presentation : executiveSummaryWritten
    Presentation --> Revision : not approved
    Revision --> Modeling : planRevised
    Presentation --> Finalized : planPresented
    Finalized --> [*]
```
