...
```mermaid
erDiagram
    ProgramGoal ||--|{ PerformanceMetric : measuredBy
    ProgramGoal }o--|| NeedsAssessment : informed_by
    ProgramPlan ||--|{ Milestone : contains
    ProgramPlan ||--|| ProgramGoal : achieves
    StakeholderAlignment }o--|| ProgramGoal : endorses
    PerformanceMetric }o--|| Milestone : tracks
```
...
stateDiagram-v2
    [*] --> Assessment
    Assessment --> GoalSetting : needsAssessmentCompleted
    GoalSetting --> MetricDefinition : goalDefined
    MetricDefinition --> Planning : performanceMetricsSet
    Planning --> Alignment : programPlanCreated
    Alignment --> GoalSetting : not aligned
    Alignment --> Approved : stakeholdersAligned
    Approved --> Active : programPlanApproved
    Active --> GoalSetting : programGoalsRevised
    Active --> [*]
```
...
graph TD
    ...
    E --> F{Aligned?}
    F -->|No| G{Goal or Plan Issue?}
    G -->|Goal| B
    G -->|Plan| D
    F -->|Yes| H[approveProgramPlan]
