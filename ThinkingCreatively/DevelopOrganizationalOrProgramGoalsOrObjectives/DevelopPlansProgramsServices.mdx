...
```mermaid
erDiagram
    ServicePlan ||--|| ProgramDesign : implements
    ServicePlan ||--|{ ResourceAllocation : allocates
    ServicePlan ||--|| ReadinessAssessment : evaluatedBy
    ServicePlan ||--|| DeliveryModel : delivers_through
    ServicePlan ||--|| ImplementationTimeline : scheduledBy
    ReadinessAssessment }o--|| ResourceAllocation : checks
    ProgramDesign }o--|| DeliveryModel : defines
```
...
stateDiagram-v2
    [*] --> NeedIdentified
    NeedIdentified --> ModelDesigned : serviceNeedIdentified
    ModelDesigned --> PlanDeveloped : serviceModelDesigned
    PlanDeveloped --> Resourced : servicePlanDeveloped
    Resourced --> ReadinessCheck : resourcesAllocated
    ReadinessCheck --> PlanDeveloped : not ready
    ReadinessCheck --> Approved : launchReadinessAssessed
    Approved --> Implementation : servicePlanApproved
    Implementation --> PlanDeveloped : servicePlanRevised
    Implementation --> [*]
```
...
graph TD
    ...
    E --> F{Ready?}
    F -->|No| G{Resource or Design Gap?}
    G -->|Resources| D
    G -->|Design| B
    F -->|Yes| H[approveServicePlan]
