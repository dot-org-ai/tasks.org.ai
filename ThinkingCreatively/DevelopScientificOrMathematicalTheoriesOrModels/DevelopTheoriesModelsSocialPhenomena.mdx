...
```mermaid
erDiagram
    Theory ||--|{ Hypothesis : generates
    Theory ||--|| SocialModel : formalizedAs
    SocialModel ||--|{ Variable : contains
    SocialModel ||--|{ Simulation : testedBy
    Dataset ||--|{ Variable : measures
    Simulation }o--|| Dataset : validatedAgainst
```
...
stateDiagram-v2
    [*] --> HypothesisReady
    HypothesisReady --> StudyDesigned : hypothesisFormulated
    StudyDesigned --> DataReady : studyDesigned
    DataReady --> ModelBuilding : dataCollected
    ModelBuilding --> Simulating : modelBuilt
    Simulating --> Validating : simulationCompleted
    Validating --> ModelBuilding : model invalid
    Validating --> Published : modelValidated
    Published --> [*]
```
