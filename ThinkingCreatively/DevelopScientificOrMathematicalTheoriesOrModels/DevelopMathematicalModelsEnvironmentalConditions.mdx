...
```mermaid
erDiagram
    EnvironmentalModel ||--|| ParameterSet : configuredBy
    EnvironmentalModel ||--|{ SimulationRun : executedAs
    EnvironmentalModel ||--|{ CalibrationResult : calibratedBy
    EnvironmentalModel ||--|{ ForecastScenario : projects
    ObservationDataset ||--|{ CalibrationResult : validatesAgainst
    SimulationRun }o--|| ForecastScenario : produces
    ParameterSet }o--|{ CalibrationResult : adjustedBy
```
...
stateDiagram-v2
    [*] --> DataAcquisition
    DataAcquisition --> StructureDefined : dataAcquired
    StructureDefined --> ParameterEstimation : modelStructureDefined
    ParameterEstimation --> Simulating : parametersEstimated
    Simulating --> Calibrating : simulationCompleted
    Calibrating --> ParameterEstimation : calibration failed
    Calibrating --> Forecasting : modelCalibrated
    Forecasting --> Exported : forecastGenerated
    Exported --> DataAcquisition : new data available
    Exported --> [*]
```
...
graph TD
    ...
    H --> I{Forecast Reasonable?}
    I -->|No| J[Review Model Structure]
    J --> B
    I -->|Yes| K[exportResults]
```
