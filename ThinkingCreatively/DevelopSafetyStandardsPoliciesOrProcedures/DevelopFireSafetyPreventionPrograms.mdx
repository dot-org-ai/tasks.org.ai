...
```mermaid
erDiagram
    PreventionProgram ||--|{ RiskAssessment : evaluates
    PreventionProgram ||--|| EvacuationPlan : includes
    PreventionProgram ||--|{ InspectionSchedule : schedules
    PreventionProgram ||--|| TrainingCurriculum : delivers
    PreventionProgram ||--|{ ComplianceRecord : tracks
    RiskAssessment }o--|{ EvacuationPlan : informs
    InspectionSchedule }o--|{ ComplianceRecord : generates
```
...
stateDiagram-v2
    [*] --> Created
    Created --> Assessed : preventionProgramCreated
    Assessed --> EvacuationPlanned : riskAssessmentCompleted
    EvacuationPlanned --> TrainingReady : evacuationPlanDesigned
    TrainingReady --> InspectionActive : trainingDelivered
    InspectionActive --> Compliant : inspectionCompleted
    InspectionActive --> NonCompliant : complianceViolationDetected
    NonCompliant --> Assessed : remediation needed
    Compliant --> Reviewed : programEffectivenessReviewed
    Reviewed --> Assessed : improvements needed
    Reviewed --> [*]
```
