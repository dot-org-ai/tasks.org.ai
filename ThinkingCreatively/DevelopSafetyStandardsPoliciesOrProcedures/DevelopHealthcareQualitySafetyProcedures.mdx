...
```mermaid
erDiagram
    Procedure ||--|{ QualityMetric : measures
    Procedure ||--|{ TrainingModule : taughtBy
    Procedure ||--|{ AuditRecord : auditedBy
    GapAnalysis ||--|{ Procedure : informs
    IncidentReport }o--|| Procedure : triggersUpdate
    AuditRecord }o--|{ QualityMetric : evaluates
```
...
stateDiagram-v2
    [*] --> GapAssessment
    GapAssessment --> EvidenceReview : gapAnalysisCompleted
    EvidenceReview --> Drafting : evidenceReviewed
    Drafting --> Approval : procedureDrafted
    Approval --> Drafting : revisions needed
    Approval --> Training : procedureApproved
    Training --> Monitoring : trainingDeployed
    Monitoring --> Updating : complianceAudited below threshold
    Updating --> Approval : procedureUpdated
    Monitoring --> Monitoring : compliant
    Monitoring --> [*]
```
...
graph TD
    ...
    E -->|No| F[Revise Draft]
    F --> C
    E -->|Yes| G[deployTraining]
    G --> H[monitorCompliance]
    H --> I{Compliant?}
    I -->|No| J[updateProcedure]
    J --> D
