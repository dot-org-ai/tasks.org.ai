...
```mermaid
erDiagram
    Choreography ||--|{ MovementPhrase : "composed of"
    Choreography ||--|{ FormationChart : "staged via"
    Choreography ||--o{ RehearsalSession : "practiced in"
    Choreography ||--|| ProductionBrief : "guided by"
    Choreography ||--o| DanceNotation : "documented in"
    MovementPhrase }o--|| FormationChart : "positioned by"
```
...
stateDiagram-v2
    [*] --> ConceptDeveloped
    ConceptDeveloped --> PhraseDesigned : movementPhraseDesigned
    PhraseDesigned --> FormationsSet : formationsArranged
    FormationsSet --> RehearsalBooked : rehearsalScheduled
    RehearsalBooked --> PerformanceReviewed : performanceReviewed
    PerformanceReviewed --> PhraseDesigned : movementPhraseDesigned
    PerformanceReviewed --> Notated : choreographyNotated
    Notated --> Finalized : choreographyFinalized
    Finalized --> [*]
```
