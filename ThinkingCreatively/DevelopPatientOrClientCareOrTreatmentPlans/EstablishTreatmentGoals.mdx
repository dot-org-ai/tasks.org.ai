...
```mermaid
erDiagram
    TreatmentGoal ||--|{ GoalMilestone : contains
    TreatmentGoal ||--|| Assessment : basedOn
    TreatmentGoal ||--|| Benchmark : comparedTo
    TreatmentGoal ||--|| PatientPriority : reflects
    GoalReview }o--|| TreatmentGoal : evaluates
    GoalMilestone }o--|| Benchmark : measured_against
```
...
stateDiagram-v2
    [*] --> ClinicalAssessment
    ClinicalAssessment --> PriorityGathering : assessmentCompleted
    PriorityGathering --> GoalDefinition : prioritiesGathered
    GoalDefinition --> MilestoneSetup : goalDefined
    MilestoneSetup --> Prioritization : milestonesSet
    Prioritization --> Approved : goalsPrioritized
    Approved --> Monitoring : goalsApproved
    Monitoring --> Achieved : milestoneReached
    Monitoring --> Monitoring : continue tracking
    Achieved --> GoalDefinition : new goals needed
    Achieved --> [*]
```
