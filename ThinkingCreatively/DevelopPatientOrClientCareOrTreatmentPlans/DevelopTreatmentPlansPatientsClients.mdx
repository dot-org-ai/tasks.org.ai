...
```mermaid
erDiagram
    TreatmentPlan ||--|| Diagnosis : addresses
    TreatmentPlan ||--|{ Intervention : includes
    TreatmentPlan ||--|{ Goal : defines
    TreatmentPlan ||--|| CareEpisode : spans
    ClinicalGuideline }o--|| Intervention : informs
    Goal }o--|| CareEpisode : trackedWithin
...
stateDiagram-v2
    [*] --> Assessment
    Assessment --> DiagnosticReview : patientAssessed
    DiagnosticReview --> InterventionSelection : diagnosticsReviewed
    InterventionSelection --> GoalSetting : interventionsSelected
    GoalSetting --> Drafting : goalsEstablished
    Drafting --> Approval : planDrafted
    Approval --> InterventionSelection : not approved
    Approval --> Active : planApproved
    Active --> Assessment : periodic review
    Active --> [*]
```
...
graph TD
    ...
    F --> G{Approved?}
    G -->|No| H{Clinical or Payer Issue?}
    H -->|Clinical| C
    H -->|Payer| E
    G -->|Yes| I[activatePlan]
```
