...
```mermaid
erDiagram
    EnergyAudit ||--|{ EfficiencyTarget : establishes
    EquipmentSpecification ||--|{ EfficiencyTarget : meets
    EquipmentSpecification ||--o| HeatRecoveryDesign : includes
    EquipmentSpecification ||--|| ControlStrategy : controlledBy
    PerformanceSimulation }o--|| EquipmentSpecification : models
    CommissioningReport }o--|| PerformanceSimulation : verifies
```
...
stateDiagram-v2
    [*] --> Audited
    Audited --> TargetsSet : energyAuditCompleted
    TargetsSet --> Specified : efficiencyTargetsSet
    Specified --> HeatRecovery : equipmentSpecified
    HeatRecovery --> ControlDesign : heatRecoveryDesigned
    ControlDesign --> Simulating : controlStrategyDeveloped
    Simulating --> Specified : below target
    Simulating --> Commissioning : performanceSimulated
    Commissioning --> [*]
```
...
graph TD
    ...
    F --> G{Meets Target?}
    G -->|No| H{Equipment or Controls Issue?}
    H -->|Equipment| C
    H -->|Controls| E
    G -->|Yes| I[commissionSystem]
```
