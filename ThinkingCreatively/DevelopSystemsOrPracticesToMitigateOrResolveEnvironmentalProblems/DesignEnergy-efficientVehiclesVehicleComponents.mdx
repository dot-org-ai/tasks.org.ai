...
```mermaid
erDiagram
    EfficiencyTarget ||--|| PowertrainDesign : constrains
    PowertrainDesign ||--|| AerodynamicModel : complementedBy
    PowertrainDesign ||--|| LightweightDesign : supports
    PowertrainDesign ||--|| BatteryPackDesign : poweredBy
    DriveCycleSimulation }o--|| PowertrainDesign : evaluates
    CertificationPackage }o--|| DriveCycleSimulation : documents
```
...
stateDiagram-v2
    [*] --> TargetsSet
    TargetsSet --> PowertrainReady : efficiencyTargetsSet
    PowertrainReady --> AeroOptimized : powertrainDesigned
    AeroOptimized --> MassReduced : aerodynamicsOptimized
    MassReduced --> BatteryReady : massReduced
    BatteryReady --> Simulating : batterySystemDesigned
    Simulating --> PowertrainReady : below target
    Simulating --> Certifying : driveCycleSimulated
    Certifying --> [*]
```
...
graph TD
    ...
    F --> G{Meets Target?}
    G -->|No| H{Powertrain or Aero Issue?}
    H -->|Powertrain| B
    H -->|Aero/Mass| C
    G -->|Yes| I[certifyVehicle]
```
