...
```mermaid
erDiagram
    MenuForecast ||--|{ IngredientRequirement : "drives"
    Recipe ||--|{ IngredientRequirement : "specifies"
    IngredientRequirement ||--o{ SupplyOrder : "triggers"
    StaffSchedule }o--|| PrepSheet : "staffs"
    PrepSheet ||--|{ IngredientRequirement : "consumes"
    MenuForecast ||--|| StaffSchedule : "informs"
```
...
stateDiagram-v2
    [*] --> Forecasting
    Forecasting --> IngredientsCalculated : demandForecasted
    IngredientsCalculated --> WasteAdjusted : ingredientsCalculated
    WasteAdjusted --> StockCheck : wasteAdjusted
    StockCheck --> StaffPlanning : stockSufficient
    StockCheck --> Ordering : stockShortageDetected
    Ordering --> StaffPlanning : supplyOrderPlaced
    StaffPlanning --> PrepReady : staffingEstimated
    PrepReady --> [*] : prepSheetGenerated
```
...
graph TD
    ...
    H -->|Yes| F
    H -->|No| I[Adjust Menu Forecast]
    I --> B
    F --> J[estimateStaffing]
```
