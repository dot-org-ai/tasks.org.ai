...
```mermaid
erDiagram
    CostEstimate ||--|{ MaterialSchedule : "itemizes"
    MaterialSchedule ||--o{ PriceComparison : "evaluated-by"
    PriceComparison ||--|{ UnitRate : "contains"
    CostEstimate ||--|| ContingencyAllowance : "includes"
    CostEstimate ||--|| CostBaseline : "approved-as"
    MaterialSchedule }o--|| UnitRate : "priced-at"
```
...
stateDiagram-v2
    [*] --> ScopeDefinition
    ScopeDefinition --> QuantitiesCalculated : estimateScopeDefined
    QuantitiesCalculated --> PricingPhase : materialQuantitiesCalculated
    PricingPhase --> RateAnalysis : supplierPricingObtained
    RateAnalysis --> ContingencyAdded : unitRatesAnalyzed
    ContingencyAdded --> BudgetReview : contingencyApplied
    BudgetReview --> QuantitiesCalculated : overBudget
    BudgetReview --> Approved : budgetReconciled
    Approved --> [*] : costBaselineApproved
```
...
graph TD
    ...
    H -->|Yes| B
    H -->|Request More Funds| I[reconcileWithBudget]
    G -->|Yes| J[approveCostBaseline]
```
