...
```mermaid
erDiagram
    QuantityTakeoff ||--|{ BillOfMaterials : "populates"
    BillOfMaterials ||--o{ WasteFactor : "adjusted-by"
    BillOfMaterials ||--o{ MaterialPriceQuote : "priced-by"
    MaterialPriceQuote }o--|| ProcurementSchedule : "scheduled-in"
    BillOfMaterials ||--|| ProcurementSchedule : "ordered-via"
```
...
stateDiagram-v2
    [*] --> DesignReview
    DesignReview --> TakeoffInProgress : designDocumentsReviewed
    TakeoffInProgress --> WasteAdjusted : quantityTakeoffCompleted
    WasteAdjusted --> InventoryCheck : wasteFactorsApplied
    InventoryCheck --> Pricing : inventoryChecked
    Pricing --> BudgetReview : materialsPriced
    BudgetReview --> TakeoffInProgress : overBudget
    BudgetReview --> BOMReady : withinBudget
    BOMReady --> Scheduled : billOfMaterialsBuilt
    Scheduled --> [*] : procurementScheduled
```
...
graph TD
    ...
    G -->|Yes| B
    G -->|Request Increase| H[buildBillOfMaterials]
    F -->|Yes| H
