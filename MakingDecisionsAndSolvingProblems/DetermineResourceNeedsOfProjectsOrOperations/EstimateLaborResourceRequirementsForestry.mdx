...
```mermaid
erDiagram
    OperationPlan ||--|{ LaborEstimate : "staffs"
    OperationPlan ||--|{ EquipmentAllocation : "provisions"
    YieldForecast ||--|| OperationPlan : "drives"
    SeasonalCalendar ||--|| OperationPlan : "schedules"
    ComplianceQuota ||--o{ OperationPlan : "constrains"
    LaborEstimate }o--|| EquipmentAllocation : "coordinates-with"
```
...
stateDiagram-v2
    [*] --> WorkloadAssessment
    WorkloadAssessment --> YieldProjection : workloadAssessed
    YieldProjection --> CrewPlanning : yieldForecasted
    CrewPlanning --> EquipmentAssignment : crewSizeEstimated
    EquipmentAssignment --> ComplianceCheck : equipmentAllocated
    ComplianceCheck --> CrewPlanning : quotaExceeded
    ComplianceCheck --> PlanReady : complianceVerified
    PlanReady --> Adjusting : conditionsChanged
    Adjusting --> CrewPlanning : estimatesAdjusted
    PlanReady --> [*] : operationPlanBuilt
```
