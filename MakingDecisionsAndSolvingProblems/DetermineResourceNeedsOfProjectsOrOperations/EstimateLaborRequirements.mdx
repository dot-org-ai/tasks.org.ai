...
```mermaid
erDiagram
    WorkloadForecast ||--|{ LaborRequirement : "drives"
    LaborRequirement ||--o{ SkillProfile : "requires"
    LaborRequirement ||--|| LaborCostModel : "costed-by"
    StaffingPlan ||--|{ LaborRequirement : "fulfills"
    StaffingPlan }o--|| ProductivityBenchmark : "benchmarked-against"
    LaborCostModel }o--|| ProductivityBenchmark : "calibrated-by"
```
...
stateDiagram-v2
    [*] --> WorkloadAnalysis
    WorkloadAnalysis --> StaffingCalculation : workloadAnalyzed
    StaffingCalculation --> SkillMatching : staffingNeedsCalculated
    SkillMatching --> CostProjection : skillProfilesMatched
    CostProjection --> BudgetValidation : laborCostsProjected
    BudgetValidation --> StaffingCalculation : overBudget
    BudgetValidation --> PlanAssembly : budgetValidated
    PlanAssembly --> Approved : staffingPlanBuilt
    Approved --> [*] : staffingPlanApproved
```
...
graph TD
    ...
    F -->|No| G{Reduce Scope?}
    G -->|Yes| B
    G -->|Request Exception| H[buildStaffingPlan]
    F -->|Yes| H
