...
```mermaid
erDiagram
    MaterialRequirement ||--|{ SourceOption : "sourced-from"
    MaterialRequirement ||--|{ PropItem : "specifies"
    PropItem }o--|| SourceOption : "available-from"
    PropItem ||--|| SelectionApproval : "authorized-by"
    SelectionApproval }o--|| BudgetAllocation : "funded-by"
```
...
stateDiagram-v2
    [*] --> RequirementsDefined
    RequirementsDefined --> SourcesEvaluated : requirementsDefined
    SourcesEvaluated --> SamplesReviewed : sourcesEvaluated
    SamplesReviewed --> Selected : samplesReceived
    Selected --> AwaitingApproval : materialsSelected
    AwaitingApproval --> Selected : notApproved
    AwaitingApproval --> Ordered : selectionApproved
    Ordered --> Received : orderPlaced
...
graph TD
    ...
    F -->|Yes| G[placeOrder]
    F -->|No| D
    G --> H{Materials Received?}
    H -->|No| I[Follow Up with Vendor]
    I --> H
    H -->|Yes| J[Inspect and Accept]
```
