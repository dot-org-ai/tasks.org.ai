...
```mermaid
erDiagram
    SiteAssessment ||--|| EquipmentSpecification : "determines"
    EquipmentSpecification ||--|{ CostComparison : "analyzed-in"
    CostComparison }o--|| RentalAgreement : "may-produce"
    EquipmentSpecification ||--|| FleetPlan : "scheduled-in"
    FleetPlan ||--|{ MaintenanceSchedule : "supported-by"
    RentalAgreement }o--|| FleetPlan : "included-in"
```
...
stateDiagram-v2
    [*] --> SiteAssessment
    SiteAssessment --> RequirementsDefined : siteAssessed
    RequirementsDefined --> Evaluation : requirementsDefined
    Evaluation --> CostAnalysis : optionsEvaluated
    CostAnalysis --> OwnDecision : costAnalyzed
    CostAnalysis --> RentDecision : costAnalyzed
    OwnDecision --> Selected : equipmentSelected
    RentDecision --> RentalSecured : rentalArranged
    Selected --> FleetPlanning : fleetPlanned
    RentalSecured --> FleetPlanning : fleetPlanned
    FleetPlanning --> [*]
```
