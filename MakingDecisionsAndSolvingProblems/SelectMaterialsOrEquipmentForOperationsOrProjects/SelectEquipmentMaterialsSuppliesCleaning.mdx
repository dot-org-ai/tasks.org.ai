...
```mermaid
erDiagram
    SurfaceProfile ||--|{ CleaningProduct : "requires"
    CleaningProduct ||--|| SafetyDataSheet : "documented-by"
    CleaningProduct }o--|| ApprovedProductList : "listed-in"
    MaintenanceEquipment }o--|| ApprovedProductList : "listed-in"
    ApprovedProductList ||--|{ SupplyInventory : "tracked-in"
    SurfaceProfile ||--o{ MaintenanceEquipment : "serviced-by"
```
...
stateDiagram-v2
    [*] --> SurfaceAssessment
    SurfaceAssessment --> ProductEvaluation : surfacesAssessed
    ProductEvaluation --> SafetyReview : productsEvaluated
    SafetyReview --> ProductEvaluation : notSafe
    SafetyReview --> Standardization : safetyDataReviewed
    Standardization --> EquipmentSelection : productsStandardized
    EquipmentSelection --> Ordering : equipmentSelected
    Ordering --> ConsumptionTracking : suppliesOrdered
    ConsumptionTracking --> Ordering : reorderNeeded
    ConsumptionTracking --> [*] : consumptionTracked
```
