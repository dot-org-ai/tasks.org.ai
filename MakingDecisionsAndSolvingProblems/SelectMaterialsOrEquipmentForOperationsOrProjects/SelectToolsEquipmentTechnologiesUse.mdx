...
```mermaid
erDiagram
    RequirementProfile ||--|{ ToolCandidate : "evaluated-against"
    ToolCandidate ||--|| EvaluationMatrix : "scored-in"
    ToolCandidate ||--o{ ProofOfConcept : "tested-in"
    EvaluationMatrix ||--|| SelectionRecommendation : "produces"
    SelectionRecommendation }o--|| DeploymentPlan : "enables"
    ProofOfConcept }o--|| SelectionRecommendation : "validates"
```
...
stateDiagram-v2
    [*] --> RequirementsDefined
    RequirementsDefined --> Researching : requirementsDefined
    Researching --> Evaluation : candidatesResearched
    Evaluation --> ProofOfConcept : evaluationMatrixCompleted
    ProofOfConcept --> RecommendationReady : proofOfConceptCompleted
    RecommendationReady --> AwaitingApproval : recommendationPrepared
    AwaitingApproval --> Evaluation : selectionDenied
    AwaitingApproval --> DeploymentPlanning : selectionApproved
    DeploymentPlanning --> [*] : deploymentPlanned
```
...
graph TD
    ...
    D --> E{POC Successful?}
    E -->|Yes| F[prepareRecommendation]
    E -->|No| C
    F --> G[approveSelection]
    G --> H{Approved?}
    H -->|Yes| I[planDeployment]
    H -->|No| C
```
