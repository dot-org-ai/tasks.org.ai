...
```mermaid
erDiagram
    TaskRequirement ||--|{ ResourceOption : "matched-by"
    ResourceOption ||--|| AvailabilityRecord : "constrained-by"
    TaskRequirement ||--|| AllocationPlan : "planned-in"
    AllocationPlan ||--|{ ResourceRequest : "generates"
    ResourceRequest }o--|| CostEstimate : "priced-in"
    AllocationPlan }o--|| CostEstimate : "budgeted-by"
```
...
stateDiagram-v2
    ...
    RequirementsAnalysis --> AvailabilityCheck : requirementsAnalyzed
    AvailabilityCheck --> OptionEvaluation : availabilityChecked
    OptionEvaluation --> Selection : optionsEvaluated
    Selection --> PlanCreation : resourcesSelected
    PlanCreation --> AwaitingApproval : allocationPlanCreated
    AwaitingApproval --> OptionEvaluation : approvalDenied
    AwaitingApproval --> Allocated : approvalGranted
...
graph TD
    ...
    C -->|Yes| D[evaluateOptions]
    C -->|No| E[Source Externally]
    E --> D
    D --> F[selectResources]
    F --> G[createAllocationPlan]
    G --> H[requestApproval]
    H --> I{Approved?}
    I -->|Yes| J[allocateResources]
    I -->|No| D
```
