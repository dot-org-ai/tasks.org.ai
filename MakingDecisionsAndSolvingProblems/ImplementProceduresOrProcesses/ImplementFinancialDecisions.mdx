...
```mermaid
erDiagram
    FinancialDecision ||--|{ Transaction : "executed-as"
    FinancialDecision ||--|| ComplianceCheck : "verified-by"
    Transaction ||--|| AuditTrail : "tracked-in"
    FinancialDecision ||--|{ CapitalAllocation : "distributes"
    FinancialDecision ||--|| FinancialPolicy : "governed-by"
    CapitalAllocation }o--|| AuditTrail : "recorded-in"
```
...
stateDiagram-v2
    [*] --> ComplianceReview
    ComplianceReview --> Compliant : complianceVerified
    ComplianceReview --> RevisionRequired : nonCompliant
    RevisionRequired --> ComplianceReview : revised
    Compliant --> Executed : transactionExecuted
    Executed --> Recorded : transactionRecorded
    Recorded --> Allocated : capitalAllocated
    Allocated --> Monitoring : outcomeMonitored
    Monitoring --> Escalated : offTrack
    Escalated --> Monitoring : adjusted
    Monitoring --> Reported : resultsReported
    Reported --> [*]
```
