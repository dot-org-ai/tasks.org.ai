...
```mermaid
erDiagram
    CampaignBrief ||--|| Campaign : "defines"
    Campaign ||--|{ CreativeAsset : "uses"
    Campaign ||--|| MediaPlan : "scheduled-by"
    Campaign ||--|{ AudienceSegment : "targets"
    Campaign ||--|| PerformanceReport : "measured-by"
    MediaPlan ||--|{ AudienceSegment : "reaches"
```
...
stateDiagram-v2
    [*] --> Briefing
    Briefing --> CreativeProduction : campaignBriefCreated
    CreativeProduction --> MediaPlanning : creativeAssetsApproved
    CreativeProduction --> Briefing : creativeRejected
    MediaPlanning --> ReadyToLaunch : mediaPlanFinalized
    ReadyToLaunch --> Live : campaignLaunched
    Live --> Optimizing : performanceThresholdReached
    Optimizing --> Live : campaignOptimized
    Live --> Closed : campaignClosed
    Closed --> [*]
```
