...
```mermaid
erDiagram
    ExpenditureRequest ||--|| BudgetAllocation : "charged to"
    ExpenditureRequest ||--|| ApprovalRecord : "decided in"
    ExpenditureRequest ||--o| PurchaseOrder : generates
    PurchaseOrder ||--|| DisbursementRecord : "paid via"
    SpendingPolicy }o--|| ExpenditureRequest : governs
    ApprovalRecord }o--|| SpendingPolicy : "validated against"
```
...
stateDiagram-v2
    ...
    Submitted --> Validated : budgetValidated
    Validated --> Routed : approvalRouted
    Routed --> Approved : expenditureApproved
    Routed --> Rejected : expenditureRejected
    Approved --> Paid : paymentProcessed
    Rejected --> Submitted : requestSubmitted
    Validated --> Flagged : policyViolationDetected
    Flagged --> Rejected : expenditureRejected
    Paid --> [*]
```
