...
```mermaid
erDiagram
    FinancialAction ||--|| AuthorizationRequest : "requires"
    AuthorizationRequest ||--|| RiskAssessment : "triggers"
    AuthorizationRequest ||--|| ComplianceCheck : "undergoes"
    AuthorizationRequest }o--|| AuthorityMatrix : "validated against"
    FinancialAction ||--o{ AuditTrail : "generates"
    RiskAssessment }o--|| AuditTrail : "recorded in"
```
...
```mermaid
stateDiagram-v2
    [*] --> Initiated : actionInitiated
    Initiated --> AuthorityChecked : authorityVerified
    AuthorityChecked --> RiskReviewed : riskAssessed
    RiskReviewed --> ComplianceReviewed : complianceChecked
    ComplianceReviewed --> Authorized : actionAuthorized
    ComplianceReviewed --> Denied : actionDenied
    Initiated --> Flagged : unauthorizedAttemptDetected
    Flagged --> Initiated : actionInitiated
    Denied --> [*]
    Authorized --> [*]
```
