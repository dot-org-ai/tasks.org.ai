...
```mermaid
erDiagram
    CostSheet ||--|{ MaterialCost : "includes"
    CostSheet ||--|{ LaborCost : "includes"
    CostSheet ||--|| OverheadAllocation : "applies"
    CostSheet ||--o{ CostStandard : "compared-to"
    CostSheet ||--|| MarginAnalysis : "feeds"
    CostStandard }o--|| MarginAnalysis : "benchmarks"
```
...
```mermaid
stateDiagram-v2
    [*] --> DirectCostCalculation
    DirectCostCalculation --> OverheadApplication : directCostsCalculated
    OverheadApplication --> TotalComputed : overheadAllocated
    TotalComputed --> StandardComparison : totalCostComputed
    StandardComparison --> RateUpdate : standardVarianceDetected
    RateUpdate --> DirectCostCalculation : costRatesUpdated
    StandardComparison --> MarginReview : withinStandard
    MarginReview --> ReportGeneration : marginAnalyzed
    ReportGeneration --> [*] : costReportGenerated
```
...
graph TD
    ...
    G --> H{Margin Target Met?}
    H -->|No| I[Review Cost Drivers]
    I --> A
