...
```mermaid
erDiagram
    CostEstimate ||--|| SalesQuotation : "priced-in"
    SalesQuotation ||--|| PaymentTerms : "governed-by"
    SalesQuotation ||--|| DeliveryTerms : "shipped-per"
    SalesQuotation ||--o{ DiscountSchedule : "discounted-by"
    SalesQuotation ||--|| ProposalDocument : "packaged-in"
    CostEstimate }o--|| DiscountSchedule : "adjusted-by"
```
...
stateDiagram-v2
    [*] --> CostEstimation
    CostEstimation --> TermsConfiguration : costsEstimated
    TermsConfiguration --> DiscountReview : termsConfigured
    DiscountReview --> QuotationReady : discountApplied
    QuotationReady --> Issued : quotationGenerated
    Issued --> Negotiating : termsNegotiated
    Negotiating --> ApprovalPending : nonStandardTerms
    ApprovalPending --> Issued : estimateApproved
    Negotiating --> Issued : withinPolicy
    Issued --> Converted : estimateConverted
    Issued --> Archived : expired
    Converted --> [*]
```
