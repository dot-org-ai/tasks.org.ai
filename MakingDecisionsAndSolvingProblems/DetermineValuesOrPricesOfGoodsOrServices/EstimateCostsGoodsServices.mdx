...
```mermaid
erDiagram
    CostEstimate ||--|| ScopeDefinition : "scoped-by"
    CostEstimate ||--|| EstimationMethod : "uses"
    CostEstimate ||--o{ PriceBenchmark : "references"
    CostEstimate ||--|| ConfidenceRange : "assessed-at"
    CostEstimate ||--|{ CostDriver : "driven-by"
    PriceBenchmark }o--|| CostDriver : "influences"
```
...
stateDiagram-v2
    [*] --> ScopeDefinition
    ScopeDefinition --> MarketResearch : scopeDefined
    MarketResearch --> MethodSelection : marketPricingResearched
    MethodSelection --> EstimateBuildup : estimationMethodSelected
    EstimateBuildup --> ConfidenceCheck : estimateBuilt
    ConfidenceCheck --> MethodSelection : lowConfidence
    ConfidenceCheck --> Submitted : confidenceAssessed
    Submitted --> Reconciliation : estimateSubmitted
    Reconciliation --> EstimateBuildup : estimateDeviationDetected
    Reconciliation --> [*]
```
