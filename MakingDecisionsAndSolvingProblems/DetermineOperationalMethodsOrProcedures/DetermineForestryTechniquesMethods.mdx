...
```mermaid
erDiagram
    ForestStand ||--|| TimberInventory : "assessed by"
    ForestStand ||--|| SilviculturalPrescription : "treated by"
    SilviculturalPrescription ||--|| HarvestPlan : "guides"
    SilviculturalPrescription ||--|| RegenerationPlan : "includes"
    SilviculturalPrescription }o--|| BestManagementPractice : "complies with"
    HarvestPlan }o--|| TimberInventory : "based on"
```
...
```mermaid
stateDiagram-v2
    [*] --> Inventoried : forestStandInventoried
    Inventoried --> SiteAssessed : siteConditionsAssessed
    SiteAssessed --> TechniquesIdentified : candidateTechniquesIdentified
    TechniquesIdentified --> OutcomesProjected : techniqueOutcomesEvaluated
    OutcomesProjected --> MethodSelected : forestryMethodSelected
    MethodSelected --> PrescriptionReady : prescriptionDeveloped
    PrescriptionReady --> ComplianceReviewed : regulatoryComplianceReviewed
    ComplianceReviewed --> MethodSelected : forestryMethodSelected
    ComplianceReviewed --> [*]
```
