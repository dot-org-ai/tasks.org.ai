...
```mermaid
erDiagram
    AnalysisRequest ||--|| DataProfile : "profiled by"
    AnalysisRequest ||--o{ MethodCandidate : "generates"
    MethodCandidate ||--|| MethodAssessment : "evaluated in"
    MethodCandidate ||--|| AssumptionCheck : "validated by"
    AnalysisRequest ||--|| AnalysisPlan : "produces"
    MethodAssessment }o--|| AnalysisPlan : "informs"
```
...
```mermaid
stateDiagram-v2
    [*] --> Profiled : dataProfiled
    Profiled --> CandidatesReady : candidateMethodsIdentified
    CandidatesReady --> Assessed : methodAssessed
    Assessed --> AssumptionsVerified : assumptionsChecked
    AssumptionsVerified --> Selected : methodSelected
    AssumptionsVerified --> CandidatesReady : candidateMethodsIdentified
    Selected --> Documented : analysisPlanDocumented
    Documented --> Validated : methodValidated
    Validated --> [*]
```
