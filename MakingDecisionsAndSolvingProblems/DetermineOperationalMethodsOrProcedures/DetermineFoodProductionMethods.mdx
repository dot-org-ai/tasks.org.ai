...
```mermaid
erDiagram
    ProductionMethod ||--|{ Recipe : "processes"
    ProductionMethod ||--|| HACCPPlan : "governed by"
    ProductionMethod ||--|| ProductionLine : "runs on"
    Recipe ||--|| QualityStandard : "must meet"
    ProductionMethod ||--|| ShelfLifeProfile : "determines"
    QualityStandard }o--|| HACCPPlan : "enforced through"
```
...
```mermaid
stateDiagram-v2
    [*] --> RequirementsAnalyzed : productRequirementsAnalyzed
    RequirementsAnalyzed --> MethodsEvaluated : processingMethodsEvaluated
    MethodsEvaluated --> TrialComplete : trialRunCompleted
    TrialComplete --> MethodsEvaluated : processingMethodsEvaluated
    TrialComplete --> ShelfLifeChecked : shelfLifeAssessed
    ShelfLifeChecked --> MethodChosen : productionMethodSelected
    MethodChosen --> HACCPReady : haccpPlanDeveloped
    HACCPReady --> Documented : methodSpecificationDocumented
    Documented --> [*]
```
