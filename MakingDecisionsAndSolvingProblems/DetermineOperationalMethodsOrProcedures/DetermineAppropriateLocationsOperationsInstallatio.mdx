...
```mermaid
erDiagram
    CandidateSite ||--|| SiteAssessment : "evaluated by"
    CandidateSite ||--|| CostModel : "costed in"
    CandidateSite ||--o| EnvironmentalReview : "reviewed by"
    CandidateSite ||--|| InfrastructureReport : "inventoried in"
    SelectionCriteria ||--o{ SiteAssessment : "applied to"
    SiteAssessment }o--|| CostModel : "informs"
```
...
```mermaid
stateDiagram-v2
    [*] --> CriteriaDefined : selectionCriteriaDefined
    CriteriaDefined --> SitesIdentified : candidateSitesIdentified
    SitesIdentified --> Assessed : siteSuitabilityAssessed
    Assessed --> Costed : locationCostsEstimated
    Costed --> EnvironmentallyReviewed : environmentalReviewCompleted
    EnvironmentallyReviewed --> Ranked : locationsRanked
    Ranked --> Selected : locationSelected
    Ranked --> Assessed : siteSuitabilityAssessed
    Selected --> [*]
```
