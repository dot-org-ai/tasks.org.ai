...
```mermaid
erDiagram
    WorkOrder ||--|| DiagnosticReport : "produces"
    WorkOrder ||--|{ ErrorCode : "references"
    WorkOrder ||--|{ SensorReading : "captures"
    DiagnosticReport ||--|| FailureMode : "identifies"
    FailureMode ||--|| RootCause : "traced-to"
    RootCause ||--|| RepairRecommendation : "addressed-by"
    WorkOrder ||--|| MaintenanceHistory : "recorded-in"
    ErrorCode }o--|| FailureMode : "indicates"
```
...
stateDiagram-v2
    [*] --> MalfunctionReported
    MalfunctionReported --> HistoryReview : malfunctionLogged
    HistoryReview --> ErrorCodeAnalysis : maintenanceHistoryReviewed
    ErrorCodeAnalysis --> VisualInspection : errorCodesInterpreted
    VisualInspection --> DiagnosticTesting : visualInspectionCompleted
    DiagnosticTesting --> DataAnalysis : diagnosticTestsRun
    DataAnalysis --> RootCauseIdentification : sensorDataAnalyzed
    RootCauseIdentification --> DiagnosticTesting : rootCauseNotFound
    RootCauseIdentification --> RepairPlanning : rootCauseIdentified
    RepairPlanning --> Verification : repairRecommended
    Verification --> DiagnosticTesting : notResolved
    Verification --> [*] : repairVerified
```
