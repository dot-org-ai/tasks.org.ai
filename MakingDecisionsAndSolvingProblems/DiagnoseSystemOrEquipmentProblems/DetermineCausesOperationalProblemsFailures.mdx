...
```mermaid
erDiagram
    IncidentReport ||--|| TimelineReconstruction : "reconstructed-in"
    IncidentReport ||--|| RootCauseAnalysis : "investigated-by"
    RootCauseAnalysis ||--|{ ContributingFactor : "identifies"
    ContributingFactor }o--|| RootCause : "leads-to"
    RootCause ||--|{ CorrectiveAction : "addressed-by"
    RootCause ||--|{ PreventiveAction : "prevented-by"
    CorrectiveAction ||--|| EffectivenessVerification : "validated-by"
    IncidentReport ||--o{ FailureModeRecord : "categorized-as"
```
...
stateDiagram-v2
    ...
    IncidentDocumentation --> EvidenceCollection : incidentDocumented
    EvidenceCollection --> TimelineMapping : evidenceCollected
    TimelineMapping --> Analysis : timelineReconstructed
    Analysis --> RootCauseIdentification : analysisMethodApplied
    RootCauseIdentification --> EvidenceCollection : insufficientEvidence
    RootCauseIdentification --> CorrectiveActionDevelopment : rootCauseIdentified
    CorrectiveActionDevelopment --> PreventiveActionDevelopment : correctiveActionsDeveloped
    PreventiveActionDevelopment --> Implementation : preventiveActionsDeveloped
    Implementation --> Verification : actionsImplemented
    Verification --> Analysis : notResolved
    Verification --> [*] : effectivenessVerified
```
