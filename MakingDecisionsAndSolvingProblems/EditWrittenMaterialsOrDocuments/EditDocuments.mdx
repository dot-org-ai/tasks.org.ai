...
```mermaid
erDiagram
    Document ||--|{ Revision : "versioned-as"
    Revision ||--|{ EditorialComment : "annotated-with"
    Document ||--|| StyleGuide : "governed-by"
    Document ||--|| ApprovalRecord : "approved-via"
    Document ||--|| VersionHistory : "tracked-in"
    EditorialComment }o--|| Revision : "applied-in"
```
...
stateDiagram-v2
    [*] --> Submitted
    Submitted --> InReview : documentSubmitted
    InReview --> Commented : editorialCommentsAdded
    Commented --> Revised : documentRevised
    Revised --> Proofreading : documentRevised
    Proofreading --> Revised : errorsFound
    Proofreading --> AwaitingApproval : documentProofread
    AwaitingApproval --> InReview : revisionRequested
    AwaitingApproval --> Approved : documentApproved
    Approved --> Published : documentPublished
    Published --> [*]
```
...
graph TD
    ...
    E --> F{Errors Found?}
    F -->|Yes| D
    F -->|No| G{Approval Required?}
    G -->|Yes| H[approveDocument]
    G -->|No| I[publishDocument]
    H --> J{Approved?}
    J -->|No| B
    J -->|Yes| I
```
