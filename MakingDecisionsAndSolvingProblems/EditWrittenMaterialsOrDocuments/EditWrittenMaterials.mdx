...
```mermaid
erDiagram
    Manuscript ||--|{ Revision : "versioned-as"
    Revision ||--|{ EditorialComment : "annotated-with"
    Manuscript ||--|| StyleGuide : "governed-by"
    Manuscript ||--|| PublicationProof : "formatted-as"
    EditorialComment }o--|| Revision : "addressed-in"
```
...
stateDiagram-v2
    [*] --> Submitted
    Submitted --> InReview : manuscriptSubmitted
    InReview --> CopyEditing : draftReviewed
    InReview --> Rejected : manuscriptRejected
    CopyEditing --> RevisionRequested : revisionRequested
    RevisionRequested --> CopyEditing : copyEditCompleted
    CopyEditing --> Proofreading : copyEditCompleted
    Proofreading --> CopyEditing : errorsFound
    Proofreading --> Approved : proofreadCompleted
    Approved --> Published : publicationApproved
    Published --> [*]
```
...
graph TD
    ...
    D --> F{Revisions Needed?}
    F -->|Yes| G[requestRevision]
    G --> H[Author Revises]
    H --> D
    F -->|No| I[proofread]
    I --> J{Errors Found?}
    J -->|Yes| D
    J -->|No| K[approveForPublication]
```
