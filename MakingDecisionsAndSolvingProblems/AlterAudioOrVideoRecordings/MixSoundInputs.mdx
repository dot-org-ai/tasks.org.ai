...
```mermaid
erDiagram
    MixSession ||--o{ AudioTrack : contains
    MixSession ||--o{ BusGroup : organizes
    AudioTrack ||--o{ EffectsChain : "processed by"
    BusGroup ||--o{ AudioTrack : groups
    MixSession ||--o{ Mixdown : renders
    MixSession ||--o| CueSheet : references
```
...
```mermaid
stateDiagram-v2
    [*] --> Created : mixSessionCreated
    Created --> TracksLoaded : tracksImported
    TracksLoaded --> LevelsBalanced : levelsSet
    LevelsBalanced --> EffectsProcessed : effectsApplied
    EffectsProcessed --> Rendered : mixdownRendered
    Rendered --> RevisionRequested : mixRevisionRequested
    RevisionRequested --> LevelsBalanced : levelsSet
    Rendered --> Approved : mixApproved
    Approved --> [*]
```
