...
```mermaid
erDiagram
    PatientHistory ||--|| ClinicalExam : "informs"
    ClinicalExam ||--|{ Radiograph : "supported-by"
    ClinicalExam ||--|| PeriodontalChart : "includes"
    ClinicalExam ||--|| Diagnosis : "produces"
    Diagnosis ||--|| TreatmentPlan : "drives"
    Radiograph }o--|| Diagnosis : "confirms"
```
...
stateDiagram-v2
    [*] --> HistoryReview
    HistoryReview --> Examination : historyReviewed
    Examination --> ImagingAnalysis : examinationCompleted
    ImagingAnalysis --> PeriodontalAssessment : radiographsInterpreted
    PeriodontalAssessment --> DiagnosticDecision : periodontalCharted
    DiagnosticDecision --> AdditionalTesting : diagnosticTestOrdered
    AdditionalTesting --> DiagnosticDecision : testResultsReceived
    DiagnosticDecision --> Diagnosed : diagnosisRendered
    Diagnosed --> TreatmentPlanning : diagnosisRendered
    TreatmentPlanning --> [*] : treatmentPlanDeveloped
```
