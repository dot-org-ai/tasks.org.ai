...
```mermaid
erDiagram
    ClinicalInterview ||--|| PsychometricAssessment : "followed-by"
    ClinicalInterview ||--o{ NeuroimagingStudy : "may-require"
    PsychometricAssessment }o--|| Diagnosis : "supports"
    NeuroimagingStudy }o--|| Diagnosis : "confirms"
    ElectrophysiologicalRecord }o--|| Diagnosis : "informs"
    Diagnosis ||--|| TreatmentRecommendation : "drives"
```
...
stateDiagram-v2
    [*] --> ClinicalInterview
    ClinicalInterview --> CollateralGathering : interviewCompleted
    CollateralGathering --> PsychometricTesting : collateralHistoryGathered
    PsychometricTesting --> NeuroimagingDecision : psychometricsAdministered
    NeuroimagingDecision --> ImagingOrdered : neuroimagingOrdered
    ImagingOrdered --> ResultsInterpretation : resultsInterpreted
    NeuroimagingDecision --> ResultsInterpretation : imagingNotNeeded
    ResultsInterpretation --> DiagnosisRendered : diagnosisRendered
    ResultsInterpretation --> PsychometricTesting : additionalTestingNeeded
    DiagnosisRendered --> TreatmentPlanning : treatmentRecommended
    TreatmentPlanning --> [*]
```
...
graph TD
    ...
    D -->|No| F
    F --> G{Findings Sufficient?}
    G -->|No| H{Additional Testing Needed?}
    H -->|Yes| C
    H -->|No| E
    G -->|Yes| I[renderDiagnosis]
    I --> J[recommendTreatment]
```
