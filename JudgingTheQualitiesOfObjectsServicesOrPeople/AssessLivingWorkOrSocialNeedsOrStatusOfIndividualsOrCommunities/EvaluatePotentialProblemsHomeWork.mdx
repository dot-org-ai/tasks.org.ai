...
```mermaid
erDiagram
    EnvironmentalAssessment ||--|{ Observation : contains
    Observation ||--o{ Hazard : identifies
    Hazard ||--o{ Recommendation : "addressed by"
    Hazard }o--|| RemediationPlan : "escalated to"
    RemediationPlan ||--o| FollowUp : "verified by"
    Recommendation }o--|| FollowUp : "checked in"
```
...
stateDiagram-v2
    ...
    Scheduled --> Inspected : walkthroughConducted
    Inspected --> HazardsFound : hazardsIdentified
    HazardsFound --> SeverityRated : severityAssessed
    SeverityRated --> Remediation : remediationPlanCreated
    SeverityRated --> Recommendations : recommendationsGenerated
    Remediation --> FollowUpSet : followUpScheduled
    Recommendations --> FollowUpSet : followUpScheduled
    FollowUpSet --> Inspected : reInspection
    FollowUpSet --> [*]
```
