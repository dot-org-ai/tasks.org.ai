...
```mermaid
erDiagram
    NeedsAssessment ||--|{ ServiceGap : identifies
    NeedsAssessment ||--|| DemographicProfile : "based on"
    NeedsAssessment ||--o| Survey : "informed by"
    ServiceGap ||--o{ ServicePlan : "addressed by"
    ServicePlan }o--|| ResourceInventory : "draws from"
    DemographicProfile }o--|| Survey : "derived from"
```
...
```mermaid
stateDiagram-v2
    [*] --> IntakeComplete : intakeCompleted
    IntakeComplete --> SurveyDone : surveyAdministered
    SurveyDone --> DemographicsReady : demographicsAnalyzed
    DemographicsReady --> GapsFound : serviceGapsIdentified
    GapsFound --> NeedsRanked : needsPrioritized
    NeedsRanked --> PlanCreated : servicePlanDeveloped
    PlanCreated --> ResourcesMatched : resourcesRecommended
    ResourcesMatched --> [*]
    ResourcesMatched --> IntakeComplete : reassessment
```
