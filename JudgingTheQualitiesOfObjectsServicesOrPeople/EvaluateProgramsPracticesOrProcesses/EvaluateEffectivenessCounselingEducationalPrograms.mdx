...
```mermaid
erDiagram
    ProgramEvaluation ||--o{ StudentOutcome : measures
    ProgramEvaluation ||--o{ ParticipantFeedback : collects
    ProgramEvaluation ||--o{ AttendanceRecord : tracks
    StudentOutcome }o--|| GoalAttainment : "determines"
    GoalAttainment }o--|| EffectivenessMetric : "contributes to"
    ProgramEvaluation ||--|{ EffectivenessMetric : produces
```
...
```mermaid
stateDiagram-v2
    [*] --> Defined : programObjectivesDefined
    Defined --> DataCollected : participantDataCollected
    DataCollected --> OutcomesMeasured : studentOutcomesMeasured
    OutcomesMeasured --> FeedbackGathered : participantFeedbackGathered
    FeedbackGathered --> AttendanceReviewed : attendanceEngagementAnalyzed
    AttendanceReviewed --> GoalsAssessed : goalAttainmentAssessed
    GoalsAssessed --> Reported : effectivenessReportGenerated
    GoalsAssessed --> DataCollected : participantDataCollected
    Reported --> [*]
```
...
graph TD
    ...
    E --> F{Adequate Engagement?}
    F -->|No| G[Adjust Program Design]
    G --> B
    F -->|Yes| H[assessGoalAttainment]
    H --> I[generateEffectivenessReport]
```
