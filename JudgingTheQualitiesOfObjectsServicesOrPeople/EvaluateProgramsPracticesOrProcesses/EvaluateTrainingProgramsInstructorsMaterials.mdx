...
```mermaid
erDiagram
    TrainingProgram ||--o{ Course : contains
    Course ||--o{ Material : uses
    Course ||--o{ Assessment : evaluates
    Assessment }o--|| CompetencyRecord : produces
    TrainingProgram ||--o{ Feedback : collects
    Feedback }o--|| CompetencyRecord : informs
```
...
```mermaid
stateDiagram-v2
    [*] --> LearningAssessed : learningAssessed
    LearningAssessed --> InstructorReviewed : instructorEvaluated
    InstructorReviewed --> MaterialsChecked : materialsReviewed
    MaterialsChecked --> ApplicationTracked : applicationMeasured
    ApplicationTracked --> SatisfactionReviewed : satisfactionAnalyzed
    SatisfactionReviewed --> GapsFound : gapsIdentified
    GapsFound --> Improved : improvementsRecommended
    SatisfactionReviewed --> LearningAssessed : learningAssessed
    Improved --> LearningAssessed : learningAssessed
    Improved --> [*]
```
