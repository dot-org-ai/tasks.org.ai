...
```mermaid
erDiagram
    PerformanceReview ||--|{ PerformanceObjective : evaluates
    PerformanceReview ||--|{ FeedbackItem : incorporates
    PerformanceReview ||--|| PerformanceRating : "results in"
    PerformanceObjective ||--o| PerformanceMetric : "measured by"
    PerformanceRating }o--o{ DevelopmentGoal : generates
    FeedbackItem }o--|| PerformanceRating : informs
```
...
```mermaid
stateDiagram-v2
    [*] --> ObjectivesSet : performanceObjectivesSet
    ObjectivesSet --> FeedbackIn : performanceFeedbackGathered
    FeedbackIn --> MetricsAnalyzed : performanceMetricsAnalyzed
    MetricsAnalyzed --> ReviewDone : reviewMeetingConducted
    ReviewDone --> RatingAssigned : performanceRatingAssigned
    RatingAssigned --> GoalsDocumented : developmentGoalsDocumented
    GoalsDocumented --> Calibrated : ratingsCalibrated
    Calibrated --> [*]
    Calibrated --> ObjectivesSet : nextPeriod
```
