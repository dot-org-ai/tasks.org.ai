...
```mermaid
erDiagram
    TestingSession ||--|{ Assessment : administers
    Assessment ||--|{ TestResponse : collects
    TestResponse ||--|| Score : "scored as"
    TestingSession ||--o{ Accommodation : "modified by"
    Score }o--|| Report : "summarized in"
    Assessment }o--|| Report : "documented in"
```
...
stateDiagram-v2
    ...
    Scheduled --> MaterialsReady : materialsPrepared
    MaterialsReady --> Administered : assessmentConducted
    Administered --> ResponsesIn : responsesCollected
    ResponsesIn --> Scored : testScored
    Scored --> ReportReady : reportGenerated
    ReportReady --> Distributed : resultsDistributed
    Distributed --> [*]
    Distributed --> Scheduled : retestNeeded
```
