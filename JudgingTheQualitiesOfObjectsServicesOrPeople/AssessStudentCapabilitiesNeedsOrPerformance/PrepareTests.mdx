...
```mermaid
erDiagram
    TestBlueprint ||--|{ TestItem : specifies
    TestItem ||--|{ Distractor : "includes"
    TestBlueprint ||--|| Specification : "governed by"
    TestForm ||--|{ TestItem : contains
    TestForm ||--|| ScoringKey : "scored by"
    TestItem }o--|| ScoringKey : "keyed in"
```
...
```mermaid
stateDiagram-v2
    [*] --> BlueprintReady : blueprintCreated
    BlueprintReady --> ItemsDrafted : itemsWritten
    ItemsDrafted --> ItemsReviewed : itemsReviewed
    ItemsReviewed --> ItemsDrafted : revisionsNeeded
    ItemsReviewed --> Assembled : testAssembled
    Assembled --> KeyReady : scoringKeyDeveloped
    KeyReady --> Validated : testValidated
    Validated --> Finalized : testFinalized
    Finalized --> [*]
```
