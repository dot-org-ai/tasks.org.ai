...
```mermaid
erDiagram
    IntegumentaryAssessment ||--o{ SkinCondition : identifies
    IntegumentaryAssessment ||--o| ScalpAnalysis : includes
    SkinCondition ||--o{ DiagnosticTest : "confirmed by"
    IntegumentaryAssessment ||--|| TreatmentProtocol : "results in"
    ProgressPhoto }o--|| IntegumentaryAssessment : "documents"
    TreatmentProtocol }o--|| SkinCondition : addresses
```
...
```mermaid
stateDiagram-v2
    [*] --> ConsultDone : consultationConducted
    ConsultDone --> Examined : visualExaminationPerformed
    Examined --> ScalpAnalyzed : scalpConditionAnalyzed
    ScalpAnalyzed --> Documented : findingsDocumented
    Documented --> TestsOrdered : diagnosticTestsOrdered
    Documented --> PlanCreated : treatmentPlanDeveloped
    TestsOrdered --> PlanCreated : treatmentPlanDeveloped
    PlanCreated --> FollowUpSet : followUpScheduled
    FollowUpSet --> ConsultDone : reassessment
    FollowUpSet --> [*]
```
