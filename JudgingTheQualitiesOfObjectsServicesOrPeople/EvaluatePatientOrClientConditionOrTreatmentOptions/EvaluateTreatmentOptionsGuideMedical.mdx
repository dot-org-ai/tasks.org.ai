...
```mermaid
erDiagram
    PatientCondition ||--|{ TreatmentOption : "treated by"
    TreatmentOption ||--o| ClinicalEvidence : "supported by"
    TreatmentOption ||--|| RiskAssessment : "evaluated with"
    TreatmentOption ||--o{ ContraindicationFlag : "excluded by"
    Recommendation }o--|| TreatmentOption : selects
    RiskAssessment }o--|| Recommendation : informs
```
...
```mermaid
stateDiagram-v2
    [*] --> ConditionReviewed : patientConditionReviewed
    ConditionReviewed --> OptionsListed : treatmentOptionsIdentified
    OptionsListed --> EfficacyCompared : treatmentEfficacyCompared
    EfficacyCompared --> RisksAssessed : treatmentRisksAssessed
    RisksAssessed --> ContraindicationsChecked : contraindicationsChecked
    ContraindicationsChecked --> SpecialistConsulted : specialistConsultRequested
    ContraindicationsChecked --> Recommended : treatmentRecommended
    SpecialistConsulted --> Recommended : treatmentRecommended
    Recommended --> [*]
```
