...
```mermaid
erDiagram
    FinancialProfile ||--|| IncomeStatement : includes
    FinancialProfile ||--|| BalanceSheet : includes
    FinancialProfile ||--|| CreditReport : "evaluated with"
    CashFlowAnalysis }o--|| IncomeStatement : "derived from"
    CashFlowAnalysis }o--|| BalanceSheet : "derived from"
    RiskRating }o--|| CreditReport : "informed by"
    RiskRating }o--|| CashFlowAnalysis : "informed by"
```
...
stateDiagram-v2
    ...
    Initiated --> DataCollected : financialDataCollected
    DataCollected --> Verified : documentationVerified
    Verified --> DataCollected : verificationFailed
    Verified --> CreditReviewed : creditHistoryAnalyzed
    CreditReviewed --> RatiosCalculated : debtRatiosCalculated
    RatiosCalculated --> CapacityEvaluated : capacityEvaluated
    CapacityEvaluated --> RiskRated : riskRatingAssigned
    RiskRated --> [*]
```
