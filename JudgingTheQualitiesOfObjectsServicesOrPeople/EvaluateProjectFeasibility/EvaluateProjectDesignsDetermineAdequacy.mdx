...
```mermaid
erDiagram
    ProjectDesign ||--o{ Requirement : satisfies
    ProjectDesign ||--o{ Standard : "complies with"
    ProjectDesign ||--o{ Constraint : "bounded by"
    ReviewCriteria }o--|| ProjectDesign : "applied to"
    Finding }o--|| ProjectDesign : "raised against"
    Finding }o--|| Requirement : references
```
...
stateDiagram-v2
    ...
    RequirementsReviewed --> ComplianceChecked : complianceChecked
    ComplianceChecked --> FeasibilityEvaluated : feasibilityEvaluated
    FeasibilityEvaluated --> RisksDocumented : risksIdentified
    RisksDocumented --> QualityReviewed : qualityAssessed
    QualityReviewed --> CostsEstimated : costsCalculated
    CostsEstimated --> Approved : approvalRecommended
    CostsEstimated --> RequirementsReviewed : requirementsReviewed
    Approved --> [*]
```
