...
erDiagram
    ...
    Product ||--o{ Measurement : tested
    EvaluationCriteria ||--o{ Characteristic : defines
    Finding }o--|| Product : "raised for"
    Comparison ||--o{ Product : includes
    Comparison ||--o{ Measurement : compares
```
...
```mermaid
stateDiagram-v2
    [*] --> CriteriaDefined : criteriaDefinedEvent
    CriteriaDefined --> Inspected : productInspectedEvent
    Inspected --> Measured : attributeMeasuredEvent
    Measured --> Documented : findingRecordedEvent
    Documented --> Compared : productsComparedEvent
    Compared --> ComplianceChecked : complianceValidatedEvent
    ComplianceChecked --> Published : evaluationPublishedEvent
    ComplianceChecked --> Inspected : productInspectedEvent
    Published --> [*]
```
