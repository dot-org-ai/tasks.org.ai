...
```mermaid
erDiagram
    Product ||--o{ CostComponent : "incurs"
    Product ||--o{ Benefit : delivers
    Project ||--o{ CostComponent : "incurs"
    Alternative ||--o{ CostEffectivenessRatio : "scored by"
    CostComponent }o--|| CostEffectivenessRatio : "feeds into"
    Benefit }o--|| CostEffectivenessRatio : "feeds into"
```
...
```mermaid
stateDiagram-v2
    [*] --> CostCalculated : totalCostCalculated
    CostCalculated --> BenefitsQuantified : benefitsQuantified
    BenefitsQuantified --> Compared : alternativesCompared
    Compared --> RatiosComputed : ratiosCalculated
    RatiosComputed --> SensitivityTested : sensitivityPerformed
    SensitivityTested --> Ranked : optionsRanked
    Ranked --> Selected : selectionRecommended
    Ranked --> CostCalculated : totalCostCalculated
    Selected --> [*]
```
