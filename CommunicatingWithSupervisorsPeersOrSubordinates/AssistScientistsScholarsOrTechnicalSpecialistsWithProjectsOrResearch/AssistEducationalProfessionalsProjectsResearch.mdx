...
```mermaid
erDiagram
    EducationalProject ||--|{ ResearchStudy : includes
    ResearchStudy ||--|{ DataCollection : "gathered via"
    ResearchStudy ||--|| LiteratureReview : "informed by"
    DataCollection ||--|| ProjectReport : "summarized in"
    EducationalProject ||--|{ CurriculumArtifact : produces
    ResearchStudy ||--o| IRBApproval : "authorized by"
```
...
stateDiagram-v2
    [*] --> ProjectJoined
    ProjectJoined --> ResearchMaterialsGathered : researchMaterialsGathered
    ResearchMaterialsGathered --> DataCollected : dataCollected
    DataCollected --> EducationalDataAnalyzed : educationalDataAnalyzed
    EducationalDataAnalyzed --> FindingsDocumented : findingsDocumented
    FindingsDocumented --> CurriculumMaterialsDeveloped : curriculumMaterialsDeveloped
    CurriculumMaterialsDeveloped --> DraftsReviewed : draftsReviewed
    DraftsReviewed --> CurriculumMaterialsDeveloped : revisionsNeeded
    DraftsReviewed --> [*]
```
...
graph TD
    ...
    C --> D{Sufficient Data?}
    D -->|No| C
    D -->|Yes| E[analyzeEducationalData]
    E --> F[documentFindings]
    F --> G[developCurriculumMaterials]
    G --> H[reviewDrafts]
    H --> I{Revisions Needed?}
    I -->|Yes| G
    I -->|No| J[presentResults]
```
