...
```mermaid
erDiagram
    EquipmentProblem ||--o{ Equipment : relates-to
    Equipment ||--o{ Notification : relates-to
    Notification ||--o{ DiagnosticReport : relates-to
    DiagnosticReport ||--o{ ResolutionRecord : relates-to
    ResolutionRecord ||--o{ ImpactAssessment : relates-to
```
...
stateDiagram-v2
    [*] --> Problem Detected
    Problem Detected --> Notification Created : notificationCreated
    Notification Created --> Notification Routed : notificationRouted
    Notification Routed --> Impact Assessed : impactAssessed
    Impact Assessed --> Repair Initiated : repairInitiated
    Repair Initiated --> Resolution Confirmed : resolutionConfirmed
    Impact Assessed --> Notification Routed : revisionRequired
