...
```mermaid
erDiagram
    Incident ||--o{ EmergencyBroadcast : triggers
    EmergencyBroadcast ||--o{ CommunicationChannel : transmits-via
    Incident ||--o{ SituationReport : updates
    ContactRoster ||--o{ EmergencyBroadcast : routes
    AcknowledgmentRecord ||--o{ EmergencyBroadcast : confirms
```
...
stateDiagram-v2
    [*] --> IncidentReported
    IncidentReported --> DetailsVerified : detailsVerified
    DetailsVerified --> AlertBroadcasted : alertBroadcasted
    AlertBroadcasted --> SituationUpdated : situationUpdated
    SituationUpdated --> ReceiptConfirmed : receiptConfirmed
    ReceiptConfirmed --> CommunicationLogged : communicationLogged
    SituationUpdated --> AlertBroadcasted : alertBroadcasted
    CommunicationLogged --> [*]
```
