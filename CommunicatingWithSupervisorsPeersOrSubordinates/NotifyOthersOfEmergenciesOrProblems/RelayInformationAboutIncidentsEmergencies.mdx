...
```mermaid
erDiagram
    Incident ||--o{ EmergencyBroadcast : relates-to
    EmergencyBroadcast ||--o{ CommunicationChannel : relates-to
    CommunicationChannel ||--o{ SituationReport : relates-to
    SituationReport ||--o{ ContactRoster : relates-to
    ContactRoster ||--o{ AcknowledgmentRecord : relates-to
```
...
stateDiagram-v2
    [*] --> Incident Reported
    Incident Reported --> Details Verified : detailsVerified
    Details Verified --> Alert Broadcasted : alertBroadcasted
    Alert Broadcasted --> Situation Updated : situationUpdated
    Situation Updated --> Receipt Confirmed : receiptConfirmed
    Receipt Confirmed --> Communication Logged : communicationLogged
    Situation Updated --> Alert Broadcasted : revisionRequired
