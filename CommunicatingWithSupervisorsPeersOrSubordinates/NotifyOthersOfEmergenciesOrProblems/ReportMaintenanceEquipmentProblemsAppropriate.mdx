...
```mermaid
erDiagram
    MaintenanceReport ||--o{ Equipment : describes
    MaintenanceReport ||--o{ Attachment : includes
    MaintenanceReport ||--o{ PriorityLevel : assigns
    WorkOrder ||--o{ MaintenanceReport : fulfills
    RepairLog ||--o{ WorkOrder : records
```
...
stateDiagram-v2
    [*] --> ReportSubmitted
    ReportSubmitted --> PriorityAssessed : priorityAssessed
    PriorityAssessed --> WorkOrderAssigned : workOrderAssigned
    WorkOrderAssigned --> ReportAcknowledged : reportAcknowledged
    ReportAcknowledged --> IssueResolved : issueResolved
    ReportAcknowledged --> ReportEscalated : reportEscalated
    ReportEscalated --> WorkOrderAssigned : workOrderAssigned
    IssueResolved --> [*]
```
