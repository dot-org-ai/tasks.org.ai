...
```mermaid
erDiagram
    HazardAlert ||--o{ Hazard : relates-to
    Hazard ||--o{ NotificationChannel : relates-to
    NotificationChannel ||--o{ AcknowledgmentLog : relates-to
    AcknowledgmentLog ||--o{ ProtectiveAction : relates-to
    ProtectiveAction ||--o{ IncidentRecord : relates-to
```
...
stateDiagram-v2
    [*] --> Hazard Identified
    Hazard Identified --> Alert Classified : alertClassified
    Alert Classified --> Notification Broadcasted : notificationBroadcasted
    Notification Broadcasted --> Protective Action Issued : protectiveActionIssued
    Protective Action Issued --> Acknowledgment Received : acknowledgmentReceived
    Acknowledgment Received --> Incident Closed : incidentClosed
    Protective Action Issued --> Notification Broadcasted : revisionRequired
    Incident Closed --> [*]
```
