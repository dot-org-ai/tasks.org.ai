...
```mermaid
erDiagram
    Violation ||--o{ Warning : triggers
    Violation ||--o{ Rule : violates
    Warning ||--o{ CorrectiveAction : prescribes
    IncidentReport ||--o{ Violation : documents
    EscalationRecord ||--o{ Violation : escalates
```
...
stateDiagram-v2
    [*] --> ViolationIdentified
    ViolationIdentified --> VerbalWarningIssued : verbalWarningIssued
    VerbalWarningIssued --> CorrectiveActionAssigned : correctiveActionAssigned
    CorrectiveActionAssigned --> ViolationClosed : violationClosed
    CorrectiveActionAssigned --> WrittenWarningIssued : writtenWarningIssued
    WrittenWarningIssued --> ViolationEscalated : violationEscalated
    ViolationEscalated --> CorrectiveActionAssigned : correctiveActionAssigned
    ViolationClosed --> [*]
```
