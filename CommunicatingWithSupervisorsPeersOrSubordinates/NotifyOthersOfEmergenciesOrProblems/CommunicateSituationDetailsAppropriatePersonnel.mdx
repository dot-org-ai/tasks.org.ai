...
```mermaid
erDiagram
    SituationReport ||--o{ Situation : describes
    SituationReport ||--o{ Recipient : targets
    SituationReport ||--o{ CommunicationChannel : delivers-via
    SituationReport ||--o{ ActionItem : generates
    Acknowledgment ||--o{ SituationReport : confirms
```
...
stateDiagram-v2
    [*] --> SituationAssessed
    SituationAssessed --> ReportComposed : reportComposed
    ReportComposed --> RecipientsIdentified : recipientsIdentified
    RecipientsIdentified --> ReportDelivered : reportDelivered
    ReportDelivered --> UnderstandingConfirmed : understandingConfirmed
    UnderstandingConfirmed --> FollowUpAssigned : followUpAssigned
    ReportDelivered --> ReportComposed : reportComposed
    FollowUpAssigned --> SituationAssessed : situationAssessed
    FollowUpAssigned --> [*]
```
