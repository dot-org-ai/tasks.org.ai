...
```mermaid
erDiagram
    ResearchReport ||--|{ DataSet : "based on"
    ResearchReport ||--|| ExecutiveSummary : "summarized as"
    ResearchReport ||--o| TechnicalAppendix : "supplemented by"
    JournalManuscript }o--|| ResearchReport : "derived from"
    ConferencePresentation }o--|| ResearchReport : "presents"
    PolicyBrief }o--|| ResearchReport : "distills"
```
...
```mermaid
stateDiagram-v2
    [*] --> ResearchReportPrepared
    ResearchReportPrepared --> StakeholdersBriefed : stakeholdersBriefed
    StakeholdersBriefed --> FindingsPresented : findingsPresented
    FindingsPresented --> JournalManuscriptSubmitted : journalManuscriptSubmitted
    JournalManuscriptSubmitted --> JournalManuscriptSubmitted : peerReviewRevisions
    JournalManuscriptSubmitted --> RegulatoryReportFiled : regulatoryReportFiled
    JournalManuscriptSubmitted --> DataSetPublished : dataSetPublished
    RegulatoryReportFiled --> DataSetPublished : dataSetPublished
    DataSetPublished --> PolicyBriefPrepared : policyBriefPrepared
    PolicyBriefPrepared --> [*]
```
...
graph TD
    ...
    E --> F{Accepted?}
    F -->|No| D
    F -->|Yes| G{Regulatory Filing Required?}
    G -->|Yes| H[fileRegulatoryReport]
    G -->|No| I[publishDataSet]
    H --> I
    I --> J[preparePolicyBrief]
```
