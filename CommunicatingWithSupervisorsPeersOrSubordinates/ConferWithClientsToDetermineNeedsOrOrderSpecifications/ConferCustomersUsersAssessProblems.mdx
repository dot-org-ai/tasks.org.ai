...
```mermaid
erDiagram
    ProblemReport ||--o{ Symptom : describes
    Symptom ||--o{ RootCause : traces-to
    RootCause ||--o{ DiagnosticSession : investigated-in
    DiagnosticSession ||--|{ AssessmentFinding : produces
    AssessmentFinding ||--o{ RecommendedSolution : recommends
```
...
```mermaid
stateDiagram-v2
    [*] --> ProblemAssessmentInitiated
    ProblemAssessmentInitiated --> SymptomsGathered : symptomsGathered
    SymptomsGathered --> IssueReproduced : issueReproduced
    IssueReproduced --> RootCauseAnalyzed : rootCauseAnalyzed
    RootCauseAnalyzed --> FindingsValidated : findingsValidated
    FindingsValidated --> SolutionRecommended : solutionRecommended
    SolutionRecommended --> AssessmentDocumented : assessmentDocumented
    RootCauseAnalyzed --> IssueReproduced : revisionRequired
