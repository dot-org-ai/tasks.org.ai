...
```mermaid
erDiagram
    CrossFunctionalInitiative ||--o{ ActionItem : contains
    CrossFunctionalInitiative ||--o{ MeetingCadence : follows
    CrossFunctionalInitiative ||--|{ DependencyMap : "documented in"
    CommunicationChannel }o--|| CrossFunctionalInitiative : supports
    StakeholderRegistry ||--o{ CommunicationChannel : lists
    ActionItem }o--|| DependencyMap : "resolves"
```
...
```mermaid
stateDiagram-v2
    [*] --> Identified : gapIdentified
    Identified --> Mapped : dependenciesMapped
    Mapped --> Connected : channelEstablished
    Connected --> Scheduled : syncMeetingScheduled
    Scheduled --> Tracking : actionItemCompleted
    Tracking --> Escalated : blockerEscalated
    Escalated --> Tracking : actionItemCompleted
    Tracking --> Reported : progressReported
    Reported --> Tracking : actionItemCompleted
    Reported --> [*]
```
