...
```mermaid
erDiagram
    WorkOrder ||--|| Schedule : "timed by"
    WorkOrder ||--|{ PartsRequest : "requires"
    WorkOrder ||--|| SiteReport : "described in"
    WorkOrder ||--|| CompletionRecord : "closed with"
    WorkOrder ||--|{ CommunicationLog : "tracked in"
    PartsRequest }o--|| Schedule : "affects"
```
...
stateDiagram-v2
    [*] --> CrewAssigned
    CrewAssigned --> JobDetailsShared : jobDetailsShared
    JobDetailsShared --> ScheduleCoordinated : scheduleCoordinated
    ScheduleCoordinated --> PartsRequested : partsRequested
    PartsRequested --> ProgressUpdated : progressUpdated
    ProgressUpdated --> IssueEscalated : issueEscalated
    ProgressUpdated --> CompletionConfirmed : completionConfirmed
    IssueEscalated --> ProgressUpdated : issueResolved
    CompletionConfirmed --> [*]
```
...
graph TD
    ...
    C --> D{Parts Needed?}
    D -->|Yes| E[requestParts]
    D -->|No| F[updateProgress]
    E --> F
    F --> G{Issue?}
