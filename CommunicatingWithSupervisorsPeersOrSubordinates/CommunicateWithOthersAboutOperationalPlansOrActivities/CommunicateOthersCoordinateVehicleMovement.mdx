...
```mermaid
erDiagram
    RouteAssignment ||--|{ DispatchOrder : "executed via"
    DispatchOrder ||--|{ VehiclePosition : "tracked by"
    RouteAssignment ||--o{ TrafficAdvisory : "affected by"
    DispatchOrder ||--|| ArrivalConfirmation : "completed with"
    RouteAssignment ||--o{ FuelStop : "includes"
    TrafficAdvisory }o--|| RouteAssignment : "may reroute"
```
...
stateDiagram-v2
    [*] --> RouteAssigned
    RouteAssigned --> VehicleDispatched : vehicleDispatched
    VehicleDispatched --> PositionUpdated : positionUpdated
    PositionUpdated --> TrafficAdvisoryIssued : trafficAdvisoryIssued
    PositionUpdated --> ArrivalConfirmed : arrivalConfirmed
    TrafficAdvisoryIssued --> VehicleRerouted : vehicleRerouted
    VehicleRerouted --> PositionUpdated : positionUpdated
    ArrivalConfirmed --> VehicleDispatched : nextStop
    ArrivalConfirmed --> [*]
```
...
graph TD
    ...
    G -->|Yes| H[logFuelStop]
    H --> C
    G -->|No| I{At Destination?}
    I -->|No| C
    I -->|Yes| J[confirmArrival]
    J --> K{More Stops?}
    K -->|Yes| B
    K -->|No| L[Complete]
```
