...
```mermaid
erDiagram
    WorkAssignment ||--|{ WorkInstruction : "contains"
    WorkInstruction ||--o{ SpecificationDocument : "references"
    WorkInstruction ||--o{ ClarificationRequest : "generates"
    ClarificationRequest ||--|{ Acknowledgment : "resolves into"
    WorkInstruction ||--|{ Acknowledgment : "confirmed by"
    StandardOperatingProcedure ||--o{ WorkInstruction : "guides"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Received : instructionReceived
    Received --> UnderReview : specificationReviewed
    UnderReview --> NeedsClarification : clarificationRequested
    NeedsClarification --> UnderReview : clarificationProvided
    UnderReview --> Acknowledged : receiptAcknowledged
    Acknowledged --> ReadinessCheck : readinessAssessed
    ReadinessCheck --> Acknowledged : readinessAssessed
    ReadinessCheck --> InProgress : executionBegun
    InProgress --> [*]
```
