...
```mermaid
erDiagram
    StatusUpdate ||--o{ ConditionChange : relates-to
    ConditionChange ||--o{ CareNotification : relates-to
    CareNotification ||--o{ Handoff : relates-to
    Handoff ||--o{ ProgressNote : relates-to
    ProgressNote ||--o{ Alert : relates-to
```
...
stateDiagram-v2
    [*] --> Status Reported
    Status Reported --> Change Notified : changeNotified
    Change Notified --> Urgent Escalated : urgentEscalated
    Urgent Escalated --> Handoff Conducted : handoffConducted
    Handoff Conducted --> Progress Documented : progressDocumented
    Progress Documented --> Receipt Confirmed : receiptConfirmed
    Receipt Confirmed --> Care Plan Updated : carePlanUpdated
    Handoff Conducted --> Urgent Escalated : revisionRequired
    Care Plan Updated --> [*]
```
