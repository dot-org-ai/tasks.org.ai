...
```mermaid
erDiagram
    TestResult ||--o{ CriticalValue : relates-to
    CriticalValue ||--o{ Report : relates-to
    Report ||--o{ Interpretation : relates-to
    Interpretation ||--o{ Acknowledgment : relates-to
    Acknowledgment ||--o{ Notification : relates-to
```
...
stateDiagram-v2
    [*] --> Results Validated
    Results Validated --> Results Sent : resultsSent
    Results Sent --> Critical Value Flagged : criticalValueFlagged
    Critical Value Flagged --> Acknowledgment Requested : acknowledgmentRequested
    Acknowledgment Requested --> Interpretation Attached : interpretationAttached
    Interpretation Attached --> Provider Notified : providerNotified
    Provider Notified --> Delivery Tracked : deliveryTracked
    Acknowledgment Requested --> Critical Value Flagged : revisionRequired
    Delivery Tracked --> [*]
```
