...
erDiagram
    ...
    CriticalValue ||--o{ Report : "included in"
    Report ||--o{ Interpretation : accompanies
    Interpretation ||--o{ Acknowledgment : requires
    Acknowledgment ||--o{ Notification : confirms
```
...
stateDiagram-v2
    [*] --> ResultsValidated
    ResultsValidated --> ResultsSent : resultsSent
    ResultsSent --> CriticalValueFlagged : criticalValueFlagged
    CriticalValueFlagged --> AcknowledgmentRequested : acknowledgmentRequested
    AcknowledgmentRequested --> InterpretationAttached : interpretationAttached
    InterpretationAttached --> ProviderNotified : providerNotified
    ProviderNotified --> DeliveryTracked : deliveryTracked
    AcknowledgmentRequested --> CriticalValueFlagged : revisionRequired
    DeliveryTracked --> [*]
```
