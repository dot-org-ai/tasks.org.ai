...
```mermaid
erDiagram
    ClientAssessment ||--o{ ProfessionalEvaluation : includes
    ProfessionalEvaluation ||--o{ NeedsPrioritization : ranks
    NeedsPrioritization ||--o{ IntegratedCarePlan : guides
    IntegratedCarePlan ||--o{ ServiceReferral : generates
    ServiceReferral ||--o{ OutcomeMeasure : tracks
```
...
stateDiagram-v2
    [*] --> AssessmentInitiated
    AssessmentInitiated --> DisciplineEvaluationConducted : disciplineEvaluationConducted
    DisciplineEvaluationConducted --> FindingsSynthesized : findingsSynthesized
    FindingsSynthesized --> ClientNeedsPrioritized : clientNeedsPrioritized
    ClientNeedsPrioritized --> IntegratedPlanDeveloped : integratedPlanDeveloped
    IntegratedPlanDeveloped --> ServiceProvidersAssigned : serviceProvidersAssigned
    ServiceProvidersAssigned --> ClientProgressTracked : clientProgressTracked
    ClientNeedsPrioritized --> FindingsSynthesized : revisionRequired
    ClientProgressTracked --> [*]
```
