...
```mermaid
erDiagram
    PatientStatus ||--o{ ClinicalHandoff : relates-to
    ClinicalHandoff ||--o{ StatusUpdate : relates-to
    StatusUpdate ||--o{ CareTransition : relates-to
    CareTransition ||--o{ CriticalAlert : relates-to
    CriticalAlert ||--o{ ProviderNotification : relates-to
```
...
stateDiagram-v2
    [*] --> Handoff Initiated
    Handoff Initiated --> Status Change Communicated : statusChangeCommunicated
    Status Change Communicated --> Clinical Summary Transmitted : clinicalSummaryTransmitted
    Clinical Summary Transmitted --> Critical Finding Alerted : criticalFindingAlerted
    Critical Finding Alerted --> Receipt Acknowledgment Confirmed : receiptAcknowledgmentConfirmed
    Receipt Acknowledgment Confirmed --> Careteam Updated : careteamUpdated
    Careteam Updated --> Handoff Completion Documented : handoffCompletionDocumented
    Critical Finding Alerted --> Clinical Summary Transmitted : revisionRequired
    Handoff Completion Documented --> [*]
```
