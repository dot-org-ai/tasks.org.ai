...
```mermaid
erDiagram
    PatientStatus ||--o{ ClinicalHandoff : triggers
    ClinicalHandoff ||--o{ StatusUpdate : communicates
    StatusUpdate ||--o{ CareTransition : initiates
    CareTransition ||--o{ CriticalAlert : "may generate"
    CriticalAlert ||--o{ ProviderNotification : dispatches
```
...
stateDiagram-v2
    [*] --> HandoffInitiated
    HandoffInitiated --> StatusChangeCommunicated : statusChangeCommunicated
    StatusChangeCommunicated --> ClinicalSummaryTransmitted : clinicalSummaryTransmitted
    ClinicalSummaryTransmitted --> CriticalFindingAlerted : criticalFindingAlerted
    CriticalFindingAlerted --> ReceiptAcknowledgmentConfirmed : receiptAcknowledgmentConfirmed
    ReceiptAcknowledgmentConfirmed --> CareteamUpdated : careteamUpdated
    CareteamUpdated --> HandoffCompletionDocumented : handoffCompletionDocumented
    CriticalFindingAlerted --> ClinicalSummaryTransmitted : revisionRequired
    HandoffCompletionDocumented --> [*]
```
