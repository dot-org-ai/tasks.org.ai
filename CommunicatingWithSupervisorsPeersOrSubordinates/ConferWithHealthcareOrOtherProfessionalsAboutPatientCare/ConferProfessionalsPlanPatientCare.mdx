...
```mermaid
erDiagram
    CarePlan ||--o{ Conference : "discussed at"
    Conference ||--o{ Recommendation : produces
    Recommendation ||--o{ Decision : informs
    Decision ||--o{ Goal : establishes
    Goal ||--o{ Assessment : measured-by
```
...
stateDiagram-v2
    [*] --> ConferenceScheduled
    ConferenceScheduled --> ParticipantsInvited : participantsInvited
    ParticipantsInvited --> AssessmentShared : assessmentShared
    AssessmentShared --> InterventionProposed : interventionProposed
    InterventionProposed --> OptionsDiscussed : optionsDiscussed
    OptionsDiscussed --> CarePlanFinalized : carePlanFinalized
    CarePlanFinalized --> ResponsibilitiesAssigned : responsibilitiesAssigned
    InterventionProposed --> AssessmentShared : revisionRequired
