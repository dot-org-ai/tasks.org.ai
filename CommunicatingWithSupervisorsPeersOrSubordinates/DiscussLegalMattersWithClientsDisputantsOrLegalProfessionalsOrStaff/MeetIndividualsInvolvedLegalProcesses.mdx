...
```mermaid
erDiagram
    CaseFile ||--o{ MeetingBrief : prepares
    CaseFile ||--o{ IssueList : identifies
    MeetingBrief ||--o{ MeetingRecord : produces
    MeetingRecord ||--o{ FollowUpAction : assigns
    ConfidentialityAgreement ||--o{ CaseFile : governs
```
...
stateDiagram-v2
    [*] --> BriefPrepared
    BriefPrepared --> MeetingScheduled : meetingScheduled
    MeetingScheduled --> InformationPresented : informationPresented
    InformationPresented --> IssuesClarified : issuesClarified
    IssuesClarified --> OutcomesDocumented : outcomesDocumented
    OutcomesDocumented --> FollowUpsAssigned : followUpsAssigned
    FollowUpsAssigned --> CaseFileUpdated : caseFileUpdated
    IssuesClarified --> InformationPresented : informationPresented
    CaseFileUpdated --> BriefPrepared : briefPrepared
    CaseFileUpdated --> [*]
```
