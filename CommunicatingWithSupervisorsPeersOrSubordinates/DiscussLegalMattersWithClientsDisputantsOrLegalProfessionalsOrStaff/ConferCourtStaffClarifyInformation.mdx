...
```mermaid
erDiagram
    CourtInquiry ||--o{ ClarificationRecord : resolves
    CourtInquiry ||--o{ FilingRequirement : clarifies
    CourtRule ||--o{ FilingRequirement : governs
    HearingSchedule ||--o{ CaseCalendar : populates
    ClarificationRecord ||--o{ CaseCalendar : updates
```
...
stateDiagram-v2
    [*] --> QuestionIdentified
    QuestionIdentified --> InquirySubmitted : inquirySubmitted
    InquirySubmitted --> ClarificationReceived : clarificationReceived
    ClarificationReceived --> ClarificationVerified : clarificationVerified
    ClarificationVerified --> CalendarUpdated : caseCalendarUpdated
    CalendarUpdated --> TeamNotified : teamNotified
    ClarificationVerified --> InquirySubmitted : inquirySubmitted
    TeamNotified --> QuestionIdentified : questionIdentified
    TeamNotified --> [*]
```
