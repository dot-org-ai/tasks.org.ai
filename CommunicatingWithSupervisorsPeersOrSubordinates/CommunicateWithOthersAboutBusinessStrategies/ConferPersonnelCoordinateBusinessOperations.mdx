...
```mermaid
erDiagram
    OperationalPlan ||--|{ StatusUpdate : "reported via"
    OperationalPlan ||--|{ DependencyMap : "visualized in"
    DependencyMap ||--o{ EscalationTicket : "may trigger"
    ActionPlan }o--|| EscalationTicket : "resolves"
    ResourceAllocation }o--|| OperationalPlan : "supports"
    ActionPlan ||--|{ ResourceAllocation : "requires"
```
...
```mermaid
stateDiagram-v2
    [*] --> CoordinationScheduled
    CoordinationScheduled --> StatusUpdateShared : statusUpdateShared
    StatusUpdateShared --> DependencyIdentified : dependencyIdentified
    DependencyIdentified --> BottleneckResolved : bottleneckResolved
    DependencyIdentified --> ActionPlanCreated : actionPlanCreated
    BottleneckResolved --> ActionPlanCreated : actionPlanCreated
    ActionPlanCreated --> ResourcesAllocated : resourcesAllocated
    ResourcesAllocated --> IssueEscalated : issueEscalated
    ResourcesAllocated --> [*]
    IssueEscalated --> BottleneckResolved : reResolved
```
