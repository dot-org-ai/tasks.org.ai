...
```mermaid
erDiagram
    Campaign ||--|| MarketingBrief : "defined by"
    Campaign ||--|{ CreativeAsset : contains
    Campaign ||--|| ChannelPlan : "distributed via"
    Campaign ||--|{ PerformanceReport : "evaluated by"
    CreativeAsset }o--|| MarketingBrief : "aligned with"
    PerformanceReport }o--|| ChannelPlan : "measured per"
```
...
stateDiagram-v2
    [*] --> BriefCreated
    BriefCreated --> StakeholdersAligned : stakeholdersAligned
    StakeholdersAligned --> CreativeReviewed : creativeReviewed
    CreativeReviewed --> CampaignLaunched : campaignLaunched
    CampaignLaunched --> PerformanceTracked : performanceTracked
    PerformanceTracked --> CampaignOptimized : campaignOptimized
    PerformanceTracked --> CampaignClosedOut : campaignClosedOut
    CampaignOptimized --> PerformanceTracked : recheck
    CampaignClosedOut --> [*]
```
