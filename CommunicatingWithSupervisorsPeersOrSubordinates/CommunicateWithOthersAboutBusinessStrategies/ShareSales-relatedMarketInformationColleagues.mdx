...
```mermaid
erDiagram
    MarketIntelligenceReport ||--|{ CustomerInsight : "aggregates"
    MarketIntelligenceReport ||--|| PipelineSnapshot : includes
    CompetitiveAlert }o--|| CustomerInsight : "triggered by"
    WinLossAnalysis }o--|| PipelineSnapshot : "examines"
    CustomerInsight }o--|| WinLossAnalysis : "informs"
    MarketIntelligenceReport ||--o{ CompetitiveAlert : "may contain"
```
...
stateDiagram-v2
    [*] --> InsightSubmitted
    InsightSubmitted --> ReportCompiled : reportCompiled
    ReportCompiled --> CompetitiveAlertIssued : competitiveAlertIssued
    ReportCompiled --> ReportDistributed : reportDistributed
    CompetitiveAlertIssued --> ReportDistributed : reportDistributed
    ReportDistributed --> PipelineReviewed : pipelineReviewed
    PipelineReviewed --> WinLossReviewCompleted : winLossReviewCompleted
    PipelineReviewed --> DashboardUpdated : dashboardUpdated
    WinLossReviewCompleted --> DashboardUpdated : dashboardUpdated
    DashboardUpdated --> InsightSubmitted : insightSubmitted
    DashboardUpdated --> [*]
```
...
graph TD
    ...
    B --> C{Competitor Move?}
    C -->|Yes| D[issueCompetitiveAlert]
    C -->|No| E[distributeReport]
    D --> E
    E --> F[reviewPipeline]
    F --> G{Notable Deal?}
    G -->|Yes| H[conductWinLossReview]
    G -->|No| I[updateDashboard]
    H --> I
    I --> J{More Insights?}
    J -->|Yes| A
    J -->|No| K[Complete]
