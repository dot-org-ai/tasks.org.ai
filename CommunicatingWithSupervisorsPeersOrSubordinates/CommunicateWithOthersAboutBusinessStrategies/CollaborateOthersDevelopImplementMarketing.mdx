...
```mermaid
erDiagram
    MarketingStrategy ||--|| MarketAnalysis : "informed by"
    MarketingStrategy ||--|| GoToMarketPlan : "executed via"
    MarketingStrategy ||--|| BudgetAllocation : "funded by"
    GoToMarketPlan ||--|| PerformanceDashboard : "measured on"
    MarketingStrategy ||--o{ PartnerPlan : "extended through"
    PerformanceDashboard }o--|| BudgetAllocation : "tracks spend from"
```
...
stateDiagram-v2
    [*] --> ResearchCompleted
    ResearchCompleted --> StrategyDrafted : strategyDrafted
    StrategyDrafted --> StrategyReviewed : strategyReviewed
    StrategyReviewed --> StrategyDrafted : revisionsRequested
    StrategyReviewed --> StrategyApproved : strategyApproved
    StrategyApproved --> GoToMarketPlanBuilt : goToMarketPlanBuilt
    GoToMarketPlanBuilt --> StrategyExecuted : strategyExecuted
    StrategyExecuted --> ResultsMeasured : resultsMeasured
    ResultsMeasured --> StrategyExecuted : adjustmentsNeeded
    ResultsMeasured --> [*]
```
