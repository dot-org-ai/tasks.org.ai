...
```mermaid
erDiagram
    Program ||--|| Curriculum : "structured as"
    Program ||--|| NeedsAssessment : "justified by"
    Program ||--|{ Partnership : "delivered through"
    Program ||--o| FundingProposal : "funded via"
    Program ||--|{ EvaluationReport : "assessed by"
    Partnership }o--|| FundingProposal : "supports"
```
...
stateDiagram-v2
    ...
    NeedsAssessmentCompleted --> PartnershipFormed : partnershipFormed
    PartnershipFormed --> CurriculumDesigned : curriculumDesigned
    CurriculumDesigned --> FundingApproved : fundingApproved
    CurriculumDesigned --> CurriculumDesigned : fundingDenied
    FundingApproved --> ProgramLaunched : programLaunched
    ProgramLaunched --> ProgramEvaluated : programEvaluated
    ProgramEvaluated --> ProgramRevised : programRevised
    ProgramEvaluated --> ProgramLaunched : continueDelivery
    ProgramRevised --> ProgramLaunched : relaunch
    ProgramEvaluated --> [*]
```
...
graph TD
    ...
    G -->|Yes| D
    G -->|No| H[Seek Alternate Funding]
    F --> I[evaluateProgram]
    I --> J{Revisions Needed?}
    J -->|Yes| K[reviseProgram]
    J -->|No| L[Continue Delivery]
    K --> F
```
