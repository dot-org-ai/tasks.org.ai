...
```mermaid
erDiagram
    PartnershipAgreement ||--|{ JointProgram : enables
    JointProgram ||--|| ResourceSharingPlan : "supported by"
    JointProgram ||--|{ ComplianceReport : "reported via"
    JointProgram ||--|{ StudentTransferRecord : manages
    FundingAllocation }o--|| PartnershipAgreement : "finances"
    ComplianceReport }o--|| FundingAllocation : "justifies"
```
...
stateDiagram-v2
    ...
    PartnershipEstablished --> JointProgramPlanned : jointProgramPlanned
    JointProgramPlanned --> FundingAllocated : fundingAllocated
    FundingAllocated --> ResourcesShared : resourcesShared
    ResourcesShared --> ComplianceCoordinated : complianceCoordinated
    ComplianceCoordinated --> TransferManaged : transferManaged
    TransferManaged --> PartnershipReviewed : partnershipReviewed
    PartnershipReviewed --> JointProgramPlanned : continuePartnership
    PartnershipReviewed --> [*]
```
...
graph TD
    ...
    B --> C[allocateFunding]
    C --> D{Funding Sufficient?}
    D -->|Yes| E[shareResources]
    D -->|No| C
    E --> F[coordinateCompliance]
    F --> G[manageTransfers]
    G --> H[reviewPartnership]
    H --> I{Continue?}
    I -->|Yes| B
    I -->|No| J[Close Partnership]
```
