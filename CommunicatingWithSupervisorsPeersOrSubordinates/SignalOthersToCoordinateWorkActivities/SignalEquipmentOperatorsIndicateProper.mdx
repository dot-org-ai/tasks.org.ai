...
```mermaid
erDiagram
    PositioningPlan ||--o{ SignalProtocol : relates-to
    SignalProtocol ||--o{ Equipment : relates-to
    Equipment ||--o{ PlacementZone : relates-to
    PlacementZone ||--o{ ClearanceCheck : relates-to
    ClearanceCheck ||--o{ PositionLog : relates-to
```
...
stateDiagram-v2
    [*] --> Operator Briefed
    Operator Briefed --> Area Cleared : areaCleared
    Area Cleared --> Approach Signaled : approachSignaled
    Approach Signaled --> Fine Position Signaled : finePositionSignaled
    Fine Position Signaled --> Equipment Halted : equipmentHalted
    Equipment Halted --> Placement Confirmed : placementConfirmed
    Placement Confirmed --> Position Logged : positionLogged
    Fine Position Signaled --> Approach Signaled : revisionRequired
    Position Logged --> [*]
```
