...
```mermaid
erDiagram
    MovementOrder ||--o{ Signal : directs
    Signal ||--o{ ClearanceZone : clears
    ClearanceZone ||--o{ VehiclePosition : tracks
    IncidentReport ||--o{ VehiclePosition : documents
```
...
stateDiagram-v2
    [*] --> PathCleared
    PathCleared --> SignalInitiated : signalInitiated
    SignalInitiated --> AcknowledgmentConfirmed : acknowledgmentConfirmed
    AcknowledgmentConfirmed --> MovementDirected : movementDirected
    MovementDirected --> MovementHalted : movementHalted
    MovementHalted --> PathCleared : pathCleared
    MovementDirected --> MovementLogged : movementLogged
    MovementLogged --> PathCleared : signalInitiated
    MovementLogged --> [*]
```
