...
```mermaid
erDiagram
    DesignIssue ||--o{ StakeholderInput : collects
    OperationalProblem ||--o{ StakeholderInput : collects
    StakeholderInput ||--o{ AnalysisReport : informs
    AnalysisReport ||--o{ ProposedFix : recommends
    ProposedFix ||--o{ ChangeApproval : authorizes
```
...
stateDiagram-v2
    [*] --> IssueReported
    IssueReported --> PersonnelEngaged : personnelEngaged
    PersonnelEngaged --> InputGathered : inputGathered
    InputGathered --> ImpactAnalyzed : impactAnalyzed
    ImpactAnalyzed --> FixProposed : fixProposed
    FixProposed --> ChangeApproved : changeApproved
    ChangeApproved --> SolutionImplemented : solutionImplemented
    FixProposed --> InputGathered : inputGathered
    SolutionImplemented --> [*]
```
