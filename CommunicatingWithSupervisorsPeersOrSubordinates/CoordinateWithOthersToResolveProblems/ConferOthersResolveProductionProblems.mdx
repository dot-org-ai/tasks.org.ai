...
```mermaid
erDiagram
    ProductionIssue ||--o{ Conference : relates-to
    Conference ||--o{ RootCauseAnalysis : relates-to
    RootCauseAnalysis ||--o{ ActionPlan : relates-to
    ActionPlan ||--o{ CorrectiveAction : relates-to
    CorrectiveAction ||--o{ ValidationReport : relates-to
```
...
stateDiagram-v2
    [*] --> Issue Identified
    Issue Identified --> Conference Convened : conferenceConvened
    Conference Convened --> Root Cause Identified : rootCauseIdentified
    Root Cause Identified --> Action Plan Developed : actionPlanDeveloped
    Action Plan Developed --> Action Implemented : actionImplemented
    Action Implemented --> Resolution Validated : resolutionValidated
    Resolution Validated --> Issue Closed : issueClosed
    Action Plan Developed --> Root Cause Identified : revisionRequired
    Issue Closed --> [*]
```
