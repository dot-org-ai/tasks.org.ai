...
```mermaid
erDiagram
    Incident ||--o{ Ticket : relates-to
    Ticket ||--o{ Escalation : relates-to
    Escalation ||--o{ Resolution : relates-to
    Resolution ||--o{ RootCause : relates-to
    RootCause ||--o{ KnowledgeArticle : relates-to
```
...
stateDiagram-v2
    [*] --> Incident Reported
    Incident Reported --> Owner Assigned : ownerAssigned
    Owner Assigned --> Issue Diagnosed : issueDiagnosed
    Issue Diagnosed --> Incident Escalated : incidentEscalated
    Incident Escalated --> Fix Implemented : fixImplemented
    Fix Implemented --> Resolution Validated : resolutionValidated
    Resolution Validated --> Knowledge Documented : knowledgeDocumented
    Incident Escalated --> Issue Diagnosed : revisionRequired
