...
```mermaid
erDiagram
    Incident ||--o{ Ticket : tracks
    Ticket ||--o{ Escalation : elevates
    Incident ||--o{ Resolution : resolves
    Resolution ||--o{ RootCause : identifies
    RootCause ||--o{ KnowledgeArticle : documents
```
...
stateDiagram-v2
    [*] --> IncidentReported
    IncidentReported --> OwnerAssigned : ownerAssigned
    OwnerAssigned --> IssueDiagnosed : issueDiagnosed
    IssueDiagnosed --> FixImplemented : fixImplemented
    IssueDiagnosed --> Escalated : incidentEscalated
    Escalated --> OwnerAssigned : ownerAssigned
    FixImplemented --> ResolutionValidated : resolutionValidated
    ResolutionValidated --> KnowledgeDocumented : knowledgeDocumented
    ResolutionValidated --> IssueDiagnosed : issueDiagnosed
    KnowledgeDocumented --> [*]
```
