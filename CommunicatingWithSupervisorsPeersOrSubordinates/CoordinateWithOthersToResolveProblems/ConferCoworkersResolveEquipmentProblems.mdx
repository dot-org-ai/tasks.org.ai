...
```mermaid
erDiagram
    EquipmentIssue ||--o{ DiagnosticSession : relates-to
    DiagnosticSession ||--o{ WorkOrder : relates-to
    WorkOrder ||--o{ RepairLog : relates-to
    RepairLog ||--|{ KnowledgeEntry : records
    KnowledgeEntry ||--o{ PerformanceTest : relates-to
```
...
stateDiagram-v2
    [*] --> Issue Reported
    Issue Reported --> Coworker Consulted : coworkerConsulted
    Coworker Consulted --> Problem Diagnosed : problemDiagnosed
    Problem Diagnosed --> Work Order Created : workOrderCreated
    Work Order Created --> Repair Executed : repairExecuted
    Repair Executed --> Repair Validated : repairValidated
    Repair Validated --> Knowledge Captured : knowledgeCaptured
    Work Order Created --> Problem Diagnosed : revisionRequired
    Knowledge Captured --> [*]
```
