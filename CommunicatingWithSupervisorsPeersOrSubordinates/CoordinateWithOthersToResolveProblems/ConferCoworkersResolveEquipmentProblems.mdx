...
```mermaid
erDiagram
    EquipmentIssue ||--o{ DiagnosticSession : investigates
    EquipmentIssue ||--o{ WorkOrder : generates
    WorkOrder ||--o{ RepairLog : documents
    RepairLog ||--|{ KnowledgeEntry : captures
    PerformanceTest ||--o{ RepairLog : validates
```
...
stateDiagram-v2
    [*] --> IssueReported
    IssueReported --> CoworkerConsulted : coworkerConsulted
    CoworkerConsulted --> ProblemDiagnosed : problemDiagnosed
    ProblemDiagnosed --> WorkOrderCreated : workOrderCreated
    WorkOrderCreated --> RepairExecuted : repairExecuted
    RepairExecuted --> RepairValidated : repairValidated
    RepairValidated --> KnowledgeCaptured : knowledgeCaptured
    RepairValidated --> ProblemDiagnosed : problemDiagnosed
    KnowledgeCaptured --> [*]
```
