...
```mermaid
erDiagram
    Meeting ||--o{ Agenda : follows
    Meeting ||--o{ Decision : produces
    Agenda ||--o{ FinancialReport : references
    Decision ||--o{ ActionItem : generates
...
stateDiagram-v2
    [*] --> MeetingScheduled
    MeetingScheduled --> AgendaPrepared : agendaPrepared
    AgendaPrepared --> FinancialsShared : financialsShared
    FinancialsShared --> AnalysisPresented : analysisPresented
    AnalysisPresented --> OptionsDiscussed : optionsDiscussed
    OptionsDiscussed --> DecisionDocumented : decisionDocumented
    DecisionDocumented --> FollowUpAssigned : followUpAssigned
    OptionsDiscussed --> FinancialsShared : analysisPresented
    FollowUpAssigned --> MeetingScheduled : meetingScheduled
    FollowUpAssigned --> [*]
```
