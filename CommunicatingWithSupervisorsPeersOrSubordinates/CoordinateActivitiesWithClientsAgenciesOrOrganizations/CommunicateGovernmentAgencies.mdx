...
```mermaid
erDiagram
    AgencyCommunication ||--o{ ComplianceReport : transmits
    AgencyCommunication ||--o{ PermitApplication : submits
    InformationRequest ||--o{ AgencyResponse : yields
    InspectionNotice ||--o{ ComplianceReport : requires
    PermitApplication ||--o{ AgencyResponse : receives
```
...
stateDiagram-v2
    [*] --> DeadlineTracked
    DeadlineTracked --> ReportSubmitted : complianceReportSubmitted
    ReportSubmitted --> UnderReview : inspectionScheduled
    UnderReview --> ResponseProvided : informationRequestResponded
    ResponseProvided --> Documented : agencyInteractionDocumented
    Documented --> DeadlineTracked : submissionDeadlineTracked
    UnderReview --> ReportSubmitted : regulatoryIssueEscalated
    Documented --> [*]
```
