...
```mermaid
erDiagram
    Activity ||--o{ Dependency : requires
    Activity ||--o{ Deliverable : produces
    Schedule ||--o{ Activity : sequences
    Schedule ||--o{ Milestone : targets
    Issue ||--o{ Activity : blocks
```
...
stateDiagram-v2
    [*] --> ActivitiesPlanned
    ActivitiesPlanned --> SchedulesSynchronized : schedulesSynchronized
    SchedulesSynchronized --> InProgress : responsibilitiesAssigned
    InProgress --> ConflictsResolved : conflictsResolved
    InProgress --> StakeholdersUpdated : stakeholdersUpdated
    ConflictsResolved --> SchedulesSynchronized : schedulesSynchronized
    StakeholdersUpdated --> MilestoneCompleted : milestoneCompleted
    MilestoneCompleted --> InProgress : progressTracked
    MilestoneCompleted --> [*]
```
