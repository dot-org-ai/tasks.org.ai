...
```mermaid
erDiagram
    ExchangeRequest ||--o{ DataPackage : fulfills
    DataPackage ||--o{ ExchangeProtocol : transfers-via
    ExchangeProtocol ||--o{ Credential : authenticates
    Confirmation ||--o{ DataPackage : verifies
    AuditTrail ||--o{ ExchangeRequest : records
```
...
stateDiagram-v2
    ...
    InformationRequested --> ProtocolEstablished : protocolEstablished
    ProtocolEstablished --> DataPackaged : dataPackaged
    DataPackaged --> Transferred : informationTransferred
    Transferred --> ReceiptConfirmed : receiptConfirmed
    ReceiptConfirmed --> Audited : exchangeAudited
    Transferred --> DataPackaged : dataPackaged
    Audited --> InformationRequested : informationRequested
    Audited --> [*]
```
