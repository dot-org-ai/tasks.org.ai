...
```mermaid
erDiagram
    Dataset ||--o{ StatisticalTest : contains
    Dataset ||--o{ Finding : generates
    Dataset ||--o{ Hypothesis : contains
    Dataset ||--o{ InterpretationReport : generates
    Dataset ||--o{ Benchmark : contains
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Prepared : datasetPrepared
    Prepared --> Processing : hypothesisTested
    Processing --> Identified : patternsIdentified
    Identified --> Validated : findingsContextualized
    Validated --> [*]
```
## Workflow
