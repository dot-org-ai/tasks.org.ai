...
```mermaid
erDiagram
    ProcessMap ||--o{ LogisticsKPI : contains
    ProcessMap ||--o{ BottleneckAnalysis : contains
    ProcessMap ||--o{ TransportationLane : contains
    ProcessMap ||--o{ WarehouseLayout : contains
    ProcessMap ||--o{ InventoryTurnoverAnalysis : contains
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Processing : logisticsProcessesMapped
    Processing --> Identified : logisticsKPIsMeasured
    Identified --> UnderReview : bottlenecksIdentified
    UnderReview --> [*]
```
## Workflow
