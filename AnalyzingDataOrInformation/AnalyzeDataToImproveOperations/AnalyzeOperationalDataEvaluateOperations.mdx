...
```mermaid
erDiagram
    ProcessCapabilityStudy ||--o{ ProductConformanceReport : generates
    ProcessCapabilityStudy ||--o{ OperationalEfficiencyMetric : contains
    ProcessCapabilityStudy }o--|| StatisticalControlChart : uses
    ProcessCapabilityStudy ||--o{ YieldAnalysis : contains
    ProcessCapabilityStudy ||--o{ DefectPareto : contains
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Processing : operationalDataExtracted
    Processing --> UnderReview : processCapabilityAnalyzed
    UnderReview --> [*]
```
## Workflow
