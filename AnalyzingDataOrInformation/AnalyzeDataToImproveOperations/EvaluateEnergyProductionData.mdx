...
```mermaid
erDiagram
    GenerationUnit ||--o{ ProductionRecord : generates
    GenerationUnit ||--o{ CapacityFactor : contains
    GenerationUnit ||--o{ OutageEvent : contains
    GenerationUnit ||--o{ FuelConsumption : contains
    GenerationUnit ||--o{ EfficiencyBenchmark : contains
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Processing : productionDataIngested
    Processing --> UnderReview : capacityFactorCalculated
    UnderReview --> Identified : efficiencyBenchmarked
    Identified --> [*]
```
## Workflow
