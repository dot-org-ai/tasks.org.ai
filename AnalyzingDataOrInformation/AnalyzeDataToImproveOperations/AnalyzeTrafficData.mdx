...
```mermaid
erDiagram
    TrafficCount ||--o{ SpeedStudy : contains
    TrafficCount ||--o{ IntersectionAnalysis : contains
    TrafficCount ||--o{ CrashRecord : generates
    TrafficCount ||--o{ TrafficForecast : contains
    TrafficCount ||--o{ SignalTimingPlan : contains
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Processing : trafficDataIngested
    Processing --> Released : levelOfServiceCalculated
    Flagged --> [*]
```
## Workflow
