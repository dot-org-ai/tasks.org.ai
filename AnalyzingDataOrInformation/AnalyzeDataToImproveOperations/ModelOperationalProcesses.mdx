...
```mermaid
erDiagram
    ProcessModel ||--o{ SimulationRun : contains
    ProcessModel ||--o{ Bottleneck : contains
    ProcessModel ||--o{ CycleTime : contains
    ProcessModel ||--o{ ResourcePool : contains
    ProcessModel ||--o{ OptimizationScenario : contains
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Processing : processCaptured
    Processing --> Complete : modelBuilt
    Complete --> Identified : simulationCompleted
    Identified --> [*]
```
## Workflow
