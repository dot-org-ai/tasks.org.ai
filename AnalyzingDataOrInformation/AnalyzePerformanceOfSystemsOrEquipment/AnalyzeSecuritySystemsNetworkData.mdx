...
```mermaid
erDiagram
    SecurityScan ||--o{ Vulnerability : contains
    SecurityScan ||--o{ ThreatIndicator : contains
    SecurityScan ||--o{ SecurityEvent : contains
    SecurityScan ||--o{ ComplianceReport : generates
    SecurityScan ||--o{ Remediation : contains
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Processing : networkScanned
    Processing --> UnderReview : trafficAnalyzed
    UnderReview --> Released : vulnerabilityAssessed
    Flagged --> [*]
```
## Workflow
