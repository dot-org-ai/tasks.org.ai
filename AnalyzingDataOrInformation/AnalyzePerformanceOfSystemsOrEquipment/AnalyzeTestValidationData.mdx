...
```mermaid
erDiagram
    ValidationTest ||--o{ TestResult : generates
    ValidationTest ||--o{ AcceptanceCriteria : contains
    ValidationTest ||--o{ ValidationReport : generates
    ValidationTest ||--o{ Defect : contains
    ValidationTest ||--o{ ValidationDecision : contains
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Processing : testResultsCollected
    Processing --> Validated : completenessVerified
    Validated --> Identified : requirementsCompared
    Identified --> [*]
```
## Workflow
