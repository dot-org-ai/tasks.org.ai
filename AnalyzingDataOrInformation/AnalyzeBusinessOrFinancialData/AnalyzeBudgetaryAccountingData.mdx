...
```mermaid
erDiagram
    GeneralLedger ||--o{ BudgetLineItem : contains
    GeneralLedger ||--o{ JournalEntry : contains
    GeneralLedger ||--o{ VarianceReport : generates
    GeneralLedger ||--o{ Reconciliation : contains
    GeneralLedger ||--o{ TrialBalance : contains
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Processing : ledgerDataExtracted
    Processing --> Flagged : variancesCalculated
    Flagged --> Prepared : accountReconciled
    Prepared --> [*]
```
## Workflow
