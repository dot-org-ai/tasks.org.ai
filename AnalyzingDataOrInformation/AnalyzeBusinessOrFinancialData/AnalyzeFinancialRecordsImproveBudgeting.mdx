...
```mermaid
erDiagram
    HistoricalSpendingRecord ||--o{ BudgetAssumption : contains
    HistoricalSpendingRecord ||--o{ SpendingVariance : contains
    HistoricalSpendingRecord ||--o{ ForecastModel : contains
    HistoricalSpendingRecord ||--o{ BudgetTemplate : contains
    HistoricalSpendingRecord ||--o{ AllocationFormula : contains
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Processing : historicalRecordsExtracted
    Processing --> UnderReview : spendingPatternsAnalyzed
    UnderReview --> Identified : budgetAssumptionsEvaluated
    Identified --> [*]
```
## Workflow
