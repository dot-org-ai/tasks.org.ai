...
```mermaid
erDiagram
    CostDriver ||--o{ CostPerUnit : contains
    CostDriver ||--o{ OverheadAllocation : contains
    CostDriver ||--o{ SpendingAnomaly : contains
    CostDriver ||--o{ EfficiencyMetric : contains
    CostDriver ||--o{ SavingsOpportunity : contains
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Processing : transactionDataExtracted
    Processing --> UnderReview : costPerUnitCalculated
    UnderReview --> Identified : overheadAnalyzed
    Identified --> [*]
```
## Workflow
