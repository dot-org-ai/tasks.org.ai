...
```mermaid
erDiagram
    LegalPrecedent ||--o{ CurrentCase : contains
    LegalPrecedent ||--o{ CaseImplication : contains
    LegalPrecedent ||--o{ DistinguishingFactor : contains
    LegalPrecedent ||--o{ LegalArgument : contains
    LegalPrecedent ||--o{ ResearchMemo : contains
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Processing : precedentsResearched
    Processing --> UnderReview : relevanceAssessed
    UnderReview --> Identified : holdingsAnalyzed
    Identified --> [*]
```
## Workflow
