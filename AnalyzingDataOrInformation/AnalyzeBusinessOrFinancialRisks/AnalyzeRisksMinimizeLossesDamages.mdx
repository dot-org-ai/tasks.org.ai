...
```mermaid
erDiagram
    LossExposure ||--o{ LossDistributionModel : contains
    LossExposure ||--o{ ClaimsHistory : contains
    LossExposure }o--|| ControlRecommendation : uses
    LossExposure ||--o{ RetentionAnalysis : contains
    LossExposure ||--o{ LossMitigationPlan : contains
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Identified : lossExposuresIdentified
    Identified --> Processing : lossDistributionsModeled
    Processing --> UnderReview : claimsHistoryAnalyzed
    UnderReview --> [*]
```
## Workflow
