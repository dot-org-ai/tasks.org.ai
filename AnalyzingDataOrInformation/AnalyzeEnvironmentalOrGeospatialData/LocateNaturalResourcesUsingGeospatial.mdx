...
```mermaid
erDiagram
    GeospatialDataset ||--o{ ResourceIndicator : contains
    GeospatialDataset ||--o{ SpatialAnalysisResult : generates
    GeospatialDataset ||--o{ ResourceMap : contains
    GeospatialDataset ||--o{ FeasibilityAssessment : contains
    GeospatialDataset ||--o{ ExplorationTarget : contains
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Processing : geospatialDataAcquired
    Processing --> Prepared : remoteSensingProcessed
    Prepared --> Complete : spatialAnalysisCompleted
    Complete --> [*]
```
## Workflow
