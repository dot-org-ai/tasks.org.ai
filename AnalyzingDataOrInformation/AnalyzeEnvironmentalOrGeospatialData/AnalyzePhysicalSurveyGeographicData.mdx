...
```mermaid
erDiagram
    SurveyMeasurement ||--o{ BoundaryDetermination : contains
    SurveyMeasurement ||--o{ DigitalTerrainModel : contains
    SurveyMeasurement ||--o{ HydrographicSounding : contains
    SurveyMeasurement ||--o{ AdjustmentResult : generates
    SurveyMeasurement ||--o{ SurveyPlat : contains
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Processing : surveyDataImported
    Processing --> Released : measurementsAdjusted
    Released --> Prepared : boundariesComputed
    Prepared --> [*]
```
## Workflow
