...
```mermaid
erDiagram
    PackagingRun ||--o{ PackagingMaterial : relates-to
    PackagingMaterial ||--o{ FillSpecification : relates-to
    FillSpecification ||--o{ SealIntegrityResult : relates-to
    SealIntegrityResult ||--o{ LabelTemplate : relates-to
    LabelTemplate ||--o{ PalletLoad : relates-to
```
...
```mermaid
stateDiagram-v2
    [*] --> Product Staged For Packing
    Product Staged For Packing --> Packaging Equipment Configured : packagingEquipmentConfigured
    Packaging Equipment Configured --> Packaging Containers Filled : packagingContainersFilled
    Packaging Containers Filled --> Packages Sealed : packagesSealed
    Packages Sealed --> Labels Applied : labelsApplied
    Labels Applied --> Package Quality Inspected : packageQualityInspected
    Package Quality Inspected --> Finished Packages Palletized : finishedPackagesPalletized
    Packages Sealed --> Packaging Containers Filled : revisionRequired
    Finished Packages Palletized --> [*]
```
