...
```mermaid
erDiagram
    MixingBatch ||--o{ Formula : follows
    Formula ||--o{ Ingredient : specifies
    MixingBatch ||--o{ MixingCycle : executes
    MixingCycle ||--o{ ConsistencyReading : produces
    MixingBatch ||--o{ BatchRecord : documents
```
...
stateDiagram-v2
    [*] --> IngredientsStaged
    IngredientsStaged --> MixingVesselLoaded : mixingVesselLoaded
    MixingVesselLoaded --> MixingCycleConfigured : mixingCycleConfigured
    MixingCycleConfigured --> MixingCycleCompleted : mixingCycleCompleted
    MixingCycleCompleted --> BatchConsistencySampled : batchConsistencySampled
    BatchConsistencySampled --> MixedBatchDischarged : mixedBatchDischarged
    MixedBatchDischarged --> MixingEquipmentCleaned : mixingEquipmentCleaned
    BatchConsistencySampled --> MixingCycleConfigured : mixingCycleCompleted
    MixingEquipmentCleaned --> [*]
```
