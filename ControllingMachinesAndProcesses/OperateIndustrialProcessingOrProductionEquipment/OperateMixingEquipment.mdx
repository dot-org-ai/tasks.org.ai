...
```mermaid
erDiagram
    MixingBatch ||--o{ Formula : relates-to
    Formula ||--o{ MixingCycle : relates-to
    MixingCycle ||--o{ Ingredient : relates-to
    Ingredient ||--o{ ConsistencyReading : relates-to
    ConsistencyReading ||--o{ BatchRecord : relates-to
```
...
stateDiagram-v2
    [*] --> Ingredients Staged
    Ingredients Staged --> Mixing Vessel Loaded : mixingVesselLoaded
    Mixing Vessel Loaded --> Mixing Cycle Configured : mixingCycleConfigured
    Mixing Cycle Configured --> Mixing Cycle Completed : mixingCycleCompleted
    Mixing Cycle Completed --> Batch Consistency Sampled : batchConsistencySampled
    Batch Consistency Sampled --> Mixed Batch Discharged : mixedBatchDischarged
    Mixed Batch Discharged --> Mixing Equipment Cleaned : mixingEquipmentCleaned
    Mixing Cycle Completed --> Mixing Cycle Configured : revisionRequired
    Mixing Equipment Cleaned --> [*]
```
