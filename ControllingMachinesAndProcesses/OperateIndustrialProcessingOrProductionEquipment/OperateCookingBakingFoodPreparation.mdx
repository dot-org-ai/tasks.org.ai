...
```mermaid
erDiagram
    CookingBatch ||--o{ Recipe : relates-to
    Recipe ||--o{ CookingProfile : relates-to
    CookingProfile ||--o{ TemperatureLog : relates-to
    TemperatureLog ||--o{ FoodProduct : relates-to
    FoodProduct ||--o{ SanitationRecord : relates-to
```
...
```mermaid
stateDiagram-v2
    [*] --> Cooking Equipment Preheated
    Cooking Equipment Preheated --> Ingredients Loaded : ingredientsLoaded
    Ingredients Loaded --> Cooking Profile Set : cookingProfileSet
    Cooking Profile Set --> Cooking Progress Monitored : cookingProgressMonitored
    Cooking Progress Monitored --> Food Safety Verified : foodSafetyVerified
    Food Safety Verified --> Cooked Product Removed : cookedProductRemoved
    Cooked Product Removed --> Cooking Equipment Cleaned : cookingEquipmentCleaned
    Cooking Progress Monitored --> Cooking Profile Set : revisionRequired
