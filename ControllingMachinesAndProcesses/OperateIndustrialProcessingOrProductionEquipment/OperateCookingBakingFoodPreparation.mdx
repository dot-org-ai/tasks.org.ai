...
```mermaid
erDiagram
    CookingBatch ||--o{ Recipe : follows
    Recipe ||--o{ CookingProfile : defines
    CookingBatch ||--o{ TemperatureLog : records
    CookingBatch ||--o{ FoodProduct : produces
    FoodProduct ||--o{ SanitationRecord : requires
```
...
```mermaid
stateDiagram-v2
    [*] --> CookingEquipmentPreheated
    CookingEquipmentPreheated --> CookingProfileSet : cookingProfileSet
    CookingProfileSet --> IngredientsLoaded : ingredientsLoaded
    IngredientsLoaded --> CookingProgressMonitored : cookingProgressMonitored
    CookingProgressMonitored --> FoodSafetyVerified : foodSafetyVerified
    CookingProgressMonitored --> CookingProgressMonitored : cookingProgressMonitored
    FoodSafetyVerified --> CookedProductRemoved : cookedProductRemoved
    CookedProductRemoved --> CookingEquipmentCleaned : cookingEquipmentCleaned
    CookingEquipmentCleaned --> CookingEquipmentPreheated : cookingEquipmentPreheated
    CookingEquipmentCleaned --> [*]
```
