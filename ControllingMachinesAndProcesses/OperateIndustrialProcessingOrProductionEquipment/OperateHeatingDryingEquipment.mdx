...
```mermaid
erDiagram
    HeatingCycle ||--o{ DryingBatch : processes
    HeatingCycle ||--o{ TemperatureProfile : follows
    DryingBatch ||--o{ MoistureReading : tests
    HeatingCycle ||--o{ EquipmentZone : operates-in
    EquipmentZone ||--o{ ThermalLog : records
```
...
stateDiagram-v2
    [*] --> EquipmentPreheated
    EquipmentPreheated --> TemperatureProfileSet : temperatureProfileSet
    TemperatureProfileSet --> DryingBatchLoaded : dryingBatchLoaded
    DryingBatchLoaded --> ThermalConditionsMonitored : thermalConditionsMonitored
    ThermalConditionsMonitored --> MoistureContentMeasured : moistureContentMeasured
    MoistureContentMeasured --> ThermalConditionsMonitored : thermalConditionsMonitored
    MoistureContentMeasured --> DryingCycleCompleted : dryingCycleCompleted
    DryingCycleCompleted --> ThermalDataLogged : thermalDataLogged
    ThermalDataLogged --> EquipmentPreheated : equipmentPreheated
    ThermalDataLogged --> [*]
```
