...
```mermaid
erDiagram
    FinishingOrder ||--o{ GarmentBatch : groups
    GarmentBatch ||--o{ FabricType : categorizedBy
    FabricType ||--o{ PressingProfile : determines
    GarmentBatch ||--o{ QualityResult : inspectedBy
    PressingProfile ||--o{ EquipmentSetting : configures
```
...
```mermaid
stateDiagram-v2
    [*] --> GarmentsSortedByFabric
    GarmentsSortedByFabric --> PressingProfileConfigured : pressingProfileConfigured
    PressingProfileConfigured --> GarmentPressed : garmentPressed
    GarmentPressed --> GarmentSteamed : garmentSteamed
    GarmentSteamed --> FinishedGarmentInspected : finishedGarmentInspected
    FinishedGarmentInspected --> FinishedGarmentPackaged : finishedGarmentPackaged
    FinishedGarmentInspected --> PressingProfileConfigured : finishedGarmentInspected
    FinishedGarmentPackaged --> [*]
```
