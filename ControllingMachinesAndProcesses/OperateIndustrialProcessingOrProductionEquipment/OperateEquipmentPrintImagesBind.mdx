...
```mermaid
erDiagram
    PrintJob ||--o{ PrintPlate : relates-to
    PrintPlate ||--o{ PressSheet : relates-to
    PressSheet ||--o{ BindingOrder : relates-to
    BindingOrder ||--o{ ColorProfile : relates-to
    ColorProfile ||--o{ FinishedProduct : relates-to
```
...
stateDiagram-v2
    [*] --> Print Plate Setup
    Print Plate Setup --> Color Output Calibrated : colorOutputCalibrated
    Color Output Calibrated --> Print Run Completed : printRunCompleted
    Print Run Completed --> Bindery Equipment Configured : binderyEquipmentConfigured
    Bindery Equipment Configured --> Printed Materials Bound : printedMaterialsBound
    Printed Materials Bound --> Print Quality Inspected : printQualityInspected
    Print Quality Inspected --> Product Trimmed And Finished : productTrimmedAndFinished
    Bindery Equipment Configured --> Print Run Completed : revisionRequired
    Product Trimmed And Finished --> [*]
```
