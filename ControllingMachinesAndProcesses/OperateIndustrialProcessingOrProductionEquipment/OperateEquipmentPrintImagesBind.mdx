...
```mermaid
erDiagram
    PrintJob ||--o{ PrintPlate : prepares
    PrintPlate ||--o{ PressSheet : prints
    PrintJob ||--o{ ColorProfile : calibrates-with
    PrintJob ||--o{ BindingOrder : requires
    BindingOrder ||--o{ FinishedProduct : assembles
```
...
stateDiagram-v2
    [*] --> PrintPlateSetup
    PrintPlateSetup --> ColorOutputCalibrated : colorOutputCalibrated
    ColorOutputCalibrated --> PrintRunCompleted : printRunCompleted
    PrintRunCompleted --> PrintQualityInspected : printQualityInspected
    PrintQualityInspected --> ColorOutputCalibrated : colorOutputCalibrated
    PrintQualityInspected --> BinderyEquipmentConfigured : binderyEquipmentConfigured
    BinderyEquipmentConfigured --> PrintedMaterialsBound : printedMaterialsBound
    PrintedMaterialsBound --> ProductTrimmedAndFinished : productTrimmedAndFinished
    ProductTrimmedAndFinished --> PrintPlateSetup : printPlateSetup
    ProductTrimmedAndFinished --> [*]
```
