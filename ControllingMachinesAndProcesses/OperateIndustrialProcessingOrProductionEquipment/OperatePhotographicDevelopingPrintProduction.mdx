...
```mermaid
erDiagram
    DevelopmentBatch ||--o{ ChemicalSolution : uses
    DevelopmentBatch ||--o{ PrintJob : processes
    PrintJob ||--o{ ExposureSetting : configures
    PrintJob ||--o{ ProcessingProfile : follows
    PrintJob ||--o{ OutputSample : produces
```
...
```mermaid
stateDiagram-v2
    [*] --> DevelopmentRunInitialized
    DevelopmentRunInitialized --> PhotographicMaterialLoaded : photographicMaterialLoaded
    PhotographicMaterialLoaded --> ExposureUnitCalibrated : exposureUnitCalibrated
    ExposureUnitCalibrated --> ChemicalLevelsMonitored : chemicalLevelsMonitored
    ChemicalLevelsMonitored --> PrintRunCompleted : printRunCompleted
    PrintRunCompleted --> OutputInspected : outputInspected
    OutputInspected --> EquipmentCleaned : equipmentCleaned
    OutputInspected --> ExposureUnitCalibrated : outputInspected
    EquipmentCleaned --> [*]
```
