...
```mermaid
erDiagram
    DevelopmentBatch ||--o{ ChemicalSolution : relates-to
    ChemicalSolution ||--o{ PrintJob : relates-to
    PrintJob ||--o{ ExposureSetting : relates-to
    ExposureSetting ||--o{ ProcessingProfile : relates-to
    ProcessingProfile ||--o{ OutputSample : relates-to
```
...
```mermaid
stateDiagram-v2
    [*] --> Development Run Initialized
    Development Run Initialized --> Photographic Material Loaded : photographicMaterialLoaded
    Photographic Material Loaded --> Exposure Unit Calibrated : exposureUnitCalibrated
    Exposure Unit Calibrated --> Chemical Levels Monitored : chemicalLevelsMonitored
    Chemical Levels Monitored --> Print Run Completed : printRunCompleted
    Print Run Completed --> Output Inspected : outputInspected
    Output Inspected --> Equipment Cleaned : equipmentCleaned
    Chemical Levels Monitored --> Exposure Unit Calibrated : revisionRequired
    Equipment Cleaned --> [*]
```
