...
```mermaid
erDiagram
    TreatmentProcess ||--o{ ChemicalDose : applies
    TreatmentProcess ||--o{ ProcessParameter : monitors
    TreatmentProcess ||--o{ WaterSample : collects
    TreatmentProcess ||--o{ DischargeRecord : produces
    ProcessParameter ||--o{ AlarmCondition : triggers
```
...
```mermaid
stateDiagram-v2
    [*] --> TreatmentProcessStarted
    TreatmentProcessStarted --> ChemicalDosingAdjusted : chemicalDosingAdjusted
    ChemicalDosingAdjusted --> ProcessParametersMonitored : processParametersMonitored
    ProcessParametersMonitored --> AlarmConditionResponded : alarmConditionResponded
    AlarmConditionResponded --> ChemicalDosingAdjusted : chemicalDosingAdjusted
    ProcessParametersMonitored --> WaterSampleCollected : waterSampleCollected
    WaterSampleCollected --> WaterQualityAnalyzed : waterQualityAnalyzed
    WaterQualityAnalyzed --> ChemicalDosingAdjusted : chemicalDosingAdjusted
    WaterQualityAnalyzed --> DischargeFlowManaged : dischargeFlowManaged
    DischargeFlowManaged --> [*]
```
