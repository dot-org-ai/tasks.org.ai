...
```mermaid
erDiagram
    FormingJob ||--o{ DieSet : uses
    FormingJob ||--o{ RawMaterial : consumes
    DieSet ||--o{ FormingCycle : executes
    FormingCycle ||--o{ FormedPart : produces
    FormedPart ||--o{ ProcessParameter : configuredBy
```
...
stateDiagram-v2
    [*] --> FormingToolingSetup
    FormingToolingSetup --> RawMaterialLoaded : rawMaterialLoaded
    RawMaterialLoaded --> ProcessParametersConfigured : processParametersConfigured
    ProcessParametersConfigured --> FormingCycleCompleted : formingCycleCompleted
    FormingCycleCompleted --> FormedPartInspected : formedPartInspected
    FormedPartInspected --> ToolingAlignmentAdjusted : toolingAlignmentAdjusted
    ToolingAlignmentAdjusted --> ProductionMetricsRecorded : productionMetricsRecorded
    FormedPartInspected --> ProcessParametersConfigured : toolingAlignmentAdjusted
    ProductionMetricsRecorded --> [*]
```
