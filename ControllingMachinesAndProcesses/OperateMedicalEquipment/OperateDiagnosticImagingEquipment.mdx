...
```mermaid
erDiagram
    ImagingOrder ||--o{ ImagingSystem : assignedTo
    ImagingSystem ||--o{ DiagnosticImage : captures
    DiagnosticImage ||--o{ ExposureRecord : logs
    ImagingOrder ||--o{ ProtocolTemplate : follows
    DiagnosticImage ||--o{ QualityCheckRecord : reviewedBy
```
...
stateDiagram-v2
    ...
    PatientIdentityVerified --> PatientPositioned : patientPositioned
    PatientPositioned --> ProtocolSelected : protocolSelected
    ProtocolSelected --> ImagesAcquired : imagesAcquired
    ImagesAcquired --> ImageQualityReviewed : imageQualityReviewed
    ImageQualityReviewed --> ImagesTransmitted : imagesTransmitted
    ImagesTransmitted --> ExposureRecorded : exposureRecorded
    ImageQualityReviewed --> ProtocolSelected : imageQualityReviewed
    ExposureRecorded --> [*]
```
