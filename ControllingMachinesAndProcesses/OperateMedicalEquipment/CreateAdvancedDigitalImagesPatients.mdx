...
```mermaid
erDiagram
    ImagingOrder ||--o{ ScanProtocol : specifies
    ScanProtocol ||--o{ DigitalImage : produces
    ImagingOrder ||--o{ PatientRecord : references
    DigitalImage ||--o{ ImageSeries : belongsTo
    ImageSeries ||--o{ QualityReport : evaluatedBy
```
...
stateDiagram-v2
    [*] --> ProtocolConfigured
    ProtocolConfigured --> PatientPositioned : patientPositioned
    PatientPositioned --> ScanAcquired : scanAcquired
    ScanAcquired --> ImagesProcessed : imagesProcessed
    ImagesProcessed --> QualityReviewed : qualityReviewed
    QualityReviewed --> ImagesDistributed : imagesDistributed
    ImagesDistributed --> StudyArchived : studyArchived
    QualityReviewed --> ScanAcquired : qualityReviewed
    StudyArchived --> [*]
```
