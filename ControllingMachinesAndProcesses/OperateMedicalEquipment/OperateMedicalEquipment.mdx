...
```mermaid
erDiagram
    MedicalDevice ||--o{ DeviceOrder : assignedBy
    MedicalDevice ||--o{ AlarmEvent : triggers
    MedicalDevice ||--o{ MaintenanceRecord : tracks
    MedicalDevice ||--o{ UsageSession : logs
    MedicalDevice ||--o{ ConsumableInventory : consumes
```
...
stateDiagram-v2
    ...
    DeviceSetupCompleted --> DeviceCalibrated : deviceCalibrated
    DeviceCalibrated --> SessionInitiated : sessionInitiated
    SessionInitiated --> ReadingsMonitored : readingsMonitored
    ReadingsMonitored --> AlarmTriggered : alarmTriggered
    AlarmTriggered --> ReadingsMonitored : alarmTriggered
    ReadingsMonitored --> SessionTerminated : sessionTerminated
    SessionTerminated --> [*]
```
