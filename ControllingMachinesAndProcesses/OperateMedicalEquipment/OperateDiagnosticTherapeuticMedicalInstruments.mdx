...
```mermaid
erDiagram
    ProcedureOrder ||--o{ ClinicalInstrument : relates-to
    ClinicalInstrument ||--o{ TestResult : relates-to
    TestResult ||--o{ TreatmentSession : relates-to
    TreatmentSession ||--o{ CalibrationRecord : relates-to
    CalibrationRecord ||--o{ PatientPreparation : relates-to
```
...
stateDiagram-v2
    [*] --> Patient Prepared
    Patient Prepared --> Instrument Configured : instrumentConfigured
    Instrument Configured --> Test Executed : testExecuted
    Test Executed --> Therapy Delivered : therapyDelivered
    Therapy Delivered --> Results Reviewed : resultsReviewed
    Results Reviewed --> Procedure Documented : procedureDocumented
    Procedure Documented --> Adverse Reaction Detected : adverseReactionDetected
    Therapy Delivered --> Test Executed : revisionRequired
    Adverse Reaction Detected --> [*]
```
