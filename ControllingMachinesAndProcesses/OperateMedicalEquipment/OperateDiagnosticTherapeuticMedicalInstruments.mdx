...
```mermaid
erDiagram
    ProcedureOrder ||--o{ ClinicalInstrument : assignedTo
    ClinicalInstrument ||--o{ TestResult : produces
    ClinicalInstrument ||--o{ TreatmentSession : delivers
    ClinicalInstrument ||--o{ CalibrationRecord : calibratedBy
    ProcedureOrder ||--o{ PatientPreparation : requires
```
...
stateDiagram-v2
    [*] --> PatientPrepared
    PatientPrepared --> InstrumentConfigured : instrumentConfigured
    InstrumentConfigured --> TestExecuted : testExecuted
    TestExecuted --> TherapyDelivered : therapyDelivered
    TherapyDelivered --> ResultsReviewed : resultsReviewed
    ResultsReviewed --> ProcedureDocumented : procedureDocumented
    ResultsReviewed --> InstrumentConfigured : resultsReviewed
    ProcedureDocumented --> [*]
```
