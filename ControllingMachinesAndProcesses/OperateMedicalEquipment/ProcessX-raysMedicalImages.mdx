...
```mermaid
erDiagram
    RawImage ||--o{ ProcessedImage : enhancedTo
    ProcessedImage ||--o{ ImageMetadata : taggedWith
    ProcessedImage ||--o{ PACSRecord : archivedIn
    RawImage ||--o{ ProcessingProtocol : processedBy
    ProcessedImage ||--o{ RejectAnalysis : flaggedBy
```
...
stateDiagram-v2
    [*] --> ImageDeveloped
    ImageDeveloped --> ImageEnhanced : imageEnhanced
    ImageEnhanced --> MetadataVerified : metadataVerified
    MetadataVerified --> ImageArchived : imageArchived
    ImageArchived --> ImageDistributed : imageDistributed
    ImageDistributed --> QualityReportGenerated : qualityReportGenerated
    MetadataVerified --> ImageEnhanced : metadataVerified
