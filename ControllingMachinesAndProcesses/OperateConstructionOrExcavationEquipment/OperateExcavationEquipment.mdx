...
```mermaid
erDiagram
    ExcavationJob ||--o{ DigPlan : follows
    ExcavationJob ||--o{ ExcavatorUnit : assigns
    ExcavationJob ||--o{ SafetyPermit : requires
    ExcavatorUnit ||--o{ EquipmentInspection : undergoes
    ExcavationJob ||--o{ SoilLog : records
```
...
stateDiagram-v2
    ...
    PreOperationCheckCompleted --> ExcavatorPositioned : excavatorPositioned
    ExcavatorPositioned --> DigCycleExecuted : digCycleExecuted
    DigCycleExecuted --> TrenchDepthReached : trenchDepthReached
    DigCycleExecuted --> DigCycleExecuted : digCycleExecuted
    DigCycleExecuted --> UtilityEncountered : utilityEncountered
    UtilityEncountered --> ExcavatorPositioned : excavatorPositioned
    TrenchDepthReached --> HaulVehicleLoaded : haulVehicleLoaded
    HaulVehicleLoaded --> TrenchBackfilled : trenchBackfilled
    TrenchBackfilled --> [*]
```
