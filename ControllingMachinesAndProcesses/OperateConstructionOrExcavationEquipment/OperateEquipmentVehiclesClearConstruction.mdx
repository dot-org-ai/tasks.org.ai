...
```mermaid
erDiagram
    ClearingJob ||--o{ HaulLoad : relates-to
    HaulLoad ||--o{ StagingArea : relates-to
    StagingArea ||--o{ DebrisManifest : relates-to
    DebrisManifest ||--o{ SiteVehicle : relates-to
    SiteVehicle ||--o{ ErosionControlPlan : relates-to
```
...
stateDiagram-v2
    [*] --> Vegetation Cleared
    Vegetation Cleared --> Debris Loaded : debrisLoaded
    Debris Loaded --> Material Hauled : materialHauled
    Material Hauled --> Fill Material Spread : fillMaterialSpread
    Fill Material Spread --> Cleared Area Graded : clearedAreaGraded
    Cleared Area Graded --> Erosion Control Installed : erosionControlInstalled
    Erosion Control Installed --> Vehicle Breakdown : vehicleBreakdown
    Fill Material Spread --> Material Hauled : revisionRequired
    Vehicle Breakdown --> [*]
```
