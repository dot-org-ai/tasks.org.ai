...
```mermaid
erDiagram
    PavingProject ||--o{ MillingPlan : specifies
    PavingProject ||--o{ AsphaltBatch : receives
    AsphaltBatch ||--o{ PavingPass : feeds
    PavingPass ||--o{ CompactionRecord : measures
    PavingPass ||--o{ SurfaceProfile : surveys
```
...
```mermaid
stateDiagram-v2
    [*] --> ExistingSurfaceMilled
    ExistingSurfaceMilled --> PaverLoaded : paverLoaded
    PaverLoaded --> TemperatureChecked : temperatureChecked
    TemperatureChecked --> AsphaltLaid : asphaltLaid
    AsphaltLaid --> SurfaceCompacted : surfaceCompacted
    SurfaceCompacted --> SurfaceProfileMeasured : surfaceProfileMeasured
    SurfaceProfileMeasured --> PaverLoaded : paverLoaded
    SurfaceProfileMeasured --> SealCoatApplied : sealCoatApplied
    SealCoatApplied --> [*]
```
