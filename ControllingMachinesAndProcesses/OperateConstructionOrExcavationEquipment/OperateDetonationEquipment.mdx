...
```mermaid
erDiagram
    BlastPlan ||--o{ DetonationSystem : relates-to
    DetonationSystem ||--o{ BlastHole : relates-to
    BlastHole ||--o{ BlastingPermit : relates-to
    BlastingPermit ||--o{ SeismicRecord : relates-to
    SeismicRecord ||--o{ ExplosivesInventory : relates-to
```
...
stateDiagram-v2
    [*] --> Blast Holes Loaded
    Blast Holes Loaded --> Detonators Connected : detonatorsConnected
    Detonators Connected --> Circuit Tested : circuitTested
    Circuit Tested --> Exclusion Zone Established : exclusionZoneEstablished
    Exclusion Zone Established --> Charges Fired : chargesFired
    Charges Fired --> Post Blast Inspection Completed : postBlastInspectionCompleted
    Post Blast Inspection Completed --> Misfire Detected : misfireDetected
    Exclusion Zone Established --> Circuit Tested : revisionRequired
    Misfire Detected --> [*]
```
