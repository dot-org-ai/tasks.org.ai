...
```mermaid
erDiagram
    ConstructionUnit ||--o{ GradingPlan : executes
    ConstructionUnit ||--o{ PileDrivingRecord : drives
    ConstructionUnit ||--o{ LiftPlan : lifts-per
    ConstructionUnit ||--o{ EquipmentInspection : undergoes
    ConstructionUnit ||--o{ FuelLog : consumes
```
...
```mermaid
stateDiagram-v2
    [*] --> PreOperationCheckCompleted
    PreOperationCheckCompleted --> EquipmentPositioned : equipmentPositioned
    EquipmentPositioned --> TerrainGraded : terrainGraded
    EquipmentPositioned --> PileDriven : pileDriven
    EquipmentPositioned --> LiftExecuted : liftExecuted
    TerrainGraded --> SubgradeCompacted : subgradeCompacted
    SubgradeCompacted --> EquipmentPositioned : equipmentPositioned
    PileDriven --> EquipmentPositioned : equipmentPositioned
    LiftExecuted --> EquipmentPositioned : equipmentPositioned
    EquipmentPositioned --> EquipmentMalfunction : equipmentMalfunction
    EquipmentMalfunction --> PreOperationCheckCompleted : preOperationCheckCompleted
    SubgradeCompacted --> [*]
```
