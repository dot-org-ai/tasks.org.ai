...
```mermaid
erDiagram
    BreakingJob ||--o{ BreakerUnit : assigns
    BreakingJob ||--o{ BreakingPermit : requires
    BreakerUnit ||--o{ MaterialFragment : produces
    MaterialFragment ||--o{ DebrisLoadout : fills
    BreakingJob ||--o{ DustControlPlan : follows
```
...
stateDiagram-v2
    [*] --> MaterialAssessed
    MaterialAssessed --> BreakingToolSelected : breakingToolSelected
    BreakingToolSelected --> BreakingExecuted : breakingExecuted
    BreakingExecuted --> DustControlled : dustControlled
    DustControlled --> DebrisLoaded : debrisLoaded
    DebrisLoaded --> ProgressInspected : progressInspected
    ProgressInspected --> BreakingExecuted : breakingExecuted
    BreakingExecuted --> ReinforcementEncountered : reinforcementEncountered
    ReinforcementEncountered --> BreakingToolSelected : breakingToolSelected
    ProgressInspected --> [*]
```
