...
```mermaid
erDiagram
    MiningUnit ||--o{ ExtractionPlan : relates-to
    ExtractionPlan ||--o{ ProductionShift : relates-to
    ProductionShift ||--o{ HaulCycle : relates-to
    HaulCycle ||--o{ EquipmentInspection : relates-to
    EquipmentInspection ||--o{ GasReading : relates-to
```
...
stateDiagram-v2
    ...
    Pre Shift Inspection Completed --> Extraction Cycle Started : extractionCycleStarted
    Extraction Cycle Started --> Material Loaded : materialLoaded
    Material Loaded --> Material Hauled : materialHauled
    Material Hauled --> Atmosphere Monitored : atmosphereMonitored
    Atmosphere Monitored --> Equipment Advanced : equipmentAdvanced
    Equipment Advanced --> Hazard Detected : hazardDetected
    Material Hauled --> Material Loaded : revisionRequired
    Hazard Detected --> [*]
```
