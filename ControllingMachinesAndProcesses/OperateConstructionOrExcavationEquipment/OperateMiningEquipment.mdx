...
```mermaid
erDiagram
    ExtractionPlan ||--o{ ProductionShift : schedules
    ProductionShift ||--o{ MiningUnit : deploys
    ProductionShift ||--o{ HaulCycle : produces
    MiningUnit ||--o{ EquipmentInspection : undergoes
    ProductionShift ||--o{ GasReading : monitors
```
...
stateDiagram-v2
    ...
    PreShiftInspectionCompleted --> ExtractionCycleStarted : extractionCycleStarted
    ExtractionCycleStarted --> MaterialLoaded : materialLoaded
    MaterialLoaded --> MaterialHauled : materialHauled
    MaterialHauled --> AtmosphereMonitored : atmosphereMonitored
    AtmosphereMonitored --> EquipmentAdvanced : equipmentAdvanced
    EquipmentAdvanced --> ExtractionCycleStarted : extractionCycleStarted
    AtmosphereMonitored --> HazardDetected : hazardDetected
    HazardDetected --> PreShiftInspectionCompleted : preShiftInspectionCompleted
    EquipmentAdvanced --> [*]
```
