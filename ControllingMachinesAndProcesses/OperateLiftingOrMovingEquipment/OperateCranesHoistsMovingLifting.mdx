...
```mermaid
erDiagram
    Crane ||--o{ Hoist : relates-to
    Hoist ||--o{ LiftPlan : relates-to
    LiftPlan ||--o{ LoadChart : relates-to
    LoadChart ||--o{ RiggingHardware : relates-to
    RiggingHardware ||--o{ InspectionRecord : relates-to
```
...
stateDiagram-v2
    ...
    Pre Operation Inspection Completed --> Lift Plan Created : liftPlanCreated
    Lift Plan Created --> Load Rigged : loadRigged
    Load Rigged --> Lift Executed : liftExecuted
    Lift Executed --> Load Set : loadSet
    Load Set --> Lift Record Logged : liftRecordLogged
    Lift Record Logged --> Safety Issue Reported : safetyIssueReported
    Lift Executed --> Load Rigged : revisionRequired
