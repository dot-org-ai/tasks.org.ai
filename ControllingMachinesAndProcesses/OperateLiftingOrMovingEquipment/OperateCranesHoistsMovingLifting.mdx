...
```mermaid
erDiagram
    Crane ||--o{ LiftPlan : executes
    Crane ||--o{ Hoist : equippedWith
    LiftPlan ||--o{ LoadChart : references
    LiftPlan ||--o{ RiggingHardware : specifies
    Crane ||--o{ InspectionRecord : documents
```
...
```mermaid
stateDiagram-v2
    [*] --> PreOperationInspectionCompleted
    PreOperationInspectionCompleted --> LiftPlanCreated : liftPlanCreated
    LiftPlanCreated --> LoadRigged : loadRigged
    LoadRigged --> LiftExecuted : liftExecuted
    LiftExecuted --> LoadSet : loadSet
    LoadSet --> LiftRecordLogged : liftRecordLogged
    LiftRecordLogged --> LiftPlanCreated : liftPlanCreated
    LiftRecordLogged --> [*]
```
