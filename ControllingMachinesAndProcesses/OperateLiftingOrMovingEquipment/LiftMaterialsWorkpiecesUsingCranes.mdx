...
```mermaid
erDiagram
    LiftRequest ||--o{ Workpiece : specifies
    Workpiece ||--o{ LoadAssessment : evaluates
    OverheadCrane ||--o{ RiggingConfiguration : uses
    LiftRequest ||--o{ OverheadCrane : assignedTo
    LiftRequest ||--o{ LiftLog : documents
```
...
stateDiagram-v2
    [*] --> LoadAssessed
    LoadAssessed --> RiggingSelected : riggingSelected
    RiggingSelected --> RiggingAttached : riggingAttached
    RiggingAttached --> LoadHoisted : loadHoisted
    LoadHoisted --> LoadPositioned : loadPositioned
    LoadPositioned --> LoadLoweredAndReleased : loadLoweredAndReleased
    LoadLoweredAndReleased --> LiftLogged : liftLogged
    LoadPositioned --> RiggingAttached : loadPositioned
    LiftLogged --> [*]
```
