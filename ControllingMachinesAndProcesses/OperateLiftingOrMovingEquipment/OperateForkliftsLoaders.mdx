...
```mermaid
erDiagram
    Forklift ||--o{ LoadTicket : moves
    Loader ||--o{ LoadTicket : moves
    Forklift ||--o{ PreOperationChecklist : requires
    Forklift ||--o{ MaintenanceRecord : tracks
    OperatorCertification ||--o{ Forklift : authorizes
```
...
stateDiagram-v2
    ...
    PreOperationCheckCompleted --> LoadPickedUp : loadPickedUp
    LoadPickedUp --> LoadTransported : loadTransported
    LoadTransported --> LoadPlaced : loadPlaced
    LoadPlaced --> TruckLoaded : truckLoaded
    LoadPlaced --> LoadPickedUp : loadPickedUp
    TruckLoaded --> TruckUnloaded : truckUnloaded
    TruckUnloaded --> [*]
```
