...
```mermaid
erDiagram
    Equipment ||--o{ ServiceArea : relates-to
    ServiceArea ||--o{ MaintenanceTask : relates-to
    MaintenanceTask ||--o{ ServiceLog : relates-to
    ServiceLog ||--o{ FuelRecord : relates-to
    FuelRecord ||--o{ InspectionReport : relates-to
```
...
```mermaid
stateDiagram-v2
    [*] --> Service Area Assigned
    Service Area Assigned --> Maintenance Task Started : maintenanceTaskStarted
    Maintenance Task Started --> Maintenance Task Completed : maintenanceTaskCompleted
    Maintenance Task Completed --> Grounds Inspected : groundsInspected
    Grounds Inspected --> Fuel Usage Logged : fuelUsageLogged
    Fuel Usage Logged --> Equipment Service Scheduled : equipmentServiceScheduled
    Equipment Service Scheduled --> Equipment Failure Reported : equipmentFailureReported
    Grounds Inspected --> Maintenance Task Completed : revisionRequired
