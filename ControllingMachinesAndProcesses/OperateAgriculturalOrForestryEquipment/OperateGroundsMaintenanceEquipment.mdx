...
```mermaid
erDiagram
    Equipment ||--o{ MaintenanceTask : performs
    MaintenanceTask ||--o{ ServiceArea : covers
    MaintenanceTask ||--o{ ServiceLog : generates
    Equipment ||--o{ FuelRecord : consumes
    ServiceArea ||--o{ InspectionReport : evaluates
```
...
stateDiagram-v2
    [*] --> ServiceAreaAssigned
    ServiceAreaAssigned --> MaintenanceTaskStarted : maintenanceTaskStarted
    MaintenanceTaskStarted --> MaintenanceTaskCompleted : maintenanceTaskCompleted
    MaintenanceTaskCompleted --> GroundsInspected : groundsInspected
    GroundsInspected --> FuelUsageLogged : fuelUsageLogged
    FuelUsageLogged --> EquipmentServiceScheduled : equipmentServiceScheduled
    MaintenanceTaskCompleted --> EquipmentFailureReported : equipmentFailureReported
    EquipmentFailureReported --> EquipmentServiceScheduled : equipmentServiceScheduled
    EquipmentServiceScheduled --> ServiceAreaAssigned : serviceAreaAssigned
    FuelUsageLogged --> [*]
```
