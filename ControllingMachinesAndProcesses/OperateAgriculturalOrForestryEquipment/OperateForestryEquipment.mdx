...
```mermaid
erDiagram
    Equipment ||--o{ HarvestBlock : operates-in
    HarvestBlock ||--o{ HarvestPlan : follows
    HarvestBlock ||--o{ ProductionLog : generates
    ProductionLog ||--o{ TimberLoad : measures
    SafetyIncident ||--o{ HarvestBlock : occurs-in
```
...
stateDiagram-v2
    [*] --> EquipmentDeployed
    EquipmentDeployed --> TreesFelled : treesFelled
    TreesFelled --> LogsProcessed : logsProcessed
    LogsProcessed --> TimberSkidded : timberSkidded
    TimberSkidded --> TimberLoaded : timberLoaded
    TimberLoaded --> ProductionLogged : productionLogged
    ProductionLogged --> TreesFelled : treesFelled
    TimberSkidded --> LogsProcessed : logsProcessed
    ProductionLogged --> [*]
```
