...
```mermaid
erDiagram
    IrrigationZone ||--o{ Schedule : relates-to
    Schedule ||--o{ FlowRate : relates-to
    FlowRate ||--o{ SoilMoistureReading : relates-to
    SoilMoistureReading ||--o{ WaterAllocation : relates-to
    WaterAllocation ||--o{ MaintenanceRecord : relates-to
```
...
stateDiagram-v2
    [*] --> Irrigation Started
    Irrigation Started --> Irrigation Completed : irrigationCompleted
    Irrigation Completed --> Flow Rate Adjusted : flowRateAdjusted
    Flow Rate Adjusted --> Moisture Threshold Reached : moistureThresholdReached
    Moisture Threshold Reached --> Schedule Updated : scheduleUpdated
    Schedule Updated --> Maintenance Performed : maintenancePerformed
    Maintenance Performed --> Water Allocation Exceeded : waterAllocationExceeded
    Moisture Threshold Reached --> Flow Rate Adjusted : revisionRequired
