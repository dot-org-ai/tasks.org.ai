...
```mermaid
erDiagram
    IrrigationZone ||--o{ Schedule : follows
    IrrigationZone ||--o{ SoilMoistureReading : monitors
    Schedule ||--o{ FlowRate : governs
    IrrigationZone ||--o{ WaterAllocation : draws-from
    IrrigationZone ||--o{ MaintenanceRecord : tracks
```
...
stateDiagram-v2
    [*] --> ScheduleUpdated
    ScheduleUpdated --> IrrigationStarted : irrigationStarted
    IrrigationStarted --> FlowRateAdjusted : flowRateAdjusted
    FlowRateAdjusted --> IrrigationCompleted : irrigationCompleted
    IrrigationCompleted --> MoistureThresholdReached : moistureThresholdReached
    MoistureThresholdReached --> IrrigationStarted : irrigationStarted
    IrrigationCompleted --> MaintenancePerformed : maintenancePerformed
    MaintenancePerformed --> ScheduleUpdated : scheduleUpdated
    IrrigationStarted --> WaterAllocationExceeded : waterAllocationExceeded
    WaterAllocationExceeded --> [*]
```
