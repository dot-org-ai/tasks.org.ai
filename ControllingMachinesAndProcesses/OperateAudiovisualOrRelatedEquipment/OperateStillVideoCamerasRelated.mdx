...
```mermaid
erDiagram
    Camera ||--o{ Lens : relates-to
    Lens ||--o{ MediaCard : relates-to
    MediaCard ||--o{ ShootSession : relates-to
    ShootSession ||--o{ MediaAsset : relates-to
    MediaAsset ||--o{ ShotList : relates-to
```
...
stateDiagram-v2
    [*] --> Camera Configured
    Camera Configured --> Image Captured : imageCaptured
    Image Captured --> Video Recording Started : videoRecordingStarted
    Video Recording Started --> Video Recording Stopped : videoRecordingStopped
    Video Recording Stopped --> Lens Changed : lensChanged
    Lens Changed --> Media Ingested : mediaIngested
    Media Ingested --> Captures Reviewed : capturesReviewed
    Video Recording Stopped --> Video Recording Started : revisionRequired
    Captures Reviewed --> [*]
```
