...
```mermaid
erDiagram
    AVSystem ||--o{ MediaAsset : relates-to
    MediaAsset ||--o{ RoomBooking : relates-to
    RoomBooking ||--o{ PlaybackQueue : relates-to
    PlaybackQueue ||--o{ ConnectionProfile : relates-to
    ConnectionProfile ||--o{ UsageRecord : relates-to
```
...
stateDiagram-v2
    [*] --> System Initialized
    System Initialized --> Media Loaded : mediaLoaded
    Media Loaded --> Playback Started : playbackStarted
    Playback Started --> Source Switch : sourceSwitch
    Source Switch --> Output Adjusted : outputAdjusted
    Output Adjusted --> Playback Ended : playbackEnded
    Playback Ended --> Connection Lost : connectionLost
    Source Switch --> Playback Started : revisionRequired
    Connection Lost --> [*]
```
