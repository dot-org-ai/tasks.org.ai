...
```mermaid
erDiagram
    AVSystem ||--o{ PlaybackQueue : plays
    PlaybackQueue ||--o{ MediaAsset : sequences
    AVSystem ||--o{ ConnectionProfile : applies
    RoomBooking ||--o{ AVSystem : reserves
    AVSystem ||--o{ UsageRecord : logs
```
...
stateDiagram-v2
    [*] --> SystemInitialized
    SystemInitialized --> MediaLoaded : mediaLoaded
    MediaLoaded --> PlaybackStarted : playbackStarted
    PlaybackStarted --> SourceSwitched : sourceSwitch
    SourceSwitched --> OutputAdjusted : outputAdjusted
    PlaybackStarted --> OutputAdjusted : outputAdjusted
    OutputAdjusted --> PlaybackEnded : playbackEnded
    PlaybackEnded --> MediaLoaded : mediaLoaded
    PlaybackStarted --> ConnectionLost : connectionLost
    ConnectionLost --> SystemInitialized : systemInitialized
    PlaybackEnded --> [*]
```
