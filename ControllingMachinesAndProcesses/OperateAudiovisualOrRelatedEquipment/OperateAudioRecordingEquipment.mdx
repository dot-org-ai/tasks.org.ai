...
```mermaid
erDiagram
    RecordingSession ||--o{ SignalChain : relates-to
    SignalChain ||--o{ AudioTrack : relates-to
    AudioTrack ||--o{ MicrophonePlacement : relates-to
    MicrophonePlacement ||--o{ RecordingProject : relates-to
    RecordingProject ||--o{ EquipmentPreset : relates-to
```
...
stateDiagram-v2
    ...
    Session Setup Completed --> Levels Set : levelsSet
    Levels Set --> Recording Started : recordingStarted
    Recording Started --> Recording Stopped : recordingStopped
    Recording Stopped --> Take Approved : takeApproved
    Take Approved --> Clipping Detected : clippingDetected
    Clipping Detected --> Session Archived : sessionArchived
    Recording Stopped --> Recording Started : revisionRequired
    Session Archived --> [*]
```
