...
```mermaid
erDiagram
    RecordingProject ||--o{ RecordingSession : organizes
    RecordingSession ||--o{ SignalChain : configures
    SignalChain ||--o{ MicrophonePlacement : positions
    SignalChain ||--o{ AudioTrack : captures
    RecordingSession ||--o{ EquipmentPreset : loads
```
...
stateDiagram-v2
    ...
    SessionSetupCompleted --> LevelsSet : levelsSet
    LevelsSet --> RecordingStarted : recordingStarted
    RecordingStarted --> RecordingStopped : recordingStopped
    RecordingStarted --> ClippingDetected : clippingDetected
    ClippingDetected --> LevelsSet : levelsSet
    RecordingStopped --> TakeApproved : takeApproved
    RecordingStopped --> RecordingStarted : recordingStarted
    TakeApproved --> SessionArchived : sessionArchived
    SessionArchived --> [*]
```
