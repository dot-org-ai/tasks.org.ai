...
```mermaid
erDiagram
    Equipment ||--o{ AVSetup : relates-to
    AVSetup ||--o{ SignalRoute : relates-to
    SignalRoute ||--o{ SessionLog : relates-to
    SessionLog ||--o{ Inventory : relates-to
    Inventory ||--o{ TroubleTicket : relates-to
```
...
stateDiagram-v2
    [*] --> Setup Configured
    Setup Configured --> Signal Routed : signalRouted
    Signal Routed --> Session Started : sessionStarted
    Session Started --> Session Ended : sessionEnded
    Session Ended --> Levels Adjusted : levelsAdjusted
    Levels Adjusted --> Issue Detected : issueDetected
    Issue Detected --> Issue Resolved : issueResolved
    Session Ended --> Session Started : revisionRequired
    Issue Resolved --> [*]
```
