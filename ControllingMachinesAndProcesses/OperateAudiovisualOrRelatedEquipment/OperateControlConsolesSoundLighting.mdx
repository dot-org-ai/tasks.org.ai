...
```mermaid
erDiagram
    Console ||--o{ CueList : relates-to
    CueList ||--o{ Cue : relates-to
    Cue ||--o{ ShowFile : relates-to
    ShowFile ||--o{ Channel : relates-to
    Channel ||--o{ Scene : relates-to
```
...
stateDiagram-v2
    [*] --> Show File Loaded
    Show File Loaded --> Cue Programmed : cueProgrammed
    Cue Programmed --> Cue Executed : cueExecuted
    Cue Executed --> Feed Switched : feedSwitched
    Feed Switched --> Channel Patched : channelPatched
    Channel Patched --> Show File Saved : showFileSaved
    Show File Saved --> Console Error : consoleError
    Feed Switched --> Cue Executed : revisionRequired
    Console Error --> [*]
```
