...
```mermaid
erDiagram
    Camera ||--o{ MonitoringZone : covers
    MonitoringZone ||--o{ Alert : triggers
    Camera ||--o{ Footage : records
    Alert ||--o{ IncidentReport : documents
    IncidentReport ||--o{ EvidencePackage : compiles
```
...
stateDiagram-v2
    [*] --> CameraConfigured
    CameraConfigured --> MotionDetected : motionDetected
    MotionDetected --> AlertTriggered : alertTriggered
    AlertTriggered --> AlertAcknowledged : alertAcknowledged
    AlertAcknowledged --> SuspiciousActivityIdentified : suspiciousActivityIdentified
    AlertAcknowledged --> CameraConfigured : alertAcknowledged
    SuspiciousActivityIdentified --> IncidentReportCreated : incidentReportCreated
    IncidentReportCreated --> EvidenceExported : evidenceExported
    EvidenceExported --> CameraConfigured : cameraConfigured
    EvidenceExported --> [*]
```
