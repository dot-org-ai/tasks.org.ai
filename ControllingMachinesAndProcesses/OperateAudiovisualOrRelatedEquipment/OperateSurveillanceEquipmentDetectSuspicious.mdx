...
```mermaid
erDiagram
    Camera ||--o{ MonitoringZone : relates-to
    MonitoringZone ||--o{ Alert : relates-to
    Alert ||--o{ Footage : relates-to
    Footage ||--o{ IncidentReport : relates-to
    IncidentReport ||--o{ EvidencePackage : relates-to
```
...
stateDiagram-v2
    [*] --> Camera Configured
    Camera Configured --> Motion Detected : motionDetected
    Motion Detected --> Alert Triggered : alertTriggered
    Alert Triggered --> Alert Acknowledged : alertAcknowledged
    Alert Acknowledged --> Suspicious Activity Identified : suspiciousActivityIdentified
    Suspicious Activity Identified --> Incident Report Created : incidentReportCreated
    Incident Report Created --> Evidence Exported : evidenceExported
    Alert Acknowledged --> Alert Triggered : revisionRequired
    Evidence Exported --> [*]
```
