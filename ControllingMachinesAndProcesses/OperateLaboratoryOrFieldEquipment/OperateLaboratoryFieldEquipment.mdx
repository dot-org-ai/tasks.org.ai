...
```mermaid
erDiagram
    Instrument ||--o{ CalibrationRecord : calibratedBy
    Instrument ||--o{ Sample : analyzes
    Sample ||--o{ TestProtocol : follows
    TestProtocol ||--o{ DataRecord : produces
    Instrument ||--o{ MaintenanceLog : tracks
```
...
stateDiagram-v2
    ...
    InstrumentCalibrated --> SamplePrepared : samplePrepared
    SamplePrepared --> TestExecuted : testExecuted
    TestExecuted --> DataRecorded : dataRecorded
    DataRecorded --> ResultsValidated : resultsValidated
    ResultsValidated --> MaintenancePerformed : maintenancePerformed
    ResultsValidated --> SamplePrepared : resultsValidated
