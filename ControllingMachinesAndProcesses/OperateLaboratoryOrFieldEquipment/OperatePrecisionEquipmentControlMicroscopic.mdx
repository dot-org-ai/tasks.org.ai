...
```mermaid
erDiagram
    ProcessRecipe ||--o{ Substrate : relates-to
    Substrate ||--o{ CharacterizationResult : relates-to
    CharacterizationResult ||--o{ EnvironmentLog : relates-to
    EnvironmentLog ||--o{ FabricationRun : relates-to
    FabricationRun ||--o{ InstrumentConfiguration : relates-to
```
...
stateDiagram-v2
    ...
    Instrument Configured --> Environment Conditioned : environmentConditioned
    Environment Conditioned --> Substrate Loaded : substrateLoaded
    Substrate Loaded --> Process Executed : processExecuted
    Process Executed --> Characterization Captured : characterizationCaptured
    Characterization Captured --> Outcome Validated : outcomeValidated
    Outcome Validated --> Recipes Saved : recipesSaved
    Process Executed --> Substrate Loaded : revisionRequired
    Recipes Saved --> [*]
```
