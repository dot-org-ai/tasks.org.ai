...
```mermaid
erDiagram
    ProcessRecipe ||--o{ Substrate : targets
    FabricationRun ||--o{ ProcessRecipe : follows
    FabricationRun ||--o{ CharacterizationResult : produces
    FabricationRun ||--o{ EnvironmentLog : records
    FabricationRun ||--o{ InstrumentConfiguration : uses
```
...
stateDiagram-v2
    ...
    InstrumentConfigured --> EnvironmentConditioned : environmentConditioned
    EnvironmentConditioned --> SubstrateLoaded : substrateLoaded
    SubstrateLoaded --> ProcessExecuted : processExecuted
    ProcessExecuted --> CharacterizationCaptured : characterizationCaptured
    CharacterizationCaptured --> OutcomeValidated : outcomeValidated
    OutcomeValidated --> RecipesSaved : recipesSaved
    OutcomeValidated --> SubstrateLoaded : outcomeValidated
    RecipesSaved --> [*]
```
