...
```mermaid
erDiagram
    Specimen ||--o{ TestOrder : orderedBy
    TestOrder ||--o{ AnalyzerRun : processedIn
    AnalyzerRun ||--o{ TestResult : produces
    AnalyzerRun ||--o{ QualityControl : validates
    TestResult ||--o{ CriticalValue : flags
```
...
stateDiagram-v2
    [*] --> SpecimenAccessioned
    SpecimenAccessioned --> AnalyzerLoaded : analyzerLoaded
    AnalyzerLoaded --> AnalysisCompleted : analysisCompleted
    AnalysisCompleted --> ResultsReviewed : resultsReviewed
    ResultsReviewed --> CriticalValueFlagged : criticalValueFlagged
    ResultsReviewed --> ReportReleased : reportReleased
    CriticalValueFlagged --> ReportReleased : reportReleased
    ResultsReviewed --> AnalyzerLoaded : resultsReviewed
    ReportReleased --> [*]
```
