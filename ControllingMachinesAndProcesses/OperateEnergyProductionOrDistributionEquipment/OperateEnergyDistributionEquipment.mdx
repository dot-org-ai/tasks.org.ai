...
```mermaid
erDiagram
    Substation ||--o{ Transformer : relates-to
    Transformer ||--o{ CircuitBreaker : relates-to
    CircuitBreaker ||--o{ DistributionFeeder : relates-to
    DistributionFeeder ||--o{ VoltageRegulator : relates-to
    VoltageRegulator ||--o{ OutageRecord : relates-to
```
...
stateDiagram-v2
    [*] --> Load Switched
    Load Switched --> Voltage Regulated : voltageRegulated
    Voltage Regulated --> Fault Isolated : faultIsolated
    Fault Isolated --> Service Restored : serviceRestored
    Service Restored --> Substation Alarm Triggered : substationAlarmTriggered
    Substation Alarm Triggered --> Field Crew Dispatched : fieldCrewDispatched
    Field Crew Dispatched --> Switching Operation Logged : switchingOperationLogged
    Service Restored --> Fault Isolated : revisionRequired
    Switching Operation Logged --> [*]
```
