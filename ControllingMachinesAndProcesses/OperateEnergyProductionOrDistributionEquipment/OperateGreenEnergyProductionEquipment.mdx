...
```mermaid
erDiagram
    WindTurbine ||--o{ Inverter : feeds
    SolarArray ||--o{ Inverter : feeds
    ResourceForecast ||--o{ WindTurbine : predicts-for
    Inverter ||--o{ GenerationMeter : exports-through
    GenerationMeter ||--o{ CurtailmentOrder : limits
```
...
stateDiagram-v2
    [*] --> ResourceAssessed
    ResourceAssessed --> GenerationStarted : generationStarted
    GenerationStarted --> GridConnectionManaged : gridConnectionManaged
    GridConnectionManaged --> OutputMonitored : outputMonitored
    OutputMonitored --> PerformanceOptimized : performanceOptimized
    PerformanceOptimized --> OutputMonitored : outputMonitored
    OutputMonitored --> GenerationCurtailed : generationCurtailed
    GenerationCurtailed --> OutputMonitored : outputMonitored
    OutputMonitored --> ProductionDataRecorded : productionDataRecorded
    ProductionDataRecorded --> [*]
```
