...
```mermaid
erDiagram
    Boiler ||--o{ Turbine : drives
    Turbine ||--o{ Generator : spins
    Generator ||--o{ DispatchSchedule : follows
    Generator ||--o{ HeatRate : measures
    Generator ||--o{ EmissionsPermit : operates-under
```
...
stateDiagram-v2
    [*] --> PlantStarted
    PlantStarted --> GeneratorLoaded : generatorLoaded
    GeneratorLoaded --> EfficiencyOptimized : efficiencyOptimized
    EfficiencyOptimized --> DispatchResponseCompleted : dispatchResponseCompleted
    DispatchResponseCompleted --> GeneratorLoaded : generatorLoaded
    EfficiencyOptimized --> SystemAlarmRaised : systemAlarmRaised
    SystemAlarmRaised --> EfficiencyOptimized : efficiencyOptimized
    EfficiencyOptimized --> EmissionsLimitReached : emissionsLimitReached
    EmissionsLimitReached --> EfficiencyOptimized : efficiencyOptimized
    GeneratorLoaded --> PlantShutdown : plantShutdown
    PlantShutdown --> [*]
```
