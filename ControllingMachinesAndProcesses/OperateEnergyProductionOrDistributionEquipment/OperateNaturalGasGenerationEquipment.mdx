...
```mermaid
erDiagram
    GasTurbine ||--o{ HeatRecoverySteamGenerator : exhausts-to
    GasTurbine ||--o{ FuelSupplyContract : consumes-under
    DispatchOrder ||--o{ GasTurbine : dispatches
    GasTurbine ||--o{ EmissionsRecord : produces
    GasTurbine ||--o{ GenerationOutput : delivers
```
...
stateDiagram-v2
    [*] --> TurbineStarted
    TurbineStarted --> GeneratorLoaded : generatorLoaded
    GeneratorLoaded --> DispatchOrderReceived : dispatchOrderReceived
    DispatchOrderReceived --> FuelFlowAdjusted : fuelFlowAdjusted
    FuelFlowAdjusted --> GeneratorLoaded : generatorLoaded
    GeneratorLoaded --> EmissionsLimitApproached : emissionsLimitApproached
    EmissionsLimitApproached --> FuelFlowAdjusted : fuelFlowAdjusted
    GeneratorLoaded --> TurbineShutdown : turbineShutdown
    TurbineShutdown --> GenerationDataLogged : generationDataLogged
    GenerationDataLogged --> [*]
```
