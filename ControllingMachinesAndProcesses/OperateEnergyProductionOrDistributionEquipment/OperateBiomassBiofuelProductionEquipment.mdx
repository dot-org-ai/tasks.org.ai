...
```mermaid
erDiagram
    Feedstock ||--o{ ConversionUnit : feeds
    ConversionUnit ||--o{ BiofuelProduct : yields
    ConversionUnit ||--o{ Byproduct : generates
    ConversionUnit ||--o{ ProcessParameter : governs
    BiofuelProduct ||--o{ QualitySample : validates
```
...
stateDiagram-v2
    [*] --> FeedstockReceived
    FeedstockReceived --> ConversionProcessStarted : conversionProcessStarted
    ConversionProcessStarted --> ParametersMonitored : parametersMonitored
    ParametersMonitored --> ProcessConditionsAdjusted : processConditionsAdjusted
    ProcessConditionsAdjusted --> ParametersMonitored : parametersMonitored
    ParametersMonitored --> ProductRecovered : productRecovered
    ProductRecovered --> QualityTestCompleted : qualityTestCompleted
    QualityTestCompleted --> ByproductsManaged : byproductsManaged
    ByproductsManaged --> FeedstockReceived : feedstockReceived
    ByproductsManaged --> [*]
```
