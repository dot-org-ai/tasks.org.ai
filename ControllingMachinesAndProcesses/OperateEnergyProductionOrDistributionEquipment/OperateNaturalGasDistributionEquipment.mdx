...
```mermaid
erDiagram
    PressureRegulatorStation ||--o{ DistributionMain : feeds
    DistributionMain ||--o{ GasMeter : delivers-to
    DistributionMain ||--o{ OdorizationSystem : injects-via
    DistributionMain ||--o{ LeakReport : surveys
    PressureRegulatorStation ||--o{ PressureReading : monitors
```
...
stateDiagram-v2
    [*] --> PressureRegulated
    PressureRegulated --> FlowMonitored : flowMonitored
    FlowMonitored --> LeakDetected : leakDetected
    LeakDetected --> LeakRemediated : leakRemediated
    LeakRemediated --> FlowMonitored : flowMonitored
    FlowMonitored --> MeterCalibrated : meterCalibrated
    FlowMonitored --> OdorizationVerified : odorizationVerified
    OdorizationVerified --> DistributionDataLogged : distributionDataLogged
    DistributionDataLogged --> PressureRegulated : pressureRegulated
    DistributionDataLogged --> [*]
```
