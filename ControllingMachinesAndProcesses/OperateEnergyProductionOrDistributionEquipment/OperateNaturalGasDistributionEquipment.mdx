...
```mermaid
erDiagram
    PressureRegulatorStation ||--o{ DistributionMain : relates-to
    DistributionMain ||--o{ GasMeter : relates-to
    GasMeter ||--o{ OdorizationSystem : relates-to
    OdorizationSystem ||--o{ LeakReport : relates-to
    LeakReport ||--o{ PressureReading : relates-to
```
...
stateDiagram-v2
    [*] --> Pressure Regulated
    Pressure Regulated --> Flow Monitored : flowMonitored
    Flow Monitored --> Leak Detected : leakDetected
    Leak Detected --> Leak Remediated : leakRemediated
    Leak Remediated --> Meter Calibrated : meterCalibrated
    Meter Calibrated --> Odorization Verified : odorizationVerified
    Odorization Verified --> Distribution Data Logged : distributionDataLogged
    Leak Remediated --> Leak Detected : revisionRequired
