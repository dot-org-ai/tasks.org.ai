...
```mermaid
erDiagram
    CuttingMachine ||--o{ FabricRoll : relates-to
    FabricRoll ||--o{ CuttingMarker : relates-to
    CuttingMarker ||--o{ CutPiece : relates-to
    CutPiece ||--o{ ProductionOrder : relates-to
    ProductionOrder ||--o{ SpreadingTable : relates-to
```
...
stateDiagram-v2
    [*] --> Fabric Spread
    Fabric Spread --> Marker Loaded : markerLoaded
    Marker Loaded --> Cut Executed : cutExecuted
    Cut Executed --> Cut Pieces Inspected : cutPiecesInspected
    Cut Pieces Inspected --> Pieces Bundled : piecesBundled
    Pieces Bundled --> Machine Adjusted : machineAdjusted
    Machine Adjusted --> Production Run Completed : productionRunCompleted
    Cut Pieces Inspected --> Cut Executed : revisionRequired
