...
```mermaid
erDiagram
    GrindingMachine ||--o{ GrindingWheel : relates-to
    GrindingWheel ||--o{ Workpiece : relates-to
    Workpiece ||--o{ WorkOrder : relates-to
    WorkOrder ||--o{ MachineSetup : relates-to
    MachineSetup ||--o{ InspectionRecord : relates-to
```
...
```mermaid
stateDiagram-v2
    [*] --> Grinder Setup Completed
    Grinder Setup Completed --> Grinding Cycle Started : grindingCycleStarted
    Grinding Cycle Started --> Workpiece Measured : workpieceMeasured
    Workpiece Measured --> Wheel Dressed : wheelDressed
    Wheel Dressed --> Parameters Adjusted : parametersAdjusted
    Parameters Adjusted --> Operation Completed : operationCompleted
    Operation Completed --> Quality Deviation Detected : qualityDeviationDetected
    Wheel Dressed --> Workpiece Measured : revisionRequired
