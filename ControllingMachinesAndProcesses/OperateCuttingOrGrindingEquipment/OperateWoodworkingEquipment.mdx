...
```mermaid
erDiagram
    WoodworkingMachine ||--o{ Workpiece : relates-to
    Workpiece ||--o{ CutList : relates-to
    CutList ||--o{ ToolingSet : relates-to
    ToolingSet ||--o{ ProductionOrder : relates-to
    ProductionOrder ||--o{ DustCollection : relates-to
```
...
stateDiagram-v2
    ...
    Machine Setup Completed --> Material Loaded : materialLoaded
    Material Loaded --> Cut Executed : cutExecuted
    Cut Executed --> Piece Inspected : pieceInspected
    Piece Inspected --> Blade Changed : bladeChanged
    Blade Changed --> Production Completed : productionCompleted
    Production Completed --> Defect Detected : defectDetected
    Piece Inspected --> Cut Executed : revisionRequired
    Defect Detected --> [*]
```
