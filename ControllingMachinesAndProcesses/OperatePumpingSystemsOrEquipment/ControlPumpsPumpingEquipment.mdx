...
```mermaid
erDiagram
    ControlSetpoint ||--o{ VariableFrequencyDrive : relates-to
    VariableFrequencyDrive ||--o{ ControlValve : relates-to
    ControlValve ||--o{ ProcessVariable : relates-to
    ProcessVariable ||--o{ ControlLoop : relates-to
    ControlLoop ||--o{ AlarmSetpoint : relates-to
```
...
stateDiagram-v2
    [*] --> Setpoint Changed
    Setpoint Changed --> Control Loop Tuned : controlLoopTuned
    Control Loop Tuned --> Pump Load Balanced : pumpLoadBalanced
    Pump Load Balanced --> Auto Control Overridden : autoControlOverridden
    Auto Control Overridden --> Alarm Acknowledged : alarmAcknowledged
    Alarm Acknowledged --> Auto Control Restored : autoControlRestored
    Auto Control Restored --> Sensor Calibrated : sensorCalibrated
    Auto Control Overridden --> Pump Load Balanced : revisionRequired
    Sensor Calibrated --> [*]
```
