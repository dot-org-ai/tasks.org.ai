...
```mermaid
erDiagram
    PumpUnit ||--o{ FlowRate : produces
    PumpUnit ||--o{ PressureReading : monitors
    PumpUnit ||--o{ MaintenanceRecord : has
    OperatingSchedule ||--o{ PumpUnit : schedules
    PumpUnit ||--o{ AlarmCondition : triggers
    FlowRate ||--o{ AlarmCondition : checks
    PressureReading ||--o{ AlarmCondition : checks
```
...
stateDiagram-v2
    [*] --> Offline
    Offline --> Starting : pumpSystemStarted
    Starting --> Running : flowRateAdjusted
    Running --> Running : pressureThresholdExceeded
    Running --> Switching : pumpUnitSwitched
    Switching --> Running : operatingDataLogged
    Running --> Shutdown : pumpSystemShutdown
    Shutdown --> [*]
```
