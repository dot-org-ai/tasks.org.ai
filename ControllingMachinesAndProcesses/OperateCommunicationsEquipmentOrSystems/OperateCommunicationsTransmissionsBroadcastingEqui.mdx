...
```mermaid
erDiagram
    Transmitter ||--o{ BroadcastSignal : amplifies
    Transmitter ||--o{ UplinkConnection : feeds-from
    TransmissionSchedule ||--o{ BroadcastSignal : schedules
    Transmitter ||--o{ SignalLog : logs
    Transmitter ||--o{ BroadcastLicense : authorizes
```
...
```mermaid
stateDiagram-v2
    [*] --> TransmitterCalibrated
    TransmitterCalibrated --> BroadcastInitiated : broadcastInitiated
    BroadcastInitiated --> TransmissionMonitored : transmissionMonitored
    TransmissionMonitored --> SignalParametersAdjusted : signalParametersAdjusted
    SignalParametersAdjusted --> TransmissionMonitored : transmissionMonitored
    TransmissionMonitored --> SignalSourceSwitched : signalSourceSwitched
    SignalSourceSwitched --> TransmissionMonitored : transmissionMonitored
    TransmissionMonitored --> BroadcastTerminated : broadcastTerminated
    BroadcastTerminated --> TransmissionEventLogged : transmissionEventLogged
    TransmissionEventLogged --> [*]
```
