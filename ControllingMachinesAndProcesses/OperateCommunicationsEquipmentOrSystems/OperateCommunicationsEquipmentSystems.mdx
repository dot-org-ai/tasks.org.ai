...
```mermaid
erDiagram
    CommunicationsSystem ||--o{ RadioUnit : includes
    CommunicationsSystem ||--o{ NetworkNode : routes-through
    NetworkNode ||--o{ Channel : allocates
    CommunicationsSystem ||--o{ ConfigurationProfile : applies
    CommunicationsSystem ||--o{ OperationsLog : records
```
...
stateDiagram-v2
    [*] --> EquipmentConfigured
    EquipmentConfigured --> SystemActivated : systemActivated
    SystemActivated --> SignalQualityMonitored : signalQualityMonitored
    SignalQualityMonitored --> CommunicationsRouted : communicationsRouted
    CommunicationsRouted --> SignalQualityMonitored : signalQualityMonitored
    SignalQualityMonitored --> IssueResolved : issueResolved
    IssueResolved --> MaintenancePerformed : maintenancePerformed
    MaintenancePerformed --> EquipmentConfigured : equipmentConfigured
    CommunicationsRouted --> SystemDeactivated : systemDeactivated
    SystemDeactivated --> [*]
```
