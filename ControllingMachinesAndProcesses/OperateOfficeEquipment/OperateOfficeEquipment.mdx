...
```mermaid
erDiagram
    OfficeDevice ||--o{ PrintJob : processes
    OfficeDevice ||--o{ ScanJob : processes
    OfficeDevice ||--o{ SupplyInventory : consumes
    OfficeDevice ||--o{ ServiceTicket : generates
    OfficeDevice ||--o{ UsageLog : records
```
...
stateDiagram-v2
    [*] --> DeviceConfigured
    DeviceConfigured --> PrintJobCompleted : printJobCompleted
    DeviceConfigured --> ScanJobCompleted : scanJobCompleted
    PrintJobCompleted --> SuppliesReplenished : suppliesReplenished
    ScanJobCompleted --> SuppliesReplenished : suppliesReplenished
    SuppliesReplenished --> SupplyLevelLow : supplyLevelLow
    SupplyLevelLow --> DeviceConfigured : suppliesReplenished
    SuppliesReplenished --> [*]
```
