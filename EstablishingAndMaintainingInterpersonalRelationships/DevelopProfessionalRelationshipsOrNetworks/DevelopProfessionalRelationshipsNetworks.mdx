...
```mermaid
erDiagram
    Contact ||--o{ Relationship : "contains"
    Relationship }o--|| Interaction : "tracks"
    Interaction ||--o{ NetworkGroup : "references"
    NetworkGroup }o--|| Referral : "generates"
    Referral ||--o{ Event : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : contactAdded
    InProgress --> InProgress : interactionLogged
    InProgress --> InProgress : relationshipCategorized
    InProgress --> Review : followUpScheduled
    Review --> InProgress
    InProgress --> InProgress : referralRequested
    InProgress --> InProgress : networkGroupJoined
    InProgress --> Completed : networkHealthAssessed
    Completed --> [*]
```
## Workflow
