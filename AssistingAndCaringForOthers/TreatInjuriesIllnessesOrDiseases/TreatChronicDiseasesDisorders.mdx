...
```mermaid
erDiagram
    CarePlan ||--|{ MedicationRegimen : prescribes
    CarePlan ||--|| LifestylePlan : includes
    CarePlan ||--|{ PatientGoal : defines
    DiseaseAssessment ||--|| CarePlan : informs
    DiseaseAssessment ||--|{ LabResult : "supported by"
    PatientGoal }o--|| LabResult : "measured by"
```
...
stateDiagram-v2
    ...
    PlanCreated --> OnMedication : medicationPrescribed
    OnMedication --> LifestyleModified : lifestylePlanUpdated
    LifestyleModified --> GoalsSet : patientGoalSet
    GoalsSet --> InFollowUp : followUpConducted
    InFollowUp --> LabsReviewed : labResultsReviewed
    LabsReviewed --> GoalsAchieved : goalsMet
    LabsReviewed --> Assessed : goalsNotMet
    GoalsAchieved --> InFollowUp : continuedMonitoring
```
