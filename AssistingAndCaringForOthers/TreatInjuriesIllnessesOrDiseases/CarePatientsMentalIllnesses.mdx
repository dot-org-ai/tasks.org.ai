...
```mermaid
erDiagram
    PsychiatricAssessment ||--|| TreatmentPlan : creates
    TreatmentPlan ||--|{ TherapySession : schedules
    TreatmentPlan ||--|{ MedicationRecord : prescribes
    TreatmentPlan ||--|| SafetyPlan : includes
    TherapySession ||--o| CrisisEvent : "may trigger"
    CrisisEvent }o--|| SafetyPlan : "updates"
```
...
stateDiagram-v2
    [*] --> Assessed
    Assessed --> PlanCreated : treatmentPlanCreated
    PlanCreated --> SafetyPlanned : safetyPlanUpdated
    SafetyPlanned --> Medicated : medicationPrescribed
    Medicated --> InTherapy : therapyDelivered
    InTherapy --> CrisisIntervention : crisisManaged
    CrisisIntervention --> InTherapy : stabilized
    InTherapy --> GoalsMet : goalsAchieved
    InTherapy --> InTherapy : ongoingTherapy
    GoalsMet --> Transitioned : careTransitionCoordinated
    Transitioned --> [*]
```
