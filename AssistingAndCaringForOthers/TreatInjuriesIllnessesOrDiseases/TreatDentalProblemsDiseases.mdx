...
```mermaid
erDiagram
    DentalExam ||--|| TreatmentPlan : creates
    DentalExam ||--|{ Radiograph : includes
    TreatmentPlan ||--|{ Restoration : schedules
    TreatmentPlan ||--o{ Prescription : "may include"
    DentalExam ||--|| PeriodontalRecord : documents
    Restoration }o--|| TreatmentPlan : "part of"
```
...
stateDiagram-v2
    [*] --> Examined
    Examined --> PlanCreated : treatmentPlanCreated
    PlanCreated --> Restoring : restorationPerformed
    PlanCreated --> Extracting : extractionPerformed
    PlanCreated --> PerioTreating : periodontalDiseaseTreated
    Restoring --> Medicated : medicationPrescribed
    Extracting --> Medicated : medicationPrescribed
    PerioTreating --> Medicated : medicationPrescribed
    Medicated --> ProstheticOrdered : prostheticOrdered
    Medicated --> FollowUp : scheduled
    ProstheticOrdered --> FollowUp : prostheticReady
    FollowUp --> [*]
```
...
graph TD
    ...
    E --> G
    F --> G
    G --> H{Prosthetic Needed?}
    H -->|Yes| I[orderProsthetic]
    H -->|No| J[Schedule Follow-Up]
    I --> J
```
