...
```mermaid
erDiagram
    PatientIntake ||--|| TreatmentPlan : creates
    TreatmentPlan ||--|{ TherapySession : schedules
    TreatmentPlan ||--o{ HerbalFormulation : "may include"
    TherapySession ||--|| Modality : uses
    TherapySession ||--|| OutcomeAssessment : "evaluated by"
    OutcomeAssessment }o--|| TreatmentPlan : "may adjust"
```
...
stateDiagram-v2
    [*] --> IntakeComplete
    IntakeComplete --> PlanCreated : treatmentPlanCreated
    PlanCreated --> InTherapy : therapyAdministered
    InTherapy --> Documented : sessionDocumented
    Documented --> Assessed : outcomeAssessed
    Assessed --> InTherapy : continueTherapy
    Assessed --> PlanAdjusted : treatmentPlanAdjusted
    PlanAdjusted --> InTherapy : therapyAdministered
    Assessed --> Complete : goalsAchieved
    Complete --> [*]
```
...
graph TD
    ...
    B --> C{Herbal Support?}
    C -->|Yes| D[prescribeFormulation]
    C -->|No| E[administerTherapy]
    D --> E
    E --> F[documentSession]
    F --> G[assessOutcome]
    G --> H{Effective?}
    H -->|Yes| I[Continue Treatment]
    H -->|No| J[adjustTreatmentPlan]
    J --> E
```
