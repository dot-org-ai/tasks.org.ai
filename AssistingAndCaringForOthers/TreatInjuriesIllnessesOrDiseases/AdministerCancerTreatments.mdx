...
```mermaid
erDiagram
    TreatmentRegimen ||--|{ TreatmentCycle : contains
    TreatmentCycle ||--|{ InfusionSession : delivers
    TreatmentRegimen ||--o| RadiationPlan : "may include"
    InfusionSession ||--o{ ToxicityReport : "may generate"
    TreatmentCycle ||--|| TumorAssessment : "evaluated by"
    ToxicityReport }o--|| TreatmentRegimen : "may adjust"
```
...
stateDiagram-v2
    [*] --> RegimenPrescribed
    RegimenPrescribed --> InfusionReady : infusionPrepared
    InfusionReady --> Administered : infusionAdministered
    Administered --> ToxicityChecked : toxicityMonitored
    ToxicityChecked --> ResponseAssessed : tumorResponseAssessed
    ToxicityChecked --> RegimenAdjusted : regimenAdjusted
    RegimenAdjusted --> InfusionReady : infusionPrepared
    ResponseAssessed --> InfusionReady : nextCycle
    ResponseAssessed --> Complete : allCyclesComplete
    Complete --> [*]
```
