...
```mermaid
erDiagram
    TriageAssessment ||--|| AcuteDiagnosis : "leads to"
    AcuteDiagnosis ||--|{ TreatmentOrder : generates
    TreatmentOrder ||--o{ DiagnosticResult : "informed by"
    AcuteDiagnosis ||--o| WoundRecord : "may include"
    TreatmentOrder ||--|| DischargeInstructions : "ends with"
    TriageAssessment ||--|{ DiagnosticResult : orders
```
...
stateDiagram-v2
    [*] --> Triaged
    Triaged --> DiagnosticsOrdered : diagnosticsOrdered
    DiagnosticsOrdered --> Diagnosed : conditionDiagnosed
    Diagnosed --> Treating : treatmentAdministered
    Treating --> Stabilized : patientStabilized
    Treating --> Treating : additionalTreatment
    Stabilized --> Discharged : patientDischarged
    Stabilized --> Admitted : patientAdmitted
    Admitted --> [*]
    Discharged --> [*]
```
...
graph TD
    ...
    F -->|No| H{Stable for Discharge?}
    H -->|Yes| I[dischargePatient]
    H -->|No| D
```
