...
```mermaid
erDiagram
    TransportIncident ||--|{ Vehicle : involves
    TransportIncident ||--|{ Casualty : affects
    TransportIncident ||--o{ HazardousCondition : "may have"
    TransportIncident ||--|| TrafficDisruption : causes
    TransportIncident ||--|| IncidentReport : "documented in"
    Casualty }o--|| Vehicle : "was in"
```
...
stateDiagram-v2
    [*] --> AlertReceived
    AlertReceived --> SceneSecured : sceneSecured
    SceneSecured --> CasualtiesTriaged : casualtiesAssessed
    CasualtiesTriaged --> Extricating : victimsExtricated
    CasualtiesTriaged --> HazardMitigation : hazardsManaged
    Extricating --> HazardMitigation : hazardsManaged
    HazardMitigation --> Clearing : trafficRestored
    CasualtiesTriaged --> Clearing : noHazards
    Clearing --> Documented : incidentDocumented
    Documented --> [*]
```
