...
```mermaid
erDiagram
    EmergencyCall ||--|| Incident : creates
    Incident ||--|{ ResponseUnit : "responded by"
    Incident ||--|{ AssistanceActivity : involves
    ResponseUnit ||--|| ResourceAllocation : "assigned via"
    Incident ||--|| IncidentReport : "documented in"
    AssistanceActivity }o--|| ResponseUnit : "performed by"
```
...
stateDiagram-v2
    [*] --> CallReceived
    CallReceived --> Dispatched : unitsDispatched
    Dispatched --> OnScene : sceneAssessed
    OnScene --> Coordinating : agenciesCoordinated
    OnScene --> Assisting : assistanceProvided
    Coordinating --> Assisting : assistanceProvided
    Assisting --> Managing : incidentManaged
    Managing --> OnScene : reassessment
    Managing --> Resolved : statusReported
    Resolved --> [*]
```
