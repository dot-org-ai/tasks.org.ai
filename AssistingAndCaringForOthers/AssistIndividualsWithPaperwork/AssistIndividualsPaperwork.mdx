...
```mermaid
erDiagram
    AssistanceRequest ||--|| FormPacket : identifies
    FormPacket ||--|{ RequiredInformation : specifies
    RequiredInformation ||--|| CompletedApplication : "compiled into"
    CompletedApplication ||--|| SubmissionRecord : "tracked by"
    SubmissionRecord ||--o{ FollowUpTask : triggers
```
...
stateDiagram-v2
    [*] --> NeedsAssessed
    NeedsAssessed --> InformationGathered : informationGathered
    InformationGathered --> FormExplained : formExplained
    FormExplained --> FormCompleted : formCompleted
    FormCompleted --> ApplicationReviewed : applicationReviewed
    ApplicationReviewed --> FormCompleted : correctionsNeeded
    ApplicationReviewed --> DocumentsSubmitted : documentsSubmitted
    DocumentsSubmitted --> StatusTracked : statusTracked
    StatusTracked --> NeedsAssessed : additionalFormsRequired
    StatusTracked --> [*]
```
## Workflow
