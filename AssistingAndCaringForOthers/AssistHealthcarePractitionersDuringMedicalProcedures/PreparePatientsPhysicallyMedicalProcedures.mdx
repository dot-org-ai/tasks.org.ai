...
```mermaid
erDiagram
    PreparationOrder ||--o{ SkinPrepProtocol : contains
    PreparationOrder ||--o{ FastingVerification : contains
    PreparationOrder ||--o{ BaselineAssessment : contains
    PreparationOrder ||--o{ AccessoryRemoval : contains
    PreparationOrder ||--o{ PreparationChecklist : contains
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Validated : instructionsVerified
    Validated --> Processing : baselineAssessed
    Processing --> Prepared : accessoriesRemoved
    Prepared --> [*]
```
## Workflow
