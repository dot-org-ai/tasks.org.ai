...
```mermaid
erDiagram
    ExaminationOrder ||--|| ConsentForm : requires
    ExaminationOrder ||--|| RoomSetup : prepares
    RoomSetup ||--|{ InstrumentTray : includes
    ExaminationOrder ||--o{ SpecimenCollection : "may produce"
    ExaminationOrder ||--|| ExaminationNote : "documented in"
```
...
stateDiagram-v2
    [*] --> RoomReady
    RoomReady --> ConsentConfirmed : consentVerified
    ConsentConfirmed --> PatientPositioned : positioningAssisted
    PatientPositioned --> InstrumentsProvided : instrumentsHanded
    InstrumentsProvided --> SpecimensTaken : specimensCollected
    InstrumentsProvided --> FindingsRecorded : findingsDocumented
    SpecimensTaken --> FindingsRecorded : findingsDocumented
    FindingsRecorded --> RoomRestored : roomCleaned
    RoomRestored --> RoomReady : nextExamination
    RoomRestored --> [*]
```
## Workflow
