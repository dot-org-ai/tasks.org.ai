...
```mermaid
erDiagram
    HoldingProtocol ||--o{ PositionRequirement : contains
    HoldingProtocol ||--o{ StabilizationRecord : generates
    PatientCooperation ||--o{ HoldingProtocol : provides
    HoldingProtocol ||--o{ ImmobilizationDevice : contains
    HoldingProtocol ||--o{ SafetyIncident : contains
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Processing : cooperationAssessed
    Processing --> Released : stabilizationPlanned
    Flagged --> [*]
```
## Workflow
