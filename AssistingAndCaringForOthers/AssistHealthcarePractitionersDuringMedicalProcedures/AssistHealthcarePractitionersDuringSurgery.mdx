...
```mermaid
erDiagram
    SurgicalCase ||--|{ InstrumentSet : requires
    SurgicalCase ||--|| SterileField : "performed in"
    SurgicalCase ||--|| SafetyChecklist : "verified by"
    SurgicalCase ||--o{ SpecimenHandling : "may produce"
    SurgicalCase ||--|| ProcedureLog : "documented in"
```
...
stateDiagram-v2
    [*] --> ORPrepared
    ORPrepared --> SterileReady : sterileFieldEstablished
    SterileReady --> SafetyCleared : safetyChecklistPerformed
    SafetyCleared --> Assisting : instrumentsPassed
    Assisting --> SpecimensRouted : specimensManaged
    SpecimensRouted --> CountsVerified : countsPerformed
    CountsVerified --> Assisting : countDiscrepancy
    CountsVerified --> Documented : procedureDocumented
    Documented --> [*]
```
## Workflow
