...
```mermaid
erDiagram
    PositioningOrder ||--|| RepositioningSchedule : follows
    PositioningOrder ||--|{ PositioningDevice : requires
    PositioningOrder ||--|| BodyAlignment : targets
    BodyAlignment ||--|{ PressurePoint : protects
...
stateDiagram-v2
    [*] --> PositionAssessed
    PositionAssessed --> RepositionPlanned : repositioningPlanned
    RepositionPlanned --> EquipmentReady : equipmentPrepared
    EquipmentReady --> PatientMoved : patientAdjusted
    PatientMoved --> AlignmentChecked : alignmentVerified
    AlignmentChecked --> PatientMoved : realignmentNeeded
    AlignmentChecked --> Documented : positionDocumented
    Documented --> PositionAssessed : scheduleTriggered
    Documented --> [*]
```
## Workflow
