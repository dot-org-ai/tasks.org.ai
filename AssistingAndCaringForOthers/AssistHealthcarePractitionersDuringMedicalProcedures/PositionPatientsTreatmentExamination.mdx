...
```mermaid
erDiagram
    PositioningOrder ||--|| PositionProtocol : follows
    PositionProtocol ||--|{ PositioningDevice : requires
    PositioningOrder ||--|| AnatomicalAlignment : targets
    AnatomicalAlignment ||--|| PressureRiskAssessment : informs
    PositioningOrder ||--|| SafetyChecklist : "verified by"
```
...
stateDiagram-v2
    ...
    RequirementsAssessed --> PositionChosen : positionSelected
    PositionChosen --> EquipmentReady : equipmentPrepared
    EquipmentReady --> PatientPlaced : patientPositioned
    PatientPlaced --> AlignmentOK : alignmentVerified
    AlignmentOK --> ToleranceChecked : toleranceMonitored
    ToleranceChecked --> PatientPlaced : repositionNeeded
    ToleranceChecked --> Documented : positionDocumented
    Documented --> [*]
```
## Workflow
