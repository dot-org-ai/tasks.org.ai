...
```mermaid
erDiagram
    ReferralOrder ||--o{ InsuranceAuthorization : requires
    ReferralOrder ||--|| AppointmentScheduling : schedules
    ReferralOrder ||--|{ ClinicalSummary : includes
    AppointmentScheduling }o--|| ProviderDirectory : "booked via"
    ReferralOrder ||--|| FollowUpTracking : monitors
    ClinicalSummary }o--|| ProviderDirectory : "sent to"
```
...
```mermaid
stateDiagram-v2
    [*] --> ClinicalNeedAssessed
    ClinicalNeedAssessed --> SpecialistIdentified : specialistIdentified
    SpecialistIdentified --> AuthorizationPending : authorizationRequested
    AuthorizationPending --> Authorized : authorizationObtained
    AuthorizationPending --> SpecialistIdentified : denied
    Authorized --> Scheduled : appointmentScheduled
    Scheduled --> RecordsShared : recordsTransferred
    RecordsShared --> AwaitingVisit : followUpTracked
    AwaitingVisit --> LoopClosed : loopClosed
    AwaitingVisit --> Scheduled : noShow
    LoopClosed --> [*]
```
