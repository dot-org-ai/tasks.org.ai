...
```mermaid
erDiagram
    CustomerInquiry ||--|| RoutingCriteria : "evaluated by"
    RoutingCriteria }o--|| ExpertiseDirectory : references
    CustomerInquiry ||--|| ReferralHandoff : "results in"
    ReferralHandoff }o--|| ExpertiseDirectory : "matched via"
    CustomerInquiry ||--o| EscalationPath : "may trigger"
    ReferralHandoff ||--|| ResolutionTracking : tracks
```
...
stateDiagram-v2
    [*] --> InquiryReceived
    InquiryReceived --> Assessed : inquiryAssessed
    Assessed --> PersonnelMatched : personnelIdentified
    PersonnelMatched --> HandoffReady : handoffPrepared
    PersonnelMatched --> Queued : personnelUnavailable
    Queued --> PersonnelMatched : personnelIdentified
    HandoffReady --> Transferred : customerTransferred
    Transferred --> Engaged : receiptConfirmed
    Engaged --> Resolved : resolutionTracked
    Resolved --> FeedbackReceived : feedbackCollected
    FeedbackReceived --> [*]
```
