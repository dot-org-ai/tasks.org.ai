...
```mermaid
erDiagram
    ServiceRequest ||--|{ Referral : generates
    Referral ||--o{ Authorization : requires
    Referral ||--|{ Appointment : schedules
    Appointment }o--|| ProviderNetwork : "booked with"
    ServiceRequest ||--o{ CareGap : identifies
    CareGap }o--|| ProviderNetwork : "resolved by"
```
...
stateDiagram-v2
    [*] --> NeedsAssessed
    NeedsAssessed --> ProvidersIdentified : providersIdentified
    ProvidersIdentified --> ReferralSubmitted : referralSubmitted
    ReferralSubmitted --> AuthorizationPending : authorizationRequested
    AuthorizationPending --> Authorized : authorizationApproved
    AuthorizationPending --> ProvidersIdentified : authorizationDenied
    Authorized --> Scheduled : appointmentScheduled
    Scheduled --> InProgress : appointmentAttended
    InProgress --> Completed : outcomeDocumented
    InProgress --> Scheduled : followUpNeeded
    Completed --> [*]
```
