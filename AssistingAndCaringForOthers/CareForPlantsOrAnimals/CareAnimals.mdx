...
erDiagram
    ...
    Animal ||--|| FeedingPlan : follows
    Animal ||--|| Habitat : "lives in"
    CareSchedule ||--|{ CareLog : generates
    Animal ||--|{ CareLog : "documented in"
    CareSchedule ||--|| FeedingPlan : includes
```
...
stateDiagram-v2
    [*] --> Admitted
    Admitted --> Monitored : healthMonitored
    Monitored --> Healthy : noIssues
    Monitored --> UnderTreatment : alertRaised
    UnderTreatment --> Medicated : medicationAdministered
    Medicated --> Monitored : healthMonitored
    Healthy --> Fed : animalFed
    Fed --> Exercised : animalExercised
    Exercised --> HabitatClean : habitatCleaned
    HabitatClean --> Logged : observationRecorded
    Logged --> Monitored : nextCycle
```
