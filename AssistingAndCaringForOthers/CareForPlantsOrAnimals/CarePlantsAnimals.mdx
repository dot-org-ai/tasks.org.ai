...
erDiagram
    ...
    Plant ||--|| Habitat : "grows in"
    Habitat ||--|{ CareSchedule : follows
    CareSchedule ||--|{ HealthRecord : generates
    Animal ||--|{ HealthRecord : has
    Plant ||--|{ HealthRecord : has
    ResourceInventory ||--o{ CareSchedule : supplies
```
...
stateDiagram-v2
    [*] --> Monitoring
    Monitoring --> IssueDetected : conditionsMonitored
    Monitoring --> Routine : noIssues
    IssueDetected --> Treated : issuesTreated
    Treated --> Monitoring : conditionsMonitored
    Routine --> CareComplete : careProvided
    CareComplete --> HabitatReady : habitatMaintained
    HabitatReady --> Adjusted : environmentAdjusted
    Adjusted --> Logged : activityRecorded
    Logged --> SupplyCheck : inventoryReviewed
    SupplyCheck --> Restocking : suppliesProcured
    SupplyCheck --> Monitoring : suppliesAdequate
    Restocking --> Monitoring : suppliesReceived
```
