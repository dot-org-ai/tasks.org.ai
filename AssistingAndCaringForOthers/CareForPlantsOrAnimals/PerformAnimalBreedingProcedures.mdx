...
```mermaid
erDiagram
    BreedingAnimal ||--o{ BreedingPair : "participates in"
    BreedingPair ||--|{ BreedingCycle : initiates
    BreedingCycle ||--o| Pregnancy : "results in"
    Pregnancy ||--|{ Offspring : produces
    Offspring ||--|| PedigreeRecord : "registered in"
    BreedingAnimal ||--|| PedigreeRecord : "listed in"
```
...
stateDiagram-v2
    [*] --> PairSelected
    PairSelected --> BreedingComplete : breedingPerformed
    BreedingComplete --> Confirmed : pregnancyConfirmed
    BreedingComplete --> Failed : conceptionFailed
    Failed --> [*]
    Confirmed --> Monitoring : pregnancyMonitored
    Monitoring --> Monitoring : pregnancyMonitored
    Monitoring --> Delivering : birthAssisted
    Delivering --> Registered : offspringRegistered
    Registered --> Evaluated : outcomeAssessed
    Evaluated --> [*]
```
