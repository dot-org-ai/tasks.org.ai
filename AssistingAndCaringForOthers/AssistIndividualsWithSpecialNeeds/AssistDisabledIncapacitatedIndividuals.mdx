...
```mermaid
erDiagram
    FunctionalAssessment ||--|| CareSchedule : determines
    CareSchedule ||--|{ ADLSupport : schedules
    CareSchedule ||--|{ MedicalMonitoring : requires
    ADLSupport ||--|| DignityProtocol : "governed by"
    MedicalMonitoring ||--o{ SafetySupervision : triggers
```
...
stateDiagram-v2
    [*] --> CareNeedsAssessed
    CareNeedsAssessed --> CareScheduleDeveloped : careScheduleDeveloped
    CareScheduleDeveloped --> PersonalCareProvided : personalCareProvided
    PersonalCareProvided --> HealthMonitored : healthMonitored
    HealthMonitored --> SafetyEnsured : safetyEnsured
    SafetyEnsured --> DignityMaintained : dignityMaintained
    DignityMaintained --> CareDocumented : careDocumented
    CareDocumented --> PersonalCareProvided : nextCareRound
    CareDocumented --> CareNeedsAssessed : functionalDecline
    CareDocumented --> [*]
```
## Workflow
