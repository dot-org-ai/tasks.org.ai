...
| AspirationRisk | Swallowing safety assessment and precautions |
...
```mermaid
erDiagram
    Patient ||--|| DietaryOrder : has
    DietaryOrder ||--|| MealPlan : guides
    Patient ||--|| FeedingAssistance : requires
    Patient ||--|| AspirationRisk : assessed-for
    FeedingAssistance ||--o{ IntakeRecord : generates
    IntakeRecord }o--|| NutritionStatus : contributes-to
    Patient ||--o{ NutritionStatus : monitored-by
```
...
stateDiagram-v2
    [*] --> OrderVerified
    OrderVerified --> Positioned : patientPositioned
    Positioned --> MealReady : mealPrepared
    MealReady --> Feeding : assistWithFeeding
    Feeding --> Feeding : swallowingMonitored
    Feeding --> Documented : intakeDocumented
    Documented --> ConcernRaised : concernsReported
    Documented --> Completed : adequateIntake
    ConcernRaised --> [*]
    Completed --> [*]
```
...
feeding.intakeDocumented(async ({ patientId, percentageEaten, date }) => {
  const recentIntakes = await feeding.getIntakeRecords({
    patientId,
    startDate: subtractDays(date, 7)
  })

  const avgIntake = calculateAverage(recentIntakes.map(r => r.percentageEaten))
  if (avgIntake < 60) {
    await feeding.reportConcerns({
      patientId,
      concern: `Low average intake over 7 days: ${avgIntake}%`,
      recommendation: 'Comprehensive nutrition assessment needed'
    })
  }
})
```
