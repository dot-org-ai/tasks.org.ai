...
erDiagram
    Student ||--|| IEP : has
    IEP ||--o{ Accommodation : specifies
    IEP ||--o{ ServiceRecord : defines
    Student ||--o{ SupportSession : receives
    Student ||--o{ ProgressReport : generates
    SupportSession }o--|| Accommodation : implements
    ProgressReport }o--|| IEP : tracks-goals-from
```
...
stateDiagram-v2
    [*] --> Identified
    Identified --> Planned : iepImplemented
    Planned --> Supported : provideSupport
    Supported --> Supported : supportProvided
    Supported --> Adapted : materialsAdapted
    Adapted --> Monitored : progressMonitored
    Monitored --> Coordinated : teamCoordinated
    Coordinated --> Adjusted : supportAdjusted
    Adjusted --> Supported : servicesDocumented
    Monitored --> Completed : goalsAchieved
    Completed --> [*]
```
