...
```mermaid
erDiagram
    Patient ||--|| CarePlan : has
    CarePlan ||--|| HygieneSchedule : defines
    CarePlan ||--o{ AssistanceLevel : specifies
    Patient ||--o{ SkinAssessment : undergoes
    Patient }o--o{ MobilityAid : uses
    PersonalCareLog }o--|| Patient : documents
    PersonalCareLog }o--|| CarePlan : implements
```
...
stateDiagram-v2
    [*] --> Scheduled
    Scheduled --> BathingInProgress : assistBathing
    BathingInProgress --> Groomed : provideGrooming
    Groomed --> Dressed : assistDressing
    Dressed --> Fed : assistFeeding
    Dressed --> Toileted : assistToileting
    Fed --> Mobilized : assistMobility
    Toileted --> Mobilized : assistMobility
    Mobilized --> Documented : documentCare
    Documented --> [*]
```
