...
```mermaid
erDiagram
    TherapyAssessment ||--|| TreatmentPlan : informs
    TreatmentPlan ||--|{ TherapySession : schedules
    TreatmentPlan ||--|| ExerciseProgram : prescribes
    TherapySession ||--|| ProgressNote : "documented in"
    TreatmentPlan ||--|{ FunctionalOutcome : tracks
```
...
stateDiagram-v2
    [*] --> Assessed
    Assessed --> PlanCreated : treatmentPlanDeveloped
    PlanCreated --> ExercisesSet : exercisesPrescribed
    ExercisesSet --> ManualDelivered : manualTherapyDelivered
    ManualDelivered --> Documented : sessionDocumented
    Documented --> OutcomesChecked : outcomesMeasured
    OutcomesChecked --> PlanAdjusted : treatmentAdjusted
    PlanAdjusted --> ExercisesSet : exercisesPrescribed
    OutcomesChecked --> [*]
```
## Workflow
