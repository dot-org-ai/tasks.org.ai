...
erDiagram
    ...
    TreatmentPlan ||--|{ FunctionalAssessment : monitors
    FunctionalAssessment ||--|{ OutcomeMeasure : produces
    TreatmentPlan ||--o| HomeExerciseProgram : prescribes
```
...
stateDiagram-v2
    [*] --> PlanDeveloped
    PlanDeveloped --> SessionScheduled : sessionScheduled
    SessionScheduled --> InterventionDelivered : interventionConducted
    InterventionDelivered --> ProgressAssessed : progressMeasured
    ProgressAssessed --> ProtocolModified : protocolAdjusted
    ProtocolModified --> SessionScheduled : sessionScheduled
    ProgressAssessed --> HomeAssigned : homeExercisesPrescribed
    HomeAssigned --> SessionScheduled : sessionScheduled
    ProgressAssessed --> Discharged : patientDischarged
    Discharged --> [*]
```
## Workflow
