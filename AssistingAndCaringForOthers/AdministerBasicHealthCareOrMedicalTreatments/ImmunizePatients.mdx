...
```mermaid
erDiagram
    Vaccine ||--o{ ImmunizationRecord : generates
    Vaccine ||--o{ VaccinationSchedule : contains
    Vaccine ||--o{ ConsentForm : contains
    Vaccine ||--o{ ScreeningQuestionnaire : contains
    Vaccine ||--o{ AdverseEvent : contains
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> Processing : patientScreened
    Processing --> Validated : consentObtained
    Validated --> Prepared : vaccineVerified
    Prepared --> [*]
```
## Workflow
