...
erDiagram
    ...
    MedicationOrder ||--|| AdministrationRoute : specifies
    VaccineOrder ||--|| Dose : prescribes
    Dose ||--|| AdministrationRecord : "recorded in"
    AdministrationRecord ||--o| PatientResponse : "may trigger"
```
...
stateDiagram-v2
    [*] --> OrderVerified
    OrderVerified --> PatientConfirmed : patientIdentified
    PatientConfirmed --> DoseReady : dosePrepared
    DoseReady --> Educated : recipientEducated
    Educated --> Administered : doseAdministered
    Administered --> Monitored : patientMonitored
    Monitored --> Documented : administrationDocumented
    Monitored --> OrderVerified : adverseReaction
    Documented --> [*]
```
## Workflow
