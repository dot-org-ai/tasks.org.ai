...
```mermaid
erDiagram
    WoundAssessment ||--|| Wound : evaluates
    Wound ||--|{ Dressing : "covered by"
    Dressing ||--|| Bandage : "secured with"
    Wound ||--o| Splint : "stabilized by"
    Dressing ||--|{ DressingChange : "replaced via"
```
...
stateDiagram-v2
    [*] --> WoundAssessed
    WoundAssessed --> MaterialsReady : materialsSelected
    MaterialsReady --> SitePrepared : sitePrepared
    SitePrepared --> DressingOn : dressingApplied
    DressingOn --> BandageSecured : bandageWrapped
    DressingOn --> SplintApplied : splintApplied
    BandageSecured --> Documented : applicationDocumented
    SplintApplied --> Documented : applicationDocumented
    Documented --> WoundAssessed : dressingChangeScheduled
    Documented --> [*]
```
## Workflow
