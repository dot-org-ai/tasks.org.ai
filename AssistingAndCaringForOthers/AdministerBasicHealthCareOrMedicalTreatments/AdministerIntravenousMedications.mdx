...
```mermaid
erDiagram
    IVOrder ||--|| MedicationBag : prescribes
    IVOrder ||--|| IVLine : requires
    IVLine ||--|| IVSite : "placed at"
    MedicationBag ||--|| InfusionPump : "delivered via"
    InfusionPump ||--|| Infusion : controls
```
...
stateDiagram-v2
    [*] --> OrderVerified
    OrderVerified --> AccessEstablished : accessEstablished
    AccessEstablished --> MedicationReady : medicationPrepared
    MedicationReady --> PumpConfigured : pumpProgrammed
    PumpConfigured --> Infusing : infusionInitiated
    Infusing --> SiteChecked : siteMonitored
    SiteChecked --> FlowModified : flowAdjusted
    FlowModified --> Infusing : infusionInitiated
    SiteChecked --> [*]
```
## Workflow
