...
```mermaid
erDiagram
    Assessment ||--|| CarePlan : informs
    CarePlan ||--|{ Treatment : prescribes
    Treatment ||--|| Protocol : "governed by"
    Treatment ||--|{ VitalSigns : monitors
    Treatment ||--|| MedicalRecord : "documented in"
```
...
stateDiagram-v2
    [*] --> Assessed
    Assessed --> VitalsRecorded : vitalSignsTaken
    VitalsRecorded --> TreatmentDelivered : treatmentAdministered
    TreatmentDelivered --> Documented : careDocumented
    Documented --> Monitoring : progressMonitored
    Monitoring --> ChangesReported : changesReported
    ChangesReported --> PlanUpdated : carePlanUpdated
    PlanUpdated --> TreatmentDelivered : treatmentAdministered
    Monitoring --> [*]
```
## Workflow
