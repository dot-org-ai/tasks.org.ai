...
```mermaid
erDiagram
    PainAssessment ||--|| DosageCalculation : determines
    DosageCalculation ||--|| AnestheticMedication : prescribes
    AnestheticMedication ||--|| AdministrationProtocol : "governed by"
    AdministrationProtocol ||--|{ VitalSignsRecord : monitors
    AnestheticMedication ||--o| AdverseEventReport : "may trigger"
```
...
stateDiagram-v2
    [*] --> PainAssessed
    PainAssessed --> MedicationSelected : medicationSelected
    MedicationSelected --> DosageSet : dosageCalculated
    DosageSet --> Administered : drugAdministered
    Administered --> Monitoring : vitalsMonitored
    Monitoring --> DosageAdjusted : dosageAdjusted
    DosageAdjusted --> Monitoring : vitalsMonitored
    Monitoring --> Documented : administrationDocumented
    Documented --> [*]
```
## Workflow
