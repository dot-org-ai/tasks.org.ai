...
```mermaid
erDiagram
    CarePanel ||--|{ HealthAssessment : schedules
    HealthAssessment ||--o| ChronicCondition : identifies
    ChronicCondition ||--|{ HealthGoal : targets
    HealthAssessment ||--o| PreventiveCare : recommends
    HealthAssessment ||--o| Referral : generates
```
...
stateDiagram-v2
    [*] --> Assessed
    Assessed --> PreventiveCareGiven : preventiveCareProvided
    Assessed --> ChronicManaged : chronicConditionManaged
    ChronicManaged --> Educated : patientEducated
    PreventiveCareGiven --> Educated : patientEducated
    Educated --> GoalTracked : healthGoalTracked
    GoalTracked --> Referred : referralCoordinated
    GoalTracked --> Documented : encounterDocumented
    Referred --> Documented : encounterDocumented
    Documented --> Assessed : followUpScheduled
    Documented --> [*]
```
## Workflow
