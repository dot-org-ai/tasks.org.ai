...
```mermaid
erDiagram
    ContrastOrder ||--|| SubstanceProtocol : "governed by"
    ContrastOrder ||--|| DoseCalculation : requires
    PatientAllergy ||--o| ContrastOrder : constrains
    ContrastOrder ||--|| InjectionRecord : "documented in"
    InjectionRecord ||--o| AdverseReaction : "may trigger"
```
...
stateDiagram-v2
    [*] --> OrderVerified
    OrderVerified --> AllergiesCleared : allergiesScreened
    AllergiesCleared --> DoseReady : doseCalculated
    DoseReady --> SubstanceReady : substancePrepared
    SubstanceReady --> Administered : substanceAdministered
    Administered --> Monitoring : reactionDetected
    Monitoring --> Administered : reactionCleared
    Administered --> Documented : administrationDocumented
    Documented --> [*]
```
## Workflow
