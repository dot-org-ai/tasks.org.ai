...
```mermaid
erDiagram
    InfusionOrder ||--|| IVAccessLine : requires
    InfusionOrder ||--o| BloodProduct : "may specify"
    InfusionOrder ||--o| FluidBag : "may specify"
    InfusionOrder ||--|| InfusionRate : sets
    IVAccessLine ||--o| AdverseReactionReport : "may trigger"
```
...
stateDiagram-v2
    [*] --> OrderVerified
    OrderVerified --> AccessReady : accessEstablished
    AccessReady --> CompatibilityConfirmed : compatibilityChecked
    CompatibilityConfirmed --> InfusionPrepared : infusionPrepared
    InfusionPrepared --> Infusing : infusionInitiated
    Infusing --> Monitored : patientMonitored
    Monitored --> Infusing : reactionCleared
    Monitored --> Documented : administrationDocumented
    Documented --> [*]
```
## Workflow
