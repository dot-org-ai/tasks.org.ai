...
```mermaid
erDiagram
    AssistanceRequest ||--|| VehicleArrival : "triggered by"
    AssistanceRequest ||--o| MobilityAid : "may require"
    AssistanceRequest ||--o{ LuggageTicket : "may include"
    AssistanceRequest ||--|| SafetyCheckRecord : "verified by"
    VehicleArrival ||--|| CurbsideZone : "arrives at"
    CurbsideZone ||--|{ VehicleArrival : receives
```
...
stateDiagram-v2
    [*] --> Requested
    Requested --> VehicleReady : vehiclePositioned
    VehicleReady --> AidDeployed : mobilityAidDeployed
    VehicleReady --> Boarding : noMobilityNeeded
    AidDeployed --> Boarding : aidReady
    Boarding --> Boarded : boardingCompleted
    Boarding --> Alighted : alightingCompleted
    Boarded --> LuggageLoaded : luggageHandled
    Alighted --> LuggageLoaded : luggageHandled
    LuggageLoaded --> SafetyVerified : safetyConfirmed
    SafetyVerified --> [*]
```
...
graph TD
    ...
    C -->|No| E{Boarding or Alighting?}
    D --> E
    E -->|Boarding| F[assistBoarding]
    E -->|Alighting| G[assistAlighting]
    F --> H[handleLuggage]
    G --> H
    H --> I[confirmSafety]
```
