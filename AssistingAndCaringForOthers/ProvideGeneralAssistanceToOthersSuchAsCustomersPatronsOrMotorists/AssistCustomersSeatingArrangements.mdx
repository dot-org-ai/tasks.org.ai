...
erDiagram
    ...
    Table ||--|| SeatingChart : "located in"
    WaitList ||--|{ Table : "awaits"
    GuestPreference }o--|| Table : "applied to"
    Table ||--|{ TurnoverRecord : tracks
    Reservation ||--o| GuestPreference : includes
```
...
stateDiagram-v2
    [*] --> AvailabilityChecked
    AvailabilityChecked --> Seated : guestSeated
    AvailabilityChecked --> OnWaitList : waitListUpdated
    OnWaitList --> Seated : guestSeated
    Seated --> PreferenceApplied : preferenceAccommodated
    PreferenceApplied --> Occupied : tableStatusChanged
    Occupied --> Cleared : tableCleared
    Cleared --> AvailabilityChecked : readyForNext
```
...
graph TD
    ...
    B -->|Yes| C{Preference Match?}
    B -->|No| D[addToWaitList]
    C -->|Yes| E[seatGuest]
    C -->|No| F[accommodatePreference]
    F --> E
    D --> G[Wait for Table]
    G --> E
    E --> H[updateTableStatus]
    H --> I[clearTable]
    I --> A
```
