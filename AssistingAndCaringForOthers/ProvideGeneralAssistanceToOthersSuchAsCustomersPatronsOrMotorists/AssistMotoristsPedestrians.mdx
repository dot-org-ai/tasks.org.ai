...
```mermaid
erDiagram
    AssistanceRequest ||--|| IncidentReport : documents
    AssistanceRequest ||--|| ServiceDispatch : triggers
    AssistanceRequest ||--|| Location : "located at"
    ServiceDispatch ||--|{ ResponseLog : generates
    AssistanceRequest ||--o| Vehicle : involves
    Location ||--|| IncidentReport : "referenced in"
```
...
stateDiagram-v2
    [*] --> RequestCreated
    RequestCreated --> Dispatched : serviceDispatched
    Dispatched --> OnScene : sceneAssessed
    OnScene --> Escalated : emergencyEscalated
    OnScene --> TrafficManaged : trafficDirected
    TrafficManaged --> DirectionsGiven : directionsProvided
    DirectionsGiven --> Closed : requestClosed
    Escalated --> Closed : requestClosed
    Closed --> [*]
```
...
graph TD
    ...
    D -->|No| F{Traffic Issue?}
    F -->|Yes| G[directTraffic]
    F -->|No| H[provideDirections]
    G --> H
    H --> I[closeRequest]
    E --> I
```
