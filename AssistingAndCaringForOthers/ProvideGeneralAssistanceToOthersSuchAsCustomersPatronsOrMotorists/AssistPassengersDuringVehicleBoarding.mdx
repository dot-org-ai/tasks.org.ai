...
```mermaid
erDiagram
    BoardingManifest ||--|{ BoardingPass : contains
    BoardingPass ||--|| SeatAssignment : "assigned to"
    BoardingPass ||--|| BoardingGroup : "belongs to"
    BoardingPass ||--o| MobilityRequest : "may have"
    BoardingPass ||--o| LuggageTag : "linked to"
    BoardingManifest ||--|| VehicleStatus : "depends on"
```
...
stateDiagram-v2
    [*] --> GroupCalled
    GroupCalled --> PassScanned : boardingPassScanned
    PassScanned --> MobilityAssisted : mobilitySupportProvided
    PassScanned --> Seated : seatAssigned
    MobilityAssisted --> Seated : seatAssigned
    Seated --> StowageComplete : stowageAssisted
    StowageComplete --> GroupCalled : nextGroup
    StowageComplete --> Reconciling : manifestReconciled
    Reconciling --> GateClosed : boardingGateClosed
    GateClosed --> [*]
```
...
graph TD
    ...
    H --> I{Missing Passengers?}
    I -->|Yes| J[Page Passengers]
    I -->|No| K[closeBoardingGate]
    J --> H
```
