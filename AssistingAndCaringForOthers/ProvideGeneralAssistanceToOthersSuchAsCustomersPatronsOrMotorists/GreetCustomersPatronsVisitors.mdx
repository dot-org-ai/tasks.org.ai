...
```mermaid
erDiagram
    VisitorRecord ||--o| Appointment : "linked to"
    VisitorRecord ||--o| BadgePass : receives
    VisitorRecord ||--|| CheckInEvent : generates
    WaitList ||--|{ VisitorRecord : queues
    Appointment }o--|| ServiceDirectory : "routed via"
    CheckInEvent }o--|| ServiceDirectory : "logged in"
```
...
stateDiagram-v2
    [*] --> CheckedIn
    CheckedIn --> AppointmentVerified : appointmentVerified
    CheckedIn --> WaitListed : waitListUpdated
    AppointmentVerified --> PassIssued : passIssued
    WaitListed --> PassIssued : passIssued
    PassIssued --> HostAlerted : hostNotified
    HostAlerted --> Routed : guestRouted
    Routed --> [*]
```
