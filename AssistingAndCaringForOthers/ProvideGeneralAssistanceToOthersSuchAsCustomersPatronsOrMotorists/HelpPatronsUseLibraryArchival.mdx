...
```mermaid
erDiagram
    ReferenceInquiry ||--|{ CatalogRecord : searches
    CatalogRecord ||--o{ CirculationTransaction : "checked out via"
    CatalogRecord ||--o| HoldRequest : "reserved via"
    DigitalResource ||--|| CatalogRecord : "cataloged as"
    ArchivalCollection ||--|{ CatalogRecord : contains
    ReferenceInquiry ||--o| ResearchGuide : "supported by"
```
...
stateDiagram-v2
    [*] --> InquiryReceived
    InquiryReceived --> CatalogSearched : catalogSearched
    CatalogSearched --> ResourceFound : resourceLocated
    CatalogSearched --> InterlibraryRequest : interlibraryRequested
    ResourceFound --> CheckedOut : itemCheckedOut
    InterlibraryRequest --> OnHold : holdPlaced
    OnHold --> CheckedOut : itemAvailable
    CheckedOut --> ResearchGuided : researchGuided
    ResearchGuided --> [*]
```
...
graph TD
    ...
    C -->|No| E{Interlibrary Option?}
    E -->|Yes| F[requestInterlibrary]
    E -->|No| G[guidResearch]
    D --> H[checkoutItem]
    F --> I[placeHold]
    H --> G
