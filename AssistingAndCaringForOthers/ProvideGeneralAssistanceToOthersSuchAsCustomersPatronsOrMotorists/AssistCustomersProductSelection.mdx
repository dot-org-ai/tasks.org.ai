...
```mermaid
erDiagram
    CustomerProfile ||--|{ Interaction : has
    Interaction ||--|{ Recommendation : generates
    Recommendation }o--|| Product : suggests
    Product ||--|| InventoryStatus : has
    Product ||--o{ ComparisonChart : "included in"
    Interaction ||--o| PurchaseDecision : "results in"
```
...
stateDiagram-v2
    [*] --> NeedsGathered
    NeedsGathered --> ProductsFound : productsSearched
    ProductsFound --> Recommended : productRecommended
    Recommended --> Compared : productsCompared
    Compared --> Demonstrated : featuresDemo
    Demonstrated --> StockChecked : inventoryChecked
    StockChecked --> Decided : decisionRecorded
    StockChecked --> Recommended : customerUndecided
    Demonstrated --> Recommended : needsRefinement
    Decided --> [*]
```
