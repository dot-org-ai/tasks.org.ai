...
```mermaid
erDiagram
    Itinerary ||--|{ FlightReservation : includes
    Itinerary ||--|{ HotelReservation : includes
    Itinerary ||--o{ EventTicket : includes
    Itinerary ||--o{ CarRental : includes
    TravelPreference ||--|| Itinerary : guides
    BookingConfirmation }o--|| Itinerary : confirms
    PaymentRecord }o--|| BookingConfirmation : "pays for"
```
...
stateDiagram-v2
    [*] --> PreferencesGathered
    PreferencesGathered --> Searching : availabilitySearched
    Searching --> FlightBooked : flightBooked
    FlightBooked --> HotelBooked : hotelBooked
    HotelBooked --> EventBooked : eventBooked
    EventBooked --> Assembled : itineraryAssembled
    Assembled --> Approved : clientApproved
    Assembled --> Searching : revisionsNeeded
    Approved --> Cancelled : reservationCancelled
    Approved --> [*]
    Cancelled --> Searching : rebook
```
