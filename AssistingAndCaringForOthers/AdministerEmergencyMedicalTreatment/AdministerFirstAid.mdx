...
```mermaid
erDiagram
    Incident ||--|| Assessment : triggers
    Assessment ||--|{ Intervention : determines
    Intervention ||--|| FirstAidKit : "supplied by"
    Incident ||--|| IncidentReport : "documented in"
    Incident ||--o| TransferOfCare : "may require"
```
...
stateDiagram-v2
    [*] --> SceneAssessed
    SceneAssessed --> PrimaryChecked : primaryAssessmentPerformed
    PrimaryChecked --> BleedingStopped : bleedingControlled
    PrimaryChecked --> InjuryImmobilized : injuryStabilized
    PrimaryChecked --> ShockManaged : shockTreated
    BleedingStopped --> EMSCalled : emsActivated
    InjuryImmobilized --> EMSCalled : emsActivated
    ShockManaged --> EMSCalled : emsActivated
    EMSCalled --> Documented : incidentDocumented
    Documented --> SceneAssessed : newVictimFound
    Documented --> [*]
```
## Workflow
