...
```mermaid
erDiagram
    MedicalDisplay ||--|{ SignalQuality : "measured by"
    MedicalDisplay ||--o{ CalibrationRecord : "verified via"
    MedicalDisplay ||--o{ DisplayAlert : "generates"
    MedicalDisplay ||--o{ PatientSession : "used during"
    PatientSession ||--o{ ComplianceReport : "documented in"
    CalibrationRecord }o--|| ComplianceReport : "referenced in"
    DisplayAlert }o--|| PatientSession : "may affect"
```
...
stateDiagram-v2
    [*] --> Inactive
    Inactive --> Active : monitoringActivated
    Active --> SignalChecked : signalVerified
    SignalChecked --> ClarityChecked : clarityAssessed
    ClarityChecked --> CalibrationValid : calibrationValidated
    CalibrationValid --> Documented : sessionDocumented
    Documented --> Active : nextCycle
    SignalChecked --> ArtifactFound : artifactDetected
    ClarityChecked --> ArtifactFound : artifactDetected
    ArtifactFound --> StaffNotified : staffAlerted
    StaffNotified --> Active : issueResolved
    StaffNotified --> Inactive : displayTakenOffline
```
...
graph TD
    ...
    I --> J{Calibration Valid?}
    J -->|Yes| K[documentSession]
    J -->|No| D
    D --> L{Resolved?}
    L -->|Yes| B
    L -->|No| M[Take Display Offline]
    K --> B
```
