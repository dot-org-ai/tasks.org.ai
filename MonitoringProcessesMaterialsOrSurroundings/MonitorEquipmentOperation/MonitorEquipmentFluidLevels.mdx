...
```mermaid
erDiagram
    FluidReservoir ||--o{ LevelReading : produces
    FluidReservoir ||--|| ChangeInterval : "serviced per"
    LevelReading ||--o{ ConsumptionRate : calculates
    ConsumptionRate ||--o{ LeakAlert : "may trigger"
    RefillEvent }o--|| FluidReservoir : replenishes
    LeakAlert }o--|| FluidReservoir : "flagged for"
```
...
stateDiagram-v2
    [*] --> Normal
    Normal --> LevelRead : levelRead
    LevelRead --> Low : lowLevelAlerted
    LevelRead --> Normal : consumptionTracked
    Low --> ScheduledRefill : refillScheduled
    ScheduledRefill --> Normal : refillRecorded
    Normal --> LeakSuspected : leakDetected
    LeakSuspected --> ScheduledRefill : refillScheduled
    Normal --> QualityCheck : qualityAnalyzed
    QualityCheck --> ScheduledRefill : refillScheduled
    QualityCheck --> Normal : levelRead
```
