...
```mermaid
erDiagram
    QualityParameter ||--o{ DefectRecord : "deviation causes"
    QualityParameter ||--|{ ControlLimit : "bounded by"
    DefectRecord ||--o{ RootCauseAnalysis : "investigated via"
    ProcessAdjustment }o--|| QualityParameter : "modifies"
    RootCauseAnalysis ||--o{ ProcessAdjustment : "recommends"
    QualityMetric }o--|| DefectRecord : "aggregates"
    QualityMetric }o--|| ControlLimit : "compared against"
```
...
stateDiagram-v2
    [*] --> Monitoring
    Monitoring --> InControl : parameterMonitored
    InControl --> Monitoring : parameterMonitored
    Monitoring --> DefectFound : defectDetected
    InControl --> DefectFound : defectDetected
    DefectFound --> Adjusting : processAdjusted
    DefectFound --> Stopped : productionStopped
    Stopped --> Investigating : rootCauseInvestigated
    Adjusting --> Investigating : rootCauseInvestigated
    Investigating --> Corrected : correctionDocumented
    Corrected --> Monitoring : metricsTracked
```
