...
```mermaid
erDiagram
    EnergySystem ||--|{ GenerationMetric : "produces"
    EnergySystem ||--o| StorageStatus : "has"
    EnergySystem ||--o{ MaintenanceEvent : "undergoes"
    WeatherCondition }o--|| GenerationMetric : "influences"
    GenerationMetric ||--o{ PerformanceReport : "aggregated in"
    StorageStatus }o--|| PerformanceReport : "included in"
```
...
stateDiagram-v2
    [*] --> Offline
    Offline --> Tracking : trackingStarted
    Tracking --> Generating : generationCaptured
    Generating --> Assessed : performanceAssessed
    Assessed --> Generating : nominal
    Assessed --> Degraded : degradationDetected
    Degraded --> Optimizing : operationOptimized
    Optimizing --> Generating : generationCaptured
    Generating --> Reporting : complianceReported
    Reporting --> Generating : generationCaptured
    Generating --> Maintenance : maintenanceRequired
    Maintenance --> Tracking : trackingStarted
```
...
graph TD
    ...
    G -->|Yes| H
    G -->|No| I{Compliance Due?}
    H --> B
    I -->|Yes| J[reportCompliance]
    I -->|No| B
    J --> B
```
