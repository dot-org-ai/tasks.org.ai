...
```mermaid
erDiagram
    Equipment ||--|{ OperatingParameter : "has"
    Equipment ||--o{ Alert : "generates"
    Equipment ||--o{ IncidentReport : "subject of"
    OperatingParameter ||--o{ Alert : "triggers"
    Equipment ||--|{ PerformanceMetric : "measured by"
    Equipment ||--o{ MaintenanceSchedule : "scheduled for"
    IncidentReport }o--|| MaintenanceSchedule : "may trigger"
```
...
stateDiagram-v2
    [*] --> Idle
    Idle --> Active : monitoringStarted
    Active --> Capturing : metricsCaptured
    Capturing --> Evaluating : performanceEvaluated
    Evaluating --> Active : nominal
    Evaluating --> AnomalyFound : anomalyDetected
    AnomalyFound --> Alerting : alertTriggered
    Alerting --> InspectionPending : inspectionScheduled
    Alerting --> Active : resolved
    InspectionPending --> Active : inspectionComplete
    Active --> Idle : monitoringStopped
```
