...
```mermaid
erDiagram
    Equipment ||--|{ Observation : "subject of"
    Equipment ||--|{ EquipmentLog : "tracked in"
    Observation ||--o{ Anomaly : "may detect"
    Anomaly ||--o| WorkOrder : "generates"
    Anomaly }o--|| FaultIndicator : "matches"
    EquipmentLog ||--o{ Observation : "contains"
    FaultIndicator }o--|| Equipment : "associated with"
```
...
stateDiagram-v2
    [*] --> BaselineSet
    BaselineSet --> Observing : observationConducted
    Observing --> Normal : noAnomalyFound
    Normal --> Observing : observationConducted
    Observing --> AnomalyFound : anomalyLogged
    AnomalyFound --> SeverityReviewed : severityAssessed
    SeverityReviewed --> Escalated : anomalyEscalated
    SeverityReviewed --> WorkOrderPending : workOrderCreated
    Escalated --> WorkOrderPending : workOrderCreated
    WorkOrderPending --> Closed : observationClosed
    Closed --> Observing : observationConducted
```
