...
```mermaid
erDiagram
    MarketCondition ||--|{ PricingTrend : "reflected in"
    MarketCondition ||--|{ DemandSignal : "indicated by"
    MarketCondition ||--o{ CompetitiveMove : "influenced by"
    EconomicIndicator }o--|| MarketCondition : "drives"
    MarketCondition ||--o{ MarketForecast : "projected in"
    CompetitiveMove }o--|| PricingTrend : "affects"
```
...
stateDiagram-v2
    [*] --> Tracking
    Tracking --> DataReady : marketDataCollected
    DataReady --> Analyzed : trendsAnalyzed
    Analyzed --> CompetitionReviewed : competitionAssessed
    CompetitionReviewed --> Forecasting : demandForecasted
    Forecasting --> Stable : noShift
    Stable --> Tracking : marketDataCollected
    Forecasting --> ShiftFound : shiftDetected
    ShiftFound --> ActionPending : actionRecommended
    ActionPending --> Tracking : actionTaken
    ShiftFound --> Tracking : shiftResolved
```
