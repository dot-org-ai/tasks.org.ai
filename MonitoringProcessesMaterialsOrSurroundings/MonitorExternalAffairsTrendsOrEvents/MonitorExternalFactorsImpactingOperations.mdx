...
```mermaid
erDiagram
    ExternalFactor ||--|{ FactorMetric : "measured by"
    ExternalFactor ||--o{ ImpactAssessment : "analyzed in"
    ExternalFactor ||--o{ OperationalAlert : "triggers"
    ExternalFactor ||--o{ Forecast : "predicted via"
    ImpactAssessment ||--o| ContingencyPlan : "activates"
    ContingencyPlan }o--|| Forecast : "informed by"
```
...
stateDiagram-v2
    [*] --> Tracking
    Tracking --> DataReady : metricsCollected
    DataReady --> Assessed : impactEvaluated
    Assessed --> Normal : noSignificantImpact
    Normal --> Tracking : metricsCollected
    Assessed --> Predicted : disruptionPredicted
    Predicted --> ContingencyActive : contingencyTriggered
    Predicted --> Adjusted : operationsAdjusted
    ContingencyActive --> Documented : outcomeDocumented
    Adjusted --> Documented : outcomeDocumented
    Documented --> Tracking : metricsCollected
```
