...
```mermaid
erDiagram
    ExternalEvent ||--o{ ImpactAssessment : "analyzed via"
    ExternalEvent }o--|{ EventSource : "reported by"
    ExternalEvent ||--o| RegulatoryChange : "may involve"
    ExternalEvent ||--o{ Alert : "triggers"
    ImpactAssessment ||--o| ResponsePlan : "leads to"
    RegulatoryChange ||--|| ResponsePlan : "requires"
```
...
stateDiagram-v2
    [*] --> Scanning
    Scanning --> EventFound : eventDetected
    Scanning --> Scanning : sourcesScanned
    EventFound --> Assessing : impactAssessed
    Assessing --> Classified : urgencyClassified
    Classified --> Notified : stakeholdersNotified
    Classified --> Scanning : lowPriority
    Notified --> Responding : responseDeveloped
    Responding --> Scanning : responseComplete
    Responding --> Assessing : situationEvolving
```
