...
```mermaid
erDiagram
    SecurityZone ||--o{ AccessCredential : "requires"
    SecurityZone ||--o{ SurveillanceFeed : "covered by"
    SecurityZone ||--o{ PatrolRoute : includes
    AlarmEvent }o--|| SecurityZone : "originates from"
    SecurityIncident ||--|| AlarmEvent : "triggered by"
    SecurityIncident }o--|| SurveillanceFeed : "captured on"
```
...
```mermaid
stateDiagram-v2
    [*] --> Monitoring : alarmTriggered
    Monitoring --> Investigating : accessDenied
    Monitoring --> Granted : accessGranted
    Investigating --> Confirmed : incidentReported
    Confirmed --> Lockdown : zoneLocked
    Investigating --> FalseAlarm : patrolCompleted
    Lockdown --> Reviewing : unauthorizedAccessDetected
    Reviewing --> Monitoring : patrolCompleted
    FalseAlarm --> Monitoring : patrolCompleted
    Granted --> [*]
```
