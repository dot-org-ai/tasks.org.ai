...
```mermaid
erDiagram
    HazardObservation ||--|| ThreatLevel : "classified by"
    HazardObservation }o--|| EnvironmentalReading : "detected via"
    WeatherAlert ||--o{ HazardObservation : triggers
    ThreatLevel ||--o{ ProtectiveAction : determines
    SituationalBrief ||--o{ HazardObservation : summarizes
    ProtectiveAction }o--|| SituationalBrief : "communicated in"
```
...
```mermaid
stateDiagram-v2
    [*] --> Scanning : hazardObserved
    Scanning --> Classified : hazardClassified
    Classified --> Alerted : alertIssued
    Classified --> Halted : operationsHalted
    Alerted --> ActionTaken : protectiveActionRecommended
    Halted --> ActionTaken : protectiveActionRecommended
    ActionTaken --> Scanning : situationalBriefUpdated
    ActionTaken --> Cleared : hazardCleared
    Cleared --> Scanning : hazardObserved
    Cleared --> [*]
```
