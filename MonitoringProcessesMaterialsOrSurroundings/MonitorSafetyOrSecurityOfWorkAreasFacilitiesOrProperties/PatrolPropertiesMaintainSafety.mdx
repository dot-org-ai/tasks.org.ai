...
```mermaid
erDiagram
    PatrolRoute ||--o{ Checkpoint : contains
    PatrolRoute ||--o{ PatrolLog : produces
    Checkpoint ||--o{ PatrolLog : "verified in"
    PatrolLog ||--o{ IncidentReport : generates
    IncidentReport ||--o{ MaintenanceRequest : triggers
    TrespassNotice }o--|| IncidentReport : "documented in"
```
...
stateDiagram-v2
    [*] --> Started : patrolBegun
    Started --> Scanning : checkpointScanned
    Scanning --> IncidentFound : incidentReported
    Scanning --> TrespasserFound : trespassNoticeIssued
    IncidentFound --> MaintenanceNeeded : maintenanceRequested
    TrespasserFound --> Detained : individualDetained
    Detained --> Scanning : checkpointScanned
    MaintenanceNeeded --> Scanning : checkpointScanned
    Scanning --> Completed : patrolCompleted
    Completed --> [*]
```
