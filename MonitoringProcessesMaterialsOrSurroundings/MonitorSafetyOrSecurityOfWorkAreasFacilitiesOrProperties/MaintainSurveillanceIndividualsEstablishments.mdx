...
```mermaid
erDiagram
    SurveillanceOperation ||--o{ ObservationLog : produces
    SurveillanceOperation ||--o{ SurveillanceFootage : captures
    SurveillanceOperation }o--|| SurveillanceAuthorization : "authorized by"
    ObservationLog ||--o{ IntelligenceReport : "feeds into"
    SubjectProfile ||--|| SurveillanceOperation : targets
    SurveillanceFootage }o--|| IntelligenceReport : supports
```
...
```mermaid
stateDiagram-v2
    [*] --> Planned : operationPlanned
    Planned --> Deployed : assetsDeployed
    Deployed --> Observing : observationConducted
    Observing --> EvidenceGathered : evidenceCaptured
    EvidenceGathered --> Analyzing : patternIdentified
    Analyzing --> Observing : observationConducted
    Analyzing --> Reporting : reportCompiled
    Reporting --> Terminated : operationTerminated
    Terminated --> [*]
```
