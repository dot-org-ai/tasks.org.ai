...
```mermaid
erDiagram
    AccessPoint ||--o{ Credential : validates
    AccessPoint ||--o{ AccessLog : records
    OccupancyCount }o--|| CapacityThreshold : "compared against"
    RestrictedZone ||--|{ AccessPoint : "controlled by"
    Credential }o--|| AccessLog : "referenced in"
    RestrictedZone ||--o{ OccupancyCount : tracks
```
...
```mermaid
stateDiagram-v2
    [*] --> Verifying : credentialVerified
    Verifying --> Granted : accessGranted
    Verifying --> Denied : accessDenied
    Granted --> Monitoring : occupancyThresholdReached
    Monitoring --> Restricted : entryRestricted
    Restricted --> Redirected : flowRedirected
    Redirected --> Verifying : credentialVerified
    Denied --> Alert : unauthorizedEntryDetected
    Alert --> [*]
    Granted --> [*]
```
