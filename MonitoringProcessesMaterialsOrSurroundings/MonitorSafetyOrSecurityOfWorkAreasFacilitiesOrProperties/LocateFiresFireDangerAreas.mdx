...
```mermaid
erDiagram
    FireReport ||--o{ FirePerimeter : defines
    FireReport }o--|| WeatherForecast : "influenced by"
    DangerZone ||--o{ FuelLoadAssessment : "assessed with"
    ThermalHotspot ||--|{ FireReport : triggers
    FirePerimeter ||--o{ DangerZone : "may expand into"
    WeatherForecast ||--o{ DangerZone : "contributes to"
```
...
```mermaid
stateDiagram-v2
    [*] --> Detected : fireSightingReported
    Detected --> Assessed : fireRiskAssessed
    Assessed --> Classified : dangerZoneClassified
    Assessed --> Mapped : firePerimeterMapped
    Mapped --> Suppressing : suppressionDispatched
    Suppressing --> Spreading : fireSpreadDetected
    Spreading --> Suppressing : suppressionDispatched
    Suppressing --> Contained : firePerimeterMapped
    Classified --> Warning : dangerWarningIssued
    Contained --> [*]
```
