...
```mermaid
erDiagram
    AlarmSystem ||--o{ Zone : contains
    Zone ||--o{ AlarmSignal : generates
    AlarmSignal ||--|| IncidentReport : "documented in"
    AlarmSystem ||--|| MaintenanceSchedule : "serviced by"
    EscalationRule }o--|| Zone : "configured for"
    IncidentReport }o--|| AlarmSignal : records
```
...
```mermaid
stateDiagram-v2
    [*] --> Monitoring : signalReceived
    Monitoring --> Verifying : alarmVerified
    Monitoring --> FalseAlarm : falseAlarmIdentified
    Verifying --> Dispatched : responseDispatched
    Dispatched --> Resolved : alarmAcknowledged
    Dispatched --> Escalated : alarmEscalated
    Escalated --> Dispatched : responseDispatched
    FalseAlarm --> Reset : systemReset
    Resolved --> Reset : systemReset
    Reset --> [*]
```
