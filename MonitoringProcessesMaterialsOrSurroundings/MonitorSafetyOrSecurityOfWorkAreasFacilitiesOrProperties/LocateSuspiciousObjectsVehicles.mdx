...
```mermaid
erDiagram
    SuspiciousItem ||--|| ThreatAssessment : "evaluated by"
    SuspiciousVehicle ||--|| ThreatAssessment : "evaluated by"
    ThreatAssessment ||--o{ IncidentPerimeter : triggers
    IncidentPerimeter ||--o{ EvacuationOrder : "may require"
    SurveillanceFootage }o--|| SuspiciousItem : documents
    SurveillanceFootage }o--|| SuspiciousVehicle : documents
```
...
```mermaid
stateDiagram-v2
    [*] --> Reported : suspiciousItemReported
    Reported --> Assessed : threatLevelAssessed
    Assessed --> Secured : perimeterEstablished
    Assessed --> Reviewing : surveillanceReviewed
    Reviewing --> Secured : perimeterEstablished
    Reviewing --> Cleared : itemCleared
    Secured --> Evacuating : evacuationInitiated
    Secured --> SpecialistCalled : specialistResponseRequested
    SpecialistCalled --> Cleared : itemCleared
    Evacuating --> SpecialistCalled : specialistResponseRequested
    Cleared --> [*]
```
