...
```mermaid
erDiagram
    ComplianceCheck ||--o{ Deviation : discovers
    Deviation ||--o{ CorrectiveAction : "addressed by"
    ComplianceCheck }o--|| Regulation : "evaluates against"
    ComplianceCheck }o--|| StandardOperatingProcedure : "evaluates against"
    CorrectiveAction ||--|| ComplianceReport : "documented in"
    ComplianceReport }o--|| ComplianceCheck : summarizes
```
...
stateDiagram-v2
    ...
    Scheduled --> InProgress : conductCheck
    InProgress --> Compliant : checkCompleted
    InProgress --> NonCompliant : deviationRecorded
    NonCompliant --> Remediating : correctiveActionAssigned
    Remediating --> Resolved : deviationResolved
    NonCompliant --> Escalated : violationEscalated
    Escalated --> Remediating : correctiveActionAssigned
    Resolved --> Reported : reportGenerated
