...
```mermaid
erDiagram
    Process ||--o{ ControlPoint : contains
    ControlPoint ||--o{ Measurement : captures
    Measurement }o--|| Standard : "compared against"
    Process ||--o{ NonConformance : "may produce"
    NonConformance ||--|{ ImprovementAction : "resolved by"
    Standard ||--o{ ControlPoint : defines
```
...
```mermaid
stateDiagram-v2
    [*] --> Defined : controlPointDefined
    Defined --> Measured : measurementRecorded
    Measured --> Evaluated : conformanceEvaluated
    Evaluated --> Conforming : nonConformanceClosed
    Evaluated --> NonConforming : nonConformanceLogged
    NonConforming --> Improving : improvementInitiated
    Improving --> Measured : measurementRecorded
    Conforming --> Measured : measurementRecorded
    Conforming --> Audited : processAudited
    Audited --> [*]
```
