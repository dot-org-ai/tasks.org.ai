...
```mermaid
erDiagram
    SafetyInspection ||--o{ Hazard : identifies
    Hazard ||--o{ Incident : "may cause"
    Incident ||--|{ CorrectiveMeasure : "resolved by"
    Hazard ||--o{ CorrectiveMeasure : "mitigated by"
    SafetyPolicy ||--o{ SafetyInspection : governs
    TrainingRecord }o--|| SafetyPolicy : "fulfills requirements of"
```
...
```mermaid
stateDiagram-v2
    [*] --> Inspecting : safetyInspectionCompleted
    Inspecting --> HazardFound : hazardIdentified
    Inspecting --> Compliant : safetyComplianceVerified
    HazardFound --> Investigating : incidentReported
    Investigating --> Investigated : incidentInvestigated
    Investigated --> Remediating : correctiveMeasureAssigned
    HazardFound --> Remediating : correctiveMeasureAssigned
    Remediating --> Verified : safetyComplianceVerified
    Verified --> Inspecting : safetyInspectionCompleted
    Verified --> PlanUpdated : emergencyPlanUpdated
    PlanUpdated --> [*]
```
...
graph TD
    ...
    J --> K{Compliance Restored?}
    K -->|Yes| L[updateEmergencyPlan]
    K -->|No| A
```
