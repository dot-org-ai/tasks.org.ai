...
```mermaid
erDiagram
    EnvironmentalPermit ||--o{ InspectionRecord : "evaluated in"
    EnvironmentalPermit ||--o{ EmissionsReport : "reported under"
    InspectionRecord ||--o{ ViolationNotice : "may produce"
    ViolationNotice ||--|| CorrectiveActionPlan : requires
    MonitoringSchedule ||--o{ InspectionRecord : schedules
    CorrectiveActionPlan }o--|| ViolationNotice : resolves
```
...
stateDiagram-v2
    [*] --> Permitted
    Permitted --> Inspected : inspectionCompleted
    Inspected --> Compliant : emissionsDataAnalyzed
    Inspected --> ViolationFound : violationDetected
    ViolationFound --> Notified : violationNoticeIssued
    Notified --> Correcting : correctiveActionRequired
    Correcting --> Resolved : complianceRestored
    Correcting --> Escalated : violationDetected
    Resolved --> Permitted : permitRenewed
    Compliant --> Permitted : permitRenewed
    Escalated --> Correcting : correctiveActionRequired
```
