...
```mermaid
erDiagram
    Treatment ||--|{ VitalSigns : "monitored via"
    Treatment ||--o{ Observation : "produces"
    Treatment ||--|| Protocol : "governed by"
    VitalSigns ||--o{ Alert : "triggers"
    Observation ||--o{ Intervention : "may require"
    Alert }o--|| Intervention : "activates"
    Protocol ||--o{ Alert : "defines thresholds for"
```
...
stateDiagram-v2
    [*] --> TreatmentStarted
    TreatmentStarted --> VitalsMonitored : vitalsRecorded
    VitalsMonitored --> Stable : withinRange
    VitalsMonitored --> AdverseDetected : adverseEventDetected
    Stable --> VitalsMonitored : vitalsRecorded
    AdverseDetected --> Escalated : conditionEscalated
    AdverseDetected --> Adjusted : treatmentAdjusted
    Escalated --> Critical : criticalAlertRaised
    Critical --> Intervened : interventionPerformed
    Intervened --> VitalsMonitored : vitalsRecorded
    Adjusted --> VitalsMonitored : vitalsRecorded
    Stable --> Complete : treatmentFinished
```
