...
```mermaid
erDiagram
    TreatmentPlan ||--|{ TreatmentMilestone : "defines"
    TreatmentPlan ||--o{ ProgressNote : "documented via"
    TreatmentPlan ||--|| BaselineAssessment : "starts with"
    ProgressNote ||--o{ OutcomeMeasure : "includes"
    ProgressNote ||--o{ AdverseReaction : "may document"
    TreatmentMilestone }o--|| OutcomeMeasure : "evaluated by"
    BaselineAssessment ||--o{ OutcomeMeasure : "establishes"
```
...
stateDiagram-v2
    ...
    BaselineRecorded --> InProgress : progressAssessed
    InProgress --> Responding : milestoneReached
    InProgress --> NotResponding : adverseReactionLogged
    Responding --> GoalMet : treatmentGoalMet
    NotResponding --> PlanAdjusted : treatmentPlanAdjusted
    PlanAdjusted --> InProgress : progressAssessed
    Responding --> InProgress : progressAssessed
    GoalMet --> Completed : allGoalsMet
    InProgress --> ConsultReferred : consultationRequested
    ConsultReferred --> PlanAdjusted : treatmentPlanAdjusted
```
...
graph TD
    ...
    E --> H{Specialist Needed?}
    H -->|Yes| I[requestConsultation]
    H -->|No| G
    I --> G
    G --> B
```
