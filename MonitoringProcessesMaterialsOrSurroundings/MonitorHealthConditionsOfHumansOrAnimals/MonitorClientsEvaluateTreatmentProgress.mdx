...
```mermaid
erDiagram
    TreatmentPlan ||--|{ Goal : "defines"
    TreatmentPlan ||--|{ Intervention : "prescribes"
    Goal ||--o{ Outcome : "measured by"
    ProgressNote }o--|| Assessment : "summarizes"
    Assessment ||--o{ Outcome : "produces"
    Intervention }o--|| ProgressNote : "documented in"
    Goal ||--o{ ProgressNote : "tracked via"
```
...
stateDiagram-v2
    [*] --> InTreatment
    InTreatment --> SessionLogged : sessionRecorded
    SessionLogged --> Assessed : progressAssessed
    Assessed --> OnTrack : outcomeMeasured
    Assessed --> BarrierFound : barrierIdentified
    OnTrack --> GoalMet : goalAchieved
    OnTrack --> InTreatment : sessionRecorded
    BarrierFound --> PlanRevised : planUpdated
    PlanRevised --> InTreatment : sessionRecorded
    GoalMet --> ReviewPending : reviewScheduled
    ReviewPending --> Discharged : allGoalsMet
    ReviewPending --> InTreatment : sessionRecorded
```
