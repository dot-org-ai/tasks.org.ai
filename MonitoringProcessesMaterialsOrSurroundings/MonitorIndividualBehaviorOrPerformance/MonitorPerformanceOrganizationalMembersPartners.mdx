...
```mermaid
erDiagram
    PerformanceGoal ||--o{ Metric : "measured by"
    PerformanceGoal ||--o{ Review : "evaluated in"
    Review ||--o{ Feedback : "includes"
    Review ||--o{ DevelopmentPlan : "may recommend"
    Metric ||--o{ Alert : "triggers"
    DevelopmentPlan }o--|| PerformanceGoal : "targets"
    Feedback }o--|| Metric : "references"
```
...
stateDiagram-v2
    [*] --> Tracking
    Tracking --> MetricsLogged : metricTracked
    MetricsLogged --> GoalsReviewed : goalsAssessed
    GoalsReviewed --> OnTrack : targetAchieved
    GoalsReviewed --> GapFound : gapIdentified
    OnTrack --> ReviewPeriod : reviewConducted
    GapFound --> FeedbackGiven : feedbackProvided
    FeedbackGiven --> PlanCreated : planCreated
    PlanCreated --> Tracking : metricTracked
    ReviewPeriod --> Tracking : metricTracked
    OnTrack --> Tracking : metricTracked
```
