...
```mermaid
erDiagram
    Policy ||--o{ Violation : "breached by"
    Violation ||--|| Incident : "originates from"
    Violation ||--o{ Consequence : "results in"
    Consequence ||--o{ Appeal : "may be challenged via"
    Incident ||--o{ InterventionPlan : "may require"
    InterventionPlan }o--|| Violation : "addresses"
    Appeal }o--|| Consequence : "reviews"
```
...
stateDiagram-v2
    [*] --> Observing
    Observing --> ViolationLogged : violationDocumented
    ViolationLogged --> Investigated : incidentInvestigated
    Investigated --> ConsequenceSet : consequenceAssigned
    Investigated --> Dismissed : notConfirmed
    ConsequenceSet --> ParentInformed : parentNotified
    ParentInformed --> PlanCreated : planDeveloped
    ParentInformed --> Monitoring : firstOffense
    PlanCreated --> ComplianceTracked : complianceTracked
    ComplianceTracked --> Observing : improvementSeen
    ComplianceTracked --> Escalated : noImprovement
    ConsequenceSet --> AppealReview : appealProcessed
    AppealReview --> Observing : appealGranted
```
