...
```mermaid
erDiagram
    Activity ||--|{ SupervisionAssignment : "covered by"
    Activity ||--o{ AttendanceRecord : "tracked via"
    Activity ||--|| SafetyProtocol : "governed by"
    SupervisionAssignment }o--|| Observation : "produces"
    Observation ||--o{ Incident : "may document"
    Incident ||--o{ Observation : "investigated via"
    AttendanceRecord }o--|| Activity : "records for"
```
...
stateDiagram-v2
    [*] --> Scheduled
    Scheduled --> Approved : activityApproved
    Approved --> Supervised : supervisionScheduled
    Supervised --> AttendanceTaken : attendanceTaken
    AttendanceTaken --> Observing : observationRecorded
    Observing --> IncidentLogged : incidentReported
    Observing --> ComplianceVerified : complianceDocumented
    IncidentLogged --> Escalated : issueEscalated
    IncidentLogged --> ComplianceVerified : minorIncident
    Escalated --> ComplianceVerified : resolved
    ComplianceVerified --> [*]
```
