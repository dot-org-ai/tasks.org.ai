...
```mermaid
erDiagram
    Observation ||--o{ Issue : "may detect"
    Issue ||--o{ Incident : "may escalate to"
    Issue ||--|| Intervention : "addressed by"
    Intervention ||--|| Resolution : "produces"
    Incident ||--o{ Alert : "triggers"
    Alert }o--|| Intervention : "activates"
    Resolution }o--|| Observation : "documented from"
```
...
stateDiagram-v2
    [*] --> Observing
    Observing --> PatronNoted : patronObserved
    PatronNoted --> IssueFound : issueIdentified
    PatronNoted --> Observing : noIssue
    IssueFound --> SeverityKnown : severityAssessed
    SeverityKnown --> Assisted : assistanceProvided
    SeverityKnown --> Intervened : interventionPerformed
    SeverityKnown --> SecurityAlerted : securityEscalated
    Assisted --> Resolved : resolutionDocumented
    Intervened --> Resolved : resolutionDocumented
    Intervened --> SecurityAlerted : securityEscalated
    SecurityAlerted --> Resolved : resolutionDocumented
    Resolved --> Observing : patronObserved
```
