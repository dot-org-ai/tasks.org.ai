...
```mermaid
erDiagram
    ProductionBatch ||--|{ ProcessMetric : "measured by"
    ProductionBatch ||--o{ QualityTest : "sampled via"
    ProductionBatch ||--o{ ProcessDeviation : "may have"
    ProcessMetric ||--o{ ProcessDeviation : "triggers"
    QualityTest ||--|| YieldReport : "contributes to"
    ProcessDeviation ||--o{ MaintenanceAlert : "may generate"
    YieldReport }o--|| ProductionBatch : "summarizes"
```
...
stateDiagram-v2
    ...
    Started --> MetricsLogged : metricRecorded
    MetricsLogged --> Normal : withinRange
    MetricsLogged --> DeviationFound : deviationDetected
    Normal --> QualityChecked : qualityTestConducted
    DeviationFound --> Alerted : alertGenerated
    Alerted --> Adjusted : processAdjusted
    Adjusted --> MetricsLogged : metricRecorded
    QualityChecked --> YieldDetermined : yieldCalculated
    QualityChecked --> Alerted : qualityFailed
    YieldDetermined --> [*]
```
