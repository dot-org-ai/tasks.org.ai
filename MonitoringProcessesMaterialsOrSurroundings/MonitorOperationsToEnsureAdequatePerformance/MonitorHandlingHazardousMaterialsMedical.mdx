...
```mermaid
erDiagram
    HazardousMaterial ||--|{ WasteContainer : "stored in"
    HazardousMaterial ||--|| TrackingManifest : "tracked via"
    WasteContainer ||--o{ SafetyIncident : "may cause"
    SafetyProtocol ||--o{ SafetyIncident : "breached by"
    TrackingManifest ||--|| DisposalRecord : "closed by"
    SafetyIncident }o--|| TrackingManifest : "documented in"
    WasteContainer }o--|| DisposalRecord : "disposed via"
```
...
```mermaid
stateDiagram-v2
    [*] --> Tracked : materialTracked
    Tracked --> SafetyChecked : safetyVerified
    SafetyChecked --> Inspected : containerInspected
    Inspected --> ManifestUpdated : manifestUpdated
    Inspected --> IncidentLogged : incidentDocumented
    IncidentLogged --> TrainingDone : trainingConducted
    TrainingDone --> Tracked : materialTracked
    ManifestUpdated --> DisposalPending : disposalCoordinated
    DisposalPending --> [*]
```
