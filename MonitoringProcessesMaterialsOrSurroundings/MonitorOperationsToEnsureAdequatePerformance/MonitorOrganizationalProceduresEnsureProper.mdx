...
```mermaid
erDiagram
    OrganizationalProcedure ||--|{ MonitoringPlan : "observed via"
    OrganizationalProcedure ||--o{ ProcedureExecution : "instances of"
    ProcedureExecution ||--o{ NonConformance : "may reveal"
    ProcedureExecution ||--o{ PerformanceMetric : "measured by"
    NonConformance ||--o{ ImprovementRecommendation : "leads to"
    PerformanceMetric ||--o{ ImprovementRecommendation : "informs"
    MonitoringPlan }o--|| PerformanceMetric : "defines"
```
...
stateDiagram-v2
    [*] --> Monitoring
    Monitoring --> ProcedureObserved : procedureMonitored
    ProcedureObserved --> ComplianceChecked : complianceAssessed
    ComplianceChecked --> Compliant : noIssues
    ComplianceChecked --> NonConformant : nonConformanceIdentified
    Compliant --> PerformanceChecked : performanceMeasured
    PerformanceChecked --> Efficient : performanceOK
    PerformanceChecked --> BottleneckFound : bottlenecksAnalyzed
    NonConformant --> Improving : improvementsRecommended
    BottleneckFound --> Improving : improvementsRecommended
    Improving --> Audited : auditReportGenerated
    Efficient --> Audited : auditReportGenerated
    Audited --> Monitoring : procedureMonitored
```
