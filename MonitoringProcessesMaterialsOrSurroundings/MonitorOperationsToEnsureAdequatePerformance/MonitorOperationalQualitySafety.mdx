...
```mermaid
erDiagram
    QualityMetric ||--o{ Inspection : "evaluated via"
    Inspection ||--o{ Hazard : "may identify"
    Inspection ||--o{ SafetyIncident : "may document"
    Hazard ||--o{ CorrectiveAction : "mitigated by"
    SafetyIncident ||--o{ CorrectiveAction : "triggers"
    CorrectiveAction }o--|| ComplianceReport : "documented in"
    QualityMetric }o--|| ComplianceReport : "included in"
```
...
stateDiagram-v2
    [*] --> Monitoring
    Monitoring --> QualityChecked : qualityMonitored
    QualityChecked --> SafetyInspected : safetyInspected
    SafetyInspected --> HazardFound : hazardIdentified
    SafetyInspected --> IncidentLogged : incidentReported
    SafetyInspected --> Compliant : noIssues
    HazardFound --> Correcting : correctiveActionInitiated
    IncidentLogged --> Correcting : correctiveActionInitiated
    Correcting --> ComplianceChecked : complianceAssessed
    ComplianceChecked --> Reported : safetyReportGenerated
    Compliant --> Reported : safetyReportGenerated
    Reported --> Monitoring : qualityMonitored
```
