...
```mermaid
erDiagram
    LoadingPlan ||--|{ LoadingActivity : "specifies"
    LoadingActivity ||--|| WeightDistribution : "verified by"
    LoadingActivity ||--o{ SafetyChecklist : "checked via"
    SafetyChecklist ||--o{ LoadingViolation : "may find"
    LoadingActivity ||--o{ EquipmentLog : "uses"
    WeightDistribution ||--o{ LoadingViolation : "may trigger"
    LoadingViolation }o--|| LoadingActivity : "halts"
```
...
stateDiagram-v2
    [*] --> Loading
    Loading --> ActivityTracked : activityMonitored
    ActivityTracked --> SafetyOK : safetyVerified
    ActivityTracked --> ViolationFound : violationDocumented
    SafetyOK --> WeightOK : weightValidated
    SafetyOK --> ViolationFound : weightFailed
    ViolationFound --> Resequenced : sequenceOptimized
    Resequenced --> Loading : activityMonitored
    WeightOK --> EquipmentLogged : equipmentUsageRecorded
    EquipmentLogged --> Approved : loadApproved
    Approved --> [*]
```
