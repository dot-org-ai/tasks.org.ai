...
```mermaid
erDiagram
    ProcessInspection ||--|{ InspectionCriteria : "evaluated against"
    ProcessInspection ||--o{ Finding : "produces"
    Finding ||--o{ CorrectiveAction : "requires"
    CorrectiveAction }o--|| Finding : "addresses"
    ProcessInspection ||--|| InspectionReport : "documented in"
    InspectionReport ||--o{ TrendAnalysis : "contributes to"
    TrendAnalysis }o--|| Finding : "aggregates"
```
...
stateDiagram-v2
    ...
    Scheduled --> Assessed : assessmentConducted
    Assessed --> FindingsLogged : findingDocumented
    Assessed --> Clean : noFindings
    FindingsLogged --> CorrectionAssigned : correctiveActionAssigned
    CorrectionAssigned --> CorrectionVerified : correctionVerified
    CorrectionVerified --> ReportReady : verified
    CorrectionVerified --> CorrectionAssigned : notVerified
    Clean --> ReportReady : noIssues
    ReportReady --> Published : reportPublished
    Published --> TrendsReviewed : trendsAnalyzed
    TrendsReviewed --> [*]
```
