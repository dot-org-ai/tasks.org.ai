...
erDiagram
    ...
    Account ||--o{ Reconciliation : "verified via"
    Transaction ||--o{ PaymentFlow : "part of"
    Transaction ||--o{ ActivityAlert : "may trigger"
    Reconciliation ||--o{ FinancialReport : "summarized in"
    ActivityAlert }o--|| FinancialReport : "documented in"
```
...
stateDiagram-v2
    [*] --> Active
    Active --> Capturing : transactionsCaptured
    Capturing --> Reconciling : accountsReconciled
    Reconciling --> Verified : complianceVerified
    Verified --> Reported : reportGenerated
    Reported --> Active : nextCycle
    Capturing --> AnomalyFound : anomalyDetected
    AnomalyFound --> Escalated : issueEscalated
    AnomalyFound --> Reconciling : anomalyResolved
    Escalated --> Reconciling : investigationComplete
```
