...
```mermaid
erDiagram
    FinancialIndicator ||--|{ Ratio : "computed as"
    FinancialIndicator ||--|{ Threshold : "bounded by"
    FinancialIndicator ||--o{ Trend : "tracked over"
    FinancialIndicator ||--o{ Alert : "triggers"
    FinancialIndicator ||--o{ BenchmarkComparison : "compared in"
    Threshold }o--|| Alert : "breach causes"
```
...
stateDiagram-v2
    [*] --> Defining
    Defining --> Calculating : indicatorsDefined
    Calculating --> Analyzing : ratiosCalculated
    Analyzing --> Assessed : healthAssessed
    Assessed --> Healthy : withinThresholds
    Assessed --> Degraded : degradationDetected
    Healthy --> Benchmarked : benchmarksCompared
    Degraded --> Benchmarked : benchmarksCompared
    Benchmarked --> Reported : indicatorsReported
    Reported --> Calculating : nextPeriod
    Degraded --> Calculating : correctionApplied
```
