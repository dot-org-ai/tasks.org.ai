...
```mermaid
erDiagram
    Indicator ||--|{ Threshold : "bounded by"
    Indicator ||--o{ PerformanceTrend : "tracked over"
    Indicator ||--o{ Alert : "triggers"
    Indicator ||--o{ BenchmarkComparison : "compared in"
    Dashboard ||--|{ Indicator : "displays"
    Alert }o--|| Threshold : "caused by breach of"
```
...
stateDiagram-v2
    [*] --> Defining
    Defining --> Collecting : indicatorsDefined
    Collecting --> Calculating : dataCollected
    Calculating --> Assessing : metricsCalculated
    Assessing --> OnTarget : performanceAssessed
    Assessing --> VarianceFound : varianceDetected
    OnTarget --> Benchmarked : performanceBenchmarked
    VarianceFound --> Benchmarked : performanceBenchmarked
    Benchmarked --> Displayed : dashboardGenerated
    Displayed --> Collecting : nextCycle
    VarianceFound --> Collecting : varianceResolved
```
