...
```mermaid
erDiagram
    NetworkDevice ||--|{ PerformanceMetric : "emits"
    PerformanceMetric ||--|| PerformanceBaseline : "compared to"
    PerformanceMetric ||--o{ Alert : "triggers"
    NetworkDevice }o--|| NetworkTopology : "mapped in"
    Alert }o--|| PerformanceBaseline : "deviation from"
    PerformanceBaseline ||--o{ CapacityForecast : "projected via"
    NetworkTopology ||--o{ CapacityForecast : "informs"
```
...
stateDiagram-v2
    [*] --> Polling
    Polling --> MetricsReceived : metricsCollected
    MetricsReceived --> BaselineSet : baselineEstablished
    BaselineSet --> Normal : withinBaseline
    BaselineSet --> AnomalyFound : anomalyDetected
    Normal --> Polling : metricsCollected
    AnomalyFound --> Alerting : alertGenerated
    Alerting --> Diagnosed : issueDiagnosed
    Diagnosed --> Optimized : trafficOptimized
    Optimized --> Polling : metricsCollected
    Normal --> Forecasting : capacityForecasted
    Forecasting --> Polling : metricsCollected
```
