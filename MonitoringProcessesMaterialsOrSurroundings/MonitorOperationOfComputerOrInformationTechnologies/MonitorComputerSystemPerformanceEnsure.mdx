...
```mermaid
erDiagram
    SystemMetric ||--o{ PerformanceAlert : "triggers"
    SystemMetric }o--|| ServiceLevelObjective : "measured against"
    HealthCheck ||--o{ PerformanceAlert : "generates"
    PerformanceAlert ||--o{ IncidentTicket : "escalates to"
    IncidentTicket }o--|| CapacityAllocation : "may adjust"
    ServiceLevelObjective ||--o{ IncidentTicket : "breached by"
    CapacityAllocation }o--|| SystemMetric : "affects"
```
...
stateDiagram-v2
    [*] --> Collecting
    Collecting --> MetricsGathered : metricsCollected
    MetricsGathered --> Healthy : healthCheckPassed
    MetricsGathered --> CheckFailed : healthCheckFailed
    Healthy --> Collecting : metricsCollected
    CheckFailed --> AlertActive : alertTriggered
    AlertActive --> Diagnosing : investigating
    Diagnosing --> RootCauseFound : rootCauseIdentified
    RootCauseFound --> Scaled : resourcesScaled
    RootCauseFound --> Optimized : optimizationApplied
    Scaled --> Collecting : metricsCollected
    Optimized --> Collecting : metricsCollected
    Healthy --> Reported : availabilityReported
```
