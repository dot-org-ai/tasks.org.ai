...
```mermaid
erDiagram
    GamingMachine ||--o{ OperationalSession : runs
    GamingMachine ||--|| PayoutConfiguration : configuredWith
    GamingMachine ||--o{ FaultReport : generates
    GamingMachine ||--o{ MeterReading : produces
    GamingMachine ||--|| ComplianceCertificate : certifiedBy
    OperationalSession ||--o{ MeterReading : captures
    OperationalSession ||--o{ FaultReport : logs
```
...
stateDiagram-v2
    ...
    Activated --> Configured : payoutSettingsConfigured
    Configured --> Certified : certificationVerified
    Certified --> Monitoring : equipmentStatusMonitored
    Monitoring --> Faulted : faultReported
    Faulted --> Monitoring : repaired
    Monitoring --> Metered : meterReadingRecorded
    Metered --> Monitoring : equipmentStatusMonitored
    Monitoring --> Deactivated : equipmentDeactivated
    Deactivated --> [*]
```
