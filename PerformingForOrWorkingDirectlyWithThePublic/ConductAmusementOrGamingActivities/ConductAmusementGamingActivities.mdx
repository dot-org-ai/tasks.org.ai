...
```mermaid
erDiagram
    ActivityStation ||--o{ GameSession : hosts
    Activity ||--|| ActivityStation : runsAt
    GameSession ||--o{ Payout : awards
    GameSession ||--|| ActivityLog : records
    GameSession ||--o| ComplianceRecord : verifies
    Activity ||--o{ GameSession : contains
    ActivityStation ||--o{ ComplianceRecord : documents
```
...
stateDiagram-v2
    [*] --> Setup : stationSetup
    Setup --> Open : gameSessionOpened
    Open --> Active : activityConducted
    Active --> Paying : payoutProcessed
    Paying --> Active : activityConducted
    Active --> Disputed : disputeResolved
    Disputed --> Active : activityConducted
    Active --> Closed : gameSessionClosed
    Closed --> Compliant : complianceCheckRecorded
    Compliant --> [*]
```
