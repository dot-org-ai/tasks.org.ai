...
```mermaid
erDiagram
    Production ||--o{ RehearsalSession : schedules
    Production ||--|| Script : based_on
    Production ||--o{ CastAssignment : defines
    Production ||--|| ProductionSchedule : follows
    Production ||--o{ CueSheet : generates
    CastAssignment }o--|| RehearsalSession : participatesIn
    RehearsalSession ||--o{ CueSheet : refines
```
...
stateDiagram-v2
    [*] --> Cast : rolesAssigned
    Cast --> Scheduled : rehearsalScheduled
    Scheduled --> Designed : designElementsCoordinated
    Designed --> Rehearsed : runThroughCompleted
    Rehearsed --> Revised : feedbackIncorporated
    Revised --> Rehearsed : runThroughCompleted
    Rehearsed --> Conflict : conflictDetected
    Conflict --> Scheduled : rehearsalScheduled
    Rehearsed --> Finalized : productionFinalized
    Finalized --> [*]
```
