...
```mermaid
erDiagram
    AuditionCall ||--o{ AuditionSubmission : receives
    AuditionSubmission ||--o| AuditionSlot : scheduledFor
    AuditionSubmission ||--o{ AuditionMaterial : includes
    AuditionSlot ||--o| Callback : generatesCallback
    Callback ||--o| RoleOffer : leadsTo
    AuditionCall ||--o{ AuditionSlot : schedules
    AuditionCall ||--o{ RoleOffer : awards
```
...
stateDiagram-v2
    ...
    Discovered --> Prepared : materialPrepared
    Prepared --> Submitted : applicationSubmitted
    Submitted --> SelfTaped : selfTapeRecorded
    Submitted --> Auditioned : auditionAttended
    SelfTaped --> Auditioned : auditionAttended
    Auditioned --> CalledBack : callbackReceived
    CalledBack --> Auditioned : auditionAttended
    Auditioned --> Offered : offerReceived
    Offered --> [*]
```
