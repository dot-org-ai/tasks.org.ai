...
```mermaid
erDiagram
    CustomerInquiry ||--|| ProductCatalog : references
    CustomerInquiry ||--o| ServiceDescription : about
    CustomerInquiry ||--o| EscalationTicket : escalatedTo
    CustomerInquiry ||--o| ResponseTemplate : answeredWith
    CustomerInquiry ||--|| InquiryTranscript : documented
    CustomerInquiry ||--o| SatisfactionRating : ratedBy
    ProductCatalog ||--o{ ResponseTemplate : supports
```
...
stateDiagram-v2
    ...
    Received --> Classified : questionClassified
    Classified --> Researched : answerResearched
    Classified --> Escalated : inquiryEscalated
    Escalated --> Researched : answerResearched
    Researched --> Delivered : responseDelivered
    Delivered --> FeedbackGathered : feedbackCollected
    FeedbackGathered --> Updated : knowledgeBaseUpdated
    FeedbackGathered --> [*]
    Updated --> [*]
```
