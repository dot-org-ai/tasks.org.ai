...
```mermaid
erDiagram
    Inquiry ||--|| Response : answeredBy
    Inquiry ||--o| KnowledgeArticle : references
    Inquiry ||--|| InteractionRecord : documented
    Inquiry ||--o| ResponseTemplate : usesTemplate
    Response ||--o| SatisfactionRating : ratedWith
    KnowledgeArticle ||--o{ ResponseTemplate : supports
    Inquiry ||--o{ Response : receives
```
...
stateDiagram-v2
    ...
    Received --> Classified : inquiryClassified
    Classified --> Searched : knowledgeSearched
    Searched --> Composed : responseComposed
    Searched --> Escalated : inquiryEscalated
    Escalated --> Composed : responseComposed
    Composed --> Delivered : responseDelivered
    Delivered --> Rated : feedbackCollected
    Rated --> [*]
```
