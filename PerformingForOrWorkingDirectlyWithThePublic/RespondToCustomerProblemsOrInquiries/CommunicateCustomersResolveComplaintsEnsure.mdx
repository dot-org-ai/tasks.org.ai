...
```mermaid
erDiagram
    ComplaintCase ||--o{ ResolutionOffer : proposes
    ComplaintCase ||--|| CommunicationLog : trackedIn
    ComplaintCase ||--o| EscalationRecord : escalates
    ComplaintCase ||--o| FollowUpRecord : followedUp
    ResolutionOffer ||--o| FollowUpRecord : confirmedBy
    SatisfactionSurvey ||--|| ComplaintCase : evaluates
    SatisfactionSurvey ||--|| SatisfactionScore : produces
```
...
```mermaid
stateDiagram-v2
    [*] --> Acknowledged : complaintAcknowledged
    Acknowledged --> Investigated : issueInvestigated
    Investigated --> Offered : resolutionOffered
    Offered --> Remedied : remedyProcessed
    Offered --> Escalated : complaintEscalated
    Escalated --> Offered : resolutionOffered
    Remedied --> FollowedUp : followUpCompleted
    FollowedUp --> Surveyed : surveyReceived
    Surveyed --> [*]
```
