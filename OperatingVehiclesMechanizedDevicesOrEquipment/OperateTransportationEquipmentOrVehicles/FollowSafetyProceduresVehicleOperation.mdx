...
```mermaid
erDiagram
    SafetyInspection ||--|| SafetyProtocol : "evaluated against"
    SafetyInspection ||--o{ ViolationRecord : "may produce"
    ViolationRecord ||--o{ IncidentReport : "escalated to"
    ComplianceLog ||--o{ SafetyInspection : aggregates
    SafetyCertification }o--|| ComplianceLog : "required for"
    IncidentReport }o--|| SafetyProtocol : "triggers update to"
```
...
```mermaid
stateDiagram-v2
    [*] --> Certifying : certificationVerified
    Certifying --> Inspecting : preTripInspectionCompleted
    Inspecting --> Compliant : safetyComplianceDocumented
    Inspecting --> Violation : safetyViolationReported
    Compliant --> Operating : safetyComplianceDocumented
    Operating --> IncidentOccurred : incidentResponseConducted
    IncidentOccurred --> Violation : safetyViolationReported
    Violation --> ProtocolUpdate : safetyProtocolUpdated
    ProtocolUpdate --> Inspecting : preTripInspectionCompleted
    Operating --> Reported : complianceReportGenerated
    Reported --> [*]
```
