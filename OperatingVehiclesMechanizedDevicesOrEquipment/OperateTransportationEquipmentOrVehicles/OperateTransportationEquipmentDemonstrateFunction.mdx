...
```mermaid
erDiagram
    TestRequest ||--o{ DiagnosticSession : initiates
    TestRequest ||--|| EquipmentRecord : references
    DiagnosticSession ||--o{ ObservationLog : produces
    DiagnosticSession ||--o{ FaultCode : retrieves
    DiagnosticSession ||--|| TestRoute : follows
    TestRequest ||--o| RepairVerification : validates
    EquipmentRecord ||--o{ FaultCode : accumulates
```
...
```mermaid
stateDiagram-v2
    [*] --> Requested : testRequestInitiated
    Requested --> Inspecting : preOperationCheckCompleted
    Inspecting --> Operating : equipmentOperated
    Operating --> Documenting : observationsRecorded
    Documenting --> Analyzing : faultCodesRetrieved
    Analyzing --> Operating : additional tests needed
    Analyzing --> Verifying : repairVerified
    Verifying --> Reported : findingsReported
    Reported --> [*]
```
