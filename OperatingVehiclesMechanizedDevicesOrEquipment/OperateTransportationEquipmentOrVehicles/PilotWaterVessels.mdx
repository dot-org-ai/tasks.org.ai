...
```mermaid
erDiagram
    VoyagePlan ||--o{ PilotageOrder : "requires"
    VoyagePlan ||--|{ NavigationChart : "references"
    VoyagePlan ||--o{ TidalSchedule : "incorporates"
    VoyagePlan ||--|| BerthAssignment : "targets"
    PilotageOrder ||--|| VesselLog : "generates"
    VesselLog ||--o{ PilotReport : "summarizedIn"
    BerthAssignment }o--|| VesselLog : "recordedIn"
```
...
stateDiagram-v2
    [*] --> Planning
    Planning --> PilotageRequested : voyagePlanned
    PilotageRequested --> PilotAboard : vesselBoarded
    PilotAboard --> BridgeBriefed : bridgeExchangeCompleted
    BridgeBriefed --> InTransit : navigateWaterway
    InTransit --> InTransit : waypointPassed
    InTransit --> Docked : vesselDocked
    Docked --> Reported : pilotReportCompleted
    Reported --> [*]
    InTransit --> BridgeBriefed : weatherHold
```
...
graph TD
    ...
    F -->|No| H{Safe to Dock?}
    G --> H
    H -->|No| I[Hold Position]
    I --> E
    H -->|Yes| J[Dock]
    J --> K[completePilotReport]
```
