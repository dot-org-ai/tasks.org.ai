...
```mermaid
erDiagram
    Train ||--|| Consist : "composed of"
    Train ||--o{ MovementAuthority : "governed by"
    Train ||--o{ SignalIndication : observes
    MovementAuthority }o--|| TrainOrder : "issued via"
    Train ||--|| TripReport : "documented in"
    SignalIndication }o--|| MovementAuthority : "enforces"
```
...
stateDiagram-v2
    [*] --> Crewed : crewAssigned
    Crewed --> Inspected : consistInspected
    Inspected --> Authorized : movementAuthorityObtained
    Authorized --> Moving : trainOperated
    Moving --> SignalCheck : signalObeyed
    SignalCheck --> Moving : trainOperated
    Moving --> Reporting : positionReported
    Reporting --> Moving : trainOperated
    Moving --> Arrived : tripReportCompleted
    Arrived --> [*]
```
