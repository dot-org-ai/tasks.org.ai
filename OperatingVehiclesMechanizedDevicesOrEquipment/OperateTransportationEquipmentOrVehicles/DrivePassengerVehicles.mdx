...
```mermaid
erDiagram
    Vehicle ||--o{ TripRecord : "used in"
    Vehicle ||--|| InspectionChecklist : "checked with"
    TripRecord ||--o{ FuelRecord : "may include"
    DrivingLog ||--o{ TripRecord : aggregates
    Vehicle ||--o{ FuelRecord : consumes
    InspectionChecklist }o--|| TripRecord : "precedes"
```
...
```mermaid
stateDiagram-v2
    [*] --> CheckedOut : vehicleCheckedOut
    CheckedOut --> Inspected : preTripInspectionCompleted
    Inspected --> Driving : drivingStarted
    Driving --> MileageLogged : mileageRecorded
    MileageLogged --> Stopped : drivingStopped
    Stopped --> Refueled : vehicleRefueled
    Stopped --> Returned : vehicleReturned
    Refueled --> Returned : vehicleReturned
    Inspected --> CheckedOut : preTripInspectionCompleted
    Returned --> [*]
```
