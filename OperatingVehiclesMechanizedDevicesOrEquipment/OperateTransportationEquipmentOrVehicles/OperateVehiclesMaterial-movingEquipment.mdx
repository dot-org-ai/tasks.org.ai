...
```mermaid
erDiagram
    OperatorAssignment ||--|| EquipmentUnit : assigns
    OperatorAssignment ||--|| CertificationRecord : requires
    OperatorAssignment ||--o{ PreTripInspection : undergoes
    OperatorAssignment ||--o{ LoadTicket : processes
    LoadTicket ||--o{ OperationLog : records
    EquipmentUnit ||--o{ IncidentReport : generates
    EquipmentUnit ||--o{ OperationLog : tracks
```
...
stateDiagram-v2
    ...
    Assigned --> Inspected : preTripInspectionCompleted
    Inspected --> Loading : loadTicketAccepted
    Loading --> Operating : equipmentOperated
    Operating --> Positioned : materialPositioned
    Positioned --> Loading : loadTicketAccepted
    Positioned --> Logged : operationHoursLogged
    Operating --> Incident : incidentReported
    Incident --> [*]
    Logged --> [*]
```
