...
erDiagram
    ...
    TrafficSignal }o--|| ControlZone : "belongs to"
    ControlZone ||--o{ EquipmentAlert : monitors
    IncidentOverride }o--|| TrafficSignal : "applied to"
    OperationLog ||--o{ IncidentOverride : records
    EquipmentAlert ||--|| OperationLog : "logged in"
```
...
```mermaid
stateDiagram-v2
    [*] --> Configured : timingPlanConfigured
    Configured --> Active : controlModeActivated
    Active --> Monitoring : trafficFlowMonitored
    Monitoring --> Overridden : overrideIssued
    Overridden --> Monitoring : normalOperationRestored
    Monitoring --> AlertRaised : equipmentAlertReported
    AlertRaised --> Monitoring : normalOperationRestored
    Monitoring --> Logged : operationLogGenerated
    Logged --> [*]
```
