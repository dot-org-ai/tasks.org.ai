...
```mermaid
erDiagram
    AdvocacyCase ||--|| NeedsAssessment : "based on"
    AdvocacyCase ||--o{ TestimonialRecord : "supported by"
    AdvocacyCase ||--|| PolicyBrief : "summarized in"
    AdvocacyCase ||--o{ Petition : "backed by"
    AdvocacyCase ||--o{ ActionItem : "results in"
    ActionItem }o--|| ImpactReport : "measured in"
```
...
stateDiagram-v2
    [*] --> NeedsIdentified
    NeedsIdentified --> CaseBuilding : needsAssessed
    CaseBuilding --> BriefReady : caseBuilt
    BriefReady --> BriefDrafted : policyBriefDrafted
    BriefDrafted --> CommunityEngaged : communityMobilized
    CommunityEngaged --> PolicymakerContact : policymakerEngaged
    PolicymakerContact --> CommitmentMade : commitmentTracked
    PolicymakerContact --> CommunityEngaged : communityMobilized
    CommitmentMade --> ImpactMeasured : impactReported
    CommitmentMade --> PolicymakerContact : commitmentTracked
    ImpactMeasured --> [*]
```
...
graph TD
    ...
    F -->|No| D
    G --> H{Commitment Fulfilled?}
    H -->|Yes| I[reportImpact]
    H -->|No| E
```
