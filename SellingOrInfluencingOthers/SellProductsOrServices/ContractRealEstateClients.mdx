...
```mermaid
erDiagram
    PropertyListing ||--o{ PurchaseOffer : receives
    BuyerProfile ||--o{ PurchaseOffer : submits
    PurchaseOffer ||--o{ CounterOffer : "responded with"
    PurchaseOffer ||--o| PurchaseAgreement : "becomes"
    PurchaseAgreement ||--|| InspectionReport : "verified by"
    PurchaseAgreement ||--|| AppraisalReport : "valued by"
    PurchaseAgreement ||--|| ClosingStatement : "settled in"
```
...
stateDiagram-v2
    [*] --> Listed
    Listed --> BuyerReady : buyerQualified
    BuyerReady --> ShowingSet : showingScheduled
    ShowingSet --> OfferIn : offerSubmitted
    OfferIn --> Negotiating : termsNegotiated
    Negotiating --> OfferIn : offerSubmitted
    Negotiating --> UnderContract : contractExecuted
    UnderContract --> DueDiligenceDone : dueDiligenceCompleted
    DueDiligenceDone --> Negotiating : termsNegotiated
    DueDiligenceDone --> Closed : transactionClosed
    Closed --> [*]
```
