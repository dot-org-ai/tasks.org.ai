...
```mermaid
erDiagram
    ProductListing ||--o{ PriceQuote : "quoted as"
    PriceQuote ||--o| SalesOrder : "confirmed as"
    SalesOrder ||--|| QualityCertificate : "certified by"
    SalesOrder ||--|| InvoiceRecord : "billed via"
    ProductListing }o--|| HarvestReport : "sourced from"
    ProductListing ||--o{ FuturesContract : "hedged by"
    MarketAnalysis }o--|| ProductListing : "informs pricing"
```
...
stateDiagram-v2
    [*] --> Listed
    Listed --> Quoted : quoteGenerated
    Quoted --> Negotiating : termsNegotiated
    Negotiating --> Quoted : quoteGenerated
    Negotiating --> OrderConfirmed : orderConfirmed
    OrderConfirmed --> QualityVerified : qualityCertified
    QualityVerified --> Shipped : shipmentArranged
    Shipped --> Paid : paymentProcessed
    Paid --> MarketReviewed : marketAnalyzed
    MarketReviewed --> [*]
```
