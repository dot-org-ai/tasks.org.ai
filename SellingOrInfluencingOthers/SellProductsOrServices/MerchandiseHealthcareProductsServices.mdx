...
erDiagram
    ...
    Campaign ||--o{ Placement : "displayed via"
    Campaign ||--o{ SampleAllocation : distributes
    Campaign ||--|| ComplianceReview : "verified by"
    Campaign ||--|| SalesReport : "measured in"
    Product ||--o{ TrainingSession : "taught in"
```
...
stateDiagram-v2
    [*] --> Planned
    Planned --> ComplianceCleared : complianceReviewed
    ComplianceCleared --> Placed : placementArranged
    ComplianceCleared --> Planned : campaignPlanned
    Placed --> SamplesOut : samplesDistributed
    SamplesOut --> StaffReady : staffTrained
    StaffReady --> Tracking : performanceTracked
    Tracking --> PriceAdjusted : pricingUpdated
    PriceAdjusted --> Tracking : performanceTracked
    Tracking --> [*]
```
