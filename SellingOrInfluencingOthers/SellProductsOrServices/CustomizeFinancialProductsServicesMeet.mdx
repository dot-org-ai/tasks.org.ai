...
```mermaid
erDiagram
    ClientProfile ||--|| NeedsAssessment : "evaluated in"
    NeedsAssessment ||--|| RiskAssessment : "informs"
    RiskAssessment ||--|| ProductConfiguration : "shapes"
    ProductConfiguration ||--|| ComplianceReview : "verified by"
    ProductConfiguration ||--|| Proposal : "presented in"
    Proposal ||--o| ServiceAgreement : "formalized as"
    ServiceAgreement ||--o{ PortfolioAllocation : "allocates"
```
...
stateDiagram-v2
    [*] --> NeedsEvaluated
    NeedsEvaluated --> RiskProfiled : riskAnalyzed
    RiskProfiled --> Configured : productConfigured
    Configured --> ComplianceChecked : complianceReviewed
    ComplianceChecked --> Configured : productConfigured
    ComplianceChecked --> ProposalReady : proposalGenerated
    ProposalReady --> Presented : solutionPresented
    Presented --> Finalized : agreementFinalized
    Presented --> Configured : productConfigured
    Finalized --> UnderReview : portfolioReviewed
    UnderReview --> Configured : productConfigured
    UnderReview --> [*]
```
