...
```mermaid
erDiagram
    Campaign ||--|{ EducationalMaterial : produces
    Campaign ||--o{ OutreachEvent : hosts
    Campaign ||--o{ AdoptionPledge : collects
    Campaign ||--o{ ImpactMetric : tracks
    Campaign ||--o{ PartnershipAgreement : "supported by"
    Campaign ||--o{ GrantApplication : "funded via"
```
...
stateDiagram-v2
    [*] --> Designed
    Designed --> Partnered : stakeholdersEngaged
    Partnered --> MaterialsReady : materialsCreated
    MaterialsReady --> EventsActive : outreachEventHosted
    EventsActive --> PledgesGathered : pledgesCollected
    PledgesGathered --> ImpactTracked : impactMeasured
    PledgesGathered --> EventsActive : outreachEventHosted
    ImpactTracked --> Reported : outcomesReported
    ImpactTracked --> EventsActive : outreachEventHosted
    Reported --> [*]
```
