...
```mermaid
erDiagram
    PromotionalCampaign ||--o{ OutreachEvent : hosts
    PromotionalCampaign ||--o{ HuntingProgram : promotes
    PromotionalCampaign ||--o{ AgriculturalProgram : promotes
    PromotionalCampaign ||--|{ PromotionalMaterial : uses
    OutreachEvent ||--o{ ParticipationRecord : generates
    PromotionalCampaign ||--|| ImpactReport : "measured by"
```
...
stateDiagram-v2
    [*] --> Developed
    Developed --> MaterialsReady : materialsCreated
    MaterialsReady --> CommunityContacted : communityEngaged
    CommunityContacted --> EventScheduled : fieldEventHosted
    EventScheduled --> Registering : participantsRegistered
    Registering --> Measuring : participationMeasured
    Registering --> EventScheduled : fieldEventHosted
    Measuring --> Reported : outcomesReported
    Measuring --> CommunityContacted : communityEngaged
    Reported --> [*]
```
