...
```mermaid
erDiagram
    PromotionalCampaign ||--|{ PressRelease : distributes
    PromotionalCampaign ||--o{ SocialPost : publishes
    PromotionalCampaign ||--o{ BrandAsset : uses
    PromotionalCampaign ||--o{ MediaCoverage : generates
    PromotionalCampaign ||--o{ SponsorshipAgreement : "funded by"
    PromotionalCampaign ||--|{ EngagementMetric : "measured by"
```
...
stateDiagram-v2
    [*] --> StrategySet
    StrategySet --> ContentReady : contentCreated
    ContentReady --> SponsorshipsLocked : sponsorshipSecured
    SponsorshipsLocked --> PressDistributed : pressReleaseDistributed
    PressDistributed --> CampaignLive : campaignExecuted
    CampaignLive --> Monitoring : engagementMonitored
    Monitoring --> CampaignLive : campaignExecuted
    Monitoring --> ROICalculated : roiReported
    ROICalculated --> [*]
```
