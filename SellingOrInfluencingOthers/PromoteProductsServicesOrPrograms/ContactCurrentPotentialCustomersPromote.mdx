...
```mermaid
erDiagram
    OutreachCampaign ||--|{ ContactList : targets
    ContactList ||--|{ ProspectProfile : contains
    OutreachCampaign ||--o{ ContactAttempt : generates
    ContactAttempt ||--o| ResponseRecord : "results in"
    ResponseRecord ||--o| FollowUpTask : triggers
    OutreachCampaign ||--|| CampaignReport : "summarized in"
```
...
stateDiagram-v2
    [*] --> ListBuilt
    ListBuilt --> CampaignActive : campaignLaunched
    CampaignActive --> ContactMade : contactInitiated
    ContactMade --> MessageSent : messageDelivered
    MessageSent --> ResponseLogged : responseRecorded
    ResponseLogged --> FollowUpPlanned : followUpScheduled
    FollowUpPlanned --> ContactMade : contactInitiated
    ResponseLogged --> CampaignActive : contactInitiated
    CampaignActive --> Reported : performanceReported
    Reported --> [*]
```
...
graph TD
    ...
    F -->|No| H{More Contacts?}
    G --> C
    H -->|Yes| C
    H -->|No| I[reportPerformance]
```
