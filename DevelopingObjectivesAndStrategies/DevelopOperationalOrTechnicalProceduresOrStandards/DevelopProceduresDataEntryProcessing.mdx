...
```mermaid
erDiagram
    EntryProcedure ||--o{ InputTemplate : uses
    EntryProcedure ||--o{ ValidationRule : applies
    EntryProcedure ||--o{ QualityCheckpoint : includes
    EntryProcedure ||--o{ BatchJob : schedules
    ValidationRule ||--o{ ErrorLog : produces
    QualityCheckpoint ||--o{ ErrorLog : identifies
```
...
stateDiagram-v2
    [*] --> FormatDef
    FormatDef --> Validation : inputFormatDefined
    Validation --> WorkflowDesign : validationRulesCreated
    WorkflowDesign --> ErrorConfig : workflowDesigned
    ErrorConfig --> Scheduling : errorHandlingConfigured
    Scheduling --> Documentation : batchProcessingScheduled
    Documentation --> Audit : procedureDocumented
    Audit --> Validation : Quality Issues
    Audit --> [*]
```
