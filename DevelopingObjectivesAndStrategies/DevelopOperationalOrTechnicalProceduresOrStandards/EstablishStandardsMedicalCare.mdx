...
```mermaid
erDiagram
    CareStandard ||--o{ ClinicalProtocol : implements
    CareStandard ||--o{ QualityMetric : measures
    CareStandard ||--o{ EvidenceReview : basedOn
    CareStandard ||--o{ ComplianceReport : validates
    CareStandard ||--o{ RevisionRecord : tracks
    QualityMetric ||--o{ ComplianceReport : evaluates
```
...
stateDiagram-v2
    ...
    Drafting --> Approval : standardDrafted
    Approval --> Publication : standardApproved
    Publication --> Monitoring : standardPublished
    Monitoring --> Revision : complianceAudited
    Revision --> Approval : standardRevised
    Monitoring --> Retired : standardRetired
    Retired --> [*]
```
