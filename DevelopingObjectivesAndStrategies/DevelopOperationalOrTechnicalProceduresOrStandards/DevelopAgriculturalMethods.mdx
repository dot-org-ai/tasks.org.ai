...
```mermaid
erDiagram
    AgriculturalMethod ||--o{ FieldTrial : validates
    FieldTrial ||--o{ CropProtocol : tests
    FieldTrial ||--o{ SoilAnalysis : requires
    FieldTrial ||--o{ YieldRecord : produces
    AgriculturalMethod ||--o{ AdoptionPlan : guides
    CropProtocol ||--o{ YieldRecord : generates
```
...
stateDiagram-v2
    [*] --> Research
    Research --> Design : techniqueResearched
    Design --> Testing : methodDesigned
    Testing --> SoilEval : fieldTrialConducted
    SoilEval --> Measurement : soilAnalyzed
    Measurement --> Review : yieldMeasured
    Review --> Design : Failed
    Review --> Documentation : Validated
    Documentation --> Adoption : methodDocumented
    Adoption --> [*] : adoptionScaled
```
