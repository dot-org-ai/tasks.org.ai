...
```mermaid
erDiagram
    EvaluationProcedure ||--o{ PerformanceMetric : defines
    EvaluationProcedure ||--o{ AssessmentSchedule : follows
    PerformanceMetric ||--o{ Benchmark : compares
    EvaluationProcedure ||--o{ EvaluationReport : generates
    EvaluationReport ||--o{ CorrectiveAction : triggers
    PerformanceMetric ||--o{ CorrectiveAction : identifies
```
...
stateDiagram-v2
    [*] --> MetricsDef
    MetricsDef --> Assessment : metricsDefined
    Assessment --> Scheduling : assessmentDesigned
    Scheduling --> Execution : cycleScheduled
    Execution --> Analysis : evaluationConducted
    Analysis --> Reporting : findingsAnalyzed
    Reporting --> Action : reportPublished
    Action --> Execution : correctiveActionAssigned
```
