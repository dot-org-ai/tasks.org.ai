...
```mermaid
erDiagram
    ProductConcept ||--o{ FeasibilityStudy : evaluates
    ProductConcept ||--o{ Prototype : generates
    Prototype ||--o{ BillOfMaterials : specifies
    Prototype ||--o{ ProductionMethod : validates
    ProductionMethod ||--o{ LaunchPlan : enables
    BillOfMaterials ||--o{ ProductionMethod : guides
```
...
stateDiagram-v2
    [*] --> Ideation
    Ideation --> Evaluation : conceptGenerated
    Evaluation --> Archived : Not Viable
    Evaluation --> Design : feasibilityAssessed
    Design --> Prototyping : productDesigned
    Prototyping --> Testing : prototypeBuilt
    Testing --> Design : Failed
    Testing --> Engineering : productValidated
    Engineering --> Market : productionEngineered
    Market --> [*] : productLaunched
```
