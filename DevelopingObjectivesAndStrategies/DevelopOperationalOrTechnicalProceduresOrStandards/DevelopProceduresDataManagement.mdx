...
```mermaid
erDiagram
    GovernanceFramework ||--o{ DataProcedure : organizes
    DataProcedure ||--o{ ClassificationPolicy : implements
    DataProcedure ||--o{ RetentionSchedule : enforces
    ClassificationPolicy ||--o{ AccessControlMatrix : defines
    DataProcedure ||--o{ DataQualityRule : applies
    RetentionSchedule ||--o{ DataQualityRule : validates
```
...
stateDiagram-v2
    [*] --> Classification
    Classification --> Retention : classificationDefined
    Retention --> AccessControl : retentionPolicyCreated
    AccessControl --> Quality : accessControlsDesigned
    Quality --> Documentation : dataQualityValidated
    Documentation --> Approval : procedureDocumented
    Approval --> Review : procedureApproved
    Review --> Quality : Updates Needed
    Review --> [*]
```
