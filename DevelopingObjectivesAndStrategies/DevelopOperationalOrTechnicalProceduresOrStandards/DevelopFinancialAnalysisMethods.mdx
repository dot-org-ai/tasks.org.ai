...
```mermaid
erDiagram
    AnalyticalMethod ||--o{ FinancialModel : implements
    FinancialModel ||--o{ ValuationFramework : applies
    AnalyticalMethod ||--o{ RiskMetric : calculates
    FinancialModel ||--o{ BacktestResult : produces
    MethodDocumentation ||--o{ AnalyticalMethod : describes
    BacktestResult ||--o{ RiskMetric : evaluates
```
...
stateDiagram-v2
    [*] --> NeedIdentification
    NeedIdentification --> Design : analyticalNeedIdentified
    Design --> Modeling : methodDesigned
    Modeling --> Testing : modelBuilt
    Testing --> Design : Unreliable
    Testing --> Documentation : methodBacktested
    Documentation --> Deployment : methodDocumented
    Deployment --> Review : methodDeployed
    Review --> Design : methodPerformanceReviewed
    Review --> [*]
```
