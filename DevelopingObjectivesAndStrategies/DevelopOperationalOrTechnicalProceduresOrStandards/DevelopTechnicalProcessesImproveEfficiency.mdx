...
```mermaid
erDiagram
    FeedstockProfile ||--o{ ConversionProcess : enables
    ConversionProcess ||--o{ CatalystFormulation : uses
    ConversionProcess ||--o{ PilotRun : validates
    PilotRun ||--o{ EfficiencyMetric : measures
    ConversionProcess ||--o{ ScaleUpPlan : guides
    EfficiencyMetric ||--o{ ScaleUpPlan : informs
```
...
stateDiagram-v2
    [*] --> Feedstock
    Feedstock --> Design : feedstockAnalyzed
    Design --> CatalystSelect : conversionProcessDesigned
    CatalystSelect --> Pilot : catalystSelected
    Pilot --> Measurement : pilotTestCompleted
    Measurement --> Optimization : Below Target
    Optimization --> Pilot : parametersOptimized
    Measurement --> Scaling : efficiencyMeasured
    Scaling --> [*] : processScaled
```
