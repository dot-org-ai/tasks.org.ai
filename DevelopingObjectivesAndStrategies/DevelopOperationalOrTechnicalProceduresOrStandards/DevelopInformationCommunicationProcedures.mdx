...
```mermaid
erDiagram
    CommunicationProcedure ||--o{ MessageTemplate : defines
    CommunicationProcedure ||--o{ CommunicationChannel : uses
    CommunicationProcedure ||--o{ EscalationPath : includes
    MessageTemplate ||--o{ DistributionList : targets
    CommunicationChannel ||--o{ RetentionRule : enforces
    EscalationPath ||--o{ DistributionList : notifies
```
...
stateDiagram-v2
    [*] --> Designing
    Designing --> TemplateCreation : channelsDefined
    TemplateCreation --> Escalation : templateCreated
    Escalation --> Distribution : escalationMapped
    Distribution --> Published : distributionConfigured
    Published --> Review : procedurePublished
    Review --> Revision : effectivenessReviewed
    Revision --> Published : procedureUpdated
    Review --> [*]
```
