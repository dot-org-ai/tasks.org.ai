...
```mermaid
erDiagram
    Vehicle ||--o{ ServiceAppointment : "scheduled for"
    Vehicle ||--o{ ServiceRecord : "tracked in"
    Vehicle ||--|| ServiceInterval : "follows"
    ServiceAppointment ||--|| MaintenanceChecklist : "uses"
    Vehicle ||--o{ FluidLevel : "measured by"
    ServiceRecord ||--o{ FluidLevel : "documents"
    MaintenanceChecklist ||--o{ ServiceRecord : "produces"
```
...
stateDiagram-v2
    [*] --> DueForService
    DueForService --> Scheduled : serviceScheduled
    Scheduled --> Inspecting : multiPointInspectionCompleted
    Inspecting --> FluidService : fluidsAndFiltersChanged
    FluidService --> BrakeAndTireCheck : brakesAndTiresInspected
    BrakeAndTireCheck --> Adjusting : minorAdjustmentsPerformed
    BrakeAndTireCheck --> Documenting : no adjustments needed
    Adjusting --> Documenting : serviceCompletionRecorded
    Documenting --> OwnerNotified : ownerNotified
    OwnerNotified --> InService
    InService --> DueForService : interval reached
    InService --> [*]
```
