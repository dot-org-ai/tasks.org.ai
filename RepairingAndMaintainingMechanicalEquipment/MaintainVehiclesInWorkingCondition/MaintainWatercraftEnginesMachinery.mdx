...
```mermaid
erDiagram
    Watercraft ||--|{ MarineEngine : "powered by"
    Watercraft ||--|| MaintenanceLog : "tracked in"
    Watercraft ||--o{ HaulOutSchedule : "scheduled for"
    Watercraft ||--o{ ClassCertificate : "certified by"
    Watercraft ||--o{ CorrosionReport : "assessed in"
    MarineEngine ||--o{ MaintenanceLog : "serviced in"
    CorrosionReport ||--o{ HaulOutSchedule : "triggers"
```
...
stateDiagram-v2
    [*] --> Operational
    Operational --> Inspecting : marineSystemsInspected
    Inspecting --> CorrosionRepair : corrodedComponentsReplaced
    Inspecting --> EngineService : engineServicePerformed
    CorrosionRepair --> EngineService : engineServicePerformed
    EngineService --> SafetyTest : safetyEquipmentTested
    SafetyTest --> LogUpdate : maintenanceLogUpdated
    SafetyTest --> SafetyRepair : test failed
    SafetyRepair --> SafetyTest : retest
    LogUpdate --> ClassSurvey : classCertificationRenewed
    LogUpdate --> Operational : no survey due
    ClassSurvey --> Operational : certified
    Operational --> HaulOut : haulOutScheduled
    HaulOut --> Inspecting : haul-out complete
```
