...
```mermaid
erDiagram
    ComputerAsset ||--|{ SoftwareInstallation : "runs"
    ComputerAsset ||--o{ MaintenanceTicket : "subject of"
    SoftwareInstallation ||--o{ Patch : "receives"
    SoftwareInstallation ||--|| License : "governed by"
    MaintenanceTicket }o--|| ServiceContract : "covered by"
    License ||--o{ ServiceContract : "included in"
    ComputerAsset ||--o{ Patch : "deployed to"
```
...
stateDiagram-v2
    [*] --> Inventoried
    Inventoried --> Diagnosed : diagnosticsCompleted
    Diagnosed --> Patching : softwarePatched
    Diagnosed --> Replacing : hardwareReplaced
    Diagnosed --> Healthy : no issues
    Patching --> Healthy : softwarePatched
    Patching --> Diagnosed : patch failed
    Replacing --> Healthy : hardwareReplaced
    Healthy --> Scheduled : maintenanceScheduled
    Scheduled --> Inventoried : maintenance cycle
    Healthy --> LicenseReview : licenseRenewed
    LicenseReview --> Inventoried : renewal complete
    Healthy --> Decommissioned : assetDecommissioned
    Decommissioned --> [*]
```
