...
erDiagram
    ...
    LaboratoryInstrument ||--o{ PerformanceQualification : "verified by"
    LaboratoryInstrument ||--|| MaintenanceLog : "tracked in"
    MaintenanceSchedule ||--o{ ConsumablesInventory : "requires"
    CalibrationRecord ||--o{ PerformanceQualification : "validated by"
    MaintenanceLog ||--o{ CalibrationRecord : "references"
```
...
stateDiagram-v2
    ...
    Scheduled --> UnderMaintenance : maintenancePerformed
    UnderMaintenance --> Calibrating : instrumentCalibrated
    Calibrating --> Qualifying : performanceQualified
    Qualifying --> Documented : maintenanceDocumented
    Qualifying --> Calibrating : out of tolerance
    Documented --> InService : compliant
    Documented --> UnderMaintenance : gaps found
    InService --> Overdue : maintenanceOverdue
    Overdue --> Scheduled : rescheduled
    InService --> ConsumableReorder : consumablesLow
    ConsumableReorder --> InService : restocked
    InService --> [*]
```
