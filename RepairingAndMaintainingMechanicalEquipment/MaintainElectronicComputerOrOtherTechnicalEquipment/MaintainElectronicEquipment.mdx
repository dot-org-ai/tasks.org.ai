...
```mermaid
erDiagram
    ElectronicSystem ||--|{ CircuitBoard : "contains"
    ElectronicSystem ||--o{ WorkOrder : "subject of"
    ElectronicSystem ||--o{ TestResult : "measured by"
    ElectronicSystem ||--|| ServiceHistory : "tracked in"
    WorkOrder ||--o{ TestResult : "produces"
    ElectronicSystem ||--o{ ComplianceCertificate : "certified by"
    CircuitBoard ||--o{ TestResult : "tested in"
```
...
stateDiagram-v2
    [*] --> Operational
    Operational --> UnderInspection : equipmentInspected
    UnderInspection --> Diagnosing : diagnosticTestRun
    UnderInspection --> PreventiveService : preventiveServicePerformed
    Diagnosing --> BoardReplacement : circuitBoardReplaced
    Diagnosing --> Calibrating : instrumentCalibrated
    BoardReplacement --> Diagnosing : re-test required
    Calibrating --> Diagnosing : re-test required
    Diagnosing --> Certifying : complianceCertified
    PreventiveService --> Certifying : complianceCertified
    Certifying --> Operational : returned to service
    Diagnosing --> AwaitingParts : parts on order
    AwaitingParts --> BoardReplacement : parts received
    Operational --> [*]
```
