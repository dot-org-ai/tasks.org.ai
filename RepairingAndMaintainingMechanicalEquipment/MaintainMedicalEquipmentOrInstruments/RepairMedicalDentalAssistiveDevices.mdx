...
```mermaid
erDiagram
    AssistiveDevice ||--|| PatientRecord : "belongs to"
    AssistiveDevice ||--|| DeviceSpecification : "defined by"
    AssistiveDevice ||--o{ RepairOrder : "serviced via"
    RepairOrder ||--o{ InsuranceAuthorization : "requires"
    RepairOrder ||--|| QualityReport : "verified by"
    DeviceSpecification ||--o{ RepairOrder : "guides"
    PatientRecord ||--o{ RepairOrder : "tracks"
```
...
stateDiagram-v2
    [*] --> Received
    Received --> Intaken : deviceIntaken
    Intaken --> Diagnosed : defectDiagnosed
    Diagnosed --> AwaitingAuth : authorizationObtained
    Diagnosed --> Fabricating : componentFabricated
    AwaitingAuth --> Fabricating : componentFabricated
    Fabricating --> Repairing : deviceRepaired
    Repairing --> Verifying : fitAndFunctionVerified
    Verifying --> Released : deviceReleasedToPatient
    Verifying --> Repairing : rework needed
    Released --> [*]
```
