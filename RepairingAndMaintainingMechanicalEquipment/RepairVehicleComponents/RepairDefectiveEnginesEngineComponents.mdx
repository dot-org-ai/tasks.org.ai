...
```mermaid
erDiagram
    Engine ||--o{ EngineComponent : "composed of"
    Engine ||--o{ FaultCode : "reports"
    Engine ||--|| RepairOrder : "serviced via"
    Engine ||--o{ DiagnosticReport : "diagnosed by"
    Engine ||--o{ PerformanceTestResult : "tested in"
    RepairOrder ||--o{ EngineComponent : "replaces"
    DiagnosticReport ||--o{ FaultCode : "interprets"
    RepairOrder ||--o{ PerformanceTestResult : "verified by"
```
...
stateDiagram-v2
    [*] --> Running
    Running --> Diagnosing : engineDiagnosed
    Diagnosing --> OrderCreated : repairOrderCreated
    OrderCreated --> TornDown : engineDisassembled
    TornDown --> ComponentSwap : componentReplaced
    ComponentSwap --> Reassembled : engineReassembled
    Reassembled --> PerformanceTest : performanceTested
    PerformanceTest --> Diagnosing : performance not OK
    PerformanceTest --> CodesCleared : faultCodesCleared
    CodesCleared --> Running
```
