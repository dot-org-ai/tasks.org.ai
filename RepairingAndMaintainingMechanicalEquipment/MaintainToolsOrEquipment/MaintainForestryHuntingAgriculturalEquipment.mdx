...
```mermaid
erDiagram
    FieldEquipment ||--|| MaintenanceSchedule : "follows"
    FieldEquipment ||--o{ ServiceRecord : "tracked in"
    FieldEquipment ||--o{ ComplianceCertificate : "certified by"
    FieldEquipment ||--o{ WorkOrder : "serviced via"
    WorkOrder ||--o{ PartInventory : "consumes from"
    MaintenanceSchedule ||--o{ WorkOrder : "generates"
    ServiceRecord ||--o{ ComplianceCertificate : "supports"
```
...
stateDiagram-v2
    [*] --> Idle
    Idle --> SeasonalPrep : seasonalServiceScheduled
    SeasonalPrep --> Inspecting : equipmentInspected
    Inspecting --> PartsOrdered : replacementPartsOrdered
    Inspecting --> FieldService : fieldMaintenancePerformed
    PartsOrdered --> FieldService : parts received
    FieldService --> Documented : serviceCompletionRecorded
    Documented --> ComplianceCheck : complianceCertified
    Documented --> Active : no certification due
    ComplianceCheck --> Active : certified
    Active --> Decommissioned : equipmentDecommissioned
    Active --> Idle : season ends
    Decommissioned --> [*]
```
