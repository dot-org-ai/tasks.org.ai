...
```mermaid
erDiagram
    FuelCellStack ||--o{ DegradationReport : "analyzed in"
    BatteryModule ||--o{ DegradationReport : "analyzed in"
    GeothermalLoop ||--o{ DegradationReport : "analyzed in"
    TidalConverter ||--o{ DegradationReport : "analyzed in"
    FuelCellStack ||--o{ MaintenanceWindow : "serviced during"
    BatteryModule ||--o{ MaintenanceWindow : "serviced during"
    TidalConverter ||--o{ MaintenanceWindow : "serviced during"
    GeothermalLoop ||--o{ MaintenanceWindow : "serviced during"
```
...
stateDiagram-v2
    [*] --> Generating
    Generating --> Monitoring : cellPerformanceMonitored
    Monitoring --> DegradationAnalysis : degradationReportGenerated
    Monitoring --> Generating : normal performance
    DegradationAnalysis --> StackReplacement : cellStackReplaced
    DegradationAnalysis --> LoopService : geothermalLoopServiced
    DegradationAnalysis --> MarineRetrieval : marineRetrievalPerformed
    StackReplacement --> Calibrating : storageBMSCalibrated
    LoopService --> Calibrating : storageBMSCalibrated
    MarineRetrieval --> Calibrating : storageBMSCalibrated
    Calibrating --> CorrosionCheck : corrosionInspected
    CorrosionCheck --> Generating : acceptable
    CorrosionCheck --> MajorOverhaul : critical corrosion
    MajorOverhaul --> Generating : overhaul complete
```
