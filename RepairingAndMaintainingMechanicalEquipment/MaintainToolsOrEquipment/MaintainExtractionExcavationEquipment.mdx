...
```mermaid
erDiagram
    HeavyEquipment ||--|{ UndercarriageComponent : "contains"
    HeavyEquipment ||--|| ServiceInterval : "follows"
    HeavyEquipment ||--o{ FluidSample : "sampled from"
    HeavyEquipment ||--o{ WorkOrder : "serviced via"
    HeavyEquipment ||--o{ HourMeterReading : "tracked by"
    FluidSample ||--o{ WorkOrder : "triggers"
    ServiceInterval ||--o{ WorkOrder : "generates"
```
...
stateDiagram-v2
    [*] --> InOperation
    InOperation --> HoursLogged : hourMeterReadingLogged
    HoursLogged --> ServiceScheduled : preventiveServiceScheduled
    HoursLogged --> InOperation : not yet due
    ServiceScheduled --> UndercarriageCheck : undercarriageInspected
    UndercarriageCheck --> FluidAnalysis : fluidSampleSubmitted
    FluidAnalysis --> HydraulicWork : hydraulicServicePerformed
    FluidAnalysis --> WearReplacement : groundEngagementReplaced
    HydraulicWork --> WearReplacement : groundEngagementReplaced
    WearReplacement --> Certified : equipmentCertifiedForOperation
    Certified --> InOperation : returned to field
```
...
graph TD
    ...
    G -->|No| I{Wear Parts Due?}
    H --> I
    I -->|Yes| J[replaceGroundEngagement]
    I -->|No| K[certifyForOperation]
    J --> K
    K --> D
```
