...
```mermaid
erDiagram
    Machine ||--|| MaintenancePlan : "follows"
    Machine ||--o{ DowntimeRecord : "tracked in"
    Machine ||--o{ ServiceReport : "documented by"
    Machine ||--|{ SafetyGuard : "equipped with"
    Machine ||--o{ ToolInventory : "consumes from"
    MaintenancePlan ||--o{ ServiceReport : "generates"
    SafetyGuard ||--o{ ServiceReport : "inspected in"
```
...
stateDiagram-v2
    [*] --> Active
    Active --> Scheduled : maintenancePlanCreated
    Scheduled --> InService : scheduledServicePerformed
    InService --> GuardCheck : safetyGuardsInspected
    GuardCheck --> PartsSwap : wearPartsReplaced
    GuardCheck --> Testing : guards OK
    PartsSwap --> Testing : machineOperationTested
    Testing --> Updated : assetRecordUpdated
    Testing --> Down : downtimeRecorded
    Down --> Scheduled : repair scheduled
    Updated --> Active : returned to service
```
