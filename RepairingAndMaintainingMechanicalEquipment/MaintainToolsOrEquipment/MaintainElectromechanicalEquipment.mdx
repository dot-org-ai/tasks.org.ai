...
```mermaid
erDiagram
    ElectricMotor ||--o{ Actuator : "drives"
    ElectricMotor ||--|| DriveCon : "controlled by"
    ElectricMotor ||--o{ Sensor : "monitored by"
    ElectricMotor ||--o{ MaintenanceRecord : "tracked in"
    Actuator ||--o{ Sensor : "positioned by"
    DriveCon ||--o{ MaintenanceRecord : "serviced in"
    LockoutTagoutPermit ||--o{ MaintenanceRecord : "authorizes"
```
...
stateDiagram-v2
    [*] --> Operating
    Operating --> LockedOut : lockoutTagoutIssued
    LockedOut --> InsulationTest : motorInsulationTested
    InsulationTest --> VibrationCheck : vibrationAnalysisPerformed
    InsulationTest --> RewindScheduled : insulation failed
    VibrationCheck --> ActuatorSwap : actuatorReplaced
    VibrationCheck --> SensorCal : sensorCalibrated
    ActuatorSwap --> SensorCal : sensorCalibrated
    SensorCal --> DriveService : driveUnitServiced
    DriveService --> Documented : maintenanceDocumented
    RewindScheduled --> Documented : maintenanceDocumented
    Documented --> Operating : lockout released
```
