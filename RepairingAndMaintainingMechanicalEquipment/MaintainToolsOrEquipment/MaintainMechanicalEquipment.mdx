...
```mermaid
erDiagram
    Equipment ||--o{ MaintenanceOrder : "serviced by"
    Equipment ||--o{ InspectionRecord : "assessed by"
    Equipment ||--|| MaintenanceSchedule : "follows"
    Equipment ||--o{ FailureReport : "documented in"
    MaintenanceOrder ||--o{ SparePart : "consumes"
    InspectionRecord ||--o{ MaintenanceOrder : "generates"
    FailureReport ||--o{ MaintenanceOrder : "triggers"
```
...
stateDiagram-v2
    [*] --> Operational
    Operational --> Scheduled : inspectionScheduled
    Scheduled --> Inspecting : conditionRecorded
    Inspecting --> UnderMaintenance : maintenancePerformed
    Inspecting --> Operational : no issues
    UnderMaintenance --> PartReplacement : componentReplaced
    UnderMaintenance --> Verifying : operationVerified
    PartReplacement --> Verifying : operationVerified
    Verifying --> Operational : maintenanceOrderClosed
    Verifying --> FailureMode : failureEscalated
    FailureMode --> UnderMaintenance : rework
    Operational --> [*]
```
