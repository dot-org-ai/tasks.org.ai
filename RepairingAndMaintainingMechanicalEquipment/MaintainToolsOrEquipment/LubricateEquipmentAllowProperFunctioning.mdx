...
erDiagram
    ...
    LubricationPoint }o--|| LubricationSchedule : "scheduled in"
    LubricationPoint ||--o{ LubricationRecord : "documented in"
    LubricationPoint ||--o{ OilSample : "sampled from"
    Lubricant }o--|| LubricantInventory : "stocked in"
    OilSample ||--o{ LubricationRecord : "referenced in"
    LubricationSchedule ||--o{ LubricationRecord : "tracks"
```
...
stateDiagram-v2
    [*] --> Scheduled
    Scheduled --> Applied : lubricantApplied
    Applied --> Documented : lubricationTaskDocumented
    Documented --> Sampling : oilSampleCollected
    Documented --> Scheduled : next interval
    Sampling --> Analyzed : oilAnalysisReviewed
    Analyzed --> IntervalAdjusted : lubricationIntervalAdjusted
    Analyzed --> Scheduled : normal results
    IntervalAdjusted --> Scheduled : new interval set
    Scheduled --> InventoryCheck : lubricantInventoryUpdated
    InventoryCheck --> Scheduled : stock adequate
    InventoryCheck --> Reordering : stock low
    Reordering --> Scheduled : restocked
```
