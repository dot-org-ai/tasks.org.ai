...
```mermaid
erDiagram
    DrillRig ||--|{ MudPump : "contains"
    DrillRig ||--|| TopDrive : "equipped with"
    DrillRig ||--|| BOP : "protected by"
    DrillRig ||--o{ DrillBit : "uses"
    DrillRig ||--o{ MaintenanceTicket : "tracked by"
    MudPump ||--o{ MaintenanceTicket : "serviced via"
    BOP ||--o{ MaintenanceTicket : "tested via"
```
...
stateDiagram-v2
    [*] --> Operational
    Operational --> Inspecting : rigComponentsInspected
    Inspecting --> TicketCreated : maintenanceTicketIssued
    Inspecting --> Operational : no issues
    TicketCreated --> PumpService : mudPumpServiced
    TicketCreated --> BOPTest : bopTested
    TicketCreated --> SealReplacement : topDriveSealReplaced
    TicketCreated --> Calibrating : torqueGaugesCalibrated
    PumpService --> Verifying : verification
    BOPTest --> Verifying : verification
    SealReplacement --> Verifying : verification
    Calibrating --> Verifying : verification
    Verifying --> Operational : passes
    Verifying --> TicketCreated : fails
    Operational --> BitTracking : bitUsageTracked
    BitTracking --> Operational : bit within limits
    BitTracking --> TicketCreated : bit replacement needed
```
