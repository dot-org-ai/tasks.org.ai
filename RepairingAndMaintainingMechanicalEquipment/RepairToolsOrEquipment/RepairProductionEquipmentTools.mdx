...
```mermaid
erDiagram
    ProductionEquipment ||--o{ BreakdownReport : "documented in"
    ProductionEquipment ||--o{ RepairOrder : "serviced via"
    ProductionEquipment ||--o{ DowntimeRecord : "tracked in"
    ProductionEquipment ||--o| SparePartsKit : "stocked for"
    ProductionTool ||--o{ RepairOrder : "serviced via"
    BreakdownReport ||--|| RepairOrder : "generates"
    RepairOrder ||--o{ DowntimeRecord : "produces"
    SparePartsKit ||--o{ RepairOrder : "consumed by"
```
...
stateDiagram-v2
    [*] --> InProduction
    InProduction --> BreakdownReported : breakdownTriaged
    BreakdownReported --> RepairScheduled : repairOrderCreated
    RepairScheduled --> MachineRepair : machineRepaired
    RepairScheduled --> ToolRepair : toolRepaired
    MachineRepair --> Testing : equipmentTested
    ToolRepair --> Testing : equipmentTested
    Testing --> RepairScheduled : test failed
    Testing --> Released : equipmentReleasedToProduction
    Released --> InProduction
    InProduction --> ChronicReview : chronicFailureIdentified
    ChronicReview --> BreakdownReported : breakdownTriaged
```
