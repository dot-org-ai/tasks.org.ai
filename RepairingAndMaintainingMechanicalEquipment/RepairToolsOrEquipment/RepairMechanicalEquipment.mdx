...
```mermaid
erDiagram
    Equipment ||--o{ WorkOrder : "serviced via"
    Equipment ||--o{ FaultDiagnosis : "diagnosed by"
    Equipment ||--o{ RepairLog : "tracked in"
    Equipment ||--o{ TestReport : "verified by"
    WorkOrder ||--|| FaultDiagnosis : "references"
    WorkOrder ||--o{ ReplacementPart : "consumes"
    WorkOrder ||--o{ TestReport : "produces"
    RepairLog ||--o{ WorkOrder : "documents"
```
...
stateDiagram-v2
    [*] --> Operational
    Operational --> Diagnosing : faultDiagnosed
    Diagnosing --> Scheduled : workOrderCreated
    Scheduled --> Isolated : equipmentIsolated
    Isolated --> UnderRepair : repairExecuted
    UnderRepair --> Testing : equipmentTested
    Testing --> Isolated : test failed
    Testing --> Closed : workOrderClosed
    Closed --> Operational
    Operational --> RepeatFault : repeatFailureDetected
    RepeatFault --> Diagnosing : faultDiagnosed
```
