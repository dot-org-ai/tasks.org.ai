...
```mermaid
erDiagram
    MechanicalPart ||--o{ WearMeasurement : "measured by"
    MechanicalPart ||--|| MaterialCertification : "certified by"
    MechanicalPart ||--o{ InspectionRecord : "verified in"
    MechanicalPart ||--|| RepairProcedure : "restored via"
    RepairProcedure ||--|| RepairCostEstimate : "estimated by"
    WearMeasurement ||--|| RepairProcedure : "informs"
    RepairProcedure ||--o{ InspectionRecord : "produces"
```
...
stateDiagram-v2
    [*] --> InService
    InService --> WearAssessed : wearMeasured
    WearAssessed --> Evaluated : repairabilityEvaluated
    Evaluated --> MethodSelected : repairMethodSelected
    Evaluated --> Scrapped : not economical
    MethodSelected --> Repaired : repairExecuted
    Repaired --> DimensionCheck : dimensionsVerified
    DimensionCheck --> Repaired : out of tolerance
    DimensionCheck --> NDTesting : ndtCompleted
    NDTesting --> Certified : partRecertified
    NDTesting --> Scrapped : not structurally sound
    Certified --> InService
```
