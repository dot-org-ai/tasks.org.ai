...
```mermaid
erDiagram
    Circuit ||--o{ WiringRun : "composed of"
    Circuit ||--o{ FaultReport : "documented in"
    Circuit ||--o{ TestMeasurement : "measured by"
    FaultReport ||--|| RepairOrder : "generates"
    RepairOrder ||--o{ TestMeasurement : "verified by"
    RepairOrder ||--o| InspectionCertificate : "certified via"
    WiringRun ||--o{ FaultReport : "referenced in"
```
...
stateDiagram-v2
    [*] --> Operational
    Operational --> FaultReported : circuitFaultDiagnosed
    FaultReported --> Isolated : circuitIsolated
    Isolated --> WiringRepair : wiringRepaired
    Isolated --> ConnectorRepair : connectorReplaced
    WiringRepair --> Testing : circuitIntegrityTested
    ConnectorRepair --> Testing : circuitIntegrityTested
    Testing --> Isolated : test failed
    Testing --> Documented : repairDocumented
    Documented --> PendingInspection : inspectionRequested
    PendingInspection --> Operational : inspection approved
    PendingInspection --> Isolated : inspection rejected
```
