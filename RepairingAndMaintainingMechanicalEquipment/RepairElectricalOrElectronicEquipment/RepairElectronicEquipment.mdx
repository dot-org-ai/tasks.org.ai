...
```mermaid
erDiagram
    ElectronicDevice ||--o{ CircuitBoard : "contains"
    ElectronicDevice ||--|| RepairTicket : "tracked via"
    ElectronicDevice ||--o{ FunctionalTestReport : "verified by"
    CircuitBoard ||--o{ SolderJoint : "connected by"
    CircuitBoard ||--o| Schematic : "documented in"
    RepairTicket ||--o{ FunctionalTestReport : "references"
    Schematic ||--o{ CircuitBoard : "describes"
```
...
stateDiagram-v2
    [*] --> Intake
    Intake --> Triaged : deviceTriaged
    Triaged --> FaultFound : faultIsolated
    FaultFound --> ComponentRemoved : componentDesoldered
    ComponentRemoved --> ComponentInstalled : replacementSoldered
    ComponentInstalled --> FirmwareRestore : firmwareRestored
    ComponentInstalled --> FunctionalTest : no firmware needed
    FirmwareRestore --> FunctionalTest : functionalTestPassed
    FunctionalTest --> FaultFound : test failed
    FunctionalTest --> Released : deviceReturnedToService
    Released --> [*]
```
