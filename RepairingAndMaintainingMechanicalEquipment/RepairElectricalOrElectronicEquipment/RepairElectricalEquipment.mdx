...
```mermaid
erDiagram
    ElectricalEquipment ||--o{ WorkOrder : "serviced via"
    ElectricalEquipment ||--o{ FaultDiagnosis : "diagnosed by"
    ElectricalEquipment ||--o{ LoadTestResult : "tested in"
    WorkOrder ||--|| SafetyPermit : "authorized by"
    WorkOrder ||--|| FaultDiagnosis : "references"
    WorkOrder ||--o{ ReplacementPart : "consumes"
    WorkOrder ||--o{ LoadTestResult : "verified by"
```
...
stateDiagram-v2
    [*] --> Operational
    Operational --> WorkOrderOpen : repairWorkOrderCreated
    WorkOrderOpen --> SafetyCleared : safetyPermitObtained
    SafetyCleared --> Diagnosing : faultDiagnosed
    Diagnosing --> UnderRepair : repairPerformed
    Diagnosing --> Condemned : not repairable
    UnderRepair --> Testing : loadTestConducted
    Testing --> Diagnosing : test failed
    Testing --> Documented : repairOutcomeDocumented
    Documented --> Operational : equipmentReleased
```
