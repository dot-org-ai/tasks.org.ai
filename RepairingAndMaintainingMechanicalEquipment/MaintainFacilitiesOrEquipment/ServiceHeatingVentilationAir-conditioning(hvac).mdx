...
```mermaid
erDiagram
    HVACUnit ||--o{ ServiceOrder : "serviced by"
    HVACUnit ||--o{ InspectionReport : "inspected in"
    HVACUnit ||--o{ PerformanceReading : "measured by"
    ServiceOrder ||--o{ ComponentReplacement : "includes"
    ServiceOrder ||--o{ RefrigerantLog : "tracks"
    InspectionReport ||--o{ ServiceOrder : "generates"
    HVACUnit ||--o{ RefrigerantLog : "logged in"
```
...
stateDiagram-v2
    [*] --> Operating
    Operating --> Scheduled : serviceScheduled
    Scheduled --> Inspecting : systemInspected
    Inspecting --> Diagnosing : issueDiagnosed
    Inspecting --> Verifying : no issues found
    Diagnosing --> Replacing : componentReplaced
    Diagnosing --> Recharging : refrigerantRecharged
    Replacing --> Verifying : performanceVerified
    Recharging --> Verifying : performanceVerified
    Verifying --> Operating : serviceOrderClosed
    Verifying --> Diagnosing : performance below standard
    Operating → [*]
```
