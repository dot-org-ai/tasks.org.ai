...
```mermaid
erDiagram
    SurveyInstrument ||--o{ TargetPopulation : contains
    TargetPopulation ||--o{ ResponseDataset : contains
    ResponseDataset ||--o{ ConfidentialityProtocol : contains
    ConfidentialityProtocol }o--|| StatisticalReport : contains
    StatisticalReport ||--o{ BenchmarkComparison : contains
    BenchmarkComparison ||--o{ FindingsPresentation : contains
```
...
stateDiagram-v2
    [*] --> surveyDesigning
    surveyDesigning --> populationDefining : surveyDesigned
    populationDefining --> surveyDistributing : populationDefined
    surveyDistributing --> responsesCollecting : surveyDistributed
    responsesCollecting --> dataAnalyzing : responsesCollected
    dataAnalyzing --> populationDefining : revisionNeeded
