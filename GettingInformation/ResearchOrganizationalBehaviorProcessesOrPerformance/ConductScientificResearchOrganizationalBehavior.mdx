...
```mermaid
erDiagram
    LiteratureReview ||--o{ ResearchHypothesis : contains
    ResearchHypothesis ||--o{ StudyDesign : contains
    StudyDesign ||--o{ SurveyInstrument : contains
    SurveyInstrument ||--o{ BehavioralDataset : contains
    BehavioralDataset ||--o{ StatisticalModel : contains
    StatisticalModel ||--o{ ResearchManuscript : contains
```
...
stateDiagram-v2
    [*] --> literatureReviewing
    literatureReviewing --> hypothesisDeveloping : literatureReviewed
    hypothesisDeveloping --> studyDesigning : hypothesisDeveloped
    studyDesigning --> behavioralDataCollecting : studyDesigned
    behavioralDataCollecting --> dataAnalyzing : behavioralDataCollected
    dataAnalyzing --> hypothesisDeveloping : revisionNeeded
    dataAnalyzing --> [*] : completed
```
