...
```mermaid
erDiagram
    ResearchProtocol ||--o{ ClinicalTrial : contains
    ClinicalTrial ||--o{ StudyCohort : contains
    StudyCohort ||--o{ ClinicalData : contains
    ClinicalData ||--o{ Finding : contains
    Finding ||--o{ Publication : contains
```
...
stateDiagram-v2
    [*] --> protocolDeveloping
    protocolDeveloping --> approvalObtaining : protocolDeveloped
    approvalObtaining --> participantsRecruiting : approvalObtained
    participantsRecruiting --> clinicalDataCollecting : participantsRecruited
    clinicalDataCollecting --> resultsAnalyzing : clinicalDataCollected
    resultsAnalyzing --> approvalObtaining : revisionNeeded
    resultsAnalyzing --> [*] : completed
```
