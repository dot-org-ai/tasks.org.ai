...
```mermaid
erDiagram
    Pathogen ||--o{ DiseaseModel : contains
    DiseaseModel ||--o{ ClinicalTrial : contains
    ClinicalTrial ||--o{ Specimen : contains
    Specimen ||--o{ Intervention : contains
    Intervention ||--o{ Finding : contains
```
...
stateDiagram-v2
    [*] --> pathogenIdentifiing
    pathogenIdentifiing --> transmissionStudiing : pathogenIdentified
    transmissionStudiing --> modelDeveloping : transmissionStudied
    modelDeveloping --> interventionTesting : modelDeveloped
    interventionTesting --> resistanceAnalyzing : interventionTested
    resistanceAnalyzing --> transmissionStudiing : revisionNeeded
    resistanceAnalyzing --> [*] : completed
```
