...
```mermaid
erDiagram
    ResearchProject ||--o{ LiteratureReview : contains
    LiteratureReview ||--o{ FieldStudy : contains
    FieldStudy ||--o{ EnvironmentalSample : contains
    EnvironmentalSample }o--|| ImpactAnalysis : contains
    ImpactAnalysis ||--o{ ResearchPublication : contains
    ResearchPublication ||--o{ Dataset : contains
```
...
stateDiagram-v2
    [*] --> researchPending
    researchPending --> literatureReviewInProgress : researchInitiated
    literatureReviewInProgress --> fieldStudyDesigning : literatureReviewConducted
    fieldStudyDesigning --> environmentalSamplesCollecting : fieldStudyDesigned
    environmentalSamplesCollecting --> impactDataAnalyzing : environmentalSamplesCollected
    impactDataAnalyzing --> literatureReviewInProgress : revisionNeeded
    impactDataAnalyzing --> [*] : completed
```
