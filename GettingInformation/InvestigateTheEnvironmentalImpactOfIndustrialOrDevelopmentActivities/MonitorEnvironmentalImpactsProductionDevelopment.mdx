...
```mermaid
erDiagram
    MonitoringPlan ||--o{ DataPoint : contains
    DataPoint ||--o{ ComplianceLimit : contains
    ComplianceLimit ||--o{ Exceedance : contains
    Exceedance }o--|| MonitoringReport : contains
    MonitoringReport ||--o{ Trend : contains
```
...
stateDiagram-v2
    [*] --> planEstablishing
    planEstablishing --> samplesCollecting : planEstablished
    samplesCollecting --> measurementsAnalyzing : samplesCollected
    measurementsAnalyzing --> exceedanceDetecting : measurementsAnalyzed
    exceedanceDetecting --> performanceTrending : exceedanceDetected
    performanceTrending --> samplesCollecting : revisionNeeded
    performanceTrending --> [*] : completed
```
