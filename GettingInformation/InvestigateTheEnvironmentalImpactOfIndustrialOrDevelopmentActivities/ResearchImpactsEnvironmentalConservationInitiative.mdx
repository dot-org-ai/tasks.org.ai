...
```mermaid
erDiagram
    ConservationInitiative ||--o{ BaselineAssessment : contains
    BaselineAssessment ||--o{ MonitoringPlan : contains
    MonitoringPlan ||--o{ EcologicalIndicator : contains
    EcologicalIndicator ||--o{ SocioeconomicSurvey : contains
    SocioeconomicSurvey ||--o{ ImpactAssessment : contains
    ImpactAssessment }o--|| EvaluationReport : contains
```
...
```mermaid
stateDiagram-v2
    [*] --> initiativeIdentifiing
    initiativeIdentifiing --> baselineEstablishing : initiativeIdentified
    baselineEstablishing --> monitoringPlanDesigning : baselineEstablished
    monitoringPlanDesigning --> monitoringDataCollecting : monitoringPlanDesigned
    monitoringDataCollecting --> impactEvaluating : monitoringDataCollected
    impactEvaluating --> baselineEstablishing : revisionNeeded
    impactEvaluating --> [*] : completed
```
