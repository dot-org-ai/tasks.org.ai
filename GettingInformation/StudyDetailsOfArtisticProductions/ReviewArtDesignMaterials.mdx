...
```mermaid
erDiagram
    DesignAsset ||--o{ ReviewSession : contains
    ReviewSession ||--o{ Annotation : contains
    Annotation ||--o{ RevisionRequest : contains
    RevisionRequest ||--|{ BrandGuideline : contains
    BrandGuideline ||--o{ ApprovalRecord : contains
    ApprovalRecord ||--o{ DesignBrief : contains
```
...
stateDiagram-v2
    [*] --> designSubmitting
    designSubmitting --> compositionEvaluating : designSubmitted
    compositionEvaluating --> designAnnotating : compositionEvaluated
    designAnnotating --> revisionPending : designAnnotated
    revisionPending --> designApproving : revisionRequested
    designApproving --> compositionEvaluating : revisionNeeded
    designApproving --> [*] : completed
```
...
const asset = await reviews.submitDesign({
  ...
  fileUrl: 'https://assets.example.com.ai/hero-banner-v2.psd',
  designer: 'jsmith'
})
