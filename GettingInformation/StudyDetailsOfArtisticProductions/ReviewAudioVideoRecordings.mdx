...
```mermaid
erDiagram
    Recording ||--o{ ReviewSession : contains
    ReviewSession ||--o{ TimecodeAnnotation : contains
    TimecodeAnnotation }o--|| QualityReport : contains
    QualityReport ||--o{ TranscriptDocument : contains
    TranscriptDocument ||--o{ ApprovalDecision : contains
    ApprovalDecision ||--o{ RevisionNote : contains
    RevisionNote ||--o{ MediaAssetRecord : contains
```
...
stateDiagram-v2
    [*] --> recordingIngesting
    recordingIngesting --> qualityAssessing : recordingIngested
    qualityAssessing --> timecodeAnnotating : qualityAssessed
    timecodeAnnotating --> editPending : timecodeAnnotated
    editPending --> recordingApproving : editRequested
    recordingApproving --> qualityAssessing : revisionNeeded
    recordingApproving --> [*] : completed
```
...
const recording = await reviews.ingestRecording({
  ...
  mediaUrl: 'https://media.example.com.ai/interview-chen-raw.mp4',
  format: 'ProRes 422'
})
