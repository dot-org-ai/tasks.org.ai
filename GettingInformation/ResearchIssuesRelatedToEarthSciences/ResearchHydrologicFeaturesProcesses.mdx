...
```mermaid
erDiagram
    WatershedStudy ||--o{ StreamflowRecord : contains
    StreamflowRecord ||--o{ GroundwaterObservation : contains
    GroundwaterObservation ||--o{ WaterQualitySample : contains
    WaterQualitySample ||--o{ PrecipitationRecord : contains
    PrecipitationRecord ||--o{ HydrologicModel : contains
    HydrologicModel ||--o{ ResearchPublication : contains
```
...
stateDiagram-v2
    [*] --> studyDesigning
    studyDesigning --> instrumentationInstalling : studyDesigned
    instrumentationInstalling --> hydrologicDataCollecting : instrumentationInstalled
    hydrologicDataCollecting --> hydrologicModelBuilt : hydrologicDataCollected
    hydrologicModelBuilt --> modelCalibrating : hydrologicModelBuilt
    modelCalibrating --> instrumentationInstalling : revisionNeeded
    modelCalibrating --> [*] : completed
```
