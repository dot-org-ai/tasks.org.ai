...
```mermaid
erDiagram
    ClimateStudy ||--o{ ObservationRecord : contains
    ObservationRecord ||--o{ ReanalysisDataset : contains
    ReanalysisDataset ||--o{ ClimateProjection : contains
    ClimateProjection }o--|| StatisticalTrendReport : contains
    StatisticalTrendReport ||--o{ ClimateModel : contains
    ClimateModel ||--o{ ResearchPublication : contains
```
...
```mermaid
stateDiagram-v2
    [*] --> researchStudyDesigning
    researchStudyDesigning --> climateDataAcquiring : researchStudyDesigned
    climateDataAcquiring --> statisticalAnalysisInProgress : climateDataAcquired
    statisticalAnalysisInProgress --> climateModelConfiguring : statisticalAnalysisPerformed
    climateModelConfiguring --> simulationRun : climateModelConfigured
    simulationRun --> climateDataAcquiring : revisionNeeded
