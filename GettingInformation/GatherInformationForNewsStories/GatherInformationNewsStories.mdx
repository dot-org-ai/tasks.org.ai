...
```mermaid
erDiagram
    StoryAssignment ||--o{ SourceContact : contains
    SourceContact ||--o{ Interview : contains
    Interview ||--o{ Document : contains
    Document ||--o{ FactCheck : contains
    FactCheck ||--o{ Lead : contains
```
...
stateDiagram-v2
    [*] --> leadIdentifiing
    leadIdentifiing --> interviewInProgress : leadIdentified
    interviewInProgress --> documentsReceiving : interviewConducted
    documentsReceiving --> factVerifiing : documentsReceived
    factVerifiing --> backgroundReSearched : factVerified
    backgroundReSearched --> interviewInProgress : revisionNeeded
    backgroundReSearched --> [*] : completed
```
