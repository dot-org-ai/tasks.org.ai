...
```mermaid
erDiagram
    LegalQuestion ||--o{ StatutorySource : contains
    StatutorySource ||--o{ CaseLawCitation : contains
    CaseLawCitation ||--o{ RegulatoryGuidance : contains
    RegulatoryGuidance ||--o{ LegislativeHistory : contains
    LegislativeHistory ||--o{ ResearchMemorandum : contains
    ResearchMemorandum ||--o{ Recommendation : contains
```
...
```mermaid
stateDiagram-v2
    [*] --> legalQuestionFraming
    legalQuestionFraming --> authoritativeSourcesIdentifiing : legalQuestionFramed
    authoritativeSourcesIdentifiing --> caseLawSearching : authoritativeSourcesIdentified
    caseLawSearching --> regulatoryGuidanceReviewing : caseLawSearched
    regulatoryGuidanceReviewing --> findingsSynthesizing : regulatoryGuidanceReviewed
    findingsSynthesizing --> authoritativeSourcesIdentifiing : revisionNeeded
    findingsSynthesizing --> [*] : completed
```
