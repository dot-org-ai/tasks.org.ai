...
```mermaid
erDiagram
    SafetyInspection ||--o{ HazardIdentification : contains
    HazardIdentification ||--o{ ExposureAssessment : contains
    ExposureAssessment ||--o{ ControlMeasure : contains
    ControlMeasure ||--o{ ComplianceGap : contains
    ComplianceGap ||--o{ CorrectiveAction : contains
    CorrectiveAction }o--|| SafetyInvestigationReport : contains
```
...
```mermaid
stateDiagram-v2
    [*] --> safetyInspectionPending
    safetyInspectionPending --> hazardsIdentifiing : safetyInspectionInitiated
    hazardsIdentifiing --> exposuresAssessing : hazardsIdentified
    exposuresAssessing --> controlMeasuresEvaluating : exposuresAssessed
    controlMeasuresEvaluating --> complianceGapsIdentifiing : controlMeasuresEvaluated
    complianceGapsIdentifiing --> hazardsIdentifiing : revisionNeeded
    complianceGapsIdentifiing --> [*] : completed
```
