...
```mermaid
erDiagram
    FailureReport ||--o{ RootCause : contains
    RootCause ||--o{ FailureMode : contains
    FailureMode ||--o{ DiagnosticData : contains
    DiagnosticData ||--o{ CorrectiveAction : contains
    CorrectiveAction ||--o{ Investigation : contains
```
...
stateDiagram-v2
    [*] --> failureReporting
    failureReporting --> diagnosticsCollecting : failureReported
    diagnosticsCollecting --> failureModeAnalyzing : diagnosticsCollected
    failureModeAnalyzing --> rootCauseDetermining : failureModeAnalyzed
    rootCauseDetermining --> impactAssessing : rootCauseDetermined
    impactAssessing --> diagnosticsCollecting : revisionNeeded
    impactAssessing --> [*] : completed
```
