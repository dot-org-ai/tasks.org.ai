...
```mermaid
erDiagram
    LivestockStudy ||--o{ AnimalCohort : contains
    AnimalCohort ||--o{ PerformanceRecord : contains
    PerformanceRecord ||--o{ FeedTrial : contains
    FeedTrial ||--o{ HealthAssessment : contains
    HealthAssessment ||--o{ ManagementSystem : contains
```
...
stateDiagram-v2
    [*] --> studyLaunching
    studyLaunching --> cohortsSelecting : studyLaunched
    cohortsSelecting --> performanceTracking : cohortsSelected
    performanceTracking --> feedEvaluating : performanceTracked
    feedEvaluating --> welfareAssessing : feedEvaluated
    welfareAssessing --> cohortsSelecting : revisionNeeded
    welfareAssessing --> [*] : completed
```
