...
```mermaid
erDiagram
    AccidentReport ||--o{ SceneInspection : contains
    SceneInspection ||--o{ EquipmentLog : contains
    EquipmentLog ||--o{ OperatorRecord : contains
    OperatorRecord ||--o{ HazardAssessment : contains
    HazardAssessment }o--|| CausationAnalysis : contains
    CausationAnalysis ||--o{ SafetyRecommendation : contains
```
...
stateDiagram-v2
    [*] --> accidentReporting
    accidentReporting --> sceneSecuring : accidentReported
    sceneSecuring --> equipmentInspecting : sceneSecured
    equipmentInspecting --> operationalLogsReviewing : equipmentInspected
    operationalLogsReviewing --> hazardsAssessing : operationalLogsReviewed
    hazardsAssessing --> sceneSecuring : revisionNeeded
    hazardsAssessing --> [*] : completed
```
