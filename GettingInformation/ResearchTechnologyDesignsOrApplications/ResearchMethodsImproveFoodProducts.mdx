...
```mermaid
erDiagram
    ProductFormulation ||--|{ ProcessMethod : contains
    ProcessMethod ||--o{ SensoryProfile : contains
    SensoryProfile ||--o{ NutritionalAnalysis : contains
    NutritionalAnalysis ||--o{ ShelfLifeStudy : contains
    ShelfLifeStudy ||--o{ ConsumerAcceptance : contains
```
...
```mermaid
stateDiagram-v2
    [*] --> formulationDeveloping
    formulationDeveloping --> processOptimizing : formulationDeveloped
    processOptimizing --> sensoryyInProgress : processOptimized
    sensoryyInProgress --> nutritionAnalyzing : sensoryyConducted
    nutritionAnalyzing --> shelfLifeTesting : nutritionAnalyzed
    shelfLifeTesting --> processOptimizing : revisionNeeded
    shelfLifeTesting --> [*] : completed
```
