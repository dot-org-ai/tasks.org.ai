...
```mermaid
erDiagram
    GenerationSystem ||--o{ ConsumptionProfile : contains
    ConsumptionProfile ||--o{ EfficiencyMeasure : contains
    EfficiencyMeasure ||--o{ ConservationStrategy : contains
    ConservationStrategy ||--o{ PerformanceMetric : contains
    PerformanceMetric ||--|{ BaselineStudy : contains
```
...
stateDiagram-v2
    [*] --> generationAnalyzing
    generationAnalyzing --> consumptionMonitoring : generationAnalyzed
    consumptionMonitoring --> efficiencyAssessing : consumptionMonitored
    efficiencyAssessing --> conservationPlanning : efficiencyAssessed
    conservationPlanning --> performanceMeasuring : conservationPlanned
    performanceMeasuring --> consumptionMonitoring : revisionNeeded
    performanceMeasuring --> [*] : completed
```
