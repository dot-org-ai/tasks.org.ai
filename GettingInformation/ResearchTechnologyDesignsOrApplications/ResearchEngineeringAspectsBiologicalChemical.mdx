...
```mermaid
erDiagram
    BioProcess ||--o{ ChemicalReaction : contains
    ChemicalReaction ||--o{ ProcessParameter : contains
    ProcessParameter ||--o{ ExperimentalRun : contains
    ExperimentalRun ||--o{ YieldOptimization : contains
    YieldOptimization ||--o{ ScaleUpPlan : contains
```
...
```mermaid
stateDiagram-v2
    [*] --> processCharacterizing
    processCharacterizing --> reactionAnalyzing : processCharacterized
    reactionAnalyzing --> yieldOptimizing : reactionAnalyzed
    yieldOptimizing --> experimentDesigning : yieldOptimized
    experimentDesigning --> scaleUpEvaluating : experimentDesigned
    scaleUpEvaluating --> reactionAnalyzing : revisionNeeded
    scaleUpEvaluating --> [*] : completed
```
