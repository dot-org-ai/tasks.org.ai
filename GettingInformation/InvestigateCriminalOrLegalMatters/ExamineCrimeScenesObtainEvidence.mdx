...
```mermaid
erDiagram
    CrimeScene ||--o{ EvidenceItem : contains
    EvidenceItem ||--o{ ScenePhotograph : contains
    ScenePhotograph ||--o{ SceneSketch : contains
    SceneSketch ||--o{ ChainOfCustody : contains
    ChainOfCustody ||--o{ SearchPattern : contains
    SearchPattern ||--o{ LatentPrint : contains
    LatentPrint ||--o{ BiologicalSample : contains
```
...
stateDiagram-v2
    [*] --> sceneSecuring
    sceneSecuring --> sceneDocumenting : sceneSecured
    sceneDocumenting --> searchInProgress : sceneDocumented
    searchInProgress --> evidenceCollecting : searchConducted
    evidenceCollecting --> evidencePackaging : evidenceCollected
    evidencePackaging --> sceneDocumenting : revisionNeeded
    evidencePackaging --> [*] : completed
```
