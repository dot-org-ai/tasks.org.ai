...
```mermaid
erDiagram
    FinancialRecord ||--o{ CommunicationLog : contains
    CommunicationLog ||--o{ PublicRecord : contains
    PublicRecord ||--o{ DigitalArtifact : contains
    DigitalArtifact }o--|| SuspiciousActivityReport : contains
    SuspiciousActivityReport ||--o{ LinkChart : contains
    LinkChart }o--|| InvestigativeReport : contains
```
...
stateDiagram-v2
    [*] --> recordsAcquiring
    recordsAcquiring --> dataIndexing : recordsAcquired
    dataIndexing --> transactionsAnalyzing : dataIndexed
    transactionsAnalyzing --> relationshipsMapping : transactionsAnalyzed
    relationshipsMapping --> anomaliesDetecting : relationshipsMapped
    anomaliesDetecting --> dataIndexing : revisionNeeded
    anomaliesDetecting --> [*] : completed
```
