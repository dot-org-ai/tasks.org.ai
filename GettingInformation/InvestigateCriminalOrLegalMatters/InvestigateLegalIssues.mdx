...
```mermaid
erDiagram
    LegalIssue ||--o{ Statute : contains
    Statute ||--o{ CasePrecedent : contains
    CasePrecedent ||--o{ Regulation : contains
    Regulation ||--o{ FactualRecord : contains
    FactualRecord ||--o{ LegalMemorandum : contains
    LegalMemorandum ||--o{ LegalOpinion : contains
```
...
stateDiagram-v2
    [*] --> issueIdentifiing
    issueIdentifiing --> statutesReSearched : issueIdentified
    statutesReSearched --> precedentAnalyzing : statutesResearched
    precedentAnalyzing --> factsGathering : precedentAnalyzed
    factsGathering --> liabilityAssessing : factsGathered
    liabilityAssessing --> statutesReSearched : revisionNeeded
    liabilityAssessing --> [*] : completed
```
