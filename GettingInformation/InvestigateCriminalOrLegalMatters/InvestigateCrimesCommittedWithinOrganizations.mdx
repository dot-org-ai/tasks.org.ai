...
```mermaid
erDiagram
    Allegation ||--o{ InvestigationCase : contains
    InvestigationCase ||--o{ InterviewRecord : contains
    InterviewRecord ||--o{ FinancialAnalysis : contains
    FinancialAnalysis ||--o{ EvidencePackage : contains
    EvidencePackage }o--|| FindingsReport : contains
    FindingsReport ||--o{ ReferralRecord : contains
```
...
stateDiagram-v2
    [*] --> allegationReceiving
    allegationReceiving --> investigationOpening : allegationReceived
    investigationOpening --> interviewInProgress : investigationOpened
    interviewInProgress --> financialsAnalyzing : interviewConducted
    financialsAnalyzing --> evidencePreserving : financialsAnalyzed
    evidencePreserving --> investigationOpening : revisionNeeded
    evidencePreserving --> [*] : completed
```
