...
```mermaid
erDiagram
    Tip ||--o{ InvestigationCase : contains
    InvestigationCase ||--o{ SurveillanceRecord : contains
    SurveillanceRecord ||--o{ IntelligenceReport : contains
    IntelligenceReport ||--o{ Warrant : contains
    Warrant ||--o{ EvidenceFile : contains
    EvidenceFile ||--o{ CaseDisposition : contains
```
...
stateDiagram-v2
    [*] --> tipReceiving
    tipReceiving --> caseOpening : tipReceived
    caseOpening --> surveillanceInProgress : caseOpened
    surveillanceInProgress --> intelligenceAnalyzing : surveillanceConducted
    intelligenceAnalyzing --> warrantObtaining : intelligenceAnalyzed
    warrantObtaining --> caseOpening : revisionNeeded
    warrantObtaining --> [*] : completed
```
