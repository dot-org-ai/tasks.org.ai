...
```mermaid
erDiagram
    ResearchQuestion ||--o{ InformationSource : contains
    InformationSource ||--|{ SearchQuery : contains
    SearchQuery ||--o{ SearchResult : contains
    SearchResult ||--o{ RelevanceAssessment : contains
    RelevanceAssessment ||--o{ ExtractedDataSet : contains
    ExtractedDataSet ||--o{ SearchLog : contains
```
...
```mermaid
stateDiagram-v2
    [*] --> researchQuestionDefining
    researchQuestionDefining --> sourcesIdentifiing : researchQuestionDefined
    sourcesIdentifiing --> queryConstructing : sourcesIdentified
    queryConstructing --> searchExecuting : queryConstructed
    searchExecuting --> resultsEvaluating : searchExecuted
    resultsEvaluating --> sourcesIdentifiing : revisionNeeded
    resultsEvaluating --> [*] : completed
```
