...
```mermaid
erDiagram
    PropertyRecord ||--o{ DeedDocument : contains
    DeedDocument }o--|| TitleReport : contains
    TitleReport ||--o{ TaxAssessment : contains
    TaxAssessment ||--o{ ZoningDesignation : contains
    ZoningDesignation ||--o{ SurveyPlat : contains
    SurveyPlat ||--o{ EncumbranceRecord : contains
    EncumbranceRecord ||--o{ AppraisalReport : contains
```
...
stateDiagram-v2
    [*] --> propertyIdentifiing
    propertyIdentifiing --> deedRetrieving : propertyIdentified
    deedRetrieving --> taxRecordsSearching : deedRetrieved
    taxRecordsSearching --> titleExamining : taxRecordsSearched
    titleExamining --> zoningObtaining : titleExamined
    zoningObtaining --> deedRetrieving : revisionNeeded
    zoningObtaining --> [*] : completed
```
