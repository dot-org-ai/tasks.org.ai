...
```mermaid
erDiagram
    RecordRequest ||--o{ BankStatement : contains
    BankStatement ||--o{ TaxReturn : contains
    TaxReturn ||--o{ GeneralLedger : contains
    GeneralLedger ||--o{ Invoice : contains
    Invoice ||--o{ PayrollRecord : contains
    PayrollRecord ||--o{ FinancialPackage : contains
    FinancialPackage ||--o{ ChainOfCustodyLog : contains
```
...
stateDiagram-v2
    [*] --> requestPending
    requestPending --> sourcesIdentifiing : requestInitiated
    sourcesIdentifiing --> documentsRetrieving : sourcesIdentified
    documentsRetrieving --> completenessVerifiing : documentsRetrieved
    completenessVerifiing --> recordsIndexing : completenessVerified
    recordsIndexing --> sourcesIdentifiing : revisionNeeded
    recordsIndexing --> [*] : completed
```
