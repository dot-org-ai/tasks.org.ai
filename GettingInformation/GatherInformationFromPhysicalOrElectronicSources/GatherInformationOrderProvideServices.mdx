...
```mermaid
erDiagram
    IntakeForm ||--|{ ClientProfile : contains
    ClientProfile ||--o{ ConsentAuthorization : contains
    ConsentAuthorization ||--o{ SupportingDocument : contains
    SupportingDocument ||--o{ NeedsAssessment : contains
    NeedsAssessment ||--o{ ServicePlan : contains
    ServicePlan ||--o{ InformationPackage : contains
```
...
stateDiagram-v2
    [*] --> intakeInProgress
    intakeInProgress --> consentObtaining : intakeConducted
    consentObtaining --> recordsPending : consentObtained
    recordsPending --> informationVerifiing : recordsRequested
    informationVerifiing --> needsAssessing : informationVerified
    needsAssessing --> consentObtaining : revisionNeeded
...
const form = await intake.conductIntake({
  ...
  demographics: {
    ...
    contactEmail: 'jane.smith@example.com.ai'
  },
