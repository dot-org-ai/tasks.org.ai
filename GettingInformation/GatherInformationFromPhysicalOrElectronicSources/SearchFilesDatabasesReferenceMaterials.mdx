...
```mermaid
erDiagram
    InformationRequest ||--|{ FileRecord : contains
    FileRecord ||--o{ DatabaseRecord : contains
    DatabaseRecord ||--o{ ReferenceMaterial : contains
    ReferenceMaterial ||--o{ SearchCriteria : contains
    SearchCriteria ||--o{ SearchResultSet : contains
    SearchResultSet ||--o{ DeliveryPackage : contains
```
...
stateDiagram-v2
    [*] --> requestReceiving
    requestReceiving --> sourcesSelecting : requestReceived
    sourcesSelecting --> searchCriteriaDefining : sourcesSelected
    searchCriteriaDefining --> searchExecuting : searchCriteriaDefined
    searchExecuting --> resultsFiltering : searchExecuted
    resultsFiltering --> sourcesSelecting : revisionNeeded
    resultsFiltering --> [*] : completed
```
