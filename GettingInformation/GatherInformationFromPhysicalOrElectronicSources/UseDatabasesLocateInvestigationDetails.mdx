...
```mermaid
erDiagram
    DatabaseQuery ||--o{ SearchResult : contains
    SearchResult ||--o{ SubjectProfile : contains
    SubjectProfile ||--o{ CriminalRecord : contains
    CriminalRecord ||--o{ VehicleRegistration : contains
    VehicleRegistration ||--o{ FinancialProfile : contains
    FinancialProfile ||--o{ IntelligenceProduct : contains
    IntelligenceProduct ||--o{ AuditTrail : contains
```
...
stateDiagram-v2
    [*] --> queryFormulating
    queryFormulating --> databaseSearching : queryFormulated
    databaseSearching --> resultsCrossReferencing : databaseSearched
    resultsCrossReferencing --> resultsValidating : resultsCrossReferenced
    resultsValidating --> subjectProfileBuilt : resultsValidated
    subjectProfileBuilt --> databaseSearching : revisionNeeded
    subjectProfileBuilt --> [*] : completed
```
