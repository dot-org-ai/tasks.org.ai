...
```mermaid
erDiagram
    ElectronicSource ||--o{ ConnectionProfile : contains
    ConnectionProfile ||--o{ RetrievalQuery : contains
    RetrievalQuery ||--o{ DataPayload : contains
    DataPayload ||--o{ TransformationRule : contains
    TransformationRule ||--|{ RetrievedDataset : contains
    RetrievedDataset ||--o{ RetrievalLog : contains
```
...
stateDiagram-v2
    [*] --> sourceIdentifiing
    sourceIdentifiing --> connectionEstablishing : sourceIdentified
    connectionEstablishing --> retrievalExecuting : connectionEstablished
    retrievalExecuting --> dataTransforming : retrievalExecuted
    dataTransforming --> payloadValidating : dataTransformed
    payloadValidating --> connectionEstablishing : revisionNeeded
    payloadValidating --> [*] : completed
```
