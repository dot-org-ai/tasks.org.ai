...
```mermaid
erDiagram
    Document ||--o{ Certificate : contains
    Certificate ||--o{ Attestation : contains
    Attestation ||--o{ Package : contains
    Package ||--o{ Verification : contains
    Verification ||--o{ Checklist : contains
```
...
stateDiagram-v2
    [*] --> checklistDraft
    checklistDraft --> documentsCollecting : checklistCreated
    documentsCollecting --> authenticityVerifiing : documentsCollected
    authenticityVerifiing --> packageAssembling : authenticityVerified
    packageAssembling --> completenessReviewing : packageAssembled
    completenessReviewing --> documentsCollecting : revisionNeeded
    completenessReviewing --> [*] : completed
```
