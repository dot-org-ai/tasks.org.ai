...
```mermaid
erDiagram
    TechnicalDocument ||--o{ WorkPlan : contains
    WorkPlan ||--o{ TaskDefinition : contains
    TaskDefinition ||--o{ ResourceEstimate : contains
    ResourceEstimate ||--o{ Constraint : contains
    Constraint ||--o{ Dependency : contains
    Dependency ||--o{ Milestone : contains
```
...
stateDiagram-v2
    [*] --> documentsRetrieving
    documentsRetrieving --> scopeDefining : documentsRetrieved
    scopeDefining --> tasksExtracting : scopeDefined
    tasksExtracting --> resourcesEstimating : tasksExtracted
    resourcesEstimating --> tasksSequencing : resourcesEstimated
    tasksSequencing --> scopeDefining : revisionNeeded
    tasksSequencing --> [*] : completed
```
