...
```mermaid
erDiagram
    Map ||--o{ Route : contains
    Route ||--o{ Waypoint : contains
    Waypoint ||--o{ RoadSegment : contains
    RoadSegment ||--o{ TerrainFeature : contains
    TerrainFeature ||--o{ Restriction : contains
    Restriction ||--o{ NavigationPlan : contains
```
...
stateDiagram-v2
    [*] --> mapLoading
    mapLoading --> routeAnalyzing : mapLoaded
    routeAnalyzing --> waypointsSet : routeAnalyzed
    waypointsSet --> terrainAssessing : waypointsSet
    terrainAssessing --> restrictionsChecking : terrainAssessed
    restrictionsChecking --> routeAnalyzing : revisionNeeded
    restrictionsChecking --> [*] : completed
```
