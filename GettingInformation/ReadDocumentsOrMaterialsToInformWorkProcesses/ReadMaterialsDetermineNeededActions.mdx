...
```mermaid
erDiagram
    Document ||--o{ ActionItem : contains
    ActionItem ||--o{ Priority : contains
    Priority ||--o{ Assignment : contains
    Assignment ||--o{ Deadline : contains
    Deadline ||--o{ DocumentCategory : contains
    DocumentCategory ||--o{ ResponseRecord : contains
```
...
stateDiagram-v2
    [*] --> documentIngesting
    documentIngesting --> contentAnalyzing : documentIngested
    contentAnalyzing --> actionsExtracting : contentAnalyzed
    actionsExtracting --> actionPrioritizing : actionsExtracted
    actionPrioritizing --> actionAssigning : actionPrioritized
    actionAssigning --> contentAnalyzing : revisionNeeded
    actionAssigning --> [*] : completed
```
...
const doc = await reader.ingestDocument({
  ...
  category: 'regulatory',
  source: 'epa-region-5',
  url: 'https://docs.example.com.ai/epa-notice-2026-04.pdf'
})
