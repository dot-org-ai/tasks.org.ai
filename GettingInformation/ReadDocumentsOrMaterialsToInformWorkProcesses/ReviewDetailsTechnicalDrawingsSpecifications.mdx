...
```mermaid
erDiagram
    TechnicalDrawing ||--o{ DimensionCallout : contains
    DimensionCallout ||--o{ GeometricTolerance : contains
    GeometricTolerance ||--o{ SurfaceFinishNote : contains
    SurfaceFinishNote ||--o{ MaterialSpecification : contains
    MaterialSpecification ||--o{ RevisionBlock : contains
    RevisionBlock ||--o{ ReviewComment : contains
```
...
stateDiagram-v2
    [*] --> drawingLoading
    drawingLoading --> dimensionsVerifiing : drawingLoaded
    dimensionsVerifiing --> tolerancesChecking : dimensionsVerified
    tolerancesChecking --> materialNotesReviewing : tolerancesChecked
    materialNotesReviewing --> surfaceFinishAssessing : materialNotesReviewed
    surfaceFinishAssessing --> dimensionsVerifiing : revisionNeeded
    surfaceFinishAssessing --> [*] : completed
```
...
const drawing = await reviewer.loadDrawing({
  ...
  revision: 'B',
  documentUrl: 'https://drawings.example.com.ai/flange-a204-rev-b.pdf'
})
