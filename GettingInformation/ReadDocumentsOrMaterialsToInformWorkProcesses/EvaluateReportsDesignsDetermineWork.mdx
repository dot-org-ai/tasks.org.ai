...
```mermaid
erDiagram
    AssessmentReport ||--o{ DesignProposal : contains
    DesignProposal ||--o{ Deficiency : contains
    Deficiency ||--o{ WorkNeed : contains
    WorkNeed ||--o{ CostEstimate : contains
    CostEstimate ||--o{ PriorityScore : contains
    PriorityScore ||--o{ RemediationPlan : contains
```
...
stateDiagram-v2
    [*] --> reportIngesting
    reportIngesting --> deficienciesIdentifiing : reportIngested
    deficienciesIdentifiing --> workNeedsScoping : deficienciesIdentified
    workNeedsScoping --> costsEstimating : workNeedsScoped
    costsEstimating --> needsPrioritizing : costsEstimated
    needsPrioritizing --> deficienciesIdentifiing : revisionNeeded
    needsPrioritizing --> [*] : completed
```
...
const report = await evaluator.ingestReport({
  ...
  documentUrl: 'https://reports.example.com.ai/bridge-441-2026.pdf'
})
