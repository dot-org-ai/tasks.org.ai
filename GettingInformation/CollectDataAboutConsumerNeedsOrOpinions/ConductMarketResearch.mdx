...
```mermaid
erDiagram
    ResearchStudy ||--o{ SurveyInstrument : "contains"
    SurveyInstrument }o--|| FocusGroup : "tracks"
    FocusGroup ||--o{ CompetitorProfile : "references"
    CompetitorProfile }o--|| MarketSegment : "generates"
    MarketSegment ||--o{ ResearchFinding : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : studyDesigned
    InProgress --> InProgress : participantsRecruited
    InProgress --> InProgress : primaryDataCollected
    InProgress --> Review : competitorsAnalyzed
    Review --> InProgress
    InProgress --> InProgress : marketSegmented
    InProgress --> InProgress : findingsSynthesized
    InProgress --> Completed : reportDelivered
    Completed --> [*]
```
## Workflow
