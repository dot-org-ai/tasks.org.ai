...
```mermaid
erDiagram
    InformationRequest ||--o{ FinancialDocument : "contains"
    FinancialDocument }o--|| IdentityDocument : "tracks"
    IdentityDocument ||--o{ CreditReport : "references"
    CreditReport }o--|| VerificationResult : "generates"
    VerificationResult ||--o{ SecureRecord : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : informationRequested
    InProgress --> InProgress : documentCollected
    InProgress --> InProgress : identityVerified
    InProgress --> Review : creditReportObtained
    Review --> InProgress
    InProgress --> InProgress : incomeVerified
    InProgress --> InProgress : dataEncrypted
    InProgress --> Completed : accessAudited
    Completed --> [*]
```
## Workflow
