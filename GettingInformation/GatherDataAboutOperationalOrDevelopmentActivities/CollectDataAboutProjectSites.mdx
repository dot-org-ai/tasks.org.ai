...
```mermaid
erDiagram
    SiteReconnaissanceReport ||--o{ SoilBoring : contains
    SoilBoring ||--o{ SoilTestResult : contains
    SoilTestResult ||--o{ EnvironmentalAssessment : contains
    EnvironmentalAssessment ||--o{ UtilityMap : contains
    UtilityMap ||--o{ GroundwaterData : contains
    GroundwaterData }o--|| SiteCharacterizationReport : contains
```
...
```mermaid
stateDiagram-v2
    [*] --> reconnaissanceInProgress
    reconnaissanceInProgress --> explorationPlanning : reconnaissanceConducted
    explorationPlanning --> soilSamplesCollecting : explorationPlanned
    soilSamplesCollecting --> environmentalAssessmentInProgress : soilSamplesCollected
    environmentalAssessmentInProgress --> utilitiesLocating : environmentalAssessmentPerformed
    utilitiesLocating --> explorationPlanning : revisionNeeded
    utilitiesLocating --> [*] : completed
```
