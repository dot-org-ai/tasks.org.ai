...
```mermaid
erDiagram
    ProductionMachine ||--|{ Workpiece : "produces"
    Workpiece ||--|| ExtractionProcedure : "follows"
    Workpiece ||--o{ CycleRecord : "logs"
    Workpiece ||--|| StagingArea : "moves-to"
    HandlingSpec ||--|| Workpiece : "governs"
```
...
```mermaid
stateDiagram-v2
    [*] --> InMachine : cycleVerified
    InMachine --> Extracted : workpieceExtracted
    Extracted --> Inspected : workpieceInspected
    Inspected --> Staged : workpieceStaged
    Staged --> Logged : extractionLogged
    Logged --> MachineReady : machineCleared
    Inspected --> Damaged : damageReported
    Damaged --> Staged : workpieceStaged
    MachineReady --> [*]
```
