...
```mermaid
erDiagram
    Mold ||--|{ Workpiece : "forms"
    Workpiece ||--|| DemoldingProcedure : "follows"
    Mold ||--|| EjectionSystem : "activates"
    Workpiece ||--o{ ExtractionRecord : "logs"
    Workpiece ||--o{ SurfaceDefect : "reveals"
```
...
stateDiagram-v2
    [*] --> InMold : cureVerified
    InMold --> MoldOpen : moldOpened
    MoldOpen --> Ejected : workpieceEjected
    Ejected --> Extracted : workpieceExtracted
    Extracted --> Inspected : demoldedInspected
    Inspected --> MoldReady : moldReconditioned
    Inspected --> Extracted : defectFound
    MoldReady --> Logged : demoldingLogged
    Logged --> [*]
```
