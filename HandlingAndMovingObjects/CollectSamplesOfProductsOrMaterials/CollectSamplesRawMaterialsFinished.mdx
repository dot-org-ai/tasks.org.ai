...
```mermaid
erDiagram
    RawMaterialLot ||--o{ Sample : "sampled-from"
    ProductionBatch ||--o{ Sample : "sampled-from"
    InspectionPlan ||--o{ Sample : "governs"
    Sample ||--|| Certificate : "certified-by"
    Sample ||--|| RetentionSample : "archived-as"
    RawMaterialLot ||--|| ProductionBatch : "feeds"
```
...
stateDiagram-v2
    [*] --> Receiving
    Receiving --> RawSampling : shipmentReceived
    RawSampling --> FinishedSampling : rawSampleCollected
    FinishedSampling --> Inspecting : finishedSampleCollected
    Inspecting --> Archiving : inspectionPerformed
    Archiving --> Releasing : retentionSampleArchived
    Releasing --> [*] : releaseApproved
    Inspecting --> Rejecting : lotRejected
    Rejecting --> [*]
```
