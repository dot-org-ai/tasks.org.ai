...
```mermaid
erDiagram
    SamplingPlan ||--o{ ProductBatch : "targets"
    ProductBatch ||--o{ Sample : "drawn-from"
...
stateDiagram-v2
    [*] --> PlanCreation
    PlanCreation --> BatchSelection : samplingPlanCreated
    BatchSelection --> Collecting : batchSelected
    Collecting --> Labeling : sampleCollected
    Labeling --> Packaging : sampleLabeled
    Packaging --> LabSubmission : samplePackaged
    LabSubmission --> Tracking : labSubmitted
    Tracking --> [*] : resultsTracked
    Collecting --> BatchSelection : sampleContaminated
```
