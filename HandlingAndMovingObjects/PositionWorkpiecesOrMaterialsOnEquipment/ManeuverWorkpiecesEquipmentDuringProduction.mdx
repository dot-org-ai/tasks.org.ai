...
```mermaid
erDiagram
    Workpiece ||--|| FeedSequence : "follows"
    ProductionRun ||--o{ Workpiece : "contains"
    Workpiece ||--o{ PositionLog : "generates"
    GuideConfiguration ||--|| FeedSequence : "defines"
    Workpiece ||--o{ SafetyCheck : "undergoes"
    PositionLog ||--|| GuideConfiguration : "validates"
```
...
stateDiagram-v2
    ...
    Feeding --> Positioned
    Positioned --> Rotating
    Rotating --> Repositioned
    Repositioned --> PositionCheck
    PositionCheck --> OutOfTolerance
    OutOfTolerance --> Paused
    Paused --> Repositioned
    PositionCheck --> Feeding
    Feeding --> ProcessComplete
    ProcessComplete --> [*]
```
