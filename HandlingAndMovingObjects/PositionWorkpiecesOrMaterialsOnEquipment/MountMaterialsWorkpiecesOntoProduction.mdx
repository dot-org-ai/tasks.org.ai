...
```mermaid
erDiagram
    Workpiece ||--|| Fixture : "mounted-in"
    WorkOrder ||--o{ Workpiece : "specifies"
    MachineSetup ||--|| Fixture : "uses"
    Workpiece ||--|| AlignmentSpec : "must-meet"
    Workpiece ||--o{ MountingRecord : "generates"
    MountingRecord ||--|| MachineSetup : "documents"
```
...
stateDiagram-v2
    ...
    Aligning --> Clamped
    Clamped --> Verifying
    Verifying --> MountingFailed
    MountingFailed --> Released
    Released --> Loaded
    Verifying --> MountingVerified
    MountingVerified --> Processing
    Processing --> Released
    Released --> [*]
```
