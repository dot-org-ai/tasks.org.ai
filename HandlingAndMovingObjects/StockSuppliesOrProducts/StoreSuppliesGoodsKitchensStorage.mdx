...
```mermaid
erDiagram
    FoodItem ||--|| StorageZone : "stored-in"
    StorageZone ||--o{ TemperatureLog : "monitored-by"
    DeliveryReceipt ||--|{ FoodItem : "receives"
    FoodItem ||--|| InventoryCount : "verified-by"
    FoodItem ||--|| ExpirationTracking : "expires-per"
```
...
```mermaid
stateDiagram-v2
    [*] --> Delivered : deliveryReceived
    Delivered --> Inspected : qualityChecked
    Inspected --> Labeled : itemsLabeled
    Labeled --> Zoned : zoneAssigned
    Zoned --> Rotated : storedByFIFO
    Rotated --> TempLogged : temperatureMonitored
    TempLogged --> Counted : inventoryConducted
    Counted --> Zoned : restockNeeded
    Counted --> [*]
```
