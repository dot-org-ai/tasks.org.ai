...
```mermaid
erDiagram
    StoredItem ||--|| StorageLocation : "placed-in"
    StoredItem ||--o{ StorageRecord : "documented-by"
    StoredItem ||--o{ AccessLog : "tracked-by"
    StoredItem ||--|| ConditionReport : "assessed-by"
    RetrievalRequest ||--|{ StoredItem : "retrieves"
```
...
stateDiagram-v2
    [*] --> Intaken : itemIntaken
    Intaken --> Located : locationAssigned
    Located --> Stored : placedInStorage
    Stored --> Labeled : itemLabeled
    Labeled --> Inspected : conditionInspected
    Inspected --> Retrieved : itemRetrieved
    Retrieved --> Returned : returnedToStorage
    Returned --> Stored : placedInStorage
    Retrieved --> [*]
```
