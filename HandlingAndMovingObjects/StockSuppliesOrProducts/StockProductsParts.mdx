...
```mermaid
erDiagram
    Product ||--|| BinLocation : "stored-in"
    Product ||--|| StockLevel : "tracked-by"
    PickList ||--|{ Product : "pulls"
    ReplenishmentOrder ||--|{ Product : "replenishes"
    Product ||--o{ InventoryTransaction : "logged-by"
```
...
stateDiagram-v2
    ...
    Received --> Located : locationAssigned
    Located --> Shelved : shelvesStocked
    Shelved --> Picked : orderPicked
    Picked --> Adjusted : inventoryAdjusted
    Adjusted --> Reordered : replenishmentTriggered
    Reordered --> Received : shipmentReceived
    Adjusted --> Shelved : stockSufficient
    Picked --> Relocated : stockRelocated
    Relocated --> Shelved : shelvesStocked
    Picked --> [*]
```
