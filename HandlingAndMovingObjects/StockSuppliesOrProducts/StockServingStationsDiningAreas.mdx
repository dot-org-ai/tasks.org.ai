...
```mermaid
erDiagram
    ServingStation ||--|{ FoodItem : "presents"
    ServingStation ||--|{ SupplyItem : "stocks"
    ServingStation ||--o{ TemperatureLog : "monitored-by"
    ParLevel ||--|| ServingStation : "governs"
    ReplenishmentSchedule ||--|| ServingStation : "schedules"
```
...
```mermaid
stateDiagram-v2
    [*] --> Prepared : foodItemsPrepared
    Prepared --> Transported : transportedToStation
    Transported --> Stocked : buffetStocked
    Stocked --> Supplied : suppliesReplenished
    Supplied --> TempChecked : temperaturesChecked
    TempChecked --> Rotated : stockRotated
    Rotated --> Stocked : suppliesReplenished
    TempChecked --> Cleaned : stationCleaned
    Cleaned --> [*]
```
