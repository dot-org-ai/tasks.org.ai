...
```mermaid
erDiagram
    Artist ||--|| Craftsperson : follows
    Artist }o--|| FinishingTechnician : generates
    Artist ||--|{ GalleryOwner : requires
    Artist ||--o{ RetailBuyer : tracks
    Artist ||--|| ConservationSpecialist : validates
    Craftsperson ||--|{ Artist : creates
    FinishingTechnician ||--o{ Artist : documents
    GalleryOwner ||--|| Artist : supports
```
...
stateDiagram-v2
    [*] --> Artist
    Artist --> Craftsperson: Craftsperson
    Craftsperson --> Finishing_Technician: FinishingTechnician
    Finishing_Technician --> Gallery_Owner: GalleryOwner
    Gallery_Owner --> Retail_Buyer: RetailBuyer
    Retail_Buyer --> Conservation_Specialist: ConservationSpecialist
    Gallery_Owner --> Craftsperson: FinishingTechnician
