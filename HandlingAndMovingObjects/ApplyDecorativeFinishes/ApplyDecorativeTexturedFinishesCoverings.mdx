...
```mermaid
erDiagram
    InteriorSurface ||--|| FinishSpecification : "finished according to"
    CoatingProduct ||--|| InteriorSurface : "applied to"
    WallCovering ||--|| InteriorSurface : "installed on"
    ApplicationTechnique ||--|| CoatingProduct : "delivers"
    QualityStandard ||--|| FinishSpecification : "defines acceptance for"
    InteriorSurface ||--o{ ApplicationTechnique : "treated using"
```
...
stateDiagram-v2
    [*] --> SurfacePrepared
    SurfacePrepared --> BaseCoatApplied : baseCoatApplied
    BaseCoatApplied --> TextureCreated : textureCreated
    TextureCreated --> DecorativeFinishApplied : decorativeFinishApplied
    DecorativeFinishApplied --> WallcoveringInstalled : wallcoveringInstalled
    WallcoveringInstalled --> FinishSealed : finishSealed
    FinishSealed --> WorkmanshipInspected : workmanshipInspected
    WorkmanshipInspected --> BaseCoatApplied : baseCoatApplied
    WorkmanshipInspected --> [*]
```
