...
```mermaid
erDiagram
    Exhibition ||--|{ StudentWork : "showcases"
    DisplaySpace ||--|{ Exhibition : "hosts"
    StudentWork ||--|| MountingMaterial : "mounted-with"
    DisplaySchedule ||--|{ Exhibition : "rotates"
    StudentWork ||--o{ RecognitionRecord : "earns"
```
...
stateDiagram-v2
    ...
    Selected --> Approved : permissionObtained
    Approved --> Prepared : displayPrepared
    Prepared --> Exhibited : exhibitionArranged
    Exhibited --> Archived : exhibitionDocumented
    Exhibited --> Rotated : displayRotated
    Rotated --> Selected : newWorkSelected
    Archived --> Returned : displayRemoved
    Returned --> [*]
```
