...
```mermaid
erDiagram
    Component ||--|| MicroTolerance : "constrained-by"
    FormingStation ||--|{ Component : "processes"
    Component ||--|| CoiningOperation : "shaped-by"
    Component ||--o{ OpticalMeasurement : "inspected-by"
    FormingStation ||--|| ProcessCapability : "rated-by"
```
...
```mermaid
stateDiagram-v2
    [*] --> Fixtured : componentFixtured
    Fixtured --> Formed : precisionForceApplied
    Formed --> Coined : featureCoinaged
    Coined --> Measured : micronsmeasureed
    Measured --> Deburred : componentDeburred
    Deburred --> Sorted : toleranceSorted
    Sorted --> Validated : capabilityValidated
    Measured --> Fixtured : outOfSpec
    Validated --> [*]
```
