...
```mermaid
erDiagram
    Specification ||--|{ Workpiece : "governs"
    Workpiece ||--|| FormingOperation : "undergoes"
    FormingOperation ||--|| Die : "employs"
    Specification ||--|| Tolerance : "sets"
    Workpiece ||--o{ Measurement : "verified-by"
```
...
```mermaid
stateDiagram-v2
    [*] --> Positioned : workpiecePositioned
    Positioned --> Formed : forceApplied
    Formed --> Measured : dimensionsMeasured
    Measured --> Adjusted : toolingAdjusted
    Adjusted --> Positioned : reposition
    Measured --> Trimmed : excessTrimmed
    Trimmed --> HeatTreated : heatTreatmentApplied
    HeatTreated --> Validated : conformanceValidated
    Validated --> [*]
```
