...
```mermaid
erDiagram
    Workpiece ||--|| Tool : "struck-with"
    Pattern ||--|| Workpiece : "guides"
    Workpiece ||--o{ HeatTreatment : "annealed-by"
    Workpiece ||--o{ MeasurementRecord : "measured-by"
    Workpiece ||--|| FinishedPart : "becomes"
```
...
```mermaid
stateDiagram-v2
    [*] --> Blanked : blankPrepared
    Blanked --> Planned : sequencePlanned
    Planned --> Contoured : contourFormed
    Contoured --> Refined : detailsRefined
    Refined --> Checked : dimensionsChecked
    Checked --> Annealed : heatTreated
    Annealed --> Contoured : reshapeNeeded
    Checked --> Polished : surfaceFinished
    Polished --> [*]
```
