...
```mermaid
erDiagram
    Material ||--|| FormingMethod : "shaped-by"
    Material ||--|| Product : "becomes"
    FormingMethod ||--|| Mold : "employs"
    Material ||--|| Consistency : "requires"
    Product ||--o{ Defect : "exhibits"
```
...
```mermaid
stateDiagram-v2
    [*] --> Prepared : materialPrepared
    Prepared --> Shaped : materialShaped
    Shaped --> Smoothed : surfaceSmoothed
    Smoothed --> Detailed : detailAdded
    Detailed --> Inspected : formInspected
    Inspected --> Trimmed : excessTrimmed
    Trimmed --> Drying : productDried
    Inspected --> Shaped : reshapeNeeded
    Drying --> [*]
```
