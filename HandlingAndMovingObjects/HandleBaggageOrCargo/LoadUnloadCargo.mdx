...
```mermaid
erDiagram
    LoadPlan ||--o{ CargoUnit : "defines"
    LoadPlan ||--|{ BillOfLading : "generates"
    LoadPlan }o--|| WeightTicket : "uses"
    LoadPlan ||--|| SecurementDevice : "tracked-by"
    LoadPlan ||--o{ DockAppointment : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : appointmentScheduled
    InProgress --> Verified : loadPlanCreated
    Verified --> Approved : cargoLoaded
    Approved --> Active : cargoUnloaded
    Active --> Complete : cargoSecured
