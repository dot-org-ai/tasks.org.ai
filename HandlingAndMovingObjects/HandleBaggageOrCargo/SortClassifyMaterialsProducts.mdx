...
```mermaid
erDiagram
    ClassificationCriteria ||--o{ GradeLabel : "defines"
    SortBatch ||--o{ SortBin : "sorted-into"
    GradeLabel ||--|| SortBin : "routed-to"
    SortBatch ||--o{ RejectPile : "rejects-to"
    SortBatch ||--|| ThroughputLog : "measured-in"
    ClassificationCriteria }o--|| SortBatch : "applied-to"
```
...
```mermaid
stateDiagram-v2
    [*] --> CriteriaDefined : sortCriteriaDefined
    CriteriaDefined --> Inspected : itemInspected
    Inspected --> Graded : gradeAssigned
    Graded --> Routed : itemRouted
    Inspected --> Rejected : itemRejected
    Routed --> Reconciled : batchReconciled
    Reconciled --> Logged : throughputLogged
    Reconciled --> Inspected : batchReconciled
    Logged --> [*]
```
## Workflow
