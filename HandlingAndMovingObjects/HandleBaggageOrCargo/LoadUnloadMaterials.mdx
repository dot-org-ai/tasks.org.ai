...
```mermaid
erDiagram
    DeliveryTicket ||--o{ MaterialLot : "lists"
    MaterialLot ||--|| StagingPad : "placed-on"
    MaterialLot ||--o| LiftPlan : "lifted-per"
    LiftPlan ||--|| EquipmentTag : "performed-by"
    StagingPad ||--o{ SpoilsPile : "accumulates"
    DeliveryTicket }o--|| StagingPad : "directed-to"
```
...
stateDiagram-v2
    ...
    Scheduled --> Rigged : riggingPrepared
    Rigged --> Lifted : materialLifted
    Lifted --> Placed : materialPlaced
    Placed --> Verified : deliveryVerified
    Verified --> Cleared : debrisRemoved
    Verified --> Scheduled : shortageDetected
    Cleared --> [*]
```
## Workflow
