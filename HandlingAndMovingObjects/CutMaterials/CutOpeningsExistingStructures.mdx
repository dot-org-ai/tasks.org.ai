...
```mermaid
erDiagram
    OpeningSpecification ||--o{ UtilityScan : "defines"
    OpeningSpecification ||--|{ TemporaryShoring : "generates"
    OpeningSpecification }o--|| HeaderBeam : "uses"
    OpeningSpecification ||--|| CuttingPermit : "tracked-by"
    OpeningSpecification ||--o{ DebrisManifest : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : utilitiesScanCompleted
    InProgress --> Verified : openingSpecReviewed
    Verified --> Approved : shoringInstalled
    Approved --> Active : openingOutlineMarked
    Active --> Complete : openingCutCompleted
