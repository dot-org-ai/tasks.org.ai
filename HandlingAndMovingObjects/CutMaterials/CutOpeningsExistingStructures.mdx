...
```mermaid
erDiagram
    OpeningSpecification ||--|| CuttingPermit : "authorized-by"
    UtilityScan ||--|| OpeningSpecification : "informs"
    OpeningSpecification ||--o{ TemporaryShoring : "requires"
    OpeningSpecification ||--|| HeaderBeam : "sizes"
    HeaderBeam ||--|| TemporaryShoring : "replaces"
    OpeningSpecification ||--o{ DebrisManifest : "generates"
```
...
```mermaid
stateDiagram-v2
    [*] --> Scanned : utilitiesScanCompleted
    Scanned --> SpecReviewed : openingSpecReviewed
    SpecReviewed --> Shored : shoringInstalled
    Shored --> Marked : openingOutlineMarked
    Marked --> CutOpen : openingCutCompleted
    CutOpen --> Framed : headerInstalled
    Framed --> Cleared : debrisRemoved
    Framed --> Shored : headerInstalled
    Cleared --> [*]
```
## Workflow
