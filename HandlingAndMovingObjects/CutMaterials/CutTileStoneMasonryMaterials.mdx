...
```mermaid
erDiagram
    TileLayout ||--o{ CutTemplate : "defines"
    TileLayout ||--|{ CutPiece : "requires"
    MaterialBatch ||--o{ CutPiece : "supplies"
    CutPiece ||--|| CuttingTool : "shaped-by"
    MaterialBatch ||--|| WasteAllowance : "budgets"
    CutTemplate ||--|| CutPiece : "guides"
```
...
```mermaid
stateDiagram-v2
    [*] --> LayoutReviewed : layoutReviewed
    LayoutReviewed --> Marked : materialMarked
    Marked --> ToolSelected : cuttingToolSelected
    ToolSelected --> Cut : tileCutCompleted
    Cut --> DryFit : dryFitApproved
    DryFit --> Smoothed : edgesSmoothed
    Cut --> Marked : breakageRecorded
    Smoothed --> [*]
```
## Workflow
