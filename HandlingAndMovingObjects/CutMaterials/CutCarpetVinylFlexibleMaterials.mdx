...
```mermaid
erDiagram
    MaterialRoll ||--o{ CuttingPlan : "defines"
    MaterialRoll ||--|{ CutPiece : "generates"
    MaterialRoll }o--|| SeamLocation : "uses"
    MaterialRoll ||--|| SubfloorProfile : "tracked-by"
    MaterialRoll ||--o{ WasteScrap : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : areaMeasured
    InProgress --> Verified : cuttingPlanCreated
    Verified --> Approved : materialUnrolled
    Approved --> Active : materialCut
    Active --> Complete : edgeTrimmed
