...
```mermaid
erDiagram
    WorkOrder ||--o{ CuttingPlan : "specifies"
    CuttingPlan ||--|{ SeamLocation : "positions"
    SubfloorProfile ||--|| CuttingPlan : "informs"
    MaterialRoll ||--o{ CutPiece : "yields"
    CutPiece }o--|| SeamLocation : "aligns-at"
    MaterialRoll ||--o{ WasteScrap : "produces"
```
...
stateDiagram-v2
    ...
    Measured --> Planned : cuttingPlanCreated
    Planned --> Staged : materialUnrolled
    Staged --> Cut : materialCut
    Cut --> Trimmed : edgeTrimmed
    Trimmed --> Verified : fitVerified
    Verified --> Logged : scrapLogged
    Verified --> Staged : fitVerified
    Logged --> [*]
```
## Workflow
