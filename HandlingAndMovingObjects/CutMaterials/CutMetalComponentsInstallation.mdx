...
```mermaid
erDiagram
    CutOrder ||--o{ MetalStock : "defines"
    CutOrder ||--|{ CutPart : "generates"
    CutOrder }o--|| CuttingMethod : "uses"
    CutOrder ||--|| BevelSpecification : "tracked-by"
    CutOrder ||--o{ HeatAffectedZone : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : cutOrderReviewed
    InProgress --> Verified : cuttingMethodSelected
    Verified --> Approved : equipmentSetupComplete
    Approved --> Active : metalCutCompleted
    Active --> Complete : cutEdgeInspected
