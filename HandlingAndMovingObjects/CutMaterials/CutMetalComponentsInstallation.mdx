...
```mermaid
erDiagram
    CutOrder ||--|{ CutPart : "specifies"
    CutOrder ||--|| CuttingMethod : "selects"
    MetalStock ||--o{ CutPart : "yields"
    CutPart ||--o| BevelSpecification : "prepared-with"
    CutPart ||--o| HeatAffectedZone : "generates"
    CutOrder }o--|| MetalStock : "consumes"
```
...
```mermaid
stateDiagram-v2
    [*] --> Reviewed : cutOrderReviewed
    Reviewed --> MethodChosen : cuttingMethodSelected
    MethodChosen --> SetUp : equipmentSetupComplete
    SetUp --> Cut : metalCutCompleted
    Cut --> Inspected : cutEdgeInspected
    Inspected --> Deburred : partDeburred
    Deburred --> Staged : partTaggedAndStaged
    Inspected --> SetUp : cutEdgeInspected
    Staged --> [*]
```
## Workflow
