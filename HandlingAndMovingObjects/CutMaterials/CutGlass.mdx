...
```mermaid
erDiagram
    CutOrder ||--o{ CuttingTemplate : "references"
    CutOrder ||--|{ GlassSheet : "consumes"
    GlassSheet ||--o{ ScoreLine : "receives"
    ScoreLine ||--|| CutPiece : "separates-into"
    CutPiece ||--|| EdgeProfile : "finished-with"
    GlassSheet ||--o{ WasteOffcut : "produces"
```
...
```mermaid
stateDiagram-v2
    [*] --> Measured : glassMeasured
    Measured --> Scored : glassScored
    Scored --> Separated : glassSeparated
    Separated --> EdgeFinished : edgeFinished
    EdgeFinished --> Inspected : cutInspected
    Inspected --> Completed : cutOrderCompleted
    Inspected --> Measured : defectDetected
    Completed --> [*]
```
## Workflow
