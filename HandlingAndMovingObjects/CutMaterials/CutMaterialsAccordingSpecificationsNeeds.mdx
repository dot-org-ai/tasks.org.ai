...
```mermaid
erDiagram
    CutSpecification ||--|| ToleranceRange : "defines"
    CutSpecification ||--|{ CuttingProgram : "drives"
    RawStock ||--|| MaterialCertificate : "certified-by"
    RawStock ||--o{ CutPiece : "yields"
    CutPiece ||--|| ToleranceRange : "measured-against"
    CutSpecification ||--o{ CutPiece : "specifies"
```
...
```mermaid
stateDiagram-v2
    [*] --> Interpreted : specificationInterpreted
    Interpreted --> StockReady : stockVerified
    StockReady --> Programmed : equipmentProgrammed
    Programmed --> Cut : cutExecuted
    Cut --> Measured : cutPieceMeasured
    Measured --> Bundled : piecesBundled
    Bundled --> Updated : inventoryUpdated
    Measured --> Programmed : cutPieceMeasured
    Updated --> [*]
```
## Workflow
