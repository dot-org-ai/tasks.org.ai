...
```mermaid
erDiagram
    Animal ||--|| TrainingProgram : "enrolled-in"
    TrainingProgram ||--|{ TrainingSession : "schedules"
    TrainingSession ||--|{ BehaviorCommand : "teaches"
    Animal ||--o{ ProgressRecord : "evaluated-by"
    Animal ||--o| Certification : "earns"
```
...
```mermaid
stateDiagram-v2
    [*] --> Assessed : animalAssessed
    Assessed --> Programmed : programCreated
    Programmed --> Scheduled : sessionScheduled
    Scheduled --> Trained : sessionCompleted
    Trained --> Logged : progressRecorded
    Logged --> Scheduled : moreSessionsNeeded
    Logged --> Evaluated : performanceEvaluated
    Evaluated --> Certified : animalCertified
    Evaluated --> Scheduled : retrainingRequired
    Certified --> [*]
```
