...
```mermaid
erDiagram
    Formulation ||--o{ IngredientCharge : "prescribes"
    IngredientCharge }o--|| Batch : "combined-in"
    Batch ||--|| MixingEquipment : "processed-on"
    Batch ||--o{ ConsistencyReading : "tested-by"
    Batch ||--|| BatchRecord : "documented-in"
```
...
```mermaid
stateDiagram-v2
    [*] --> FormulationReady : formulationPrepared
    FormulationReady --> Weighed : ingredientsWeighed
    Weighed --> Sequenced : additionsSequenced
    Sequenced --> Mixing : mixingStarted
    Mixing --> Mixed : mixingCompleted
    Mixed --> Checked : consistencyChecked
    Checked --> Approved : batchApproved
    Checked --> Mixing : batchRejected
    Approved --> [*]
```
## Workflow
