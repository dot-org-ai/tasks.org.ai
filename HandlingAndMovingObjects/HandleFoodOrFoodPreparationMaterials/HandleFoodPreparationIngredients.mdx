...
```mermaid
erDiagram
    DeliveryInvoice ||--o{ Ingredient : "manifests"
    Ingredient ||--|| StorageLocation : "shelved-in"
    Ingredient ||--o{ AllergenTag : "flagged-with"
    StorageLocation ||--o{ TemperatureReading : "monitored-by"
    RecipeBatch ||--o{ Ingredient : "consumes"
```
...
```mermaid
stateDiagram-v2
    [*] --> Received : ingredientsReceived
    Received --> TempChecked : temperatureChecked
    TempChecked --> Stored : ingredientStored
    Stored --> Rotated : stockRotated
    Rotated --> AllergenLabeled : allergenFlagged
    AllergenLabeled --> Measured : ingredientMeasured
    Measured --> Staged : productionStaged
    TempChecked --> Received : temperatureViolation
    Staged --> [*]
```
## Workflow
