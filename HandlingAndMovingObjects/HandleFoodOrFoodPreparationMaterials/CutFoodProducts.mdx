...
```mermaid
erDiagram
    CutSpecification ||--o{ Portion : "governs"
    Portion ||--o{ TrimWaste : "produces"
    CuttingBoard ||--o{ Portion : "prepared-on"
    BladeInventory ||--o{ CuttingBoard : "assigned-to"
    YieldReport ||--o{ Portion : "summarizes"
    CutSpecification ||--|| YieldReport : "benchmarked-by"
```
...
```mermaid
stateDiagram-v2
    [*] --> SpecSelected : specificationSelected
    SpecSelected --> Sanitized : workstationSanitized
    Sanitized --> Portioned : productPortioned
    Portioned --> Trimmed : excessTrimmed
    Trimmed --> Weighed : portionWeighed
    Weighed --> Packaged : productPackaged
    Weighed --> Portioned : portionOutOfSpec
    Packaged --> Yielded : yieldRecorded
    Yielded --> [*]
```
## Workflow
