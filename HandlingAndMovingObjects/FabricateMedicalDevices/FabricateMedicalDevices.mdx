...
```mermaid
erDiagram
    DeviceMasterRecord ||--|{ DeviceComponent : "specifies"
    DeviceComponent ||--o{ MedicalDevice : "assembled-into"
    MedicalDevice ||--|| BatchRecord : "produced-under"
    MedicalDevice ||--|| InspectionReport : "verified-by"
    MedicalDevice ||--|| SterilizationRecord : "sterilized-per"
    InspectionReport ||--o| NonconformanceReport : "triggers"
```
...
```mermaid
stateDiagram-v2
    [*] --> Reviewed : designReviewed
    Reviewed --> Procured : materialProcured
    Procured --> Machined : componentsMachined
    Machined --> Assembled : deviceAssembled
    Assembled --> Inspected : deviceInspected
    Inspected --> Sterilized : deviceSterilized
    Sterilized --> Released : deviceReleased
    Inspected --> Machined : deviceInspected
    Released --> [*]
```
## Workflow
