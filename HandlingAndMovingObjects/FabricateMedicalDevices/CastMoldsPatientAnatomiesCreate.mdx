...
```mermaid
erDiagram
    DevicePrescription ||--|| Impression : "orders"
    ImpressionMaterial ||--|| Impression : "captures"
    Impression ||--|| CastModel : "poured-into"
    CastingStone ||--|| CastModel : "forms"
    CastModel ||--|| DimensionalReport : "measured-in"
    CastModel ||--o| ArticulatorMount : "mounted-on"
```
...
```mermaid
stateDiagram-v2
    [*] --> Prepared : patientPrepared
    Prepared --> Impressed : impressionTaken
    Impressed --> Poured : castPoured
    Poured --> Trimmed : modelTrimmed
    Trimmed --> Verified : accuracyVerified
    Verified --> Mounted : articulatorMounted
    Mounted --> Released : castReleased
    Verified --> Impressed : accuracyVerified
    Released --> [*]
```
## Workflow
