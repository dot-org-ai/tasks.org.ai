...
```mermaid
erDiagram
    Prescription ||--|| AssistiveDevice : "orders"
    PatientMeasurements ||--|| AssistiveDevice : "sizes"
    AssistiveDevice ||--o| DeviceModel : "prototyped-as"
    AssistiveDevice ||--|| MaterialBillOfMaterials : "built-from"
    AssistiveDevice ||--o{ FittingRecord : "adjusted-per"
    AssistiveDevice ||--|| ComplianceCertificate : "certified-by"
```
...
```mermaid
stateDiagram-v2
    [*] --> Prescribed : prescriptionReviewed
    Prescribed --> Measured : patientMeasured
    Measured --> Designed : deviceDesigned
    Designed --> Fabricated : componentsFabricated
    Fabricated --> Assembled : deviceAssembled
    Assembled --> Fitted : deviceFitted
    Fitted --> Documented : complianceDocumented
    Fitted --> Designed : deviceFitted
    Documented --> [*]
```
## Workflow
