...
```mermaid
erDiagram
    CableSchedule ||--o{ Cable : "specifies"
    Cable ||--o{ Terminal : "fastened-to"
    Cable ||--o{ JunctionBox : "spliced-in"
    Cable ||--|| CircuitBreaker : "protected-by"
    Cable ||--|| ConnectionLog : "documented-in"
    Terminal ||--|| ConnectionLog : "torque-recorded-in"
```
...
stateDiagram-v2
    [*] --> Preparing
    Preparing --> Routing : cablePrepared
    Routing --> Terminating : cableRouted
    Terminating --> Labeling : cableTerminated
    Labeling --> Testing : connectionLabeled
    Testing --> Energizing : continuityVerified
    Energizing --> [*] : circuitEnergized
    Testing --> Terminating : continuityFailed
```
