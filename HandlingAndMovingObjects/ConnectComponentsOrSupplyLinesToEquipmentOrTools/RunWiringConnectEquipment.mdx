...
```mermaid
erDiagram
    CircuitSchedule ||--o{ WireRun : "assigns"
    WireRun ||--|| Conduit : "routed-through"
    Conduit ||--o{ PullBox : "accessed-via"
    WireRun ||--|| WireLabel : "identified-by"
    Conduit ||--|| ConduitFill : "measured-by"
    WireRun ||--|| ConduitFill : "contributes-to"
```
...
stateDiagram-v2
    [*] --> Planning
    Planning --> Measuring : wireRunPlanned
    Measuring --> Pulling : wireMeasured
    Pulling --> Terminating : wirePulled
    Terminating --> Labeling : wireTerminated
    Labeling --> Verifying : wireLabeled
    Verifying --> ClosingOut : runVerified
    ClosingOut --> [*] : runClosedOut
    Verifying --> Terminating : continuityFailed
```
## Workflow
