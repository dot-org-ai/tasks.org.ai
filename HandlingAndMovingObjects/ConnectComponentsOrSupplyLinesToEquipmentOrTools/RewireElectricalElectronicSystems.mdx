...
```mermaid
erDiagram
    RewireScope ||--o{ WiringHarness : "targets"
    WiringHarness ||--o{ Conductor : "bundled-from"
    Conductor ||--|| TerminalBlock : "fastened-to"
    CircuitDiagram ||--o{ Conductor : "routes"
    Conductor ||--|| InsulationTest : "validated-by"
    RewireScope ||--|| CircuitDiagram : "redesigned-in"
```
...
stateDiagram-v2
    [*] --> Assessing
    Assessing --> DeEnergizing : wiringAssessed
    DeEnergizing --> Removing : systemDeEnergized
    Removing --> Installing : oldWiringRemoved
    Installing --> Labeling : newWiringInstalled
    Labeling --> InsulationTesting : circuitsLabeled
    InsulationTesting --> ReEnergizing : insulationTested
    ReEnergizing --> [*] : systemReEnergized
    InsulationTesting --> Installing : insulationFailed
```
## Workflow
