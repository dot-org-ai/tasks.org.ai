...
```mermaid
erDiagram
    WorkOrder ||--o{ HoseAssembly : "requisitions"
    HoseAssembly ||--o{ Fitting : "terminated-with"
    Fitting ||--|| Port : "attached-to"
    HoseAssembly ||--|| HoseRoute : "routed-along"
    HoseAssembly ||--|| PressureTest : "validated-by"
    WorkOrder ||--|| PressureTest : "requires"
```
...
stateDiagram-v2
    [*] --> Selecting
    Selecting --> Routing : hoseSelected
    Routing --> Attaching : hoseRouted
    Attaching --> PressureTesting : fittingAttached
    PressureTesting --> Securing : pressureTestPassed
    Securing --> Documenting : hoseSecured
    Documenting --> [*] : connectionDocumented
    PressureTesting --> Attaching : leakDetected
```
