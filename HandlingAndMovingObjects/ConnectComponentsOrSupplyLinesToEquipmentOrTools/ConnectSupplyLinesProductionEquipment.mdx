...
```mermaid
erDiagram
    ConnectionManifold ||--o{ SupplyLine : "distributes"
    SupplyLine ||--|| InletPort : "connected-to"
    SupplyLine ||--|| Regulator : "regulated-by"
    SupplyLine ||--|| FilterAssembly : "filtered-through"
    SupplyLine ||--|| SupplyLineTag : "labeled-with"
    Regulator ||--|| InletPort : "mounted-at"
```
...
stateDiagram-v2
    [*] --> Surveying
    Surveying --> LineRouting : requirementsSurveyed
    LineRouting --> RegulatorSetup : supplyLineRouted
    RegulatorSetup --> FilterSetup : regulatorInstalled
    FilterSetup --> PortConnection : filterInstalled
    PortConnection --> FlowTesting : portConnected
    FlowTesting --> Tagging : supplyLineTested
    Tagging --> [*] : supplyLineTagged
    FlowTesting --> PortConnection : leakDetected
```
## Workflow
