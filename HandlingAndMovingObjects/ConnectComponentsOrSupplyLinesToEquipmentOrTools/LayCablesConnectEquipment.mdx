...
```mermaid
erDiagram
    CablePullSheet ||--|| CableRun : "specifies"
    CableRun ||--|| CableReel : "deployed-from"
    CableRun ||--|| CableTray : "supported-by"
    CableRun ||--|| Trench : "buried-in"
    CableRun ||--o{ TerminationPoint : "terminated-at"
    CableTray ||--|| Trench : "routes-through"
```
...
stateDiagram-v2
    [*] --> RoutePlanning
    RoutePlanning --> TrenchPrep : routePlanned
    TrenchPrep --> Deploying : trenchPrepared
    Deploying --> Securing : cableDeployed
    Securing --> Terminating : cableSecured
    Terminating --> Testing : cableTerminated
    Testing --> Documenting : cableRunTested
    Documenting --> [*] : installationDocumented
    Testing --> Terminating : testFailed
```
## Workflow
