...
```mermaid
erDiagram
    DesignConcept ||--o{ RawMaterial : "defines"
    DesignConcept ||--|{ Workpiece : "generates"
    DesignConcept }o--|| FinishingTreatment : "uses"
    DesignConcept ||--|| CompletedObject : "tracked-by"
    DesignConcept ||--o{ CommissionAgreement : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : conceptDeveloped
    InProgress --> Verified : materialsSelected
    Verified --> Approved : objectFormed
    Approved --> Active : componentsAssembled
    Active --> Complete : finishApplied
