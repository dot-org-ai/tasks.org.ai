...
```mermaid
erDiagram
    NailService ||--|| NailShape : "achieves"
    NailService ||--o{ NailEnhancement : "includes"
    NailEnhancement ||--|| ProductFormula : "built from"
    NailArtDesign ||--|| NailService : "decorates"
    AppointmentRecord ||--|| NailService : "schedules"
    SanitationLog ||--|| NailService : "verifies hygiene for"
```
...
stateDiagram-v2
    [*] --> ClientConsulted
    ClientConsulted --> NailPlatePrepared : nailPlatePrepared
    NailPlatePrepared --> NailsShaped : nailsShaped
    NailsShaped --> EnhancementApplied : enhancementApplied
    EnhancementApplied --> NailsDecorated : nailsDecorated
    NailsDecorated --> ServiceCured : serviceCured
    ServiceCured --> NailsDecorated : nailsDecorated
    ServiceCured --> EnhancementRemoved : enhancementRemoved
    EnhancementRemoved --> [*]
```
