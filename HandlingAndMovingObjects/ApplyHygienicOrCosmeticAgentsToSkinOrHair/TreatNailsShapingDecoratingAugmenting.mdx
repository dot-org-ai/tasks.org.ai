...
```mermaid
erDiagram
    Client ||--|| ProductSupplier : follows
    Client }o--|| SalonOwner : generates
    Client ||--|{ StateCosmetologyBoard : requires
    Client ||--o{ InsuranceProvider : tracks
    ProductSupplier ||--|{ Client : creates
    SalonOwner ||--o{ Client : documents
    StateCosmetologyBoard ||--|| Client : supports
```
...
stateDiagram-v2
    [*] --> Client
    Client --> Product_Supplier: ProductSupplier
    Product_Supplier --> Salon_Owner: SalonOwner
    Salon_Owner --> State_Cosmetology_Board: StateCosmetologyBoard
    State_Cosmetology_Board --> Insurance_Provider: InsuranceProvider
    Salon_Owner --> Product_Supplier: SalonOwner
    Insurance_Provider --> [*]
```
