...
```mermaid
erDiagram
    Client ||--|| CosmeticsSupplier : follows
    Client }o--|| ProductionDirector : generates
    Client ||--|{ Photographer : requires
    Client ||--o{ DermatologistOrAllergist : tracks
    CosmeticsSupplier ||--|{ Client : creates
    ProductionDirector ||--o{ Client : documents
    Photographer ||--|| Client : supports
```
...
stateDiagram-v2
    [*] --> Client
    Client --> Cosmetics_Supplier: CosmeticsSupplier
    Cosmetics_Supplier --> Production_Director: ProductionDirector
    Production_Director --> Photographer: Photographer
    Photographer --> Dermatologist_Or_Allergist: DermatologistOrAllergist
    Production_Director --> Cosmetics_Supplier: ProductionDirector
    Dermatologist_Or_Allergist --> [*]
```
