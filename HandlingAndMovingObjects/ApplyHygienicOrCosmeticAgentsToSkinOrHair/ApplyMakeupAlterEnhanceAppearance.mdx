...
```mermaid
erDiagram
    MakeupLook ||--|| FaceChart : "documented on"
    SkinAnalysis ||--|| ColorPalette : "determines"
    ProductKit ||--o{ ApplicationSequence : "organized for"
    ContinuityPhoto ||--|| MakeupLook : "references"
    ColorPalette ||--o{ MakeupLook : "defines shades for"
    ApplicationSequence ||--|| MakeupLook : "achieves"
```
...
stateDiagram-v2
    [*] --> LookConsulted
    LookConsulted --> SkinAnalyzed : skinAnalyzed
    SkinAnalyzed --> SkinPrepared : skinPrepared
    SkinPrepared --> BaseApplied : baseApplied
    BaseApplied --> ContourCompleted : contourCompleted
    ContourCompleted --> ColorCosmeticsApplied : colorCosmeticsApplied
    ColorCosmeticsApplied --> LookFinished : lookFinished
    ColorCosmeticsApplied --> BaseApplied : baseApplied
    LookFinished --> [*]
```
