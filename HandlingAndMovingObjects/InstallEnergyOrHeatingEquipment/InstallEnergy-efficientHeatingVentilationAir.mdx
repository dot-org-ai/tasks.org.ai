...
```mermaid
erDiagram
    HVACUnit ||--o{ Ductwork : "defines"
    HVACUnit ||--|{ RefrigerantLine : "generates"
    HVACUnit }o--|| Thermostat : "uses"
    HVACUnit ||--|| LoadCalculation : "tracked-by"
    HVACUnit ||--o{ RebateApplication : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : loadCalcCompleted
    InProgress --> Verified : equipmentSelected
    Verified --> Approved : equipmentMounted
    Approved --> Active : linesConnected
    Active --> Complete : controlsWired
