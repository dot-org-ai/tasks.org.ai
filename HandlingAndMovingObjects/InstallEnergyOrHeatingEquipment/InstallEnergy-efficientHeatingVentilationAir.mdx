...
```mermaid
erDiagram
    LoadCalculation ||--|| HVACUnit : "sizes"
    HVACUnit ||--o{ Ductwork : "distributes-through"
    HVACUnit ||--|| RefrigerantLine : "circulated-via"
    HVACUnit ||--|| Thermostat : "controlled-by"
    HVACUnit ||--o| RebateApplication : "incentivized-by"
    CommissioningReport ||--|| HVACUnit : "validates"
```
...
```mermaid
stateDiagram-v2
    [*] --> Calculated : loadCalcCompleted
    Calculated --> Selected : equipmentSelected
    Selected --> Mounted : equipmentMounted
    Mounted --> Piped : linesConnected
    Piped --> Wired : controlsWired
    Wired --> Commissioned : systemCommissioned
    Commissioned --> Wired : systemCommissioned
    Commissioned --> RebateFiled : rebateFiled
    RebateFiled --> [*]
```
## Workflow
