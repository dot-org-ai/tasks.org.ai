...
```mermaid
erDiagram
    MechanicalPermit ||--|| HVACUnit : "authorizes"
    HVACUnit ||--o{ Ductwork : "distributes-through"
    HVACUnit ||--|| Lineset : "piped-via"
    Lineset ||--|| RefrigerantCharge : "filled-with"
    HVACUnit ||--|| Thermostat : "controlled-by"
    StartupChecklist ||--|| HVACUnit : "commissions"
```
...
```mermaid
stateDiagram-v2
    [*] --> Surveyed : systemSurveyed
    Surveyed --> Permitted : permitObtained
    Permitted --> Mounted : equipmentMounted
    Mounted --> Ducted : ductworkInstalled
    Ducted --> Charged : refrigerantConnected
    Charged --> Wired : circuitsWired
    Wired --> StartedUp : startupCompleted
    StartedUp --> Wired : startupCompleted
    StartedUp --> [*]
```
## Workflow
