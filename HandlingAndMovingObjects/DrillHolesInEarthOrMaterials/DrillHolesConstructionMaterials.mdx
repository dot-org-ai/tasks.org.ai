...
```mermaid
erDiagram
    DrillingPlan ||--o{ HoleLocation : "specifies"
    HoleLocation ||--|| DrillBit : "requires"
    HoleLocation ||--|| Hole : "becomes"
    Hole ||--|| DebrisCollection : "generates"
    Hole ||--|| InspectionReport : "verified-by"
    DrillingPlan }o--|| InspectionReport : "summarized-in"
```
...
```mermaid
stateDiagram-v2
    [*] --> LaidOut : holesLaidOut
    LaidOut --> BitReady : drillBitSelected
    BitReady --> Drilled : holeDrilled
    Drilled --> Verified : dimensionsVerified
    Verified --> Cleaned : debrisCleaned
    Cleaned --> Inspected : qualityInspected
    Inspected --> Documented : completionDocumented
    Inspected --> BitReady : qualityInspected
    Documented --> [*]
```
## Workflow
