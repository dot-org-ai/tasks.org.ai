...
erDiagram
    ...
    DrillRig ||--o{ Borehole : "creates"
    Borehole ||--o{ CoreSample : "yields"
    Borehole ||--|| BoreholeLog : "documented-in"
    Borehole ||--|| DrillingFluid : "circulated-with"
    CoreSample }o--|| BoreholeLog : "recorded-in"
```
...
```mermaid
stateDiagram-v2
    [*] --> SiteReady : drillSiteSetup
    SiteReady --> RigReady : rigMobilized
    RigReady --> Advancing : boreholeAdvanced
    Advancing --> Sampled : coreSampleCollected
    Sampled --> Flushed : fluidCirculated
    Flushed --> Logged : conditionsLogged
    Logged --> Advancing : conditionsLogged
    Logged --> Completed : boreholeCompleted
    Completed --> [*]
```
## Workflow
