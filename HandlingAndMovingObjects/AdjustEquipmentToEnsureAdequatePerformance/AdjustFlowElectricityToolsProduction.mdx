...
```mermaid
erDiagram
    UtilityProvider ||--|| ElectricalContractor : follows
    UtilityProvider }o--|| SafetyInspector : generates
    UtilityProvider ||--|{ EquipmentManufacturer : requires
    UtilityProvider ||--o{ MaintenanceTeam : tracks
    UtilityProvider ||--|| ProductionOperator : validates
    ElectricalContractor ||--|{ UtilityProvider : creates
    SafetyInspector ||--o{ UtilityProvider : documents
    EquipmentManufacturer ||--|| UtilityProvider : supports
```
...
stateDiagram-v2
    [*] --> Utility_Provider
    Utility_Provider --> Electrical_Contractor: ElectricalContractor
    Electrical_Contractor --> Safety_Inspector: SafetyInspector
    Safety_Inspector --> Equipment_Manufacturer: EquipmentManufacturer
    Equipment_Manufacturer --> Maintenance_Team: MaintenanceTeam
    Maintenance_Team --> Production_Operator: ProductionOperator
    Equipment_Manufacturer --> Electrical_Contractor: SafetyInspector
    Production_Operator --> [*]
```
