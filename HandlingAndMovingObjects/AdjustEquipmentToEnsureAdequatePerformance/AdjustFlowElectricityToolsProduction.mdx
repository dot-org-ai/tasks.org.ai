...
```mermaid
erDiagram
    PowerCircuit ||--o{ VoltageReading : "measured by"
    PowerCircuit ||--o{ CurrentLoad : "draws"
    CircuitBreaker ||--|| PowerCircuit : "protects"
    PowerAdjustment ||--|| PowerCircuit : "modifies"
    EnergyConsumption ||--o{ PowerCircuit : "recorded for"
    PowerAdjustment ||--o{ VoltageReading : "informed by"
```
...
stateDiagram-v2
    [*] --> VoltageMonitoring
    VoltageMonitoring --> LoadMeasured : loadMeasured
    LoadMeasured --> VoltageAdjusted : voltageAdjusted
    VoltageAdjusted --> LoadBalancing : loadBalanced
    LoadBalancing --> CircuitTesting : circuitTested
    CircuitTesting --> ConsumptionLogged : consumptionLogged
    CircuitTesting --> VoltageMonitoring : voltageMonitored
    ConsumptionLogged --> VoltageMonitoring : voltageMonitored
    ConsumptionLogged --> [*]
```
