...
```mermaid
erDiagram
    EquipmentOperator ||--|| GasSupplier : follows
    EquipmentOperator }o--|| SafetyInspector : generates
    EquipmentOperator ||--|{ ProcessEngineer : requires
    EquipmentOperator ||--o{ MaintenanceTechnician : tracks
    EquipmentOperator ||--|| EnvironmentalCompliance : validates
    GasSupplier ||--|{ EquipmentOperator : creates
    SafetyInspector ||--o{ EquipmentOperator : documents
...
stateDiagram-v2
    [*] --> Equipment_Operator
    Equipment_Operator --> Gas_Supplier: GasSupplier
    Gas_Supplier --> Safety_Inspector: SafetyInspector
    Safety_Inspector --> Process_Engineer: ProcessEngineer
    Process_Engineer --> Maintenance_Technician: MaintenanceTechnician
    Maintenance_Technician --> Environmental_Compliance: EnvironmentalCompliance
    Process_Engineer --> Gas_Supplier: SafetyInspector
