...
```mermaid
erDiagram
    ProductionOperator ||--|| MaintenanceTechnician : follows
    ProductionOperator }o--|| QualityInspector : generates
    ProductionOperator ||--|{ ProcessEngineer : requires
    ProductionOperator ||--o{ EnergyManager : tracks
    ProductionOperator ||--|| SafetyOfficer : validates
    MaintenanceTechnician ||--|{ ProductionOperator : creates
    QualityInspector ||--o{ ProductionOperator : documents
    ProcessEngineer ||--|| ProductionOperator : supports
```
...
stateDiagram-v2
    [*] --> Production_Operator
    Production_Operator --> Maintenance_Technician: MaintenanceTechnician
    Maintenance_Technician --> Quality_Inspector: QualityInspector
    Quality_Inspector --> Process_Engineer: ProcessEngineer
    Process_Engineer --> Energy_Manager: EnergyManager
    Energy_Manager --> Safety_Officer: SafetyOfficer
    Process_Engineer --> Maintenance_Technician: QualityInspector
    Safety_Officer --> [*]
```
