...
```mermaid
erDiagram
    HeatingEquipment ||--o{ TemperatureProfile : "programmed with"
    HeatingEquipment ||--o{ TemperatureReading : "monitored by"
    ControlParameter ||--|| HeatingEquipment : "governs"
    AlarmThreshold ||--|| HeatingEquipment : "protects"
    CalibrationRecord ||--o{ TemperatureReading : "validates"
    TemperatureProfile ||--o{ ControlParameter : "defines"
```
...
stateDiagram-v2
    [*] --> TemperatureSet
    TemperatureSet --> ProfileProgrammed : profileProgrammed
    ProfileProgrammed --> TargetReached : temperatureReached
    TargetReached --> Stabilized : temperatureStabilized
    Stabilized --> SetpointAdjusting : setpointAdjusted
    SetpointAdjusting --> TargetReached : temperatureReached
    Stabilized --> AlarmState : alarmTriggered
    AlarmState --> SensorCalibrating : sensorCalibrated
    SensorCalibrating --> TemperatureSet : temperatureSet
    Stabilized --> [*]
```
