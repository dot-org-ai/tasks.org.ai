...
```mermaid
erDiagram
    MedicalDevice ||--o{ Configuration : "operates with"
    MedicalDevice ||--o{ PositionRecord : "placed per"
    Configuration ||--o{ CalibrationLog : "verified in"
    AdjustmentRequest ||--|| MedicalDevice : "targets"
    SafetyProtocol ||--|| Configuration : "governs"
    CalibrationLog ||--o{ AdjustmentRequest : "triggered by"
```
...
stateDiagram-v2
    [*] --> SettingsAdjusted
    SettingsAdjusted --> DeviceCalibrated : deviceCalibrated
    DeviceCalibrated --> ConfigurationValidated : configurationValidated
    ConfigurationValidated --> SettingsLocked : settingsLocked
    SettingsLocked --> Documented : adjustmentDocumented
    Documented --> Repositioned : equipmentRepositioned
    Repositioned --> SettingsAdjusted : settingsAdjusted
    Documented --> CalibrationDue : calibrationDue
    CalibrationDue --> DeviceCalibrated : deviceCalibrated
    SettingsLocked --> [*]
```
