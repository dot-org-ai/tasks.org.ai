...
```mermaid
erDiagram
    PowerConnection ||--|| DistributionPanel : "routed through"
    DistributionPanel ||--o{ LoadProfile : "characterized by"
    SafetyInterlock ||--|| PowerConnection : "protects"
    PowerMonitor ||--|| PowerConnection : "measures"
    EmergencyShutoff ||--|| DistributionPanel : "disconnects"
    LoadProfile ||--o{ PowerMonitor : "informed by"
```
...
stateDiagram-v2
    ...
    ConnectionEstablished --> VoltageMeasured : voltageMeasured
    VoltageMeasured --> InterlockActivated : interlockActivated
    InterlockActivated --> LoadMonitoring : loadMonitored
    LoadMonitoring --> SourcesSwitched : sourcesSwitched
    SourcesSwitched --> LoadMonitoring : loadMonitored
    LoadMonitoring --> BreakerTripped : breakerTripped
    BreakerTripped --> PowerDisconnected : powerDisconnected
    PowerDisconnected --> ConnectionEstablished : connectionEstablished
    PowerDisconnected --> [*]
```
