...
```mermaid
erDiagram
    FlowLine ||--o{ ControlValve : "regulated by"
    FlowLine ||--o{ Conveyor : "transported along"
    Feeder ||--|| FlowLine : "delivers material to"
    FlowSensor ||--|| FlowLine : "measures rate in"
    ThroughputLog ||--o{ FlowSensor : "records data from"
    Conveyor ||--o{ ThroughputLog : "tracked in"
```
...
stateDiagram-v2
    [*] --> FlowMonitoring
    FlowMonitoring --> ValveAdjusting : valveAdjusted
    ValveAdjusting --> ConveyorSpeedSetting : conveyorSpeedSet
    ConveyorSpeedSetting --> FeederCalibrating : feederCalibrated
    FeederCalibrating --> ThroughputOptimized : throughputOptimized
    ThroughputOptimized --> Logged : performanceLogged
    ThroughputOptimized --> BlockageTroubleshooting : blockageTroubleshooted
    BlockageTroubleshooting --> FlowMonitoring : flowMonitored
    Logged --> FlowMonitoring : flowMonitored
    Logged --> [*]
```
