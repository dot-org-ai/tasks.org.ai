...
```mermaid
erDiagram
    ProductionOperator ||--|| ProcessEngineer : follows
    ProductionOperator }o--|| MaterialSupplier : generates
    ProductionOperator ||--|{ QualityInspector : requires
    ProductionOperator ||--o{ MaintenanceCrew : tracks
    ProductionOperator ||--|| SafetyOfficer : validates
    ProcessEngineer ||--|{ ProductionOperator : creates
    MaterialSupplier ||--o{ ProductionOperator : documents
    QualityInspector ||--|| ProductionOperator : supports
```
...
stateDiagram-v2
    [*] --> Production_Operator
    Production_Operator --> Process_Engineer: ProcessEngineer
    Process_Engineer --> Material_Supplier: MaterialSupplier
    Material_Supplier --> Quality_Inspector: QualityInspector
    Quality_Inspector --> Maintenance_Crew: MaintenanceCrew
    Maintenance_Crew --> Safety_Officer: SafetyOfficer
    Quality_Inspector --> Process_Engineer: MaterialSupplier
    Safety_Officer --> [*]
```
