...
```mermaid
erDiagram
    MachineOperator ||--|| ProcessEngineer : follows
    MachineOperator }o--|| QualityInspector : generates
    MachineOperator ||--|{ SetupTechnician : requires
    MachineOperator ||--o{ ToolingSupplier : tracks
    MachineOperator ||--|| ProductionPlanner : validates
    ProcessEngineer ||--|{ MachineOperator : creates
    QualityInspector ||--o{ MachineOperator : documents
    SetupTechnician ||--|| MachineOperator : supports
```
...
stateDiagram-v2
    [*] --> Machine_Operator
    Machine_Operator --> Process_Engineer: ProcessEngineer
    Process_Engineer --> Quality_Inspector: QualityInspector
    Quality_Inspector --> Setup_Technician: SetupTechnician
    Setup_Technician --> Tooling_Supplier: ToolingSupplier
    Tooling_Supplier --> Production_Planner: ProductionPlanner
    Setup_Technician --> Process_Engineer: QualityInspector
    Production_Planner --> [*]
```
