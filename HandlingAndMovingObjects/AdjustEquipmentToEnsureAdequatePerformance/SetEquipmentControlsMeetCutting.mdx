...
```mermaid
erDiagram
    CuttingMachine ||--o{ CuttingParameters : "configured with"
    ToolingSetup ||--|| CuttingMachine : "loaded into"
    CuttingSpecification ||--|| CuttingParameters : "defines targets for"
    SetupSheet ||--|| CuttingMachine : "documents configuration of"
    FirstPieceInspection ||--|| CuttingSpecification : "verifies against"
    SetupSheet ||--o{ ToolingSetup : "records"
```
...
stateDiagram-v2
    [*] --> ToolSelected
    ToolSelected --> SpindleSpeedSet : spindleSpeedSet
    SpindleSpeedSet --> FeedRateSet : feedRateSet
    FeedRateSet --> CutDepthSet : cutDepthSet
    CutDepthSet --> CoolantFlowSet : coolantFlowSet
    CoolantFlowSet --> ToolPathProgrammed : toolPathProgrammed
    ToolPathProgrammed --> SetupValidated : setupValidated
    SetupValidated --> SpindleSpeedSet : spindleSpeedSet
    SetupValidated --> [*]
```
