...
```mermaid
erDiagram
    ClimateZone ||--o{ Thermostat : "controlled by"
    Thermostat ||--o{ Sensor : "reads from"
    ClimateZone ||--o{ AirHandler : "conditioned by"
    SetpointSchedule ||--|| ClimateZone : "governs"
    PerformanceLog ||--o{ Sensor : "records readings from"
    AirHandler ||--o{ PerformanceLog : "tracked in"
```
...
stateDiagram-v2
    [*] --> Monitoring
    Monitoring --> SetpointModified : setpointAdjusted
    SetpointModified --> SensorCalibrating : sensorCalibrated
    SensorCalibrating --> AirflowBalancing : airflowBalanced
    AirflowBalancing --> ScheduleOptimizing : scheduleOptimized
    ScheduleOptimizing --> Troubleshooting : issueTroubleshooted
    Troubleshooting --> Monitoring : conditionsMonitored
    ScheduleOptimizing --> Logged : performanceLogged
    Logged --> Monitoring : conditionsMonitored
    Logged --> [*]
```
