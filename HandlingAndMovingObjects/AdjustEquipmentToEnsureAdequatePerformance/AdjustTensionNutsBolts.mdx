...
```mermaid
erDiagram
    AssemblyTechnician ||--|| MaintenanceWorker : follows
    AssemblyTechnician }o--|| QualityInspector : generates
    AssemblyTechnician ||--|{ DesignEngineer : requires
    AssemblyTechnician ||--o{ ToolSupplier : tracks
    AssemblyTechnician ||--|| SafetyInspector : validates
    MaintenanceWorker ||--|{ AssemblyTechnician : creates
    QualityInspector ||--o{ AssemblyTechnician : documents
    DesignEngineer ||--|| AssemblyTechnician : supports
```
...
stateDiagram-v2
    [*] --> Assembly_Technician
    Assembly_Technician --> Maintenance_Worker: MaintenanceWorker
    Maintenance_Worker --> Quality_Inspector: QualityInspector
    Quality_Inspector --> Design_Engineer: DesignEngineer
    Design_Engineer --> Tool_Supplier: ToolSupplier
    Tool_Supplier --> Safety_Inspector: SafetyInspector
    Design_Engineer --> Maintenance_Worker: QualityInspector
    Safety_Inspector --> [*]
```
