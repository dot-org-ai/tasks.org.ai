...
```mermaid
erDiagram
    OfficeEquipment ||--o{ EquipmentSettings : "configured with"
    SupportTicket ||--|| OfficeEquipment : "reported for"
    MaintenanceTask ||--|| OfficeEquipment : "scheduled on"
    ConfigurationChange ||--|| EquipmentSettings : "modifies"
    ServiceRecord ||--o{ MaintenanceTask : "documents"
    ServiceRecord ||--o{ ConfigurationChange : "logs"
```
...
stateDiagram-v2
    [*] --> Inspected
    Inspected --> IssueDiagnosed : issueDiagnosed
    IssueDiagnosed --> SettingsModified : settingsAdjusted
    IssueDiagnosed --> ComponentsSwapped : componentsReplaced
    SettingsModified --> OperationVerified : operationTested
    ComponentsSwapped --> OperationVerified : operationTested
    OperationVerified --> Documented : serviceDocumented
    OperationVerified --> Inspected : deviceInspected
    Documented --> [*]
```
