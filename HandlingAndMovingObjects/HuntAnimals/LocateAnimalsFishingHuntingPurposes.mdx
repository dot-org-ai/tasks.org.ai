...
```mermaid
erDiagram
    TargetSpecies ||--o{ ScoutingArea : "defines"
    TargetSpecies ||--|{ Sign : "generates"
    TargetSpecies }o--|| Pattern : "uses"
    TargetSpecies ||--|| Location : "tracked-by"
    TargetSpecies ||--o{ Observation : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : reconnaissancePlanned
    InProgress --> Verified : habitatSurveyed
    Verified --> Approved : signIdentified
    Approved --> Active : movementTracked
    Active --> Complete : technologyDeployed
