...
```mermaid
erDiagram
    ScoutingArea ||--o{ TargetSpecies : "surveyed-for"
    TargetSpecies ||--o{ Sign : "evidenced-by"
    Sign ||--o{ Pattern : "reveals"
    Pattern ||--o{ Location : "predicts"
    Location ||--o{ Observation : "confirmed-by"
```
...
```mermaid
stateDiagram-v2
    [*] --> Planned : reconnaissancePlanned
    Planned --> Surveyed : habitatSurveyed
    Surveyed --> Monitored : technologyDeployed
    Monitored --> SignFound : signIdentified
    SignFound --> Tracked : movementTracked
    Tracked --> Observed : observationRecorded
    Observed --> Analyzed : patternAnalyzed
    Analyzed --> Surveyed : reconnaissancePlanned
    Analyzed --> [*]
```
## Workflow
