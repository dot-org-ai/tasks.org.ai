...
```mermaid
erDiagram
    Permit ||--o{ TargetSpecies : "authorizes"
    Location ||--o{ TargetSpecies : "inhabited-by"
    TargetSpecies ||--|| PopulationData : "surveyed-in"
    Method ||--o{ Harvest : "employed-for"
    Harvest ||--|| Permit : "reported-against"
```
...
```mermaid
stateDiagram-v2
    [*] --> Permitted : permitObtained
    Permitted --> Located : locationSelected
    Located --> Deployed : methodDeployed
    Deployed --> Harvested : harvestExecuted
    Harvested --> Documented : outcomeDocumented
    Documented --> Reported : complianceReported
    Reported --> Processed : harvestProcessed
    Reported --> Deployed : methodDeployed
    Processed --> [*]
```
## Workflow
