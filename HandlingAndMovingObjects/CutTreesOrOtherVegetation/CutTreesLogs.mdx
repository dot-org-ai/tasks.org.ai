...
```mermaid
erDiagram
    TimberStand ||--o{ CuttingBlock : "divided-into"
    CuttingBlock ||--|| HarvestPermit : "authorized-by"
    CuttingBlock ||--|| FellingPlan : "governed-by"
    TimberStand ||--o{ Log : "yields"
    Log ||--|| LoadTicket : "shipped-on"
    TimberStand ||--o{ StumpRecord : "documented-by"
```
...
```mermaid
stateDiagram-v2
    [*] --> Assessed : standAssessed
    Assessed --> Planned : fellingPlanned
    Planned --> Felled : treeFelled
    Felled --> Limbed : treeLimbed
    Limbed --> Bucked : logBucked
    Bucked --> Scaled : logScaled
    Scaled --> Decked : logsDecked
    Scaled --> Felled : logScaled
    Decked --> [*]
```
## Workflow
