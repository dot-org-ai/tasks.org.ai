...
```mermaid
erDiagram
    TreeInventory ||--o{ PruningPlan : "defines"
    TreeInventory ||--|{ WorkOrder : "generates"
    TreeInventory }o--|| BranchDebris : "uses"
    TreeInventory ||--|| AerialLift : "tracked-by"
    TreeInventory ||--o{ SafetyZone : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : treeAssessed
    InProgress --> Verified : pruningPlanCreated
    Verified --> Approved : canopyAccessed
    Approved --> Active : canopyPruned
    Active --> Complete : vegetationShaped
