...
```mermaid
erDiagram
    Equipment ||--o{ WorkOrder : "defines"
    Equipment ||--|{ LockoutTagout : "generates"
    Equipment }o--|| DisassemblySequence : "uses"
    Equipment ||--|| WornComponent : "tracked-by"
    Equipment ||--o{ ServiceManual : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : lockoutIssued
    InProgress --> Verified : guardsRemoved
    Verified --> Approved : utilitiesDisconnected
    Approved --> Active : subassemblyDisassembled
    Active --> Complete : componentsInspected
