...
```mermaid
erDiagram
    WorkOrder ||--|| Equipment : "targets"
    Equipment ||--|| LockoutTagout : "isolated-by"
    Equipment ||--|| ServiceManual : "documented-in"
    ServiceManual ||--|| DisassemblySequence : "prescribes"
    Equipment ||--o{ WornComponent : "exposes"
    WornComponent ||--o{ InspectionFindings : "recorded-in"
```
...
```mermaid
stateDiagram-v2
    [*] --> LockedOut : lockoutIssued
    LockedOut --> GuardsOff : guardsRemoved
    GuardsOff --> Disconnected : utilitiesDisconnected
    Disconnected --> Disassembled : subassemblyDisassembled
    Disassembled --> Inspected : componentsInspected
    Inspected --> Documented : findingsDocumented
    Documented --> PartsReady : partsStaged
    Inspected --> Disassembled : componentsInspected
    PartsReady --> [*]
```
## Workflow
