...
```mermaid
erDiagram
    Vehicle ||--o{ VehiclePart : "contains"
    RemovalOrder ||--|{ VehiclePart : "targets"
    VehiclePart ||--|| DisconnectionSequence : "removed-per"
    VehiclePart ||--|| PartConditionReport : "assessed-in"
    VehiclePart ||--o{ HazardousMaterial : "contains"
    VehiclePart ||--|| InventoryRecord : "cataloged-in"
```
...
```mermaid
stateDiagram-v2
    [*] --> Identified : partIdentified
    Identified --> Drained : fluidsDrained
    Drained --> Disconnected : systemsDisconnected
    Disconnected --> Extracted : partExtracted
    Extracted --> Assessed : conditionAssessed
    Assessed --> Cataloged : partCataloged
    Cataloged --> ReadyForReinstall : reinstallReady
    Assessed --> Disconnected : conditionAssessed
    ReadyForReinstall --> [*]
```
## Workflow
