...
```mermaid
erDiagram
    HeavyMachine ||--o{ LiftPlan : "defines"
    HeavyMachine ||--|{ CuttingSequence : "generates"
    HeavyMachine }o--|| RiggedLoad : "uses"
    HeavyMachine ||--|| HazmatSurvey : "tracked-by"
    HeavyMachine ||--o{ TransportPermit : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : hazardsSurveyed
    InProgress --> Verified : liftPlanEngineered
    Verified --> Approved : foundationDisconnected
    Approved --> Active : structureCut
    Active --> Complete : sectionLifted
