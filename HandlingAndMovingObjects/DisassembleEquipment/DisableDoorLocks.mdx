...
```mermaid
erDiagram
    LockMechanism ||--o{ AuthorizationRecord : "defines"
    LockMechanism ||--|{ BypassTool : "generates"
    LockMechanism }o--|| EntryLog : "uses"
    LockMechanism ||--|| DamageAssessment : "tracked-by"
    LockMechanism ||--o{ ReplacementLock : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : authorizationVerified
    InProgress --> Verified : lockAssessed
    Verified --> Approved : methodSelected
    Approved --> Active : lockDisabled
    Active --> Complete : entryDocumented
