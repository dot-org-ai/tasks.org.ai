...
```mermaid
erDiagram
    InspectionOrder ||--o{ Deficiency : "defines"
    InspectionOrder ||--|{ FailureReport : "generates"
    InspectionOrder }o--|| NDTResult : "uses"
    InspectionOrder ||--|| MeasurementLog : "tracked-by"
    InspectionOrder ||--o{ PhotographicEvidence : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : orderReceived
    InProgress --> Verified : equipmentIsolated
    Verified --> Approved : accessAchieved
    Approved --> Active : ndtCompleted
    Active --> Complete : tolerancesMeasured
