...
```mermaid
erDiagram
    EquipmentComponent ||--o{ RemovalProcedure : "defines"
    EquipmentComponent ||--|{ EnergyIsolation : "generates"
    EquipmentComponent }o--|| ReplacementPart : "uses"
    EquipmentComponent ||--|| PartTag : "tracked-by"
    EquipmentComponent ||--o{ TorqueRecord : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : componentIdentified
    InProgress --> Verified : energyIsolated
    Verified --> Approved : procedureFollowed
    Approved --> Active : componentExtracted
    Active --> Complete : partLabeledAndTracked
