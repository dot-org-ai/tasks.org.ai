...
```mermaid
erDiagram
    ChangeoverOrder ||--|{ Accessory : "targets"
    Accessory ||--o| QuickConnect : "attached-via"
    Accessory ||--|| ConditionTag : "labeled-with"
    Accessory ||--|| ToolingRegister : "tracked-in"
    RemovalProcedure ||--|| Accessory : "guides"
    Accessory ||--o| CalibrationRecord : "measured-by"
```
...
```mermaid
stateDiagram-v2
    [*] --> Identified : accessoryIdentified
    Identified --> SafeChecked : safetyCheckPassed
    SafeChecked --> Released : attachmentReleased
    Released --> Removed : accessoryRemoved
    Removed --> Tagged : conditionTagged
    Tagged --> Stored : returnedToStorage
    Stored --> Updated : registerUpdated
    Tagged --> SafeChecked : conditionTagged
    Updated --> [*]
```
## Workflow
