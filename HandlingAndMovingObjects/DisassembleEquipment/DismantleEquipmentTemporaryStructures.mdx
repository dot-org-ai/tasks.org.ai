...
```mermaid
erDiagram
    TemporaryStructure ||--o{ DismantlingPlan : "defines"
    TemporaryStructure ||--|{ RecoveredMaterial : "generates"
    TemporaryStructure }o--|| DebrisLoad : "uses"
    TemporaryStructure ||--|| SiteRestorationReport : "tracked-by"
    TemporaryStructure ||--o{ LiftPlan : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : teardownPlanDeveloped
    InProgress --> Verified : exclusionEstablished
    Verified --> Approved : componentsRemoved
    Approved --> Active : materialsLowered
    Active --> Complete : materialsSorted
