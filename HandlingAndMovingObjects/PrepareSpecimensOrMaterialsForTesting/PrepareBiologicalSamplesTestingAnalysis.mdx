...
```mermaid
erDiagram
    Sample ||--o{ Aliquot : "divided-into"
    Sample ||--|| PreservationMethod : "stabilized-by"
    ProcessingProtocol ||--|| Sample : "governs"
    Sample ||--|| ChainOfCustody : "tracked-in"
    Sample ||--|| QualityMetric : "assessed-by"
```
...
stateDiagram-v2
    [*] --> SampleReceived
    SampleReceived --> QualityValidated : qualityValidated
    QualityValidated --> SampleLabeled : sampleLabeled
    SampleLabeled --> SampleCentrifuged : sampleCentrifuged
    SampleCentrifuged --> SampleAliquoted : sampleAliquoted
    SampleAliquoted --> SamplePreserved : samplePreserved
    SamplePreserved --> SampleStored : sampleStored
    QualityValidated --> SampleReceived : sampleReceived
    SampleStored --> [*]
```
