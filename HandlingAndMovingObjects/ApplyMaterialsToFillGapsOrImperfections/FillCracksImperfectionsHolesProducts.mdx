...
```mermaid
erDiagram
    FillerSupplier ||--|| Customer : follows
    FillerSupplier }o--|| ProductionManager : generates
    FillerSupplier ||--|{ SafetyOfficer : requires
    FillerSupplier ||--o{ EquipmentVendor : tracks
    Customer ||--|{ FillerSupplier : creates
    ProductionManager ||--o{ FillerSupplier : documents
    SafetyOfficer ||--|| FillerSupplier : supports
```
...
stateDiagram-v2
    [*] --> Filler_Supplier
    Filler_Supplier --> Customer: Customer
    Customer --> Production_Manager: ProductionManager
    Production_Manager --> Safety_Officer: SafetyOfficer
    Safety_Officer --> Equipment_Vendor: EquipmentVendor
    Production_Manager --> Customer: ProductionManager
    Equipment_Vendor --> [*]
```
