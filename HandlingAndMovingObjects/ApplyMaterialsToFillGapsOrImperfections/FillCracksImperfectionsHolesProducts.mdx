...
```mermaid
erDiagram
    Defect ||--|| Workpiece : "found on"
    FillerMaterial ||--|| Defect : "fills"
    MixBatch ||--|| FillerMaterial : "prepares"
    SurfaceStandard ||--|| Workpiece : "defines quality for"
    RepairRecord ||--|| Defect : "documents treatment of"
    RepairRecord ||--|| FillerMaterial : "records usage of"
```
...
stateDiagram-v2
    [*] --> DefectIdentified
    DefectIdentified --> SurfacePrepared : surfacePrepared
    SurfacePrepared --> FillerMixed : fillerMixed
    FillerMixed --> FillerApplied : fillerApplied
    FillerApplied --> FillerCured : fillerCured
    FillerCured --> SurfaceFinished : surfaceFinished
    SurfaceFinished --> RepairRejected : repairRejected
    RepairRejected --> SurfacePrepared : surfacePrepared
    SurfaceFinished --> [*]
```
