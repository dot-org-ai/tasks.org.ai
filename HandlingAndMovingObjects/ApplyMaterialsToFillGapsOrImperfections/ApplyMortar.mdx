...
```mermaid
erDiagram
    MortarBatch ||--|| MortarType : "classified as"
    MortarBatch ||--o{ BedJoint : "spread into"
    MortarBatch ||--o{ HeadJoint : "buttered onto"
    JointProfile ||--|| BedJoint : "shapes"
    JointProfile ||--|| HeadJoint : "shapes"
    CubeTest ||--|| MortarBatch : "verifies strength of"
```
...
stateDiagram-v2
    [*] --> MortarBatched
    MortarBatched --> BedJointSpread : bedJointSpread
    BedJointSpread --> HeadJointButtered : headJointButtered
    HeadJointButtered --> UnitSet : unitSet
    UnitSet --> BedJointSpread : bedJointSpread
    UnitSet --> JointsTooled : jointsTooled
    JointsTooled --> MortarStrengthTested : mortarStrengthTested
    MortarBatched --> MortarExpired : mortarExpired
    MortarStrengthTested --> [*]
```
