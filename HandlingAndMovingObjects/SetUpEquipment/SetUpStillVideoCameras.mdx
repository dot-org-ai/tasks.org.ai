...
```mermaid
erDiagram
    ShotList ||--|{ CameraRig : "directs"
    CameraRig ||--|| CameraSettings : "configured-with"
    CameraRig ||--|| LightingSetup : "lit-by"
    CameraRig ||--|| AudioKit : "records-with"
    EquipmentManifest ||--|{ CameraRig : "inventories"
```
...
```mermaid
stateDiagram-v2
    [*] --> GripBuilt : gripSetupBuilt
    GripBuilt --> Mounted : cameraMounted
    Mounted --> Configured : cameraSettingsConfigured
    Configured --> Lit : lightingPositioned
    Lit --> WhiteBalanced : whiteBalanceCalibrated
    WhiteBalanced --> Tested : testRecordingCompleted
    Tested --> Configured : adjustSettings
    Tested --> Shooting
    Shooting --> PackedDown : equipmentPackedDown
    PackedDown --> [*]
```
