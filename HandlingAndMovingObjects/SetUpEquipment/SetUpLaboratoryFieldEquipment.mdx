...
```mermaid
erDiagram
    Instrument ||--o{ CalibrationRecord : "calibrated-by"
    Instrument ||--|| SamplingKit : "paired-with"
    MethodProtocol ||--|| Instrument : "governs"
    FieldStation ||--|{ Instrument : "deploys"
    Instrument ||--o{ MaintenanceLog : "serviced-per"
```
...
```mermaid
stateDiagram-v2
    [*] --> Assembled : instrumentAssembled
    Assembled --> Calibrated : calibrationCompleted
    Calibrated --> Deployed : fieldStationDeployed
    Deployed --> Verified : readinessVerified
    Verified --> Logging : dataLoggerConfigured
    Logging --> Tested : functionalTestPassed
    Tested --> Documented : setupDocumented
    Calibrated --> Assembled : recalibrationNeeded
    Documented --> [*]
```
