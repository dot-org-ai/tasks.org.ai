...
```mermaid
erDiagram
    SetupWorkOrder ||--|{ RiggingAssembly : "directs"
    RiggingAssembly ||--|| LoadChart : "rated-by"
    RiggingAssembly ||--|| InspectionTag : "certified-with"
    SetupWorkOrder ||--|| PackagingStation : "configures"
    SetupWorkOrder ||--|| TemporaryStructure : "erects"
```
...
```mermaid
stateDiagram-v2
    [*] --> Assembled : riggingAssembled
    Assembled --> Configured : packagingStationConfigured
    Configured --> Erected : temporaryStructureErected
    Erected --> Verified : loadRatingsVerified
    Verified --> Tagged : equipmentTagged
    Tagged --> InUse
    InUse --> Dismantled : setupDismantled
    Dismantled --> Logged : setupCompletionLogged
    Verified --> Assembled : ratingsFailed
    Logged --> [*]
```
