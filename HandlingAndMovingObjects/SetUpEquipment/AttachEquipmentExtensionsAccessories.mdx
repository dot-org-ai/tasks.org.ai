...
```mermaid
erDiagram
    BaseEquipment ||--|{ AttachmentPoint : "exposes"
    Extension ||--|| AttachmentPoint : "connects-to"
    Accessory ||--|| BaseEquipment : "enhances"
    Extension ||--o{ CompatibilityRecord : "validates"
    Extension ||--|| LoadChart : "adjusts"
```
...
stateDiagram-v2
    ...
    Verified --> Prepared : attachmentPointPrepared
    Prepared --> Mounted : extensionMounted
    Mounted --> Secured : fastenersSecured
    Secured --> LoadUpdated : loadChartUpdated
    LoadUpdated --> Approved : stabilityTestPassed
    LoadUpdated --> Rejected : configurationRejected
    Rejected --> Prepared : reconfigure
    Approved --> [*]
```
