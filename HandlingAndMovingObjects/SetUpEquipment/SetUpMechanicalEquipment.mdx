...
```mermaid
erDiagram
    Equipment ||--|| Foundation : "anchored-to"
    Equipment ||--o{ LevelingRecord : "measured-by"
    Equipment ||--|{ UtilityConnection : "powered-by"
    Equipment ||--|| CommissioningReport : "verified-by"
    Equipment ||--|{ SafetyGuard : "protected-by"
```
...
```mermaid
stateDiagram-v2
    [*] --> Positioned : equipmentPositioned
    Positioned --> Anchored : anchoredToFoundation
    Anchored --> Leveled : machineLeveled
    Leveled --> Connected : utilitiesConnected
    Connected --> Guarded : safetyGuardsInstalled
    Guarded --> Calibrated : machineCalibrated
    Calibrated --> Commissioned : equipmentCommissioned
    Calibrated --> Leveled : realignmentNeeded
    Commissioned --> [*]
```
