...
```mermaid
erDiagram
    Equipment ||--|{ MechanicalComponent : "houses"
    MechanicalComponent ||--|| InstallationProcedure : "follows"
    TorqueSpecification ||--|| MechanicalComponent : "constrains"
    MechanicalComponent ||--o{ AlignmentRecord : "measures"
    WorkOrder ||--|{ MechanicalComponent : "authorizes"
```
...
```mermaid
stateDiagram-v2
    [*] --> LockedOut : equipmentLockedOut
    LockedOut --> Removed : wornComponentRemoved
    Removed --> Prepped : mountPrepared
    Prepped --> Installed : componentInstalled
    Installed --> Torqued : fastenersTorqued
    Torqued --> Aligned : componentAligned
    Aligned --> Tested : operationTested
    Aligned --> Torqued : realignmentNeeded
    Tested --> [*]
```
