...
```mermaid
erDiagram
    SetupSheet ||--|{ FixtureSetting : "specifies"
    FixtureSetting ||--|| Guide : "positions"
    FixtureSetting ||--|| Stop : "limits"
    FixtureSetting ||--|| Spacer : "offsets"
...
```mermaid
stateDiagram-v2
    [*] --> Reviewed : setupSheetReviewed
    Reviewed --> GuidesSet : guidesPositioned
    GuidesSet --> StopsSet : stopsSet
    StopsSet --> SpacersIn : spacersInserted
    SpacersIn --> Locked : fixturesLocked
    Locked --> Produced : firstArticleProduced
    Produced --> Verified : dimensionsVerified
    Verified --> GuidesSet : outOfTolerance
    Verified --> [*]
```
