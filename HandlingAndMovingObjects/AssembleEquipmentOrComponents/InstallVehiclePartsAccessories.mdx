...
```mermaid
erDiagram
    Vehicle ||--o{ Part : "receives"
    Part ||--|| InstallationKit : "installed-with"
    Vehicle ||--|| WorkOrder : "authorized-by"
    WorkOrder ||--o{ Part : "specifies"
    Part ||--|| DiagnosticReport : "validated-by"
    Part ||--|| WarrantyRegistration : "covered-by"
```
...
stateDiagram-v2
    [*] --> Consulting
    Consulting --> Ordering : customerConsulted
    Ordering --> Removing : partsOrdered
    Removing --> Installing : oldPartRemoved
    Installing --> Connecting : newPartInstalled
    Connecting --> Testing : systemsConnected
    Testing --> Inspecting : functionTested
    Testing --> Installing : testFailed
    Inspecting --> [*] : installationInspected
```
