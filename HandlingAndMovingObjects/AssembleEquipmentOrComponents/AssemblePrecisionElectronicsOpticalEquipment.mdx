...
```mermaid
erDiagram
    ComponentVendor ||--|| EngineeringGroup : follows
    ComponentVendor }o--|| CleanroomServices : generates
    ComponentVendor ||--|{ CalibrationLab : requires
    ComponentVendor ||--o{ CustomerService : tracks
    ComponentVendor ||--|| RegulatorAgency : validates
    EngineeringGroup ||--|{ ComponentVendor : creates
    CleanroomServices ||--o{ ComponentVendor : documents
    CalibrationLab ||--|| ComponentVendor : supports
```
...
stateDiagram-v2
    [*] --> Component_Vendor
    Component_Vendor --> Engineering_Group: EngineeringGroup
    Engineering_Group --> Cleanroom_Services: CleanroomServices
    Cleanroom_Services --> Calibration_Lab: CalibrationLab
    Calibration_Lab --> Customer_Service: CustomerService
    Customer_Service --> Regulator_Agency: RegulatorAgency
    Calibration_Lab --> Engineering_Group: CleanroomServices
    Regulator_Agency --> [*]
```
