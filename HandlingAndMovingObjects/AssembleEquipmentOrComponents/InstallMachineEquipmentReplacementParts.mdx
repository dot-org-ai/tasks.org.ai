...
```mermaid
erDiagram
    WorkOrder ||--|| FailedComponent : "identifies"
    WorkOrder ||--|| ReplacementPart : "procures"
    ReplacementPart ||--|| InstallationProcedure : "installed-per"
    FailedComponent ||--|| ReplacementPart : "replaced-by"
    ReplacementPart ||--|| FunctionalTest : "validated-by"
    WorkOrder ||--|| MaintenanceRecord : "documented-in"
```
...
stateDiagram-v2
    [*] --> Diagnosing
    Diagnosing --> Ordering : failureDiagnosed
    Ordering --> Removing : replacementPartOrdered
    Removing --> Preparing : failedComponentRemoved
    Preparing --> Installing : mountingSurfacePrepared
    Installing --> Adjusting : replacementPartInstalled
    Adjusting --> Testing : alignmentAdjusted
    Testing --> Complete : functionTested
    Testing --> Adjusting : testFailed
    Complete --> [*]
```
