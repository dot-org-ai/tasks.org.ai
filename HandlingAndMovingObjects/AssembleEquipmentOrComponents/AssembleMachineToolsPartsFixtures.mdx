...
```mermaid
erDiagram
    ToolingVendor ||--|| QualityControl : follows
    ToolingVendor }o--|| ProductionPlanner : generates
    ToolingVendor ||--|{ Machinist : requires
    ToolingVendor ||--o{ CalibrationLab : tracks
    ToolingVendor ||--|| MaintenanceTeam : validates
    QualityControl ||--|{ ToolingVendor : creates
    ProductionPlanner ||--o{ ToolingVendor : documents
    Machinist ||--|| ToolingVendor : supports
```
...
stateDiagram-v2
    [*] --> Tooling_Vendor
    Tooling_Vendor --> Quality_Control: QualityControl
    Quality_Control --> Production_Planner: ProductionPlanner
    Production_Planner --> Machinist: Machinist
    Machinist --> Calibration_Lab: CalibrationLab
    Calibration_Lab --> Maintenance_Team: MaintenanceTeam
    Machinist --> Quality_Control: ProductionPlanner
    Maintenance_Team --> [*]
```
