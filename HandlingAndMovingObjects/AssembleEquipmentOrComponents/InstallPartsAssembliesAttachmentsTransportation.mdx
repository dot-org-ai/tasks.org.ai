...
```mermaid
erDiagram
    EquipmentDealer ||--|| AttachmentManufacturer : follows
    EquipmentDealer }o--|| FleetManagement : generates
    EquipmentDealer ||--|{ SafetyInspector : requires
    EquipmentDealer ||--o{ EquipmentOperator : tracks
    EquipmentDealer ||--|| RegulatorAgency : validates
    AttachmentManufacturer ||--|{ EquipmentDealer : creates
    FleetManagement ||--o{ EquipmentDealer : documents
    SafetyInspector ||--|| EquipmentDealer : supports
```
...
stateDiagram-v2
    [*] --> Equipment_Dealer
    Equipment_Dealer --> Attachment_Manufacturer: AttachmentManufacturer
    Attachment_Manufacturer --> Fleet_Management: FleetManagement
    Fleet_Management --> Safety_Inspector: SafetyInspector
    Safety_Inspector --> Equipment_Operator: EquipmentOperator
    Equipment_Operator --> Regulator_Agency: RegulatorAgency
    Safety_Inspector --> Attachment_Manufacturer: FleetManagement
    Regulator_Agency --> [*]
```
