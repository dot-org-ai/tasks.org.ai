...
```mermaid
erDiagram
    SubassemblySupplier ||--|| ManufacturingEngineering : follows
    SubassemblySupplier }o--|| ProductionScheduler : generates
    SubassemblySupplier ||--|{ QualityDepartment : requires
    SubassemblySupplier ||--o{ ShippingLogistics : tracks
    SubassemblySupplier ||--|| ServiceTeam : validates
    ManufacturingEngineering ||--|{ SubassemblySupplier : creates
    ProductionScheduler ||--o{ SubassemblySupplier : documents
    QualityDepartment ||--|| SubassemblySupplier : supports
```
...
```mermaid
stateDiagram-v2
    [*] --> Subassembly_Supplier
    Subassembly_Supplier --> Manufacturing_Engineering: ManufacturingEngineering
    Manufacturing_Engineering --> Production_Scheduler: ProductionScheduler
    Production_Scheduler --> Quality_Department: QualityDepartment
    Quality_Department --> Shipping_Logistics: ShippingLogistics
    Shipping_Logistics --> Service_Team: ServiceTeam
    Quality_Department --> Manufacturing_Engineering: ProductionScheduler
    Service_Team --> [*]
```
