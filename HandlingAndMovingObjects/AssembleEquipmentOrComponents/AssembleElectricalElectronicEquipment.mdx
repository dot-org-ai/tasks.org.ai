...
```mermaid
erDiagram
    PartsSupplier ||--|| ProductEngineering : follows
    PartsSupplier }o--|| TestEquipmentProvider : generates
    PartsSupplier ||--|{ Customer : requires
    PartsSupplier ||--o{ ComplianceAuthority : tracks
    PartsSupplier ||--|| CalibrationLab : validates
    ProductEngineering ||--|{ PartsSupplier : creates
    TestEquipmentProvider ||--o{ PartsSupplier : documents
    Customer ||--|| PartsSupplier : supports
```
...
stateDiagram-v2
    [*] --> Parts_Supplier
    Parts_Supplier --> Product_Engineering: ProductEngineering
    Product_Engineering --> Test_Equipment_Provider: TestEquipmentProvider
    Test_Equipment_Provider --> Customer: Customer
    Customer --> Compliance_Authority: ComplianceAuthority
    Compliance_Authority --> Calibration_Lab: CalibrationLab
    Customer --> Product_Engineering: TestEquipmentProvider
    Calibration_Lab --> [*]
```
