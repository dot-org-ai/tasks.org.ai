...
```mermaid
erDiagram
    WorkOrder ||--o{ Kit : "staged-by"
    Kit ||--o{ Component : "contains"
    WorkOrder ||--o{ AssemblyStep : "sequences"
    AssemblyStep ||--|| Fixture : "uses"
    AssemblyStep ||--|| InspectionPoint : "checked-at"
    Component ||--|| InspectionPoint : "verified-by"
```
...
stateDiagram-v2
    [*] --> Kitting
    Kitting --> Sequencing : componentsKitted
    Sequencing --> Aligning : sequenceFollowed
    Aligning --> Fastening : partsAligned
    Fastening --> Verifying : componentsFastened
    Verifying --> Inspecting : fitVerified
    Inspecting --> Packing : assemblyInspected
    Verifying --> Aligning : fitFailed
    Packing --> [*] : productPacked
```
