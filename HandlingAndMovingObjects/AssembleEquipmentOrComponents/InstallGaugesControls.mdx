...
```mermaid
erDiagram
    Gauge ||--|| ControlPanel : "mounted-on"
    Sensor ||--|| SignalWiring : "connected-via"
    SignalWiring ||--|| ControlPanel : "routed-to"
    Gauge ||--|| CalibrationCertificate : "certified-by"
    Gauge ||--|| InstallationDrawing : "located-per"
    Sensor ||--|| CalibrationCertificate : "calibrated-in"
```
...
stateDiagram-v2
    [*] --> SitePrep
    SitePrep --> Mounting : installationSitePrepared
    Mounting --> Wiring : instrumentsMounted
    Wiring --> Configuring : signalsConnected
    Configuring --> Calibrating : softwareConfigured
    Calibrating --> OperationTest : instrumentsCalibrated
    OperationTest --> Documenting : operationTested
    Calibrating --> Configuring : outOfTolerance
    Documenting --> [*] : installationDocumented
```
