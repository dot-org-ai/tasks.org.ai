...
```mermaid
erDiagram
    SystemOrder ||--|| MechanicalAssembly : "builds"
    MechanicalAssembly ||--|| HydraulicCircuit : "powered-by"
    HydraulicCircuit ||--|| ControlSystem : "governed-by"
    HydraulicCircuit ||--|| PressureTest : "verified-by"
    SystemOrder ||--|| FunctionalTest : "validated-by"
    ControlSystem ||--|| FunctionalTest : "tested-in"
```
...
stateDiagram-v2
    [*] --> MechanicalBuild
    MechanicalBuild --> HydraulicInstall : mechanicalAssembled
    HydraulicInstall --> WiringControls : hydraulicsInstalled
    WiringControls --> Filling : controlsWired
    Filling --> PressureTesting : systemFilled
    PressureTesting --> Commissioning : pressureTested
    Commissioning --> Handover : systemCommissioned
    PressureTesting --> HydraulicInstall : leakDetected
    Handover --> [*] : handoverDocumented
```
