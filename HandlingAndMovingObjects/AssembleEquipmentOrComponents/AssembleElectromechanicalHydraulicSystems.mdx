...
```mermaid
erDiagram
    HydraulicSupplier ||--|| MechanicalSupplier : follows
    HydraulicSupplier }o--|| ElectricalSupplier : generates
    HydraulicSupplier ||--|{ SystemEngineer : requires
    HydraulicSupplier ||--o{ SafetyInspector : tracks
    HydraulicSupplier ||--|| EndUser : validates
    MechanicalSupplier ||--|{ HydraulicSupplier : creates
    ElectricalSupplier ||--o{ HydraulicSupplier : documents
    SystemEngineer ||--|| HydraulicSupplier : supports
```
...
stateDiagram-v2
    [*] --> Hydraulic_Supplier
    Hydraulic_Supplier --> Mechanical_Supplier: MechanicalSupplier
    Mechanical_Supplier --> Electrical_Supplier: ElectricalSupplier
    Electrical_Supplier --> System_Engineer: SystemEngineer
    System_Engineer --> Safety_Inspector: SafetyInspector
    Safety_Inspector --> End_User: EndUser
    System_Engineer --> Mechanical_Supplier: ElectricalSupplier
    End_User --> [*]
```
