...
```mermaid
erDiagram
    BoltedJoint ||--o{ Fastener : "secured-with"
    BoltedJoint ||--|| BoltPattern : "arranged-per"
    Fastener ||--|| TorqueSpecification : "tightened-to"
    BoltedJoint ||--|| TorqueRecord : "documented-in"
    TorqueRecord ||--|| JointInspection : "verified-by"
    BoltPattern ||--|| TorqueSpecification : "prescribes"
```
...
stateDiagram-v2
    [*] --> Aligning
    Aligning --> Inserting : componentsAligned
    Inserting --> PrelimTorque : fastenersInserted
    PrelimTorque --> FinalTorque : preliminaryTorqueApplied
    FinalTorque --> Verifying : finalTorqueApplied
    Verifying --> Marking : torqueVerified
    Marking --> Inspecting : fastenersMarked
    Verifying --> FinalTorque : torqueOutOfSpec
    Inspecting --> [*] : jointInspected
```
