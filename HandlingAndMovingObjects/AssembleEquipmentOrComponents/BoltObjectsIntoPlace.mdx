...
```mermaid
erDiagram
    FastenerSupplier ||--|| EngineeringStandards : follows
    FastenerSupplier }o--|| StructuralDesigner : generates
    FastenerSupplier ||--|{ SafetyInspector : requires
    FastenerSupplier ||--o{ MaintenanceGroup : tracks
    FastenerSupplier ||--|| RegulatorBody : validates
    EngineeringStandards ||--|{ FastenerSupplier : creates
    StructuralDesigner ||--o{ FastenerSupplier : documents
    SafetyInspector ||--|| FastenerSupplier : supports
```
...
stateDiagram-v2
    [*] --> Fastener_Supplier
    Fastener_Supplier --> Engineering_Standards: EngineeringStandards
    Engineering_Standards --> Structural_Designer: StructuralDesigner
    Structural_Designer --> Safety_Inspector: SafetyInspector
    Safety_Inspector --> Maintenance_Group: MaintenanceGroup
    Maintenance_Group --> Regulator_Body: RegulatorBody
    Safety_Inspector --> Engineering_Standards: StructuralDesigner
    Regulator_Body --> [*]
```
