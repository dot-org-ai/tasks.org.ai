...
```mermaid
erDiagram
    Equipment ||--o{ Component : "rebuilt-from"
    Equipment ||--|| RepairRecord : "documented-in"
    RepairRecord ||--|| AssemblySequence : "follows"
    AssemblySequence ||--o{ Component : "orders"
    Equipment ||--|| FunctionalTest : "validated-by"
    Equipment ||--|| ReturnToServiceCertification : "authorized-by"
```
...
stateDiagram-v2
    [*] --> Reviewing
    Reviewing --> Preparing : repairWorkReviewed
    Preparing --> Installing : componentsPrepared
    Installing --> Torquing : componentsInstalled
    Torquing --> Adjusting : fastenersTorqued
    Adjusting --> Testing : clearancesAdjusted
    Testing --> Certifying : functionTested
    Testing --> Adjusting : testFailed
    Certifying --> [*] : completionCertified
```
