...
```mermaid
erDiagram
    RepairShop ||--|| PartsSupplier : follows
    RepairShop }o--|| QualityAssurance : generates
    RepairShop ||--|{ EquipmentOwner : requires
    RepairShop ||--o{ CertificationBody : tracks
    RepairShop ||--|| InsuranceProvider : validates
    PartsSupplier ||--|{ RepairShop : creates
    QualityAssurance ||--o{ RepairShop : documents
    EquipmentOwner ||--|| RepairShop : supports
```
...
stateDiagram-v2
    [*] --> Repair_Shop
    Repair_Shop --> Parts_Supplier: PartsSupplier
    Parts_Supplier --> Quality_Assurance: QualityAssurance
    Quality_Assurance --> Equipment_Owner: EquipmentOwner
    Equipment_Owner --> Certification_Body: CertificationBody
    Certification_Body --> Insurance_Provider: InsuranceProvider
    Equipment_Owner --> Parts_Supplier: QualityAssurance
    Insurance_Provider --> [*]
```
