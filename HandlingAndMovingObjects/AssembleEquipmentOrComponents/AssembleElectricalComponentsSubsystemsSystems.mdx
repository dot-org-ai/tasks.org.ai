...
```mermaid
erDiagram
    ComponentSupplier ||--|| EngineeringDesign : follows
    ComponentSupplier }o--|| QualityLab : generates
    ComponentSupplier ||--|{ Customer : requires
    ComponentSupplier ||--o{ CertificationBody : tracks
    ComponentSupplier ||--|| ToolingVendor : validates
    EngineeringDesign ||--|{ ComponentSupplier : creates
    QualityLab ||--o{ ComponentSupplier : documents
    Customer ||--|| ComponentSupplier : supports
```
...
stateDiagram-v2
    [*] --> Component_Supplier
    Component_Supplier --> Engineering_Design: EngineeringDesign
    Engineering_Design --> Quality_Lab: QualityLab
    Quality_Lab --> Customer: Customer
    Customer --> Certification_Body: CertificationBody
    Certification_Body --> Tooling_Vendor: ToolingVendor
    Customer --> Engineering_Design: QualityLab
    Tooling_Vendor --> [*]
```
