...
```mermaid
erDiagram
    AssemblyOrder ||--|| BillOfMaterials : "specifies"
    BillOfMaterials ||--o{ Component : "lists"
    Component ||--o{ Subsystem : "integrates-into"
    AssemblyOrder ||--|| WorkInstruction : "governed-by"
    Subsystem ||--|| TestResult : "validated-by"
    WorkInstruction ||--o{ Component : "sequences"
```
...
stateDiagram-v2
    ...
    Assembling --> Installing : instructionsFollowed
    Installing --> Wiring : componentInstalled
    Wiring --> Testing : connectionsWired
    Testing --> Inspecting : subsystemTested
    Inspecting --> Packaging : assemblyInspected
    Testing --> Assembling : testFailed
    Packaging --> [*] : systemPackaged
```
