...
```mermaid
erDiagram
    Blueprint ||--|{ Component : "specifies"
    WorkOrder ||--o{ Batch : "produces"
    Batch ||--o{ Component : "contains"
    Component ||--|| Tolerance : "measured-against"
    Device ||--o{ Component : "assembled-from"
    WorkOrder ||--|| Blueprint : "references"
```
...
```mermaid
stateDiagram-v2
    [*] --> Ordered : workOrderCreated
    Ordered --> MaterialReady : materialsPrepared
    MaterialReady --> ToolingReady : toolingSetup
    ToolingReady --> Fabricated : componentFabricated
    Fabricated --> Inspected : dimensionsInspected
    Inspected --> Assembled : deviceAssembled
    Assembled --> Certified : complianceCertified
    Inspected --> ToolingReady : dimensionsInspected
    Certified --> [*]
```
## Workflow
