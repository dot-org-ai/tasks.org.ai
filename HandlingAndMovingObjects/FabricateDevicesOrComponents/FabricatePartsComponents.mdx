...
```mermaid
erDiagram
    Specification ||--|{ Part : "defines"
    Material ||--o{ Part : "shaped-into"
    ToolPath ||--o{ Part : "machines"
    Part ||--|| InspectionReport : "verified-by"
    BatchRun ||--o{ Part : "contains"
    Part }o--|| Specification : "conforms-to"
```
...
```mermaid
stateDiagram-v2
    [*] --> MaterialChosen : materialSelected
    MaterialChosen --> Programmed : toolPathGenerated
    Programmed --> Allocated : stockAllocated
    Allocated --> Fabricated : partFabricated
    Fabricated --> Measured : dimensionsMeasured
    Measured --> Validated : propertiesValidated
    Validated --> Released : partReleased
    Measured --> Allocated : dimensionsMeasured
    Released --> [*]
```
## Workflow
