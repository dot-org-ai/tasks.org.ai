...
```mermaid
erDiagram
    TreatmentArea ||--|| CleaningProtocol : "disinfected-per"
    TreatmentArea ||--|| EquipmentSetup : "configured-with"
    TreatmentArea ||--|| EnvironmentalControl : "regulated-by"
    ReadinessChecklist ||--|| TreatmentArea : "validates"
    TreatmentArea ||--o{ TurnoverRecord : "documented-in"
```
...
stateDiagram-v2
    [*] --> AreaCleaned
    AreaCleaned --> EquipmentConfigured : equipmentConfigured
    EquipmentConfigured --> SuppliesStaged : suppliesStaged
    SuppliesStaged --> EnvironmentControlled : environmentControlled
    EnvironmentControlled --> ReadinessVerified : readinessVerified
    ReadinessVerified --> EquipmentConfigured : equipmentConfigured
    ReadinessVerified --> AreaLocked : areaLocked
    AreaLocked --> TurnoverDocumented : turnoverDocumented
    TurnoverDocumented --> [*]
```
