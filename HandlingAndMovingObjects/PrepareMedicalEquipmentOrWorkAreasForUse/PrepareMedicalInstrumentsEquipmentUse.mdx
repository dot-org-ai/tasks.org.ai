...
```mermaid
erDiagram
    Instrument ||--|| SterilizationCycle : "sterilized-in"
    SterilizationCycle ||--|| BiologicalIndicator : "validated-by"
    Instrument ||--o{ CalibrationRecord : "calibrated-per"
    ProcedureKit ||--o{ Instrument : "assembles"
    Instrument ||--o{ EquipmentLog : "tracked-in"
```
...
stateDiagram-v2
    ...
    InstrumentsSterilized --> ReadinessVerified : readinessVerified
    ReadinessVerified --> InstrumentsSterilized : instrumentsSterilized
    ReadinessVerified --> InstrumentsPackaged : instrumentsPackaged
    InstrumentsPackaged --> KitAssembled : kitAssembled
    KitAssembled --> EquipmentCalibrated : equipmentCalibrated
    EquipmentCalibrated --> PreparationDocumented : preparationDocumented
    PreparationDocumented --> KitDeployed : kitDeployed
    KitDeployed --> [*]
```
