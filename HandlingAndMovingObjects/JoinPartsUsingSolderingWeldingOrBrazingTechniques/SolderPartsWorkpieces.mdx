...
```mermaid
erDiagram
    SolderJoint ||--o{ Solder : "defines"
    SolderJoint ||--|{ Flux : "generates"
    SolderJoint }o--|| Workpiece : "uses"
    SolderJoint ||--|| SolderingProfile : "tracked-by"
    SolderJoint ||--o{ InspectionRecord : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : surfacesCleaned
    InProgress --> Verified : fluxApplied
    Verified --> Approved : solderSelected
    Approved --> Active : solderApplied
    Active --> Complete : excessRemoved
