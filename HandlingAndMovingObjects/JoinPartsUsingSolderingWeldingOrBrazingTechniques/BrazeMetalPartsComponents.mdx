...
```mermaid
erDiagram
    BrazingProcedure ||--o{ BrazedJoint : "governs"
    BrazedJoint ||--|| JointClearance : "gapped-to"
    BrazedJoint ||--|| FillerMetal : "bonded-with"
    Flux ||--|| BrazedJoint : "protects"
    BrazedJoint ||--|| TestReport : "inspected-in"
```
...
```mermaid
stateDiagram-v2
    [*] --> Prepared : surfacesPrepared
    Prepared --> FillerChosen : fillerMetalSelected
    FillerChosen --> Fluxed : fluxApplied
    Fluxed --> Heated : jointHeated
    Heated --> Flowed : fillerFlowed
    Flowed --> Cooled : jointCooled
    Cooled --> Inspected : jointInspected
    Inspected --> Prepared : jointInspected
    Inspected --> [*]
```
## Workflow
