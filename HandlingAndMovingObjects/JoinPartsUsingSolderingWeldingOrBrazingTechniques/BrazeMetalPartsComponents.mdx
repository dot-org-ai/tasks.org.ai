...
```mermaid
erDiagram
    BrazedJoint ||--o{ FillerMetal : "defines"
    BrazedJoint ||--|{ Flux : "generates"
    BrazedJoint }o--|| JointClearance : "uses"
    BrazedJoint ||--|| BrazingProcedure : "tracked-by"
    BrazedJoint ||--o{ TestReport : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : surfacesPrepared
    InProgress --> Verified : fluxApplied
    Verified --> Approved : fillerMetalSelected
    Approved --> Active : jointHeated
    Active --> Complete : fillerFlowed
