...
```mermaid
erDiagram
    WeldingProcedure ||--|| WeldingMachine : "executed-on"
    WeldingMachine ||--|| ShieldingGas : "shielded-by"
    WeldingMachine ||--|| Electrode : "fed-with"
    OperatorQualification ||--o{ WeldingProcedure : "certifies"
    WeldingMachine ||--o{ MaintenanceLog : "serviced-per"
```
...
stateDiagram-v2
    ...
    Selected --> Configured : parametersConfigured
    Configured --> Loaded : consumablesInstalled
    Loaded --> Checked : preWeldCheckCompleted
    Checked --> Welding : weldExecuted
    Welding --> Monitored : arcMonitored
    Monitored --> Welding : weldExecuted
    Monitored --> ShutDown : equipmentShutdown
    ShutDown --> [*]
```
## Workflow
