...
```mermaid
erDiagram
    WeldingMachine ||--o{ WeldingProcedure : "defines"
    WeldingMachine ||--|{ ShieldingGas : "generates"
    WeldingMachine }o--|| Electrode : "uses"
    WeldingMachine ||--|| OperatorQualification : "tracked-by"
    WeldingMachine ||--o{ MaintenanceLog : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : equipmentSelected
    InProgress --> Verified : parametersConfigured
    Verified --> Approved : consumablesInstalled
    Approved --> Active : preWeldCheckCompleted
    Active --> Complete : weldExecuted
