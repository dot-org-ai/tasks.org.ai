...
```mermaid
erDiagram
    MetalPart ||--|{ Burr : "exhibits"
    MetalPart ||--|| DeburringTool : "smoothed-by"
    TumblingBatch ||--|{ MetalPart : "finishes"
    EdgeSpec ||--|| MetalPart : "constrains"
    MetalPart ||--|| FinishReport : "documented-by"
```
...
```mermaid
stateDiagram-v2
    [*] --> Inspected : edgesInspected
    Inspected --> MethodChosen : methodSelected
    MethodChosen --> Deburred : partDeburred
    MethodChosen --> Tumbled : batchTumbled
    Deburred --> Verified : edgesVerified
    Tumbled --> Verified : edgesVerified
    Verified --> Logged : finishingLogged
    Verified --> Deburred : edgesNotMet
    Logged --> [*]
```
