...
```mermaid
erDiagram
    AbrasiveProduct ||--|| Workpiece : "smooths"
    GritSequence ||--|{ AbrasiveProduct : "orders"
    Workpiece ||--|| SurfacePrep : "prepared-by"
    AbrasiveProduct ||--|| DustCollection : "captured-by"
    Workpiece ||--o{ RoughnessReading : "measured-by"
    FinishSpec ||--|| Workpiece : "targets"
```
...
```mermaid
stateDiagram-v2
    [*] --> AbrasiveChosen : abrasiveSelected
    AbrasiveChosen --> Prepped : surfacePrepared
    Prepped --> Abraded : abrasiveApplied
    Abraded --> FinerGrit : gritSteppedDown
    FinerGrit --> Abraded : nextPass
    Abraded --> Measured : finishMeasured
    Measured --> Cleaned : surfaceCleaned
    Measured --> Abraded : roughnessExceeded
    Cleaned --> [*]
```
