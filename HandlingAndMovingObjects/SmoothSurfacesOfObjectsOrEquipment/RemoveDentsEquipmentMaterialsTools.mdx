...
```mermaid
erDiagram
    RepairOrder ||--|| DamageReport : "initiates-from"
    DamageReport ||--|| Panel : "locates"
    Panel ||--|| RepairTool : "restored-with"
    Panel ||--|| SurfaceProfile : "measured-by"
    DamageReport ||--o{ BeforeAfterPhoto : "documents"
```
...
```mermaid
stateDiagram-v2
    [*] --> Assessed : damageAssessed
    Assessed --> MethodChosen : methodSelected
    MethodChosen --> Repaired : dentRemoved
    Repaired --> Verified : surfaceVerified
    Verified --> Refinished : surfaceRefinished
    Refinished --> Documented : repairDocumented
    Verified --> Repaired : contourInsufficient
    Documented --> [*]
```
