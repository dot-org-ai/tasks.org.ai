...
```mermaid
erDiagram
    Workpiece ||--|| PolishingCompound : "buffed-with"
    PolishingCompound ||--|| BuffingWheel : "applied-via"
    LusterSpec ||--|| Workpiece : "targets"
    PolishingSequence ||--|{ PolishingCompound : "orders"
    Workpiece ||--o{ FinishRecord : "documented-by"
```
...
```mermaid
stateDiagram-v2
    [*] --> Prepared : workpiecePrepared
    Prepared --> CompoundChosen : compoundSelected
    CompoundChosen --> Polished : polishApplied
    Polished --> Progressed : compoundProgressed
    Progressed --> Polished : nextPass
    Polished --> Measured : lusterMeasured
    Measured --> Inspected : finalInspected
    Measured --> Polished : insufficientLuster
    Inspected --> [*]
```
