...
```mermaid
erDiagram
    MarkingJob ||--|| Material : "targets"
    MarkingPattern ||--o{ GuideLine : "defines"
    Material ||--o{ GuideLine : "marked-with"
    GuideLine ||--|| MarkingSpec : "constrained-by"
    MarkingJob ||--o{ Verification : "validated-by"
```
...
stateDiagram-v2
    [*] --> MarkingJobCreated
    MarkingJobCreated --> PatternLoaded : patternLoaded
    PatternLoaded --> GuideLinesDrawn : guideLinesDrawn
    GuideLinesDrawn --> MarkingsVerified : markingsVerified
    MarkingsVerified --> PatternAdjusted : markingErrorDetected
    PatternAdjusted --> GuideLinesDrawn : guideLinesDrawn
    MarkingsVerified --> MarkingsApproved : markingsApproved
    MarkingsApproved --> [*]
```
## Workflow
