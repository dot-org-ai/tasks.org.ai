...
```mermaid
erDiagram
    StructuralComponent ||--o{ PositioningJob : "defines"
    StructuralComponent ||--|{ PlacementSpec : "generates"
    StructuralComponent }o--|| TemporarySupport : "uses"
    StructuralComponent ||--|| AlignmentCheck : "tracked-by"
    StructuralComponent ||--o{ Assembly : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : positioningJobCreated
    InProgress --> Verified : supportsPrepared
    Verified --> Approved : componentLifted
    Approved --> Active : componentAligned
    Active --> Complete : positionVerified
