...
```mermaid
erDiagram
    MasonryUnit ||--o{ CourseLine : "defines"
    MasonryUnit ||--|{ JointProfile : "generates"
    MasonryUnit }o--|| AlignmentCheck : "uses"
    MasonryUnit ||--|| WallSection : "tracked-by"
    MasonryUnit ||--o{ BondPattern : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : courseLineSet
    InProgress --> Verified : unitPositioned
    Verified --> Approved : alignmentChecked
    Approved --> Active : unitAdjusted
    Active --> Complete : bondPatternVerified
