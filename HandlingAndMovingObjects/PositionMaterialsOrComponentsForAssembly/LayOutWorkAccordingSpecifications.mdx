...
```mermaid
erDiagram
    Specification ||--|| LayoutPlan : "documented-in"
    LayoutPlan ||--o{ ReferencePoint : "establishes"
    LayoutJob ||--|| Specification : "interprets"
    LayoutJob ||--o{ Component : "positions"
    LayoutJob ||--o{ Verification : "validated-by"
```
...
stateDiagram-v2
    [*] --> LayoutJobCreated
    LayoutJobCreated --> SpecsInterpreted : specsInterpreted
    SpecsInterpreted --> DatumsEstablished : datumsEstablished
    DatumsEstablished --> ComponentsPositioned : componentsPositioned
    ComponentsPositioned --> LayoutVerified : layoutVerified
    LayoutVerified --> ComponentsPositioned : deviationDetected
    LayoutVerified --> LayoutApproved : layoutApproved
    LayoutApproved --> [*]
```
## Workflow
