...
```mermaid
erDiagram
    Product ||--o{ StoragePackage : "defines"
    Product ||--|{ ShipmentPackage : "generates"
    Product }o--|| StorageLocation : "uses"
    Product ||--|| InventoryRecord : "tracked-by"
    Product ||--o{ ShipmentOrder : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : productPackaged
    InProgress --> Verified : locationAssigned
    Verified --> Approved : inventoryRecorded
    Approved --> Active : packageRetrieved
    Active --> Complete : shipmentPrepared
