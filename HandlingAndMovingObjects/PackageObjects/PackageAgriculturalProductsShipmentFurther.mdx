...
```mermaid
erDiagram
    Product ||--|| Grade : "classified-by"
    Product ||--o{ Container : "packed-into"
    Container ||--|| Label : "identified-by"
    Batch ||--o{ Container : "groups"
    Batch ||--|| Shipment : "dispatched-as"
```
...
stateDiagram-v2
    [*] --> ProductGraded
    ProductGraded --> ContainerSelected : containerSelected
    ContainerSelected --> PackageFilled : packageFilled
    PackageFilled --> LabelApplied : labelApplied
    LabelApplied --> PackageInspected : packageInspected
    PackageInspected --> ContainerSelected : containerSelected
    PackageInspected --> ShipmentPalletized : shipmentPalletized
    ShipmentPalletized --> BatchDocumented : batchDocumented
    BatchDocumented --> [*]
```
## Workflow
