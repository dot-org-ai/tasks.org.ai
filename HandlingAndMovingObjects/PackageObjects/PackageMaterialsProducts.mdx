...
```mermaid
erDiagram
    PackagingOrder ||--o{ Product : "requests"
    Product ||--|| Container : "packed-into"
    Container ||--|| Cushioning : "protected-by"
    Container ||--|| Label : "identified-by"
    Material ||--o{ Container : "stored-in"
```
...
stateDiagram-v2
    [*] --> OrderCreated
    OrderCreated --> PackagingSelected : packagingSelected
    PackagingSelected --> ItemsPrepared : itemsPrepared
    ItemsPrepared --> CushioningInserted : cushioningInserted
    CushioningInserted --> ItemsPlaced : itemsPlaced
    ItemsPlaced --> LabelingApplied : labelingApplied
    LabelingApplied --> PackageSealed : packageSealed
    PackageSealed --> ItemsPrepared : itemsPrepared
    PackageSealed --> [*]
```
## Workflow
