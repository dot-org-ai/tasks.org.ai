...
```mermaid
erDiagram
    Shipment ||--|| TransportMode : "shipped-via"
    Shipment ||--|| ProtectivePackaging : "protected-by"
    Shipment ||--|| Pallet : "loaded-onto"
    Shipment ||--|| BillOfLading : "documented-in"
    Shipment ||--o{ HazmatDeclaration : "classified-by"
```
...
```mermaid
stateDiagram-v2
    [*] --> TransportNeedsAssessed
    TransportNeedsAssessed --> ProtectionSelected : protectionSelected
    ProtectionSelected --> ShockAbsorptionApplied : shockAbsorptionApplied
    ShockAbsorptionApplied --> LoadSecured : loadSecured
    LoadSecured --> DocumentationGenerated : documentationGenerated
    DocumentationGenerated --> ComplianceInspected : complianceInspected
    ComplianceInspected --> ProtectionSelected : protectionSelected
    ComplianceInspected --> ShipmentReleased : shipmentReleased
    ShipmentReleased --> [*]
```
## Workflow
