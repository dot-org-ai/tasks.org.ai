...
```mermaid
erDiagram
    RepairOrder ||--|| GarmentItem : "repairs"
    RepairOrder ||--|| DamageAssessment : "starts-from"
    RepairOrder ||--|| RepairTechnique : "applies"
    MaterialSpec ||--|| RepairTechnique : "specifies"
    RepairOrder ||--|| QualityReport : "concludes-with"
```
...
```mermaid
stateDiagram-v2
    [*] --> Received : damageAssessed
    Received --> TechniqueChosen : techniqueSelected
    TechniqueChosen --> Sourcing : materialsSourced
    Sourcing --> Repairing : repairCompleted
    Repairing --> Inspected : repairInspected
    Inspected --> Repairing : reworkNeeded
    Inspected --> Estimated : costEstimated
    Estimated --> Returned : itemReturned
    Returned --> [*]
```
