...
```mermaid
erDiagram
    SewingMachine ||--|{ ProductionRun : "executes"
    SewingMachine ||--|| StitchProfile : "applies"
    SewingMachine ||--|| ThreadSetup : "loaded-with"
    SewingMachine ||--o{ MaintenanceLog : "accumulates"
    ProductionRun ||--o{ DefectRecord : "reports"
```
...
```mermaid
stateDiagram-v2
    [*] --> Idle : machineSetupComplete
    Idle --> Configured : stitchConfigured
    Configured --> Running : productionStarted
    Running --> Paused : productionPaused
    Paused --> Running : productionStarted
    Running --> Servicing : maintenancePerformed
    Servicing --> Idle : machineSetupComplete
    Running --> Complete : productionCompleted
    Running --> Defective : defectDetected
    Defective --> Paused : productionPaused
    Complete --> [*]
```
