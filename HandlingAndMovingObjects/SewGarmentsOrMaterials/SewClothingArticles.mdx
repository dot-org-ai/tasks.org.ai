...
```mermaid
erDiagram
    GarmentOrder ||--|| Pattern : "follows"
    Pattern ||--|{ FabricPiece : "produces"
    Garment ||--|{ Seam : "joins-with"
    GarmentOrder ||--|| Garment : "assembles"
    FinishingSpec ||--|| Garment : "finishes"
```
...
stateDiagram-v2
    [*] --> CutReady : fabricCut
    CutReady --> Assembled : piecesAssembled
    Assembled --> Stitched : seamStitched
    Stitched --> Closed : closureAttached
    Closed --> Hemmed : hemFinished
    Hemmed --> Pressed : garmentPressed
    Pressed --> Inspected : garmentInspected
    Inspected --> Stitched : reworkRequired
    Inspected --> [*]
```
