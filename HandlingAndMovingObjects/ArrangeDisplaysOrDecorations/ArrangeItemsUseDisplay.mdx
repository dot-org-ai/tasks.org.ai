...
```mermaid
erDiagram
    LayoutPlan ||--o{ ShelfSection : "organizes"
    ShelfSection ||--o{ ItemLabel : "displays"
    RestockingSchedule ||--|| ShelfSection : "replenishes"
    ArrangementChecklist ||--|| LayoutPlan : "verifies"
    ServiceStation ||--|| LayoutPlan : "equipped according to"
    ItemLabel ||--|| ArrangementChecklist : "validated by"
```
...
stateDiagram-v2
    [*] --> LayoutPlanCreated
    LayoutPlanCreated --> ItemsPositioned : itemsPositioned
    ItemsPositioned --> LabelsApplied : labelsApplied
    LabelsApplied --> ArrangementVerified : arrangementVerified
    ArrangementVerified --> SpacingAdjusted : spacingAdjusted
    SpacingAdjusted --> ArrangementVerified : arrangementVerified
    ArrangementVerified --> ItemsRestocked : itemsRestocked
    ItemsRestocked --> ArrangementCleared : arrangementCleared
    ArrangementCleared --> [*]
```
