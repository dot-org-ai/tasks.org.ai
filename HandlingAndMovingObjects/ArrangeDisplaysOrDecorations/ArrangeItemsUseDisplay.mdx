...
```mermaid
erDiagram
    Supplier ||--|| FacilityManager : follows
    Supplier }o--|| TradeShowOrganizer : generates
    Supplier ||--|{ CateringClient : requires
    Supplier ||--o{ MaintenanceTeam : tracks
    FacilityManager ||--|{ Supplier : creates
    TradeShowOrganizer ||--o{ Supplier : documents
    CateringClient ||--|| Supplier : supports
```
...
stateDiagram-v2
    [*] --> Supplier
    Supplier --> Facility_Manager: FacilityManager
    Facility_Manager --> Trade_Show_Organizer: TradeShowOrganizer
    Trade_Show_Organizer --> Catering_Client: CateringClient
    Catering_Client --> Maintenance_Team: MaintenanceTeam
    Trade_Show_Organizer --> Facility_Manager: TradeShowOrganizer
    Maintenance_Team --> [*]
```
