...
```mermaid
erDiagram
    Planogram ||--o{ DisplayFixture : "positions"
    Planogram ||--|| ProductAssortment : "allocates"
    DisplayFixture ||--o{ Signage : "displays"
    PromotionalCalendar ||--o{ Planogram : "schedules"
    ProductAssortment ||--|| DisplayPerformanceReport : "measured-by"
    DisplayFixture ||--|| DisplayPerformanceReport : "evaluated-in"
```
...
stateDiagram-v2
    [*] --> Designing
    Designing --> Assembling : displayLayoutDesigned
    Assembling --> Stocking : fixturesAssembled
    Stocking --> Signing : displayStocked
    Signing --> Lighting : signageInstalled
    Lighting --> Auditing : displayLit
    Auditing --> Live : complianceAudited
    Auditing --> Stocking : nonCompliant
    Live --> Rotating : displayRotated
    Rotating --> [*]
```
