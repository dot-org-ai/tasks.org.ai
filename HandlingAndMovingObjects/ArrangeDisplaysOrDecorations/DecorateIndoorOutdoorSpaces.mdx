...
```mermaid
erDiagram
    EventClient ||--|| FloralWholesaler : follows
    EventClient }o--|| RentalCompany : generates
    EventClient ||--|{ VenueManager : requires
    EventClient ||--o{ LightingVendor : tracks
    FloralWholesaler ||--|{ EventClient : creates
    RentalCompany ||--o{ EventClient : documents
    VenueManager ||--|| EventClient : supports
```
...
stateDiagram-v2
    [*] --> Event_Client
    Event_Client --> Floral_Wholesaler: FloralWholesaler
    Floral_Wholesaler --> Rental_Company: RentalCompany
    Rental_Company --> Venue_Manager: VenueManager
    Venue_Manager --> Lighting_Vendor: LightingVendor
    Rental_Company --> Floral_Wholesaler: RentalCompany
    Lighting_Vendor --> [*]
```
