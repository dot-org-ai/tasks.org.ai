...
```mermaid
erDiagram
    DesignConcept ||--o{ DecorationInventory : "specifies"
    DesignConcept ||--|| InstallationPlan : "drives"
    InstallationPlan ||--o{ FlooringLayout : "includes"
    InstallationPlan ||--|| LightingScheme : "references"
    LightingScheme }o--|| DecorationInventory : "draws-from"
    TeardownChecklist ||--o{ DecorationInventory : "returns-to"
```
...
stateDiagram-v2
    [*] --> Conceptualizing
    Conceptualizing --> Sourcing : designConceptApproved
    Sourcing --> Installing : materialsSourced
    Installing --> Lighting : decorationsInstalled
    Lighting --> Styling : lightingArranged
    Styling --> Reviewing : tablescapesStyled
    Reviewing --> TearingDown : walkthroughCompleted
    Reviewing --> Installing : adjustmentsRequired
    TearingDown --> [*] : decorationsTornDown
```
