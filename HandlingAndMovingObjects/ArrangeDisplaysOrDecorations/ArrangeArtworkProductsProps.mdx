...
```mermaid
erDiagram
    GalleryDirector ||--|| ProductionDesigner : follows
    GalleryDirector }o--|| Photographer : generates
    GalleryDirector ||--|{ ArtistLender : requires
    GalleryDirector ||--o{ PropHouse : tracks
    ProductionDesigner ||--|{ GalleryDirector : creates
    Photographer ||--o{ GalleryDirector : documents
    ArtistLender ||--|| GalleryDirector : supports
```
...
stateDiagram-v2
    [*] --> Gallery_Director
    Gallery_Director --> Production_Designer: ProductionDesigner
    Production_Designer --> Photographer: Photographer
    Photographer --> Artist_Lender: ArtistLender
    Artist_Lender --> Prop_House: PropHouse
    Photographer --> Production_Designer: Photographer
    Prop_House --> [*]
```
