...
erDiagram
    Carcass ||--|| Hide : "yields"
    Carcass ||--|{ Offal : "produces"
    Carcass ||--o{ TrimRecord : "documents"
    InspectionResult ||--|| Carcass : "evaluates"
    ProductionBatch ||--|{ Carcass : "groups"
```
...
```mermaid
stateDiagram-v2
    [*] --> Positioned : carcassPrepared
    Positioned --> Skinned : hideRemoved
    Skinned --> Eviscerated : organsExtracted
    Eviscerated --> Trimmed : extremitiesTrimmed
    Trimmed --> Inspected : carcassInspected
    Inspected --> Sorted : byproductsSorted
    Sorted --> Documented : dispositionRecorded
    Inspected --> Trimmed : reTrimRequired
    Documented --> [*]
```
