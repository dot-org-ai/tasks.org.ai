...
```mermaid
erDiagram
    Carcass ||--|{ CarcassHalf : "splits-into"
    Carcass ||--|| GradeReport : "receives"
    CarcassHalf ||--o{ ChillRecord : "produces"
    Carcass ||--|| YieldReport : "summarizes"
    Carcass ||--o{ HACCPLog : "documents"
```
...
```mermaid
stateDiagram-v2
    [*] --> Dressed : carcassSplit
    Dressed --> Washed : carcassWashed
    Washed --> Chilling : carcassChilled
    Chilling --> Grading : carcassGraded
    Grading --> Weighed : weightsRecorded
    Weighed --> Labeled : carcassLabeled
    Labeled --> InStorage : carcassRouted
    InStorage --> Chilling : reChillRequired
    InStorage --> [*]
```
