...
```mermaid
erDiagram
    TrafficControlPlan ||--o{ WorkZone : "defines"
    TrafficControlPlan ||--|{ SignalDevice : "generates"
    TrafficControlPlan }o--|| TrafficIncident : "uses"
    TrafficControlPlan ||--|| PermitAuthorization : "tracked-by"
    TrafficControlPlan ||--o{ ShiftAssignment : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : planDeveloped
    InProgress --> Verified : permitObtained
    Verified --> Approved : signageDeployed
    Approved --> Active : flowDirected
    Active --> Complete : conditionsMonitored
