...
```mermaid
erDiagram
    EventOrder ||--o{ TaskAssignment : "generates"
    TaskAssignment ||--|| Station : "performed-at"
    TaskAssignment ||--|| PrepChecklist : "tracked-on"
    IngredientRequest ||--o{ TaskAssignment : "triggers"
    TaskAssignment ||--o{ CleaningLog : "produces"
```
...
stateDiagram-v2
    [*] --> IngredientsGathered
    IngredientsGathered --> ComponentsMeasured : componentsMeasured
    ComponentsMeasured --> BasicPrepCompleted : basicPrepCompleted
    BasicPrepCompleted --> StationStocked : stationStocked
    StationStocked --> ItemsTransported : itemsTransported
    ItemsTransported --> WorkSurfaceCleaned : workSurfaceCleaned
    WorkSurfaceCleaned --> IngredientsGathered : ingredientsGathered
    WorkSurfaceCleaned --> ChecklistCompleted : checklistCompleted
    ChecklistCompleted --> [*]
```
