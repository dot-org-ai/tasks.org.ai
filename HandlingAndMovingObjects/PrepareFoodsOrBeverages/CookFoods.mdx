...
```mermaid
erDiagram
    CookingOrder ||--|| Recipe : "follows"
    CookingOrder ||--|| CookingStation : "prepared-at"
    CookingOrder ||--o{ TemperatureLog : "monitored-by"
    CookingBatch ||--o{ CookingOrder : "groups"
    DonenessSpec ||--|| Recipe : "defines-target-for"
```
...
stateDiagram-v2
    [*] --> EquipmentPreheated
    EquipmentPreheated --> FoodSauteed : foodSauteed
    EquipmentPreheated --> FoodGrilled : foodGrilled
    EquipmentPreheated --> FoodRoasted : foodRoasted
    FoodSauteed --> DonenessConfirmed : donenessConfirmed
    FoodGrilled --> DonenessConfirmed : donenessConfirmed
    FoodRoasted --> DonenessConfirmed : donenessConfirmed
    DonenessConfirmed --> EquipmentPreheated : equipmentPreheated
    DonenessConfirmed --> [*]
```
