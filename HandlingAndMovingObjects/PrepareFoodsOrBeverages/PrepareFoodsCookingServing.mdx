...
```mermaid
erDiagram
    PrepList ||--o{ PrepTask : "contains"
    PrepTask ||--|| Ingredient : "processes"
    Recipe ||--o{ PrepTask : "requires"
    PrepTask ||--|| PrepBatch : "produces"
    PrepBatch ||--|| StorageContainer : "stored-in"
```
...
stateDiagram-v2
    [*] --> PrepListCreated
    PrepListCreated --> IngredientWashed : ingredientWashed
    IngredientWashed --> IngredientChopped : ingredientChopped
    IngredientChopped --> IngredientMarinated : ingredientMarinated
    IngredientMarinated --> IngredientPortioned : ingredientPortioned
    IngredientPortioned --> IngredientWashed : prepDelayed
    IngredientPortioned --> PrepCompleted : prepCompleted
    PrepCompleted --> [*]
```
