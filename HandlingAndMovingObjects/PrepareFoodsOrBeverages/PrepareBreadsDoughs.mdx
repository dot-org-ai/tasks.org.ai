...
```mermaid
erDiagram
    ProductionOrder ||--o{ Recipe : "schedules"
    Recipe ||--o{ Ingredient : "requires"
    Recipe ||--|| DoughBatch : "produces"
    DoughBatch ||--|| ProofingRecord : "fermented-per"
    DoughBatch ||--|| BakeLog : "baked-per"
```
...
stateDiagram-v2
    [*] --> IngredientsScaled
    IngredientsScaled --> DoughMixed : doughMixed
    DoughMixed --> DoughFermented : doughFermented
    DoughFermented --> DoughShaped : doughShaped
    DoughShaped --> DoughProofed : doughProofed
    DoughProofed --> BreadBaked : breadBaked
    BreadBaked --> IngredientsScaled : ingredientsScaled
    DoughMixed --> IngredientsScaled : batchFailed
    BreadBaked --> [*]
```
