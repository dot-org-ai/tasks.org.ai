...
```mermaid
erDiagram
    ServiceOrder ||--o| Wig : "services"
    ServiceOrder ||--o| Hairpiece : "services"
    Wig ||--|| CleaningProtocol : "cleaned-per"
    Wig ||--|| StyleSpecification : "styled-to"
    ServiceOrder ||--|| MaintenanceRecord : "documented-in"
    Hairpiece ||--|| CleaningProtocol : "cleaned-per"
```
...
stateDiagram-v2
    ...
    Assessed --> Cleaned : hairpieceCleaned
    Cleaned --> Detangled : hairDetangled
    Detangled --> Styled : hairpieceStyled
    Styled --> Inspected : resultInspected
    Inspected --> Returned : wigReturned
    Inspected --> Styled : resultInspected
    Returned --> [*]
```
## Workflow
