...
```mermaid
erDiagram
    PropertyOwner ||--|| RoofingSupplier : follows
    PropertyOwner }o--|| Architect : generates
    PropertyOwner ||--|{ BuildingInspector : requires
    PropertyOwner ||--o{ ManufacturerRep : tracks
    PropertyOwner ||--|| InsuranceAdjuster : validates
    RoofingSupplier ||--|{ PropertyOwner : creates
    Architect ||--o{ PropertyOwner : documents
    BuildingInspector ||--|| PropertyOwner : supports
```
...
stateDiagram-v2
    [*] --> Property_Owner
    Property_Owner --> Roofing_Supplier: RoofingSupplier
    Roofing_Supplier --> Architect: Architect
    Architect --> Building_Inspector: BuildingInspector
    Building_Inspector --> Manufacturer_Rep: ManufacturerRep
    Manufacturer_Rep --> Insurance_Adjuster: InsuranceAdjuster
    Building_Inspector --> Roofing_Supplier: Architect
    Insurance_Adjuster --> [*]
```
