...
```mermaid
erDiagram
    SteelFabricator ||--|| GeneralContractor : follows
    SteelFabricator }o--|| CraneOperator : generates
    SteelFabricator ||--|{ WeldingInspector : requires
    SteelFabricator ||--o{ StructuralEngineer : tracks
    SteelFabricator ||--|| SafetyOfficer : validates
    GeneralContractor ||--|{ SteelFabricator : creates
    CraneOperator ||--o{ SteelFabricator : documents
    WeldingInspector ||--|| SteelFabricator : supports
```
...
stateDiagram-v2
    [*] --> Steel_Fabricator
    Steel_Fabricator --> General_Contractor: GeneralContractor
    General_Contractor --> Crane_Operator: CraneOperator
    Crane_Operator --> Welding_Inspector: WeldingInspector
    Welding_Inspector --> Structural_Engineer: StructuralEngineer
    Structural_Engineer --> Safety_Officer: SafetyOfficer
    Welding_Inspector --> General_Contractor: CraneOperator
    Safety_Officer --> [*]
```
