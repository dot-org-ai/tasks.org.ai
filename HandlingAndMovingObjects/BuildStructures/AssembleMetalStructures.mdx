...
```mermaid
erDiagram
    Structure ||--o{ Member : "composed-of"
    Member ||--o{ Connection : "joined-by"
    Structure ||--|| ErectionDrawing : "assembled-per"
    Structure ||--|| LiftPlan : "hoisted-per"
    Connection ||--|{ InspectionReport : "verified-in"
    Member ||--|| LiftPlan : "rigged-per"
```
...
stateDiagram-v2
    [*] --> Unloading
    Unloading --> Rigging : steelUnloaded
    Rigging --> Erecting : memberRigged
    Erecting --> Aligning : memberErected
    Aligning --> Connecting : structureAligned
    Connecting --> Inspecting : membersConnected
    Inspecting --> Released : connectionsInspected
    Inspecting --> Connecting : repairRequired
    Released --> [*] : structureReleased
```
