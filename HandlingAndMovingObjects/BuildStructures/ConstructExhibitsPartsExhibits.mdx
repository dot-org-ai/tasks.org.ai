...
```mermaid
erDiagram
    Exhibit ||--o{ DisplayCase : "contains"
    Exhibit ||--o{ Panel : "displays"
    Exhibit ||--o{ Interactive : "features"
    Exhibit ||--|| ShopDrawing : "fabricated-per"
    Exhibit ||--|| InstallationPlan : "installed-per"
    Panel ||--|| ShopDrawing : "specified-in"
```
...
stateDiagram-v2
    [*] --> Interpreting
    Interpreting --> Fabricating : designInterpreted
    Fabricating --> GraphicsMount : structureFabricated
    GraphicsMount --> InteractiveAssembly : graphicsInstalled
    InteractiveAssembly --> Crating : interactivesAssembled
    Crating --> Installing : exhibitCrated
    Installing --> Operational : exhibitInstalled
    Installing --> Fabricating : modificationNeeded
    Operational --> [*] : exhibitDismantled
```
