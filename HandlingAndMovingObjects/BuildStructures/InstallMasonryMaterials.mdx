...
```mermaid
erDiagram
    MasonryPlan ||--|| MaterialOrder : "requires"
    MasonryPlan ||--|| FoundationPrep : "built-on"
    WorkArea ||--|| MaterialOrder : "stages"
    MaterialOrder ||--|| CourseProgress : "consumed-by"
    CourseProgress ||--|| InspectionReport : "verified-in"
    MasonryPlan ||--|| InspectionReport : "compliant-per"
```
...
stateDiagram-v2
    [*] --> Specifying
    Specifying --> Ordering : masonrySpecified
    Ordering --> FoundationPrep : materialsOrdered
    FoundationPrep --> Mixing : foundationPrepared
    Mixing --> Laying : mortarMixed
    Laying --> JointFinish : unitsLaid
    JointFinish --> Inspecting : jointsFinished
    Inspecting --> Complete : workInspected
    Inspecting --> Laying : reworkRequired
    Complete --> [*]
```
