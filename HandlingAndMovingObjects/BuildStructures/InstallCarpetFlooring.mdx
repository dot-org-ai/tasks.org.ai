...
```mermaid
erDiagram
    FlooringSupplier ||--|| GeneralContractor : follows
    FlooringSupplier }o--|| SubfloorContractor : generates
    FlooringSupplier ||--|{ PropertyOwner : requires
    FlooringSupplier ||--o{ BuildingInspector : tracks
    FlooringSupplier ||--|| FurnitureInstaller : validates
    GeneralContractor ||--|{ FlooringSupplier : creates
    SubfloorContractor ||--o{ FlooringSupplier : documents
    PropertyOwner ||--|| FlooringSupplier : supports
```
...
stateDiagram-v2
    [*] --> Flooring_Supplier
    Flooring_Supplier --> General_Contractor: GeneralContractor
    General_Contractor --> Subfloor_Contractor: SubfloorContractor
    Subfloor_Contractor --> Property_Owner: PropertyOwner
    Property_Owner --> Building_Inspector: BuildingInspector
    Building_Inspector --> Furniture_Installer: FurnitureInstaller
    Property_Owner --> General_Contractor: SubfloorContractor
    Furniture_Installer --> [*]
```
