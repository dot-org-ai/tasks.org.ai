...
```mermaid
erDiagram
    SafetyEquipment ||--|| InstallationPlan : "installed-per"
    SafetyEquipment ||--o{ MountingHardware : "secured-with"
    InstallationPlan ||--|| WorkOrder : "scheduled-in"
    SafetyEquipment ||--|| InspectionReport : "verified-by"
    InspectionReport ||--|| Certification : "certifies"
    Certification ||--|| SafetyEquipment : "approves"
```
...
stateDiagram-v2
    [*] --> Planning
    Planning --> Mounting : installationPlanned
    Mounting --> Securing : equipmentMounted
    Securing --> LoadTesting : fastenersSecured
    LoadTesting --> Inspecting : stabilityTested
    Inspecting --> Certifying : installationInspected
    Certifying --> Scheduled : completionCertified
    Inspecting --> Mounting : deficiencyFound
    Scheduled --> [*] : reinspectionScheduled
```
