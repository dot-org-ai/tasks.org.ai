...
```mermaid
erDiagram
    TrimProfile ||--|{ CutList : "dimensioned-into"
    CutList ||--o{ PanelSheet : "specifies"
    PanelSheet ||--|| Adhesive : "bonded-with"
    PanelSheet ||--o{ Fastener : "secured-by"
    Fastener ||--|| JointCompound : "concealed-with"
    TrimProfile ||--|| JointCompound : "finished-by"
```
...
stateDiagram-v2
    [*] --> Measuring
    Measuring --> Cutting : wallsMeasured
    Cutting --> Gluing : materialsCut
    Gluing --> Paneling : adhesiveApplied
    Paneling --> Trimming : panelsInstalled
    Trimming --> Filling : trimSecured
    Filling --> Inspecting : jointsFilled
    Inspecting --> Trimming : alignmentOff
    Inspecting --> [*] : finishInspected
```
