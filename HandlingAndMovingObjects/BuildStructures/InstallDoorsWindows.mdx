...
```mermaid
erDiagram
    DoorUnit ||--|| RoughOpening : "installed-in"
    WindowUnit ||--|| RoughOpening : "installed-in"
    DoorUnit ||--o{ Hardware : "fitted-with"
    WindowUnit ||--o{ Weatherseal : "sealed-with"
    DoorUnit ||--|| TestReport : "verified-by"
    WindowUnit ||--|| TestReport : "verified-by"
```
...
stateDiagram-v2
    [*] --> Verifying
    Verifying --> Framing : openingVerified
    Framing --> Hanging : frameSet
    Hanging --> HardwareInstall : unitHung
    HardwareInstall --> Sealing : hardwareInstalled
    Sealing --> Testing : perimeterSealed
    Testing --> Adjusting : operationTested
    Adjusting --> Accepted : fitAdjusted
    Testing --> Adjusting : adjustmentNeeded
    Accepted --> [*]
```
