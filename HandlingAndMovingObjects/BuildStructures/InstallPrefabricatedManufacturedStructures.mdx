...
```mermaid
erDiagram
    ModularManufacturer ||--|| SiteOwner : follows
    ModularManufacturer }o--|| TransportCoordinator : generates
    ModularManufacturer ||--|{ CraneOperator : requires
    ModularManufacturer ||--o{ UtilityContractor : tracks
    ModularManufacturer ||--|| BuildingInspector : validates
    SiteOwner ||--|{ ModularManufacturer : creates
    TransportCoordinator ||--o{ ModularManufacturer : documents
    CraneOperator ||--|| ModularManufacturer : supports
```
...
stateDiagram-v2
    ...
    Modular_Manufacturer --> Site_Owner: SiteOwner
    Site_Owner --> Transport_Coordinator: TransportCoordinator
    Transport_Coordinator --> Crane_Operator: CraneOperator
    Crane_Operator --> Utility_Contractor: UtilityContractor
    Utility_Contractor --> Building_Inspector: BuildingInspector
    Crane_Operator --> Site_Owner: TransportCoordinator
    Building_Inspector --> [*]
```
