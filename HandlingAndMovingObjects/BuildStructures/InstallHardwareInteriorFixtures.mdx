...
```mermaid
erDiagram
    HardwareSchedule ||--o{ FixtureSpecification : "lists"
    FixtureSpecification ||--|| InstallationPackage : "grouped-in"
    InstallationPackage ||--|| WorkOrder : "scheduled-by"
    WorkOrder ||--|| PunchList : "generates"
    WorkOrder ||--|| ComplianceReport : "verified-in"
    HardwareSchedule ||--|| ComplianceReport : "compliant-per"
```
...
stateDiagram-v2
    [*] --> Specifying
    Specifying --> Ordering : hardwareSpecified
    Ordering --> Staging : fixturesOrdered
    Staging --> SubstratePrep : hardwareStaged
    SubstratePrep --> DoorInstall : substratePrepared
    DoorInstall --> CabinetInstall : doorHardwareInstalled
    CabinetInstall --> Inspecting : cabinetHardwareInstalled
    Inspecting --> Complete : installationInspected
    Inspecting --> CabinetInstall : adjustmentNeeded
    Complete --> [*]
```
