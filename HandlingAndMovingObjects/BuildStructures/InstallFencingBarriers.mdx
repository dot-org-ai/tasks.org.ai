...
```mermaid
erDiagram
    FenceProject ||--|| PropertySurvey : "surveyed-by"
    FenceProject ||--|| MaterialOrder : "supplied-by"
    FenceProject ||--|| InstallationPermit : "authorized-by"
    FenceProject ||--|| WorkOrder : "scheduled-in"
    WorkOrder ||--|| InspectionReport : "verified-by"
    MaterialOrder ||--|| WorkOrder : "consumed-by"
```
...
stateDiagram-v2
    [*] --> Surveying
    Surveying --> Designing : propertySurveyed
    Designing --> Permitting : fenceDesigned
    Permitting --> Ordering : permitObtained
    Ordering --> SitePrep : materialsOrdered
    SitePrep --> PostInstall : sitePrepared
    PostInstall --> PanelAttach : postsInstalled
    PanelAttach --> Complete : fenceCompleted
    Permitting --> Designing : permitDenied
    Complete --> [*]
```
