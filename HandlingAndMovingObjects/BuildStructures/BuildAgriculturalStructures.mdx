...
```mermaid
erDiagram
    Structure ||--|| Foundation : "built-on"
    Structure ||--|| Frame : "supported-by"
    Frame ||--|| Enclosure : "enclosed-by"
    Structure ||--o{ Utility : "serviced-by"
    Structure ||--|| Permit : "authorized-by"
    Enclosure ||--o{ Utility : "houses"
```
...
stateDiagram-v2
    [*] --> SitePrep
    SitePrep --> FoundationPour : sitePrepared
    FoundationPour --> Framing : foundationPoured
    Framing --> Enclosing : frameErected
    Enclosing --> UtilityInstall : enclosureInstalled
    UtilityInstall --> Inspecting : utilitiesInstalled
    Inspecting --> Commissioned : structureInspected
    Inspecting --> UtilityInstall : correctionsRequired
    Commissioned --> [*] : buildingCommissioned
```
