...
```mermaid
erDiagram
    FoundationDesign ||--|| Excavation : "prepared-by"
    FoundationDesign ||--|| Reinforcement : "strengthened-with"
    Reinforcement ||--|| FormWork : "placed-in"
    FormWork ||--|| ConcretePour : "receives"
    ConcretePour ||--|| CureSchedule : "cured-per"
    FoundationDesign ||--|| CureSchedule : "monitored-by"
```
...
stateDiagram-v2
    [*] --> Excavating
    Excavating --> FormworkSetup : siteExcavated
    FormworkSetup --> RebarPlace : formworkInstalled
    RebarPlace --> Pouring : reinforcementPlaced
    Pouring --> Finishing : concreteePoured
    Finishing --> Curing : surfaceFinished
    Curing --> LoadTest : curingMonitored
    LoadTest --> Certified : loadTestPassed
    LoadTest --> Finishing : testFailed
    Certified --> [*]
```
