...
```mermaid
erDiagram
    EnergyAuditor ||--|| BuildingOwner : follows
    EnergyAuditor }o--|| InsulationSupplier : generates
    EnergyAuditor ||--|{ BuildingInspector : requires
    BuildingOwner ||--|{ EnergyAuditor : creates
    InsulationSupplier ||--o{ EnergyAuditor : documents
    BuildingInspector ||--|| EnergyAuditor : supports
```
...
stateDiagram-v2
    [*] --> Energy_Auditor
    Energy_Auditor --> Building_Owner: BuildingOwner
    Building_Owner --> Insulation_Supplier: InsulationSupplier
    Insulation_Supplier --> Building_Inspector: BuildingInspector
    Insulation_Supplier --> Building_Owner: InsulationSupplier
    Building_Inspector --> [*]
```
