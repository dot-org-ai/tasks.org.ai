...
```mermaid
erDiagram
    Deceased ||--|| EmbalmingCase : "subject-of"
    EmbalmingCase ||--|| AuthorizationForm : "authorized-by"
    EmbalmingCase ||--o{ EmbalmingFluid : "treated-with"
    EmbalmingCase ||--|| PreparationRoom : "performed-in"
    EmbalmingCase ||--|| CaseReport : "documented-in"
    Deceased }o--|| AuthorizationForm : "consented-for"
```
...
```mermaid
stateDiagram-v2
    [*] --> Received : remainsReceived
    Received --> Authorized : authorizationObtained
    Authorized --> Prepared : sitePrepared
    Prepared --> Injected : arterialInjectionPerformed
    Injected --> Aspirated : cavitiesAspirated
    Aspirated --> CavityTreated : cavitiesTreated
    CavityTreated --> Restored : restorationPerformed
    CavityTreated --> Prepared : cavitiesTreated
    Restored --> [*]
```
## Workflow
