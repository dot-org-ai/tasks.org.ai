...
```mermaid
erDiagram
    Furnace ||--o{ Workpiece : "heats"
    Furnace ||--|| AtmosphereGas : "filled-with"
    HeatTreatmentCycle ||--|| Furnace : "programs"
    Thermocouple ||--|| Furnace : "monitors"
    Workpiece ||--|| TreatmentRecord : "logged-in"
```
...
stateDiagram-v2
    [*] --> FurnaceLoaded
    FurnaceLoaded --> ParametersSet : parametersSet
    ParametersSet --> HeatingCycleStarted : heatingCycleStarted
    HeatingCycleStarted --> TargetTemperatureReached : targetTemperatureReached
    TargetTemperatureReached --> SoakCompleted : soakCompleted
    SoakCompleted --> CoolingCompleted : coolingCompleted
    CoolingCompleted --> PropertiesInspected : propertiesInspected
    PropertiesInspected --> FurnaceLoaded : furnaceLoaded
    PropertiesInspected --> [*]
```
