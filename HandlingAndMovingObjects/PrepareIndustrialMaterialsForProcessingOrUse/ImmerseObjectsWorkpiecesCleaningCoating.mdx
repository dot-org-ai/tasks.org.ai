...
```mermaid
erDiagram
    DipTank ||--|| SolutionBatch : "filled-with"
    CoatingSpec ||--|| ImmersionCycle : "defines"
    ImmersionCycle ||--o{ Workpiece : "treats"
    DipTank ||--|| RinseStation : "followed-by"
    Workpiece ||--|| CoatingSpec : "measured-against"
```
...
stateDiagram-v2
    [*] --> SolutionPrepared
    SolutionPrepared --> PartsLoaded : partsLoaded
    PartsLoaded --> PartsImmersed : partsImmersed
    PartsImmersed --> BathMonitored : bathMonitored
    BathMonitored --> PartsRinsed : partsRinsed
    PartsRinsed --> PartsDrained : partsDrained
    PartsDrained --> SurfaceInspected : surfaceInspected
    SurfaceInspected --> PartsImmersed : partsImmersed
    SurfaceInspected --> [*]
```
