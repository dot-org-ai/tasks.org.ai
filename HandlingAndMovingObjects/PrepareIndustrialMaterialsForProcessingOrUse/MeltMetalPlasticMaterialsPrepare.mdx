...
```mermaid
erDiagram
    ChargeTicket ||--|| Furnace : "loaded-into"
    Furnace ||--|| MoltenBath : "produces"
    MoltenBath ||--o{ AlloyAddition : "adjusted-with"
    MoltenBath ||--|| SpectrographSample : "analyzed-via"
    MoltenBath ||--|| Ladle : "tapped-into"
```
...
stateDiagram-v2
    [*] --> FurnaceCharged
    FurnaceCharged --> MeltInitiated : meltInitiated
    MeltInitiated --> MeltTemperatureReached : meltTemperatureReached
    MeltTemperatureReached --> MeltSampled : meltSampled
    MeltSampled --> AlloysAdded : alloysAdded
    AlloysAdded --> MeltSampled : meltSampled
    MeltSampled --> FurnaceTapped : furnaceTapped
    FurnaceTapped --> MeltDataLogged : meltDataLogged
    MeltDataLogged --> [*]
```
