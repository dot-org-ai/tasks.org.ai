...
```mermaid
erDiagram
    PurchaseOrder ||--o{ MaterialLot : "delivers"
    MaterialLot ||--|| MaterialCertificate : "verified-by"
    MaterialLot ||--|| InspectionReport : "assessed-in"
    ConditioningSpec ||--|| MaterialLot : "prescribes-treatment-for"
    MaterialLot ||--|| StagingArea : "positioned-at"
    WorkOrder ||--o{ MaterialLot : "consumes"
```
...
stateDiagram-v2
    [*] --> MaterialReceived
    MaterialReceived --> MaterialInspected : materialInspected
    MaterialInspected --> MaterialRejected : materialRejected
    MaterialInspected --> LotAssigned : lotAssigned
    LotAssigned --> MaterialConditioned : materialConditioned
    MaterialConditioned --> MaterialStaged : materialStaged
    MaterialStaged --> MaterialReleased : materialReleased
    MaterialReleased --> [*]
    MaterialRejected --> MaterialReceived : materialReceived
```
