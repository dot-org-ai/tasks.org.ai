...
```mermaid
erDiagram
    WasteSample ||--|| WasteProfile : "characterizes"
    WasteProfile ||--|| Container : "packaged-in"
    Container ||--|| HazardLabel : "marked-with"
    Manifest ||--o{ Container : "tracks"
    Container ||--|| AccumulationArea : "staged-in"
    DisposalRecord ||--|| Manifest : "closes-out"
```
...
stateDiagram-v2
    [*] --> WasteCharacterized
    WasteCharacterized --> ContainerSelected : containerSelected
    ContainerSelected --> WastePackaged : wastePackaged
    WastePackaged --> ContainerLabeled : containerLabeled
    ContainerLabeled --> ManifestGenerated : manifestGenerated
    ManifestGenerated --> WasteStaged : wasteStaged
    WasteStaged --> ComplianceVerified : complianceVerified
    ComplianceVerified --> ContainerLabeled : containerLabeled
    ComplianceVerified --> [*]
```
