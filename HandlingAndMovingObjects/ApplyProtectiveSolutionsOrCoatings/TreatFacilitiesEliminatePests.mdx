...
```mermaid
erDiagram
    PesticideDistributor ||--|| FacilityManager : follows
    PesticideDistributor }o--|| HealthInspector : generates
    PesticideDistributor ||--|{ RegulatoryAgency : requires
    PesticideDistributor ||--o{ Laboratory : tracks
    PesticideDistributor ||--|| TenantOccupant : validates
    FacilityManager ||--|{ PesticideDistributor : creates
    HealthInspector ||--o{ PesticideDistributor : documents
    RegulatoryAgency ||--|| PesticideDistributor : supports
```
...
```mermaid
stateDiagram-v2
    [*] --> Pesticide_Distributor
    Pesticide_Distributor --> Facility_Manager: FacilityManager
    Facility_Manager --> Health_Inspector: HealthInspector
    Health_Inspector --> Regulatory_Agency: RegulatoryAgency
    Regulatory_Agency --> Laboratory: Laboratory
    Laboratory --> Tenant_Occupant: TenantOccupant
    Regulatory_Agency --> Facility_Manager: HealthInspector
    Tenant_Occupant --> [*]
```
