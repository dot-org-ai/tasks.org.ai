...
```mermaid
erDiagram
    FinishSupplier ||--|| Customer : follows
    FinishSupplier }o--|| ChemicalDisposer : generates
    FinishSupplier ||--|{ MaterialLab : requires
    FinishSupplier ||--o{ ToolProvider : tracks
    FinishSupplier ||--|| DesignConsultant : validates
    Customer ||--|{ FinishSupplier : creates
    ChemicalDisposer ||--o{ FinishSupplier : documents
    MaterialLab ||--|| FinishSupplier : supports
```
...
stateDiagram-v2
    [*] --> Finish_Supplier
    Finish_Supplier --> Customer: Customer
    Customer --> Chemical_Disposer: ChemicalDisposer
    Chemical_Disposer --> Material_Lab: MaterialLab
    Material_Lab --> Tool_Provider: ToolProvider
    Tool_Provider --> Design_Consultant: DesignConsultant
    Material_Lab --> Customer: ChemicalDisposer
    Design_Consultant --> [*]
```
