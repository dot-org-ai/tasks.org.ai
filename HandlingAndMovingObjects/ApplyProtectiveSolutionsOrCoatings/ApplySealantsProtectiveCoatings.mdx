...
```mermaid
erDiagram
    SealantManufacturer ||--|| StructuralEngineer : follows
    SealantManufacturer }o--|| BuildingOwner : generates
    SealantManufacturer ||--|{ Inspector : requires
    SealantManufacturer ||--o{ MaterialTestingLab : tracks
    SealantManufacturer ||--|| ToolSupplier : validates
    StructuralEngineer ||--|{ SealantManufacturer : creates
    BuildingOwner ||--o{ SealantManufacturer : documents
    Inspector ||--|| SealantManufacturer : supports
```
...
```mermaid
stateDiagram-v2
    [*] --> Sealant_Manufacturer
    Sealant_Manufacturer --> Structural_Engineer: StructuralEngineer
    Structural_Engineer --> Building_Owner: BuildingOwner
    Building_Owner --> Inspector: Inspector
    Inspector --> Material_Testing_Lab: MaterialTestingLab
    Material_Testing_Lab --> Tool_Supplier: ToolSupplier
    Inspector --> Structural_Engineer: BuildingOwner
    Tool_Supplier --> [*]
```
