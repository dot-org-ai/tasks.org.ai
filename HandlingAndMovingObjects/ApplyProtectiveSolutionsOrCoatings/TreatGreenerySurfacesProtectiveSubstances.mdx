...
```mermaid
erDiagram
    HorticultureSupplier ||--|| Arborist : follows
    HorticultureSupplier }o--|| PropertyOwner : generates
    HorticultureSupplier ||--|{ EnvironmentalAgency : requires
    HorticultureSupplier ||--o{ WeatherService : tracks
    HorticultureSupplier ||--|| NurseryConsultant : validates
    Arborist ||--|{ HorticultureSupplier : creates
    PropertyOwner ||--o{ HorticultureSupplier : documents
    EnvironmentalAgency ||--|| HorticultureSupplier : supports
```
...
```mermaid
stateDiagram-v2
    [*] --> Horticulture_Supplier
    Horticulture_Supplier --> Arborist: Arborist
    Arborist --> Property_Owner: PropertyOwner
    Property_Owner --> Environmental_Agency: EnvironmentalAgency
    Environmental_Agency --> Weather_Service: WeatherService
    Weather_Service --> Nursery_Consultant: NurseryConsultant
    Environmental_Agency --> Arborist: PropertyOwner
    Nursery_Consultant --> [*]
```
