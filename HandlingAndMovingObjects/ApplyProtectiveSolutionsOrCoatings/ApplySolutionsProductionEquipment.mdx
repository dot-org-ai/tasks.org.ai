...
```mermaid
erDiagram
    ChemicalSupplier ||--|| EquipmentManufacturer : follows
    ChemicalSupplier }o--|| ProductionManager : generates
    ChemicalSupplier ||--|{ SafetyCoordinator : requires
    ChemicalSupplier ||--o{ QualityControl : tracks
    ChemicalSupplier ||--|| WasteTreatmentProvider : validates
    EquipmentManufacturer ||--|{ ChemicalSupplier : creates
    ProductionManager ||--o{ ChemicalSupplier : documents
    SafetyCoordinator ||--|| ChemicalSupplier : supports
```
...
stateDiagram-v2
    [*] --> Chemical_Supplier
    Chemical_Supplier --> Equipment_Manufacturer: EquipmentManufacturer
    Equipment_Manufacturer --> Production_Manager: ProductionManager
    Production_Manager --> Safety_Coordinator: SafetyCoordinator
    Safety_Coordinator --> Quality_Control: QualityControl
    Quality_Control --> Waste_Treatment_Provider: WasteTreatmentProvider
    Safety_Coordinator --> Equipment_Manufacturer: ProductionManager
    Waste_Treatment_Provider --> [*]
```
