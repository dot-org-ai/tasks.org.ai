...
```mermaid
erDiagram
    Equipment ||--o{ LevelingSpec : "defines"
    Equipment ||--|{ LevelingJob : "generates"
    Equipment }o--|| Measurement : "uses"
    Equipment ||--|| LevelingReport : "tracked-by"
    Equipment ||--o{ LevelCertificate : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : levelingJobCreated
    InProgress --> Verified : levelMeasured
    Verified --> Approved : footingAdjusted
    Approved --> Active : levelVerified
    Active --> Complete : levelApproved
