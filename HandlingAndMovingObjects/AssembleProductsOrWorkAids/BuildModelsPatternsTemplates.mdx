...
```mermaid
erDiagram
    Designer ||--|| MaterialSupplier : follows
    Designer }o--|| Foundry : generates
    Designer ||--|{ ManufacturingEngineer : requires
    Designer ||--o{ QualityLab : tracks
    Designer ||--|| StorageWarehouse : validates
    MaterialSupplier ||--|{ Designer : creates
    Foundry ||--o{ Designer : documents
    ManufacturingEngineer ||--|| Designer : supports
```
...
stateDiagram-v2
    [*] --> Designer
    Designer --> Material_Supplier: MaterialSupplier
    Material_Supplier --> Foundry: Foundry
    Foundry --> Manufacturing_Engineer: ManufacturingEngineer
    Manufacturing_Engineer --> Quality_Lab: QualityLab
    Quality_Lab --> Storage_Warehouse: StorageWarehouse
    Manufacturing_Engineer --> Material_Supplier: Foundry
    Storage_Warehouse --> [*]
```
