...
```mermaid
erDiagram
    Pattern ||--|| Specification : "built-to"
    Pattern ||--|| BuildSheet : "fabricated-per"
    Template ||--|| Specification : "dimensioned-by"
    Model ||--|| Specification : "represents"
    Pattern ||--|{ InspectionReport : "measured-in"
    BuildSheet ||--o{ Pattern : "sequences"
```
...
stateDiagram-v2
    [*] --> Interpreting
    Interpreting --> MaterialSelection : drawingInterpreted
    MaterialSelection --> Fabricating : materialSelected
    Fabricating --> Finishing : patternFabricated
    Finishing --> Measuring : finishApplied
    Measuring --> Approving : dimensionsMeasured
    Approving --> Archiving : patternApproved
    Measuring --> Fabricating : outOfTolerance
    Archiving --> [*] : patternArchived
```
