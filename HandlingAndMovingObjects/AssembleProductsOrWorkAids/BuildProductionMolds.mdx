...
```mermaid
erDiagram
    ToolingSteelSupplier ||--|| CNCMachiningCenter : follows
    ToolingSteelSupplier }o--|| HeatTreatmentShop : generates
    ToolingSteelSupplier ||--|{ InjectionMoldingPlant : requires
    ToolingSteelSupplier ||--o{ QualityInspector : tracks
    ToolingSteelSupplier ||--|| MaintenanceProvider : validates
    CNCMachiningCenter ||--|{ ToolingSteelSupplier : creates
    HeatTreatmentShop ||--o{ ToolingSteelSupplier : documents
    InjectionMoldingPlant ||--|| ToolingSteelSupplier : supports
```
...
```mermaid
stateDiagram-v2
    [*] --> Tooling_Steel_Supplier
    Tooling_Steel_Supplier --> C_N_C_Machining_Center: CNCMachiningCenter
    C_N_C_Machining_Center --> Heat_Treatment_Shop: HeatTreatmentShop
    Heat_Treatment_Shop --> Injection_Molding_Plant: InjectionMoldingPlant
    Injection_Molding_Plant --> Quality_Inspector: QualityInspector
    Quality_Inspector --> Maintenance_Provider: MaintenanceProvider
    Injection_Molding_Plant --> C_N_C_Machining_Center: HeatTreatmentShop
