...
```mermaid
erDiagram
    Assembly ||--o{ Component : "constructed-from"
    WorkOrder ||--|| Assembly : "produces"
    WorkOrder ||--|| CutList : "cut-per"
    CutList ||--o{ Component : "specifies"
    Assembly ||--|| JoinerySpec : "joined-per"
    Assembly ||--|| FinishedProduct : "becomes"
```
...
stateDiagram-v2
    [*] --> Staging
    Staging --> Joining : componentsStaged
    Joining --> Clamping : partsJoined
    Clamping --> Routing : assemblyGluedUp
    Routing --> Sanding : edgesRouted
    Sanding --> Inspecting : surfacesSanded
    Inspecting --> Packaging : qualityApproved
    Inspecting --> Sanding : reworkRequired
