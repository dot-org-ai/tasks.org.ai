...
```mermaid
erDiagram
    RainwaterSystem ||--o{ GreywaterSystem : "defines"
    RainwaterSystem ||--|{ LowFlowFixture : "generates"
    RainwaterSystem }o--|| FiltrationUnit : "uses"
    RainwaterSystem ||--|| WaterMeter : "tracked-by"
    RainwaterSystem ||--o{ ComplianceReport : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : waterUsageAssessed
    InProgress --> Verified : greenSystemDesigned
    Verified --> Approved : collectionInstalled
    Approved --> Active : filtrationInstalled
    Active --> Complete : fixturesRetrofitted
