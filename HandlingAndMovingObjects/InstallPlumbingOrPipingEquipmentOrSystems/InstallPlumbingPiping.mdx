...
```mermaid
erDiagram
    WorkOrder ||--o{ PipeRun : "specifies"
    PipeRun ||--o{ Fitting : "joined-by"
    PipeRun ||--o{ Valve : "regulated-by"
    PipeRun ||--o{ Fixture : "terminates-at"
    PipeRun ||--|| PressureTest : "verified-by"
```
...
```mermaid
stateDiagram-v2
    [*] --> Surveyed : siteAssessed
    Surveyed --> Cut : pipeCut
    Cut --> Assembled : pipingAssembled
    Assembled --> FixturesOn : fixturesConnected
    FixturesOn --> Tested : pressureTestPassed
    FixturesOn --> Assembled : pressureTestFailed
    Tested --> Completed : installationCompleted
    Completed --> [*]
```
## Workflow
