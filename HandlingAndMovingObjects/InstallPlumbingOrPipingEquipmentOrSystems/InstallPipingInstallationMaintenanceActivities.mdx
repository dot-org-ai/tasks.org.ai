...
```mermaid
erDiagram
    MaintenanceOrder ||--o{ PipeSection : "scopes"
    MaintenanceOrder ||--|| ShutdownPermit : "requires"
    PipeSection ||--o{ WeldRecord : "joined-per"
    PipeSection ||--|| LeakTest : "pressure-tested-in"
    SparePartsInventory ||--o{ PipeSection : "supplies"
```
...
```mermaid
stateDiagram-v2
    [*] --> Assessed : conditionAssessed
    Assessed --> ShutDown : shutdownAuthorized
    ShutDown --> Removed : damagedPipingRemoved
    Removed --> Fabricated : pipingFabricated
    Fabricated --> Installed : replacementInstalled
    Installed --> Tested : integrityTestPassed
    Tested --> Installed : integrityTestPassed
    Tested --> Restored : systemRestored
    Restored --> [*]
```
## Workflow
