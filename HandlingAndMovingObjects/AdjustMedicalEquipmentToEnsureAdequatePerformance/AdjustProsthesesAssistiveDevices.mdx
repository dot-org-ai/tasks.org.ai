...
```mermaid
erDiagram
    ProstheticDevice ||--o{ AlignmentSetting : "configured with"
    ProstheticDevice ||--o{ SocketModification : "reshaped by"
    OrthoticDevice ||--o{ ComponentReplacement : "restored with"
    FunctionalAssessment ||--|| ProstheticDevice : "evaluates performance of"
    FunctionalAssessment ||--|| OrthoticDevice : "evaluates performance of"
    AdjustmentRecord ||--|| SocketModification : "documents"
    AdjustmentRecord ||--|| AlignmentSetting : "records changes to"
```
...
stateDiagram-v2
    [*] --> FitEvaluated
    FitEvaluated --> IssuesIdentified : issuesIdentified
    IssuesIdentified --> AlignmentAdjusted : alignmentAdjusted
    IssuesIdentified --> SocketModified : socketModified
    AlignmentAdjusted --> FunctionTested : functionTested
    SocketModified --> FunctionTested : functionTested
    FunctionTested --> ComponentsTuned : componentsTuned
    ComponentsTuned --> FunctionTested : functionTested
    FunctionTested --> AdjustmentDocumented : adjustmentDocumented
    AdjustmentDocumented --> [*]
```
