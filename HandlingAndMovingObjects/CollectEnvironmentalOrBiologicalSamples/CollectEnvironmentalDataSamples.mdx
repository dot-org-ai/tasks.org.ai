...
```mermaid
erDiagram
    SamplingPlan ||--o{ SamplingSite : "defines"
    SamplingPlan ||--|{ Sample : "generates"
    SamplingPlan }o--|| PreservationMethod : "uses"
    SamplingPlan ||--|| ChainOfCustody : "tracked-by"
    SamplingPlan ||--o{ AnalysisRequest : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : samplingPlanDesigned
    InProgress --> Verified : sitesSelected
    Verified --> Approved : sampleCollected
    Approved --> Active : samplePreserved
    Active --> Complete : sampleLabeled
