...
```mermaid
erDiagram
    SamplingPlan ||--o{ GeologicalSample : "defines"
    SamplingPlan ||--|{ SampleContainer : "generates"
    SamplingPlan }o--|| ProvenanceRecord : "uses"
    SamplingPlan ||--|| ChainOfCustody : "tracked-by"
    SamplingPlan ||--o{ CoreBox : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : samplingPlanApproved
    InProgress --> Verified : sampleExtracted
    Verified --> Approved : sampleLabeled
    Approved --> Active : samplePreserved
    Active --> Complete : provenanceRecorded
