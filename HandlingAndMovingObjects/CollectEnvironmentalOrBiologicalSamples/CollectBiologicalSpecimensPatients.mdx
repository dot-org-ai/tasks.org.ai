...
```mermaid
erDiagram
    SpecimenOrder ||--o{ Specimen : "defines"
    SpecimenOrder ||--|{ Container : "generates"
    SpecimenOrder }o--|| Label : "uses"
    SpecimenOrder ||--|| TransportLog : "tracked-by"
    SpecimenOrder ||--o{ QualityCheck : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : orderVerified
    InProgress --> Verified : patientPrepared
    Verified --> Approved : specimenCollected
    Approved --> Active : containerLabeled
    Active --> Complete : qualityCheckPerformed
