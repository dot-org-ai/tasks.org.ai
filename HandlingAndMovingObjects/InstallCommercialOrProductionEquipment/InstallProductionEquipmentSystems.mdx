...
```mermaid
erDiagram
    ProductionMachine ||--|| FoundationPad : "anchored-to"
    ProductionMachine ||--|| AlignmentRecord : "leveled-per"
    UtilityDrop ||--o{ ProductionMachine : "powers"
    ProductionMachine ||--o{ SafetyInterlock : "guarded-by"
    ProductionMachine ||--|| IOQ : "qualified-in"
    IOQ ||--o{ AcceptanceTestResult : "documented-by"
```
...
```mermaid
stateDiagram-v2
    [*] --> Received : equipmentReceived
    Received --> Positioned : riggedIntoPosition
    Positioned --> Anchored : anchoredAndLeveled
    Anchored --> UtilitiesReady : utilitiesConnected
    UtilitiesReady --> SafetySet : safetyIntegrated
    SafetySet --> Qualified : qualificationCompleted
    Qualified --> SafetySet : qualificationCompleted
    Qualified --> HandedOff : handedOffToProduction
    HandedOff --> [*]
```
## Workflow
