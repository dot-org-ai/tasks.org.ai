...
```mermaid
erDiagram
    HeadendRack ||--o{ SpeakerSystem : "amplifies"
    CableRun ||--|| SpeakerSystem : "feeds"
    CableRun ||--|| PunchDownBlock : "terminated-at"
    SpeakerSystem }o--|| PagingZone : "covers"
    PagingZone ||--|| CoverageMap : "mapped-in"
    CommissioningReport ||--o{ PagingZone : "validates"
```
...
```mermaid
stateDiagram-v2
    [*] --> Surveyed : siteSurveyed
    Surveyed --> CablePulled : cablePulled
    CablePulled --> Mounted : devicesMounted
    Mounted --> Terminated : cablingTerminated
    Terminated --> Configured : systemConfigured
    Configured --> Tested : coverageTested
    Tested --> Configured : coverageTested
    Tested --> Documented : systemDocumented
    Documented --> [*]
```
## Workflow
