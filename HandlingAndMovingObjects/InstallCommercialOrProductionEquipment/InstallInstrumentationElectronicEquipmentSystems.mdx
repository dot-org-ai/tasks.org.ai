...
```mermaid
erDiagram
    InstrumentDataSheet ||--|| FieldInstrument : "specifies"
    FieldInstrument ||--|| InstrumentCable : "wired-via"
    InstrumentCable ||--|| JunctionBox : "terminated-at"
    JunctionBox ||--|| MarshallingCabinet : "routed-to"
    FieldInstrument ||--|| CalibrationCertificate : "certified-by"
    FieldInstrument ||--|| LoopCheckSheet : "verified-in"
```
...
```mermaid
stateDiagram-v2
    [*] --> SpecReviewed : specificationsReviewed
    SpecReviewed --> Mounted : instrumentMounted
    Mounted --> Cabled : cablePulled
    Cabled --> Terminated : wiringTerminated
    Terminated --> Calibrated : deviceCalibrated
    Calibrated --> LoopChecked : loopCheckCompleted
    LoopChecked --> Calibrated : loopCheckCompleted
    LoopChecked --> Commissioned : loopCommissioned
    Commissioned --> [*]
```
## Workflow
