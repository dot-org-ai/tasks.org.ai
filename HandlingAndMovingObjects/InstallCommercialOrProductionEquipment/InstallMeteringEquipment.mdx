...
```mermaid
erDiagram
    Meter ||--o{ MeterBase : "defines"
    Meter ||--|{ CommunicationModule : "generates"
    Meter }o--|| MeterRegister : "uses"
    Meter ||--|| CalibrationReport : "tracked-by"
    Meter ||--o{ ServiceConnection : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : locationAssessed
    InProgress --> Verified : mountingPrepared
    Verified --> Approved : serviceConnected
    Approved --> Active : meterInstalled
    Active --> Complete : meterCalibrated
