...
```mermaid
erDiagram
    RigUpPlan ||--|| DrillingRig : "sequences-assembly-of"
    DrillingRig ||--|| Derrick : "supported-by"
    DrillingRig ||--|| Drawworks : "hoisted-by"
    DrillingRig ||--|| MudSystem : "circulated-by"
    DrillingRig ||--|| BOP : "controlled-by"
    DrillingPermit ||--|| DrillingRig : "authorizes"
```
...
```mermaid
stateDiagram-v2
    [*] --> SiteReady : sitePrepared
    SiteReady --> DerrickUp : derrickRigged
    DerrickUp --> DrawworksSet : drawworksInstalled
    DrawworksSet --> MudReady : mudSystemAssembled
    MudReady --> BOPSet : bopInstalled
    BOPSet --> Powered : powerConnected
    Powered --> Commissioned : rigCommissioned
    Commissioned --> Powered : rigCommissioned
    Commissioned --> [*]
```
## Workflow
