...
```mermaid
erDiagram
    DeliveryOrder ||--|| Appliance : "schedules"
    Appliance ||--|| InstallationKit : "connected-with"
    Appliance ||--|| LevelingAdjustment : "leveled-by"
    Appliance ||--o| LeakTestReport : "pressure-tested-in"
    Appliance ||--o| OldApplianceRemoval : "replaces"
    Appliance ||--|| WarrantyRegistration : "activated-by"
```
...
```mermaid
stateDiagram-v2
    [*] --> Delivered : applianceDelivered
    Delivered --> Positioned : unitPositioned
    Positioned --> Connected : utilitiesConnected
    Connected --> Leveled : applianceLeveled
    Leveled --> Tested : operationTested
    Tested --> Oriented : customerOriented
    Tested --> Connected : operationTested
    Oriented --> Registered : warrantyRegistered
    Registered --> [*]
```
## Workflow
