...
```mermaid
erDiagram
    Appliance ||--o{ InstallationKit : "defines"
    Appliance ||--|{ DeliveryOrder : "generates"
    Appliance }o--|| LeakTestReport : "uses"
    Appliance ||--|| LevelingAdjustment : "tracked-by"
    Appliance ||--o{ OldApplianceRemoval : "includes"
```
...
stateDiagram-v2
    [*] --> Pending
    Pending --> InProgress : applianceDelivered
    InProgress --> Verified : unitPositioned
    Verified --> Approved : utilitiesConnected
    Approved --> Active : applianceLeveled
    Active --> Complete : operationTested
