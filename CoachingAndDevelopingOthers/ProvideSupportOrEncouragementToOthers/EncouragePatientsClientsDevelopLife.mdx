...
erDiagram
    ...
    Assessment ||--|| CarePlan : creates
    CarePlan ||--|{ LifeSkill : targets
    CarePlan ||--|{ Intervention : schedules
    Intervention ||--|{ ProgressNote : generates
    LifeSkill }o--|| ProgressNote : "tracked in"
```
...
stateDiagram-v2
    [*] --> Assessed
    Assessed --> PlanCreated : carePlanCreated
    PlanCreated --> GoalSet : skillGoalSet
    GoalSet --> InterventionActive : interventionDelivered
    InterventionActive --> Encouraged : encouragementProvided
    Encouraged --> Documented : progressDocumented
    Documented --> GoalAchieved : goalMet
    Documented --> PlanAdjusted : planAdjusted
    PlanAdjusted --> InterventionActive : interventionDelivered
    GoalAchieved --> GoalSet : nextSkill
    GoalAchieved --> [*]
```
