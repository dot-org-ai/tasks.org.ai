...
```mermaid
erDiagram
    Student ||--|{ Goal : pursues
    Student ||--|{ Challenge : faces
    Goal ||--|{ Progress : "measured by"
    Challenge ||--o{ Intervention : "addressed by"
    Goal ||--o| Achievement : "results in"
    Intervention }o--|| Progress : tracks
```
...
stateDiagram-v2
    [*] --> ChallengeIdentified
    ChallengeIdentified --> Encouraged : encouragementProvided
    Encouraged --> GoalSet : goalSet
    GoalSet --> InterventionPlanned : interventionPlanned
    InterventionPlanned --> Tracking : progressTracked
    Tracking --> Encouraged : goalNotMet
    Tracking --> Achieved : goalMet
    Achieved --> Celebrated : successCelebrated
    Celebrated --> GoalSet : nextGoal
    Celebrated --> [*]
```
