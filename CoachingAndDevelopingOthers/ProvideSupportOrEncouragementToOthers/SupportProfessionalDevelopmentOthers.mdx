...
```mermaid
erDiagram
    DevelopmentPlan ||--|{ Objective : contains
    DevelopmentPlan ||--|{ Competency : targets
    Objective ||--|{ Activity : "achieved via"
    Competency ||--|| Assessment : "measured by"
    Objective ||--o| Milestone : "results in"
    Activity }o--|| Competency : develops
```
...
stateDiagram-v2
    [*] --> Assessed
    Assessed --> PlanCreated : developmentPlanCreated
    PlanCreated --> ObjectiveSet : objectiveSet
    ObjectiveSet --> ActivityAssigned : activityRecommended
    ActivityAssigned --> InProgress : progressTracked
    InProgress --> FeedbackGiven : feedbackProvided
    FeedbackGiven --> InProgress : objectiveNotMet
    FeedbackGiven --> MilestoneAchieved : milestoneRecognized
    MilestoneAchieved --> ObjectiveSet : moreObjectives
    MilestoneAchieved --> Complete : allObjectivesMet
    Complete --> [*]
```
