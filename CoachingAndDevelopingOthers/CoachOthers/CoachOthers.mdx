...
```mermaid
erDiagram
    CoachingRelationship ||--|{ DevelopmentGoal : defines
    CoachingRelationship ||--|{ CoachingSession : schedules
    CoachingSession ||--|{ FeedbackRecord : produces
    CoachingSession ||--|| ActionPlan : "results in"
    DevelopmentGoal ||--|{ ProgressAssessment : "measured by"
    DevelopmentGoal }o--|| CompetencyModel : "aligned to"
```
...
stateDiagram-v2
    [*] --> Established
    Established --> GoalsDefined : developmentGoalSet
    GoalsDefined --> SessionPlanned : sessionScheduled
    SessionPlanned --> FeedbackGiven : feedbackDelivered
    FeedbackGiven --> ActionsDefined : actionPlanCreated
    ActionsDefined --> ProgressReviewed : progressAssessed
    ProgressReviewed --> SessionPlanned : goalsInProgress
    ProgressReviewed --> Closed : engagementClosed
    Closed --> [*]
```
