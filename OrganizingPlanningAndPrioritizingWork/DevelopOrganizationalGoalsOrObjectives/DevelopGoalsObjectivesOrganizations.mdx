...
```mermaid
erDiagram
    EnvironmentalScan ||--|| MissionStatement : "informs"
    MissionStatement ||--|| VisionStatement : "guides"
    VisionStatement ||--|{ StrategicGoal : "drives"
    StrategicGoal ||--|{ DepartmentalObjective : "decomposesInto"
    DepartmentalObjective ||--|{ KeyResult : "measuredBy"
    GoalCascadeMap ||--|{ StrategicGoal : "aligns"
    GoalCascadeMap ||--|{ DepartmentalObjective : "maps"
```
...
stateDiagram-v2
    [*] --> Scanning
    Scanning --> MissionDefined : environmentalScanCompleted
    MissionDefined --> GoalSetting : missionArticulated
    GoalSetting --> ObjectiveSetting : strategicGoalsEstablished
    ObjectiveSetting --> KRAssignment : objectivesDefined
    KRAssignment --> Cascading : keyResultsAssigned
    Cascading --> Monitoring : goalsCascaded
    Monitoring --> Adjusting : progressReviewed
    Adjusting --> Monitoring : objectivesRefined
    Adjusting --> GoalSetting : majorPivot
    Monitoring --> [*] : planningCycleComplete
```
