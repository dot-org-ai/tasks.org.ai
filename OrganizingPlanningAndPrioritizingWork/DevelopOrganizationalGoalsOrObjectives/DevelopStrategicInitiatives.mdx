...
```mermaid
erDiagram
    StrategicPlan ||--|{ InitiativeCharter : "spawns"
    InitiativeCharter ||--|| BusinessCase : "justifiedBy"
    InitiativePortfolio ||--|{ InitiativeCharter : "contains"
    InitiativeCharter ||--o{ ResourceAllocation : "funded"
    InitiativeCharter ||--|{ SuccessMetric : "measuredBy"
    GovernanceDashboard ||--|{ SuccessMetric : "displays"
    GovernanceDashboard ||--|| InitiativePortfolio : "monitors"
```
...
stateDiagram-v2
    [*] --> GapAnalysis
    GapAnalysis --> CaseBuilding : strategicGapsIdentified
    CaseBuilding --> CaseBuilding : revisionsRequested
    CaseBuilding --> Chartered : initiativeChartered
    Chartered --> Prioritized : portfolioPrioritized
    Prioritized --> Resourced : resourcesAllocated
    Resourced --> Executing : successMetricsDefined
    Executing --> Reviewing : performanceTracked
    Reviewing --> Executing : onTrack
    Reviewing --> Adjusting : offTrack
    Adjusting --> Resourced : reallocated
    Reviewing --> Complete : targetsAchieved
    Complete --> [*]
```
