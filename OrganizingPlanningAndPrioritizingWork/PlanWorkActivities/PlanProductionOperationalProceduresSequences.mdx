...
```mermaid
erDiagram
    ProductionOrder ||--|| ProcessSequence : "follows"
    ProcessSequence ||--|{ WorkInstruction : "contains"
    ProcessSequence ||--|{ QualityCheckpoint : "embeds"
    ProductionOrder ||--|| BillOfMaterials : "consumes"
    ProductionOrder }o--|| CapacityPlan : "scheduledWithin"
    WorkInstruction ||--o{ QualityCheckpoint : "referencesAt"
```
...
stateDiagram-v2
    [*] --> Forecasting
    Forecasting --> ProcessDesign : demandForecasted
    ProcessDesign --> Documenting : processSequenceDefined
    Documenting --> QualitySetup : workInstructionsCreated
    QualitySetup --> Scheduling : qualityGatesEstablished
    Scheduling --> Resourcing : productionScheduled
    Resourcing --> Executing : resourcesAllocated
    Executing --> Optimizing : reviewCycle
    Optimizing --> ProcessDesign : processOptimized
...
graph TD
    ...
    G -->|No| H[Resolve Shortages]
    H --> F
    G -->|Yes| I[Execute Production]
    I --> J[reviewAndOptimize]
    J --> K{Targets Met?}
    K -->|No| B
