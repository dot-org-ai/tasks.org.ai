...
```mermaid
erDiagram
    HarvestPlan ||--|| Permit : "authorizedBy"
    HarvestPlan ||--|| Territory : "conductedIn"
    HarvestPlan ||--|| EquipmentManifest : "equippedWith"
    HarvestPlan ||--o| HarvestReport : "documentedIn"
    SpeciesSurvey ||--|| HarvestPlan : "informs"
    Territory ||--|| SpeciesSurvey : "assessedBy"
    HarvestReport }o--|| Permit : "fulfillsRequirementsOf"
```
...
stateDiagram-v2
    [*] --> SpeciesReview
    SpeciesReview --> Permitting : speciesAssessed
    Permitting --> TerritorySelection : permitsObtained
    Permitting --> SpeciesReview : permitDenied
    TerritorySelection --> GearPrep : territorySelected
    GearPrep --> LogisticsPlanning : equipmentPrepared
    LogisticsPlanning --> InField : logisticsPlanned
    InField --> Reporting : activityCompleted
    InField --> InField : harvestContinues
    Reporting --> [*] : harvestReported
```
...
graph TD
    ...
    B --> C{Permits Granted?}
    C -->|No| D[Appeal or Reapply]
    D --> A
    C -->|Yes| E[selectTerritory]
    E --> F[prepareEquipment]
    F --> G[planLogistics]
    G --> H[conductActivity]
    H --> I[reportHarvest]
    I --> J{Season Open?}
    J -->|Yes| H
    J -->|No| K[Close Season Records]
```
