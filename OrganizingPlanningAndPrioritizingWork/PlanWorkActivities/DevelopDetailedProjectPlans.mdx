...
```mermaid
erDiagram
    ProjectPlan ||--|| WorkBreakdownStructure : "decomposedIn"
    WorkBreakdownStructure ||--|{ Task : "contains"
    Task ||--o{ Milestone : "achieves"
    Task ||--o{ ResourceAssignment : "staffedBy"
    ProjectPlan ||--|| RiskRegister : "assessesRisksIn"
    Milestone }o--|| ProjectPlan : "marksProgressIn"
```
...
stateDiagram-v2
    [*] --> Scoping
    Scoping --> Decomposing : scopeDefined
    Decomposing --> Estimating : workBreakdownCreated
    Estimating --> Sequencing : durationsEstimated
    Sequencing --> Resourcing : tasksSequenced
    Resourcing --> RiskAssessment : resourcesAllocated
    RiskAssessment --> PendingApproval : risksIdentified
    PendingApproval --> Decomposing : revisionsRequired
    PendingApproval --> Baselined : planBaselined
    Baselined --> [*]
```
