...
```mermaid
erDiagram
    ProjectCharter ||--|| WorkBreakdownStructure : "defines"
    WorkBreakdownStructure ||--|{ ProjectSchedule : "sequences"
    ProjectSchedule ||--o{ StatusReport : "trackedIn"
    ProjectCharter ||--o{ ChangeRequest : "amendedBy"
    ProjectSchedule ||--o{ RiskRegister : "informsRisksIn"
    ProjectCharter ||--|| LessonsLearned : "produces"
    ChangeRequest }o--|| ProjectSchedule : "modifies"
```
...
stateDiagram-v2
    [*] --> Initiation
    Initiation --> Planning : projectInitiated
    Planning --> Baselined : scheduleBaselined
    Baselined --> Executing : executeWorkPackage
    Executing --> Monitoring : progressReviewed
    Monitoring --> Executing : onTrack
    Monitoring --> RiskManagement : riskEscalated
    RiskManagement --> Executing : mitigationApplied
    Executing --> Closing : allPackagesComplete
    Closing --> [*] : projectClosed
```
