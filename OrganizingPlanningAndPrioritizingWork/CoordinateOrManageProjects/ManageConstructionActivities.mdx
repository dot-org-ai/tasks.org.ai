...
```mermaid
erDiagram
    ConstructionSchedule ||--|{ WorkOrder : "sequences"
    WorkOrder ||--o{ ChangeOrder : "modifiedBy"
    WorkOrder ||--o{ RFI : "clarifiesDesignFor"
    WorkOrder ||--|{ DailyLog : "trackedIn"
    ConstructionSchedule ||--o{ Permit : "requires"
    WorkOrder ||--o{ PunchList : "generatesDeficienciesIn"
    Permit }o--|| WorkOrder : "authorizes"
```
...
stateDiagram-v2
    [*] --> Mobilization
    Mobilization --> Scheduled : schedulePublished
    Scheduled --> InProgress : workOrderIssued
    InProgress --> InProgress : dailyLogUpdated
    InProgress --> UnderReview : conductInspection
    UnderReview --> InProgress : inspectionFailed
    UnderReview --> PhaseComplete : inspectionCompleted
    PhaseComplete --> InProgress : nextPhase
    PhaseComplete --> PunchListPhase : punchListCompiled
    PunchListPhase --> Closeout : deficienciesResolved
    Closeout --> [*]
```
