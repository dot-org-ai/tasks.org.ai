...
```mermaid
erDiagram
    Event ||--|| EventBudget : "fundedBy"
    Event ||--|{ GuestList : "invitesVia"
    Event ||--|{ VendorAgreement : "contractsWith"
    Event ||--|| Timeline : "scheduledBy"
    Event ||--|| FloorPlan : "laidOutIn"
    GuestList }o--|| FloorPlan : "seatedPer"
```
...
stateDiagram-v2
    [*] --> Vision
    Vision --> Budgeting : eventVisionDefined
    Budgeting --> VendorSelection : budgetApproved
    VendorSelection --> LayoutDesign : vendorsSelected
    LayoutDesign --> GuestManagement : floorPlanFinalized
    GuestManagement --> ReadyToExecute : guestListFinalized
    ReadyToExecute --> EventDay : coordinateExecution
    EventDay --> Review : eventCompleted
    Review --> [*] : postEventReviewCompleted
    VendorSelection --> Budgeting : overBudget
```
...
graph TD
    ...
    B --> C{Budget Approved?}
    C -->|No| A
    C -->|Yes| D[selectVendors]
    D --> E[designFloorPlan]
    E --> F[manageGuestList]
    F --> G{All RSVPs In?}
    G -->|No| F
    G -->|Yes| H[coordinateExecution]
    H --> I[conductPostEventReview]
```
