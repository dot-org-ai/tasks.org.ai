...
```mermaid
erDiagram
    RecyclingProgram ||--|{ CollectionRoute : "servicesVia"
    RecyclingProgram ||--|{ MaterialStream : "accepts"
    RecyclingProgram ||--o{ OutreachCampaign : "promotedBy"
    RecyclingProgram ||--|{ DiversionReport : "trackedIn"
    CollectionRoute ||--o{ ContaminationLog : "monitorsQuality"
    MaterialStream ||--o{ DiversionReport : "reportedIn"
```
...
stateDiagram-v2
    [*] --> Analysis
    Analysis --> Design : wasteStreamAnalyzed
    Design --> RouteSetup : programDesigned
    RouteSetup --> Outreach : routesEstablished
    Outreach --> Operating : outreachLaunched
    Operating --> Operating : diversionMonitored
    Operating --> Remediation : contaminationFlagged
    Remediation --> Outreach : educationRefreshed
    Operating --> Reporting : complianceReported
    Reporting --> Operating : continueOperations
    Reporting --> Design : targetsNotMet
    Reporting --> [*] : programComplete
```
