...
```mermaid
erDiagram
    Program ||--|{ Activity : "contains"
    Program ||--|| Budget : "fundedBy"
    Program ||--o{ RegistrationList : "enrollsParticipantsVia"
    Program ||--o{ FeedbackSurvey : "evaluatedBy"
    Budget ||--o{ GrantApplication : "supportedBy"
    Activity ||--o{ RegistrationList : "tracksAttendanceIn"
```
...
stateDiagram-v2
    [*] --> NeedsAssessment
    NeedsAssessment --> Designing : needsAssessed
    Designing --> FundingSought : programDesigned
    FundingSought --> Designing : fundingDenied
    FundingSought --> VenueBooking : fundingSecured
    VenueBooking --> Promotion : venuesReserved
    Promotion --> Execution : programPromoted
    Execution --> Evaluation : programExecuted
    Evaluation --> Designing : continueProgram
    Evaluation --> [*] : outcomesEvaluated
```
...
graph TD
    ...
    B --> C[secureFunding]
    C --> D{Funding Approved?}
    D -->|No| B
    D -->|Yes| E[reserveVenues]
    E --> F[promoteProgram]
    F --> G[executeProgram]
    G --> H[evaluateOutcomes]
    H --> I{Continue Program?}
    I -->|Yes| B
    I -->|No| J[Archive Results]
```
