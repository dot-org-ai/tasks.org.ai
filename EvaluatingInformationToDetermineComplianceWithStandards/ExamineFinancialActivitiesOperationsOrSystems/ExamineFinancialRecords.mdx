...
```mermaid
erDiagram
    GeneralLedger ||--o{ JournalEntry : "contains"
    JournalEntry }o--|| BankStatement : "tracks"
    BankStatement ||--o{ Invoice : "references"
    Invoice }o--|| Reconciliation : "generates"
    Reconciliation ||--o{ Discrepancy : "validates"
    Discrepancy }o--|| ExaminationReport : "supports"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : recordsSelected
    InProgress --> InProgress : transactionsTraced
    InProgress --> InProgress : balancesReconciled
    InProgress --> Review : classificationVerified
    Review --> InProgress
    InProgress --> InProgress : discrepancyIdentified
    InProgress --> InProgress : clarificationRequested
    InProgress --> Completed : examinationReportGenerated
    Completed --> [*]
```
## Workflow
