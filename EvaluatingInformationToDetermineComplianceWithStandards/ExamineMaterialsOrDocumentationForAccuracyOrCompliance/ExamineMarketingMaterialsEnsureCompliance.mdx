...
```mermaid
erDiagram
    MarketingMaterial ||--o{ ComplianceRule : "contains"
    ComplianceRule }o--|| ReviewSubmission : "tracks"
    ReviewSubmission ||--o{ ComplianceFinding : "references"
    ComplianceFinding }o--|| CorrectionRequest : "generates"
    CorrectionRequest ||--o{ ApprovalRecord : "validates"
```
...
stateDiagram-v2
    [*] --> Initialized
    Initialized --> InProgress : materialSubmitted
    InProgress --> InProgress : rulesChecked
    InProgress --> InProgress : findingAnnotated
    InProgress --> Review : correctionRequested
    Review --> InProgress
    InProgress --> InProgress : materialApproved
    InProgress --> InProgress : materialRejected
    InProgress --> Completed : reviewOutcomeLogged
    Completed --> [*]
```
## Workflow
