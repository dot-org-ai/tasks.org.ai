...
```mermaid
erDiagram
    ProgramSchedule ||--o{ AirTimeSlot : allocates
    AirTimeSlot ||--o{ MediaAsset : broadcasts
    MediaAsset ||--o{ StandardsReview : reviewedBy
    MediaAsset ||--o{ ContentRating : classifiedBy
    ProgramSchedule ||--o{ Rundown : structures
    ProgramSchedule ||--o{ BroadcastLog : records
```
...
```mermaid
stateDiagram-v2
    [*] --> ProgramLineupCurated
    ProgramLineupCurated --> ContentStandardsReviewed : contentStandardsReviewed
    ContentStandardsReviewed --> EditApproved : editApproved
    EditApproved --> AirTimeScheduled : airTimeScheduled
    AirTimeScheduled --> RundownAssembled : rundownAssembled
    RundownAssembled --> BroadcastLogGenerated : broadcastLogGenerated
    ContentStandardsReviewed --> ProgramLineupCurated : contentFlagged
    BroadcastLogGenerated --> [*]
```
