...
```mermaid
erDiagram
    PerformanceSeason ||--o{ Concert : schedules
    Concert ||--o{ Repertoire : programs
    Concert ||--o{ RehearsalSession : preparedBy
    RehearsalSession ||--o{ CallSheet : distributes
    Concert ||--o{ StageLayout : configures
    Concert ||--o{ MusicianRoster : staffedBy
```
...
stateDiagram-v2
    [*] --> ConcertProgrammed
    ConcertProgrammed --> RehearsalScheduled : rehearsalScheduled
    RehearsalScheduled --> CallSheetsDistributed : callSheetsDistributed
    CallSheetsDistributed --> RepertoireReady : repertoireReady
    RepertoireReady --> StageLayoutFinalized : stageLayoutFinalized
    StageLayoutFinalized --> PerformanceCompleted : performanceCompleted
    RepertoireReady --> RehearsalScheduled : rehearsalScheduled
