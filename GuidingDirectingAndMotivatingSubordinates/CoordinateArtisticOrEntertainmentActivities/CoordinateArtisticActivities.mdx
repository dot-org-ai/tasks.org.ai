...
```mermaid
erDiagram
    Exhibition ||--o{ ArtisticProject : showcases
    ArtisticProject ||--o{ Commission : fundedBy
    Exhibition ||--o{ InstallationPlan : requires
    Exhibition ||--o{ CollectionCatalog : curates
    ArtisticProject ||--o{ ArtistAgreement : governedBy
```
...
stateDiagram-v2
    [*] --> ExhibitionScheduled
    ExhibitionScheduled --> CollaborationInitiated : collaborationInitiated
    CollaborationInitiated --> CommissionApproved : commissionApproved
    CommissionApproved --> CollectionCurated : collectionCurated
    CollectionCurated --> InstallationCompleted : installationCompleted
    InstallationCompleted --> ProjectMilestoneReached : projectMilestoneReached
    ProjectMilestoneReached --> ExhibitionClosed : exhibitionClosed
    ProjectMilestoneReached --> CollaborationInitiated : collaborationInitiated
    ExhibitionClosed --> [*]
```
