...
```mermaid
erDiagram
    Tournament ||--o{ SportingEvent : contains
    GameSchedule ||--o{ SportingEvent : lists
    SportingEvent ||--o{ OfficialAssignment : staffedBy
    SportingEvent ||--o{ VenueReservation : hostedAt
    SportingEvent ||--o{ EventResult : produces
    Tournament ||--o{ TeamRoster : registers
```
...
stateDiagram-v2
    [*] --> EventScheduled
    EventScheduled --> TournamentBracketCreated : tournamentBracketCreated
    TournamentBracketCreated --> TeamRegistered : teamRegistered
    TeamRegistered --> OfficialsAssigned : officialsAssigned
    OfficialsAssigned --> VenueReserved : venueReserved
    VenueReserved --> ResultRecorded : resultRecorded
    ResultRecorded --> EventScheduled : eventScheduled
    ResultRecorded --> [*]
```
