...
```mermaid
erDiagram
    LaborContractor ||--o{ SafetyInspector : includes
    SafetyInspector }o--|| HRDepartment : "relates to"
    SafetyInspector }o--|| TrainingProvider : "relates to"
    SafetyInspector }o--|| EquipmentSupplier : "relates to"
    SafetyInspector }o--|| HealthcareProvider : "relates to"
    HRDepartment ||--o{ TrainingProvider : includes
    TrainingProvider }o--|| EquipmentSupplier : "relates to"
    TrainingProvider }o--|| HealthcareProvider : "relates to"
```
...
stateDiagram-v2
    [*] --> Initiated
    Initiated --> LaborContractor: LaborContractor
    LaborContractor --> SafetyInspector: SafetyInspector
    SafetyInspector --> HRDepartment: HRDepartment
    HRDepartment --> ValidationCheck: needs review
    ValidationCheck --> SafetyInspector: corrections needed
    ValidationCheck --> TrainingProvider: approved
    ValidationCheck --> TrainingProvider: TrainingProvider
    TrainingProvider --> EquipmentSupplier: EquipmentSupplier
    EquipmentSupplier --> HealthcareProvider: HealthcareProvider
    HealthcareProvider --> [*]
```
