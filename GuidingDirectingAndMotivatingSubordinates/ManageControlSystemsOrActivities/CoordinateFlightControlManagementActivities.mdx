...
```mermaid
erDiagram
    Airline ||--o{ FederalAviationAuthority : includes
    FederalAviationAuthority ||--o{ MilitaryAirCommand : includes
    MilitaryAirCommand ||--o{ WeatherService : includes
```
...
stateDiagram-v2
    [*] --> Initiated
    Initiated --> Airline: Airline
    Airline --> FederalAviationAuthority: FederalAviationAuthority
    FederalAviationAuthority --> MilitaryAirCommand: MilitaryAirCommand
    MilitaryAirCommand --> ValidationCheck: needs review
    ValidationCheck --> FederalAviationAuthority: corrections needed
    ValidationCheck --> WeatherService: approved
    ValidationCheck --> WeatherService: WeatherService
    WeatherService --> AirportOperations: AirportOperations
    AirportOperations --> PilotInCommand: PilotInCommand
    PilotInCommand --> [*]
```
