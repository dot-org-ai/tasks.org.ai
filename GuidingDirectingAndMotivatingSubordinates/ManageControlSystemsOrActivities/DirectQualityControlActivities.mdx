...
```mermaid
erDiagram
    QualityPlan ||--o{ InspectionRecord : "defines"
    InspectionRecord ||--o{ NonConformanceReport : "identifies"
    NonConformanceReport ||--|| CorrectiveAction : "requires"
    StatisticalProcessControl ||--o{ InspectionRecord : "monitors"
    CalibrationRecord ||--o{ InspectionRecord : "validates"
    QualityPlan ||--|| StatisticalProcessControl : "establishes"
```
...
stateDiagram-v2
    [*] --> PlanCreated
    PlanCreated --> InspectionCompleted : inspectionCompleted
    InspectionCompleted --> NonConformanceIssued : nonConformanceIssued
    NonConformanceIssued --> CorrectiveActionInitiated : correctiveActionInitiated
    CorrectiveActionInitiated --> CorrectiveActionClosed : correctiveActionClosed
    CorrectiveActionClosed --> InspectionCompleted : inspectionCompleted
    InspectionCompleted --> SPCAlarmTriggered : spcAlarmTriggered
    SPCAlarmTriggered --> InspectionCompleted : inspectionCompleted
    InspectionCompleted --> ProductReleased : productReleased
    ProductReleased --> [*]
```
