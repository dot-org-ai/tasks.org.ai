...
```mermaid
erDiagram
    Assignment ||--o{ PerformanceReview : "evaluated in"
    Schedule ||--o{ Assignment : "organizes"
    Assignment ||--o{ SupervisionNote : "documented by"
    PerformanceReview ||--o{ CorrectiveAction : "may trigger"
    AttendanceRecord ||--|| Schedule : "tracked against"
    CorrectiveAction ||--o{ SupervisionNote : "recorded in"
```
...
stateDiagram-v2
    [*] --> Assigned
    Assigned --> Monitored : workAssigned
    Monitored --> Reviewed : performanceReviewed
    Monitored --> Coaching : coachingProvided
    Coaching --> Monitored : coachingProvided
    Reviewed --> CorrectiveAction : correctiveActionIssued
    CorrectiveAction --> Monitored : scheduleAdjusted
    Reviewed --> OnLeave : timeOffApproved
    OnLeave --> Assigned : scheduleAdjusted
    Monitored --> BelowThreshold : performanceThresholdBreached
    BelowThreshold --> Coaching : coachingProvided
    Reviewed --> [*]
```
