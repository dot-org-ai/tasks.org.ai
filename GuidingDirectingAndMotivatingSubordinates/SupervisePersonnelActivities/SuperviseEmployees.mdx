...
```mermaid
erDiagram
    Employee ||--|| HumanResources : follows
    Employee }o--|| SeniorManagement : generates
    Employee ||--|{ UnionRepresentative : requires
    Employee ||--o{ Client : tracks
    HumanResources ||--|{ Employee : creates
    SeniorManagement ||--o{ Employee : documents
    UnionRepresentative ||--|| Employee : supports
```
...
stateDiagram-v2
    [*] --> Employee
    Employee --> Human_Resources: HumanResources
    Human_Resources --> Senior_Management: SeniorManagement
    Senior_Management --> Union_Representative: UnionRepresentative
    Union_Representative --> Client: Client
    Senior_Management --> Human_Resources: SeniorManagement
    Client --> [*]
```
