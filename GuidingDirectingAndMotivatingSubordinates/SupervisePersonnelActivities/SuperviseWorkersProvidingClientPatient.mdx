...
```mermaid
erDiagram
    Caseload ||--o{ CarePlan : "guided by"
    CarePlan ||--o{ ServiceNote : "documented in"
    SupervisionSession ||--o{ Caseload : "reviews"
    ComplianceRecord ||--|| ServiceNote : "audits"
    Caseload ||--o{ IncidentReport : "may produce"
    SupervisionSession ||--o{ ComplianceRecord : "generates"
```
...
stateDiagram-v2
    [*] --> CaseloadAssigned
    CaseloadAssigned --> CarePlanReviewed : carePlanReviewed
    CarePlanReviewed --> SupervisionDone : supervisionConducted
    SupervisionDone --> Audited : documentationAudited
    Audited --> SafetyConcern : safetyConcernRaised
    SafetyConcern --> CarePlanReviewed : carePlanReviewed
    Audited --> DischargeReady : dischargeApproved
    Audited --> CarePlanReviewed : carePlanReviewed
    CaseloadAssigned --> Imbalanced : caseloadImbalanceDetected
    Imbalanced --> CaseloadAssigned : caseloadAssigned
    DischargeReady --> [*]
```
