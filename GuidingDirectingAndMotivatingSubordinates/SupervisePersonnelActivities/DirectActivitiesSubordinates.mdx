...
```mermaid
erDiagram
    OperationalPlan ||--o{ TaskQueue : "prioritizes"
    TaskQueue ||--o{ Directive : "generates"
    Directive ||--o{ ResourceAllocation : "requires"
    Directive ||--o{ Dependency : "constrained by"
    Directive ||--o{ ProgressReport : "tracked in"
    ProgressReport ||--|| OperationalPlan : "informs"
```
...
stateDiagram-v2
    [*] --> Planned
    Planned --> Prioritized : tasksPrioritized
    Prioritized --> Directed : directiveIssued
    Directed --> InProgress : progressTracked
    InProgress --> Blocked : issueEscalated
    Blocked --> InProgress : blockerResolved
    InProgress --> Reallocated : resourcesReallocated
    Reallocated --> Directed : directiveIssued
    InProgress --> Reported : statusReported
    Reported --> [*]
```
