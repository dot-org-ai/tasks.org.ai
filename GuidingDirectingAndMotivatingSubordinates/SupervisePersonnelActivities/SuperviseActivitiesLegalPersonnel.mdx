...
```mermaid
erDiagram
    CaseFile ||--o{ Assignment : "delegates"
    Assignment ||--o{ WorkProduct : "produces"
    CaseFile ||--o{ Deadline : "governed by"
    WorkProduct ||--o{ BillingEntry : "generates"
    CaseFile ||--|| ConflictCheck : "cleared by"
    Assignment ||--|| Deadline : "constrained by"
```
...
stateDiagram-v2
    [*] --> ConflictCleared
    ConflictCleared --> Assigned : caseWorkAssigned
    Assigned --> DeadlineTracked : deadlineApproaching
    DeadlineTracked --> UnderReview : workProductReviewed
    UnderReview --> Assigned : workProductReviewed
    UnderReview --> FilingApproved : filingApproved
    FilingApproved --> Evaluated : performanceEvaluated
    DeadlineTracked --> Overdue : deadlineMissed
    Overdue --> Assigned : caseWorkAssigned
    Evaluated --> [*]
```
