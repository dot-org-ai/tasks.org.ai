...
```mermaid
erDiagram
    Paralegal ||--|| Client : follows
    Paralegal }o--|| Court : generates
    Paralegal ||--|{ OpposingCounsel : requires
    Paralegal ||--o{ BarAssociation : tracks
    Client ||--|{ Paralegal : creates
    Court ||--o{ Paralegal : documents
    OpposingCounsel ||--|| Paralegal : supports
```
...
stateDiagram-v2
    [*] --> Paralegal
    Paralegal --> Client: Client
    Client --> Court: Court
    Court --> Opposing_Counsel: OpposingCounsel
    Opposing_Counsel --> Bar_Association: BarAssociation
    Court --> Client: Court
    Bar_Association --> [*]
```
