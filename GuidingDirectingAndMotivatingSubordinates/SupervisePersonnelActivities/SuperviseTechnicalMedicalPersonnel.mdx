...
```mermaid
erDiagram
    MedicalTechnician ||--|| Physician : follows
    MedicalTechnician }o--|| AccreditationBody : generates
    MedicalTechnician ||--|{ EquipmentVendor : requires
    MedicalTechnician ||--o{ Patient : tracks
    MedicalTechnician ||--|| RegulatoryAgency : validates
    Physician ||--|{ MedicalTechnician : creates
    AccreditationBody ||--o{ MedicalTechnician : documents
    EquipmentVendor ||--|| MedicalTechnician : supports
```
...
stateDiagram-v2
    [*] --> Medical_Technician
    Medical_Technician --> Physician: Physician
    Physician --> Accreditation_Body: AccreditationBody
    Accreditation_Body --> Equipment_Vendor: EquipmentVendor
    Equipment_Vendor --> Patient: Patient
    Patient --> Regulatory_Agency: RegulatoryAgency
    Equipment_Vendor --> Physician: AccreditationBody
    Regulatory_Agency --> [*]
```
