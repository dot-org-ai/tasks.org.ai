...
```mermaid
erDiagram
    Credential ||--o{ CompetencyAssessment : "evaluated by"
    ClinicalProtocol ||--o{ QualityMetric : "measured by"
    CompetencyAssessment ||--|| ClinicalProtocol : "qualifies for"
    EquipmentCalibration ||--|| ClinicalProtocol : "validates for"
    IncidentReport ||--o{ QualityMetric : "impacts"
    Credential ||--|| EquipmentCalibration : "authorizes"
```
...
stateDiagram-v2
    [*] --> CredentialVerified
    CredentialVerified --> DutyAssigned : clinicalDutyAssigned
    DutyAssigned --> CompetencyChecked : competencyAssessed
    CompetencyChecked --> MetricsReviewed : qualityMetricsReviewed
    MetricsReviewed --> ProtocolUpdated : protocolChangeApproved
    ProtocolUpdated --> DutyAssigned : clinicalDutyAssigned
    MetricsReviewed --> IncidentReview : incidentInvestigated
    IncidentReview --> MetricsReviewed : qualityMetricsReviewed
    CredentialVerified --> Expiring : credentialExpiring
    Expiring --> CredentialVerified : credentialVerified
    MetricsReviewed --> [*]
```
