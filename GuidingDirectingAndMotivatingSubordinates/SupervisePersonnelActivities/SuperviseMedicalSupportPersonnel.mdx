...
```mermaid
erDiagram
    Credential ||--o{ TaskDelegation : "qualifies for"
    TaskDelegation ||--o{ ProcedureLog : "recorded in"
    ComplianceChecklist ||--o{ QualityMetric : "measured by"
    StaffSchedule ||--o{ TaskDelegation : "organizes"
    ProcedureLog ||--o{ QualityMetric : "evaluated against"
    Credential ||--|| ComplianceChecklist : "verified in"
```
...
stateDiagram-v2
    [*] --> CredentialVerified
    CredentialVerified --> Scheduled : staffSchedulePublished
    Scheduled --> Delegated : taskDelegated
    Delegated --> ProcedureComplete : procedureResultApproved
    ProcedureComplete --> QualityReviewed : qualityReviewCompleted
    QualityReviewed --> ComplianceGap : complianceGapIdentified
    ComplianceGap --> Counseled : staffCounselingDocumented
    Counseled --> Delegated : taskDelegated
    QualityReviewed --> Scheduled : staffSchedulePublished
    CredentialVerified --> Expiring : credentialExpiring
    Expiring --> CredentialVerified : credentialExpiring
    ProcedureComplete --> [*]
```
