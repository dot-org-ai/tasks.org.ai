...
```mermaid
erDiagram
    Patient ||--|| AttendingPhysician : follows
    Patient }o--|| AccreditationBody : generates
    Patient ||--|{ MedicalSupplyVendor : requires
    Patient ||--o{ InsurancePayer : tracks
    Patient ||--|| StateLicensingBoard : validates
    AttendingPhysician ||--|{ Patient : creates
    AccreditationBody ||--o{ Patient : documents
    MedicalSupplyVendor ||--|| Patient : supports
```
...
stateDiagram-v2
    [*] --> Patient
    Patient --> Attending_Physician: AttendingPhysician
    Attending_Physician --> Accreditation_Body: AccreditationBody
    Accreditation_Body --> Medical_Supply_Vendor: MedicalSupplyVendor
    Medical_Supply_Vendor --> Insurance_Payer: InsurancePayer
    Insurance_Payer --> State_Licensing_Board: StateLicensingBoard
    Medical_Supply_Vendor --> Attending_Physician: AccreditationBody
