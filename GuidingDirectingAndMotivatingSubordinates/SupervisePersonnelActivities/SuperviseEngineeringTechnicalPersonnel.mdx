...
```mermaid
erDiagram
    ProjectAssignment ||--o{ DesignReview : "evaluated by"
    ProjectAssignment ||--|| TechnicalSpecification : "governed by"
    DesignReview ||--o{ ChangeOrder : "may produce"
    SkillMatrix ||--|| ProjectAssignment : "informs"
    ProjectAssignment ||--o{ PerformanceReview : "assessed in"
    ChangeOrder ||--|| TechnicalSpecification : "modifies"
```
...
stateDiagram-v2
    [*] --> TeamAssigned
    TeamAssigned --> InDesign : projectTeamAssigned
    InDesign --> UnderReview : designReviewCompleted
    UnderReview --> Approved : designApproved
    UnderReview --> InDesign : designRejected
    Approved --> Rebalanced : workloadRebalanced
    Rebalanced --> InDesign : projectTeamAssigned
    Approved --> Evaluated : performanceReviewCompleted
    InDesign --> Escalated : technicalIssueEscalated
    Escalated --> InDesign : designReviewCompleted
    Evaluated --> [*]
```
