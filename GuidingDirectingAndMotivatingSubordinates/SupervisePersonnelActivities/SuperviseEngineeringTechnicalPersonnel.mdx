...
```mermaid
erDiagram
    Client ||--|| RegulatoryAuthority : follows
    Client }o--|| SubcontractorFirm : generates
    Client ||--|{ EquipmentSupplier : requires
    Client ||--o{ ProfessionalBoard : tracks
    Client ||--|| AcademicInstitution : validates
    RegulatoryAuthority ||--|{ Client : creates
    SubcontractorFirm ||--o{ Client : documents
    EquipmentSupplier ||--|| Client : supports
```
...
stateDiagram-v2
    [*] --> Client
    Client --> Regulatory_Authority: RegulatoryAuthority
    Regulatory_Authority --> Subcontractor_Firm: SubcontractorFirm
    Subcontractor_Firm --> Equipment_Supplier: EquipmentSupplier
    Equipment_Supplier --> Professional_Board: ProfessionalBoard
    Professional_Board --> Academic_Institution: AcademicInstitution
    Equipment_Supplier --> Regulatory_Authority: SubcontractorFirm
