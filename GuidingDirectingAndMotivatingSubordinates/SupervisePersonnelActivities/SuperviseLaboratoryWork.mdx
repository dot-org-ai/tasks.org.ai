...
```mermaid
erDiagram
    LabTechnician ||--|| PrincipalInvestigator : follows
    LabTechnician }o--|| SafetyOfficer : generates
    LabTechnician ||--|{ RegulatoryAuditor : requires
    LabTechnician ||--o{ EquipmentSupplier : tracks
    LabTechnician ||--|| Client : validates
    PrincipalInvestigator ||--|{ LabTechnician : creates
    SafetyOfficer ||--o{ LabTechnician : documents
    RegulatoryAuditor ||--|| LabTechnician : supports
```
...
stateDiagram-v2
    [*] --> Lab_Technician
    Lab_Technician --> Principal_Investigator: PrincipalInvestigator
    Principal_Investigator --> Safety_Officer: SafetyOfficer
    Safety_Officer --> Regulatory_Auditor: RegulatoryAuditor
    Regulatory_Auditor --> Equipment_Supplier: EquipmentSupplier
    Equipment_Supplier --> Client: Client
    Regulatory_Auditor --> Principal_Investigator: SafetyOfficer
    Client --> [*]
```
