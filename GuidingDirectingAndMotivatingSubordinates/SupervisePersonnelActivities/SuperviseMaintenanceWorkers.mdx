...
```mermaid
erDiagram
    WorkOrder ||--o{ CrewAssignment : "staffed by"
    MaintenanceSchedule ||--o{ WorkOrder : "generates"
    WorkOrder ||--o{ InspectionReport : "verified by"
    EquipmentLog ||--o{ WorkOrder : "tracks"
    WorkOrder ||--o{ SafetyIncident : "may produce"
    CrewAssignment ||--|| MaintenanceSchedule : "scheduled in"
```
...
stateDiagram-v2
    [*] --> Submitted
    Submitted --> Assigned : workOrderAssigned
    Assigned --> SafetyBriefed : workOrderAssigned
    SafetyBriefed --> InProgress : workOrderAssigned
    InProgress --> Inspected : inspectionPassed
    InProgress --> Failed : inspectionFailed
    Failed --> InProgress : workOrderAssigned
    InProgress --> SafetyViolation : safetyViolationReported
    SafetyViolation --> InProgress : overtimeApproved
    Inspected --> Completed : maintenanceCompleted
    Completed --> Scheduled : equipmentDowntimeExceeded
    Completed --> [*]
```
