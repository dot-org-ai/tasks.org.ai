...
```mermaid
erDiagram
    FacilityOwner ||--|| EquipmentVendor : follows
    FacilityOwner }o--|| SafetyInspector : generates
    FacilityOwner ||--|{ BuildingOccupant : requires
    FacilityOwner ||--o{ ContractorFirm : tracks
    FacilityOwner ||--|| RegulatoryAgency : validates
    EquipmentVendor ||--|{ FacilityOwner : creates
    SafetyInspector ||--o{ FacilityOwner : documents
    BuildingOccupant ||--|| FacilityOwner : supports
```
...
stateDiagram-v2
    [*] --> Facility_Owner
    Facility_Owner --> Equipment_Vendor: EquipmentVendor
    Equipment_Vendor --> Safety_Inspector: SafetyInspector
    Safety_Inspector --> Building_Occupant: BuildingOccupant
    Building_Occupant --> Contractor_Firm: ContractorFirm
    Contractor_Firm --> Regulatory_Agency: RegulatoryAgency
    Building_Occupant --> Equipment_Vendor: SafetyInspector
    Regulatory_Agency --> [*]
```
