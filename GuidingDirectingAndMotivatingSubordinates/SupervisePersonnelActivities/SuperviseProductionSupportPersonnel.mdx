...
```mermaid
erDiagram
    CustomerAccount ||--|| MaterialSupplier : follows
    CustomerAccount }o--|| SafetyRegulator : generates
    CustomerAccount ||--|{ UnionRepresentative : requires
    CustomerAccount ||--o{ QualityAuditor : tracks
    CustomerAccount ||--|| EquipmentManufacturer : validates
    MaterialSupplier ||--|{ CustomerAccount : creates
    SafetyRegulator ||--o{ CustomerAccount : documents
    UnionRepresentative ||--|| CustomerAccount : supports
```
...
stateDiagram-v2
    [*] --> Customer_Account
    Customer_Account --> Material_Supplier: MaterialSupplier
    Material_Supplier --> Safety_Regulator: SafetyRegulator
    Safety_Regulator --> Union_Representative: UnionRepresentative
    Union_Representative --> Quality_Auditor: QualityAuditor
    Quality_Auditor --> Equipment_Manufacturer: EquipmentManufacturer
    Union_Representative --> Material_Supplier: SafetyRegulator
