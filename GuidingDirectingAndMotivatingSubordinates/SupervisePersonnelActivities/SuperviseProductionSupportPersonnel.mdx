...
```mermaid
erDiagram
    ProductionOrder ||--o{ ShiftReport : "summarized in"
    ShiftReport ||--o{ QualityCheckpoint : "inspected at"
    QualityCheckpoint ||--o{ DowntimeLog : "may trigger"
    TrainingRecord ||--|| ProductionOrder : "qualifies for"
    SafetyObservation ||--|| ShiftReport : "documented in"
    DowntimeLog ||--|| ProductionOrder : "delays"
```
...
stateDiagram-v2
    [*] --> CrewAssigned
    CrewAssigned --> Producing : shiftCrewAssigned
    Producing --> Inspected : qualityDefectDetected
    Inspected --> Producing : safetyObservationRecorded
    Producing --> TargetMet : productionTargetMet
    Producing --> TargetMissed : productionTargetMissed
    TargetMissed --> Producing : shiftCrewAssigned
    Producing --> DowntimeAlert : downtimeExceeded
    DowntimeAlert --> Producing : shiftCrewAssigned
    TargetMet --> HandedOff : shiftHandoffCompleted
    HandedOff --> [*]
```
