...
```mermaid
erDiagram
    Patient ||--|| PatientFamily : follows
    Patient }o--|| Physician : generates
    Patient ||--|{ RegulatoryBody : requires
    Patient ||--o{ InsuranceProvider : tracks
    Patient ||--|| StaffingAgency : validates
    PatientFamily ||--|{ Patient : creates
    Physician ||--o{ Patient : documents
    RegulatoryBody ||--|| Patient : supports
```
...
stateDiagram-v2
    [*] --> Patient
    Patient --> Patient_Family: PatientFamily
    Patient_Family --> Physician: Physician
    Physician --> Regulatory_Body: RegulatoryBody
    Regulatory_Body --> Insurance_Provider: InsuranceProvider
    Insurance_Provider --> Staffing_Agency: StaffingAgency
    Regulatory_Body --> Patient_Family: Physician
    Staffing_Agency --> [*]
```
