...
```mermaid
erDiagram
    SalesQuota ||--o{ Pipeline : "targets"
    Pipeline ||--o{ LeadAssignment : "fed by"
    SupportTicket ||--|| ServiceLevelAgreement : "governed by"
    PerformanceScorecard ||--o{ SalesQuota : "measured against"
    Pipeline ||--o{ PerformanceScorecard : "tracked in"
    LeadAssignment ||--o{ Pipeline : "converts into"
```
...
stateDiagram-v2
    [*] --> QuotaSet
    QuotaSet --> LeadsDistributed : leadsAssigned
    LeadsDistributed --> PipelineActive : pipelineReviewed
    PipelineActive --> Coached : representativeCoached
    Coached --> PipelineActive : pipelineReviewed
    PipelineActive --> TicketEscalated : ticketEscalated
    TicketEscalated --> SLABreach : slaBreached
    SLABreach --> PipelineActive : ticketEscalated
    PipelineActive --> ScorecardReady : scorecardGenerated
    ScorecardReady --> QuotaSet : quotaSet
    ScorecardReady --> [*]
```
