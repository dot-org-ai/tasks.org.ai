...
```mermaid
erDiagram
    Sprint ||--o{ ProjectAllocation : "staffed by"
    Sprint ||--o{ TechnicalReview : "includes"
    ProjectAllocation ||--o{ Certification : "requires"
    OnCallRotation ||--o{ Incident : "responds to"
    Incident ||--o{ TechnicalReview : "investigated in"
    Certification ||--|| ProjectAllocation : "qualifies for"
```
...
stateDiagram-v2
    [*] --> SprintPlanned
    SprintPlanned --> Allocated : projectAllocationChanged
    Allocated --> UnderReview : technicalReviewCompleted
    UnderReview --> OnCall : onCallRotationUpdated
    OnCall --> IncidentResponse : incidentEscalated
    IncidentResponse --> UnderReview : technicalReviewCompleted
    OnCall --> Evaluated : technicalPerformanceEvaluated
    Evaluated --> CertExpiring : certificationExpiring
    CertExpiring --> SprintPlanned : sprintPlanned
    Evaluated --> [*]
```
