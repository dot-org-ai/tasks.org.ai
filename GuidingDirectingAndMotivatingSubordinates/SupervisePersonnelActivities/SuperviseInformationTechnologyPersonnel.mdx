...
```mermaid
erDiagram
    ITStaff ||--|| BusinessStakeholder : follows
    ITStaff }o--|| Vendor : generates
    ITStaff ||--|{ SecurityTeam : requires
    ITStaff ||--o{ EndUser : tracks
    ITStaff ||--|| ComplianceAuditor : validates
    BusinessStakeholder ||--|{ ITStaff : creates
    Vendor ||--o{ ITStaff : documents
    SecurityTeam ||--|| ITStaff : supports
```
...
stateDiagram-v2
    [*] --> I_T_Staff
    I_T_Staff --> Business_Stakeholder: BusinessStakeholder
    Business_Stakeholder --> Vendor: Vendor
    Vendor --> Security_Team: SecurityTeam
    Security_Team --> End_User: EndUser
    End_User --> Compliance_Auditor: ComplianceAuditor
    Security_Team --> Business_Stakeholder: Vendor
    Compliance_Auditor --> [*]
```
