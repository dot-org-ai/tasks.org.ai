...
```mermaid
erDiagram
    ShiftRoster ||--o{ WorkOrder : "assigns"
    ServiceStandard ||--o{ WorkOrder : "governs"
    WorkOrder ||--o{ CustomerComplaint : "may trigger"
    InspectionReport ||--|| ServiceStandard : "verifies"
    SupplyInventory ||--o{ WorkOrder : "supplies"
    CustomerComplaint ||--o{ InspectionReport : "investigated in"
```
...
stateDiagram-v2
    [*] --> Briefed
    Briefed --> ShiftActive : shiftAssigned
    ShiftActive --> AreaInspected : workAreaInspected
    AreaInspected --> Monitoring : serviceQualityReviewed
    Monitoring --> Escalated : escalationHandled
    Escalated --> IncidentLogged : incidentDocumented
    IncidentLogged --> Monitoring : serviceQualityReviewed
    Monitoring --> SuppliesOrdered : supplyOrderApproved
    SuppliesOrdered --> Monitoring : briefingConducted
    Monitoring --> ShiftEnd : shiftAssigned
    ShiftEnd --> [*]
```
