...
```mermaid
erDiagram
    InternalDepartment ||--|| ExternalClient : follows
    InternalDepartment }o--|| ITSupport : generates
    InternalDepartment ||--|{ HumanResources : requires
    InternalDepartment ||--o{ OfficeSupplyVendor : tracks
    InternalDepartment ||--|| BuildingManagement : validates
    ExternalClient ||--|{ InternalDepartment : creates
    ITSupport ||--o{ InternalDepartment : documents
    HumanResources ||--|| InternalDepartment : supports
```
...
stateDiagram-v2
    [*] --> Internal_Department
    Internal_Department --> External_Client: ExternalClient
    External_Client --> I_T_Support: ITSupport
    I_T_Support --> Human_Resources: HumanResources
    Human_Resources --> Office_Supply_Vendor: OfficeSupplyVendor
    Office_Supply_Vendor --> Building_Management: BuildingManagement
    Human_Resources --> External_Client: ITSupport
    Building_Management --> [*]
```
