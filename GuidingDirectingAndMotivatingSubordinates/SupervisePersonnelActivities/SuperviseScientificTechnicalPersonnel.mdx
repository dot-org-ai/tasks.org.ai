...
```mermaid
erDiagram
    ResearchProject ||--o{ Experiment : "conducts"
    Experiment ||--|| LabProtocol : "follows"
    ResearchProject ||--|| GrantBudget : "funded by"
    Experiment ||--o{ Publication : "reported in"
    LabSafetyRecord ||--|| Experiment : "governs"
    GrantBudget ||--o{ Publication : "supports"
```
...
stateDiagram-v2
    [*] --> TaskAssigned
    TaskAssigned --> InExperiment : researchTaskAssigned
    InExperiment --> ExperimentDone : experimentCompleted
    ExperimentDone --> ResultsReviewed : resultsReviewed
    ResultsReviewed --> InExperiment : protocolApproved
    ResultsReviewed --> PublicationReady : publicationSubmitted
    PublicationReady --> MilestoneReached : grantMilestoneReached
    InExperiment --> SafetyAudited : safetyAuditCompleted
    SafetyAudited --> InExperiment : safetyAuditCompleted
    MilestoneReached --> TaskAssigned : researchTaskAssigned
    MilestoneReached --> [*]
```
