...
```mermaid
erDiagram
    TrainingProgram ||--o{ TrainingSession : "delivers"
    TrainingProgram ||--|| Curriculum : "follows"
    TrainingSession ||--o{ Enrollment : "registers"
    Enrollment ||--|| CompletionRecord : "produces"
    TrainingMaterial ||--o{ TrainingSession : "supports"
    Curriculum ||--o{ TrainingMaterial : "contains"
```
...
stateDiagram-v2
    [*] --> ProgramPlanned
    ProgramPlanned --> SessionScheduled : sessionScheduled
    SessionScheduled --> InstructorAssigned : instructorAssigned
    InstructorAssigned --> ParticipantsEnrolled : participantsEnrolled
    ParticipantsEnrolled --> ResourcesProvided : resourcesProvided
    ResourcesProvided --> AttendanceTracked : attendanceTracked
    AttendanceTracked --> OutcomesAssessed : outcomesAssessed
    OutcomesAssessed --> SessionScheduled : sessionScheduled
    OutcomesAssessed --> [*]
```
