...
```mermaid
erDiagram
    FundAccount ||--o{ Transaction : "records"
    FinancialStatement ||--o{ FundAccount : "reports-on"
    CashFlowForecast ||--|| FundAccount : "projects"
    Transaction ||--|| ComplianceReport : "documents"
    RevenueStream ||--o{ FundAccount : "deposits-to"
    ComplianceReport ||--|| FinancialStatement : "validates"
```
...
stateDiagram-v2
    [*] --> TransactionRecorded
    TransactionRecorded --> AccountsReconciled : accountsReconciled
    AccountsReconciled --> StatementPrepared : financialStatementPrepared
    StatementPrepared --> CashFlowForecasted : cashFlowForecasted
    CashFlowForecasted --> FundsAllocated : programFundsAllocated
    FundsAllocated --> DisbursementApproved : disbursementApproved
    DisbursementApproved --> ComplianceSubmitted : complianceReportSubmitted
    ComplianceSubmitted --> TransactionRecorded : transactionRecorded
    ComplianceSubmitted --> [*]
```
