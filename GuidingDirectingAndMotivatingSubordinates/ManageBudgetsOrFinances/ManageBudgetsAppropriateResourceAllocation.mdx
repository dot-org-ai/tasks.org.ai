...
```mermaid
erDiagram
    ResourcePool ||--o{ AllocationPlan : "distributes"
    AllocationPlan ||--o{ FundingRequest : "addresses"
    PriorityMatrix ||--o{ FundingRequest : "ranks"
    AllocationScenario ||--|| AllocationPlan : "models"
    AllocationPlan ||--|| UtilizationReport : "measures"
    FundingRequest ||--|| PriorityMatrix : "evaluated-by"
```
...
stateDiagram-v2
    [*] --> RequestsCollected
    RequestsCollected --> RequestsPrioritized : requestsPrioritized
    RequestsPrioritized --> ScenariosModeled : allocationScenariosModeled
    ScenariosModeled --> PlanApproved : allocationPlanApproved
    PlanApproved --> FundsDistributed : fundsDistributed
    FundsDistributed --> UtilizationReviewed : utilizationReviewed
    UtilizationReviewed --> AllocationsRebalanced : allocationsRebalanced
    AllocationsRebalanced --> UtilizationReviewed : utilizationReviewed
    UtilizationReviewed --> [*]
```
