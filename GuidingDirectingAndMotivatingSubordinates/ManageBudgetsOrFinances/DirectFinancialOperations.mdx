...
```mermaid
erDiagram
    Invoice ||--o{ PaymentBatch : "groups-into"
    PayrollRun ||--|| GeneralLedger : "posts-to"
    PaymentBatch ||--|| BankReconciliation : "reconciles"
    GeneralLedger ||--|| CashPosition : "summarizes"
    BankReconciliation ||--|| GeneralLedger : "verifies"
    Invoice ||--|| GeneralLedger : "records"
```
...
stateDiagram-v2
    [*] --> InvoicesProcessed
    InvoicesProcessed --> PaymentBatchExecuted : paymentBatchExecuted
    PaymentBatchExecuted --> PayrollCompleted : payrollCompleted
    PayrollCompleted --> BankReconciled : bankReconciliationCompleted
    BankReconciled --> CashPositionUpdated : cashPositionUpdated
    CashPositionUpdated --> ControlViolationDetected : internalControlViolationDetected
    ControlViolationDetected --> InvoicesProcessed : invoicesProcessed
    CashPositionUpdated --> PeriodClosed : accountingPeriodClosed
    PeriodClosed --> [*]
```
