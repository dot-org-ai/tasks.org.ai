...
```mermaid
erDiagram
    Auditor ||--o{ ConsultingFirm : includes
    ConsultingFirm ||--o{ TechnologyVendor : includes
    TechnologyVendor ||--o{ RegulatoryBody : includes
```
...
stateDiagram-v2
    [*] --> Initiated
    Initiated --> Auditor: Auditor
    Auditor --> ConsultingFirm: ConsultingFirm
    ConsultingFirm --> TechnologyVendor: TechnologyVendor
    TechnologyVendor --> ValidationCheck: needs review
    ValidationCheck --> ConsultingFirm: corrections needed
    ValidationCheck --> RegulatoryBody: approved
    ValidationCheck --> RegulatoryBody: RegulatoryBody
    RegulatoryBody --> Customer: Customer
    Customer --> BoardOfDirectors: BoardOfDirectors
    BoardOfDirectors --> [*]
```
