...
```mermaid
erDiagram
    SensorTelemetry ||--o{ PredictiveAlert : "triggers"
    PredictiveAlert ||--|| ComponentLifecycle : "evaluates"
    GenerationForecast ||--|| SensorTelemetry : "informs"
    EnvironmentalPermit ||--o{ GridCurtailment : "constrains"
    RenewableEnergyCredit ||--|| GenerationForecast : "validates"
    ComponentLifecycle ||--o{ SensorTelemetry : "monitors"
```
...
stateDiagram-v2
    [*] --> Monitoring
    Monitoring --> Analyzed : telemetryAnalyzed
    Analyzed --> MaintenancePlanned : predictiveMaintenanceScheduled
    Analyzed --> Optimized : energyOutputOptimized
    MaintenancePlanned --> OutageCoordinated : gridOutageCoordinated
    OutageCoordinated --> Investigated : underperformanceInvestigated
    Investigated --> Monitoring : telemetryAnalyzed
    Optimized --> ComplianceChecked : environmentalComplianceMonitored
    ComplianceChecked --> Certified : renewableGenerationCertified
    Certified --> [*]
```
