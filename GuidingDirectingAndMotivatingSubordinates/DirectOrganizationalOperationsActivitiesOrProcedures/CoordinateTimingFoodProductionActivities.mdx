...
```mermaid
erDiagram
    ProductionSchedule ||--o{ BatchRun : "sequences"
    OrderTicket ||--o{ PrepTask : "requires"
    BatchRun ||--|| CookingTimer : "tracks"
    OrderTicket ||--|| ExpeditingQueue : "enters"
    ExpeditingQueue ||--o| HoldingLog : "records"
```
...
stateDiagram-v2
    [*] --> Scheduled
    Scheduled --> Sequenced : orderSequenced
    Sequenced --> BatchInProgress : batchStarted
    BatchInProgress --> CookingComplete : cookingTimeElapsed
    CookingComplete --> Expedited : orderExpedited
    Expedited --> Holding : holdingTimeExceeded
    Holding --> Scheduled : timingAdjusted
    BatchInProgress --> Scheduled : timingAdjusted
    Expedited --> [*]
```
