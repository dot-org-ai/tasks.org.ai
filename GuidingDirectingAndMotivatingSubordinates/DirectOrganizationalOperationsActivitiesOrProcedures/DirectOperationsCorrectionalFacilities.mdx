...
```mermaid
erDiagram
    Inmate ||--|| CustodyClassification : "assessed-by"
    Inmate ||--|| HousingAssignment : "placed-in"
    Inmate ||--o{ RehabilitationProgram : "enrolled-in"
    Inmate ||--o{ IncidentReport : "involved-in"
    ReleaseOrder ||--|| Inmate : "authorizes"
    FacilityInspection ||--o{ HousingAssignment : "reviews"
    VisitationSchedule ||--|| Inmate : "grants"
```
...
stateDiagram-v2
    [*] --> Intake
    Intake --> Classified : inmateClassified
    Classified --> Housed : housingAssigned
    Housed --> ProgramEnrolled : programEnrolled
    ProgramEnrolled --> HeadcountVerified : headcountCompleted
    HeadcountVerified --> IncidentLogged : incidentReported
    IncidentLogged --> Housed : housingAssigned
    HeadcountVerified --> ReleaseReady : releaseProcessed
    ReleaseReady --> Inspected : inspectionCompleted
    Inspected --> [*]
```
