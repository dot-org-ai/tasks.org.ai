...
```mermaid
erDiagram
    ServiceArrangement ||--|| Decedent : "honors"
    ServiceArrangement ||--|| DeathCertificate : "requires"
    DeathCertificate ||--|| BurialPermit : "authorizes"
    ServiceArrangement ||--|| CasketSelection : "includes"
    ServiceArrangement ||--|| ServiceSchedule : "follows"
    PreNeedContract ||--o| ServiceArrangement : "funds"
    TransportOrder ||--|| ServiceArrangement : "supports"
```
...
stateDiagram-v2
    [*] --> ArrangementCreated
    ArrangementCreated --> Scheduled : serviceScheduled
    Scheduled --> RemainsPrepared : remainsPrepared
    RemainsPrepared --> CertificateFiled : deathCertificateFiled
    CertificateFiled --> PermitObtained : burialPermitObtained
    PermitObtained --> Transported : transportCompleted
    Transported --> ServiceConducted : serviceScheduled
    ServiceConducted --> PaymentSettled : paymentProcessed
    PaymentSettled --> [*]
```
