...
```mermaid
erDiagram
    DietOrder ||--|| AllergenProfile : "references"
    DietOrder ||--|| ModifiedMenu : "produces"
    ModifiedMenu ||--o{ IngredientSubstitution : "applies"
    ModifiedMenu ||--|| NutritionalAnalysis : "validates"
    PreparationProtocol ||--|| ModifiedMenu : "governs"
    MealDeliveryRecord ||--|| DietOrder : "fulfills"
```
...
stateDiagram-v2
    [*] --> OrderReceived
    OrderReceived --> MenuDeveloped : modifiedMenuDeveloped
    MenuDeveloped --> AllergenChecked : allergenAlertTriggered
    AllergenChecked --> SubstitutionApproved : substitutionApproved
    SubstitutionApproved --> MenuDeveloped : modifiedMenuDeveloped
    MenuDeveloped --> NutritionAnalyzed : nutritionAnalysisCompleted
    NutritionAnalyzed --> Delivered : mealDeliveryVerified
    Delivered --> ComplianceAudited : preparationComplianceAudited
    ComplianceAudited --> OrderReceived : dietOrderProcessed
    ComplianceAudited --> [*]
```
