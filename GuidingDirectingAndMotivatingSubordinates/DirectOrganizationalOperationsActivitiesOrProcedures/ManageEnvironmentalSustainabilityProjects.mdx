...
```mermaid
erDiagram
    SustainabilityProject ||--o{ SustainabilityTarget : "pursues"
    SustainabilityProject ||--|| CarbonFootprint : "measures"
    SustainabilityProject ||--o{ ImpactReport : "produces"
    CertificationRecord ||--|| SustainabilityProject : "certifies"
    WasteReductionPlan ||--|| SustainabilityTarget : "supports"
    EnergyAudit ||--|| CarbonFootprint : "informs"
    ESGReport ||--o{ ImpactReport : "consolidates"
```
...
stateDiagram-v2
    [*] --> TargetDefined
    TargetDefined --> Audited : auditCompleted
    Audited --> ProjectLaunched : projectLaunched
    ProjectLaunched --> FootprintMeasured : footprintMeasured
    FootprintMeasured --> TargetAchieved : targetAchieved
    FootprintMeasured --> WasteTracking : footprintMeasured
    WasteTracking --> FootprintMeasured : footprintMeasured
    TargetAchieved --> CertificationFiled : certificationSubmitted
    CertificationFiled --> ESGPublished : esgReportPublished
    ESGPublished --> TargetDefined : targetDefined
    ESGPublished --> [*]
```
