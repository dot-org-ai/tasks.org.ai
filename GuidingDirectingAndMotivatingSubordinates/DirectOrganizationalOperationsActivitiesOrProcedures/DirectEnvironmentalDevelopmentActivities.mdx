...
```mermaid
erDiagram
    EnvironmentalProject ||--o{ ImpactAssessment : "requires"
    ImpactAssessment ||--|| RemediationPlan : "produces"
    EnvironmentalProject ||--o{ Permit : "authorizes"
    RemediationPlan ||--o{ MitigationMeasure : "defines"
    EcologicalSurvey ||--|| ImpactAssessment : "informs"
    EnvironmentalProject ||--o{ ComplianceReport : "generates"
```
...
stateDiagram-v2
    [*] --> Assessing
    Assessing --> RemediationApproved : remediationApproved
    RemediationApproved --> FieldDeployed : fieldTeamDeployed
    FieldDeployed --> Monitoring : ecologicalTargetMet
    FieldDeployed --> ViolationDetected : violationDetected
    ViolationDetected --> Assessing : assessmentInitiated
    Monitoring --> Reported : complianceReportSubmitted
    Monitoring --> PermitRenewal : permitExpiring
    PermitRenewal --> Monitoring : complianceReportSubmitted
    Reported --> [*]
```
