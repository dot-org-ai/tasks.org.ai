...
```mermaid
erDiagram
    HealthProgram ||--o{ CarePathway : "defines"
    HealthProgram ||--o{ PatientEnrollment : "enrolls"
    CarePathway ||--o{ QualityMetric : "measures"
    HealthProgram ||--o{ StaffCredential : "requires"
    HealthProgram ||--|| AccreditationRecord : "achieves"
    QualityMetric ||--o{ OutcomeReport : "produces"
    ComplianceReport ||--|| HealthProgram : "validates"
```
...
stateDiagram-v2
    [*] --> Designed
    Designed --> Staffed : clinicalStaffAssigned
    Staffed --> Enrolling : patientEnrolled
    Enrolling --> QualityTracking : qualityMetricTracked
    QualityTracking --> OutcomesEvaluated : outcomesEvaluated
    QualityTracking --> Enrolling : patientEnrolled
    OutcomesEvaluated --> ComplianceFiled : complianceReportSubmitted
    ComplianceFiled --> AccreditationReady : accreditationPrepared
    AccreditationReady --> Designed : programDesigned
    AccreditationReady --> [*]
```
