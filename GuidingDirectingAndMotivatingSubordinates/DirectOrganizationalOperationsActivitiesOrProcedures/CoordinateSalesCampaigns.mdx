...
```mermaid
erDiagram
    Campaign ||--o{ Territory : assigns
    Campaign ||--o{ Prospect : targets
    Campaign ||--o{ CampaignAsset : uses
    Campaign ||--o{ PerformanceMetric : measures
    Campaign ||--o{ Incentive : motivates
    Campaign ||--o{ CampaignBudget : allocates
```
...
stateDiagram-v2
    [*] --> CampaignDesigned
    CampaignDesigned --> TerritoriesAssigned : territoriesAssigned
    TerritoriesAssigned --> CampaignLaunched : campaignLaunched
    CampaignLaunched --> EngagementTracked : engagementTracked
    EngagementTracked --> RoiCalculated : roiCalculated
    RoiCalculated --> CampaignClosed : campaignClosed
    EngagementTracked --> StrategyAdjusted : strategyAdjusted
    StrategyAdjusted --> EngagementTracked : engagementTracked
    CampaignClosed --> [*]
```
