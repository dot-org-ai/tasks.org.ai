...
```mermaid
erDiagram
    ReceivingOrder ||--|| DockAppointment : "schedules"
    ReceivingOrder ||--o{ StorageLocation : "assigns"
    PickList ||--o{ StorageLocation : "retrieves-from"
    LoadPlan ||--|| HandlingEquipment : "utilizes"
    PickList ||--|| LoadPlan : "stages"
    HazmatRecord ||--o{ ReceivingOrder : "classifies"
```
...
stateDiagram-v2
    [*] --> DockScheduled
    DockScheduled --> Received : shipmentReceived
    Received --> Stored : storageLocationAssigned
    Stored --> Picked : pickListGenerated
    Picked --> Loaded : loadingCompleted
    Loaded --> Delayed : materialFlowDelayed
    Delayed --> Picked : pickListGenerated
    Loaded --> HazmatChecked : hazmatViolationDetected
    HazmatChecked --> Stored : storageLocationAssigned
    Loaded --> [*]
```
