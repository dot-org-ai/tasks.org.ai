...
```mermaid
erDiagram
    SeasonSchedule ||--o{ Production : "programs"
    Production ||--o{ TalentContract : "engages"
    Production ||--|| TechnicalRider : "specifies"
    Production ||--o{ RehearsalSchedule : "prepares"
    Production ||--|| TicketInventory : "sells"
    SponsorshipAgreement ||--o{ SeasonSchedule : "funds"
```
...
stateDiagram-v2
    [*] --> SeasonPlanned
    SeasonPlanned --> ProductionScheduled : productionScheduled
    ProductionScheduled --> TalentEngaged : talentEngaged
    TalentEngaged --> InRehearsal : rehearsalsScheduled
    InRehearsal --> TechReady : technicalCoordinated
    TechReady --> OnSale : ticketSalesOpened
    OnSale --> Performing : attendanceTracked
    Performing --> SeasonPlanned : seasonPlanned
    Performing --> [*]
```
