...
```mermaid
erDiagram
    DeliveryOrder ||--o{ DeliveryRoute : assignedTo
    DeliveryOrder ||--o{ CarrierContract : fulfilledBy
    DeliveryRoute ||--o{ ShipmentTracking : monitors
    DeliveryOrder ||--o{ ProofOfDelivery : confirmedBy
    DeliveryOrder ||--o{ DeliveryException : flaggedBy
    DeliveryOrder ||--o{ ServiceAppointment : schedules
```
...
stateDiagram-v2
    [*] --> DeliveryScheduled
    DeliveryScheduled --> CarrierSelected : carrierSelected
    CarrierSelected --> RouteOptimized : routeOptimized
    RouteOptimized --> DeliveryDispatched : deliveryDispatched
    DeliveryDispatched --> ShipmentInTransit : shipmentInTransit
    ShipmentInTransit --> DeliveryConfirmed : deliveryConfirmed
    ShipmentInTransit --> DeliveryExceptionRaised : deliveryExceptionRaised
    DeliveryExceptionRaised --> DeliveryScheduled : deliveryScheduled
    DeliveryConfirmed --> [*]
```
