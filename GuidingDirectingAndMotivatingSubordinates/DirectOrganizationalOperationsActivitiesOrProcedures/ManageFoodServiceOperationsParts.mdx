...
```mermaid
erDiagram
    FoodDistributor ||--o{ HealthDepartment : includes
    HealthDepartment ||--o{ Guest : includes
    Guest }o--|| FranchiseOrganization : "relates to"
    Guest }o--|| ReviewPlatform : "relates to"
    FranchiseOrganization }o--|| ReviewPlatform : "relates to"
```
...
stateDiagram-v2
    [*] --> Initiated
    Initiated --> FoodDistributor: FoodDistributor
    FoodDistributor --> HealthDepartment: HealthDepartment
    HealthDepartment --> Guest: Guest
    Guest --> ValidationCheck: needs review
    ValidationCheck --> HealthDepartment: corrections needed
    ValidationCheck --> FranchiseOrganization: approved
    ValidationCheck --> FranchiseOrganization: FranchiseOrganization
    FranchiseOrganization --> ReviewPlatform: ReviewPlatform
    ReviewPlatform --> [*]
```
