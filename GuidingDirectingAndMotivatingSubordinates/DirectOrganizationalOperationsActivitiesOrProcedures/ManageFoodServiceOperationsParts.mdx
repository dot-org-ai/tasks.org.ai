...
```mermaid
erDiagram
    Menu ||--o{ RecipeCard : "standardizes"
    Menu ||--o{ SupplierOrder : "triggers"
    SupplierOrder ||--|| FoodCostReport : "feeds"
    HealthInspection ||--|| ServiceStandard : "validates"
    WasteLog ||--|| FoodCostReport : "impacts"
    DailySalesReport ||--|| Menu : "evaluates"
```
...
stateDiagram-v2
    [*] --> MenuUpdated
    MenuUpdated --> SuppliesOrdered : supplierOrderPlaced
    SuppliesOrdered --> WasteMonitored : wasteThresholdExceeded
    WasteMonitored --> CostTracked : foodCostTracked
    CostTracked --> QualityReviewed : serviceQualityReviewed
    CostTracked --> MenuUpdated : menuUpdated
    QualityReviewed --> HealthChecked : healthCheckCompleted
    HealthChecked --> SalesReported : salesReportGenerated
    SalesReported --> MenuUpdated : menuUpdated
    SalesReported --> [*]
```
