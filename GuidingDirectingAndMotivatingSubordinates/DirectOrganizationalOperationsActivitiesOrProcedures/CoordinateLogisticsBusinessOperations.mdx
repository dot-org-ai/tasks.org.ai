...
erDiagram
    ...
    Shipment ||--o{ BillOfLading : documents
    Shipment ||--o{ CarrierContract : fulfilledBy
    WarehouseOrder ||--o{ InventoryPosition : adjusts
    Shipment ||--o{ DeliverySchedule : targets
    Shipment ||--o{ FreightInvoice : billed
```
...
stateDiagram-v2
    [*] --> ShipmentPlanned
    ShipmentPlanned --> CarrierAssigned : carrierAssigned
    CarrierAssigned --> ShipmentDispatched : shipmentDispatched
    ShipmentDispatched --> CustomsCleared : customsCleared
    CustomsCleared --> DeliveryCompleted : deliveryCompleted
    DeliveryCompleted --> FreightReconciled : freightReconciled
    ShipmentDispatched --> DeliveryDelayed : deliveryDelayed
    DeliveryDelayed --> ShipmentDispatched : shipmentDispatched
    FreightReconciled --> [*]
```
