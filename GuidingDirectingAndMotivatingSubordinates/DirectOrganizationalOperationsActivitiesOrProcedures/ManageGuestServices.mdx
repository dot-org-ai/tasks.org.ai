...
```mermaid
erDiagram
    Reservation ||--|| RoomAssignment : "allocates"
    Reservation ||--|| GuestProfile : "references"
    GuestProfile ||--|| LoyaltyAccount : "tracks"
    RoomAssignment ||--o{ ServiceRequest : "generates"
    ServiceRequest ||--o| Incident : "escalates-to"
    HousekeepingTask ||--|| RoomAssignment : "prepares"
    SatisfactionScore ||--|| GuestProfile : "evaluates"
```
...
stateDiagram-v2
    [*] --> Reserved
    Reserved --> RoomAssigned : roomAssigned
    RoomAssigned --> CheckedIn : guestCheckedIn
    CheckedIn --> RequestFulfilled : requestFulfilled
    RequestFulfilled --> IncidentResolved : incidentResolved
    IncidentResolved --> RequestFulfilled : requestFulfilled
    RequestFulfilled --> CheckedOut : guestCheckedOut
    CheckedOut --> FeedbackReceived : feedbackCollected
    FeedbackReceived --> LoyaltyUpdated : loyaltyUpdated
    LoyaltyUpdated --> [*]
```
