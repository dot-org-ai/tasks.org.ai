...
```mermaid
erDiagram
    ProductionOrder ||--|| ProductionLine : "runs-on"
    ProductionOrder ||--|| BillOfMaterials : "consumes"
    ProductionLine ||--o{ QualityInspection : "undergoes"
    QualityInspection ||--o| DefectReport : "generates"
    ProductionLine ||--o{ OutputMetric : "produces"
    ProductionLine ||--o| MaintenanceWindow : "schedules"
    ProductionOrder ||--o{ WorkInProgress : "tracks"
```
...
stateDiagram-v2
    [*] --> Scheduled
    Scheduled --> MaterialsReady : materialsAllocated
    MaterialsReady --> InProduction : lineOutputRecorded
    InProduction --> Inspected : qualityInspectionCompleted
    Inspected --> DefectFound : defectReported
    DefectFound --> PlanAdjusted : productionPlanAdjusted
    PlanAdjusted --> InProduction : lineOutputRecorded
    Inspected --> Optimized : throughputOptimized
    Optimized --> Scheduled : productionRunScheduled
    Optimized --> [*]
```
