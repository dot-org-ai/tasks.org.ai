...
```mermaid
erDiagram
    ShiftRoster ||--o{ CrewAssignment : contains
    CrewAssignment ||--o{ SkillMatrix : matchedBy
    CrewAssignment ||--o{ ProductivityMetric : measures
    ShiftRoster ||--o{ OvertimeRequest : triggers
    SkillMatrix ||--o{ CrossTrainingPlan : addresses
    LaborForecast ||--o{ ShiftRoster : informs
```
...
stateDiagram-v2
    [*] --> LaborForecasted
    LaborForecasted --> ShiftRosterBuilt : shiftRosterBuilt
    ShiftRosterBuilt --> CrewAssigned : crewAssigned
    CrewAssigned --> ProductivityTracked : productivityTracked
    ProductivityTracked --> OvertimeApproved : overtimeApproved
    OvertimeApproved --> CrewAssigned : crewAssigned
    ProductivityTracked --> SkillMatrixUpdated : skillMatrixUpdated
    SkillMatrixUpdated --> CrossTrainingCompleted : crossTrainingCompleted
    CrossTrainingCompleted --> [*]
```
