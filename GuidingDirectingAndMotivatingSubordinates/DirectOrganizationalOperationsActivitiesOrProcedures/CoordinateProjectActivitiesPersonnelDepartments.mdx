...
```mermaid
erDiagram
    ProjectPlan ||--o{ Milestone : defines
    ProjectPlan ||--o{ CrossTeamDependency : identifies
    ProjectPlan ||--o{ ResourceAllocation : assigns
    ProjectPlan ||--o{ StatusReport : publishes
    ProjectPlan ||--o{ RiskRegister : assesses
    Milestone ||--o{ ChangeRequest : modifiedBy
```
...
stateDiagram-v2
    [*] --> DeliverablesAligned
    DeliverablesAligned --> ResourcesAllocated : resourcesAllocated
    ResourcesAllocated --> DependencyCompleted : dependencyCompleted
    DependencyCompleted --> MilestoneReviewed : milestoneReviewed
    MilestoneReviewed --> StatusReportPublished : statusReportPublished
    StatusReportPublished --> ScopeChangeApproved : scopeChangeApproved
    ScopeChangeApproved --> DeliverablesAligned : deliverablesAligned
    DependencyCompleted --> BlockerEscalated : blockerEscalated
    BlockerEscalated --> ResourcesAllocated : resourcesAllocated
    StatusReportPublished --> [*]
```
