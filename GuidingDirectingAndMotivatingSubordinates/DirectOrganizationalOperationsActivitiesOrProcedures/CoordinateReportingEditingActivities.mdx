...
```mermaid
erDiagram
    EditorialCalendar ||--o{ StoryAssignment : plans
    StoryAssignment ||--o{ DraftManuscript : produces
    DraftManuscript ||--o{ EditorialReview : receives
    DraftManuscript ||--o{ FactCheckRecord : verifiedBy
    EditorialCalendar ||--o{ PublicationSchedule : releases
    DraftManuscript ||--o{ StyleGuide : conformsTo
```
...
```mermaid
stateDiagram-v2
    [*] --> EditorialCalendarUpdated
    EditorialCalendarUpdated --> StoryAssigned : storyAssigned
    StoryAssigned --> DraftSubmitted : draftSubmitted
    DraftSubmitted --> EditorialReviewCompleted : editorialReviewCompleted
    EditorialReviewCompleted --> FactCheckPassed : factCheckPassed
    FactCheckPassed --> PublicationScheduled : publicationScheduled
    EditorialReviewCompleted --> DeadlineMissed : deadlineMissed
    DeadlineMissed --> StoryAssigned : storyAssigned
