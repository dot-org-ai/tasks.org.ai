...
```mermaid
erDiagram
    AdminTask ||--|| TaskQueue : "queued-in"
    AdminTask ||--o{ DocumentRecord : "produces"
    DocumentRecord ||--|| FilingSystem : "stored-in"
    DocumentRecord ||--|| RetentionPolicy : "governed-by"
    CorrespondenceLog ||--o{ AdminTask : "triggers"
    WorkloadReport ||--o{ TaskQueue : "summarizes"
```
...
stateDiagram-v2
    [*] --> TaskAssigned
    TaskAssigned --> DocumentProcessed : documentProcessed
    DocumentProcessed --> CorrespondenceHandled : correspondenceHandled
    CorrespondenceHandled --> Filed : filingSystemUpdated
    Filed --> Completed : taskCompleted
    Completed --> Rebalanced : workloadRebalanced
    Rebalanced --> TaskAssigned : adminTaskAssigned
    Filed --> RetentionApplied : retentionEnforced
    RetentionApplied --> [*]
```
