...
```mermaid
erDiagram
    ServiceRequest ||--|| VendorContract : "fulfilled-by"
    VendorContract ||--|| ServiceLevelAgreement : "governs"
    OfficeSpace ||--o{ MeetingRoom : "contains"
    ServiceRequest ||--o{ ExpenseReport : "generates"
    PolicyDocument ||--o{ ServiceRequest : "guides"
    SupplyOrder ||--|| VendorContract : "references"
```
...
stateDiagram-v2
    [*] --> RequestReceived
    RequestReceived --> Routed : serviceRequestRouted
    Routed --> ContractAssigned : vendorContractUpdated
    ContractAssigned --> SpaceAllocated : officeSpaceAllocated
    SpaceAllocated --> UnderReview : serviceLevelsReviewed
    UnderReview --> Routed : serviceLevelsReviewed
    UnderReview --> Completed : expenseReportProcessed
    Completed --> [*]
```
