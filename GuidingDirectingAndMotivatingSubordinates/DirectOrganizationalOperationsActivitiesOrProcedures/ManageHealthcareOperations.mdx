...
```mermaid
erDiagram
    FacilityBudget ||--o{ DepartmentSchedule : "funds"
    BedCapacity ||--|| PatientVolume : "constrains"
    DepartmentSchedule ||--|| StaffingModel : "follows"
    RevenueCycleRecord ||--|| FacilityBudget : "feeds"
    PatientVolume ||--|| StaffingModel : "informs"
    RegulatoryReport ||--|| OperationalDashboard : "populates"
```
...
stateDiagram-v2
    [*] --> VolumeForecasted
    VolumeForecasted --> StaffingOptimized : staffingOptimized
    StaffingOptimized --> CapacityManaged : bedCapacityUpdated
    CapacityManaged --> DepartmentsAligned : departmentsCoordinated
    DepartmentsAligned --> RevenueCycleReviewed : revenueCycleReviewed
    RevenueCycleReviewed --> MetricsReviewed : operationalMetricsReviewed
    MetricsReviewed --> BudgetAdjusted : budgetVarianceDetected
    BudgetAdjusted --> MetricsReviewed : operationalMetricsReviewed
    MetricsReviewed --> RegulatoryFiled : regulatoryReportSubmitted
    RegulatoryFiled --> VolumeForecasted : patientVolumeForecasted
    RegulatoryFiled --> [*]
```
