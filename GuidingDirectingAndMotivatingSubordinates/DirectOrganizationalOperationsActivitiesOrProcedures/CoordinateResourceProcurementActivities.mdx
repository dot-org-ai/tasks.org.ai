...
```mermaid
erDiagram
    PurchaseRequisition ||--o{ RequestForProposal : triggers
    RequestForProposal ||--o{ VendorProposal : solicits
    VendorProposal ||--o{ ProcurementContract : awards
    ProcurementContract ||--o{ PurchaseOrder : issues
    PurchaseOrder ||--o{ VendorScorecard : evaluates
    PurchaseOrder ||--o{ SpendAnalysis : analyzedIn
```
...
stateDiagram-v2
    ...
    RequisitionSubmitted --> RfpIssued : rfpIssued
    RfpIssued --> ProposalsEvaluated : proposalsEvaluated
    ProposalsEvaluated --> ContractAwarded : contractAwarded
    ContractAwarded --> PurchaseOrderIssued : purchaseOrderIssued
    PurchaseOrderIssued --> FulfillmentCompleted : fulfillmentCompleted
    FulfillmentCompleted --> SpendThresholdExceeded : spendThresholdExceeded
    SpendThresholdExceeded --> RequisitionSubmitted : requisitionSubmitted
