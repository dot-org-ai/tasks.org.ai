...
```mermaid
erDiagram
    ShiftSchedule ||--o{ StationAssignment : allocates
    ShiftSchedule ||--o{ BreakRotation : plans
    ServicePeriod ||--o{ ShiftSchedule : staffedBy
    LaborForecast ||--o{ ShiftSchedule : informs
    StationAssignment ||--o{ PerformanceReview : evaluates
    StationAssignment ||--o{ TrainingRecord : requires
```
...
stateDiagram-v2
    [*] --> LaborForecasted
    LaborForecasted --> ShiftScheduleCreated : shiftScheduleCreated
    ShiftScheduleCreated --> StationsAssigned : stationsAssigned
    StationsAssigned --> ServiceStarted : serviceStarted
    ServiceStarted --> PacingAdjusted : pacingAdjusted
    PacingAdjusted --> BreakRotated : breakRotated
    BreakRotated --> PerformanceReviewed : performanceReviewed
    PacingAdjusted --> ServiceStarted : serviceStarted
    PerformanceReviewed --> [*]
```
