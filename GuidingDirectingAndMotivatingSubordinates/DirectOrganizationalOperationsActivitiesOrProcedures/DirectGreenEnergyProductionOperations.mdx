...
```mermaid
erDiagram
    GenerationUnit ||--o{ ProductionForecast : "predicts"
    GenerationUnit ||--|| GridConnection : "delivers-via"
    PowerPurchaseAgreement ||--|| GenerationUnit : "contracts"
    GenerationUnit ||--o{ RenewableEnergyCredit : "earns"
    WeatherData ||--|| ProductionForecast : "informs"
    CapacityFactor ||--|| GenerationUnit : "measures"
    ComplianceCertificate ||--|| GenerationUnit : "certifies"
```
...
stateDiagram-v2
    [*] --> Forecasted
    Forecasted --> Optimized : capacityOptimized
    Optimized --> Dispatched : powerDispatched
    Dispatched --> Monitoring : performanceThresholdBreached
    Monitoring --> MaintenanceScheduled : maintenanceWindowScheduled
    MaintenanceScheduled --> Dispatched : powerDispatched
    Dispatched --> Credited : renewableCreditIssued
    Credited --> Reported : complianceReported
    Reported --> Forecasted : generationForecasted
    Reported --> [*]
```
