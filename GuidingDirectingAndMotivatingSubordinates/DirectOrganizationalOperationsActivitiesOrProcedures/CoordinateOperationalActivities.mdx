...
```mermaid
erDiagram
    SupplyChainPartner ||--o{ ServiceClient : includes
    ServiceClient ||--o{ RegulatoryAuthority : includes
    RegulatoryAuthority }o--|| TechnologyProvider : "relates to"
```
...
stateDiagram-v2
    [*] --> Initiated
    Initiated --> SupplyChainPartner: SupplyChainPartner
    SupplyChainPartner --> ServiceClient: ServiceClient
    ServiceClient --> RegulatoryAuthority: RegulatoryAuthority
    RegulatoryAuthority --> TechnologyProvider: TechnologyProvider
    TechnologyProvider --> [*]
```
