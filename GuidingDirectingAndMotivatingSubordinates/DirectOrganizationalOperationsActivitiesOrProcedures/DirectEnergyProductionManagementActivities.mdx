...
```mermaid
erDiagram
    GenerationUnit ||--o{ DispatchSchedule : "executes"
    FuelContract ||--|| GenerationUnit : "supplies"
    GenerationUnit ||--o{ EmissionsRecord : "produces"
    LoadForecast ||--|| DispatchSchedule : "informs"
    GenerationUnit ||--o| OutageReport : "logs"
    EnergyTrade ||--|| DispatchSchedule : "settles"
    TransmissionConstraint ||--o{ DispatchSchedule : "limits"
```
...
stateDiagram-v2
    [*] --> Forecasted
    Forecasted --> Dispatched : generationDispatched
    Dispatched --> FuelUpdated : fuelInventoryUpdated
    FuelUpdated --> EmissionsMonitored : emissionsThresholdApproached
    EmissionsMonitored --> Traded : energyTradeExecuted
    EmissionsMonitored --> OutageScheduled : outageScheduled
    OutageScheduled --> Dispatched : generationDispatched
    Traded --> Reported : regulatoryComplianceReported
    Reported --> Forecasted : loadForecasted
    Reported --> [*]
```
