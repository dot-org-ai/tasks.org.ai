...
```mermaid
erDiagram
    FreightCarrier ||--o{ FreightForwarder : includes
    FreightForwarder ||--o{ CustomsBroker : includes
    CustomsBroker ||--o{ ReceivingCustomer : includes
```
...
stateDiagram-v2
    ...
    FreightCarrier --> FreightForwarder: FreightForwarder
    FreightForwarder --> CustomsBroker: CustomsBroker
    CustomsBroker --> ValidationCheck: needs review
    ValidationCheck --> FreightForwarder: corrections needed
    ValidationCheck --> ReceivingCustomer: approved
    ValidationCheck --> ReceivingCustomer: ReceivingCustomer
    ReceivingCustomer --> PortAuthority: PortAuthority
    PortAuthority --> [*]
```
