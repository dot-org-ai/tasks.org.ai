...
```mermaid
erDiagram
    ShipmentOrder ||--o{ CarrierBooking : "reserves"
    CarrierBooking ||--|| BillOfLading : "generates"
    ShipmentOrder ||--o{ CommercialInvoice : "requires"
    ShipmentOrder ||--|| CustomsDeclaration : "triggers"
    CarrierBooking ||--o{ TrackingRecord : "produces"
    TrackingRecord ||--o| DeliveryConfirmation : "confirms"
```
...
stateDiagram-v2
    [*] --> Booked
    Booked --> Documented : documentationPrepared
    Documented --> DeclaredForCustoms : customsDeclared
    DeclaredForCustoms --> Dispatched : shipmentDispatched
    Dispatched --> InTransit : consignmentTracked
    InTransit --> Delivered : deliveryConfirmed
    InTransit --> ExceptionRaised : exceptionRaised
    ExceptionRaised --> InTransit : consignmentTracked
    Delivered --> [*]
```
