...
```mermaid
erDiagram
    SalesPipeline ||--o{ Lead : "qualifies"
    Campaign ||--o{ Lead : "generates"
    Lead ||--o| CustomerAccount : "converts-to"
    CustomerAccount ||--o{ ServiceTicket : "submits"
    RevenueTarget ||--|| SalesPipeline : "benchmarks"
    Promotion ||--o{ Campaign : "supports"
    SatisfactionSurvey ||--|| CustomerAccount : "evaluates"
```
...
stateDiagram-v2
    [*] --> TargetsSet
    TargetsSet --> CampaignLive : campaignLaunched
    CampaignLive --> LeadsAssigned : leadAssigned
    LeadsAssigned --> PipelineReviewed : pipelineReviewed
    PipelineReviewed --> Forecasted : revenueForecasted
    PipelineReviewed --> PromotionApproved : promotionApproved
    PromotionApproved --> CampaignLive : campaignLaunched
    Forecasted --> SatisfactionMeasured : satisfactionCollected
    SatisfactionMeasured --> TicketEscalated : ticketEscalated
    TicketEscalated --> TargetsSet : salesTargetSet
