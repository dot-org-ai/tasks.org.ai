...
```mermaid
erDiagram
    Vehicle ||--o{ Route : "operates-on"
    Route ||--|| ServiceSchedule : "follows"
    Vehicle ||--|| DriverRecord : "assigned-to"
    Vehicle ||--o{ MaintenanceLog : "tracks"
    FreightManifest ||--|| Vehicle : "loaded-on"
    CapacityPlan ||--o{ Route : "forecasts"
    IncidentReport ||--o| Vehicle : "documents"
```
...
stateDiagram-v2
    [*] --> Scheduled
    Scheduled --> Dispatched : vehicleDispatched
    Dispatched --> InTransit : fleetPositionUpdated
    InTransit --> IncidentLogged : incidentReported
    IncidentLogged --> Dispatched : vehicleDispatched
    InTransit --> ComplianceChecked : driverComplianceViolation
    ComplianceChecked --> RouteOptimized : routeOptimized
    RouteOptimized --> CapacityPlanned : capacityThresholdReached
    CapacityPlanned --> Scheduled : serviceScheduled
    RouteOptimized --> [*]
```
