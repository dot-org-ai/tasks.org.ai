...
```mermaid
erDiagram
    TenantRequest ||--|| BuildingSystem : "targets"
    BuildingSystem ||--o{ InspectionSchedule : "requires"
    BuildingSystem ||--o{ EnergyConsumption : "generates"
    SpaceAllocation ||--o{ TenantRequest : "originates"
    EmergencyProtocol ||--|| BuildingSystem : "protects"
    AccessCoordination ||--o{ TenantRequest : "enables"
```
...
stateDiagram-v2
    [*] --> RequestReceived
    RequestReceived --> Routed : tenantRequestRouted
    Routed --> HVACOptimized : hvacOptimized
    Routed --> EmergencyResponse : plumbingEmergencyResponded
    EmergencyResponse --> InfrastructureManaged : electricalInfrastructureManaged
    HVACOptimized --> InspectionScheduled : liftInspectionScheduled
    InspectionScheduled --> FireSafetyReviewed : fireSafetyInspectionConducted
    FireSafetyReviewed --> Routed : tenantRequestRouted
    InfrastructureManaged --> SpaceAssigned : spaceAllocated
    SpaceAssigned --> [*]
```
