...
```mermaid
erDiagram
    GuestList ||--o{ CheckIn : tracks
    ProgramAgenda ||--o{ Session : sequences
    Session ||--o{ Feedback : collects
    ProgramAgenda ||--o{ Incident : logs
    GuestList ||--o{ ProgramAgenda : attends
```
...
stateDiagram-v2
    [*] --> GuestsWelcomed
    GuestsWelcomed --> AttendeeCheckedIn : attendeeCheckedIn
    AttendeeCheckedIn --> ProgramDelivered : programDelivered
    ProgramDelivered --> SessionTransitioned : sessionTransitioned
    SessionTransitioned --> FeedbackCollected : feedbackCollected
    FeedbackCollected --> EventClosed : eventClosed
    ProgramDelivered --> IncidentManaged : incidentManaged
    IncidentManaged --> ProgramDelivered : programDelivered
    EventClosed --> [*]
```
