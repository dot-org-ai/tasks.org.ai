...
```mermaid
erDiagram
    Activity ||--o{ Membership : enrolls
    Activity ||--o{ Schedule : calendars
    Activity ||--o{ Competition : enters
    Membership ||--o{ PermissionForm : requires
    Activity ||--o{ Budget : funds
```
...
stateDiagram-v2
    [*] --> ActivityCreated
    ActivityCreated --> BudgetAllocated : budgetAllocated
    BudgetAllocated --> StudentEnrolled : studentEnrolled
    StudentEnrolled --> PermissionObtained : permissionObtained
    PermissionObtained --> SessionScheduled : sessionScheduled
    SessionScheduled --> CompetitionPlanned : competitionPlanned
    CompetitionPlanned --> ComplianceMonitored : complianceMonitored
    ComplianceMonitored --> SessionScheduled : sessionScheduled
    ComplianceMonitored --> [*]
```
