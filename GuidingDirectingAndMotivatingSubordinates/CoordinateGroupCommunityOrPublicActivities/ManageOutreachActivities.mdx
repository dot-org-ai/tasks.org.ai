...
```mermaid
erDiagram
    OutreachProgram ||--o{ OutreachEvent : organizes
    OutreachEvent ||--o{ VolunteerShift : staffedBy
    OutreachProgram ||--o{ ImpactMetric : measures
    OutreachProgram ||--o{ CommunityContact : engages
    OutreachEvent ||--o{ CommunityContact : serves
```
...
stateDiagram-v2
    [*] --> ProgramCreated
    ProgramCreated --> EventScheduled : eventScheduled
    EventScheduled --> VolunteersRecruited : volunteersRecruited
    VolunteersRecruited --> ShiftsAssigned : shiftsAssigned
    ShiftsAssigned --> EventExecuted : eventExecuted
    EventExecuted --> ImpactMeasured : impactMeasured
    ImpactMeasured --> ReportPublished : reportPublished
    ImpactMeasured --> EventScheduled : eventScheduled
    ReportPublished --> [*]
```
