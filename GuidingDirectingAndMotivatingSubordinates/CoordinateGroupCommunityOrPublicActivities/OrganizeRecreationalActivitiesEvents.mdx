...
```mermaid
erDiagram
    Activity ||--o{ Event : schedules
    Event ||--o{ Registration : enrolls
    Event ||--o{ Venue : hostedAt
    Event ||--o{ Equipment : requires
    Registration ||--o{ Waiver : signs
```
...
stateDiagram-v2
    [*] --> ActivityCreated
    ActivityCreated --> EventScheduled : eventScheduled
    EventScheduled --> StaffAssigned : staffAssigned
    StaffAssigned --> ParticipantRegistered : participantRegistered
    ParticipantRegistered --> EventCapacityReached : eventCapacityReached
    EventCapacityReached --> EventCompleted : eventCompleted
    EventCompleted --> ActivityCreated : activityCreated
    ParticipantRegistered --> EventCancelled : eventCancelled
    EventCompleted --> [*]
```
