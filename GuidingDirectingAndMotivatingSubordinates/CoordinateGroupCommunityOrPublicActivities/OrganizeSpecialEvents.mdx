...
```mermaid
erDiagram
    Event ||--o{ Budget : allocates
    Event ||--o{ VendorContract : engages
    Event ||--o{ GuestList : invites
    Event ||--o{ Itinerary : follows
    VendorContract ||--o{ Invoice : generates
```
...
stateDiagram-v2
    [*] --> EventCreated
    EventCreated --> VenueBooked : venueBooked
    VenueBooked --> VendorContracted : vendorContracted
    VendorContracted --> GuestListFinalized : guestListFinalized
    GuestListFinalized --> ItineraryPublished : itineraryPublished
    ItineraryPublished --> EventExecuted : eventExecuted
    EventExecuted --> EventClosed : eventClosed
    VendorContracted --> VenueBooked : vendorContracted
    EventClosed --> [*]
```
