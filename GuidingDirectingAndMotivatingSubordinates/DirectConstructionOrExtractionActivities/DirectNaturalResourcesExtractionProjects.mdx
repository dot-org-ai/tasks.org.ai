...
```mermaid
erDiagram
    ExtractionSite ||--o{ ExtractionPermit : authorizedBy
    ExtractionSite ||--o{ ProductionLog : records
    ExtractionSite ||--o{ ResourceEstimate : assessedBy
    ExtractionSite ||--o{ EquipmentDeployment : operates
    ExtractionSite ||--o{ ReclamationPlan : restores
    ExtractionSite ||--o{ EnvironmentalMonitoringReport : monitors
```
...
```mermaid
stateDiagram-v2
    [*] --> ExtractionSitePlanned
    ExtractionSitePlanned --> PermitRenewalProcessed : permitRenewalProcessed
    PermitRenewalProcessed --> EquipmentDeployed : equipmentDeployed
    EquipmentDeployed --> ProductionTargetMet : productionTargetMet
    ProductionTargetMet --> ReclamationMilestoneReached : reclamationMilestoneReached
    EquipmentDeployed --> ProductionShortfall : productionShortfall
    ProductionShortfall --> EquipmentDeployed : equipmentDeployed
    EquipmentDeployed --> EnvironmentalThresholdExceeded : environmentalThresholdExceeded
    EnvironmentalThresholdExceeded --> EquipmentDeployed : equipmentDeployed
    ReclamationMilestoneReached --> [*]
```
