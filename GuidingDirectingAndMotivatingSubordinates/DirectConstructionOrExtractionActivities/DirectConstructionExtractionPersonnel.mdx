...
```mermaid
erDiagram
    LaborUnion ||--o{ SafetyRegulator : includes
    SafetyRegulator }o--|| EquipmentRentalFirm : "relates to"
    SafetyRegulator }o--|| TrainingProvider : "relates to"
    SafetyRegulator }o--|| Subcontractor : "relates to"
    EquipmentRentalFirm ||--o{ TrainingProvider : includes
    TrainingProvider }o--|| Subcontractor : "relates to"
```
...
stateDiagram-v2
    [*] --> Initiated
    Initiated --> LaborUnion: LaborUnion
    LaborUnion --> SafetyRegulator: SafetyRegulator
    SafetyRegulator --> EquipmentRentalFirm: EquipmentRentalFirm
    EquipmentRentalFirm --> ValidationCheck: needs review
    ValidationCheck --> SafetyRegulator: corrections needed
    ValidationCheck --> TrainingProvider: approved
    ValidationCheck --> TrainingProvider: TrainingProvider
    TrainingProvider --> Subcontractor: Subcontractor
    Subcontractor --> [*]
```
