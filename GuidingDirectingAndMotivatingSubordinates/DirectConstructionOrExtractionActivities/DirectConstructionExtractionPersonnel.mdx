...
```mermaid
erDiagram
    Crew ||--o{ CrewMember : comprises
    CrewMember ||--o{ TaskAssignment : receives
    CrewMember ||--o{ Certification : holds
    TaskAssignment ||--o{ DailyReport : documentedIn
    Crew ||--o{ SafetyIncident : reports
```
...
stateDiagram-v2
    ...
    CertificationVerified --> SafetyBriefingCompleted : safetyBriefingCompleted
    SafetyBriefingCompleted --> TaskAssigned : taskAssigned
    TaskAssigned --> PerformanceEvaluated : performanceEvaluated
    PerformanceEvaluated --> DailyReportSubmitted : dailyReportSubmitted
    TaskAssigned --> IncidentReported : incidentReported
    IncidentReported --> StaffingAdjusted : staffingAdjusted
    StaffingAdjusted --> TaskAssigned : taskAssigned
