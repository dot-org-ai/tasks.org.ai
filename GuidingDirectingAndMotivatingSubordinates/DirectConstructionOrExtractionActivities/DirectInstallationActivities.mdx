...
```mermaid
erDiagram
    Manufacturer ||--o{ BuildingInspector : includes
    Manufacturer ||--o{ SystemsIntegrator : contains
    BuildingInspector ||--o{ SystemsIntegrator : contains
    BuildingInspector }o--|| EndUser : "relates to"
    BuildingInspector }o--|| WarrantyProvider : "relates to"
    SystemsIntegrator ||--o{ EndUser : includes
```
...
stateDiagram-v2
    [*] --> Initiated
    Initiated --> Manufacturer: Manufacturer
    Manufacturer --> BuildingInspector: BuildingInspector
    BuildingInspector --> SystemsIntegrator: SystemsIntegrator
    SystemsIntegrator --> ValidationCheck: needs review
    ValidationCheck --> BuildingInspector: corrections needed
    ValidationCheck --> EndUser: approved
    ValidationCheck --> EndUser: EndUser
    EndUser --> WarrantyProvider: WarrantyProvider
    WarrantyProvider --> [*]
```
