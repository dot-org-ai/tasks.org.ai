...
```mermaid
erDiagram
    Project ||--o{ Phase : dividedInto
    Project ||--o{ ChangeOrder : modifiedBy
    Project ||--o{ Milestone : tracks
    Phase ||--o{ RFI : clarifies
    Phase ||--o{ PayApplication : funds
```
...
stateDiagram-v2
    [*] --> ProjectInitiated
    ProjectInitiated --> PhaseAdvanced : phaseAdvanced
    PhaseAdvanced --> RfiIssued : rfiIssued
    RfiIssued --> ChangeOrderApproved : changeOrderApproved
    ChangeOrderApproved --> MilestoneAchieved : milestoneAchieved
    MilestoneAchieved --> PayApplicationApproved : payApplicationApproved
    PayApplicationApproved --> ProjectClosed : projectClosed
    MilestoneAchieved --> PhaseAdvanced : phaseAdvanced
    ProjectClosed --> [*]
```
