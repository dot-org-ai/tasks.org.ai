...
```mermaid
erDiagram
    ConstructionProject ||--o{ Budget : allocates
    ConstructionProject ||--o{ Schedule : plans
    ConstructionProject ||--o{ RiskRegister : assesses
    ConstructionProject ||--o{ QualityPlan : enforces
    ConstructionProject ||--o{ ProgressReport : summarizes
```
...
```mermaid
stateDiagram-v2
    [*] --> ProjectControlsEstablished
    ProjectControlsEstablished --> ScheduleDelayIdentified : scheduleDelayIdentified
    ScheduleDelayIdentified --> BudgetVarianceDetected : budgetVarianceDetected
    BudgetVarianceDetected --> QualityNonconformanceFound : qualityNonconformanceFound
    QualityNonconformanceFound --> RiskMitigated : riskMitigated
    RiskMitigated --> ProgressReportPublished : progressReportPublished
    ProgressReportPublished --> ProjectClosedOut : projectClosedOut
    QualityNonconformanceFound --> ScheduleDelayIdentified : scheduleDelayIdentified
    ProjectClosedOut --> [*]
```
