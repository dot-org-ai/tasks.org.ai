...
```mermaid
erDiagram
    ProjectOwner ||--o{ DesignTeam : includes
    DesignTeam ||--o{ GeneralContractor : includes
    GeneralContractor ||--o{ SuretyCompany : includes
    SuretyCompany }o--|| InsuranceCarrier : "relates to"
    SuretyCompany }o--|| RegulatoryAuthority : "relates to"
    InsuranceCarrier }o--|| RegulatoryAuthority : "relates to"
```
...
stateDiagram-v2
    [*] --> Initiated
    Initiated --> ProjectOwner: ProjectOwner
    ProjectOwner --> DesignTeam: DesignTeam
    DesignTeam --> GeneralContractor: GeneralContractor
    GeneralContractor --> ValidationCheck: needs review
    ValidationCheck --> DesignTeam: corrections needed
    ValidationCheck --> SuretyCompany: approved
    ValidationCheck --> SuretyCompany: SuretyCompany
    SuretyCompany --> InsuranceCarrier: InsuranceCarrier
    InsuranceCarrier --> RegulatoryAuthority: RegulatoryAuthority
    RegulatoryAuthority --> [*]
```
