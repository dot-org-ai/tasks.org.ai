...
```mermaid
erDiagram
    CountSchedule ||--o{ Headcount : "triggers"
    Headcount ||--|| InmateRoster : "verified against"
    Headcount ||--o{ Discrepancy : "may produce"
    Discrepancy ||--|| Headcount : "resolved within"
    Headcount ||--|| PersonnelLog : "staffed by"
    Headcount ||--o{ CountReport : "summarized in"
```
...
stateDiagram-v2
    [*] --> Scheduled
    Scheduled --> Initiated : countInitiated
    Initiated --> Locked : lockdownActivated
    Locked --> Recording : countRecorded
    Recording --> Reconciled : countReconciled
    Recording --> DiscrepancyFlagged : discrepancyFlagged
    DiscrepancyFlagged --> Recording : discrepancyResolved
    Reconciled --> Reported : countReportSubmitted
    Reported --> [*]
```
