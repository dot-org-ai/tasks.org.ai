...
```mermaid
erDiagram
    InmateRecord ||--o{ ActivitySchedule : "follows"
    InmateRecord ||--o{ WorkAssignment : "assigned to"
    ActivitySchedule ||--o{ HeadCount : "verified by"
    InmateRecord ||--o{ IncidentReport : "documented in"
    IncidentReport ||--o{ DisciplinaryAction : "results in"
    WorkAssignment ||--|| ActivitySchedule : "scheduled within"
```
...
stateDiagram-v2
    [*] --> CountVerified
    CountVerified --> MovementAuthorized : movementAuthorized
    MovementAuthorized --> WorkAssigned : workDetailAssigned
    WorkAssigned --> ProgramScheduled : programParticipationScheduled
    ProgramScheduled --> SecurityCleared : securityCheckCompleted
    SecurityCleared --> CountVerified : headCountCompleted
    SecurityCleared --> IncidentDocumented : incidentDocumented
    IncidentDocumented --> DisciplinaryIssued : disciplinaryActionIssued
    DisciplinaryIssued --> CountVerified : headCountCompleted
    CountVerified --> [*]
```
