...
```mermaid
erDiagram
    Committee ||--o{ CommitteeMeeting : "convenes"
    CommitteeMeeting ||--o{ Proposal : "considers"
    Proposal ||--o{ Vote : "decided by"
    CommitteeMeeting ||--|| MeetingMinutes : "documented in"
    Vote ||--o{ Recommendation : "produces"
    Committee ||--o{ Recommendation : "delivers"
```
...
stateDiagram-v2
    [*] --> Chartered
    Chartered --> MemberAppointed : committeeJoined
    MemberAppointed --> InSession : meetingAttended
    InSession --> ProposalUnderReview : proposalReviewed
    ProposalUnderReview --> VotingComplete : voteCast
    VotingComplete --> InSession : feedbackContributed
    VotingComplete --> RecommendationDrafted : recommendationDrafted
    RecommendationDrafted --> Reported : reportSubmitted
    Reported --> [*]
```
