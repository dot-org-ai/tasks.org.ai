...
```mermaid
erDiagram
    ShiftRoster ||--o{ SecurityPost : "staffs"
    SecurityPost ||--o{ PatrolRoute : "patrols"
    PatrolRoute ||--o{ IncidentReport : "generates"
    AlarmEvent ||--|| IncidentReport : "documents"
    SecurityBriefing ||--|| ShiftRoster : "prepares"
    VisitorPass ||--o{ SecurityPost : "accesses"
```
...
stateDiagram-v2
    [*] --> BriefingConducted
    BriefingConducted --> PersonnelDeployed : personnelDeployed
    PersonnelDeployed --> PatrolCompleted : patrolCompleted
    PatrolCompleted --> AlarmTriggered : alarmTriggered
    AlarmTriggered --> AlarmResolved : alarmResolved
    AlarmResolved --> IncidentReported : incidentReported
    IncidentReported --> PersonnelDeployed : personnelDeployed
    PatrolCompleted --> VisitorProcessed : visitorProcessed
    VisitorProcessed --> PatrolCompleted : patrolCompleted
    IncidentReported --> [*]
```
