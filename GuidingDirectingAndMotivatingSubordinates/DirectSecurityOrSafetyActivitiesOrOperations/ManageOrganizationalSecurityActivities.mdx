...
```mermaid
erDiagram
    SecurityPolicy ||--o{ AccessCredential : "governs"
    RiskAssessment ||--o{ ThreatAlert : "identifies"
    ThreatAlert ||--|| SecurityIncident : "escalates-to"
    SurveillanceSystem ||--o{ ThreatAlert : "detects"
    SecurityIncident ||--|| ComplianceReport : "documents"
    SecurityPolicy ||--|| RiskAssessment : "informs"
```
...
stateDiagram-v2
    [*] --> RiskAssessed
    RiskAssessed --> CredentialIssued : accessCredentialIssued
    CredentialIssued --> Monitoring : securityAlertTriggered
    Monitoring --> IncidentInvestigated : incidentInvestigated
    IncidentInvestigated --> CountermeasureDeployed : countermeasureImplemented
    CountermeasureDeployed --> RiskAssessed : riskAssessmentCompleted
    CountermeasureDeployed --> ComplianceReported : complianceReported
    Monitoring --> ThreatResponseActive : threatResponseActivated
    ThreatResponseActive --> IncidentInvestigated : incidentInvestigated
    ComplianceReported --> [*]
```
