...
```mermaid
erDiagram
    FederalEmergencyAgency ||--o{ LocalGovernment : includes
    LocalGovernment ||--o{ RedCross : includes
    RedCross ||--o{ UtilityCompany : includes
    MilitaryNationalGuard }o--|| MediaOutlet : "relates to"
```
...
stateDiagram-v2
    [*] --> Initiated
    Initiated --> FederalEmergencyAgency: FederalEmergencyAgency
    FederalEmergencyAgency --> LocalGovernment: LocalGovernment
    LocalGovernment --> RedCross: RedCross
    RedCross --> ValidationCheck: needs review
    ValidationCheck --> LocalGovernment: corrections needed
    ValidationCheck --> UtilityCompany: approved
    ValidationCheck --> UtilityCompany: UtilityCompany
    UtilityCompany --> MilitaryNationalGuard: MilitaryNationalGuard
    MilitaryNationalGuard --> MediaOutlet: MediaOutlet
    MediaOutlet --> [*]
```
