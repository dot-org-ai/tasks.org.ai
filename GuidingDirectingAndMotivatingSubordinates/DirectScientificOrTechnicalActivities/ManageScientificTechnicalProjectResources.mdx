...
```mermaid
erDiagram
    ResourceAllocation ||--o{ Equipment : "assigns"
    ResourceAllocation ||--o{ Material : "provisions"
    ResourceAllocation ||--|| Facility : "reserves"
    Budget ||--o{ ResourceAllocation : "funds"
    Equipment ||--|| MaintenanceSchedule : "requires"
    Facility ||--|| MaintenanceSchedule : "schedules"
```
...
stateDiagram-v2
    [*] --> Planned
    Planned --> EquipmentAssigned : equipmentAllocated
    EquipmentAssigned --> MaterialsProcured : materialsProcured
    MaterialsProcured --> UtilizationTracked : utilizationTracked
    UtilizationTracked --> Optimized : allocationOptimized
    Optimized --> UtilizationTracked : utilizationTracked
    UtilizationTracked --> MaintenancePlanned : maintenanceScheduled
    MaintenancePlanned --> SpendingReported : spendingReported
    SpendingReported --> Planned : resourcesPlanned
    SpendingReported --> [*]
```
