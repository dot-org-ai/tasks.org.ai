...
```mermaid
erDiagram
    EquipmentSupplier ||--o{ FacilityProvider : includes
    FacilityProvider ||--o{ MaintenanceProvider : includes
    MaintenanceProvider ||--o{ FundingAuthority : includes
    SafetyInspector }o--|| WasteManagementService : "relates to"
```
...
stateDiagram-v2
    [*] --> Initiated
    Initiated --> EquipmentSupplier: EquipmentSupplier
    EquipmentSupplier --> FacilityProvider: FacilityProvider
    FacilityProvider --> MaintenanceProvider: MaintenanceProvider
    MaintenanceProvider --> ValidationCheck: needs review
    ValidationCheck --> FacilityProvider: corrections needed
    ValidationCheck --> FundingAuthority: approved
    ValidationCheck --> FundingAuthority: FundingAuthority
    FundingAuthority --> SafetyInspector: SafetyInspector
    SafetyInspector --> WasteManagementService: WasteManagementService
