...
```mermaid
erDiagram
    DeploymentPlan ||--|| ChangeRequest : "submits"
    DeploymentPlan ||--o{ InstallationPackage : "deploys"
    InstallationPackage ||--|| EnvironmentProfile : "targets"
    DeploymentPlan ||--|| RollbackPlan : "prepares"
    InstallationPackage ||--o{ TestResult : "validates"
```
...
stateDiagram-v2
    [*] --> Planned
    Planned --> ChangeApproved : changeRequestApproved
    ChangeApproved --> EnvironmentReady : environmentPrepared
    EnvironmentReady --> Installed : installationExecuted
    Installed --> Validated : installationValidated
    Installed --> RolledBack : installationRolledBack
    RolledBack --> Planned : deploymentPlanCreated
    Validated --> Closed : deploymentClosed
    Closed --> [*]
```
