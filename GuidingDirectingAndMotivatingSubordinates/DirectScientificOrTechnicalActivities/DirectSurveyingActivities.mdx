...
```mermaid
erDiagram
    SurveyProject ||--o{ FieldObservation : "collects"
    SurveyProject ||--o{ ControlPoint : "establishes"
    FieldObservation ||--|| BoundaryPlat : "produces"
    FieldObservation ||--|| TopographicMap : "generates"
    InstrumentCalibration ||--o{ FieldObservation : "validates"
    ControlPoint ||--|| BoundaryPlat : "references"
```
...
stateDiagram-v2
    [*] --> Planned
    Planned --> CrewAssigned : fieldCrewAssigned
    CrewAssigned --> Calibrated : fieldDataCollected
    Calibrated --> DataCollected : fieldDataCollected
    DataCollected --> DataReviewed : fieldDataReviewed
    DataReviewed --> CrewAssigned : fieldCrewAssigned
    DataReviewed --> Computed : coordinatesComputed
    Computed --> PlatDrafted : platPrepared
    PlatDrafted --> Certified : surveyCertified
    Certified --> [*]
```
