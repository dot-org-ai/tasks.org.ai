...
```mermaid
erDiagram
    ExecutiveLeadership ||--o{ Customer : includes
    Customer ||--o{ PatentCounsel : includes
    PatentCounsel ||--o{ TechnologyPartner : includes
    TechnologyPartner }o--|| TestingLaboratory : "relates to"
```
...
stateDiagram-v2
    [*] --> Initiated
    Initiated --> ExecutiveLeadership: ExecutiveLeadership
    ExecutiveLeadership --> Customer: Customer
    Customer --> PatentCounsel: PatentCounsel
    PatentCounsel --> ValidationCheck: needs review
    ValidationCheck --> Customer: corrections needed
    ValidationCheck --> TechnologyPartner: approved
    ValidationCheck --> TechnologyPartner: TechnologyPartner
    TechnologyPartner --> TestingLaboratory: TestingLaboratory
    TestingLaboratory --> [*]
```
