...
```mermaid
erDiagram
    ConservationProgram ||--o{ ManagementPlan : "defines"
    ConservationProgram ||--o{ ProtectedArea : "manages"
    EnvironmentalAssessment ||--|| ProtectedArea : "evaluates"
    ProtectedArea ||--o{ Species : "protects"
    Permit ||--|| ProtectedArea : "authorizes"
    ManagementPlan ||--o{ EnvironmentalAssessment : "informs"
```
...
stateDiagram-v2
    [*] --> Established
    Established --> Assessed : assessmentCompleted
    Assessed --> PlanApproved : managementPlanApproved
    PlanApproved --> FieldDeployed : fieldTeamsDeployed
    FieldDeployed --> Monitoring : resourceAlertTriggered
    Monitoring --> ViolationFound : complianceViolationFound
    ViolationFound --> FieldDeployed : fieldTeamsDeployed
    Monitoring --> Reported : outcomesReported
    Reported --> Established : programEstablished
    Reported --> [*]
```
