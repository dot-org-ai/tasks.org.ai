...
```mermaid
erDiagram
    ShiftAssignment ||--o{ RotationPattern : follows
    ShiftAssignment ||--o{ BreakSchedule : includes
    ShiftAssignment ||--o{ AccommodationRequest : modifiedBy
    ScheduleTemplate ||--o{ ShiftAssignment : generates
    ShiftAssignment ||--o{ OvertimeProjection : calculatedIn
```
...
stateDiagram-v2
    [*] --> RotationConfigured
    RotationConfigured --> ShiftAssignmentsDrafted : shiftAssignmentsDrafted
    ShiftAssignmentsDrafted --> BreaksScheduled : breaksScheduled
    BreaksScheduled --> AccommodationProcessed : accommodationProcessed
    AccommodationProcessed --> OvertimeProjected : overtimeProjected
    OvertimeProjected --> IndividualSchedulePublished : individualSchedulePublished
    OvertimeProjected --> ShiftAssignmentsDrafted : overtimeProjected
    IndividualSchedulePublished --> [*]
```
