...
```mermaid
erDiagram
    DutyRoster ||--o{ DutyAssignment : contains
    DutyAssignment ||--o{ SchedulePlacement : placedIn
    DutyAssignment ||--o{ AssignmentAcknowledgment : confirmedBy
    DutyAssignment ||--o{ ReassignmentRequest : changedBy
    DutyRoster ||--o{ WorkloadEquityReport : analyzedIn
```
...
stateDiagram-v2
    [*] --> DutyAssigned
    DutyAssigned --> SchedulePlaced : schedulePlaced
    SchedulePlaced --> AssignmentAcknowledged : assignmentAcknowledged
    AssignmentAcknowledged --> DutiesRotated : dutiesRotated
    DutiesRotated --> WorkloadImbalanceDetected : workloadImbalanceDetected
    WorkloadImbalanceDetected --> DutyAssigned : dutyAssigned
    DutiesRotated --> [*]
```
