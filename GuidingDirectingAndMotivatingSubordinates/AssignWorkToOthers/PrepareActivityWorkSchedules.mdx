...
```mermaid
erDiagram
    ActivitySchedule ||--o{ TaskDependency : defines
    ActivitySchedule ||--o{ TimeBlock : allocates
    TimeBlock ||--o{ DurationEstimate : basedOn
    ActivitySchedule ||--o{ ScheduleConstraint : limitedBy
    ActivitySchedule ||--o{ WorkCalendar : publishedIn
```
...
stateDiagram-v2
    [*] --> DurationsEstimated
    DurationsEstimated --> ActivitySequenceBuilt : activitySequenceBuilt
    ActivitySequenceBuilt --> DependenciesIdentified : dependenciesIdentified
    DependenciesIdentified --> ConstraintsResolved : constraintsResolved
    ConstraintsResolved --> WorkCalendarPublished : workCalendarPublished
    WorkCalendarPublished --> ScheduleUpdated : scheduleUpdated
    ScheduleUpdated --> ActivitySequenceBuilt : activitySequenceBuilt
