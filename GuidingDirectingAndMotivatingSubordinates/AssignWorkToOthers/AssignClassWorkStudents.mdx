...
```mermaid
erDiagram
    Assignment ||--o{ Rubric : evaluatedBy
    Assignment ||--o{ SubmissionRecord : trackedBy
    Assignment ||--o{ AccommodationPlan : modifiedBy
    CourseCalendar ||--o{ Assignment : schedules
    Assignment ||--o{ WorkloadAnalysis : assessedIn
```
...
stateDiagram-v2
    [*] --> AssignmentCreated
    AssignmentCreated --> AssignmentDifferentiated : assignmentDifferentiated
    AssignmentDifferentiated --> DeadlineSet : deadlineSet
    DeadlineSet --> AssignmentDistributed : assignmentDistributed
    AssignmentDistributed --> SubmissionReceived : submissionReceived
    SubmissionReceived --> LateSubmissionDetected : lateSubmissionDetected
    LateSubmissionDetected --> AssignmentDistributed : assignmentDistributed
    SubmissionReceived --> [*]
```
