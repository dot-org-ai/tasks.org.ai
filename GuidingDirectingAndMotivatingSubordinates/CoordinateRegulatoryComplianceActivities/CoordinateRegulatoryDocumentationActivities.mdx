...
```mermaid
erDiagram
    ComplianceCalendar ||--o{ FilingDeadline : tracks
    FilingDeadline ||--o{ RegulatoryFiling : triggers
    RegulatoryFiling ||--o{ DocumentPackage : assembles
    DocumentPackage ||--o{ ReviewComment : receives
    RegulatoryFiling ||--o{ SubmissionRecord : confirms
    RegulatoryFiling ||--o{ DeficiencyNotice : flaggedBy
```
...
```mermaid
stateDiagram-v2
    [*] --> ComplianceCalendarUpdated
    ComplianceCalendarUpdated --> FilingDeadlineApproaching : filingDeadlineApproaching
    FilingDeadlineApproaching --> DocumentPackageAssembled : documentPackageAssembled
    DocumentPackageAssembled --> ReviewCompleted : reviewCompleted
    ReviewCompleted --> FilingSubmitted : filingSubmitted
    FilingSubmitted --> SubmissionArchived : submissionArchived
    FilingSubmitted --> DeficiencyReceived : deficiencyReceived
    DeficiencyReceived --> DocumentPackageAssembled : documentPackageAssembled
    SubmissionArchived --> [*]
```
