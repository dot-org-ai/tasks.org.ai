...
```mermaid
erDiagram
    OSHAInspector ||--o{ EnvironmentalRegulator : includes
    EnvironmentalRegulator }o--|| InsuranceCarrier : "relates to"
    EnvironmentalRegulator }o--|| SafetyEquipmentVendor : "relates to"
    EnvironmentalRegulator }o--|| IndustryAssociation : "relates to"
    InsuranceCarrier ||--o{ SafetyEquipmentVendor : includes
```
...
stateDiagram-v2
    [*] --> Initiated
    Initiated --> OSHAInspector: OSHAInspector
    OSHAInspector --> EnvironmentalRegulator: EnvironmentalRegulator
    EnvironmentalRegulator --> InsuranceCarrier: InsuranceCarrier
    InsuranceCarrier --> ValidationCheck: needs review
    ValidationCheck --> EnvironmentalRegulator: corrections needed
    ValidationCheck --> SafetyEquipmentVendor: approved
    ValidationCheck --> SafetyEquipmentVendor: SafetyEquipmentVendor
    SafetyEquipmentVendor --> IndustryAssociation: IndustryAssociation
    IndustryAssociation --> [*]
```
