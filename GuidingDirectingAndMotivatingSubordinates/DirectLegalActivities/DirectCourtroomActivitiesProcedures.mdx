...
```mermaid
erDiagram
    ProsecutingAttorney ||--o{ DefenseCounsel : includes
    DefenseCounsel ||--o{ JuryPool : includes
    JuryPool }o--|| WitnessService : "relates to"
    JuryPool }o--|| AppellateCourt : "relates to"
```
...
stateDiagram-v2
    [*] --> Initiated
    Initiated --> ProsecutingAttorney: ProsecutingAttorney
    ProsecutingAttorney --> DefenseCounsel: DefenseCounsel
    DefenseCounsel --> JuryPool: JuryPool
    JuryPool --> ValidationCheck: needs review
    ValidationCheck --> DefenseCounsel: corrections needed
    ValidationCheck --> WitnessService: approved
    ValidationCheck --> WitnessService: WitnessService
    WitnessService --> AppellateCourt: AppellateCourt
    AppellateCourt --> [*]
```
