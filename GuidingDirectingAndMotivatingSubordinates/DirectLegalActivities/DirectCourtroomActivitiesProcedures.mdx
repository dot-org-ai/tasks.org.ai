...
```mermaid
erDiagram
    CourtDocket ||--o{ TrialProceeding : schedules
    TrialProceeding ||--o{ JuryPanel : heardBy
    TrialProceeding ||--o{ Exhibit : admits
    TrialProceeding ||--o{ Transcript : recordedIn
    TrialProceeding ||--o{ CourtOrder : issues
    TrialProceeding ||--o{ Verdict : decidedBy
```
...
stateDiagram-v2
    [*] --> DocketSet
    DocketSet --> JurySelected : jurySelected
    JurySelected --> WitnessTestified : witnessTestified
    WitnessTestified --> ExhibitAdmitted : exhibitAdmitted
    ExhibitAdmitted --> ProceedingsRecorded : proceedingsRecorded
    ProceedingsRecorded --> VerdictRendered : verdictRendered
    VerdictRendered --> CourtOrderIssued : courtOrderIssued
    WitnessTestified --> ProceedingsRecorded : proceedingsRecorded
    CourtOrderIssued --> [*]
```
