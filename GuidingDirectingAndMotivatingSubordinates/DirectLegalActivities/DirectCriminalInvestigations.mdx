...
```mermaid
erDiagram
    CriminalCase ||--o{ EvidenceItem : collects
    CriminalCase ||--o{ WitnessStatement : gathers
    CriminalCase ||--o{ SuspectProfile : develops
    CriminalCase ||--o{ SearchWarrant : obtains
    CriminalCase ||--o{ CaseFile : compiledIn
    CaseFile ||--o{ InvestigationTimeline : chronicles
```
...
stateDiagram-v2
    ...
    InvestigatorAssigned --> EvidenceCollected : evidenceCollected
    EvidenceCollected --> WitnessInterviewed : witnessInterviewed
    WitnessInterviewed --> SuspectIdentified : suspectIdentified
    SuspectIdentified --> WarrantObtained : warrantObtained
    WarrantObtained --> CaseReferredForProsecution : caseReferredForProsecution
    SuspectIdentified --> CaseProgressReviewed : caseProgressReviewed
    CaseProgressReviewed --> EvidenceCollected : evidenceCollected
    CaseReferredForProsecution --> [*]
```
