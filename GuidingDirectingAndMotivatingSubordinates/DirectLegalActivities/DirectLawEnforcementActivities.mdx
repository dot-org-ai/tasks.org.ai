...
```mermaid
erDiagram
    BeatAssignment ||--o{ CallForService : dispatches
    CallForService ||--o{ FieldReport : documents
    BeatAssignment ||--o{ SpecialOperation : targets
    BeatAssignment ||--o{ TrafficCitation : issues
    BeatAssignment ||--o{ CommunityProgram : supports
    CallForService ||--o{ UseOfForceReport : records
```
...
stateDiagram-v2
    [*] --> PatrolUnitsDeployed
    PatrolUnitsDeployed --> OfficerDispatched : officerDispatched
    OfficerDispatched --> SpecialOperationConducted : specialOperationConducted
    SpecialOperationConducted --> TrafficCitationIssued : trafficCitationIssued
    TrafficCitationIssued --> ActivityReportGenerated : activityReportGenerated
    ActivityReportGenerated --> UseOfForceReviewed : useOfForceReviewed
    UseOfForceReviewed --> PatrolUnitsDeployed : patrolUnitsDeployed
    OfficerDispatched --> CommunityProgramLaunched : communityProgramLaunched
    CommunityProgramLaunched --> ActivityReportGenerated : activityReportGenerated
