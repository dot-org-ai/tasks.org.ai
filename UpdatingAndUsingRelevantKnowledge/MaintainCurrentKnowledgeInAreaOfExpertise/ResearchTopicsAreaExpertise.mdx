...
```mermaid
erDiagram
    ResearchQuestion ||--|| LiteratureReview : "guides"
    LiteratureReview ||--|{ DataSource : "identifies"
    DataSource ||--|{ ResearchFinding : "produces"
    ResearchFinding ||--|{ Citation : "references"
    ResearchFinding ||--o{ ResearchReport : "documented in"
    LiteratureReview ||--|{ Citation : "collects"
```
...
stateDiagram-v2
    [*] --> QuestionDefined
    QuestionDefined --> LiteratureReviewed : literatureReviewCompleted
    LiteratureReviewed --> DataCollected : dataCollected
    DataCollected --> Analyzed : findingsAnalyzed
    Analyzed --> Synthesized : resultsSynthesized
    Synthesized --> Published : researchPublished
    Synthesized --> Archived : researchArchived
    Published --> Archived : researchArchived
    Archived --> [*]
```
