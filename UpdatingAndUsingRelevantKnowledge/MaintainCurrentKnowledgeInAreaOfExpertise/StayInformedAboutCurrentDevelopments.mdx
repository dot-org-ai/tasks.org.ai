...
```mermaid
erDiagram
    InformationSource ||--|{ DevelopmentUpdate : "publishes"
    AlertSubscription ||--|| InformationSource : "monitors"
    DevelopmentUpdate ||--o{ KnowledgeBase : "added to"
    DevelopmentUpdate ||--o{ TrendReport : "summarized in"
    ReadingList ||--|{ DevelopmentUpdate : "queues"
    TrendReport }o--|| KnowledgeBase : "stored in"
```
...
stateDiagram-v2
    [*] --> Subscribed
    Subscribed --> Reviewing : developmentReviewed
    Reviewing --> Curated : findingCurated
    Reviewing --> Subscribed : notRelevant
    Curated --> ImpactEvaluated : impactAssessed
    ImpactEvaluated --> KBUpdated : knowledgeBaseUpdated
    KBUpdated --> Shared : developmentShared
    KBUpdated --> Subscribed
    Shared --> Subscribed
```
