...
```mermaid
erDiagram
    KnowledgeUpdate ||--|{ WorkProcedure : "modifies"
    PolicyChange ||--|{ WorkProcedure : "triggers revision of"
    KnowledgeGap ||--|{ RefresherModule : "addressed by"
    RefresherModule ||--o{ CompetencyRecord : "updates"
    CompetencyRecord }o--|| KnowledgeGap : "reveals"
    PolicyChange ||--o{ CompetencyRecord : "requires update of"
```
...
stateDiagram-v2
    [*] --> Monitoring
    Monitoring --> GapDetected : knowledgeGapIdentified
    GapDetected --> ProcedureReviewed : procedureUpdateReviewed
    ProcedureReviewed --> RefresherInProgress : refresherCompleted
    RefresherInProgress --> MethodApplied : updatedMethodApplied
    MethodApplied --> RecordUpdated : competencyRecordUpdated
    RecordUpdated --> PolicyAck : policyChangeAcknowledged
    PolicyAck --> Monitoring
    RecordUpdated --> Monitoring
```
