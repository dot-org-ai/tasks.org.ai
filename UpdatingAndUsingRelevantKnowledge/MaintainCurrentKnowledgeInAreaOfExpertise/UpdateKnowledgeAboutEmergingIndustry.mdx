...
```mermaid
erDiagram
    TrendReport ||--|{ TechnologyAssessment : "triggers"
    TechnologyAssessment ||--o{ IndustryBriefing : "summarized in"
    LearningEvent ||--|{ TrendReport : "informs"
    TrendAlert ||--|| TrendReport : "highlights"
    KnowledgeBase ||--|{ TrendReport : "stores"
    IndustryBriefing }o--|| KnowledgeBase : "added to"
```
...
stateDiagram-v2
    [*] --> Scanning
    Scanning --> TrendFound : trendIdentified
    TrendFound --> Evaluated : technologyEvaluated
    Scanning --> EventAttended : eventAttended
    EventAttended --> Evaluated
    Evaluated --> ImpactReviewed : impactAssessed
    ImpactReviewed --> BriefingPublished : briefingPublished
    ImpactReviewed --> KBUpdated : knowledgeBaseUpdated
    BriefingPublished --> KBUpdated
    KBUpdated --> Scanning
    TrendFound --> DisruptionAlert : disruptionAlertRaised
    DisruptionAlert --> ImpactReviewed
```
