...
```mermaid
erDiagram
    ResearchBrief ||--|{ ReferenceSource : "guides collection of"
    ReferenceSource ||--o{ SourceAnnotation : "has"
    ResearchBrief ||--o{ MoodBoard : "produces"
    MoodBoard ||--|{ ReferenceSource : "curates"
    ResearchBrief ||--o{ FieldStudy : "requires"
    MoodBoard ||--o{ DesignConcept : "informs"
    FieldStudy }o--|| ReferenceSource : "yields"
```
...
stateDiagram-v2
    [*] --> BriefDefined
    BriefDefined --> Collecting : sourcesCollected
    Collecting --> Annotating : sourcesAnnotated
    Annotating --> MoodBoardReady : moodBoardAssembled
    MoodBoardReady --> Synthesized : findingsSynthesized
    Synthesized --> Collecting : needsMoreSources
    Synthesized --> Applied : researchAppliedToDesign
    Applied --> [*]
```
