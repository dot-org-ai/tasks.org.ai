...
```mermaid
erDiagram
    Subscription ||--|{ Publication : "delivers"
    Publication ||--|{ Annotation : "has"
    Publication ||--o{ LiteratureSummary : "produces"
    ReadingList ||--|{ Publication : "prioritizes"
    LiteratureSummary ||--o{ KnowledgeLibrary : "stored in"
    Annotation }o--|| LiteratureSummary : "informs"
```
...
stateDiagram-v2
    [*] --> Identified
    Identified --> Queued : readingListUpdated
    Queued --> Reviewing : articleReviewed
    Reviewing --> Annotated : publicationAnnotated
    Annotated --> Summarized : findingsSummarized
    Summarized --> Shared : reviewShared
    Summarized --> Applied : insightsApplied
    Shared --> Applied
    Applied --> [*]
```
