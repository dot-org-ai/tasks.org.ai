...
```mermaid
erDiagram
    ProductSpecification ||--|{ TrainingModule : "covered by"
    TrainingModule ||--o{ CertificationExam : "prepares for"
    ProductSpecification ||--o{ ProductComparison : "featured in"
    ProductCatalog ||--|{ ProductSpecification : "contains"
    KnowledgeBaseArticle }o--|| ProductSpecification : "explains"
    CertificationExam }o--|| KnowledgeBaseArticle : "references"
```
...
stateDiagram-v2
    [*] --> SpecsReviewed
    SpecsReviewed --> TrainingCompleted : trainingModuleCompleted
    TrainingCompleted --> HandsOn : handOnPracticeCompleted
    HandsOn --> Compared : productsCompared
    Compared --> Documented : knowledgeNoteCreated
    Documented --> Certified : certificationPassed
    Documented --> Updated : productKnowledgeUpdated
    Certified --> Updated
    Updated --> [*]
```
