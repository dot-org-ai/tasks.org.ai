...
```mermaid
erDiagram
    SkillGap ||--|{ LearningPlan : "drives"
    LearningPlan ||--|{ Enrollment : "schedules"
    Enrollment ||--|| TrainingProgram : "registers for"
    TrainingProgram ||--|{ CompletionRecord : "produces"
    CompletionRecord ||--o{ Certificate : "awards"
    Certificate }o--|| SkillGap : "closes"
```
...
stateDiagram-v2
    [*] --> GapIdentified
    GapIdentified --> PlanUpdated : learningPlanUpdated
    PlanUpdated --> Enrolled : trainingEnrolled
    Enrolled --> InProgress : moduleCompleted
    InProgress --> Assessed : assessmentSubmitted
    Assessed --> InProgress : failed
    Assessed --> Certified : certificateEarned
    Certified --> Complete : trainingCompleted
    Complete --> [*]
```
